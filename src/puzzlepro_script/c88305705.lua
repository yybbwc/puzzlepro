...
function c88305705.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c88305705.atkfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
end
...
function c88305705.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(bc)
  return aux.dsercon(e, tp, eg, ep, ev, re, r, rp) and c == Duel.GetAttacker() and c:IsStatus(STATUS_OPPO_BATTLE) and bc and bc:IsOnField() and bc:IsRelateToBattle()
end
function c88305705.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetLabelObject():IsAbleToRemove()
  end
