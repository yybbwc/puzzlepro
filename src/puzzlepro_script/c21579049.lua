...
function s.filter(c, e, tp, check)
  ...
  return c:IsRace(RACE_FISH) and c:IsFaceup() and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, check2, c:GetCode())
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.filter(chkc, e, tp, check)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp, check)
  end
...
function s.tgfilter(c, e, tp)
  return c:IsCanBeEffectTarget(e) and c:IsRace(RACE_FISH) and c:IsAbleToDeck() or c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function s.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:CheckSubGroup(s.fselect, 2, 2, e, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_OPERATECARD)
