...
function s.mvfilter(c, tp)
  ...
  if not c:IsControler(c:GetOwner()) then
    if not c:IsAbleToChangeControler() then
      return false
    end
...
function s.mvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_MZONE) and s.mvfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.mvfilter, tp, LOCATION_GRAVE + LOCATION_MZONE, LOCATION_GRAVE + LOCATION_MZONE, 1, nil, tp)
  end
...
function s.mvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsType(TYPE_MONSTER) and aux.NecroValleyFilter()(tc) and not tc:IsImmuneToEffect(e) and Duel.MoveToField(tc, tp, tc:GetOwner(), LOCATION_SZONE, POS_FACEUP, true) then
...
function s.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.desfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
