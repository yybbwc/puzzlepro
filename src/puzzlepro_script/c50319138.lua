...
function c50319138.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c50319138.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c50319138.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(c50319138.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
    return
  end
