...
function c45354718.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c45354718.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c45354718.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if res and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c45354718.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, tc:GetCode()) and Duel.SelectYesNo(tp, aux.Stringid(45354718, 1)) then
    Duel.BreakEffect()
