...
function c41209827.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41209827.atkfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
end
...
function c41209827.copycost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(41209827) == 0
  end
...
function c41209827.copytg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c41209827.copyfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41209827.copyfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
...
function c41209827.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41209827.desfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
