...
function c83195035.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c83195035.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c83195035.filter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil, e, tp)
  end
