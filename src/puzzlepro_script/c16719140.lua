...
function c16719140.costfilter(c, e, tp, mg, rlv, mc)
  if not (c:IsLevelAbove(0) and c:IsSetCard(0xed) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_DEFENSE)) then
    return false
  end
...
function c16719140.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    e:SetLabel(0)
    if not c:IsLevelAbove(1) or not c:IsReleasableByEffect() or mg:GetCount() == 0 then
      return false
    end
