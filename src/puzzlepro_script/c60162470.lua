...
function c60162470.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c60162470.filter(chkc, tg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60162470.filter, tp, LOCATION_MZONE, 0, 1, nil, tg)
  end
...
function c60162470.atktg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c60162470.cfilter, 1, nil, tp)
  end
