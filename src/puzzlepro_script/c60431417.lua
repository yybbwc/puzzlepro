...
function c60431417.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c60431417.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60431417.filter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsPlayerCanDiscardDeck(tp, 4)
  end
...
function c60431417.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c60431417.thfilter(chkc)
  end
  local rg = Duel.GetDecktopGroup(tp, 4)
  if chk == 0 then
    return Duel.IsExistingTarget(c60431417.thfilter, tp, LOCATION_GRAVE, 0, 1, nil) and rg:FilterCount(Card.IsAbleToRemove, nil) == 4
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
