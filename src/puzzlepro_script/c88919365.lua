...
function c88919365.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c88919365.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88919365.thfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil)
  end
...
function c88919365.mvcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsLocation(LOCATION_MZONE) and (c:GetPreviousSequence() ~= c:GetSequence() or c:GetPreviousControler() ~= tp) and ph ~= PHASE_DAMAGE and ph ~= PHASE_DAMAGE_CAL
end
...
function c88919365.ffilter2(c, e, tp, m, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x17d) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, nil, chkf)
end
