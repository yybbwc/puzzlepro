...
function c76403456.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return val > 0 and Duel.IsPlayerCanDiscardDeck(tp, val) and not Duel.IsPlayerAffectedByEffect(tp, EFFECT_AVOID_BATTLE_DAMAGE)
  end
...
function c76403456.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if og:FilterCount(Card.IsLocation, nil, LOCATION_GRAVE) < val then
    return
  end
