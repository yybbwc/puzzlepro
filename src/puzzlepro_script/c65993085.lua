...
function c65993085.costfilter(c, e, tp)
  ...
  return lv > 1 and c:IsType(TYPE_MONSTER) and c:IsDiscardable() and Duel.IsExistingTarget(c65993085.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, lv)
end
...
function c65993085.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c65993085.spfilter(chkc, e, tp, e:GetLabel())
  end
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c65993085.costfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
