...
function c84080938.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c84080938.filter(chkc, tid)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c84080938.filter, tp, LOCATION_GRAVE, 0, 1, nil, tid)
  end
