...
function c31002402.rfilter2(c, e, tp, m1)
  ...
  if bit.band(c:GetType(), 0x81) ~= 0x81 or not c:IsSetCard(0x145) or not c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_RITUAL, tp, false, true) then
    return false
  end
...
function c31002402.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return Duel.IsExistingMatchingCard(c31002402.rfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp, mg2) or Duel.IsExistingMatchingCard(aux.RitualUltimateFilter, tp, LOCATION_HAND, 0, 1, nil, c31002402.filter, e, tp, mg1, nil, Card.GetLevel, "Equal")
  end
