...
function c38761908.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c38761908.filter2(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c38761908.filter1, tp, LOCATION_HAND, 0, 1, e:GetHandler()) and Duel.IsExistingTarget(c38761908.filter2, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
...
function c38761908.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.DiscardHand(tp, c38761908.filter1, 1, 1, REASON_EFFECT + REASON_DISCARD, nil) ~= 0 and tc:IsRelateToEffect(e) then
...
function c38761908.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 38761908, 0xd4, TYPES_NORMAL_TRAP_MONSTER, 1200, 0, 2, RACE_AQUA, ATTRIBUTE_WATER)
  end
