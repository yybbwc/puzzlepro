...
function c85119159.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c85119159.filter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85119159.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c85119159.filter2, tp, LOCATION_MZONE, 0, 2, nil)
  end
