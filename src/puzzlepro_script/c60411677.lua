...
function s.sprcon(e, c)
  ...
  return Duel.GetMatchingGroup(s.sprfilter, tp, LOCATION_GRAVE, 0, nil):GetClassCount(Card.GetCode) >= 3 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.sprfilter, tp, LOCATION_GRAVE, 0, 2, nil)
end
...
function s.cfilter(c, tp)
  return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsReason(REASON_EFFECT) and c:GetPreviousTypeOnField() & (TYPE_SPELL + TYPE_TRAP) ~= 0
end
