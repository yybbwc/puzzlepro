...
function c91951471.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c91951471.filter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91951471.filter1, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c91951471.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToChain() and tc:IsLocation(LOCATION_MZONE) and Duel.ChangePosition(tc, POS_FACEUP_DEFENSE, POS_FACEDOWN_DEFENSE, POS_FACEUP_ATTACK, POS_FACEUP_ATTACK) > 0 then
