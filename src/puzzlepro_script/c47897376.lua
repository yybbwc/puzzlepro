...
function c47897376.dttg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c47897376.dtfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c47897376.dttg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c47897376.texfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47897376.texfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, c) and c:IsAbleToDeck()
  end
