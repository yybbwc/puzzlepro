...
function c90432163.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c90432163.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c90432163.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c90432163.operation(e, tp, eg, ep, ev, re, r, rp)
  if not e:GetHandler():IsRelateToEffect(e) or Duel.ChangePosition(e:GetHandler(), POS_FACEUP_DEFENSE) == 0 then
    return
  end
...
function c90432163.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90432163.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
