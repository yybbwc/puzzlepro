...
function c4591250.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsSummonType(SUMMON_TYPE_FUSION) and (c:IsReason(REASON_BATTLE) or (c:GetReasonPlayer() == 1 - tp and c:IsReason(REASON_EFFECT))) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c4591250.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c4591250.filter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
