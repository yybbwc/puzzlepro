...
function c49430782.checkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  while tc do
    if tc:IsPreviousLocation(LOCATION_ONFIELD) and tc:IsSetCard(0xf9) then
      ...
      if (typ == TYPE_MONSTER and Duel.GetFlagEffect(0, 49430782) == 0) or (typ == TYPE_SPELL and Duel.GetFlagEffect(0, 49430783) == 0) or (typ == TYPE_TRAP and Duel.GetFlagEffect(0, 49430784) == 0) then
...
function c49430782.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, c49430782.counter)
  end
...
function c49430782.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c49430782.sumfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c49430782.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsType, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil, TYPE_SPELL + TYPE_TRAP)
  end
