...
function c96795312.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96795312.costfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c96795312.repfilter(c, tp)
  return not c:IsReason(REASON_REPLACE) and c:IsType(TYPE_LINK) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsReason(REASON_BATTLE + REASON_EFFECT)
end
...
function c96795312.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c96795312.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c96795312.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c96795312.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c96795312.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c96795312.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
