...
function c15894048.cfilter(c)
  if c:IsFacedown() or not c:IsCode(15894048) or not c:IsAttackable() then
    return false
  end
...
function c15894048.cacon(e)
  return Duel.GetCurrentPhase() > PHASE_MAIN1 and Duel.GetCurrentPhase() < PHASE_MAIN2 and Duel.IsExistingMatchingCard(c15894048.cfilter, e:GetHandlerPlayer(), LOCATION_MZONE, 0, 1, nil)
end
