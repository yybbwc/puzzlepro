...
function c56495147.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56495147.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56495147.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 20001444, 0x16b, TYPES_TOKEN_MONSTER + TYPE_TUNER, 0, 0, 4, RACE_WYRM, ATTRIBUTE_WATER)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOKEN, nil, 1, 0, 0)
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, 0, 0)
end
function c56495147.spop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 20001444, 0x16b, TYPES_TOKEN_MONSTER + TYPE_TUNER, 0, 0, 4, RACE_WYRM, ATTRIBUTE_WATER) then
...
function c56495147.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56495147.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
