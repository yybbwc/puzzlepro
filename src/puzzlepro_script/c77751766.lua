...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsCanAddCounter(tp, 0x6d, 5, c)
  end
  c:AddCounter(0x6d, 5)
end
...
function s.ccon1(e, tp, eg, ep, ev, re, r, rp)
  return re and re:IsActiveType(TYPE_TRAP + TYPE_SPELL) and eg:IsExists(s.cfilter1, 1, nil, LOCATION_EXTRA)
end
function s.ccon2(e, tp, eg, ep, ev, re, r, rp)
  return re and re:IsActiveType(TYPE_TRAP + TYPE_SPELL) and eg:IsExists(s.cfilter2, 1, nil, LOCATION_DECK)
end
