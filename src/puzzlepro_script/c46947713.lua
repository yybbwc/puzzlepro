...
function c46947713.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(46947713, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c46947713.filter(c, e, tp, zone)
  return c:IsRace(RACE_CYBERSE) and c:IsLinkBelow(3) and c:IsType(TYPE_LINK) and not c:IsCode(46947713) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone)
end
function c46947713.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local zone = bit.band(e:GetHandler():GetLinkedZone(tp), 0x1f)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c46947713.filter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c46947713.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, zone)
  end
