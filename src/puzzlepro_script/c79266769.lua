...
function c79266769.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c79266769.filter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c79266769.filter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp, zone)
  end
...
function c79266769.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) and zone ~= 0 and Duel.SpecialSummonStep(tc, 0, tp, tp, false, false, POS_FACEUP_DEFENSE, zone) then
    local e1 = Effect.CreateEffect(c)
