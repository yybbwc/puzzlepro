...
function c58604027.spfilter(c, tp)
  return c:IsSetCard(0x40) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c58604027.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingMatchingCard(c58604027.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
