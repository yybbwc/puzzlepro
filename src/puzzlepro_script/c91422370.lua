...
function c91422370.costfilter(c, ft, tp)
  return c:IsSetCard(0x58) and c:IsAttackAbove(1500) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup())
end
function c91422370.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c91422370.costfilter, 1, nil, ft, tp)
  end
...
function c91422370.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91422370.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
