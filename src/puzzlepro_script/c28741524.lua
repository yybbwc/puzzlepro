...
function c28741524.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c28741524.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c28741524.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c28741524.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if ft < g:GetCount() or (g:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) then
    return
  end
