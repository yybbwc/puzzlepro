...
function c18446701.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c18446701.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c18446701.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c18446701.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsRace(RACE_SPELLCASTER)
end
