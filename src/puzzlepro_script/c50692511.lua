...
function c50692511.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(50692511, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c50692511.filter1(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0xba) and Duel.IsExistingMatchingCard(c50692511.filter2, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp, c:GetCode())
end
...
function c50692511.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and chkc:IsCode(e:GetLabel())
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c50692511.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
