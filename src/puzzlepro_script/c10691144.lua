...
function c10691144.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 10691144) ~= 0 then
    return
  end
...
function c10691144.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  while tc do
    ...
    if tc:IsReason(REASON_EFFECT) and not tc:IsReason(REASON_REDIRECT) and bit.band(ploc, 0x1e) ~= 0 and tc:IsPreviousControler(tp) and te:GetOwnerPlayer() == 1 - tp and te:IsActiveType(TYPE_MONSTER) and te:IsActivated() then
  ...
  if bit.band(flag, LOCATION_HAND) ~= 0 then
    ...
    if rg:GetCount() > 0 then
      ...
      if rg:GetCount() > 1 then
        ct = Duel.SelectOption(tp, aux.Stringid(10691144, 3), aux.Stringid(10691144, 4)) + 1
      end
