...
function c80621422.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c80621422.eqfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c80621422.eqfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c80621422.spfilter(c, e, tp)
  return c:IsSetCard(0x13f) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0 and Duel.IsExistingMatchingCard(c80621422.mfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, LOCATION_MZONE + LOCATION_GRAVE, 1, nil, c:GetAttribute())
end
...
function c80621422.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80621422.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c80621422.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not ec:IsRelateToEffect(e) then
    return
  end
  Duel.BreakEffect()
  if not Duel.Equip(tp, ec, tc, false) then
    return
  end
  --Atk up
