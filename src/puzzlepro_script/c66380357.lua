...
function c66380357.filter(c, e, tp)
  if not (c:IsType(TYPE_LINK) and c:IsSetCard(0x14b)) then
    return false
  end
...
function c66380357.gfilter(c, e, tp, zone)
  return c:IsSetCard(0x14b) and c:IsType(TYPE_LINK) and c:IsLink(1) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone)
end
function c66380357.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c66380357.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c66380357.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
