...
function c62481203.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(62481203, tp, ACTIVITY_SUMMON) == 0 and Duel.GetCustomActivityCount(62481203, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c62481203.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 51208047, 0xfb, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT)
  end
...
function c62481203.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 51208047, 0xfb, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT) then
    for i = 1, 2 do
...
function c62481203.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSummonLocation(LOCATION_EXTRA) and c:IsSetCard(0xfb) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c62481203.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c62481203.repfilter, 1, nil, tp) and e:GetHandler():IsAbleToRemove()
  end
