...
function c78751195.tetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c78751195.tefilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c78751195.tefilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(c78751195.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
