...
function c4997565.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingMatchingCard(c4997565.spfilter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c4997565.discon(e, tp, eg, ep, ev, re, r, rp)
  return re:IsActiveType(TYPE_MONSTER) and Duel.GetChainInfo(ev, CHAININFO_TRIGGERING_LOCATION) == LOCATION_MZONE
end
...
function c4997565.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4997565.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and not tc:IsDisabled() and tc:IsRelateToEffect(re) and tc:IsCanBeEffectTarget(e)
  end
