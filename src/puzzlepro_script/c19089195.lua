...
function c19089195.filter(c, tp)
  return c:IsCode(22702055) and (c:IsType(TYPE_FIELD) or Duel.GetLocationCount(tp, LOCATION_SZONE) > 0) and c:GetActivateEffect() and c:GetActivateEffect():IsActivatable(tp, true, true)
end
...
function c19089195.remcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19089195.costfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
