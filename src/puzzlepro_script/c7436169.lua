...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(s.negfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) and tc:IsCanBeDisabledByEffect(e) then
    ...
    if lc:IsRelateToEffect(e) and lc:IsControler(tp) and lc:IsFaceup() then
      local e1 = Effect.CreateEffect(c)
      e1:SetType(EFFECT_TYPE_SINGLE)
...
function s.thfilter(c)
  return not c:IsCode(id) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsAbleToHand() and aux.IsCodeListed(c, 56099748)
end
