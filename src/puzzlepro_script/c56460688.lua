...
function c56460688.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToRemove, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsAbleToRemove, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c56460688.retcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():GetFlagEffect(56460688) ~= 0 and bit.band(r, REASON_DESTROY) ~= 0 and e:GetLabelObject():GetLabelObject() ~= nil
end
