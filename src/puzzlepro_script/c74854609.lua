...
function c74854609.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and a:IsOnField() and a:IsCanBeEffectTarget(e)
  end
...
function c74854609.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsAttackable() and not tc:IsStatus(STATUS_ATTACK_CANCELED) and ec:IsLocation(LOCATION_MZONE) and ec:IsFaceup() then
