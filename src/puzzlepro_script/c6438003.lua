...
function c6438003.tgfilter(c)
  return c:IsFaceup() and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsType(TYPE_CONTINUOUS) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(Card.IsFaceup, 0, LOCATION_MZONE, LOCATION_MZONE, 2, c)
end
function c6438003.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c6438003.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c6438003.tgfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c6438003.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c6438003.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c6438003.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
