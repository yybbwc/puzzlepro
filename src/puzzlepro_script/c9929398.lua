...
function c9929398.tkntg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 9929399, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_WINDBEAST, ATTRIBUTE_DARK)
  end
...
function c9929398.tknop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 9929399, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_WINDBEAST, ATTRIBUTE_DARK) then
    for i = 1, 2 do
...
function c9929398.spfilter(c, e, tp, ct)
  ...
  return c:IsType(TYPE_SYNCHRO) and c:IsSetCard(0x33) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and rg:CheckWithSumEqual(Card.GetLevel, rlv, ct, 63)
end
function c9929398.chkcfilter(c, e, tp, lv)
  return c:IsControler(tp) and c:IsLocation(LOCATION_GRAVE) and c:IsType(TYPE_SYNCHRO) and c:IsSetCard(0x33) and c:IsLevel(lv) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c9929398.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local ct = -Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chkc then
    return c9929398.chkcfilter(chkc, e, tp, e:GetLabel())
  end
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c9929398.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, ct)
  end
