...
function c60759087.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c60759087.cfilter, tp, LOCATION_MZONE, 0, 1, nil, 0x152) and Duel.IsExistingMatchingCard(c60759087.cfilter, tp, LOCATION_MZONE, 0, 1, nil, 0x153)
end
function c60759087.tgfilter1a(c)
  local tp = c:GetControler()
  return c:IsSetCard(0x152, 0x153) and c:IsFaceup() and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
...
function c60759087.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return e:GetLabel() == 1 and c60759087.tgfilter1(chkc) and chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(1 - tp)
  end
  local b1 = Duel.IsExistingTarget(c60759087.tgfilter1a, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c60759087.tgfilter1b, tp, 0, LOCATION_MZONE, 1, nil)
  local b2 = Duel.IsExistingTarget(c60759087.tgfilter2, tp, 0, LOCATION_SZONE, 1, nil)
  if chk == 0 then
    return b1 or b2
  end
  local op = aux.SelectFromOptions(tp, { b1, aux.Stringid(60759087, 0), 0 }, { b2, aux.Stringid(60759087, 1), 1 })
  e:SetLabel(op)
  if op == 0 then
