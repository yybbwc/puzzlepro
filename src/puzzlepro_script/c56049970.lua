...
function c56049970.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and c56049970.desfilter(chkc)
  end
  if chk == 0 then
    ...
    e:SetLabel(loc)
    return Duel.IsExistingTarget(c56049970.desfilter, tp, loc, 0, 1, nil) and Duel.IsExistingMatchingCard(c56049970.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c56049970.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c56049970.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
