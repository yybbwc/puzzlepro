...
function c33609093.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.nzatk, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c33609093.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c33609093.tffilter(c, tp)
  return c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsType(TYPE_CONTINUOUS) and c:IsSetCard(0x137) and not c:IsCode(33609093) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
function c33609093.tftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c33609093.tffilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, tp)
  end
end
