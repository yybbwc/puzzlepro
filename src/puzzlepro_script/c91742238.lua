...
function c91742238.rmfilter(c, e, tp)
  ...
  return c:IsFaceup() and c:IsRace(RACE_ZOMBIE) and c:IsAbleToRemove() and Duel.GetMZoneCount(p, c, p, LOCATION_REASON_TOFIELD) and Duel.IsExistingMatchingCard(c91742238.spfilter, p, LOCATION_GRAVE, 0, 1, nil, e, tp, p)
end
...
function c91742238.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91742238.rmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e, tp)
  end
...
function c91742238.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsSSetable() and Duel.IsExistingMatchingCard(c91742238.setfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
