...
function c19159413.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c19159413.filter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19159413.filter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, e:GetHandler())
  end
