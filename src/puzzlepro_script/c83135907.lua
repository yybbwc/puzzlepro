...
function c83135907.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(83135907) ~= 0
  end
...
function c83135907.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c83135907.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83135907.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
