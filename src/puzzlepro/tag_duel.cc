...
namespace ygo {
  ...
  void TagDuel::JoinGame(DuelPlayer *dp, unsigned char *pdata, bool is_creater) {
    ...
    dp->game = this;
    if (!players[0] && !players[1] && !players[2] && !players[3] && observers.empty()) {
      host_player = dp;
    }
    STOC_JoinGame scjg;
    scjg.info = host_info;
  ...
  void TagDuel::TPResult(DuelPlayer *dp, unsigned char tp) {
    ...
    last_replay.WriteData(players[2]->name, 40);
    last_replay.WriteData(players[3]->name, 40);
  ...
  void TagDuel::TagTimer(evutil_socket_t fd, short events, void *arg) {
    auto *sd = static_cast<TagDuel *>(arg);
    sd->time_elapsed++;
