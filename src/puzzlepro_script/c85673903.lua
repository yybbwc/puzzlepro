...
function c85673903.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c85673903.tgfilter(chkc, e, tp) and chkc ~= eg:GetFirst()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85673903.tgfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, eg:GetFirst(), e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
