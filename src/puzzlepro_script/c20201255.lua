...
function c20201255.filter(c, e, tp)
  return c:IsSetCard(0x1019) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and not Duel.IsExistingMatchingCard(c20201255.filter1, tp, LOCATION_MZONE, 0, 1, c, c:GetRace())
end
...
function c20201255.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c20201255.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
