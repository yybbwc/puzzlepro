...
function c66727115.rfilter(c, e, tp)
  return c:IsSetCard(0x88) and c:IsRace(RACE_BEASTWARRIOR) and Duel.IsExistingTarget(c66727115.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, c:GetCode(), e, tp)
end
...
function c66727115.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c66727115.spfilter(chkc, e:GetLabel(), e, tp)
  end
  if chk == 0 then
