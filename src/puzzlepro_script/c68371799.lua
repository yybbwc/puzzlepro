...
function c68371799.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c68371799.filter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function c68371799.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsOnField() and c68371799.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c68371799.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c68371799.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c68371799.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
