...
function c1735088.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c1735088.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c1735088.cfilter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x58) and c:IsAbleToRemoveAsCost(POS_FACEDOWN) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c1735088.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c1735088.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c1735088.matfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c1735088.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanOverlay()
  end
