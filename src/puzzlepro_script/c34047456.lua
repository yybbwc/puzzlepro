...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(0x14) and s.filter(chkc)
  end
  local ct = math.abs(Duel.GetFieldGroupCount(tp, LOCATION_REMOVED, 0) - Duel.GetFieldGroupCount(tp, 0, LOCATION_REMOVED))
  if chk == 0 then
    return ct > 0 and Duel.IsExistingTarget(s.filter, tp, 0x14, 0x14, ct, nil)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Remove(g, POS_FACEUP, REASON_EFFECT) < 1 or Duel.GetLocationCount(1 - tp, LOCATION_MZONE) < 1 then
    return
  end
