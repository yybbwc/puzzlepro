...
function c98847704.filter1(c)
  return c:IsFaceup() and c:IsType(TYPE_EQUIP) and (not c:IsType(TYPE_TRAP) or c:IsPreviousLocation(LOCATION_MZONE))
end
...
function c98847704.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c98847704.filter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98847704.filter1, tp, LOCATION_SZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c98847704.filter2, tp, 0, LOCATION_SZONE, 1, nil)
  end
