...
function c16960351.rfilter(c, e, tp, ft)
  ...
  return lv > 0 and c:IsRace(RACE_DRAGON) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c16960351.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, lv)
end
...
function c16960351.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c16960351.rfilter, 1, nil, e, tp, ft)
  end
