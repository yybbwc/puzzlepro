...
function c24050692.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c24050692.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c24050692.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) > 0 and Duel.GetOperatedGroup():GetFirst():IsLocation(LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetMatchingGroup(c24050692.cfilter, tp, LOCATION_GRAVE, 0, nil):GetClassCount(Card.GetCode) >= 5 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c24050692.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(24050692, 0)) then
    Duel.BreakEffect()
