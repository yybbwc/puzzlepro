...
function c29432356.scfilter(c, pc)
  return c:IsType(TYPE_PENDULUM) and c:IsSetCard(0xc4) and c:GetLeftScale() ~= pc:GetLeftScale()
end
function c29432356.sctg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29432356.scfilter, tp, LOCATION_DECK, 0, 1, nil, e:GetHandler())
  end
...
function c29432356.hspcon(e, c)
  ...
  return (g:GetCount() > 0 or rg:GetCount() > 0) and g:FilterCount(Card.IsReleasable, nil, REASON_SPSUMMON) == g:GetCount() and g:FilterCount(Card.IsSetCard, nil, 0xc4) >= 3 and Duel.GetLocationCountFromEx(tp, tp, g, c) > 0
end
...
function c29432356.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c29432356.spcfilter, 1, nil, ft, tp)
  end
...
function c29432356.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29432356.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
