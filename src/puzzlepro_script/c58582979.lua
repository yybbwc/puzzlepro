...
function c58582979.hspcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c58582979.hspcfilter, tp, LOCATION_HAND, 0, 1, c)
end
...
function c58582979.spcfilter1(c, tp, ec)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c58582979.spcfilter2, tp, LOCATION_GRAVE, 0, 2, Group.FromCards(ec, c), c:GetCode())
end
...
function c58582979.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58582979.spcfilter1, tp, LOCATION_GRAVE, 0, 1, c, tp, c)
  end
