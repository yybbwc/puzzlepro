...
function c79559912.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg, REASON_EFFECT) ~= 0 and c:IsRelateToEffect(e) and c:IsFaceup() and Duel.IsExistingMatchingCard(c79559912.atkfilter, tp, LOCATION_MZONE, 0, 1, c) and Duel.SelectYesNo(tp, aux.Stringid(79559912, 1)) then
    Duel.BreakEffect()
...
function c79559912.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c79559912.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
