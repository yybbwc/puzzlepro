...
function c8972398.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c8972398.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0 and Duel.IsExistingTarget(c8972398.tgfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
