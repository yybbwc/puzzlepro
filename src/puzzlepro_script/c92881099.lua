...
function c92881099.cfilter(c, tp)
  return (c:GetPreviousCodeOnField() == 76812113 or c:GetPreviousCodeOnField() == 12206212) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE)
end
...
function c92881099.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c92881099.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, g0)
  end
...
function c92881099.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92881099.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return true
  end
