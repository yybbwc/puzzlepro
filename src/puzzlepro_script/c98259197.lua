...
function c98259197.filter(c)
  return c:IsFaceup() and c:IsCode(90140980) and Duel.IsExistingMatchingCard(c98259197.filter2, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
...
function c98259197.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c98259197.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98259197.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
