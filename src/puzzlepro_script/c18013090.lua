...
function c18013090.atcon(e, tp, eg, ep, ev, re, r, rp)
  return ep == tp and Duel.GetTurnPlayer() == tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_SPELL)
end
function c18013090.atop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:GetFlagEffect(FLAG_ID_CHAINING) == 0 or c:GetFlagEffect(18013090) ~= 0 then
    return
  end
  local e2 = Effect.CreateEffect(c)
...
function c18013090.catg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c18013090.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c18013090.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
