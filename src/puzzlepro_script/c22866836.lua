...
function c22866836.tgfilter1(c)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsType(TYPE_XYZ) and c:IsPosition(POS_FACEUP_ATTACK) and c:IsCanChangePosition()
end
function c22866836.target1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c22866836.tgfilter1, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c22866836.operation1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if pc:IsRelateToEffect(e) and dc:IsRelateToEffect(e) and pc:IsControler(tp) and Duel.ChangePosition(pc, POS_FACEUP_DEFENSE, POS_FACEUP_ATTACK, POS_FACEUP_ATTACK, POS_FACEUP_ATTACK) ~= 0 and dc:IsControler(1 - tp) then
    Duel.Destroy(dc, REASON_EFFECT)
  end
end
function c22866836.tgfilter2(c)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsType(TYPE_XYZ) and c:IsPosition(POS_FACEUP_DEFENSE) and c:IsCanChangePosition()
end
function c22866836.target2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, 0, LOCATION_SZONE, 1, nil) and Duel.IsExistingTarget(c22866836.tgfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_POSCHANGE)
