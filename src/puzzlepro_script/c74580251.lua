...
function c74580251.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74580251.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c74580251.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xc4) and c:IsOnField() and c:IsControler(tp) and c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE)
end
function c74580251.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and aux.exccon(e) and eg:IsExists(c74580251.repfilter, 1, nil, tp)
  end
