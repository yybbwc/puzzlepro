...
function c98552723.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c98552723.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c98552723.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c98552723.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) then
    if not Duel.SpecialSummonStep(tc, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) then
      return
    end
    Duel.Equip(tp, c, tc)
    --Add Equip limit
