...
function c81913510.filter1(c, e, tp)
  ...
  return c:IsFaceup() and c:IsSetCard(0x1f) and Duel.IsExistingMatchingCard(c81913510.filter2, tp, LOCATION_EXTRA, 0, 1, nil, code, e, tp, c)
end
...
function c81913510.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c81913510.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81913510.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
