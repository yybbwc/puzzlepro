...
function c82773292.fcheck(c, g, e, tp)
  return c:IsLocation(LOCATION_HAND + LOCATION_DECK) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE, 1 - tp) and g:IsExists(c82773292.fcheck2, 1, c)
end
...
function c82773292.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and g:CheckSubGroup(c82773292.fselect, 2, 2, e, tp)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_HAND + LOCATION_DECK)
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, tp, LOCATION_DECK)
end
function c82773292.cfilter(c, e, tp)
  return c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE, 1 - tp) and (c:IsLocation(LOCATION_HAND) or not c:IsAbleToHand())
end
function c82773292.cfilter2(c, e, tp)
  return not c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE, 1 - tp) and c:IsLocation(LOCATION_DECK) and c:IsAbleToHand()
end
