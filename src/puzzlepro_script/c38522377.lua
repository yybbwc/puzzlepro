...
function c38522377.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c38522377.spfilter, c:GetControler(), LOCATION_MZONE, 0, 3, nil)
end
function c38522377.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingTarget(c38522377.spfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c38522377.damop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dp ~= PLAYER_ALL then
    Duel.Damage(dp, 1000, REASON_EFFECT)
  else
