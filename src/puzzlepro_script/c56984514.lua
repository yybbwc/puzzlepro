...
function c56984514.condition(e, tp, eg, ep, ev, re, r, rp)
  return (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev) and Duel.IsExistingMatchingCard(c56984514.filter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c56984514.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c56984514.cfilter, 1, nil, tp)
  end
