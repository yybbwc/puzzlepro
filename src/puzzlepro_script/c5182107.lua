...
function c5182107.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5182107.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c5182107.spfilter(c, e, tp)
  return c:IsAttackBelow(1500) and c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_EARTH) and not c:IsCode(5182107) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c5182107.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5182107.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
