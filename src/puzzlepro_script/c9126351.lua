...
function c9126351.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c9126351.spfilter, tp, LOCATION_HAND, 0, 1, c)
end
...
function c9126351.tgfilter(c)
  return c:IsLevelBelow(2) and c:IsAttribute(ATTRIBUTE_WATER) and c:IsRace(RACE_AQUA) and (c:IsLocation(LOCATION_DECK) or c:IsFaceup()) and c:IsAbleToGrave()
end
function c9126351.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9126351.tgfilter, tp, LOCATION_DECK + LOCATION_MZONE, 0, 1, nil)
  end
...
function c9126351.excost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 9126352) == 0 and Duel.IsExistingMatchingCard(Card.IsAbleToHandAsCost, tp, LOCATION_MZONE, 0, 1, nil)
  end
