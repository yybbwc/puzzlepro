...
function c54912977.condition(e, tp, eg, ep, ev, re, r, rp)
  return r ~= REASON_REPLACE and Duel.GetAttackTarget() == e:GetHandler() and Duel.GetAttacker():IsControler(1 - tp) and e:GetHandler():GetBattlePosition() == POS_FACEDOWN_DEFENSE
end
function c54912977.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, 0, LOCATION_MZONE, 1, Duel.GetAttacker())
  end
...
function c54912977.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) and a:IsRelateToBattle() and a:IsAttackable() and not a:IsImmuneToEffect(e) then
...
function c54912977.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c54912977.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c54912977.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not e:GetHandler():IsRelateToEffect(e) or e:GetHandler():IsFacedown() then
    return
  end
