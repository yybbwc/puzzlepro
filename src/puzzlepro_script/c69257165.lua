...
function c69257165.ctfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsDiscardable() and Duel.IsExistingTarget(c69257165.ctffilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetLevel())
end
...
function c69257165.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c69257165.ctffilter(chkc, e:GetLabel())
  end
  if chk == 0 then
