...
function s.atkfilter(c, code)
  return c:IsFaceup() and bit.band(c:GetOriginalType(), TYPE_MONSTER) == TYPE_MONSTER and c:GetFlagEffect(code) ~= 0
end
...
function s.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.spcfilter, tp, LOCATION_ONFIELD, 0, 1, c, tp)
  end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.sfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
