...
function c10300821.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, 2, REASON_EFFECT)
  end
...
function c10300821.tgcon(e, tp, eg, ep, ev, re, r, rp)
  if not e:GetHandler():GetOverlayGroup():IsExists(Card.IsCode, 1, nil, 71921856) then
    return false
  end
...
function c10300821.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc:IsCanOverlay() and Duel.IsExistingMatchingCard(c10300821.tgfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
...
function c10300821.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) ~= 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) then
    ...
    if c:IsRelateToChain() and tc:IsRelateToChain() and tc:IsControler(1 - tp) and not tc:IsImmuneToEffect(e) then
