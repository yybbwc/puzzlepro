...
function c33545259.regcon(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c33545259.regfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c33545259.regfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c33545259.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c33545259.costfilter, tp, LOCATION_ONFIELD + LOCATION_HAND, 0, 1, c, tp)
  end
...
function c33545259.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and bc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
  end
