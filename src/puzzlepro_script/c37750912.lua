...
function c37750912.tgfilter(c, e, tp, check)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xa3) and (c:IsAbleToGrave() or check and c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
...
function c37750912.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    local check = Duel.IsExistingMatchingCard(c37750912.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
...
function c37750912.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc then
    if check and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not tc:IsAbleToGrave() or Duel.SelectOption(tp, 1191, 1152) == 1) then
...
function c37750912.lvltg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c37750912.lvlfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37750912.lvlfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_OPERATECARD)
