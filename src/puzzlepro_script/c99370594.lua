...
function c99370594.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99370594.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c99370594.target1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c99370594.filter0(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c99370594.filter0, tp, LOCATION_REMOVED, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
