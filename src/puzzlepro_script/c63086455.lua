...
function c63086455.setfilter(c, cc, e, tp)
  ...
  return b1 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE, 1 - tp) or (b2 or c:IsType(TYPE_FIELD)) and c:IsSSetable(true)
end
function c63086455.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c63086455.tgfilter, tp, 0, LOCATION_ONFIELD, 1, nil, e, tp)
  end
