...
function c48589580.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c48589580.thfilter, tp, LOCATION_DECK, 0, 1, nil, thchk)
  end
...
function c48589580.cfilter(c, tp)
  return bit.band(c:GetPreviousRaceOnField(), RACE_FAIRY) ~= 0 and c:IsRace(RACE_FAIRY) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c48589580.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c48589580.costfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
