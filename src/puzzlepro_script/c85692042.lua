...
function c85692042.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ct = e:GetLabel()
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, ct, REASON_COST)
  end
...
function c85692042.thfilter(c, ct)
  ...
  elseif ct == 3 then
    return c:IsType(TYPE_MONSTER) and c:IsLevel(4)
  else
...
function c85692042.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85692042.thfilter, tp, LOCATION_DECK, 0, 1, nil, e:GetLabel())
  end
...
function c85692042.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c85692042.costfilter, 1, nil, tp)
  end
...
function c85692042.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85692042.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
