...
function c18712704.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tg:IsOnField() and tg:IsDestructable() and tg:IsCanBeEffectTarget(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c18712704.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c18712704.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dc:IsRelateToEffect(e) and dc:IsAttackPos() then
    ...
    if sc:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and sc:IsCanBeSpecialSummoned(e, 0, tp, false, false) then
