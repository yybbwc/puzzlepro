...
function c5325424.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c5325424.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5325424.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
