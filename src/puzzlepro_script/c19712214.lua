...
function c19712214.spfilter(c, e, tp)
  return c:IsCode(67712104) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_GRAVE) and Duel.GetMZoneCount(tp) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)
end
function c19712214.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19712214.spfilter, tp, LOCATION_GRAVE + LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c19712214.attg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c19712214.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19712214.tgfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
