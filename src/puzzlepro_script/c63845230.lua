...
function c63845230.spcon(e, c)
  ...
  if ft <= 0 and Duel.GetMatchingGroupCount(Card.IsAbleToRemoveAsCost, tp, LOCATION_MZONE, 0, c, POS_FACEDOWN) <= -ft then
    return false
  end
...
function c63845230.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc and tc:IsControler(1 - tp) and tc:IsAbleToRemove(tp, POS_FACEDOWN)
  end
