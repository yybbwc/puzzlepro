...
function s.raatfilter(c, ec)
  return c:IsFaceupEx() and c:IsRace(RACE_DRAGON) and (not c:IsAttribute(ec:GetAttribute()) or not c:IsLevel(ec:GetLevel()))
end
function s.raattg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.raatfilter, tp, LOCATION_REMOVED, 0, 1, nil, c)
  end
end
...
function s.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.atkfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.atkfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
