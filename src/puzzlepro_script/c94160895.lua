...
function c94160895.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if ft > -1 then
    local loc = 0
    if Duel.IsPlayerAffectedByEffect(tp, 88581108) then
      loc = LOCATION_MZONE
    end
  ...
  if chk == 0 then
    return ft > -2 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:GetCount() >= 2 and g:IsExists(Card.IsAttribute, 1, nil, ATTRIBUTE_WIND) and (ft ~= 0 or g:IsExists(c94160895.mzfilter, 1, nil, tp))
  end
...
function c94160895.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() < 2 or not g:IsExists(Card.IsAttribute, 1, nil, ATTRIBUTE_WIND) then
    return
  end
  local g1 = nil
  ...
  if Duel.Destroy(g1, REASON_EFFECT) == 2 then
    ...
    if rm and rg:GetCount() > 0 and rg:FilterCount(Card.IsAbleToRemove, nil) == 4 and Duel.SelectYesNo(tp, aux.Stringid(94160895, 2)) then
...
function c94160895.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94160895.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
