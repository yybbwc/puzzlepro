...
function c17197110.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return maxc > 0 and rg:CheckSubGroup(c17197110.fselect, 1, maxc, tp)
  end
...
function c17197110.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft > 0 and Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    ft = 1
  end
  local ct = e:GetLabel()
  if ft < ct or ft <= 0 then
    return
  end
...
function c17197110.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c17197110.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17197110.thfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
