...
function c97173708.rfilter1(c, e, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsType(TYPE_TOKEN) and Duel.CheckReleaseGroup(tp, c97173708.rfilter2, 1, c, lv) and Duel.IsExistingTarget(c97173708.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, lv)
end
...
function c97173708.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c97173708.spfilter(chkc, e, tp, e:GetLabel())
  end
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -2 and Duel.CheckReleaseGroup(tp, c97173708.rfilter1, 1, nil, e, tp)
  end
