...
function c53241226.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return ct > 0 and Duel.IsExistingTarget(c53241226.eqfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c53241226.tgfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c53241226.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if hc:IsControler(tp) and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) and tc:IsLocation(LOCATION_MZONE) and tc:IsAbleToChangeControler() and Duel.Equip(tp, tc, hc, false) then
    --Add Equip limit
    local e1 = Effect.CreateEffect(c)
