...
function c97452817.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 97452818, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT)
  end
...
function c97452817.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 97452818, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT) then
    return
  end
...
function c97452817.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and chkc:IsAbleToChangeControler()
  end
  if chk == 0 then
    return true
  end
