...
function c95886782.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c95886782.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c95886782.eqfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c95886782.atkcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():GetEquipTarget() and eg:IsExists(c95886782.cfilter, 1, nil, e, tp)
end
...
function c95886782.repcon(e, tp, eg, ep, ev, re, r, rp)
  return bit.band(r, REASON_COST) ~= 0 and re:IsActivated() and re:IsActiveType(TYPE_XYZ) and re:GetHandler():GetOverlayCount() >= ev - 1 and e:GetHandler():GetEquipTarget() == re:GetHandler() and e:GetHandler():IsAbleToGraveAsCost() and ep == e:GetOwnerPlayer()
end
