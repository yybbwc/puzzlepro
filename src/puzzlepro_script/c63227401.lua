...
function c63227401.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
...
function c63227401.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:GetFlagEffect(63227401) == 0 or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
    return false
  end
