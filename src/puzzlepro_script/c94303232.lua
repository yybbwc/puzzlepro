...
function c94303232.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c94303232.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(nil, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c94303232.rmcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_LOST_TARGET) and c:GetPreviousEquipTarget():IsReason(REASON_DESTROY) and c:IsHasCardTarget(e:GetLabelObject():GetLabelObject())
end
