...
function c10065487.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c10065487.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c10065487.filter2, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c10065487.spfilter(c, e, tp)
  return c:IsType(TYPE_FUSION) and aux.IsMaterialListCode(c, 68468459) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c10065487.spop(e, p)
  if Duel.IsExistingMatchingCard(c10065487.spfilter, p, LOCATION_EXTRA, 0, 1, nil, e, p) and Duel.SelectYesNo(p, aux.Stringid(10065487, 1)) then
