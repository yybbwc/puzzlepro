...
function c31588572.setfilter(c, e, tp)
  if not c:IsSetCard(0x130) or c:IsCode(31588572) then
    return false
  end
...
function c31588572.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c31588572.setfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c31588572.setfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c31588572.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousLocation(LOCATION_ONFIELD) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and not re:GetHandler():IsCode(31588572)))
end
...
function c31588572.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c31588572.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
