...
function c69809989.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c69809989.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c69809989.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.IsExistingMatchingCard(c69809989.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(69809989, 2)) then
    Duel.BreakEffect()
...
function c69809989.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsPosition(LOCATION_MZONE) and c69809989.xyzfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69809989.xyzfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c69809989.mfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
