...
function c60944809.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c60944809.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c60944809.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function c60944809.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c60944809.matfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60944809.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(Card.IsCanOverlay, tp, LOCATION_HAND, 0, 1, nil)
  end
