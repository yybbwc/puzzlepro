...
function c59805313.rmfilter(c, tp)
  return c:IsSetCard(0xb9) and c:IsFaceupEx() and c:IsAbleToRemove() and (c:IsLocation(LOCATION_SZONE) or Duel.GetLocationCount(tp, LOCATION_SZONE) > 0)
end
...
function c59805313.optg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b1 = Duel.GetFlagEffect(tp, 59805313) == 0 and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c59805313.tgfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  local b2 = Duel.GetFlagEffect(tp, 59805314) == 0 and Duel.IsExistingMatchingCard(c59805313.rmfilter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil, tp) and Duel.IsExistingMatchingCard(c59805313.setfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, tp)
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(59805313, 1), aux.Stringid(59805313, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(59805313, 1))
  elseif b2 then
    op = Duel.SelectOption(tp, aux.Stringid(59805313, 2)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
