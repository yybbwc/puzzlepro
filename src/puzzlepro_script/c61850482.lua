...
function c61850482.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c61850482.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(1 - tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c61850482.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
