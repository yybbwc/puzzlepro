...
function c55063751.spcon2(e, c)
  ...
  local tp = c:GetControler()
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c55063751.cfilter, tp, 0, LOCATION_MZONE, 1, nil)
end
function c55063751.negcon(e, tp, eg, ep, ev, re, r, rp)
  if ep == tp or e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) then
    return false
  end
