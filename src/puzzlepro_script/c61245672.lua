...
function c61245672.cfilter(c, e, tp, mc)
  return c:IsRace(RACE_CYBERSE) and c:IsType(TYPE_LINK) and c:IsLinkBelow(3) and not c:IsCode(61245672) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
...
function c61245672.drop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) ~= 0 and Duel.GetLP(tp) <= 2000 and c:IsRelateToEffect(e) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c61245672.cfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c) and Duel.SelectYesNo(tp, aux.Stringid(61245672, 1)) then
    Duel.BreakEffect()
