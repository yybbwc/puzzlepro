...
function c63748694.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63748694.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c63748694.filter(c, e, tp, oc)
  return c:IsSetCard(0x166) and not c:IsCode(24639891) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c63748694.xyzfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, oc, c)
end
function c63748694.xyzfilter(c, e, tp, oc, mc)
  return c:IsType(TYPE_XYZ) and aux.IsCodeListed(c, mc:GetCode()) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and oc:IsCanBeXyzMaterial(c) and mc:IsCanBeXyzMaterial(c) and Duel.GetLocationCountFromEx(tp, tp, Group.FromCards(oc, mc), c) > 0 and aux.MustMaterialCheck(Group.FromCards(oc, mc), tp, EFFECT_MUST_BE_XMATERIAL)
end
function c63748694.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonCount(tp, 2) and mg:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(63748694, 0)) then
    Duel.BreakEffect()
