...
function s.dfilter(c, e, tp)
  return c:IsFaceupEx() and c:IsType(TYPE_MONSTER) and c:IsSetCard(0x1a1) and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c)
end
function s.filter(c, e, tp, tc)
  return c:IsSetCard(0x1a1) and c:IsType(TYPE_MONSTER) and not c:IsCode(tc:GetCode()) and (c:IsAbleToHand() or Duel.GetMZoneCount(tp, tc) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE))
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.dfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dc == nil or Duel.Destroy(dc, REASON_EFFECT) < 1 then
    return
  end
  ...
  if tc then
    local op = aux.SelectFromOptions(tp, { tc:IsAbleToHand(), 1190 }, {
      Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE),
      1152,
    })
    if op == 1 then
...
function s.prctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.cfilter(chkc)
  end
  if chk == 0 then
