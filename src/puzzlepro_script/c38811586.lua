...
function c38811586.gcheck(g, e, tp)
  ...
  local ac = g:GetFirst()
  local bc = g:GetNext()
  return c38811586.spsumfilter1(ac, e, tp) and c38811586.spsumfilter2(bc, e, tp) or c38811586.spsumfilter1(bc, e, tp) and c38811586.spsumfilter2(ac, e, tp)
end
function c38811586.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    ...
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and ft1 > 0 and ft2 > 0 and g:CheckSubGroup(c38811586.gcheck, 2, 2, e, tp)
  end
...
function c38811586.spop(e, tp, eg, ep, ev, re, r, rp)
  local ft1 = Duel.GetLocationCount(tp, LOCATION_MZONE)
  local ft2 = Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp)
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) or ft1 <= 0 or ft2 <= 0 then
    return
  end
  local g = Duel.GetTargetsRelateToChain()
  if not g:CheckSubGroup(c38811586.gcheck, 2, 2, e, tp) then
    return
  end
...
function c38811586.spittg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and #rg >= 4 and Duel.GetMZoneCount(tp, rg) > 0
  end
