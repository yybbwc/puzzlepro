...
function c29228529.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29228529.costfilter, tp, LOCATION_HAND, 0, 2, e:GetHandler())
  end
...
function c29228529.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c29228529.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29228529.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
