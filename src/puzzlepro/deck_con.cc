...
namespace ygo {
  ...
  void DeckBuilder::Initialize() {
    ...
    mainGame->static_text_deck_edit_main_deck_size->setVisible(true);
    mainGame->static_text_deck_edit_extra_deck_size->setVisible(true);
    mainGame->static_text_deck_edit_side_deck_size->setVisible(true);
    mainGame->static_text_deck_edit_search_result_size->setVisible(true);
  ...
  void DeckBuilder::Terminate() {
    ...
    mainGame->static_text_deck_edit_search_result_size->setVisible(false);
  ...
  bool DeckBuilder::OnEvent(const irr::SEvent &event) {
    ...
    switch (event.EventType) {
      case irr::EET_GUI_EVENT: {
        ...
        //~ if (((mainGame->wCategories->isVisible() && id != BUTTON_CATEGORY_OK) || (mainGame->wQuery->isVisible() && id != BUTTON_YES && id != BUTTON_NO) || (mainGame->wLinkMarks->isVisible() && id != BUTTON_MARKERS_OK) || (mainGame->wDMQuery->isVisible() && id != BUTTON_DM_OK && id != BUTTON_DM_CANCEL) || (mainGame->wDeckManage->isVisible() && !(id >= WINDOW_DECK_MANAGE && id < COMBOBOX_LFLIST))) && event.GUIEvent.EventType != irr::gui::EGET_LISTBOX_CHANGED && event.GUIEvent.EventType != irr::gui::EGET_COMBO_BOX_CHANGED) {
        //~ if (((mainGame->wCategories->isVisible() && id != BUTTON_CATEGORY_OK) || (mainGame->wQuery->isVisible() && id != BUTTON_YES && id != BUTTON_NO) || (mainGame->wLinkMarks->isVisible() && id != BUTTON_MARKERS_OK) || (mainGame->wDMQuery->isVisible() && id != BUTTON_DM_OK && id != BUTTON_DM_CANCEL) || (!(id >= WINDOW_DECK_MANAGE && id < COMBOBOX_LFLIST))) && event.GUIEvent.EventType != irr::gui::EGET_LISTBOX_CHANGED && event.GUIEvent.EventType != irr::gui::EGET_COMBO_BOX_CHANGED) {
        //~ if (((mainGame->wCategories->isVisible() && id != BUTTON_CATEGORY_OK) || (mainGame->wQuery->isVisible() && id != BUTTON_YES && id != BUTTON_NO) || (mainGame->wLinkMarks->isVisible() && id != BUTTON_MARKERS_OK) || (mainGame->wDMQuery->isVisible() && id != BUTTON_DM_OK && id != BUTTON_DM_CANCEL) || (!(id >= WINDOW_DECK_MANAGE && id < COMBOBOX_LFLIST))) && event.GUIEvent.EventType != irr::gui::EGET_LISTBOX_CHANGED && event.GUIEvent.EventType != irr::gui::EGET_COMBO_BOX_CHANGED) {
        ...
        switch (event.GUIEvent.EventType) {
          ...
          case irr::gui::EGET_BUTTON_CLICKED: {
            ...
            switch (id) {
              ...
              case BUTTON_SIDE_OK: {
                if (deckManager.current_deck.main.size() != pre_mainc || deckManager.current_deck.extra.size() != pre_extrac || deckManager.current_deck.side.size() != pre_sidec) {
                  soundManager.PlaySoundEffect(SOUND_INFO);
                  mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1410));
                  break;
                }
                mainGame->ClearCardInfo();
          ...
          case irr::gui::EGET_COMBO_BOX_CHANGED: {
            switch (id) {
              ...
              case COMBOBOX_DBDECKS: {
                ...
                //~ mainGame->cbDBDecks->setToolTipText(mainGame->cbDBDecks->getItem(decksel));
                prev_deck = decksel;
                is_modified = false;
                break;
              }
              case COMBOBOX_MAINTYPE: {
                ...
                mainGame->guiEnv->setFocus(nullptr);
                InstantSearch();
                break;
              }
      ...
      case irr::EET_MOUSE_INPUT_EVENT: {
        switch (event.MouseInput.Event) {
          ...
          case irr::EMIE_MOUSE_WHEEL: {
            ...
            if (mainGame->guiEnv->hasFocus(mainGame->scrFilter)) {
              break;
            }
            if (root->getElementFromPoint(mouse_pos) != root) {
              break;
  ...
  void DeckBuilder::GetHoveredCard() {
    ...
    int y = pos.Y;
    is_lastcard = 0;
    int64_t gradient_background_main_deck_x1 = gui_xy["gradient_background_main_deck"]["x1"];
    ...
    for (auto ptr = dataManager.datas_begin(); ptr != dataManager.datas_end(); ++ptr) {
      ...
      switch (filter_type) {
        case 1: {
          ...
          if (filter_atktype) {
            if ((filter_atktype == 1 && data.attack != filter_atk) || (filter_atktype == 2 && data.attack < filter_atk) || (filter_atktype == 3 && data.attack <= filter_atk) || (filter_atktype == 4 && (data.attack > filter_atk || data.attack < 0)) || (filter_atktype == 5 && (data.attack >= filter_atk || data.attack < 0)) || (filter_atktype == 6 && data.attack != -2)) {
              continue;
            }
          }
          if (filter_deftype) {
            if ((filter_deftype == 1 && data.defense != filter_def) || (filter_deftype == 2 && data.defense < filter_def) || (filter_deftype == 3 && data.defense <= filter_def) || (filter_deftype == 4 && (data.defense > filter_def || data.defense < 0)) || (filter_deftype == 5 && (data.defense >= filter_def || data.defense < 0)) || (filter_deftype == 6 && data.defense != -2) || (data.type & TYPE_LINK)) {
              continue;
            }
          }
          if (filter_lvtype) {
            if ((filter_lvtype == 1 && data.level != filter_lv) || (filter_lvtype == 2 && data.level < filter_lv) || (filter_lvtype == 3 && data.level <= filter_lv) || (filter_lvtype == 4 && data.level > filter_lv) || (filter_lvtype == 5 && data.level >= filter_lv) || filter_lvtype == 6) {
              continue;
            }
          }
          if (filter_scltype) {
            if ((filter_scltype == 1 && data.lscale != filter_scl) || (filter_scltype == 2 && data.lscale < filter_scl) || (filter_scltype == 3 && data.lscale <= filter_scl) || (filter_scltype == 4 && (data.lscale > filter_scl)) || (filter_scltype == 5 && (data.lscale >= filter_scl)) || filter_scltype == 6 || !(data.type & TYPE_PENDULUM)) {
              continue;
            }
          }
          break;
      ...
      if (filter_lm) {
        if (filter_lm <= 3 && (!filterList->content.contains(ptr->first) || filterList->content.at(ptr->first) != filter_lm - 1)) {
          continue;
        }
      ...
      for (auto elements_iterator = query_elements.begin(); elements_iterator != query_elements.end(); ++elements_iterator) {
        ...
        if (elements_iterator->type == element_t::type_t::name) {
          ...
          match = CardNameContains(text.name.c_str(), elements_iterator->keyword.c_str()) || text.text.find(elements_iterator->keyword) != std::wstring::npos || data.is_setcodes(elements_iterator->setcodes);
        }
  ...
  void DeckBuilder::RefreshDeckList() {
    ...
    lstDecks->clear();
    mainGame->RefreshDeck(catepath, [lstDecks](const wchar_t *item) {
      lstDecks->addItem(item);
    });
  }
  ...
  void DeckBuilder::ShowBigCard(int code, float zoom) {
    bigcard_code = code;
    bigcard_zoom = zoom;
    irr::video::ITexture *img = imageManager.GetBigPicture(code, zoom);
  ...
  void DeckBuilder::ZoomBigCard(irr::s32 centerx, irr::s32 centery) {
    ...
    irr::video::ITexture *img = imageManager.GetBigPicture(bigcard_code, bigcard_zoom);
    mainGame->imgBigCard->setImage(img);
    auto size = img->getSize();
