...
function c80402389.check(e, tp, eg, ep, ev, re, r, rp)
  ...
  while tc do
    if tc:IsPreviousLocation(LOCATION_MZONE) and tc:IsReason(REASON_DESTROY) and tc:IsRace(RACE_INSECT) and tc:IsLevelAbove(1) and tc:IsPreviousPosition(POS_FACEUP) then
...
function c80402389.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80402389.filter, tp, LOCATION_DECK, 0, 1, nil, ev)
  end
