...
function c35026117.tffilter(c, tp)
  return c:IsFaceupEx() and c:IsCode(25955164, 62340868, 98434877) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
function c35026117.tftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c35026117.tffilter, tp, LOCATION_DECK + LOCATION_REMOVED + LOCATION_HAND, 0, 1, nil, tp)
  end
end
...
function c35026117.tfop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.MoveToField(tc, tp, tp, LOCATION_SZONE, POS_FACEUP, true) then
    ...
    if Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c35026117.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(35026117, 2)) then
      Duel.BreakEffect()
