...
function c3693034.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c3693034.posfilter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c3693034.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if hc:IsRelateToEffect(e) and hc:IsControler(tp) and Duel.ChangePosition(hc, POS_FACEUP_DEFENSE, POS_FACEUP_DEFENSE, POS_FACEUP_ATTACK, POS_FACEUP_ATTACK) ~= 0 and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(1 - tp) and tc:IsCanBeDisabledByEffect(e) then
...
function c3693034.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c3693034.posfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c3693034.posfilter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_POSCHANGE)
