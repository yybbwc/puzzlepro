...
function c82340056.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 82340057, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_ROCK, ATTRIBUTE_EARTH) and Duel.IsExistingMatchingCard(c82340056.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c82340056.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) < 2 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 82340057, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_ROCK, ATTRIBUTE_EARTH) then
    return
  end
