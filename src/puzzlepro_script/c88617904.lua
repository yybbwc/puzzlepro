...
function c88617904.tgfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsLevelBelow(3) and c:IsAbleToGrave() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c88617904.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c88617904.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88617904.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and Duel.IsExistingMatchingCard(c88617904.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
