...
function c2362787.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c2362787.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c2362787.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
