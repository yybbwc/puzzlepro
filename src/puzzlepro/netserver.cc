...
namespace ygo {
  ...
  bool NetServer::StartServer(unsigned short port) {
    ...
    listener = evconnlistener_new_bind(net_evbase, ServerAccept, nullptr, LEV_OPT_CLOSE_ON_FREE | LEV_OPT_REUSEABLE, -1, (sockaddr *)&sin, sizeof(sin));
    if (!listener) {
  ...
  void NetServer::ServerEchoRead(bufferevent *bev, void *ctx) {
    ...
    auto *net_server_read = new unsigned char[SIZE_NETWORK_BUFFER];
