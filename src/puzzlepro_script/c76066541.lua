...
function c76066541.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (c:IsReason(REASON_BATTLE) or rp == 1 - tp and c:IsReason(REASON_DESTROY) and c:IsPreviousControler(tp)) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
function c76066541.filter(c, e, tp)
  return c:IsType(TYPE_PENDULUM) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c76066541.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c76066541.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
