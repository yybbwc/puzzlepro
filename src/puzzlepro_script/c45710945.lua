...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not c:IsRelateToEffect(e) or (not c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and not c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp)) then
    return
  end
...
function s.mttg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return at:IsType(TYPE_XYZ) and at:IsOnField() and at:IsRace(RACE_DRAGON)
  end
end
