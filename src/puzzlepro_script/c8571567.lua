...
function c8571567.tftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c8571567.tffilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
...
function c8571567.cfilter(c, tp, se)
  return c:IsFaceup() and ((c:IsSetCard(0x33) and c:IsType(TYPE_SYNCHRO)) or c:IsCode(9012916)) and c:IsControler(tp) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function c8571567.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c8571567.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8571567.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
