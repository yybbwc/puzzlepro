...
function c12940613.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c12940613.tdfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c12940613.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c12940613.tdfilter1, tp, LOCATION_GRAVE, 0, 1, nil)
  end
