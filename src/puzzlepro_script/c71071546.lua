...
function c71071546.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and e:GetHandler():IsCanRemoveCounter(tp, 0x2c, ct, REASON_COST)
  end
  e:GetHandler():RemoveCounter(tp, 0x2c, ct, REASON_COST)
end
function c71071546.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsAttack(2000)
  end
end
...
function c71071546.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c71071546.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71071546.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
