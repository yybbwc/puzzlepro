...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_MZONE) and chkc:IsControler(tp) and s.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, LOCATION_GRAVE + LOCATION_MZONE, 0, 1, nil)
  end
...
function s.rfilter(c, e, tp)
  return c:IsSetCard(0x192) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(s.sfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetCode())
end
function s.sfilter(c, e, tp, code)
  return c:IsSetCard(0x192) and aux.IsCodeListed(c, code) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function s.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.bfgcost(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.IsExistingMatchingCard(s.rfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
