...
function c67775894.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c67775894.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67775894.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c67775894.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and c:GetControler() == c:GetEquipTarget():GetControler() and c:GetEquipTarget():IsAbleToGraveAsCost()
  end
