...
function c70479321.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tc:IsOnField() and tc:IsCanBeEffectTarget(e)
  end
...
function c70479321.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(c:GetControler(), LOCATION_MZONE, LOCATION_MZONE) == 0
end
