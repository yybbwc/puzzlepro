...
function s.lpcon(e, tp, eg, ep, ev, re, r, rp)
  return re:GetHandler():IsAllTypes(TYPE_RITUAL + TYPE_MONSTER) and eg:IsExists(s.cfilter2, 1, nil)
end
...
function s.cfilter3(c, tp, e)
  return c:IsLocation(LOCATION_MZONE) and c:IsSummonPlayer(tp) and c:IsFaceup() and c:IsCanBeEffectTarget(e) and c:IsType(TYPE_RITUAL) and c:IsLevelAbove(1)
end
