...
function c90027012.tg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90027012.filter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c90027012.filter2, tp, LOCATION_DECK, 0, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 2, tp, LOCATION_DECK)
end
function c90027012.op(e, tp, eg, ep, ev, re, r, rp)
  if not (Duel.IsExistingMatchingCard(c90027012.filter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c90027012.filter2, tp, LOCATION_DECK, 0, 1, nil)) then
    return
  end
