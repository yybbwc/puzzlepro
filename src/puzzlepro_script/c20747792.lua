...
function c20747792.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c20747792.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c20747792.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c20747792.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c20747792.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or Duel.ChangePosition(c, POS_FACEUP_DEFENSE, POS_FACEUP_DEFENSE, POS_FACEUP_ATTACK, POS_FACEUP_ATTACK) == 0 then
    return
  end
