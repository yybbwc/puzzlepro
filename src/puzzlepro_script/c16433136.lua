...
function c16433136.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c16433136.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c16433136.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c16433136.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToHand, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_MZONE, 1, nil)
  end
