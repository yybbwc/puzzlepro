...
function c84569886.atkcon1(e, tp, eg, ep, ev, re, r, rp)
  ...
  return a and d and a:IsFaceup() and a:IsRelateToBattle() and a:IsSetCard(0x10af) and a:IsType(TYPE_FUSION) and d:IsFaceup() and d:IsRelateToBattle() and d:GetAttack() > 0 and a:GetControler() ~= d:GetControler()
end
...
function c84569886.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and chkc:IsPosition(POS_FACEUP_ATTACK)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsPosition, tp, 0, LOCATION_MZONE, 1, nil, POS_FACEUP_ATTACK)
  end
