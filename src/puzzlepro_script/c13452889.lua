...
function c13452889.cfilter(c, tp, g, zone)
  return g:IsContains(c) and (Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_TOFIELD, zone[tp]) > 0 or Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp, LOCATION_REASON_TOFIELD, zone[1 - tp]) > 0)
end
function c13452889.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c13452889.cfilter, 1, nil, tp, lg, zone)
  end
...
function c13452889.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return bc:IsCanBeSpecialSummoned(e, 0, tp, false, false) or bc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp, zone)
  end
...
function c13452889.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    ...
    if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone1) and (not tc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp, zone2) or Duel.SelectYesNo(tp, aux.Stringid(13452889, 1))) then
      Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP, zone1)
    else
      if Duel.SpecialSummon(tc, 0, tp, 1 - tp, false, false, POS_FACEUP, zone2) ~= 0 and c:IsRelateToBattle() then
        local e1 = Effect.CreateEffect(c)
