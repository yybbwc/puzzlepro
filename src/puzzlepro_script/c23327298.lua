...
function c23327298.cfilter(c, tp)
  return c:IsType(TYPE_SYNCHRO) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEUP) and c:IsControler(tp) and c:IsPreviousControler(tp) and c:GetReasonPlayer() == 1 - tp
end
...
function c23327298.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c23327298.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c23327298.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
