...
function c31817415.filter(c, tp)
  return c:IsRace(RACE_DRAGON) and c:IsLevelAbove(1) and Duel.IsExistingMatchingCard(c31817415.desfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetLevel())
end
...
function c31817415.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReleasable() and Duel.IsExistingMatchingCard(c31817415.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
