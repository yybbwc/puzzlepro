...
function c34479658.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return matc > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 34479659, 0, TYPES_TOKEN_MONSTER, 2000, 0, 1, RACE_REPTILE, ATTRIBUTE_DARK)
  end
...
function c34479658.tkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 34479659, 0, TYPES_TOKEN_MONSTER, 2000, 0, 1, RACE_REPTILE, ATTRIBUTE_DARK) then
    return
  end
  local ctn = true
  while matc > 0 and ctn do
    ...
    matc = matc - 1
    if matc <= 0 or not Duel.SelectYesNo(tp, aux.Stringid(34479658, 2)) then
      ctn = false
    end
  end
...
function c34479658.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c34479658.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
