...
function c78917791.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c78917791.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingTarget(c78917791.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c78917791.actlimit(e, re, rp)
  ...
  return re:IsActiveType(TYPE_MONSTER) and rc:IsOnField() and rc:IsSummonLocation(LOCATION_EXTRA) and not rc:IsSetCard(0x12c)
end
