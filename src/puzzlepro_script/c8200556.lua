...
function c8200556.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c8200556.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c8200556.spcostfilter(c, e, tp, tc)
  ...
  return c:IsAbleToRemoveAsCost() and c8200556.spcostexcheck(c, e, tp) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and Duel.GetMZoneCount(tp, tg) > 0
end
function c8200556.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    e:SetLabel(100)
    return Duel.IsExistingMatchingCard(c8200556.spcostfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, e:GetHandler(), e, tp, e:GetHandler()) and e:GetHandler():IsReleasable()
  end
