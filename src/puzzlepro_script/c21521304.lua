...
function c21521304.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingTarget(c21521304.rmfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c21521304.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c21521304.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc1:IsRelateToEffect(e) or Duel.Remove(tc1, POS_FACEUP, REASON_EFFECT) == 0 then
    return
  end
  local tc2 = g2:GetFirst()
  if not tc2:IsRelateToEffect(e) or Duel.SpecialSummon(tc2, 0, tp, tp, false, false, POS_FACEUP) == 0 then
    return
  end
  Duel.BreakEffect()
