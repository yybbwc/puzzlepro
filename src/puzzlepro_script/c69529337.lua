...
function c69529337.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c69529337.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69529337.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(c69529337.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c69529337.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tc and tc:IsFaceup() and tc:IsSetCard(0xe2) and tc:IsAbleToGrave() and tc:IsCanBeEffectTarget(e) and Duel.IsExistingMatchingCard(c69529337.filter, tp, LOCATION_DECK, 0, 1, nil, tp, tc:GetCode())
  end
