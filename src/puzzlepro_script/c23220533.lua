...
function c23220533.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c23220533.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c23220533.spfilter(c, e, tp)
  return c:IsSetCard(0x159) and c:IsType(TYPE_MONSTER) and not c:IsCode(23220533) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c23220533.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c23220533.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c23220533.spfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
