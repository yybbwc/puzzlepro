...
function c88234365.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and not c:IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c88234365.repfilter, 1, nil, tp)
  end
...
function c88234365.cptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c88234365.cpfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
