...
function c21420702.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21420702.costfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c21420702.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetActivityCount(tp, ACTIVITY_ATTACK) == 0
  end
...
function c21420702.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21420702.disfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingTarget(aux.TRUE, tp, 0, LOCATION_MZONE, 1, nil)
  end
