...
function c22662014.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22662014.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c22662014.costfilter(c, ft)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_TRAP) and c:IsSetCard(0x15c) and c:IsAbleToGraveAsCost() and (ft > 0 or c:IsLocation(LOCATION_SZONE) and ft > -1)
end
function c22662014.setcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ft = Duel.GetLocationCount(tp, LOCATION_SZONE)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22662014.costfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil, ft)
  end
...
function c22662014.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22662014.setfilter, tp, LOCATION_DECK, 0, 1, nil, true)
  end
end
