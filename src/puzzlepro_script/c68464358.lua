...
function c68464358.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c68464358.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68464358.tgfilter, tp, LOCATION_ONFIELD, 0, 1, nil, tp) and Duel.IsExistingMatchingCard(c68464358.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
