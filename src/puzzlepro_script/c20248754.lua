...
function c20248754.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c20248754.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c20248754.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c20248754.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c20248754.rmfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c20248754.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c20248754.repfilter, 1, nil, tp) and e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_EFFECT)
  end
