...
function c25166510.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c25166510.tdfilter(chkc)
  end
  local ct = Duel.GetMatchingGroupCount(Card.IsFaceup, tp, 0, LOCATION_ONFIELD, nil)
  if chk == 0 then
    return ct > 0 and Duel.IsExistingTarget(c25166510.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
