...
function c57261568.hspfilter(c, tp)
  return c:IsSetCard(0xe6) and not c:IsCode(57261568) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c57261568.syncheck(c, g, mg, tp, lv, syncard, minc, maxc)
  g:AddCard(c)
  local ct = g:GetCount()
  local res = c57261568.syngoal(g, tp, lv, syncard, minc, ct) or (ct < maxc and mg:IsExists(c57261568.syncheck, 1, g, g, mg, tp, lv, syncard, minc, maxc))
  g:RemoveCard(c)
  return res
end
function c57261568.syngoal(g, tp, lv, syncard, minc, ct)
  return ct >= minc and Duel.GetLocationCountFromEx(tp, tp, g, syncard) > 0 and (g:CheckWithSumEqual(Card.GetSynchroLevel, lv, ct, ct, syncard) or g:CheckWithSumEqual(c57261568.cardiansynlevel, lv, ct, ct, syncard)) and aux.MustMaterialCheck(g, tp, EFFECT_MUST_BE_SMATERIAL)
end
function c57261568.syntg(e, syncard, f, min, max)
  ...
  if lv <= c:GetLevel() and lv <= c57261568.cardiansynlevel(c) then
    return false
  end
