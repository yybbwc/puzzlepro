...
function c99189322.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c99189322.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c99189322.rfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
...
function c99189322.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not regc:IsRelateToEffect(e) then
    return
  end
  if Duel.Remove(regc, POS_FACEUP, REASON_EFFECT) == 0 or not regc:IsLocation(LOCATION_REMOVED) then
    return
  end
...
function c99189322.rec_effect(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:GetFlagEffect(99189322) == 0 then
    return
  end
