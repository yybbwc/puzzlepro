...
function c17688543.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsSpecialSummonableCard()
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(Card.IsSpecialSummonableCard, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
