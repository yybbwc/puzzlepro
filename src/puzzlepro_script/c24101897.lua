...
function c24101897.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(24101897) == 0
  end
...
function c24101897.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c24101897.filter, 1, nil)
  end
...
function c24101897.operation(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c24101897.sfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) then
    return
  end
