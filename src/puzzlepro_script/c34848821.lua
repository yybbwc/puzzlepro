...
function c34848821.thfilter(c, e, tp)
  if not (c:IsSetCard(0x14d) and c:IsType(TYPE_MONSTER) or c:IsCode(68468459)) then
    return false
  end
...
function c34848821.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c34848821.thfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
end
