...
function c49370016.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c49370016.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c49370016.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c49370016.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c49370016.drcon(e, tp, eg, ep, ev, re, r, rp)
  if not (tp == ep and re and re:IsActivated() and re:GetActivateLocation() == LOCATION_MZONE) then
    return false
  end
  local rc = re:GetHandler()
  return rc:IsRelateToEffect(re) and rc:IsRace(RACE_PSYCHO) or not rc:IsRelateToEffect(re) and rc:GetPreviousRaceOnField() & RACE_PSYCHO ~= 0
end
