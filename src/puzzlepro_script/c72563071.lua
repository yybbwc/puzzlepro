...
function c72563071.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72563071.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
  Duel.DiscardHand(tp, c72563071.cfilter, 1, 1, REASON_COST + REASON_DISCARD)
end
function c72563071.spfilter(c, e, tp)
  return c:IsLevel(6) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsRace(RACE_MACHINE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c72563071.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72563071.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
