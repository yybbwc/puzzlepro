...
function s.synfilter(c, e, tp, lv)
  return c:IsSetCard(0x35) and c:IsLevel(lv) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and c:IsType(TYPE_SYNCHRO) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function s.syntg(e, tp, eg, ep, ev, re, r, rp, chk)
  local g = Duel.GetMatchingGroup(s.dfilter, tp, LOCATION_HAND, 0, nil)
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_SMATERIAL) and g:CheckSubGroup(s.fselect, 2, 99, e, tp)
  end
...
function s.synop(e, tp, eg, ep, ev, re, r, rp)
  local g = Duel.GetMatchingGroup(s.dfilter, tp, LOCATION_HAND, 0, nil)
  if not aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_SMATERIAL) or not g:CheckSubGroup(s.fselect, 2, 99, e, tp) then
    return
  end
