...
function c68045685.costfilter(c)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x88) and c:IsAbleToGraveAsCost() and (c:IsFaceup() or c:IsLocation(LOCATION_HAND))
end
function c68045685.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c68045685.costfilter, tp, LOCATION_MZONE + LOCATION_HAND, 0, 1, nil)
  end
...
function c68045685.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and aux.NegateAnyFilter(chkc)
  end
  if chk == 0 then
...
function c68045685.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c68045685.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c68045685.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
