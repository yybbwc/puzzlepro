...
function c1948619.setcfilter(c, tp, ec)
  if c:IsLocation(LOCATION_MZONE) then
    ...
    return c:IsPreviousSetCard(0x8) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.extract(ec:GetLinkedZone(tp), c:GetPreviousSequence()) ~= 0
  end
end
...
function c1948619.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c1948619.setfilter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c1948619.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_BATTLE) or (rp == 1 - tp and c:IsReason(REASON_DESTROY) and c:IsPreviousControler(tp))
end
...
function c1948619.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c1948619.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
