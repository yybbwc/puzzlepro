...
function c95477924.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c95477924.tgfilter(chkc, e, tp, g)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c95477924.tgfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e, tp, g) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
