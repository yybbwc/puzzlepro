...
function c56099748.tfilter(c, sc, tp)
  return c:IsFaceup() and Duel.GetMZoneCount(tp, c) > 0 and not c:IsAttribute(sc:GetAttribute()) and not c:IsRace(sc:GetRace())
end
function c56099748.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c56099748.tfilter(chkc, c, tp)
  end
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c56099748.tfilter, tp, LOCATION_MZONE, 0, 1, nil, c, tp)
  end
