...
function c2102065.atktg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c2102065.atkfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c2102065.atkfilter1, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c2102065.atktg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c2102065.atkfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c2102065.atkfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
