...
function c88654892.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c88654892.filter1, 1, nil, e, tp)
  end
...
function c88654892.filter1(c, e, tp)
  local lv = c:GetLevel()
  return lv > 0 and (c:IsType(TYPE_FUSION) or c:IsType(TYPE_SYNCHRO)) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c88654892.filter2, tp, LOCATION_DECK, 0, 1, nil, lv, e, tp)
end
