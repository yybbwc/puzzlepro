...
function c53610653.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c53610653.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c53610653.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c53610653.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return e:GetHandler():IsReason(REASON_LOST_TARGET) and ec and ec:IsReason(REASON_DESTROY) and ec:IsLocation(LOCATION_GRAVE) and ec:GetReasonPlayer() == 1 - tp
end
function c53610653.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetPreviousEquipTarget()
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and ec:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
