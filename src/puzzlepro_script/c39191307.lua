...
function c39191307.filter(c, e, tp)
  return c:IsAttackBelow(1500) and c:IsRace(RACE_DRAGON) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c39191307.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c39191307.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
