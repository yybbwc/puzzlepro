...
function s.rmfilter(c, tp)
  ...
  if ct > 2 then
    return Duel.IsExistingMatchingCard(s.rmfdfilter, tp, 0, LOCATION_ONFIELD + LOCATION_GRAVE, 1, nil, c:GetCode(), tp)
  end
  return false
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and s.rmfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.rmfilter, tp, 0, LOCATION_ONFIELD, 1, nil, tp)
  end
