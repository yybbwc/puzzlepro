...
function c17888577.checkfilter(c, tp)
  ...
  return c:IsFaceup() and bit.band(c:GetType(), 0x81) == 0x81 and Duel.IsExistingMatchingCard(c17888577.tgfilter, tp, LOCATION_DECK, 0, 1, nil, att, race)
end
...
function c17888577.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c17888577.checkfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17888577.checkfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
