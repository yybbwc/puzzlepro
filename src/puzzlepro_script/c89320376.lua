...
function c89320376.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 89320376) == 0
  end
...
function c89320376.postg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and chkc:IsFacedown()
  end
  if chk == 0 then
...
function c89320376.posop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFacedown() then
    ...
    if not tc:IsPosition(POS_FACEUP_ATTACK) then
      pos1 = pos1 + POS_FACEUP_ATTACK
    end
    if not tc:IsPosition(POS_FACEUP_DEFENSE) then
      pos1 = pos1 + POS_FACEUP_DEFENSE
    end
...
function c89320376.poscost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return c89320376.cost(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.IsExistingMatchingCard(c89320376.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c89320376.postg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c89320376.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c89320376.posfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_POSCHANGE)
