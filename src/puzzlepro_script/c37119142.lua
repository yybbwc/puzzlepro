...
function c37119142.matval(e, lc, mg, c, tp)
  if not lc:IsSetCard(0x101) then
    return false, nil
  end
...
function c37119142.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c37119142.thfilter(chkc, e, tp, check)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37119142.thfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, check)
  end
...
function c37119142.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsAbleToHand() and (e:GetLabel() == 0 or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) or Duel.SelectOption(tp, 1190, 1152) == 0) then
