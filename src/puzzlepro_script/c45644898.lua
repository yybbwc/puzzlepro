...
function c45644898.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c45644898.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45644898.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c45644898.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c45644898.gvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45644898.gvfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c45644898.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
