...
function c47766694.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c47766694.filter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47766694.filter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, e:GetHandler())
  end
...
function c47766694.descon(e, tp, eg, ep, ev, re, r, rp)
  return bit.band(r, REASON_DESTROY) ~= 0 and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():IsPreviousPosition(POS_FACEDOWN)
end
...
function c47766694.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c47766694.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
