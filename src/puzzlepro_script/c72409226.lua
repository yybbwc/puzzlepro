...
function s.thorsptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.thorspfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:CheckSubGroup(s.thcheck, 1, 2)
  end
end
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local g = c:GetOverlayGroup()
  if g:CheckSubGroup(s.thcheck, 1) == false then
    return
  end
  ...
  if #tg > 0 and Duel.SendtoHand(tg, nil, REASON_EFFECT) ~= 0 and tg:IsExists(Card.IsLocation, 1, nil, LOCATION_HAND) then
    ...
    if Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_GRAVE, 0, 1, nil, TYPE_NORMAL) and Duel.IsExistingMatchingCard(Card.IsAbleToHand, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
      Duel.BreakEffect()
