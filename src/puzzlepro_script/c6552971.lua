...
function c6552971.chkfilter(c, tp)
  return c:IsRace(RACE_CYBERSE) and c:GetAttack() == 2300 and not c:IsPublic() and Duel.IsExistingMatchingCard(c6552971.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetAttribute())
end
...
function c6552971.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6552971.chkfilter, tp, LOCATION_HAND + LOCATION_EXTRA, 0, 1, nil, tp)
  end
