...
function c10547580.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c10547580.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c10547580.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsType, tp, LOCATION_ONFIELD, 0, 1, nil, TYPE_SPELL + TYPE_TRAP) and Duel.IsExistingTarget(c10547580.desfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c10547580.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsControler(tp) and Duel.Destroy(tc, REASON_EFFECT) ~= 0 and lc:IsRelateToEffect(e) and lc:IsControler(1 - tp) and lc:IsFaceup() then
    local e1 = Effect.CreateEffect(c)
