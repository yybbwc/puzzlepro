...
function c10793085.costfilter(c, tp)
  return c:IsAbleToGraveAsCost() and c:IsSetCard(0x14d) and Duel.IsExistingTarget(c10793085.tgfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, c:GetType())
end
function c10793085.tgfilter(c, type)
  if type == nil then
    return false
  end
  if not (c:IsFaceup() and c:IsType(TYPE_EFFECT)) then
    return false
  end
...
function c10793085.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c10793085.tgfilter(chkc, e:GetLabel())
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingMatchingCard(c10793085.costfilter, tp, LOCATION_EXTRA + LOCATION_DECK, 0, 1, nil, tp)
  end
