...
function c99937011.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99937011.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c99937011.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c99937011.stfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(99937011, 0)) then
    Duel.BreakEffect()
...
function c99937011.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local ct = 5
  if not Duel.IsExistingMatchingCard(c99937011.filter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil) then
    ct = 3
  end
