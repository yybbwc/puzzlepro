...
function c81945678.cpfilter(c)
  return c:IsType(TYPE_SPELL) and c:IsAbleToGraveAsCost() and c:CheckActivateEffect(false, true, false) ~= nil and aux.IsCodeListed(c, 25652259) and aux.IsCodeListed(c, 64788463) and aux.IsCodeListed(c, 90876561)
end
...
function c81945678.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c81945678.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81945678.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil) and e:GetHandler():IsAbleToHand()
  end
