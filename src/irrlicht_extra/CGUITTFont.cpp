  ...
  namespace gui {
    ...
    bool SGUITTGlyph::renderSpans(FT_Library &library, FT_Face &face, float outline) {
      ...
      if (face->glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
        ...
        FT_Stroker_Set(stroker, (int)(outline * 64.f), FT_STROKER_LINECAP_ROUND, FT_STROKER_LINEJOIN_ROUND, 0);
    ...
    bool CGUITTGlyphPage::reserveGlyphSpace(irr::core::recti &rect, u32 width, u32 height) {
      if (Texture) {
        ...
        if (LastRow.LowerRightCorner.X + width <= texSize.Width && LastRow.UpperLeftCorner.Y + height <= texSize.Height) {
        ...
        if (width <= texSize.Width && LastRow.LowerRightCorner.Y + height <= texSize.Height) {
    ...
    //! Constructor.
    CGUITTFont::CGUITTFont(irr::video::IVideoDriver *driver, irr::io::IFileSystem *fileSystem) : UseMonochrome(false), UseTransparency(true), UseHinting(false), UseAutoHinting(false), Outline(0.f), OutlineColor(255, 255, 255, 255), Size(0), MaxFontHeight(0), LineHeight(0), BatchLoadSize(1), Driver(driver), FileSystem(fileSystem), Logger(0), GlobalKerningWidth(0), GlobalKerningHeight(0) {
#ifdef _DEBUG
