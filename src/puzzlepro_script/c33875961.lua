...
function c33875961.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_GRAVE, 0, ct, nil)
  end
...
function c33875961.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c33875961.filter(chkc)
  end
  local ct = e:GetHandler():GetCounter(0x28)
  if chk == 0 then
    return Duel.IsExistingTarget(c33875961.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, ct, nil)
  end
