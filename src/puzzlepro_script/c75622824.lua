...
function c75622824.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c75622824.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c75622824.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and (ct == 1 or not Duel.IsPlayerAffectedByEffect(tp, 59822133)) and Duel.GetLocationCount(tp, LOCATION_MZONE) > ct - 2 and Duel.IsPlayerCanSpecialSummonMonster(tp, 75622825, 0, TYPES_TOKEN_MONSTER, 2000, 1200, 5, RACE_SPELLCASTER, ATTRIBUTE_WIND, POS_FACEUP_DEFENSE)
  end
...
function c75622824.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 75622825, 0, TYPES_TOKEN_MONSTER, 2000, 1200, 5, RACE_SPELLCASTER, ATTRIBUTE_WIND, POS_FACEUP_DEFENSE) then
    return
  end
  for i = 1, ct do
