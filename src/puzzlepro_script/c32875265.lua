...
function c32875265.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) < Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) and Duel.IsExistingMatchingCard(c32875265.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c32875265.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft > 0 and ct > 0 then
    ...
    if sg and Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.GetLP(tp) <= Duel.GetLP(1 - tp) - 2000 and Duel.IsExistingMatchingCard(c32875265.setfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(32875265, 0)) then
      Duel.BreakEffect()
