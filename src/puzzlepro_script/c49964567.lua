...
function c49964567.indtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c49964567.indfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c49964567.indfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c49964567.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c49964567.tgfilter, tp, LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil)
  end
