...
function c92422871.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c92422871.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c92422871.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    local tc = g:GetFirst()
    if Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 and c:IsFaceup() and c:IsRelateToChain() and c:GetLevel() > 0 and c:GetLevel() ~= tc:GetLevel() and Duel.SelectYesNo(tp, aux.Stringid(92422871, 2)) then
      Duel.BreakEffect()
      local e1 = Effect.CreateEffect(c)
...
function c92422871.negcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not Duel.IsExistingMatchingCard(c92422871.cfilter, tp, LOCATION_MZONE, 0, 1, nil) then
    return false
  end
