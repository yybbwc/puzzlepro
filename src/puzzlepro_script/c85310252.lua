...
function c85310252.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c85310252.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85310252.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  ...
  if tc:IsLevel(1) then
    op = Duel.SelectOption(tp, aux.Stringid(85310252, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(85310252, 1), aux.Stringid(85310252, 2))
  end
  e:SetLabel(op)
end
