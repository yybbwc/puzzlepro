...
function c71620241.filter(c)
  return c:IsSetCard(0x5034) and c:IsType(TYPE_MONSTER) and not c:IsForbidden() and (not c:IsLocation(LOCATION_REMOVED) or c:IsFaceup())
end
function c71620241.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsLocation(LOCATION_MZONE) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c71620241.filter, tp, LOCATION_DECK + LOCATION_GRAVE + LOCATION_HAND + LOCATION_REMOVED, 0, 1, nil)
  end
end
