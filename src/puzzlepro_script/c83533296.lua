...
function s.filter2(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsFaceupEx() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(s.filter1, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 2, c)
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(s.filter2, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
