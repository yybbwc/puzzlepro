...
function c15939229.sumsuc(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsSummonType(SUMMON_TYPE_XYZ) then
    return
  end
  local e1 = Effect.CreateEffect(c)
...
function c15939229.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c15939229.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c15939229.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c15939229.setfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c15939229.setfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
