...
function c90587641.spdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToBattle() and not Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_GRAVE, 0, 1, nil, TYPE_SPELL + TYPE_TRAP) and tc:IsControler(1 - tp) and tc:IsFaceup() and (tc:GetAttack() > 0 or aux.NegateMonsterFilter(tc)) then
    Duel.BreakEffect()
...
function c90587641.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP) > 0 and c:IsRelateToEffect(e) and (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1)) and Duel.SelectYesNo(tp, aux.Stringid(90587641, 2)) then
    Duel.BreakEffect()
