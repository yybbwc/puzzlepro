...
function c43256007.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c43256007.spfilter1, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c43256007.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c43256007.spfilter2(chkc)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingTarget(c43256007.spfilter2, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c43256007.spop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFacedown() or not tc:IsRelateToEffect(e) or tc:IsImmuneToEffect(e) or tc:GetLevel() < 3 then
    return
  end
  local c = e:GetHandler()
  local e1 = Effect.CreateEffect(c)
