...
function c25396150.filter(c, tp, pcon)
  return ((c:IsFaceup() and c:IsLocation(LOCATION_EXTRA) and c:IsType(TYPE_PENDULUM)) or c:IsLocation(LOCATION_GRAVE)) and c:IsSetCard(0x4) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or c:IsType(TYPE_PENDULUM) and pcon)
end
...
function c25396150.rctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c25396150.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
    return eg:IsExists(c25396150.tgfilter, 1, nil, e, tp)
  end
