...
function c68299524.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 100, true) and Duel.IsExistingMatchingCard(c68299524.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler())
  end
...
function c68299524.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c68299524.desfilter(chkc, c:GetAttack())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68299524.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, c:GetAttack())
  end
