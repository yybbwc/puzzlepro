...
function c58820923.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_DECK) > 2 and Duel.IsPlayerCanRemove(1 - tp)
  end
end
...
function c58820923.rmop(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsPlayerCanRemove(1 - tp) then
    return
  end
...
function c58820923.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetEffectCount(EFFECT_EXTRA_ATTACK) == 0 and e:GetHandler():GetEffectCount(EFFECT_EXTRA_ATTACK_MONSTER) == 0
  end
end
