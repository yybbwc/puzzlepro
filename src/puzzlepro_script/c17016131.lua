...
function c17016131.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rc:IsRelateToBattle() and rc:IsStatus(STATUS_OPPO_BATTLE) and rc:IsFaceup() and rc:IsSetCard(0x13f) and rc:IsControler(tp)
end
...
function c17016131.tgcond(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsStatus(STATUS_EFFECT_ENABLED) and Duel.IsExistingMatchingCard(c17016131.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c17016131.tgtg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToGrave, 1 - tp, LOCATION_EXTRA, 0, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
