...
function c84766279.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84766279.costfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c84766279.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c84766279.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c84766279.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
