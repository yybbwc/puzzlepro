...
function c51412776.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c51412776.filter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c51412776.checkfilter(c, tp)
  return c:IsSetCard(0x107a) and c:IsReason(REASON_BATTLE) and c:IsControler(tp) and c:GetEquipCount() > 0 and c:GetEquipGroup():IsExists(Card.IsSetCard, 1, nil, 0x207a)
end
function c51412776.checkop(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c51412776.checkfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c51412776.checkfilter, 1, nil, 1) then
    v = v + 2
  end
  if v > 0 then
