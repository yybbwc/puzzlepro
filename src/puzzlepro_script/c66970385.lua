...
function c66970385.filter(c, e, tp)
  return c:IsSetCard(0x107a) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c66970385.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp, c)
end
...
function c66970385.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c66970385.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
