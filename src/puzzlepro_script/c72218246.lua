...
function c72218246.spfilter1(c, e, tp, mg)
  return c:IsType(TYPE_SYNCHRO) and (c:IsSetCard(0x123) or c:IsRace(RACE_PLANT)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mg, c) > 0
end
function c72218246.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReleasable() and Duel.CheckReleaseGroup(tp, c72218246.rfilter, 1, c, e, tp, c)
  end
...
function c72218246.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72218246.spfilter2, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
