...
function c30539496.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:GetCount() >= 2 and g:IsExists(Card.IsAttribute, 1, nil, ATTRIBUTE_EARTH) and (ft > 0 or g:IsExists(c30539496.locfilter, -ft + 1, nil, tp))
  end
...
function c30539496.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local g = Duel.GetMatchingGroup(c30539496.desfilter, tp, loc, loc2, c)
  if g:GetCount() < 2 or not g:IsExists(Card.IsAttribute, 1, nil, ATTRIBUTE_EARTH) then
    return
  end
  local g1 = nil
  local g2 = nil
...
function c30539496.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c30539496.thfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
