...
function c77610772.repfilter(c, tp, hc)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and hc:GetLinkedGroup():IsContains(c)
end
function c77610772.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c77610772.repfilter, 1, nil, tp, e:GetHandler())
  end
...
function c77610772.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c77610772.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
