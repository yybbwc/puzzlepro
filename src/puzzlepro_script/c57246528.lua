...
function c57246528.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c57246528.opfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57246528.opfilter, tp, 0, LOCATION_MZONE, 1, nil, e, tp)
  end
