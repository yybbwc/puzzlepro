...
function c71880877.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c71880877.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c71880877.rmfilter(c, tp)
  return c:IsRace(RACE_MACHINE) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c71880877.thfilter, tp, LOCATION_GRAVE, 0, 1, c)
end
...
function c71880877.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c71880877.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c71880877.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c71880877.thfilter(chkc)
  end
  if chk == 0 then
    return true
  end
