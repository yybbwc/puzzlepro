...
function c56562619.cfilter1(c, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_MONSTER) and c:IsSetCard(0x2b) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c56562619.cfilter2, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, c, c)
end
function c56562619.cfilter2(c, cc)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsSetCard(0x61) and c:IsAbleToGraveAsCost() and Duel.IsExistingTarget(c56562619.rmfilter, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c, cc)
end
...
function c56562619.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56562619.cfilter1, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c56562619.spfilter(c, e, tp)
  return c:GetFlagEffect(56562619) ~= 0 and (c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp))
end
...
function c56562619.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if (ft1 <= 0 and ft2 <= 0) or rg:GetCount() <= 0 then
    return
  end
  local sg = nil
