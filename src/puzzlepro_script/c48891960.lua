...
function c48891960.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c48891960.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c48891960.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and chkc:IsAbleToRemove()
  end
  if chk == 0 then
...
function c48891960.spfilter(c, e, tp)
  return c:IsSetCard(0x29) and c:IsAttackBelow(3000) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
...
function c48891960.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_SMATERIAL) and Duel.IsExistingMatchingCard(c48891960.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
