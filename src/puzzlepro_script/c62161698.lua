...
function c62161698.checkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if p1 then
    Duel.RegisterFlagEffect(0, 62161698, RESET_PHASE + PHASE_END, 0, 1)
  end
  if p2 then
    Duel.RegisterFlagEffect(1, 62161698, RESET_PHASE + PHASE_END, 0, 1)
  end
end
...
function c62161698.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummon(tp) and Duel.GetFieldGroupCount(tp, LOCATION_DECK, 0) > 4
  end
end
...
function c62161698.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(62161698, 1)) then
