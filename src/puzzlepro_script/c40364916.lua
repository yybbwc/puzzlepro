...
function c40364916.limcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c40364916.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c40364916.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c40364916.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c40364916.rmfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function c40364916.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) ~= 0 and g:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(40364916, 2)) then
    Duel.BreakEffect()
