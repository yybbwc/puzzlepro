...
function c41830887.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c41830887.filter(chkc)
  end
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c41830887.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c41830887.spop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (c:IsRelateToEffect(e) and tc:IsRelateToEffect(e)) then
    return
  end
...
function c41830887.spfilter1(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x14b) and Duel.IsExistingMatchingCard(c41830887.spfilter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp, c:GetCode())
end
...
function c41830887.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c41830887.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c41830887.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
