...
function c28506708.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c28506708.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c28506708.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) > 0 or Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) == 0 then
    return
  end
