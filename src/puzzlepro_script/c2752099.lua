...
function c2752099.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c2752099.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c2752099.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
...
function c2752099.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsLocation(LOCATION_HAND) and tc:GetOriginalLevel() >= 7 and Duel.IsExistingMatchingCard(c2752099.tgfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(2752099, 2)) then
    Duel.BreakEffect()
