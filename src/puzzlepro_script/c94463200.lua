...
function c94463200.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c94463200.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c94463200.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c94463200.lpcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetCurrentPhase() == PHASE_STANDBY and ep == tp and re:GetHandler() == e:GetHandler():GetFirstCardTarget()
end
