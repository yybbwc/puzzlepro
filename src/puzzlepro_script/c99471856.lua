...
function s.spfilter(c, e, tp)
  return not c:IsCode(id) and c:IsSetCard(0xea) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_EXTRA, 0, 1, nil, c)
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function s.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetCount() > 0 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingMatchingCard(s.spfilter2, tp, LOCATION_DECK, 0, 2, nil, e, tp)
  end
...
function s.spop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    ...
    if Duel.Destroy(g, REASON_EFFECT) ~= 0 and Duel.GetMatchingGroupCount(s.spfilter2, tp, LOCATION_DECK, 0, nil, e, tp) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 then
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
