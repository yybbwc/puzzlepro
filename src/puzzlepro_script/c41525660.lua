...
function c41525660.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c41525660.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41525660.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler())
  end
...
function c41525660.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if seq > 0 and Duel.CheckLocation(p, LOCATION_MZONE, seq - 1) then
    flag = flag | (1 << (seq - 1))
  end
  if seq < 4 and Duel.CheckLocation(p, LOCATION_MZONE, seq + 1) then
    flag = flag | (1 << (seq + 1))
  end
  if flag == 0 then
    return
...
function c41525660.mvcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsLocation(LOCATION_MZONE) and (c:GetPreviousSequence() ~= c:GetSequence() or c:GetPreviousControler() ~= tp)
end
function c41525660.spfilter(c, e, tp)
  local zone = 1 << c:GetSequence()
  return c:IsSetCard(0x17d) and c:IsFaceup() and c:GetSequence() <= 4 and c:GetOriginalType() & TYPE_MONSTER ~= 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone)
end
function c41525660.mvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c41525660.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41525660.spfilter, tp, LOCATION_SZONE, 0, 1, nil, e, tp)
  end
