...
function c41114306.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c41114306.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler(), tp) > 0 and Duel.IsExistingTarget(c41114306.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  local ft = 2
