...
function s.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(s.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(s.eqfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, e:GetHandler(), tp)
  end
...
function s.thfilter(c)
  return c:IsFaceup() and c:IsType(TYPE_EQUIP) and c:IsAbleToHand() and c:GetOriginalType() & TYPE_MONSTER == TYPE_MONSTER and c:GetOriginalRace() & RACE_SPELLCASTER == RACE_SPELLCASTER
end
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_SZONE) and s.thfilter(chkc)
  end
  if chk == 0 then
