...
function s.cfilter(c)
  return c:IsFaceup() and bit.band(c:GetOriginalType(), TYPE_MONSTER) == TYPE_MONSTER and c:IsSetCard(0x1a2) and c:IsAbleToRemoveAsCost()
end
...
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and chkc:IsAbleToRemove()
  end
  if chk == 0 then
...
function s.setcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN) and re:IsActivated()
end
function s.filter(c)
...
function s.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetClassCount(Card.GetCode) >= 2 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 1 and e:GetHandler():GetFlagEffect(id) == 0
  end
