...
function c64389297.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c64389297.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c64389297.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c64389297.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsReleasable() and tc:IsReleasable() and (tc:IsControler(tp) or tc:IsHasEffect(EFFECT_EXTRA_RELEASE))
  end
...
function c64389297.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c64389297.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
