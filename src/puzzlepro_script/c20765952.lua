...
function c20765952.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c20765952.filter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c20765952.filter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, c)
  end
