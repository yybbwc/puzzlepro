...
function c4008212.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c4008212.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4008212.desfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c4008212.thfilter(c)
  return c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsAbleToHand() and not c:IsCode(4008212) and (aux.IsCodeListed(c, 76263644) or aux.IsSetNameMonsterListed(c, 0xc008))
end
function c4008212.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4008212.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
