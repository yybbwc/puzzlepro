...
function c37781520.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c37781520.cfilter, tp, LOCATION_HAND, 0, 3, e:GetHandler())
  end
...
function c37781520.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c37781520.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37781520.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c37781520.hdcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c37781520.costfilter, 1, e:GetHandler())
  end
