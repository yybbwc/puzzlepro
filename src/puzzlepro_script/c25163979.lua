...
function c25163979.mvfilter2(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x112) and c:GetSequence() < 5 and Duel.IsExistingMatchingCard(c25163979.mvfilter3, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c25163979.mvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local b1 = Duel.IsExistingMatchingCard(c25163979.mvfilter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0) > 0
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(25163979, 1), aux.Stringid(25163979, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(25163979, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(25163979, 2)) + 1
  end
  e:SetLabel(op)
end
