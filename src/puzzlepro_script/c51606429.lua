...
function c51606429.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 51606429, 0x10db, TYPES_NORMAL_TRAP_MONSTER, 0, 0, 3, RACE_WARRIOR, ATTRIBUTE_DARK, POS_FACEUP_ATTACK)
  end
...
function c51606429.negtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c51606429.negfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51606429.negfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
