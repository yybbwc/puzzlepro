...
function c89974904.cfilter(c, tc)
  return c:IsRace(RACE_DRAGON + RACE_FIEND) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsSynchroSummonable(tc)
end
function c89974904.spfilter(c, e, tp)
  return c:IsCanBeSpecialSummoned(e, SUMMON_VALUE_SYNCHRO_MATERIAL, tp, false, false) and Duel.IsExistingMatchingCard(c89974904.cfilter, tp, LOCATION_EXTRA, 0, 1, nil, c)
end
function c89974904.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c89974904.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c89974904.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
