...
function c74586817.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_HAND, 1, nil)
  end
...
function c74586817.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() == 0 or not c:IsRelateToEffect(e) or c:IsFacedown() then
    return
  end
...
function c74586817.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil)
  end
...
function c74586817.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsAbleToDeck() and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return e:GetHandler():IsAbleToExtra() and Duel.IsExistingTarget(Card.IsAbleToDeck, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, e:GetHandler())
  end
