...
function c52575195.tgcon1(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  e:SetLabelObject(tc)
  return tc:IsControler(tp) and tc:IsLocation(LOCATION_MZONE) and tc:IsFaceup() and tc:IsAttribute(ATTRIBUTE_LIGHT) and tc:IsRace(RACE_BEASTWARRIOR)
end
...
function c52575195.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc:IsAbleToRemove() and Duel.GetLocationCount(tp, LOCATION_MZONE) >= 0 and not e:GetHandler():IsStatus(STATUS_CHAINING) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
