...
function c70465810.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c70465810.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c70465810.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c70465810.rfilter, 1, nil, tp) and Duel.GetCustomActivityCount(70465810, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c70465810.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonMonster(tp, 70465811, 0, TYPES_TOKEN_MONSTER, 700, 700, 2, RACE_WINDBEAST, ATTRIBUTE_DARK)
  end
