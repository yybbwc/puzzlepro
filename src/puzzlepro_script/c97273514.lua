...
function c97273514.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c97273514.filter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97273514.filter, tp, LOCATION_MZONE, 0, 1, c) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c97273514.eqfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
...
function c97273514.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c97273514.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c97273514.tdfilter, tp, LOCATION_GRAVE, 0, 3, nil)
  end
