...
function c70473293.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c70473293.costfilter, 1, nil, tp)
  end
...
function c70473293.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c70473293.spfilter(chkc, e, tp) and chkc ~= e:GetLabelObject()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70473293.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
