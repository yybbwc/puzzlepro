...
function c73866096.cfilter(c)
  return c:IsSetCard(0x82) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
end
function c73866096.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73866096.cfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
