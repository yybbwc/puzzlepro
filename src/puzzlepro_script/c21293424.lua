...
function c21293424.valcheck(e, tp, eg, ep, ev, re, r, rp)
  ...
  local flag = 0
  if c:GetMaterial():FilterCount(Card.IsCode, nil, 24639891) > 0 then
    flag = flag | 1
  end
  if c:GetMaterial():FilterCount(Card.IsCode, nil, 78362751) > 0 then
    flag = flag | 2
  end
...
function c21293424.efftg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return chk1 and Duel.IsPlayerCanDraw(tp, 1) or chk2 and Duel.IsExistingMatchingCard(c21293424.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
  e:SetCategory(0)
  if chk1 then
