...
function c56161953.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56161953.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56161953.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c56161953.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c56161953.tdfilter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil)
  end
