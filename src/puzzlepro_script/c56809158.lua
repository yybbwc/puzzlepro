...
function c56809158.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56809158.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56809158.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  cg:AddCard(c)
  if chkc then
    return c56809158.filter(chkc, cg) and chkc:IsLocation(LOCATION_MZONE)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c56809158.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, cg)
  end
