...
function c35998832.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(1 - tp, 1) and Duel.IsExistingMatchingCard(c35998832.drfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c35998832.drop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.Draw(1 - tp, 1, REASON_EFFECT) ~= 0 then
    ...
    if g:GetCount() > 0 then
      ...
      if tg1:GetCount() < 1 or tg2:GetCount() < 1 then
        return
      end
      Duel.BreakEffect()
