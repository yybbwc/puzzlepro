...
function c15177750.reccost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c15177750.costfilter, tp, LOCATION_MZONE + LOCATION_HAND, 0, 1, nil)
  end
...
function c15177750.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c15177750.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c15177750.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c15177750.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c15177750.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
