...
function c43973174.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c43973174.cfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingMatchingCard(c43973174.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c43973174.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(c43973174.cfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) then
    return
  end
