...
function c74094021.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c74094021.spfilter, c:GetControler(), LOCATION_ONFIELD, 0, 1, nil)
end
...
function c74094021.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c74094021.costfilter, 1, e:GetHandler(), tp)
  end
