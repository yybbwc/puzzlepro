...
function c67586735.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rc:IsRelateToEffect(e) and rc:IsFaceup()
  end
...
function c67586735.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c67586735.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67586735.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
