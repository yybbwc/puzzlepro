...
function c72083436.datg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c72083436.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c72083436.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c72083436.rdcon(e)
  ...
  return Duel.GetAttackTarget() == nil and c:GetEffectCount(EFFECT_DIRECT_ATTACK) < 2 and Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and c:GetFlagEffect(72083436) > 0
end
