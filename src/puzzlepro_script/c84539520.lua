...
function c84539520.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84539520.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c84539520.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84539520.costfilter, tp, LOCATION_ONFIELD + LOCATION_HAND, 0, 1, c, ft)
  end
  local g = nil
...
function c84539520.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c84539520.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
