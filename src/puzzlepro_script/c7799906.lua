...
function c7799906.cfilter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousAttackOnField() > c:GetBaseAttack()
end
...
function c7799906.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7799906.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c7799906.drcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:GetAttack() > c:GetBaseAttack() and not Duel.IsExistingMatchingCard(c7799906.exfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
