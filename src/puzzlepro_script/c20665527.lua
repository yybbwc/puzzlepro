...
function c20665527.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c20665527.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c20665527.spfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function c20665527.cfilter(c, se)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsType(TYPE_LINK) and c:IsLinkBelow(3) and c:IsPreviousLocation(LOCATION_MZONE) and (se == nil or c:GetReasonEffect() ~= se)
end
