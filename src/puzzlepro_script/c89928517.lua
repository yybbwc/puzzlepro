...
function c89928517.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMatchingGroupCount(c89928517.filter, tp, LOCATION_HAND, 0, nil) * Duel.GetMatchingGroupCount(c89928517.filter, tp, 0, LOCATION_HAND, nil) > 0
  end
end
function c89928517.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsType(TYPE_MONSTER) and tc2:IsType(TYPE_MONSTER) then
    ...
    while i <= 1 do
      ...
      if Duel.GetLocationCount(p, LOCATION_MZONE, p) > 0 and tc:IsCanBeSpecialSummoned(e, 0, p, false, false) and Duel.SelectYesNo(p, aux.Stringid(89928517, 1)) then
  ...
  elseif tc1:IsType(TYPE_TRAP) and tc2:IsType(TYPE_TRAP) and Duel.IsExistingMatchingCard(Card.IsAbleToGrave, tp, LOCATION_DECK, 0, 2, nil) and Duel.IsExistingMatchingCard(Card.IsAbleToGrave, tp, 0, LOCATION_DECK, 2, nil) then
    for p = 0, 1 do
