...
function c20403123.hspfilter(c, tp)
  return c:IsSetCard(0x9f) and not c:IsType(TYPE_PENDULUM) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c20403123.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c20403123.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c20403123.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
