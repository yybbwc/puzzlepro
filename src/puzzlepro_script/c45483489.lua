...
function c45483489.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_HAND, 1, nil, 1 - tp)
  end
...
function c45483489.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c45483489.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45483489.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
