...
function s.gcheck(g, tp)
  return aux.gfcheck(g, Card.IsType, TYPE_MONSTER, TYPE_SPELL + TYPE_TRAP) and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, g)
end
...
function s.eqfilter(c, tp)
  return c:IsSetCard(0x129) and c:IsType(TYPE_EQUIP) and c:CheckUniqueOnField(tp) and not c:IsForbidden() and Duel.IsExistingMatchingCard(s.eqtgfilter, tp, LOCATION_MZONE, 0, 1, nil, c)
end
...
function s.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(s.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
