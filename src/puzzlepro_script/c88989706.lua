...
function c88989706.discon(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():GetFlagEffect(88989707) == 0 then
    return false
  end
...
function c88989706.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c88989706.filter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
