...
function s.tgfilter(c, e, tp)
  return c:IsCanBeEffectTarget(e) and c:IsRace(RACE_DRAGON + RACE_SPELLCASTER) and (c:IsAbleToDeck() or c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
...
function s.fselect(g, e, tp)
  return g:IsExists(s.spfilter, 1, nil, g, e, tp) and g:IsExists(s.racefilter, 1, nil, g)
end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g == 0 or Duel.GetLocationCount(tp, LOCATION_MZONE) == 0 then
    return
  end
