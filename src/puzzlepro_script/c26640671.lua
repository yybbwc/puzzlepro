...
function c26640671.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c26640671.filter(chkc, e:GetHandler())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c26640671.filter, tp, LOCATION_MZONE, 0, 1, nil, e:GetHandler())
  end
