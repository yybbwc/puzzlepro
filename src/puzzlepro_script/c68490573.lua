...
function c68490573.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c68490573.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c68490573.discon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c68490573.disfilter, tp, LOCATION_MZONE, 0, 1, nil) and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainDisablable(ev)
end
