...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil) and c:IsAbleToDeck()
  end
...
function s.relfilter(c, tp, chk)
  return c:IsReleasableByEffect() and c:IsAttackPos() and (not chk or Duel.GetMZoneCount(tp, c) > 0)
end
function s.spfilter(c, e, tp)
  return c:IsCode(30243636) and c:IsAttack(2000) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false)
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.relfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp, true) and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Release(g, REASON_EFFECT) == 0 or Duel.GetLocationCount(tp, LOCATION_MZONE) == 0 then
    return
  end
