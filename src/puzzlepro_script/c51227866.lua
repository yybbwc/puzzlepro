...
function c51227866.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local spchk = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_GRAVE, 0, nil, TYPE_SPELL) >= 3
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and c51227866.filter(chkc, e, tp, spchk)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51227866.filter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp, spchk)
  end
...
function c51227866.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) then
    ...
    if Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_GRAVE, 0, nil, TYPE_SPELL) >= 3 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not tc:IsAbleToRemove() or Duel.SelectYesNo(tp, aux.Stringid(51227866, 0))) then
