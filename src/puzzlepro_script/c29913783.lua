...
function c29913783.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29913783.rmfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c29913783.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c29913783.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(29913783, 2)) then
    Duel.BreakEffect()
