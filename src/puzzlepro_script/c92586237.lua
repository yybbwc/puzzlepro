...
function c92586237.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c92586237.spcfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c92586237.thcfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x88) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c92586237.thfilter, tp, LOCATION_GRAVE, 0, 1, nil, c:GetCode())
end
...
function c92586237.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c92586237.thcfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c92586237.tgcfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x88) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c92586237.tgfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c92586237.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c92586237.tgcfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
