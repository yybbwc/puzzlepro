...
function c50485594.spfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:IsLevelBelow(5) and Duel.IsExistingMatchingCard(c50485594.spfilter2, tp, LOCATION_DECK, 0, 2, nil, c:GetCode(), e, tp)
end
...
function c50485594.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c50485594.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
