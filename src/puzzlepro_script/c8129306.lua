...
function c8129306.regcon(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c8129306.spfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c8129306.spfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c8129306.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c8129306.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8129306.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
