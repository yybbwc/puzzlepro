...
function c69711728.cfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:IsSummonType(SUMMON_TYPE_FUSION) and Duel.IsExistingTarget(c69711728.filter, tp, LOCATION_GRAVE, 0, 1, nil, c:GetLevel(), e, tp)
end
function c69711728.filter(c, lv, e, tp)
  return c:GetLevel() > 0 and c:GetLevel() < lv and c:IsSetCard(0xe1) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c69711728.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c69711728.filter(chkc, eg:GetFirst():GetLevel(), e, tp)
  end
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(69711728) == 0 and eg:IsExists(c69711728.cfilter, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
...
function c69711728.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69711728.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
