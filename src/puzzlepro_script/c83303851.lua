...
function c83303851.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c83303851.cfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c83303851.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local ct = -ft + 1
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c83303851.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83303851.filter, tp, LOCATION_ONFIELD, 0, 2, nil) and ct <= 2 and (ct <= 0 or Duel.IsExistingTarget(c83303851.filter, tp, LOCATION_MZONE, 0, ct, nil)) and not c:IsStatus(STATUS_CHAINING) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
