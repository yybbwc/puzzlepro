...
function c41735184.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41735184.cfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c41735184.filter(c)
  return (aux.IsCodeListed(c, 46986414) or aux.IsCodeListed(c, 38033121)) and c:IsType(TYPE_SPELL + TYPE_TRAP) and not c:IsCode(41735184) and c:IsAbleToHand()
end
function c41735184.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41735184.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
