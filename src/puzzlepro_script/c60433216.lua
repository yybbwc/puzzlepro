...
function c60433216.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 60433216, 0, TYPES_NORMAL_TRAP_MONSTER + TYPE_TUNER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_FIRE)
  end
...
function c60433216.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 60433216, 0, TYPES_NORMAL_TRAP_MONSTER + TYPE_TUNER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_FIRE) then
    return
  end
