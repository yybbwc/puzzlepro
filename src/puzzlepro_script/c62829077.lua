...
function c62829077.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and mg:IsExists(c62829077.mfilter1, 1, nil, mg, exg)
  end
...
function c62829077.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  Duel.AdjustAll()
  if g:FilterCount(Card.IsLocation, nil, LOCATION_MZONE) < 3 then
    return
  end
