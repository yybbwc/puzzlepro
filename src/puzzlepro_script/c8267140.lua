...
function c8267140.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c8267140.filter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8267140.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
