...
function c90246973.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c90246973.rmfilter(chkc, ec:GetLevel())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c90246973.rmfilter, tp, 0, LOCATION_GRAVE, 1, nil, ec:GetLevel())
  end
