...
function c34079868.spfilter1(c, e, tp)
  return c:IsSetCard(0xc7) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c34079868.spfilter2, tp, LOCATION_DECK, 0, 1, c, e, tp)
end
...
function c34079868.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingMatchingCard(c34079868.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
