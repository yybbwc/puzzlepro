...
function c39711336.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(39711336) == 0
  end
...
function c39711336.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c39711336.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c39711336.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c39711336.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tg or not tg:IsContains(e:GetHandler()) then
    return false
  end
