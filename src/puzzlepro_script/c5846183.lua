...
function c5846183.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0x14b) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c5846183.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c5846183.repfilter, 1, nil, tp) and not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED)
  end
