...
function s.thfilter(c)
  return c:IsSetCard(0x1c0) and c:IsAbleToHand() and not c:IsType(TYPE_PENDULUM)
end
...
function s.rfilter(c, tp, ec)
  return c:IsSetCard(0x1c0) and c:IsReleasableByEffect() and c:IsLevel(10) and c:IsType(TYPE_MONSTER) and Duel.GetLocationCountFromEx(tp, tp, c, ec) > 0
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  local g = Duel.GetMatchingGroup(s.rfilter, tp, LOCATION_MZONE, 0, c, tp, c)
  if chk == 0 then
    return #g > 0 and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_RITUAL, tp, true, true)
  end
...
function s.pfilter(c, tp)
  return bit.band(c:GetType(), TYPE_SPELL + TYPE_CONTINUOUS) == TYPE_SPELL + TYPE_CONTINUOUS and c:IsSetCard(0x1c0) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Release(g, REASON_EFFECT) ~= 0 and c:IsRelateToEffect(e) and Duel.SpecialSummon(c, SUMMON_TYPE_RITUAL, tp, tp, true, true, POS_FACEUP) ~= 0 then
    c:CompleteProcedure()
    if Duel.IsExistingMatchingCard(s.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil, tp) and Duel.IsExistingMatchingCard(s.pfilter, tp, LOCATION_DECK, 0, 1, nil, tp) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
      Duel.BreakEffect()
