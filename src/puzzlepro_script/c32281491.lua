...
function c32281491.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 1 and Duel.IsExistingMatchingCard(c32281491.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c32281491.eqfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c32281491.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummonStep(tc, 0, tp, tp, false, false, POS_FACEUP) then
    ...
    if Duel.GetLocationCount(tp, LOCATION_SZONE) < 2 or not c:IsRelateToEffect(e) or c:IsFacedown() then
      return
    end
...
function c32281491.atkcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:GetFlagEffect(32281491) ~= 0 and Duel.GetAttacker() == c:GetEquipTarget()
end
