...
function c66594927.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c66594927.filter(chkc, atk)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c66594927.filter, tp, LOCATION_MZONE, 0, 1, nil, atk)
  end
...
function c66594927.repfilter(c, tp)
  return c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsFaceup() and c:IsSetCard(0x13) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c66594927.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c66594927.repfilter, 1, nil, tp)
  end
