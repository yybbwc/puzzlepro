...
function c86547356.costfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevelAbove(1) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsFaceup() or c:IsControler(tp)) and Duel.IsExistingMatchingCard(c86547356.spfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetCode(), c:GetLevel())
end
...
function c86547356.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c86547356.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86547356.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
