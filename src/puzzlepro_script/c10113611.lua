...
function s.spfilter(c, e, tp, g)
  return c:IsSetCard(0xe) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:CheckSubGroup(s.gcheck, 1, #g, tp, e:GetHandler(), c)
end
...
function s.sstg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsReleasableByEffect() and c:GetLevel() > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, g)
  end
...
function s.ssop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local g = Duel.GetReleaseGroup(tp, true, REASON_EFFECT):Filter(s.mfilter, c)
  if not (c:IsRelateToEffect(e) and c:IsReleasableByEffect()) or #g == 0 then
    return
  end
