...
function c3580032.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.GetCustomActivityCount(3580032, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c3580032.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c3580032.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c3580032.sctg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c3580032.scfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c3580032.xyztg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c3580032.xyzfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_EXTRA)
end
