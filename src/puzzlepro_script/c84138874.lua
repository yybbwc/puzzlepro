...
function c84138874.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84138874.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
  local g = Duel.SelectMatchingCard(tp, c84138874.cfilter, tp, LOCATION_GRAVE, 0, 1, 1, nil)
  Duel.Remove(g, POS_FACEUP, REASON_COST)
  if g:GetFirst():IsSetCard(0x32) then
    e:SetLabel(1)
  else
    e:SetLabel(0)
  end
...
function c84138874.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Damage(p, d, REASON_EFFECT) > 0 and e:GetLabel() > 0 and Duel.SelectYesNo(tp, aux.Stringid(84138874, 2)) then
    Duel.BreakEffect()
...
function c84138874.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c84138874.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c84138874.tdfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
