...
function c51225407.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c51225407.drfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c51225407.cfilter(c, e, tp)
  return c:IsRace(RACE_ZOMBIE) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c51225407.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
...
function c51225407.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c51225407.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c51225407.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c51225407.spfilter(chkc, e, tp)
  end
  if chk == 0 then
