...
function c98881700.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c98881700.spfilter(chkc)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingTarget(c98881700.spfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
