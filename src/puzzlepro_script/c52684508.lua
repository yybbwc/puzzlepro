...
function c52684508.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(52684508, tp, ACTIVITY_ATTACK) == 0
  end
...
function c52684508.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c52684508.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c52684508.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
