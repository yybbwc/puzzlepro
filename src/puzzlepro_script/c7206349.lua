...
function c7206349.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7206349.costfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c7206349.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c7206349.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c7206349.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_MZONE) and c7206349.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c7206349.thfilter, tp, LOCATION_GRAVE + LOCATION_MZONE, LOCATION_GRAVE + LOCATION_MZONE, 1, nil)
  end
