...
function c30587695.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30587695.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil, tp)
  end
...
function c30587695.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c30587695.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30587695.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
