...
function c87074380.cfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_MACHINE) and Duel.IsExistingMatchingCard(c87074380.desfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttack())
end
...
function c87074380.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c87074380.cfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c87074380.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c87074380.sfilter(c, tp)
  return not c:IsCode(87074380) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousAttributeOnField(), ATTRIBUTE_EARTH) ~= 0 and bit.band(c:GetPreviousRaceOnField(), RACE_MACHINE) ~= 0 and c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE)
end
