...
function c14536035.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c14536035.spfilter, c:GetControler(), LOCATION_HAND, 0, 1, c)
end
...
function c14536035.sgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c14536035.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c14536035.sgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c14536035.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
