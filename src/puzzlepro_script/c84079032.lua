...
function s.filter(c, e, tp)
  return c:IsCanChangePosition() and c:IsCanBeEffectTarget(e) and c:IsLocation(LOCATION_MZONE) and c:IsSummonPlayer(1 - tp) and (not c:IsPosition(POS_FACEUP_ATTACK) or c:IsCanTurnSet())
end
function s.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and s.filter(chkc, e, tp)
  end
  if chk == 0 then
    return eg:IsExists(s.filter, 1, nil, e, tp) and not eg:IsContains(e:GetHandler(), tp)
  end
...
function s.cfilter(c)
  return c:IsPreviousPosition(POS_FACEDOWN) and c:IsFaceup() and (not c:IsPosition(POS_FACEUP_ATTACK) or c:IsCanTurnSet()) or c:IsPreviousPosition(POS_FACEUP) and c:IsFacedown()
end
...
function s.postg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and s.cfilter(chkc) and eg:IsContains(chkc)
  end
  if chk == 0 then
    return eg:Filter(Card.IsOnField, nil):IsExists(s.cfilter, 1, nil, e) and not eg:IsContains(e:GetHandler(), tp)
  end
