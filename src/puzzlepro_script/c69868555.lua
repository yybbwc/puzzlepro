...
function c69868555.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(69868555) == 0
  end
...
function c69868555.tkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c69868555.tkcostfilter, tp, loc, 0, 1, e:GetHandler())
  end
...
function c69868555.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonMonster(tp, 69868556, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_DRAGON, ATTRIBUTE_LIGHT)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOKEN, nil, 1, 0, 0)
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, 0, 0)
end
function c69868555.tkop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not e:GetHandler():IsRelateToEffect(e) then
    return
  end
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 69868556, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_DRAGON, ATTRIBUTE_LIGHT) then
    return
  end
...
function c69868555.thcostfilter(c)
  return c:IsRace(RACE_DRAGON) and c:IsLevel(7, 8) and c:IsAbleToGraveAsCost() and (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
end
function c69868555.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69868555.thcostfilter, tp, LOCATION_MZONE + LOCATION_HAND, 0, 1, e:GetHandler())
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
