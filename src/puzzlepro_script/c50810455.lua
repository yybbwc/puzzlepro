...
function c50810455.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c50810455.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c50810455.spfilter(c, e, tp)
  return c:IsType(TYPE_LINK) and c:IsRace(RACE_BEAST + RACE_BEASTWARRIOR + RACE_WINDBEAST) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
