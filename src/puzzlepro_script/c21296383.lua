...
function c21296383.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return sg:IsContains(chkc) and chkc:IsLevelBelow(e:GetLabel())
  end
  ...
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c21296383.cfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), lv)
  end
