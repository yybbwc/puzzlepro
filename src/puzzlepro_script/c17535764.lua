...
function c17535764.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c17535764.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c17535764.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if tc:IsType(TYPE_NORMAL) and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(17535764, 1)) then
      ...
      if Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and g:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(17535764, 2)) then
        Duel.BreakEffect()
