...
function c80033124.ffilter(c, e)
  return c:IsFusionCode(41230939, 77625948, 3019642) and c:IsCanBeFusionMaterial() and c:IsAbleToDeck() and not c:IsImmuneToEffect(e)
end
function c80033124.spfilter(c, e, tp, sg)
  return c:IsCode(40418351) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and (not sg or Duel.GetLocationCountFromEx(tp, tp, sg, c) > 0)
end
...
function c80033124.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    if not Duel.IsExistingMatchingCard(c80033124.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, nil) then
      return false
    end
