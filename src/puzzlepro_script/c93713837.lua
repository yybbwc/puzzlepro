...
function c93713837.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c93713837.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93713837.posfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c93713837.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:GetReasonPlayer() == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEUP)
end
