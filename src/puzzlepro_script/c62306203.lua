...
function c62306203.cfilter(c, tp)
  return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and bit.band(c:GetPreviousTypeOnField(), TYPE_LINK) ~= 0 and bit.band(c:GetPreviousRaceOnField(), RACE_CYBERSE) ~= 0
end
...
function c62306203.spcost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62306203.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c62306203.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c62306203.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c62306203.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
