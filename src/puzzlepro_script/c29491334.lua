...
function c29491334.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 29491335, 0, TYPES_TOKEN_MONSTER, 2000, 2400, 6, RACE_MACHINE, ATTRIBUTE_DARK)
  end
...
function c29491334.tkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 29491335, 0, TYPES_TOKEN_MONSTER, 2000, 2400, 6, RACE_MACHINE, ATTRIBUTE_DARK) then
    return
  end
...
function c29491334.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c29491334.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
