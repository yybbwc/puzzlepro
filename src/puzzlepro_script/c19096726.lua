...
function c19096726.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.IsExistingMatchingCard(c19096726.disfilter, tp, LOCATION_MZONE, 0, 1, nil) and rp == 1 - tp and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainDisablable(ev) and (c:IsLocation(LOCATION_MZONE) and not c:IsStatus(STATUS_BATTLE_DESTROYED) or c:IsLocation(LOCATION_HAND))
end
...
function c19096726.thfilter(c)
  return (c:IsCode(68468459) or aux.IsCodeListed(c, 68468459) and c:IsType(TYPE_MONSTER) and not c:IsCode(19096726)) and c:IsAbleToHand()
end
function c19096726.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19096726.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
