...
field::field(duel *pd) : pduel(pd) {
  for (int32_t i = 0; i < 2; ++i) {
    player[i].list_mzone.resize(7, nullptr);
    player[i].list_szone.resize(8, nullptr);
...
void field::move_card(uint8_t playerid, card *pcard, uint8_t location, uint8_t sequence, uint8_t pzone) {
  ...
  if (pcard->current.location) {
    if (pcard->current.location == location && pcard->current.pzone == !!pzone) {
      ...
      if ((pcard->data.type & TYPE_PENDULUM) && (location == LOCATION_GRAVE) && pcard->is_capable_send_to_extra(playerid) && (((pcard->current.location == LOCATION_MZONE) && !pcard->is_status(STATUS_SUMMON_DISABLED)) || ((pcard->current.location == LOCATION_SZONE) && !pcard->is_status(STATUS_ACTIVATE_DISABLED)))) {
        location = LOCATION_EXTRA;
...
void field::swap_card(card *pcard1, card *pcard2, uint8_t new_sequence1, uint8_t new_sequence2) {
  ...
  if (new_sequence1 != s1 && !is_location_useable(p1, l1, new_sequence1) || new_sequence2 != s2 && !is_location_useable(p2, l2, new_sequence2)) {
    return;
  }
...
int32_t field::get_spsummonable_count_fromex_rule4(card *pcard, uint8_t playerid, uint8_t uplayer, uint32_t zone, uint32_t *list) {
  ...
  if (player[playerid].list_mzone[5] && is_location_useable(playerid, LOCATION_MZONE, 6) && check_extra_link(playerid, pcard, 6)) {
    flag |= 1u << 5;
  }
  else if (player[playerid].list_mzone[6] && is_location_useable(playerid, LOCATION_MZONE, 5) && check_extra_link(playerid, pcard, 5)) {
    flag |= 1u << 6;
  }
...
uint32_t field::get_rule_zone_fromex(int32_t playerid, card *pcard) {
  if (core.duel_rule >= NEW_MASTER_RULE) {
    if (core.duel_rule >= MASTER_RULE_2020 && pcard && (pcard->data.type & (TYPE_FUSION | TYPE_SYNCHRO | TYPE_XYZ)) && (pcard->is_position(POS_FACEDOWN) || !(pcard->data.type & TYPE_PENDULUM))) {
      return 0x7f;
    }
    else {
...
void field::shuffle(uint8_t playerid, uint8_t location) {
  ...
  if (location == LOCATION_HAND || !(core.duel_options & DUEL_PSEUDO_SHUFFLE)) {
    auto s = (int32_t)svector.size();
    if (location == LOCATION_EXTRA) {
      s = s - (int32_t)player[playerid].extra_p_count;
    }
    if (s > 1) {
      pduel->random_xoshiro256pp_1.shuffle_vector(svector);
...
void field::reverse_deck(uint8_t playerid) {
  auto count = (int32_t)player[playerid].list_main.size();
  if (count == 0) {
    return;
  }
...
void field::tag_swap(uint8_t playerid) {
  ...
  pduel->write_buffer8((uint8_t)player[playerid].extra_p_count);
  pduel->write_buffer8((uint8_t)player[playerid].list_hand.size());
  if (core.deck_reversed && !player[playerid].list_main.empty()) {
  ...
  for (uint32_t p = 0; p < 2; ++p) {
    if (location & LOCATION_MZONE) {
      for (auto &pcard : player[self].list_mzone) {
        if (pcard && !pcard->is_treated_as_not_on_field() && pcard != pexception && !(pexgroup && pexgroup->has_card(pcard)) && pduel->lua->check_filter(L, pcard, findex, extraargs) && (!is_target || pcard->is_capable_be_effect_target(core.reason_effect, core.reason_player))) {
          if (pret) {
            *pret = pcard;
            return TRUE;
          }
    ...
    if (location & LOCATION_PZONE) {
      for (int32_t i = 0; i < 2; ++i) {
        ...
        if (pcard && pcard->current.pzone && !pcard->is_treated_as_not_on_field() && pcard != pexception && !(pexgroup && pexgroup->has_card(pcard)) && pduel->lua->check_filter(L, pcard, findex, extraargs) && (!is_target || pcard->is_capable_be_effect_target(core.reason_effect, core.reason_player))) {
          if (pret) {
            *pret = pcard;
            return TRUE;
          }
    ...
    if (location & LOCATION_DECK) {
      for (auto cit = player[self].list_main.rbegin(); cit != player[self].list_main.rend(); ++cit) {
        if (*cit != pexception && !(pexgroup && pexgroup->has_card(*cit)) && pduel->lua->check_filter(L, *cit, findex, extraargs) && (!is_target || (*cit)->is_capable_be_effect_target(core.reason_effect, core.reason_player))) {
          if (pret) {
            *pret = *cit;
            return TRUE;
          }
    ...
    if (location & LOCATION_HAND) {
      for (auto &pcard : player[self].list_hand) {
        if (pcard != pexception && !(pexgroup && pexgroup->has_card(pcard)) && pduel->lua->check_filter(L, pcard, findex, extraargs) && (!is_target || pcard->is_capable_be_effect_target(core.reason_effect, core.reason_player))) {
          if (pret) {
            *pret = pcard;
            return TRUE;
          }
...
int32_t field::get_release_list(lua_State *L, uint8_t playerid, card_set *release_list, card_set *ex_list, card_set *ex_list_oneof, int32_t use_con, int32_t use_hand, int32_t fun, int32_t exarg, card *exc, group *exg, uint32_t reason) {
  ...
  for (auto &pcard : player[playerid].list_mzone) {
    if (pcard && pcard != exc && !(exg && exg->has_card(pcard)) && pcard->is_releasable_by_nonsummon(playerid, reason) && (reason != REASON_EFFECT || pcard->is_releasable_by_effect(playerid, re)) && (!use_con || pduel->lua->check_filter(L, pcard, fun, exarg))) {
      if (release_list) {
  ...
  for (auto &pcard : player[1 - playerid].list_mzone) {
    if (pcard && pcard != exc && !(exg && exg->has_card(pcard)) && (pcard->is_position(POS_FACEUP) || !use_con) && pcard->is_releasable_by_nonsummon(playerid, reason) && (reason != REASON_EFFECT || pcard->is_releasable_by_effect(playerid, re)) && (!use_con || pduel->lua->check_filter(L, pcard, fun, exarg))) {
      pcard->release_param = 1;
...
int32_t field::check_release_list(lua_State *L, uint8_t playerid, int32_t count, int32_t use_con, int32_t use_hand, int32_t fun, int32_t exarg, card *exc, group *exg, uint32_t reason) {
  ...
  auto rcount = (int32_t)relcard.size();
...
int32_t field::get_summon_release_list(card *target, card_set *release_list, card_set *ex_list, card_set *ex_list_oneof, group *mg, uint32_t ex, uint32_t releasable, uint32_t pos) {
  ...
  for (const auto &pcard : ex_tribute) {
...
void field::get_ritual_material(uint8_t playerid, effect *peffect, card_set *material, uint8_t no_level) {
  for (auto &pcard : player[playerid].list_mzone) {
    if (pcard && pcard->is_affect_by_effect(peffect) && pcard->is_releasable_by_nonsummon(playerid, REASON_EFFECT) && pcard->is_releasable_by_effect(playerid, peffect) && (no_level || pcard->get_level() > 0 || pcard->is_affected_by_effect(EFFECT_RITUAL_LEVEL_EX))) {
  ...
  for (auto &pcard : player[playerid].list_grave) {
    if ((pcard->data.type & TYPE_MONSTER) && pcard->is_affected_by_effect(EFFECT_EXTRA_RITUAL_MATERIAL) && pcard->is_removeable(playerid, POS_FACEUP, REASON_EFFECT) && (no_level || pcard->get_level() > 0 || pcard->is_affected_by_effect(EFFECT_RITUAL_LEVEL_EX))) {
      material->insert(pcard);
    }
  }
  for (auto &pcard : player[playerid].list_extra) {
    if (pcard->is_affected_by_effect(EFFECT_EXTRA_RITUAL_MATERIAL) && (no_level || pcard->get_level() > 0 || pcard->is_affected_by_effect(EFFECT_RITUAL_LEVEL_EX))) {
      material->insert(pcard);
    }
  }
}
...
void field::get_xyz_material(lua_State *L, card *scard, int32_t findex, uint32_t lv, int32_t maxc, group *mg) {
  ...
  uint32_t xyz_level;
  if (mg) {
    for (const auto &pcard : mg->container) {
      if (pcard->is_can_be_xyz_material(scard) && (xyz_level = pcard->check_xyz_level(scard, lv)) && (findex == 0 || pduel->lua->check_filter(L, pcard, findex, 0))) {
    ...
    for (auto &pcard : player[playerid].list_mzone) {
      if (pcard && pcard->is_position(POS_FACEUP) && !pcard->is_treated_as_not_on_field() && pcard->is_can_be_xyz_material(scard) && (xyz_level = pcard->check_xyz_level(scard, lv)) && (findex == 0 || pduel->lua->check_filter(L, pcard, findex, 0))) {
        core.xmaterial_lst.emplace((xyz_level >> 12) & 0xf, pcard);
      }
    }
    for (auto &pcard : player[1 - playerid].list_mzone) {
      if (pcard && pcard->is_position(POS_FACEUP) && !pcard->is_treated_as_not_on_field() && pcard->is_can_be_xyz_material(scard) && (xyz_level = pcard->check_xyz_level(scard, lv)) && pcard->is_affected_by_effect(EFFECT_XYZ_MATERIAL) && (findex == 0 || pduel->lua->check_filter(L, pcard, findex, 0))) {
        core.xmaterial_lst.emplace((xyz_level >> 12) & 0xf, pcard);
      }
    }
  }
...
void field::get_overlay_group(uint8_t self, uint8_t s, uint8_t o, card_set *pset) {
  ...
  for (int32_t p = 0; p < 2; ++p) {
    if (c) {
      for (auto &pcard : player[self].list_mzone) {
        if (pcard && !pcard->is_treated_as_not_on_field() && !pcard->xyz_materials.empty()) {
...
void field::adjust_disable_check_list() {
  if (effects.disable_check_list.empty()) {
    return;
  }
  do {
    card_set checked;
    while (!effects.disable_check_list.empty()) {
...
void field::adjust_self_destroy_set() {
  ...
  for (int32_t i = 0; i < 2; ++i) {
    ...
    for (const auto &ucard : core.unique_cards[check_player]) {
      if (ucard->is_position(POS_FACEUP) && ucard->get_status(STATUS_EFFECT_ENABLED) && !ucard->get_status(STATUS_DISABLED | STATUS_FORBIDDEN)) {
        card_set cset;
    ...
    std::sort(uniq_set.begin(), uniq_set.end(), [](card *lhs, card *rhs) {
      return lhs->fieldid < rhs->fieldid;
    });
    for (auto &pcard : uniq_set) {
      add_process(PROCESSOR_SELF_DESTROY, 0, nullptr, nullptr, check_player, 0, 0, 0, pcard);
  ...
  if (!core.self_destroy_set.empty()) {
    add_process(PROCESSOR_SELF_DESTROY, 10, nullptr, nullptr, 0, 0);
  }
  if (!core.self_tograve_set.empty()) {
    add_process(PROCESSOR_SELF_DESTROY, 20, nullptr, nullptr, 0, 0);
  }
}
...
int32_t field::adjust_grant_effect() {
  ...
  for (auto &eit : effects.grant_effect) {
    ...
    auto *geffect = (effect *)peffect->get_label_object();
...
effect *field::check_unique_onfield(card *pcard, uint8_t controler, uint8_t location, card *icard) {
  for (const auto &ucard : core.unique_cards[controler]) {
    if ((ucard != pcard) && (ucard != icard) && ucard->is_position(POS_FACEUP) && ucard->get_status(STATUS_EFFECT_ENABLED) && !ucard->get_status(STATUS_DISABLED | STATUS_FORBIDDEN) && ucard->unique_fieldid && ucard->check_unique_code(pcard) && (ucard->unique_location & location)) {
...
void field::check_card_counter(card *pcard, int32_t counter_type, int32_t playerid) {
  auto &counter_map = (counter_type == ACTIVITY_SUMMON) ? core.summon_counter : (counter_type == ACTIVITY_NORMALSUMMON) ? core.normalsummon_counter : (counter_type == ACTIVITY_SPSUMMON) ? core.spsummon_counter : (counter_type == ACTIVITY_FLIPSUMMON) ? core.flipsummon_counter : core.attack_counter;
...
int32_t field::check_spsummon_counter(uint8_t playerid, uint8_t ct) {
  if (core.global_flag & GLOBALFLAG_SPSUMMON_COUNT) {
    for (const auto &peffect : effects.spsummon_count_eff) {
      card *pcard = peffect->get_handler();
      auto val = (uint16_t)peffect->value;
      if (peffect->is_available()) {
...
int32_t field::get_attack_target(card *pcard, card_vector *v, uint8_t chain_attack, bool select_target) {
  ...
  if (!chain_attack && pcard->announce_count > extra_count && (!extra_count_m || pcard->announce_count > extra_count_m || pcard->announced_cards.findcard(nullptr))) {
    effect *peffect;
  ...
  if ((mcount == 0 || pcard->is_affected_by_effect(EFFECT_DIRECT_ATTACK) || core.attack_player) && !pcard->is_affected_by_effect(EFFECT_CANNOT_DIRECT_ATTACK) && !(!chain_attack && extra_count_m && pcard->announce_count > extra_count) && !(chain_attack && core.chain_attack_target)) {
...
bool field::confirm_attack_target() {
  ...
  return core.attack_target && std::find(cv.begin(), cv.end(), core.attack_target) != cv.end() || !core.attack_target && core.attacker->direct_attackable;
}
...
int32_t field::check_must_material(group *mg, uint8_t playerid, uint32_t limit) {
  ...
  for (const auto &pcard : must_list) {
...
int32_t field::check_synchro_material(lua_State *L, card *pcard, int32_t findex1, int32_t findex2, int32_t min, int32_t max, card *smat, group *mg) {
  if (mg) {
    for (const auto &tuner : mg->container) {
...
int32_t field::check_tuner_material(lua_State *L, card *pcard, card *tuner, int32_t findex1, int32_t findex2, int32_t min, int32_t max, card *smat, group *mg) {
  ...
  if (mg) {
    ...
    for (const auto &pm : material_set) {
...
int32_t field::check_tribute(card *pcard, int32_t min, int32_t max, group *mg, uint8_t toplayer, uint32_t zone, uint32_t releasable, uint32_t pos) {
  ...
  for (const auto &release_card : release_list) {
...
int32_t field::check_xyz_material(lua_State *L, card *scard, int32_t findex, int32_t lv, int32_t min, int32_t max, group *mg) {
  ...
  if (mct > 0) {
    if (ct == 0 && std::none_of(mcset.begin(), mcset.end(), [=](card *pcard) {
  ...
  if (!(core.global_flag & GLOBALFLAG_TUNE_MAGICIAN)) {
    if (std::any_of(mcset.begin(), mcset.end(), [=](card *pcard) {
          return std::find_if(core.xmaterial_lst.begin(), core.xmaterial_lst.end(), [=](const std::pair<int32_t, card *> &v) {
                   return v.second == pcard;
                 }) == core.xmaterial_lst.end();
        })) {
      return FALSE;
    }
    return (int32_t)core.xmaterial_lst.size() >= min;
  }
  for (const auto &pcard : mcset) {
  ...
  for (const auto &pcard : mcset) {
    auto it = std::find_if(core.xmaterial_lst.begin(), core.xmaterial_lst.end(), [=](const std::pair<int32_t, card *> &v) {
      return v.second == pcard;
    });
  ...
  for (auto cit = core.xmaterial_lst.begin(); cit != core.xmaterial_lst.end();) {
    ...
    effect *peffect = pcard->is_affected_by_effect(EFFECT_TUNE_MAGICIAN_X);
    if (peffect) {
      if (std::any_of(mcset.begin(), mcset.end(), [=](card *mcard) {
            return !!peffect->get_value(mcard);
          })) {
        cit = core.xmaterial_lst.erase(cit);
        continue;
      }
  ...
  ct += (int32_t)std::count_if(mcset.begin(), mcset.end(), [=](card *pcard) {
    return handover_zone_cards.find(pcard) != handover_zone_cards.end();
  });
...
chain *field::get_chain(uint32_t chaincount) {
  if (chaincount == 0 && !core.continuous_chain.empty() && (core.reason_effect->type & EFFECT_TYPE_CONTINUOUS)) {
...
int32_t field::check_cteffect_hint(effect *peffect, uint8_t playerid) {
  ...
  for (auto &efit : phandler->field_effect) {
    ...
    if (code == EVENT_FREE_CHAIN || code == EVENT_PHASE + infos.phase) {
      tevent test_event;
      test_event.event_code = code;
      if (get_cteffect_evt(feffect, playerid, test_event, FALSE) && (code != EVENT_FREE_CHAIN || check_hint_timing(feffect))) {
        return TRUE;
      }
    }
    else {
...
int32_t field::check_nonpublic_trigger(chain &ch) {
  effect *peffect = ch.triggering_effect;
  card *phandler = peffect->get_handler();
  if (!peffect->is_flag(EFFECT_FLAG_FIELD_ONLY) && ((peffect->type & EFFECT_TYPE_SINGLE) && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE) && phandler->is_has_relation(ch) && (ch.triggering_location & (LOCATION_HAND | LOCATION_DECK)) || (peffect->range & (LOCATION_HAND | LOCATION_DECK)))) {
    ch.flag |= CHAIN_HAND_TRIGGER;
    core.new_ochain_h.push_back(ch);
    if (ch.triggering_location == LOCATION_HAND && phandler->is_position(POS_FACEDOWN) || ch.triggering_location == LOCATION_DECK || peffect->range && !peffect->in_range(ch)) {
      return FALSE;
    }
  }
  return TRUE;
...
int32_t field::check_trigger_effect(const chain &ch) const {
  ...
  if (core.duel_rule >= MASTER_RULE_2020) {
    ...
    if (peffect->type & EFFECT_TYPE_FIELD) {
      ...
      if ((ch.triggering_location & (LOCATION_DECK | LOCATION_HAND | LOCATION_EXTRA)) && (ch.triggering_position & POS_FACEDOWN)) {
        return TRUE;
      }
      if (!(phandler->current.location & (LOCATION_DECK | LOCATION_HAND | LOCATION_EXTRA)) || phandler->is_position(POS_FACEUP)) {
        return TRUE;
      }
      return FALSE;
    }
...
int32_t field::check_spself_from_hand_trigger(const chain &ch) const {
  ...
  if ((peffect->status & EFFECT_STATUS_SPSELF) && (ch.flag & CHAIN_HAND_TRIGGER)) {
    return std::none_of(core.current_chain.begin(), core.current_chain.end(), [tp](chain ch) {
      return ch.triggering_player == tp && (ch.triggering_effect->status & EFFECT_STATUS_SPSELF) && (ch.flag & CHAIN_HAND_TRIGGER);
    });
  }
  return TRUE;
}
...
int32_t field::is_able_to_enter_bp() {
  return ((core.duel_options & DUEL_ATTACK_FIRST_TURN) || infos.turn_id != 1) && infos.phase < PHASE_BATTLE_START && !is_player_affected_by_effect(infos.turn_player, EFFECT_CANNOT_BP);
}
