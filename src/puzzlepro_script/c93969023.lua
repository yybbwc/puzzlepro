...
function c93969023.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c93969023.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c93969023.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c93969023.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c93969023.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
