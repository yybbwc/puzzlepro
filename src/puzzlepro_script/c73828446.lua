...
function c73828446.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c73828446.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c73828446.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c73828446.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  e:SetLabelObject(bc)
  if chk == 0 then
    return bc:IsAbleToRemove() and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_ONFIELD, 1, bc)
  end
...
function c73828446.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c73828446.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c73828446.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
