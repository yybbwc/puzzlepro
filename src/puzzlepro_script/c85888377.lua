...
function c85888377.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 85888377, 0, TYPES_NORMAL_TRAP_MONSTER, 2000, 2200, 6, RACE_REPTILE, ATTRIBUTE_EARTH)
  end
...
function c85888377.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not Duel.IsPlayerCanSpecialSummonMonster(tp, 85888377, 0, TYPES_NORMAL_TRAP_MONSTER, 2000, 2200, 6, RACE_REPTILE, ATTRIBUTE_EARTH) then
    return
  end
