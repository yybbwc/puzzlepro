...
function c75147529.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c75147529.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c75147529.rmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c75147529.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_ONFIELD, 0, 1, c)
  end
