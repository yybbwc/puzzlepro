...
function c64454614.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c64454614.costfilter, tp, LOCATION_HAND, 0, 1, nil, rtype)
  end
...
function c64454614.spfilter(c, e, tp)
  return c:IsType(TYPE_NORMAL) and c:IsLevel(4) and c:IsRace(RACE_WARRIOR) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c64454614.thfilter, tp, LOCATION_DECK, 0, 1, c)
end
...
function c64454614.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c64454614.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
