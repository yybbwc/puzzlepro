...
function c12822541.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c12822541.filter1(chkc)
  end
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(12822541, 0), aux.Stringid(12822541, 1))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(12822541, 0))
  else
    op = Duel.SelectOption(tp, aux.Stringid(12822541, 1)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
