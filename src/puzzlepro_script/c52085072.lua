...
function c52085072.btcon(e, tp, eg, ep, ev, re, r, rp)
  return (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE) and not e:GetHandler():IsStatus(STATUS_CHAINING) and (Duel.GetAttacker() == e:GetHandler() or Duel.GetAttackTarget() == e:GetHandler())
end
...
function c52085072.btcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52085072.btcfilter, tp, LOCATION_GRAVE, 0, 1, nil) and e:GetHandler():GetFlagEffect(52085072) == 0
  end
