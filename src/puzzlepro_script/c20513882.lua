...
function c20513882.cfilter(c, tp)
  return c:GetOriginalLevel() > 0 and Duel.IsExistingMatchingCard(c20513882.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, c)
end
function c20513882.thfilter(c, tc)
  return c:IsType(TYPE_MONSTER) and c:GetOriginalLevel() == tc:GetOriginalLevel() and c:GetOriginalRace() == tc:GetOriginalRace() and c:GetOriginalAttribute() == tc:GetOriginalAttribute() and not c:IsOriginalCodeRule(tc:GetOriginalCodeRule()) and c:IsAbleToHand()
end
