...
function c97783338.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and aux.nzatk(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(aux.nzatk, tp, 0, LOCATION_MZONE, 1, nil) and c:GetFlagEffect(97783338) == 0
  end
...
function c97783338.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c97783338.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
