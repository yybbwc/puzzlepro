...
function c61587183.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_DECK) > 0 or Duel.IsExistingTarget(Card.IsAbleToHand, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
  local op = 0
  Duel.Hint(HINT_SELECTMSG, tp, aux.Stringid(61587183, 0))
  if Duel.IsExistingTarget(Card.IsAbleToHand, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and Duel.GetFieldGroupCount(tp, 0, LOCATION_DECK) > 0 then
