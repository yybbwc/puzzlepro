...
function c51011872.cfilter(c, ec)
  if c:IsLocation(LOCATION_MZONE) then
    ...
    return c:IsPreviousSetCard(0xfb) and c:IsPreviousPosition(POS_FACEUP) and bit.extract(ec:GetLinkedZone(c:GetPreviousControler()), c:GetPreviousSequence()) ~= 0
  end
end
...
function c51011872.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c51011872.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
  Duel.DiscardHand(tp, c51011872.costfilter, 1, 1, REASON_COST + REASON_DISCARD)
end
function c51011872.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ct = 1
  if Duel.GetLP(tp) >= Duel.GetLP(1 - tp) + 2000 then
    ct = 2
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, ct) and Duel.IsPlayerCanDraw(1 - tp, 1)
  end
  e:SetLabel(ct)
