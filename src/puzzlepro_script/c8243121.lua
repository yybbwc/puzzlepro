...
function c8243121.cfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xf7) and (Duel.IsExistingMatchingCard(c8243121.atkfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttack()) or Duel.IsExistingMatchingCard(c8243121.lvfilter, tp, 0, LOCATION_MZONE, 1, nil))
end
...
function c8243121.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c8243121.cfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8243121.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c8243121.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  local a, d = Duel.GetBattleMonster(1 - tp)
  if chk == 0 then
    return d:GetAttack() ~= a:GetAttack()
  end
end
