...
function c22091647.costfilter(c)
  return (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup() or c:GetEquipTarget()) and (c:GetType() & (TYPE_EQUIP + TYPE_SPELL)) == TYPE_EQUIP + TYPE_SPELL and c:IsAbleToRemoveAsCost()
end
function c22091647.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22091647.costfilter, tp, LOCATION_SZONE + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c22091647.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c22091647.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler(), tp)
  end
end
...
function c22091647.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22091647.costfilter2, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
