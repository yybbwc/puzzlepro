...
function c1825445.filter1(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0xdc) and Duel.IsExistingMatchingCard(c1825445.filter2, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c1825445.mttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c1825445.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c1825445.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
