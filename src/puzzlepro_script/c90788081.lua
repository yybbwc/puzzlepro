...
function c90788081.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsLevelAbove(1) and Duel.GetFieldGroupCount(tp, LOCATION_ONFIELD, 0) > 0
  end
end
...
function c90788081.indtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c90788081.indfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c90788081.indfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
