...
function c81146288.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:CheckSubGroup(aux.gfcheck, 2, 2, Card.IsLocation, LOCATION_HAND, LOCATION_GRAVE)
end
...
function c81146288.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(aux.NegateAnyFilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
end
