...
function c60621361.spcheckop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if p1 then
    Duel.RegisterFlagEffect(0, 60621361, RESET_PHASE + PHASE_END, 0, 1)
  end
  if p2 then
    Duel.RegisterFlagEffect(1, 60621361, RESET_PHASE + PHASE_END, 0, 1)
  end
end
...
function c60621361.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c60621361.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c60621361.filter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function c60621361.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and Duel.SpecialSummonStep(tc, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) then
    c:SetCardTarget(tc)
  end
