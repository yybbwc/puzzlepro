...
function c89326990.filter(c, e, tp, mc)
  ...
  return c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c89326990.scfilter, tp, LOCATION_EXTRA, 0, 1, nil, mg)
end
...
function c89326990.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c89326990.filter(chkc, e, tp, e:GetHandler())
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c89326990.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp, e:GetHandler())
  end
