...
function c92870717.spfilter(c, e, tp)
  return c:IsAttackBelow(2000) and c:IsRace(RACE_WARRIOR + RACE_SPELLCASTER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE)
end
function c92870717.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92870717.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c92870717.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
