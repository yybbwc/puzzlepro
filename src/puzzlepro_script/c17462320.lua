...
function c17462320.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c17462320.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c17462320.cfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and bit.band(c:GetPreviousTypeOnField(), TYPE_TUNER) ~= 0 and not c:IsType(TYPE_TOKEN) and c:IsReason(REASON_BATTLE + REASON_EFFECT)
end
function c17462320.thcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c17462320.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and eg:IsExists(c17462320.cfilter, 1, nil, tp) and not eg:IsContains(e:GetHandler())
end
