...
function c64161630.cfilter(c, e, tp)
  local rk = c:GetRank()
  return rk > 1 and c:IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c64161630.filter, tp, LOCATION_EXTRA, 0, 1, nil, rk - 1, c:GetRace(), e, tp, c)
end
function c64161630.filter(c, rk, rc, e, tp, mc)
  return c:IsType(TYPE_XYZ) and c:IsSetCard(0x48) and c:IsRank(rk) and c:IsRace(rc) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c64161630.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c64161630.cfilter, 1, nil, e, tp)
  end
