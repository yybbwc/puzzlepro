...
function c40159926.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c40159926.cfilter, tp, 0, LOCATION_MZONE, 1, nil) and not Duel.IsExistingMatchingCard(c40159926.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c40159926.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c40159926.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c40159926.lvfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
