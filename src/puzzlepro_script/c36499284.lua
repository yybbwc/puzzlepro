...
function c36499284.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c36499284.filter(chkc)
  end
  if chk == 0 then
    return true
  end
  if Duel.IsExistingTarget(c36499284.filter, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingMatchingCard(c36499284.filter2, tp, LOCATION_HAND, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(36499284, 0)) then
