...
function c35659410.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c35659410.showfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingMatchingCard(c35659410.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(35659410, 0)) then
...
function c35659410.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c35659410.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c35659410.thfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c35659410.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and Duel.SendtoHand(tc, nil, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if code == 89631139 and #g1 > 0 and Duel.SelectYesNo(tp, aux.Stringid(35659410, 1)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
    ...
    if code ~= 89631139 and #g2 > 0 and Duel.SelectYesNo(tp, aux.Stringid(35659410, 1)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
