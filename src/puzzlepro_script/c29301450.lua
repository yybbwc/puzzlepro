...
function s.srmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_ONFIELD) and chkc:IsAbleToRemove()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToRemove, tp, LOCATION_GRAVE + LOCATION_ONFIELD, LOCATION_GRAVE + LOCATION_ONFIELD, 1, nil)
  end
...
function s.cfilter1(c, tp)
  return c:IsFaceup() and c:IsAbleToRemove() and Duel.IsExistingTarget(s.cfilter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
...
function s.drmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.cfilter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function s.drmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g ~= 2 or Duel.Remove(g, 0, REASON_EFFECT + REASON_TEMPORARY) == 0 or not g:IsExists(Card.IsLocation, 1, nil, LOCATION_REMOVED) then
    return
  end
