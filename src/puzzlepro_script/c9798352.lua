...
function c9798352.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9798352.costfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c9798352.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsRelateToEffect(e) and c:IsLevelAbove(1)
  end
end
...
function c9798352.lvtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c9798352.lvfilter(chkc)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsLevelAbove(1) and Duel.IsExistingTarget(c9798352.lvfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function c9798352.lvop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_REMOVED) and c:IsRelateToEffect(e) and c:IsFaceup() then
    local e1 = Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_SINGLE)
    e1:SetCode(EFFECT_UPDATE_LEVEL)
    e1:SetValue(1)
...
function c9798352.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9798352.setfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
end
