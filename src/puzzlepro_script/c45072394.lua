...
function c45072394.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToGrave, tp, LOCATION_MZONE, loc, 1, nil)
  end
end
...
function c45072394.filter(c, e, tp)
  return c == e:GetLabelObject():GetLabelObject() and c:GetFlagEffect(45072394) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c45072394.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c45072394.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c45072394.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
