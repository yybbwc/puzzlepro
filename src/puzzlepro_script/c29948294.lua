...
function c29948294.checkop(e, tp, eg, ep, ev, re, r, rp)
  if eg:IsExists(c29948294.checkfilter, 1, nil, 0) then
    Duel.RegisterFlagEffect(0, 29948294, RESET_PHASE + PHASE_END, 0, 1)
  end
  if eg:IsExists(c29948294.checkfilter, 1, nil, 1) then
    Duel.RegisterFlagEffect(1, 29948294, RESET_PHASE + PHASE_END, 0, 1)
  end
end
function c29948294.cfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and not c:IsPublic() and Duel.IsExistingMatchingCard(c29948294.tgfilter, tp, LOCATION_EXTRA, 0, 1, nil, c:GetRace())
end
function c29948294.tgfilter(c, race)
  return (c:IsAttack(2500) or c:IsDefense(2500)) and c:IsRace(race) and c:IsLevel(8) and c:IsType(TYPE_FUSION) and c:IsAbleToGrave()
end
function c29948294.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29948294.cfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
