...
function c14731897.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c14731897.filter(chkc, sum)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c14731897.filter, tp, LOCATION_MZONE, 0, 1, nil, sum)
  end
