...
function c59120809.immval(e, re)
  ...
  --~ aux.ShowConstant(re:GetActivateLocation(), "LOCATION")
  return re:IsActiveType(TYPE_MONSTER) and re:IsActivated() and re:GetActivateLocation() == LOCATION_MZONE and (rc:IsRelateToEffect(re) and rc:IsDefensePos() or not rc:IsRelateToEffect(re) and rc:IsPreviousPosition(POS_DEFENSE))
end
...
function c59120809.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c59120809.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c59120809.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c59120809.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
