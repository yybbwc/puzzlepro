...
function c67680512.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c67680512.tgfilter, tp, 0, LOCATION_MZONE, 1, nil, tp, e:GetHandler())
  end
...
function c67680512.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c67680512.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67680512.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
