...
function c74458486.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return phase == PHASE_DAMAGE and not Duel.IsDamageCalculated() and Duel.GetAttacker():IsControler(1 - tp) and Duel.GetAttackTarget()
end
function c74458486.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, Duel.GetAttackTarget())
  end
