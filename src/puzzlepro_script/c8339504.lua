...
function c8339504.spfilter1(c, rk, race, att, code, e, tp, mc)
  return c:IsRank(rk) and c:IsRace(race) and c:IsAttribute(att) and not c:IsCode(code) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c8339504.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return e:IsHasType(EFFECT_TYPE_ACTIVATE) and e:GetHandler():IsCanOverlay() and Duel.CheckReleaseGroup(tp, c8339504.cfilter, 1, nil, e, tp)
  end
  e:SetLabel(0)
