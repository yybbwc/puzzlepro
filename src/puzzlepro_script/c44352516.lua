...
function c44352516.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c44352516.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44352516.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c44352516.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rp == 1 - tp and bit.band(r, 0x41) == 0x41 and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
...
function c44352516.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44352516.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
