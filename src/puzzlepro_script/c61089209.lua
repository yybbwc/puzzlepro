...
function c61089209.drcon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and ep == 1 - tp and re:IsActiveType(TYPE_TRAP)
end
...
function c61089209.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c61089209.thtgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61089209.thtgfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
