...
function c42548470.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c42548470.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c42548470.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsFaceup() and tc1:IsRelateToEffect(e) and tc2:IsFaceup() and tc2:IsRelateToEffect(e) then
    ...
    if sg:GetFirst() == tc1 then
      ...
      if lv1 < lv2 and Duel.IsPlayerCanDraw(tp, 1) and Duel.SelectYesNo(tp, aux.Stringid(42548470, 0)) then
      ...
      if lv2 < lv1 and Duel.IsPlayerCanDraw(tp, 1) and Duel.SelectYesNo(tp, aux.Stringid(42548470, 0)) then
        Duel.Draw(tp, 1, REASON_EFFECT)
      end
    end
  end
