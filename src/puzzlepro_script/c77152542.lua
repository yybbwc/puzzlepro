...
function c77152542.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c77152542.cfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c77152542.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c77152542.filter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c77152542.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
