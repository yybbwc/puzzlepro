...
function c25209168.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft1 > 1 and Duel.IsPlayerAffectedByEffect(p, 59822133) then
    ft1 = 1
  end
  ...
  if ft2 > 1 and Duel.IsPlayerAffectedByEffect(1 - p, 59822133) then
    ft2 = 1
  end
  ...
  if tg1:GetCount() > 0 or tg2:GetCount() > 0 then
    ...
    if Duel.IsExistingMatchingCard(Card.IsCode, tp, LOCATION_GRAVE, 0, 1, nil, 17484499) and stg:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(25209168, 2)) then
      Duel.BreakEffect()
