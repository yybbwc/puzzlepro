...
function c45948430.filter(c, e, tp)
  if not c:IsSetCard(0x10cf) or bit.band(c:GetType(), 0x81) ~= 0x81 or not c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_RITUAL, tp, false, true) then
    return false
  end
  return Duel.IsExistingMatchingCard(c45948430.matfilter1, tp, LOCATION_HAND, 0, 1, c, tp, c)
end
function c45948430.matfilter1(c, tp, rc)
  return c:IsLevelBelow(7) and c:IsAttribute(ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) and c:IsAbleToGrave() and c:IsCanBeRitualMaterial(rc) and Duel.IsExistingMatchingCard(c45948430.matfilter2, tp, LOCATION_DECK, 0, 1, c, c:GetLevel(), c:GetAttribute(), rc)
end
function c45948430.matfilter2(c, lv, att, rc)
  return ((c:IsAttribute(ATTRIBUTE_LIGHT) and att == ATTRIBUTE_DARK) or (c:IsAttribute(ATTRIBUTE_DARK) and att == ATTRIBUTE_LIGHT)) and c:IsLevel(8 - lv) and c:IsAbleToGrave() and c:IsCanBeRitualMaterial(rc)
end
function c45948430.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c45948430.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
