...
function c39973386.costfilter(c, e, tp)
  return (c:IsControler(tp) or c:IsFaceup()) and c:IsAttribute(ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) and c:GetOriginalLevel() > 0 and Duel.IsExistingMatchingCard(c39973386.spfilter, tp, LOCATION_HAND + LOCATION_EXTRA, 0, 1, nil, c, e, tp)
end
function c39973386.spfilter(c, tc, e, tp)
  if c:GetOriginalAttribute() == tc:GetOriginalAttribute() then
    return
  end
  ...
  return c:IsAttribute(ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) and c:GetOriginalRace() == tc:GetOriginalRace() and c:GetOriginalLevel() == tc:GetOriginalLevel() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (b1 or b2)
end
function c39973386.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.CheckReleaseGroup(tp, c39973386.costfilter, 1, nil, e, tp)
  end
...
function c39973386.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c39973386.tdfilter(chkc)
  end
  local g = Duel.GetMatchingGroup(c39973386.tdfilter, tp, LOCATION_GRAVE, 0, nil, e)
  if chk == 0 then
    return g:CheckSubGroup(aux.dabcheck, 2, 2) and Duel.IsPlayerCanDraw(tp, 1)
  end
