...
function c76685519.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c76685519.spfilter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c76685519.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp, zone)
  end
...
function c76685519.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c76685519.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c76685519.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
