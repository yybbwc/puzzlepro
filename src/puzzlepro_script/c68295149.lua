...
function c68295149.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c68295149.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68295149.atkfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c68295149.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or (tc:IsLocation(LOCATION_MZONE) and tc:IsFacedown()) then
    return
  end
...
function c68295149.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(c68295149.repfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
