...
function c22056710.cfilter(c, e, tp)
  return c:IsRace(RACE_ZOMBIE) and c:IsDiscardable() and Duel.IsExistingTarget(c22056710.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetOriginalLevel())
end
...
function c22056710.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c22056710.cfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
