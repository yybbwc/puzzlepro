...
function c54082269.acop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(re) or not re:IsActiveType(TYPE_MONSTER) or tc:IsFacedown() or tc:GetCounter(0x1002) > 0 then
    return
  end
...
function c54082269.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c54082269.ctfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c54082269.ctfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c54082269.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c54082269.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
