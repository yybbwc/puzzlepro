...
function c340002.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c340002.cfilter, c:GetControler(), LOCATION_ONFIELD, 0, 2, nil)
end
...
function c340002.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c340002.filter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
