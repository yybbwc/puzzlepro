...
function c4433488.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and c4433488.spfilter(chkc, e, tp, lc:GetMaterial())
  end
  if chk == 0 then
    return lc and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c4433488.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, lc:GetMaterial())
  end
