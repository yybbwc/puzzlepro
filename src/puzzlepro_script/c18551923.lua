...
function c18551923.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c18551923.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c18551923.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c18551923.rmtg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and c18551923.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c18551923.rmfilter, tp, 0, LOCATION_GRAVE, 1, nil)
  end
