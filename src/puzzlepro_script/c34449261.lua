...
function c34449261.filter1(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:IsSetCard(0xad) and Duel.IsExistingTarget(c34449261.filter2, tp, 0, LOCATION_MZONE, 1, nil, tp, c)
end
function c34449261.filter2(c, tp, tc)
  local tg = Group.FromCards(c, tc)
  return c:IsFaceup() and c:IsType(TYPE_FUSION) and Duel.IsExistingMatchingCard(c34449261.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tg)
end
...
function c34449261.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c34449261.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c34449261.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.Destroy(g, REASON_EFFECT) ~= 0 and dam > 0 then
    ...
    Duel.BreakEffect()
    if dg:IsExists(Card.IsPreviousControler, 1, nil, tp) then
      Duel.Damage(tp, dam, REASON_EFFECT, true)
    end
    if dg:IsExists(Card.IsPreviousControler, 1, nil, 1 - tp) then
