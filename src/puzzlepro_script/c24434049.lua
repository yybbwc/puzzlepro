...
function s.gmattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.gmattgfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, c)
  end
...
function s.matop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToChain() and tc:IsRelateToChain() and tc:IsCanOverlay() and not tc:IsImmuneToEffect(e) then
    tc:CancelToGrave()
    Duel.Overlay(c, tc)
    if Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
      Duel.BreakEffect()
