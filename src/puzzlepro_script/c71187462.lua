...
function c71187462.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c71187462.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c71187462.sfilter(c, e, tp, mc)
  return c:IsCode(9012916) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c71187462.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return clv > 0 and lv > 0 and g:CheckWithSumEqual(Card.GetLevel, lv, 1, 99) and Duel.IsExistingMatchingCard(c71187462.sfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
