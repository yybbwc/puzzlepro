...
function c93149655.atkcon(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(Card.IsSetCard, tp, LOCATION_PZONE, 0, 1, e:GetHandler(), 0x99) then
    return
  end
  local a = Duel.GetAttacker()
  local d = Duel.GetAttackTarget()
  if d and a:GetControler() ~= d:GetControler() then
    if a:IsControler(tp) and a:IsFaceup() then
      e:SetLabelObject(a)
    elseif d:IsFaceup() then
      e:SetLabelObject(d)
    else
      return false
    end
...
function c93149655.damcon(e, tp, eg, ep, ev, re, r, rp)
  return ep ~= tp and e:GetHandler():IsSummonType(SUMMON_TYPE_PENDULUM) and Duel.GetAttacker() == e:GetHandler()
end
