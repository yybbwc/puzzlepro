...
function c20007374.filter(c, e, tp)
  return c:IsCode(44508094) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c20007374.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_SMATERIAL) and Duel.IsExistingMatchingCard(c20007374.filter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c20007374.cacost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c20007374.cafilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c20007374.catg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ec:IsChainAttackable(0, true)
  end
end
