...
function c35058588.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c35058588.atkfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c35058588.countertg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and chkc:IsCanAddCounter(0x1038, 1)
  end
  local tg = Duel.GetMatchingGroup(c35058588.tgfilter, tp, 0, LOCATION_MZONE, nil, e)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and tg:GetCount() > 0 and Duel.IsExistingMatchingCard(c35058588.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
