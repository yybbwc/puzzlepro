...
function c56401775.thop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:IsRelateToEffect(e) and Duel.SendtoHand(c, nil, REASON_EFFECT) ~= 0 and c:IsLocation(LOCATION_HAND) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c56401775.thfilter), tp, LOCATION_GRAVE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(56401775, 2)) then
    Duel.BreakEffect()
