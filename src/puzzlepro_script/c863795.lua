...
function c863795.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c863795.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c863795.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c863795.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and chkc ~= c and c863795.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c863795.lvfilter, tp, LOCATION_MZONE, 0, 1, c)
  end
