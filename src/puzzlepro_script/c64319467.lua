...
function c64319467.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(64319467, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c64319467.filter(c, e, tp)
  return c:IsLevel(3, 4) and c:IsRace(RACE_FISH) and c:IsAttribute(ATTRIBUTE_WATER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c64319467.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c64319467.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c64319467.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
