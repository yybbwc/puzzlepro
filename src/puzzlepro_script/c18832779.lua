...
function c18832779.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c18832779.spfilter, tp, LOCATION_SZONE + LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c18832779.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c18832779.discfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c18832779.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg, REASON_EFFECT) > 0 and c:IsRelateToEffect(e) and c:GetEquipGroup():IsExists(c18832779.eqfilter, 1, nil) and Duel.IsExistingMatchingCard(c18832779.thfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(18832779, 2)) then
    Duel.BreakEffect()
