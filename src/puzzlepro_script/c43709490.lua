...
function c43709490.spcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsEnvironment(34572613, tp, LOCATION_FZONE) and Duel.GetTurnPlayer() ~= tp and (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2)
end
...
function c43709490.sp2costfilter(c, tp, tc)
  ...
  return c:IsAbleToRemoveAsCost() and (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and Duel.GetMZoneCount(tp, tg) > 0
end
function c43709490.sp2cost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReleasable() and Duel.IsExistingMatchingCard(c43709490.sp2costfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, c, tp, c)
  end
...
function c43709490.sp2tgfilter(c, e, tp)
  return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsSetCard(0x157) and c:IsLevel(8) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function c43709490.sp2tg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c43709490.sp2tgfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
