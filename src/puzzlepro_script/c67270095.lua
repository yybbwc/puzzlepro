...
function c67270095.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return d and d:IsAttackPos() and d:IsCanChangePosition() and d:IsCanBeEffectTarget(e)
  end
...
function c67270095.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c67270095.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp, ev)
  end
