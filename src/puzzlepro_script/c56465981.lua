...
function c56465981.thfilter(c, check)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and ((check and c:IsRace(RACE_WYRM)) or c:IsSetCard(0x16b))
end
...
function c56465981.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c56465981.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c56465981.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
