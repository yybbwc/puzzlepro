...
function c16209941.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoGrave(tc, REASON_DISCARD + REASON_EFFECT) ~= 0 and not tc:IsCode(16209941) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
...
function c16209941.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c16209941.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c16209941.spfilter2, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
