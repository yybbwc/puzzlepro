...
function c40634253.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c40634253.desfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsFaceup, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c40634253.damcon1(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc:IsPreviousControler(1 - tp) and tc:IsLocation(LOCATION_GRAVE) and tc:GetTextAttack() > 0 and bc:IsControler(tp) and bc:GetOriginalAttribute() == ATTRIBUTE_DARK and bc:GetOriginalRace() == RACE_MACHINE and bc:IsType(TYPE_MONSTER)
end
function c40634253.damfilter2(c, tp)
  return c:IsReason(REASON_EFFECT) and c:IsType(TYPE_MONSTER) and c:IsReason(REASON_DESTROY) and c:IsLocation(LOCATION_GRAVE) and c:IsPreviousControler(1 - tp) and c:GetTextAttack() > 0
end
function c40634253.damcon2(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tgp == tp and loc == LOCATION_MZONE and rc:GetOriginalAttribute() == ATTRIBUTE_DARK and rc:GetOriginalRace() == RACE_MACHINE and eg:IsExists(c40634253.damfilter2, 1, nil, tp)
end
