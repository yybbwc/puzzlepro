...
function c84218527.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c84218527.filter(chkc, e, tp)
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > 0 and Duel.IsExistingTarget(c84218527.filter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c84218527.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft < 1 or g:GetCount() < 1 or (g:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) then
    return
  end
