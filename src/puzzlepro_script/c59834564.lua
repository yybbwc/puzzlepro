...
function c59834564.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c59834564.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
function c59834564.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c59834564.filter(chkc)
  end
  if chk == 0 then
    return true
  end
