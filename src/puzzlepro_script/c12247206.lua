...
function c12247206.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and tc:IsLocation(LOCATION_MZONE) and tc:IsControler(tp) and tc:IsFaceup() and tc:IsAttackBelow(1500) and Duel.IsExistingMatchingCard(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function c12247206.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft2 > 1 and Duel.IsPlayerAffectedByEffect(1 - tp, 59822133) then
    ft2 = 1
  end
