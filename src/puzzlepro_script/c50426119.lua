...
function c50426119.sprcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c50426119.sprfilter, tp, LOCATION_GRAVE, 0, 2, nil)
end
...
function c50426119.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c50426119.rmfilter(chkc, atk)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c50426119.rmfilter, tp, 0, LOCATION_MZONE, 1, nil, atk)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
