...
function s.immtg(e, c)
  if c:IsSetCard(0x1073, 0x1048) then
    return true
  end
...
function s.cfilter(c, tp)
  return c:IsFaceup() and c:IsSummonPlayer(tp) and c:IsType(TYPE_XYZ) and c:GetSpecialSummonInfo(SUMMON_INFO_TYPE) & TYPE_SPELL ~= 0 and c:IsSpecialSummonSetCard(0x95)
end
...
function s.xyztg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.tgfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, g, tp)
  end
  local tg1
