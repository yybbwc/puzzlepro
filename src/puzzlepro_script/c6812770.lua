...
function s.ovtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.ovfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.ovfilter, tp, LOCATION_MZONE, 0, 1, c) and c:IsCanOverlay()
  end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:GetOverlayCount() > 0 and tc:RemoveOverlayCard(tp, 1, 1, REASON_EFFECT) ~= 0 and c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 then
...
function s.xyzop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg:GetCount() ~= 2 or not tg:IsExists(s.xyzfilter2, 1, nil) then
    return
  end
