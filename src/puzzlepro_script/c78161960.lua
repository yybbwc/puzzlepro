...
function c78161960.filter(c, e, tp)
  return c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c78161960.filter2, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
function c78161960.filter2(c, e, tp)
  return c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c78161960.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 78161960) == 0 then
    return
  end
  local c = e:GetHandler()
  Duel.SkipPhase(1 - tp, PHASE_BATTLE, RESET_PHASE + PHASE_BATTLE_STEP, 1)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingMatchingCard(c78161960.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(78161960, 0)) then
    Duel.BreakEffect()
