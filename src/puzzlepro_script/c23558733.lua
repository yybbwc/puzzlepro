...
function c23558733.damcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsPreviousControler(tp) and bit.band(r, REASON_DESTROY) ~= 0 and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD)
end
...
function c23558733.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c23558733.cfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
