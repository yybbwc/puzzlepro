...
function c84941194.sprcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c84941194.sprfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function c84941194.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84941194.costfilter, tp, LOCATION_HAND, 0, 1, c) and c:IsAbleToGraveAsCost()
  end
...
function c84941194.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84941194.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c84941194.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg and Duel.SendtoHand(tg, nil, REASON_EFFECT) ~= 0 and Duel.ConfirmCards(1 - tp, tg) ~= 0 and Duel.IsPlayerCanDraw(tp, 1) and Duel.SelectYesNo(tp, aux.Stringid(84941194, 2)) then
    Duel.ShuffleDeck(tp)
    Duel.BreakEffect()
...
function c84941194.cfilter(c, tp)
  return c:IsPreviousControler(tp) and c:GetReasonPlayer() == 1 - tp and c:IsReason(REASON_EFFECT)
end
