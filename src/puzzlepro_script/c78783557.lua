...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsLocation(LOCATION_FZONE)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_FZONE, LOCATION_FZONE, 1, nil) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, 1 - tp, false, false, POS_FACEUP) > 0 then
    ...
    if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) > 0 then
      ...
      local op = aux.SelectFromOptions(tp, { b1, 1190 }, { b2, 1153 }, { true, aux.Stringid(id, 2) })
      if op < 3 then
        Duel.BreakEffect()
      end
      if op == 1 then
