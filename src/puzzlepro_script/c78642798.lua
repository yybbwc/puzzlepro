...
function c78642798.rntg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMatchingGroupCount(aux.NOT(Card.IsPublic), tp, 0, LOCATION_HAND, nil) > 0
  end
end
...
function c78642798.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c78642798.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c78642798.filter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c78642798.eqfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
