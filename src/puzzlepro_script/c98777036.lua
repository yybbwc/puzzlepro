...
function c98777036.ctfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and Duel.IsExistingTarget(c98777036.ctffilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetLevel())
end
function c98777036.ctcos(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98777036.ctfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c98777036.cttar(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c98777036.ctffilter(chkc, e:GetLabel())
  end
  if chk == 0 then
    return true
  end
...
function c98777036.lvtar(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c98777036.lvfilter(chkc, lv)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98777036.lvfilter, tp, LOCATION_GRAVE, 0, 1, nil, lv)
  end
