...
function c50951254.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c50951254.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c50951254.sfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousAttributeOnField(), ATTRIBUTE_FIRE) ~= 0 and c:IsReason(REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE)
end
