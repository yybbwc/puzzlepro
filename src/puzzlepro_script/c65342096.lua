...
function c65342096.spcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetAttackTarget() ~= nil and (c65342096.spconcheck(Duel.GetAttacker(), tp) or c65342096.spconcheck(Duel.GetAttackTarget(), tp))
end
...
function c65342096.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c65342096.spfilter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
