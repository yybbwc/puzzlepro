...
function c66957584.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c66957584.disfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c66957584.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c66957584.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c66957584.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c66957584.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sg:GetCount() == 0 or ft < sg:GetCount() or (sg:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) then
    return
  end
