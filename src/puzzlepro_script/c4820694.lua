...
function c4820694.lptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c4820694.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4820694.filter, tp, LOCATION_MZONE, 0, 2, nil)
  end
...
function c4820694.lpop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsRelateToEffect(e) and tc2:IsRelateToEffect(e) then
    ...
    og1:Merge(og2)
    if Duel.SendtoGrave(og1, REASON_EFFECT) < og1:GetCount() then
      return
    end
...
function c4820694.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c4820694.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c4820694.mfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
...
function c4820694.matop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFacedown() or not tc:IsRelateToEffect(e) or tc:IsImmuneToEffect(e) then
    return
  end
