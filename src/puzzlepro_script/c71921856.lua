...
function c71921856.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c71921856.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil)
  end
end
...
function c71921856.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabel(ct)
  return c:IsReason(REASON_DESTROY) and c:GetReasonPlayer() == 1 - tp and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and ct > 0
end
...
function c71921856.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c71921856.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c71921856.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  local ct = e:GetLabel()
