...
function c12743620.filter1(c, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsType(TYPE_TUNER) and c:IsAbleToRemove() and Duel.IsExistingTarget(c12743620.filter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, lv)
end
...
function c12743620.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c12743620.filter1, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, tp)
  end
