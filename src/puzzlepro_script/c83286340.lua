...
function c83286340.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return (c83286340.check(Duel.GetAttacker(), tp) or c83286340.check(Duel.GetAttackTarget(), tp)) and Duel.IsExistingMatchingCard(c83286340.thfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c83286340.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c83286340.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c83286340.spfilter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
