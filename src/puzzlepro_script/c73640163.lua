...
function c73640163.cfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x5a) and c:IsControler(tp) and c:GetReasonPlayer() == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE)
end
...
function c73640163.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return c73640163.spfilter(chkc, e, tp, g)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c73640163.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, g)
  end
...
function c73640163.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SendtoGrave(c, REASON_EFFECT + REASON_DISCARD) ~= 0 and tc:IsRelateToEffect(e) then
...
function c73640163.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c73640163.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c73640163.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
