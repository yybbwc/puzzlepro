...
effect::effect(duel *pd) : pduel(pd) {
  label.reserve(4);
}
  ...
  if (!is_flag(EFFECT_FLAG_FIELD_ONLY)) {
    if (type & EFFECT_TYPE_ACTIVATE) {
      ...
      if (!(handler->data.type & TYPE_COUNTER)) {
        if ((code < 1132 || code > 1149) && pduel->game_field->infos.phase == PHASE_DAMAGE && !is_flag(EFFECT_FLAG_DAMAGE_STEP) && !pduel->game_field->get_cteffect(this, playerid, FALSE)) {
          return FALSE;
        }
        if ((code < 1134 || code > 1136) && pduel->game_field->infos.phase == PHASE_DAMAGE_CAL && !is_flag(EFFECT_FLAG_DAMAGE_CAL) && !pduel->game_field->get_cteffect(this, playerid, FALSE)) {
          return FALSE;
        }
      }
      uint32_t zone = 0xff;
      ...
      if (handler->current.location == LOCATION_SZONE) {
        ...
        if (handler->data.type & TYPE_MONSTER) {
          ...
          if (!pduel->game_field->is_location_useable(playerid, LOCATION_PZONE, 0) && !pduel->game_field->is_location_useable(playerid, LOCATION_PZONE, 1)) {
            return FALSE;
          }
        }
        else if (!(handler->data.type & TYPE_FIELD) && pduel->game_field->get_useable_count(handler, playerid, LOCATION_SZONE, playerid, LOCATION_REASON_TOFIELD, zone) <= 0) {
          return FALSE;
        }
      }
      // check activate in hand/in set turn
      ...
      if (phandler->current.location == LOCATION_DECK || pduel->game_field->core.duel_rule >= MASTER_RULE_2020 && phandler->current.location == LOCATION_EXTRA && (phandler->current.position & POS_FACEDOWN)) {
      ...
      if (((type & EFFECT_TYPE_FIELD) || ((type & EFFECT_TYPE_SINGLE) && is_flag(EFFECT_FLAG_SINGLE_RANGE))) && (phandler->current.location & LOCATION_ONFIELD) && (!phandler->is_position(POS_FACEUP) || !phandler->is_status(STATUS_EFFECT_ENABLED))) {
        return FALSE;
      }
...
int32_t effect::is_target(card *pcard) {
  ...
  if (!is_flag(EFFECT_FLAG_SET_AVAILABLE) && (pcard->current.location & LOCATION_ONFIELD) && !pcard->is_position(POS_FACEUP)) {
    return FALSE;
  }
...
int32_t effect::reset(uint32_t reset_level, uint32_t reset_type) {
  switch (reset_type) {
    ...
    case RESET_PHASE: {
      ...
      if ((((reset_flag & RESET_SELF_TURN) && pid == tp) || ((reset_flag & RESET_OPPO_TURN) && pid != tp)) && (reset_level & 0x3ff & reset_flag)) {
        --reset_count;
      }
      if (reset_count == 0) {
        return TRUE;
