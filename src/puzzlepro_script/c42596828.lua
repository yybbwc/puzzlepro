...
function c42596828.defcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c42596828.defcostfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c42596828.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bc and (ac == c or bc == c) and ac:IsPosition(POS_ATTACK) and ac:IsDefenseAbove(0) and bc:IsPosition(POS_ATTACK) and bc:IsDefenseAbove(0)
end
function c42596828.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:GetFlagEffect(42596828) == 0
  end
