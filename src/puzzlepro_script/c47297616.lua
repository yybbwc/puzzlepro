...
function c47297616.tgdisable(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(47297616) == 0
  end
...
function c47297616.tgspsum(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and chkc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp)
  end
  if chk == 0 then
    return true
  end
