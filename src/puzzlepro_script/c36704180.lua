...
function c36704180.filter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsSetCard(0x10db) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c36704180.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c36704180.drcfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
