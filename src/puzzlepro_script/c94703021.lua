...
function c94703021.tktg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 94703022, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_DARK)
  end
...
function c94703021.tkcost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.bfgcost(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.GetAttackTarget() and Duel.GetAttackTarget():IsAbleToRemoveAsCost() and Duel.GetMZoneCount(tp, Duel.GetAttackTarget()) > 0
  end
...
function c94703021.tktg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsType, tp, 0, LOCATION_MZONE, 1, nil, TYPE_LINK) and Duel.IsPlayerCanSpecialSummonMonster(tp, 94703022, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_DARK)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, 0)
  Duel.SetOperationInfo(0, CATEGORY_TOKEN, nil, 1, tp, 0)
end
function c94703021.tkop2(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 94703022, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_DARK) then
    return
  end
