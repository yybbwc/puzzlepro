...
function c17241941.rmcon(e)
  ...
  return c:IsSummonLocation(LOCATION_EXTRA) and bit.band(c:GetReason(), REASON_MATERIAL + REASON_SYNCHRO) == REASON_MATERIAL + REASON_SYNCHRO
end
...
function c17241941.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c17241941.posfilter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsCanChangePosition, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c17241941.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c17241941.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17241941.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c17241941.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToHand, tp, LOCATION_PZONE, 0, 1, nil)
  end
