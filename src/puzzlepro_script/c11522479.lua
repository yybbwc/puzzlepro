...
function c11522479.thfilter(c)
  return (c:IsSetCard(0x1084) and c:IsType(TYPE_MONSTER) and not c:IsCode(11522479) or c:IsSetCard(0x199) and c:IsType(TYPE_COUNTER)) and c:IsAbleToHand()
end
function c11522479.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c11522479.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c11522479.lgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local b1 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c11522479.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(11522479, 1), aux.Stringid(11522479, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(11522479, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(11522479, 2)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
