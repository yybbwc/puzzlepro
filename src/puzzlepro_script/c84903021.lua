...
function c84903021.tscon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 or not Duel.IsExistingMatchingCard(c84903021.cfilter, tp, LOCATION_MZONE, 0, 1, nil))
end
...
function c84903021.dstg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return (c84903021.check(Duel.GetAttacker(), tp) or c84903021.check(Duel.GetAttackTarget(), tp)) and Duel.IsExistingTarget(Card.IsType, tp, 0, LOCATION_ONFIELD, 1, nil, TYPE_SPELL + TYPE_TRAP) and Duel.IsExistingMatchingCard(c84903021.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
