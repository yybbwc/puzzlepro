...
function c30128445.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c30128445.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c30128445.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c30128445.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30128445.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c30128445.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    local c = e:GetHandler()
    if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and g:FilterCount(Card.IsLocation, nil, LOCATION_DECK) > 0 and c:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(30128445, 2)) then
      Duel.BreakEffect()
