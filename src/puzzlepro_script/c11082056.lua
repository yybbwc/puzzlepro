...
function c11082056.spfilter(c, e, tp, code, tc)
  return c:IsType(TYPE_FUSION) and c.material_trap and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and code == c.material_trap and Duel.GetLocationCountFromEx(tp, tp, tc, c) > 0
end
function c11082056.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c11082056.tgfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil, e, tp)
  end
