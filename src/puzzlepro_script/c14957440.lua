...
function c14957440.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanAddCounter(0x59, 1)
  end
...
function c14957440.tkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetHandler():AddCounter(0x59, 1) ~= 0 and ft > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 14957441, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_BEAST, ATTRIBUTE_EARTH) then
    Duel.BreakEffect()
    ...
    while ct > 0 do
      ...
      ct = ct - 1
      if ct > 0 and not Duel.SelectYesNo(tp, aux.Stringid(14957440, 1)) then
        ct = 0
      end
    end
