...
function c33837653.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and ((Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0) or Duel.IsExistingMatchingCard(c33837653.cfilter, tp, LOCATION_MZONE, 0, 1, nil))
end
...
function c33837653.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c33837653.filter1, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c33837653.discon(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
    return false
  end
...
function c33837653.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAttackAbove(500)
  end
