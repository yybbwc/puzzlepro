...
function c6182103.cfilter(c, rtype)
  return not c:IsStatus(STATUS_BATTLE_DESTROYED) and c:IsAbleToRemoveAsCost() and (not c:IsOnField() or c:IsFaceup()) and c:IsType(rtype) and c:IsSetCard(0x157)
end
function c6182103.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local rtype = re:GetActiveType() & 0x7
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6182103.cfilter, tp, LOCATION_GRAVE + LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil, rtype)
  end
...
function c6182103.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsSummonType(SUMMON_TYPE_FUSION)
end
function c6182103.thfilter(c, typ)
  return c:IsAbleToHand() and c:IsSetCard(0x157) and c:IsFaceup()
end
function c6182103.gcheck(g)
  return g:FilterCount(Card.IsType, nil, TYPE_MONSTER) <= 1 and g:FilterCount(Card.IsType, nil, TYPE_SPELL) <= 1 and g:FilterCount(Card.IsType, nil, TYPE_TRAP) <= 1
end
function c6182103.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6182103.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
