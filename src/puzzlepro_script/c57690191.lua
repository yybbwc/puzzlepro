...
function c57690191.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c57690191.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c57690191.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c57690191.drfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c57690191.drfilter, tp, LOCATION_GRAVE, 0, 3, nil)
  end
