...
function c39185163.disrmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_MZONE + LOCATION_GRAVE, LOCATION_MZONE + LOCATION_GRAVE, 1, nil) and Duel.GetFlagEffect(tp, 39185164) == 0
  if chk == 0 then
    return b1 or b2
  end
end
...
function c39185163.disrmop(e, tp, eg, ep, ev, re, r, rp)
  local b1 = Duel.IsChainDisablable(ev) and Duel.GetFlagEffect(tp, 39185163) == 0
  local b2 = Duel.IsExistingMatchingCard(c39185163.filter, tp, LOCATION_MZONE + LOCATION_GRAVE, LOCATION_MZONE + LOCATION_GRAVE, 1, nil) and Duel.GetFlagEffect(tp, 39185164) == 0
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(39185163, 1), aux.Stringid(39185163, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(39185163, 1))
  elseif b2 then
    op = Duel.SelectOption(tp, aux.Stringid(39185163, 2)) + 1
  else
    return
  end
  if op == 0 then
