...
function c26964762.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c26964762.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c26964762.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c26964762.deckcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c26964762.cfilter, tp, LOCATION_GRAVE, 0, 1, c)
  end
...
function c26964762.decktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_DECK) > 0 and Duel.IsExistingMatchingCard(c26964762.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
