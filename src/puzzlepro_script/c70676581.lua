...
function c70676581.distg(e, c)
  if c:GetCardTargetCount() == 0 or not c:IsType(TYPE_SPELL) then
    return false
  end
...
function c70676581.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.IsPlayerCanNormalDraw(tp) and Duel.IsExistingMatchingCard(c70676581.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
