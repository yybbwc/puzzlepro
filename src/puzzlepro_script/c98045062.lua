...
function c98045062.cfilter(c, tp)
  return Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0 and Duel.IsExistingTarget(c98045062.filter2, tp, 0, LOCATION_MZONE, 1, c)
end
function c98045062.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if e:GetLabel() == 9 then
    ...
    b2 = Duel.GetLocationCount(tp, LOCATION_MZONE, tp, LOCATION_REASON_CONTROL) > 0 and Duel.IsExistingTarget(c98045062.filter2, tp, 0, LOCATION_MZONE, 1, nil)
  end
  if chk == 0 then
    e:SetLabel(0)
    return b1 or b2
