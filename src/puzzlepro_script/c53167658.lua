...
function c53167658.repfilter1(c, tp)
  return c:IsOnField() and c:IsControler(tp) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)) and not c:IsReason(REASON_REPLACE)
end
function c53167658.reptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return eg:IsExists(c53167658.repfilter1, 1, c, tp) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
  end
...
function c53167658.repfilter2(c, tp)
  return c:IsOnField() and c:IsControler(1 - tp) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c53167658.reptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return eg:IsExists(c53167658.repfilter2, 1, nil, tp) and c:IsAbleToRemove()
  end
  if Duel.SelectEffectYesNo(tp, c, 96) then
