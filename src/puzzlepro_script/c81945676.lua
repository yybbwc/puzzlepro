...
function s.concon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetTurnPlayer() == 1 - tp and (ph == PHASE_MAIN1 or ph == PHASE_MAIN2) and Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function s.contg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and s.tfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.tfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
