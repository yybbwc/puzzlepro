...
function c88021907.ctcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local rt = 3
  if not Duel.IsExistingMatchingCard(aux.TRUE, tp, 0, LOCATION_HAND, 1, nil) then
    rt = rt - 1
  end
  if not Duel.IsExistingMatchingCard(aux.TRUE, tp, 0, LOCATION_MZONE, 1, nil) then
    rt = rt - 1
  end
  if not Duel.IsExistingMatchingCard(Card.IsType, tp, 0, LOCATION_ONFIELD, 1, nil, TYPE_SPELL + TYPE_TRAP) then
    rt = rt - 1
  end
  if chk == 0 then
...
function c88021907.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return eg:IsExists(c88021907.repfilter, 1, nil, tp) and c:CheckRemoveOverlayCard(tp, 1, REASON_EFFECT)
  end
