...
function c12079734.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    if e:GetLabel() == 0 then
      return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c12079734.filter1(chkc, c)
    else
      return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c12079734.filter2(chkc)
    end
  end
  local b1 = Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c12079734.filter1, tp, LOCATION_GRAVE, 0, 1, nil, c)
  ...
  elseif b1 then
    ...
    op = Duel.SelectOption(tp, aux.Stringid(12079734, 2)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
...
function c12079734.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabel() == 0 then
    local c = e:GetHandler()
    if c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and aux.CheckUnionEquip(tc, c) and Duel.Equip(tp, tc, c, false) then
