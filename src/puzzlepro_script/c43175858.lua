...
function c43175858.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:FilterCount(Card.IsAbleToRemove, nil, tp, POS_FACEDOWN) == 3
  end
...
function c43175858.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsType(TYPE_TOON) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c43175858.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ct = eg:FilterCount(c43175858.repfilter, nil, tp)
  local g = Duel.GetDecktopGroup(tp, ct)
  if chk == 0 then
    return g:IsExists(Card.IsAbleToRemove, ct, nil, tp, POS_FACEDOWN)
  end
