...
function c8372133.spfilter(c, e, tp)
  return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c8372133.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c8372133.costfilter, 1, nil, tp)
  end
...
function c8372133.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c8372133.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8372133.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c8372133.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c8372133.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8372133.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
