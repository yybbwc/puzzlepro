...
function c76871889.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c76871889.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c76871889.atcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(ec)
  return ec:IsControler(tp) and ec:IsSetCard(0x106) and bc and bc:IsType(TYPE_MONSTER) and ec:IsChainAttackable(0, true) and ec:IsStatus(STATUS_OPPO_BATTLE)
end
...
function c76871889.atcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c76871889.atcfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c76871889.atop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not ec or not ec:IsRelateToBattle() then
    return
  end
  Duel.ChainAttack()
  local e1 = Effect.CreateEffect(c)
