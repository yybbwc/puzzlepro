...
function c36368606.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c36368606.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c36368606.spfilter(c, e, tp)
  return c:IsRace(RACE_CYBERSE) and c:IsType(TYPE_LINK) and c:IsLocation(LOCATION_GRAVE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, c:GetControler())
end
function c36368606.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  for p in aux.TurnPlayers() do
    ...
    if ft > 1 and Duel.IsPlayerAffectedByEffect(p, 59822133) then
      ft = 1
    end
...
function c36368606.immtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c36368606.immfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
