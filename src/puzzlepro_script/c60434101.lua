...
function c60434101.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c60434101.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c60434101.tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c60434101.filter(chkc, c:GetAttack())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60434101.filter, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttack())
  end
