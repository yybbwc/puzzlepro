...
function c78348934.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c78348934.filter1(chkc, e)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c78348934.filter1, tp, 0, LOCATION_GRAVE, 1, nil, e) and Duel.IsExistingMatchingCard(c78348934.filter3, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c78348934.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c78348934.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
