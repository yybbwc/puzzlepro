...
function c10000010.splimit(e, se, sp, st)
  return Duel.IsPlayerAffectedByEffect(sp, 41044418) and (st & SUMMON_VALUE_MONSTER_REBORN > 0 or se:GetHandler():IsCode(83764718)) and e:GetHandler():IsControler(sp) and e:GetHandler():IsLocation(LOCATION_GRAVE)
end
...
function c10000010.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckLPCost(tp, lp - 100, true)
  end
  e:SetLabel(lp - 100)
