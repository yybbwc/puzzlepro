...
function c89544521.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and c89544521.desfilter1(chkc)
  end
  if chk == 0 then
    ...
    e:SetLabel(loc)
    return Duel.IsExistingTarget(c89544521.desfilter1, tp, loc, 0, 1, nil) and Duel.IsExistingMatchingCard(c89544521.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c89544521.desfilter2(c)
  return c:IsFaceup() and c:IsSetCard(0xab) and Duel.IsExistingTarget(c89544521.desfilter3, 0, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, c)
end
...
function c89544521.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c89544521.desfilter2, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DESTROY)
