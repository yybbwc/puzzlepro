...
function c25343017.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c25343017.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c25343017.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c25343017.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabel(0)
  if act == 1 and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():GetFlagEffect(25343017) then
    return true
  else
    rg:Clear()
    return false
...
function c25343017.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local rg = e:GetLabelObject()
  if chkc then
    return rg:IsContains(chkc) and c25343017.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    if rg:IsExists(c25343017.spfilter, 1, nil, e, tp) then
      return true
    else
      rg:Clear()
      return false
