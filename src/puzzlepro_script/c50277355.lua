...
function c50277355.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c50277355.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and lg:IsExists(c50277355.lkfilter, 1, nil, TYPE_FUSION)
...
function c50277355.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local b2 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c50277355.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and lg:IsExists(c50277355.lkfilter, 1, nil, TYPE_FUSION)
  if b2 then
    if res ~= 0 then
      Duel.BreakEffect()
    end
