...
function c47158777.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c47158777.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c47158777.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) ~= 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c47158777.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(47158777, 2)) then
    Duel.BreakEffect()
...
function c47158777.damtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c47158777.damfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47158777.damfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
