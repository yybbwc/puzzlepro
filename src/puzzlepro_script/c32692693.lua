...
function c32692693.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsCanAddCounter(tp, 0x60, 7, c)
  end
  c:AddCounter(0x60, 7)
end
...
function c32692693.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanRemoveCounter(tp, 0x60, 1, REASON_COST)
  end
...
function c32692693.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c32692693.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c32692693.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
