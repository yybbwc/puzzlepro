...
function c54326448.cfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_PYRO) and Duel.IsExistingMatchingCard(c54326448.filter, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c54326448.attg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c54326448.cfilter, 1, nil, tp)
  end
