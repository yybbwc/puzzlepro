...
function c44139064.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(44139064, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c44139064.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c44139064.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c44139064.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44139064.atkfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.GetCustomActivityCount(44139064, tp, ACTIVITY_SPSUMMON) == 0
  end
