...
function c49456901.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c49456901.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c49456901.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c49456901.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and ep ~= tp and loc == LOCATION_MZONE and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainNegatable(ev) and e:GetHandler():GetOverlayGroup():IsExists(Card.IsCode, 1, nil, 2061963)
end
