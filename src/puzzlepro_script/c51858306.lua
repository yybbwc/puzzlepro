...
function c51858306.filter(c)
  return c:IsLevelAbove(7) and c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) and c:IsAbleToRemove()
end
...
function c51858306.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc and tc:GetFlagEffect(51858306) ~= 0 and tc:IsAbleToHand()
  end
