...
function c73887236.filter(c)
  return c:IsFaceup() and c:GetAttack() > 0 and c:IsSummonType(SUMMON_TYPE_SPECIAL)
end
function c73887236.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c73887236.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c73887236.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
