...
function s.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return p == 1 - tp and bit.band(loc, LOCATION_ONFIELD) ~= 0 and g:GetClassCount(Card.GetRace) > 2 and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev)
end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
