...
function c45236142.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c45236142.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c45236142.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c45236142.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45236142.filter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c45236142.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) then
    if tc:IsAbleToHand() and (not tc:IsAbleToDeck() or Duel.SelectOption(tp, aux.Stringid(45236142, 2), aux.Stringid(45236142, 3)) == 0) then
