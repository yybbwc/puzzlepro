...
function c16474916.effop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsExistingMatchingCard(c16474916.cfilter1, tp, LOCATION_ONFIELD, 0, 1, nil) then
    Duel.BreakEffect()
...
function c16474916.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c16474916.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
