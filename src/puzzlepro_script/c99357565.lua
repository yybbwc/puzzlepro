...
function c99357565.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  ct = math.min(ct, (Duel.GetLocationCount(tp, LOCATION_MZONE)), Duel.GetMatchingGroupCount(c99357565.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE + LOCATION_DECK, 0, nil, e, tp))
...
function c99357565.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c99357565.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c99357565.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99357565.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
