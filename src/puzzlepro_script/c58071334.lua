...
function s.sprfilter(c, sc)
  return c:GetOriginalType() & TYPE_MONSTER ~= 0 and c:IsFaceup() and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(sc, SUMMON_TYPE_SPECIAL)
end
...
function s.mvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.mvfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.mvfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function s.mvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsType(TYPE_MONSTER) and not tc:IsImmuneToEffect(e) and Duel.MoveToField(tc, tp, tc:GetOwner(), LOCATION_SZONE, POS_FACEUP, true) then
