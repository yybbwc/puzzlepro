...
function c92729410.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(92729410, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c92729410.filter(c, e, tp)
  return not c:IsCode(92729410) and c:IsLevel(2) and c:IsRace(RACE_BEAST) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE)
end
function c92729410.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c92729410.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
