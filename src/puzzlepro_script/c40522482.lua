...
function c40522482.filter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x3008) and c:IsType(TYPE_FUSION) and Duel.IsExistingMatchingCard(c40522482.desfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttack())
end
...
function c40522482.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c40522482.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c40522482.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
