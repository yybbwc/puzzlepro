...
function c8662794.atktg(e, c)
  ...
  return c:IsFaceup() and c:IsType(TYPE_LINK) and not c:IsSetCard(0x13c) and (lg1 and lg1:IsContains(e:GetHandler()) or lg2 and lg2:IsContains(c))
end
function c8662794.desfilter(c)
  return c:IsPreviousPosition(POS_FACEUP) and c:GetPreviousTypeOnField() & TYPE_LINK ~= 0 and c:IsPreviousSetCard(0x13c) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE)
end
...
function c8662794.srtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c8662794.srfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
