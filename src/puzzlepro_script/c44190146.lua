...
function c44190146.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44190146.costfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c44190146.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c44190146.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c44190146.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetMZoneCount(tp) < 2 or Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    max = 1
  end
...
function c44190146.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local rc = re:GetHandler()
  if chk == 0 then
    return rc:IsCanBeEffectTarget(e) and Duel.IsExistingTarget(nil, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c44190146.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) > 0 and rc:IsRelateToEffect(e) and rc:IsFaceup() then
