...
function c42198835.spcon(e, tp, eg, ep, ev, re, r, rp)
  if not (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2) then
    return false
  end
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 or Duel.IsExistingMatchingCard(c42198835.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function c42198835.rhtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return e:GetHandler():IsControlerCanBeChanged() and Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
