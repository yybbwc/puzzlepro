...
function c706925.spfilter(c, e, tp)
  return not c:IsCode(706925) and c:IsLevelBelow(4) and c:IsSetCard(0x77) and c:IsRace(RACE_SEASERPENT) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c706925.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c706925.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c706925.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c706925.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
