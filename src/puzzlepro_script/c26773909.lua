...
function c26773909.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c26773909.filter(chkc, lp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c26773909.filter, tp, LOCATION_MZONE, 0, 1, nil, lp)
  end
