...
function c36894320.actcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c36894320.acfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c36894320.acttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return c36894320.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  end
  if chk == 0 then
    return true
  end
  local b1 = c36894320.spcon(e, tp, eg, ep, ev, re, r, rp) and c36894320.cost(e, tp, eg, ep, ev, re, r, rp, 0) and c36894320.sptg(e, tp, eg, ep, ev, re, r, rp, 0)
  local b2 = c36894320.cost(e, tp, eg, ep, ev, re, r, rp, 0) and c36894320.tdtg(e, tp, eg, ep, ev, re, r, rp, 0)
  local op = -1
...
function c36894320.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(36894320) == 0
  end
...
function c36894320.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c36894320.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_HAND)
end
function c36894320.spop(e, tp, eg, ep, ev, re, r, rp)
  if not e:GetHandler():IsRelateToEffect(e) or Duel.GetLocationCount(tp, LOCATION_MZONE) < 1 then
    return
  end
...
function c36894320.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c36894320.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c36894320.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
