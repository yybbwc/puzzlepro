...
function c69248256.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69248256.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c69248256.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69248256.costfilter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil)
  end
...
function c69248256.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler())
  end
