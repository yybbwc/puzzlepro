...
function c89516305.sttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsFacedown, tp, 0, LOCATION_SZONE, 1, e:GetHandler())
  end
...
function c89516305.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c89516305.setfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c89516305.setfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
