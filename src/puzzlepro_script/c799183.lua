...
function c799183.condition(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(Card.IsSetCard, tp, LOCATION_MZONE, 0, 1, nil, 0x10cf) then
    return false
  end
  ...
  return g and g:IsExists(Card.IsLocation, 1, nil, LOCATION_MZONE) and Duel.IsChainNegatable(ev) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE))
end
...
function c799183.setcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x1, 1, REASON_COST)
  end
