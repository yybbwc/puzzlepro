...
function s.val(e, c)
  local r = c:GetRace()
  if bit.band(r, RACE_INSECT + RACE_PLANT + RACE_REPTILE) ~= 0 then
    return 300
  else
    return -300
  end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local b2 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp)
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
