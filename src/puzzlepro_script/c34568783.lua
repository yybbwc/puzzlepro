...
function s.cgcost(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.cgfilter, tp, LOCATION_GRAVE, 0, 1, nil, e:GetHandler())
  end
...
function s.cgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  c:RegisterEffect(e1)
  if tuner == 1 and not c:IsType(TYPE_TUNER) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
    Duel.BreakEffect()
    local e2 = Effect.CreateEffect(c)
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.tgfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.tgfilter, tp, LOCATION_GRAVE, 0, 1, c) and c:IsAbleToHand() and c:IsCanBeEffectTarget(e)
  end
