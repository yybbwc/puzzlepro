...
function c10443957.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 10443957) == 0
  end
...
function c10443957.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c10443957.filter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingTarget(c10443957.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler())
  end
