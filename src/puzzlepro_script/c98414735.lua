...
function c98414735.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c98414735.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98414735.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c98414735.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 98414735, 0xd4, TYPES_NORMAL_TRAP_MONSTER, 1200, 0, 2, RACE_AQUA, ATTRIBUTE_WATER)
  end
