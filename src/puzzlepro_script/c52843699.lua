...
function c52843699.sumcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == tp and (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2) and eg:IsExists(c52843699.sumcfilter, 1, nil, tp)
end
...
function c52843699.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52843699.filter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
...
function c52843699.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52843699.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c52843699.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52843699.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
