...
function c94212438.plcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() ~= tp and e:GetHandler():GetFlagEffect(94212438) < 4 and e:GetHandler():IsStatus(STATUS_EFFECT_ENABLED)
end
...
function c94212438.plop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local res = Duel.IsPlayerAffectedByEffect(tp, 16625614) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, id, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP, tp, SUMMON_VALUE_DARK_SANCTUARY)
  if Duel.GetLocationCount(tp, LOCATION_SZONE) <= 0 and not res then
    return
  end
  ...
  elseif tc and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.MoveToField(tc, tp, tp, LOCATION_SZONE, POS_FACEUP, true) then
    c:RegisterFlagEffect(94212438, RESET_EVENT + RESETS_STANDARD, 0, 0)
  end
end
