...
function c95474755.extg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_EXTRA, 1, nil, tp, POS_FACEDOWN)
  end
...
function c95474755.grtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToRemove, tp, 0, LOCATION_GRAVE, 1, nil, tp, POS_FACEDOWN)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function c95474755.dktg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and tg:FilterCount(Card.IsAbleToRemove, nil, tp, POS_FACEDOWN) == ct
  end
