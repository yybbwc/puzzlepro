...
function c52615248.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52615248.costfilter, tp, LOCATION_MZONE, 0, 1, c, tp, c) and c:IsAbleToRemoveAsCost()
  end
...
function c52615248.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c52615248.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c52615248.spfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
