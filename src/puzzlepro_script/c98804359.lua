...
function c98804359.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98804359.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c98804359.repfilter(c, tp)
  return c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsFaceup() and c:IsAttribute(ATTRIBUTE_WATER) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c98804359.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c98804359.repfilter, 1, nil, tp)
  end
