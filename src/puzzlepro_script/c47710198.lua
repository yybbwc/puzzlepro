...
function c47710198.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and tc ~= e:GetHandler() and tc:IsSummonType(SUMMON_TYPE_SYNCHRO) and tc:IsSummonPlayer(tp) and tc:IsRace(RACE_WYRM)
end
...
function c47710198.filter(c, e, tp)
  return c:IsSummonPlayer(1 - tp) and c:IsLocation(LOCATION_MZONE) and c:IsAbleToRemove() and (not e or c:IsRelateToEffect(e))
end
function c47710198.remtg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c47710198.filter, 1, nil, nil, tp)
  end
