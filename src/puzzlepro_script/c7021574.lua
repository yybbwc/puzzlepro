...
function c7021574.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7021574.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c7021574.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7021574.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c7021574.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() and chkc:IsControler(1 - tp)
  end
  if chk == 0 then
