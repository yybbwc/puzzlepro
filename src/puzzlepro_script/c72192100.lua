...
function c72192100.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tg or not tg:IsContains(e:GetHandler()) or not Duel.IsChainDisablable(ev) then
    return false
  end
...
function c72192100.filter(c, e, tp)
  return c:IsCode(35975813) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c72192100.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c72192100.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and eg:IsExists(c72192100.filter, 1, nil, e, tp)
  end
