...
function c67865534.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c67865534.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c67865534.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1)) and c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SelectEffectYesNo(tp, c, aux.Stringid(67865534, 3)) then
...
function c67865534.xyztg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c67865534.xyzfilter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingTarget(c67865534.xyzfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
