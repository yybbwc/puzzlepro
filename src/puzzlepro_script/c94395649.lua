...
function c94395649.cfilter(c, tp)
  return c:IsRace(RACE_BEAST + RACE_BEASTWARRIOR + RACE_WINDBEAST) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsFaceup() or c:IsControler(tp))
end
function c94395649.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c94395649.cfilter, 1, nil, tp)
  end
...
function c94395649.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94395649.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
