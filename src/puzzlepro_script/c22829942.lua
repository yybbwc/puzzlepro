...
function c22829942.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22829942.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c22829942.thtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c22829942.thfilter2(chkc, tid)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22829942.thfilter2, tp, LOCATION_GRAVE, 0, 1, nil, tid)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
