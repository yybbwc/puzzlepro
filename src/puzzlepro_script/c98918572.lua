...
function c98918572.rfilter(c, oc, tp)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:CheckRemoveOverlayCard(tp, oc, REASON_COST)
end
function c98918572.rcon(e, tp, eg, ep, ev, re, r, rp)
  local rc = re:GetHandler()
  return bit.band(r, REASON_COST) ~= 0 and re:IsActivated() and re:IsActiveType(TYPE_XYZ) and ep == e:GetOwnerPlayer() and rc:IsSetCard(0xf1) and Duel.IsExistingMatchingCard(c98918572.rfilter, tp, LOCATION_MZONE, 0, 1, rc, ev, tp)
end
...
function c98918572.xyztg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c98918572.xyzfilter, tp, LOCATION_MZONE, 0, 2, nil)
  end
