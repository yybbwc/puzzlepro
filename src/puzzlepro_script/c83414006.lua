...
function c83414006.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c83414006.cfilter1, 1, nil, tp)
  end
...
function c83414006.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonMonster(tp, 83414006, 0x5008, TYPE_MONSTER + TYPE_EFFECT, 900, 1100, 3, RACE_WARRIOR, ATTRIBUTE_DARK)
  end
...
function c83414006.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(aux.nzatk, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
end
