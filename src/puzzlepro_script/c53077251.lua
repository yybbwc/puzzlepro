...
function c53077251.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c53077251.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c53077251.filter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
