...
function s.thorspfilter(c, e, tp)
  if not c:IsType(TYPE_NORMAL) or not c:IsLevel(3) then
    return false
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.thorspfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
      Duel.BreakEffect()
