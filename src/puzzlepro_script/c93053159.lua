...
function s.Alba_System_Drugmata_Fusion_Filter(c, mg, fc, tp, chkf, gc)
  if not c:IsFusionCode(68468459) and not c:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then
    return false
  end
...
function s.Alba_System_Drugmata_Fusion_Gcheck(g, fc, tp, ec, chkf, gc)
  ...
  return g:GetClassCount(Card.GetFusionCode) == g:GetCount() and (chkf == PLAYER_NONE or Duel.GetLocationCountFromEx(tp, tp, sg, fc) > 0)
end
...
function s.tgcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION) and g:GetClassCount(Card.GetCode) > 5
end
function s.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToGrave, tp, LOCATION_EXTRA, LOCATION_EXTRA, 1, nil)
  end
