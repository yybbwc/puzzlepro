...
function c73694478.filter(c, e, tp, ft)
  return c:IsFaceup() and c:IsSetCard(0x10cf) and (ft > 0 or c:GetSequence() < 5) and Duel.IsExistingMatchingCard(c73694478.spfilter, tp, LOCATION_HAND, 0, 1, nil, c:GetCode(), e, tp)
end
...
function c73694478.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c73694478.filter(chkc, e, tp, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c73694478.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp, ft)
  end
...
function c73694478.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c73694478.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c73694478.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
