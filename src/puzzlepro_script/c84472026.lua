...
function c84472026.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(84472026) == 0
  end
...
function c84472026.indestg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c84472026.sfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c84472026.sfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
