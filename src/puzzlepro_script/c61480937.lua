...
function s.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_BEAST + RACE_FAIRY + RACE_PLANT) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
...
function s.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function s.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or not aux.NecroValleyFilter()(tc) then
    return
  end
