...
function c30794966.rfilter(c)
  return c:IsRace(RACE_DRAGON) and c:IsAbleToRemoveAsCost() and (c:IsAttribute(ATTRIBUTE_LIGHT) or c:IsType(TYPE_NORMAL))
end
function c30794966.hspcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:CheckSubGroup(aux.gffcheck, 2, 2, Card.IsAttribute, ATTRIBUTE_LIGHT, Card.IsType, TYPE_NORMAL)
end
...
function c30794966.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and Duel.CheckReleaseGroupEx(tp, c30794966.repfilter, 1, REASON_EFFECT, false, c)
  end
