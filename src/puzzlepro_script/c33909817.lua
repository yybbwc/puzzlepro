...
function c33909817.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDiscardDeck(tp, 1) and Duel.IsExistingMatchingCard(Card.IsAbleToHand, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c33909817.cfilter(c, ft)
  return (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsRace(RACE_PLANT) and c:IsAbleToGraveAsCost() and (ft > 0 or c:GetSequence() < 5)
end
function c33909817.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c33909817.cfilter, tp, loc, 0, 1, e:GetHandler(), ft)
  end
...
function c33909817.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c33909817.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c33909817.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
