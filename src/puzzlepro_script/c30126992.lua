...
function c30126992.tdcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30126992.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c30126992.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c30126992.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30126992.filter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c30126992.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFacedown() then
    ...
    if Duel.GetFieldGroupCount(tp, LOCATION_DECK, 0) == 0 then
      ...
      if tc:IsExtraDeckMonster() or Duel.SelectOption(tp, aux.Stringid(30126992, 1), aux.Stringid(30126992, 2)) == 0 then
