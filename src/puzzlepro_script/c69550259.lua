...
function c69550259.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rc:IsRelateToEffect(e) and rc:IsFaceup() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 69550260, 0x2016, TYPES_TOKEN_MONSTER, 0, 0, lv, RACE_MACHINE, ATTRIBUTE_WIND)
  end
...
function c69550259.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not rc:IsRelateToChain() or rc:IsFacedown() then
    return
  end
  local lv = rc:GetOriginalLevel()
