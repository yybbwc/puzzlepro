...
function c4740489.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4740489.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c4740489.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c4740489.atcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(d)
  return a:IsRace(RACE_ROCK) and a:IsAttribute(ATTRIBUTE_EARTH) and d:IsRelateToBattle() and d:IsOnField()
end
