...
function c32912040.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32912040.costfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c32912040.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c32912040.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c32912040.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c32912040.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tc and tc:IsFaceup() and tc:IsSetCard(0xe2) and tc:IsAbleToGrave() and tc:IsCanBeEffectTarget(e) and Duel.IsExistingMatchingCard(c32912040.filter, tp, LOCATION_DECK, 0, 1, nil, tp, tc:GetCode())
  end
