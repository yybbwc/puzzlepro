...
function c86976918.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86976918.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c86976918.tdfilter(c, e, tp)
  return c:IsSetCard(0x2016) and c:IsType(TYPE_MONSTER) and c:IsAbleToDeck() and Duel.IsExistingMatchingCard(c86976918.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetLevel(), c:GetCode())
end
function c86976918.spfilter(c, e, tp, lv, code)
  return c:IsLevel(lv) and not c:IsCode(code) and c:IsType(TYPE_SYNCHRO) and c:IsSetCard(0x2016) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c86976918.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c86976918.tdfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86976918.tdfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
