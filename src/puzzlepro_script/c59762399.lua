...
function c59762399.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return at:IsControler(1 - tp) and Duel.GetAttackTarget() == nil and Duel.IsExistingMatchingCard(nil, tp, LOCATION_PZONE, 0, 1, e:GetHandler())
end
...
function c59762399.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c59762399.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c59762399.atkfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
