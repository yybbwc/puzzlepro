...
function c56532353.atcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsSummonType(SUMMON_TYPE_FUSION) and Duel.GetAttacker() == c and c:IsChainAttackable(0) and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_ONFIELD, 0, 1, c)
end
function c56532353.atcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56532353.costfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c56532353.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return g and g:IsExists(c56532353.filter, 1, nil, tp) and Duel.IsChainNegatable(ev)
end
