...
function c25920413.spfilter(c, tp)
  return c:IsLevelBelow(3) and c:IsFaceup() and c:IsReleasable(REASON_SPSUMMON) and Duel.GetMZoneCount(1 - tp, c, tp)
end
function c25920413.spcon(e, c)
  ...
  return Duel.GetActivityCount(tp, ACTIVITY_NORMALSUMMON) == 0 and Duel.IsExistingMatchingCard(c25920413.spfilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
end
