...
function c52430902.filter1(c)
  ...
  return c:IsFaceup() and c:IsRace(RACE_PSYCHO) and not c:IsCode(52430902) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
...
function c52430902.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c52430902.filter2, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c52430902.filter1, tp, LOCATION_MZONE, 0, 1, nil)
  end
