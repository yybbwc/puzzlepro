...
function c38179121.fgoal(sg, e, tp)
  return sg:FilterCount(Card.IsRace, nil, RACE_DINOSAUR) == 1 and Duel.IsExistingMatchingCard(c38179121.filter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, sg, e, tp)
end
...
function c38179121.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rg:CheckSubGroup(c38179121.fgoal, 2, 2, e, tp)
  end
...
function c38179121.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c38179121.filter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
