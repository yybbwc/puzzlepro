...
function c95440946.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95440946.costfilter, tp, LOCATION_HAND, 0, 1, c) and c:IsAbleToGraveAsCost()
  end
...
function c95440946.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsAbleToGrave()
  end
  if chk == 0 then
...
function c95440946.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95440946.costfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
...
function c95440946.thop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:IsRelateToEffect(e) and Duel.SendtoHand(c, nil, REASON_EFFECT) ~= 0 and c:IsLocation(LOCATION_HAND) and Duel.IsExistingMatchingCard(c95440946.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(95440946, 2)) then
    Duel.ShuffleHand(tp)
    Duel.BreakEffect()
