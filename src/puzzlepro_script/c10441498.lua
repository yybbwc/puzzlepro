...
function c10441498.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c10441498.lvfilter(chkc, e:GetLabel())
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c10441498.cfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c10441498.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c10441498.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c10441498.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
