...
function c1003840.cfilter(c, e, tp, ft)
  local lv = c:GetLevel()
  return lv > 0 and c:IsType(TYPE_TUNER) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c1003840.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, lv)
end
...
function c1003840.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c1003840.cfilter, 1, nil, e, tp, ft)
  end
...
function c1003840.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetTurnPlayer() ~= tp and ec:IsPreviousLocation(LOCATION_EXTRA) and ec:IsPreviousControler(tp) and ec:IsType(TYPE_SYNCHRO)
end
