...
function c30303854.sumop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 30303854) ~= 0 then
    return
  end
...
function c30303854.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c30303854.cfilter(chkc, tp)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingTarget(c30303854.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c30303854.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local c = e:GetHandler()
  if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) ~= 0 and c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) ~= 0 then
    local e1 = Effect.CreateEffect(c)
