...
function c25090294.costfilter(c, tp)
  return c:IsCode(62107981) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
function c25090294.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c25090294.costfilter, 1, nil, tp)
  end
