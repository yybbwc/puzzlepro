...
function c99666430.ctrtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c99666430.ctrfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c99666430.ctrfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c99666430.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroupEx(tp, c99666430.rlfilter, 1, REASON_EFFECT, false, nil, tp)
  end
