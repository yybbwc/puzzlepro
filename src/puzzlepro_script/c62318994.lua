...
function s.chcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == 1 - tp and re:IsActiveType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(s.confilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function s.chtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.repfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
end
...
function s.thoscon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == tp and Duel.IsExistingMatchingCard(s.rccfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function s.thosop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) then
    if ft > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not c:IsAbleToHand() or Duel.SelectOption(tp, 1190, 1152) == 1) then
