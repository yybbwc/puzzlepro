...
function c16556849.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16556849.costfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c16556849.tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c16556849.filter(chkc, e:GetHandler():GetAttack())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c16556849.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e:GetHandler():GetAttack())
  end
...
function c16556849.op(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:GetAttack() > c:GetAttack() then
