...
function c94185340.filter1(c, e, tp)
  local rk = c:GetRank()
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:GetOverlayCount() >= 2 and Duel.IsExistingMatchingCard(c94185340.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, rk + 1)
end
...
function c94185340.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c94185340.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsPlayerCanRemove(tp) and Duel.IsExistingTarget(c94185340.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
