...
function c92510265.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c92510265.rgfilter, tp, LOCATION_REMOVED, 0, 1, nil) and Duel.IsExistingTarget(Card.IsAbleToRemove, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c92510265.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c92510265.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c92510265.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
