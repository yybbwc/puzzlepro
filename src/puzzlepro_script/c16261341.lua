...
function c16261341.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local ct = -ft + 1
  if chk == 0 then
    return ft > -2 and rg:GetCount() > 1 and (ft > 0 or rg:IsExists(c16261341.mzfilter, ct, nil))
  end
  local g = nil
  if ft > 0 then
...
function c16261341.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16261341.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
