...
function c65430555.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE, 0) > 0 and Duel.IsExistingTarget(c65430555.tdfilter1, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(c65430555.tdfilter2, tp, 0, LOCATION_GRAVE, 1, nil)
  end
...
function c65430555.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or Duel.GetLocationCount(tp, LOCATION_MZONE, 0) <= 0 or Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) == 0 then
    return
  end
...
function c65430555.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c65430555.schfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
