...
function s.discon(e, tp, eg, ep, ev, re, r, rp)
  if rp ~= 1 - tp or not (re:IsActiveType(TYPE_MONSTER) and Duel.IsChainDisablable(ev)) then
    return false
  end
...
function s.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsAttackAbove(1000) and tc:IsDefenseAbove(1000)) or tc:IsImmuneToEffect(e) then
    return
  end
