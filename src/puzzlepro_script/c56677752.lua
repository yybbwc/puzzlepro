...
function c56677752.negtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56677752.negfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56677752.negop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoExtraP(tc, nil, REASON_EFFECT) and tc:IsLocation(LOCATION_EXTRA) and Duel.NegateAttack() and tc:GetAttack() > 0 and Duel.SelectYesNo(tp, aux.Stringid(56677752, 1)) then
    Duel.BreakEffect()
...
function c56677752.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c56677752.spfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c56677752.spfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
