...
function c41044418.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41044418.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c41044418.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41044418.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c41044418.rbtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 41044418) == 0
  end
end
function c41044418.rbop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 41044418) ~= 0 then
    return
  end
  local c = e:GetHandler()
  --rebirth
