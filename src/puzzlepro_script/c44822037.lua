...
function c44822037.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 44822037, 0, TYPES_EFFECT_TRAP_MONSTER, 1800, 1800, 4, RACE_FAIRY, ATTRIBUTE_LIGHT)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, e:GetHandler(), 1, 0, 0)
end
function c44822037.activate(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 44822037, 0, TYPES_EFFECT_TRAP_MONSTER, 1800, 1800, 4, RACE_FAIRY, ATTRIBUTE_LIGHT) then
    return
  end
...
function c44822037.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44822037.discfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
