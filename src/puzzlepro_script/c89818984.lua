...
function c89818984.hspfilter(c, tp)
  return c:IsSetCard(0xe6) and c:IsLevel(11) and not c:IsCode(89818984) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c89818984.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) ~= 0 then
    ...
    if tc:IsType(TYPE_MONSTER) and tc:IsSetCard(0xe6) then
      if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(89818984, 1)) then
...
function c89818984.syncheck(c, g, mg, tp, lv, syncard, minc, maxc)
  g:AddCard(c)
  local ct = g:GetCount()
  local res = c89818984.syngoal(g, tp, lv, syncard, minc, ct) or (ct < maxc and mg:IsExists(c89818984.syncheck, 1, g, g, mg, tp, lv, syncard, minc, maxc))
  g:RemoveCard(c)
  return res
end
function c89818984.syngoal(g, tp, lv, syncard, minc, ct)
  return ct >= minc and Duel.GetLocationCountFromEx(tp, tp, g, syncard) > 0 and (g:CheckWithSumEqual(Card.GetSynchroLevel, lv, ct, ct, syncard) or g:CheckWithSumEqual(c89818984.cardiansynlevel, lv, ct, ct, syncard)) and aux.MustMaterialCheck(g, tp, EFFECT_MUST_BE_SMATERIAL)
end
function c89818984.syntg(e, syncard, f, min, max)
  ...
  if lv <= c:GetLevel() and lv <= c89818984.cardiansynlevel(c) then
    return false
  end
