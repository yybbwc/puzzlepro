...
function c28529976.cfilter(c, e, tp, ft)
  return c:IsLevelBelow(2) and c:IsRace(RACE_PLANT) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c28529976.filter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, c:GetLevel() + 3, e, tp)
end
...
function c28529976.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c28529976.cfilter, 1, nil, e, tp, ft)
  end
