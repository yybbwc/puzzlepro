...
function c60025883.costfilter(c, e, tp)
  return c:IsType(TYPE_SYNCHRO) and c:IsAbleToRemoveAsCost() and (c:IsSetCard(0xc2) or c:IsRace(RACE_DRAGON) and (c:IsLevel(7) or c:IsLevel(8))) and Duel.IsPlayerCanSpecialSummonMonster(tp, 60025884, 0, TYPES_TOKEN_MONSTER, c:GetAttack(), c:GetDefense(), c:GetLevel(), c:GetRace(), c:GetAttribute())
end
function c60025883.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c60025883.costfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c60025883.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE, tp, LOCATION_REASON_TOFIELD, zone) > 0 and zone ~= 0
  end
...
function c60025883.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE, tp, LOCATION_REASON_TOFIELD, zone) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 60025884, 0, TYPES_TOKEN_MONSTER, atk, def, lv, race, att) then
    return
  end
