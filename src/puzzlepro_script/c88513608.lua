...
function c88513608.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetActivityCount(tp, ACTIVITY_SUMMON) == 0 and Duel.GetActivityCount(tp, ACTIVITY_FLIPSUMMON) == 0 and Duel.GetActivityCount(tp, ACTIVITY_SPSUMMON) == 0 and Duel.GetFlagEffect(tp, 88513608) == 0
  end
...
function c88513608.activate(e, tp, eg, ep, ev, re, r, rp)
  if not c88513608.check(tp) then
    return
  end
