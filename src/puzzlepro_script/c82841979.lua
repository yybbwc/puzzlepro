...
function c82841979.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c82841979.spfilter, c:GetControler(), LOCATION_GRAVE, 0, 1, nil)
end
...
function c82841979.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c82841979.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82841979.filter, tp, 0, LOCATION_ONFIELD, 1, e:GetHandler())
  end
