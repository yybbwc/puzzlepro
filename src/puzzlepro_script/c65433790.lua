...
function c65433790.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c65433790.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c65433790.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c65433790.spop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(ep, LOCATION_MZONE) <= 0 then
    return
  end
