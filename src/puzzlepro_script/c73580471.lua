...
function c73580471.poscost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73580471.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c73580471.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and chkc:IsDefensePos()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsDefensePos, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c73580471.posop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc or tc:IsAttackPos() or not tc:IsRelateToEffect(e) then
    return
  end
