...
function c21637210.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD + LOCATION_GRAVE) and chkc:IsControler(1 - tp) and chkc:IsAbleToHand()
  end
  local g = Duel.GetMatchingGroup(c21637210.filter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, nil)
  if chk == 0 then
    return #g > 0 and Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_ONFIELD + LOCATION_GRAVE, 1, nil)
  end
  local ct = 0
...
function c21637210.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c21637210.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c21637210.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
