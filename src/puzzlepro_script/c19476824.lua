...
function c19476824.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rg:FilterCount(Card.IsAbleToRemove, nil) == 3
  end
...
function c19476824.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c19476824.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
