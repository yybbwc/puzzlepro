...
namespace ygo {
  ...
  struct Deck {
    ...
    Deck(const Deck &ndeck) : main(ndeck.main), extra(ndeck.extra), side(ndeck.side) {
    }
  ...
  public:
    ...
    static int LoadDeck(Deck &deck, int *dbuf, int mainc, int sidec, bool is_packlist = false);
    static int LoadDeckFromStream(Deck &deck, std::istringstream &deckStream, bool is_packlist = false);
    static bool LoadSide(Deck &deck, int *dbuf, int mainc, int sidec);
    ...
    static bool SaveDeckBuffer(const int deckbuf[], const wchar_t *name);
  };
