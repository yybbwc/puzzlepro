...
function c75524092.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tg and tg:IsReason(REASON_BATTLE)
  end
  return true
end
function c75524092.desrepop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Destroy(g, REASON_EFFECT) > 0 and Duel.Damage(1 - tp, 600, REASON_EFFECT) ~= 0 then
    if Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 75524093, 0, TYPES_TOKEN_MONSTER, 2500, 2500, 7, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP, 1 - tp) then
