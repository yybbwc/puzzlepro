...
function c98093548.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98093548.costfilter, tp, LOCATION_ONFIELD, 0, 1, nil) or Duel.IsPlayerAffectedByEffect(tp, 46241344)
  end
  if Duel.IsExistingMatchingCard(c98093548.costfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and (not Duel.IsPlayerAffectedByEffect(tp, 46241344) or not Duel.SelectYesNo(tp, aux.Stringid(46241344, 0))) then
...
function c98093548.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c98093548.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c98093548.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c98093548.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98093548.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
