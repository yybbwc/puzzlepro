...
function c85360035.cfilter(c, tp)
  return c:GetFlagEffect(85360035) ~= 0 and c:IsReason(REASON_DESTROY) and c:IsControler(tp) and c:IsFaceup() and c:IsSetCard(0x14f) and c:IsType(TYPE_FUSION) and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)
end
function c85360035.regcon(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c85360035.cfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c85360035.cfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c85360035.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c85360035.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85360035.desfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c85360035.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
