...
function c29843091.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 2 and Duel.IsPlayerCanSpecialSummonMonster(tp, 29843092, 0xf, TYPES_TOKEN_MONSTER, 0, 1000, 2, RACE_BEAST, ATTRIBUTE_LIGHT, POS_FACEUP_DEFENSE, 1 - tp)
  end
...
function c29843091.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) < 3 then
    return
  end
