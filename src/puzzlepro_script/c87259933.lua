...
function c87259933.cfilter(c, tp)
  return c:IsSetCard(0x1034) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp)
end
...
function c87259933.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c87259933.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c87259933.thfilter1(c, rc)
  return c:IsFaceup() and c:IsSetCard(0x1034) and c:IsAbleToHand() and Duel.IsExistingTarget(c87259933.thfilter2, 0, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, c, rc)
end
...
function c87259933.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_CHAINING) and Duel.IsExistingTarget(c87259933.thfilter1, tp, LOCATION_ONFIELD, 0, 1, nil, e:GetHandler())
  end
