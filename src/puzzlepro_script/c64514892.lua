...
function c64514892.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c64514892.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.IsExistingTarget(c64514892.desfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c64514892.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and c64514892.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c64514892.thfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
