...
function c41546.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41546.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c41546.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_PZONE) and chkc:IsControler(tp) and c41546.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41546.desfilter, tp, LOCATION_PZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c41546.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
