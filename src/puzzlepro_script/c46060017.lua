...
function c46060017.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    ...
    e:SetLabel(loc)
    return Duel.IsExistingTarget(Card.IsFaceup, tp, loc, 0, 1, nil) and Duel.IsExistingMatchingCard(c46060017.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c46060017.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c46060017.matfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46060017.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanOverlay()
  end
