...
function c7736719.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 2) and Duel.IsExistingMatchingCard(Card.IsAbleToDeck, tp, LOCATION_HAND, 0, 2, nil)
  end
...
function c7736719.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() < 2 or not Duel.IsPlayerCanDraw(tp) then
    return
  end
