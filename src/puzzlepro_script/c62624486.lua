...
function c62624486.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and tc:GetReasonCard() == e:GetHandler() and tc:IsLocation(LOCATION_GRAVE) and tc:IsReason(REASON_BATTLE)
end
...
function c62624486.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c62624486.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
