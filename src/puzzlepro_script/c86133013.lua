...
function c86133013.tgfilter(c, e, tp, rp, g, ft)
  local lv = c:GetLevel()
  if not ((c:IsReason(REASON_BATTLE) or (rp == 1 - tp and c:IsReason(REASON_EFFECT))) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and c:IsCanBeEffectTarget(e) and lv > 0) then
    return false
  end
...
function c86133013.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return eg:IsContains(chkc) and c86133013.tgfilter(chkc, e, tp, rp, g, ft)
  end
  if chk == 0 then
    return ft > 0 and eg:IsExists(c86133013.tgfilter, 1, nil, e, tp, rp, g, ft)
  end
  local tg
  if #eg == 1 then
    tg = eg
