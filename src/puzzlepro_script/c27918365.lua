...
function c27918365.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsSummonLocation(LOCATION_EXTRA) and loc == LOCATION_MZONE and Duel.IsChainNegatable(ev)
end
...
function c27918365.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27918365.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
