...
function c68987122.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c68987122.costfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c68987122.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c68987122.spfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp, tid)
  end
...
function c68987122.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c68987122.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68987122.posfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
