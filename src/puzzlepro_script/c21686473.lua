...
function c21686473.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21686473.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c21686473.hspfilter(c, tp)
  return c:IsSetCard(0x10af) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
