...
function c62950604.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c62950604.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62950604.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c62950604.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  local c = e:GetHandler()
  e:SetLabel(0)
  return tc and act == 1 and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and c:GetFlagEffect(62950604) ~= 0
end
function c62950604.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local tc = e:GetLabelObject()
  if chk == 0 then
    return tc:GetFlagEffect(62950604) ~= 0
  end
