...
function c81977953.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:CheckSubGroupEach(c81977953.spchecks)
end
...
function c81977953.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c81977953.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81977953.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
