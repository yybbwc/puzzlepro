...
function c27107590.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27107590.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c27107590.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetActivityCount(tp, ACTIVITY_SPSUMMON) == 0 and c:IsAbleToRemoveAsCost()
  end
...
function c27107590.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c27107590.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
