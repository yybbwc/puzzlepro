...
function c42469671.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dc ~= 0 and Duel.IsTurnPlayer(tp) and Duel.IsMainPhase() and Duel.IsPlayerCanDraw(tp, dc) and Duel.SelectYesNo(tp, aux.Stringid(42469671, 0)) then
    Duel.BreakEffect()
...
function c42469671.checkop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 42469671) ~= 0 then
    return
  end
