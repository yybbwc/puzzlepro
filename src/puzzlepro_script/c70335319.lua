...
function c70335319.atcon(e, tp, eg, ep, ev, re, r, rp)
  local at = Duel.GetAttacker()
  return at:IsControler(tp) and Duel.GetAttackTarget() ~= nil and at:IsRace(RACE_DRAGON) and at:IsType(TYPE_PENDULUM) and at:IsChainAttackable(0)
end
function c70335319.atcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return c70335319[tp] == 0
  end
  local at = Duel.GetAttacker()
...
function c70335319.hspcon(e, c)
  ...
  return g:GetCount() >= 2 and g:FilterCount(Card.IsReleasable, nil, REASON_SPSUMMON) == g:GetCount() and g:IsExists(c70335319.hspfilter, 1, nil) and (c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, g, c) > 0 or c:IsLocation(LOCATION_HAND) and Duel.GetMZoneCount(tp, g, tp) > 0)
end
...
function c70335319.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c70335319.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
