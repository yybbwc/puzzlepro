...
function c40619741.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c40619741.filter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():GetEquipCount() > 0 and Duel.IsExistingTarget(c40619741.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
