...
function c32909498.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32909498.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c32909498.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c32909498.rmfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c32909498.rmfilter, tp, 0, LOCATION_ONFIELD, 1, nil, tp)
  end
...
function c32909498.rmcon(e, tp, eg, ep, ev, re, r, rp)
  --~ aux.DebugHint(rp == 1 - tp and re:IsActiveType(TYPE_MONSTER))
...
function c32909498.rmtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  aux.DebugHint("2")
  if chk == 0 then
    return rp == 1 - tp and Duel.IsExistingTarget(c32909498.rmfilter, tp, 0, LOCATION_ONFIELD, 1, nil, tp)
  end
  aux.DebugHint("3")
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
  local g = Duel.SelectTarget(tp, c32909498.rmfilter, tp, 0, LOCATION_ONFIELD, 1, 1, nil, tp)
