...
function c96746083.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -2 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:GetCount() >= 2 and g:IsExists(Card.IsAttribute, 1, nil, ATTRIBUTE_FIRE) and (ft ~= 0 or g:IsExists(c96746083.mzfilter, 1, nil, tp))
  end
...
function c96746083.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() < 2 or not g:IsExists(Card.IsAttribute, 1, nil, ATTRIBUTE_FIRE) then
    return
  end
  local g1 = nil
...
function c96746083.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96746083.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
