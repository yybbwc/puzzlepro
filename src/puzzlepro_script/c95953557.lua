...
function c95953557.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95953557.costfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
...
function c95953557.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  local lv = e:GetLabel()
  if Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) == 1 and lv > 0 and c:GetLevel() ~= lv and Duel.SelectYesNo(tp, aux.Stringid(95953557, 2)) then
    Duel.BreakEffect()
    local e1 = Effect.CreateEffect(c)
...
function c95953557.spfilter(c, e, tp)
  return (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c95953557.spop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  ct = ct + 1
  c:SetTurnCounter(ct)
  if ct == 2 and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c95953557.spfilter), tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(95953557, 3)) then
    Duel.BreakEffect()
