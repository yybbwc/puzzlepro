...
function c60082869.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c60082869.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60082869.filter, tp, 0, LOCATION_ONFIELD, 1, e:GetHandler())
  end
