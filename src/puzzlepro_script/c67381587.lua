...
function c67381587.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() ~= tp and (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE) and Duel.IsExistingMatchingCard(Card.IsPosition, tp, LOCATION_MZONE, 0, 2, nil, POS_FACEDOWN_DEFENSE)
end
function c67381587.activate(e, tp, eg, ep, ev, re, r, rp)
  Duel.SkipPhase(1 - tp, PHASE_BATTLE, RESET_PHASE + PHASE_BATTLE_STEP, 1)
end
function c67381587.posfilter1(c)
  return c:IsFaceup() and c:IsSetCard(0xcc) and c:IsCanTurnSet() and Duel.IsExistingTarget(c67381587.posfilter2, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
...
function c67381587.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c67381587.posfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
