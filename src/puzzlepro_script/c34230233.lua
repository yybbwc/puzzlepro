...
function c34230233.spfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x6) and not c:IsCode(34230233) and c:IsAbleToHandAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c34230233.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) ~= 0 and rp == 1 - tp and tp == e:GetLabel() then
    ...
    if hg:GetCount() > 0 then
      ...
      if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and cc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(34230233, 1)) then
