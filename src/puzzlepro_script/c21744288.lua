...
function c21744288.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(c21744288.costfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil, tp)
  end
...
function c21744288.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c21744288.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c21744288.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21744288.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
