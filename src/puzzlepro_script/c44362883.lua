...
function c44362883.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(44362883, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c44362883.filter2(c, e, tp, m, f, chkf)
  if not (c:IsType(TYPE_FUSION) and aux.IsMaterialListCode(c, 68468459) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false)) then
    return false
  end
