...
function c99092624.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    for p = 0, 1 do
      if Duel.GetLocationCount(p, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 71645243, 0, TYPES_TOKEN_MONSTER, 800, 800, 2, RACE_PLANT, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE, p) then
        return true
      end
    end
    return false
...
function c99092624.rmfilter(c)
  return c:IsFaceup() and c:IsAbleToRemove() and (c:IsCode(73580471) or c:IsRace(RACE_PLANT))
end
function c99092624.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c99092624.rmfilter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():IsAbleToDeck() and Duel.IsExistingTarget(c99092624.rmfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
