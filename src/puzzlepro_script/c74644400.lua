...
function c74644400.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74644400.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c74644400.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74644400.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c74644400.spfilter(c, e, tp)
  return aux.IsCodeListed(c, 80280737) and not c:IsCode(74644400) and c:IsLevelAbove(1) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c74644400.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c74644400.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c74644400.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
