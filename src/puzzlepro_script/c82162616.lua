...
function c82162616.filter1(c, e, tp)
  local rk = c:GetRank()
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c82162616.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c, rk + 4) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL)
end
function c82162616.filter2(c, e, tp, mc, rk)
  return c:IsRank(rk) and c:IsSetCard(0x55, 0x7b) and mc:IsCanBeXyzMaterial(c) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c82162616.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c82162616.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82162616.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
