...
function c11711438.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c11711438.tgfilter(chkc, tp, g)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c11711438.tgfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp, g)
  end
...
function c11711438.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c11711438.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
