...
function c61322713.filter(c, e, tp)
  return c:IsFaceup() and c:IsAttribute(ATTRIBUTE_DARK) and c:GetOriginalLevel() > 0 and Duel.IsExistingMatchingCard(c61322713.spfilter, tp, LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil, e, tp, c)
end
function c61322713.spfilter(c, e, tp, tc)
  ...
  return c:IsAttribute(ATTRIBUTE_LIGHT) and c:GetOriginalRace() == tc:GetOriginalRace() and c:GetOriginalLevel() == tc:GetOriginalLevel() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (b1 or b2)
end
function c61322713.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c61322713.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61322713.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c61322713.cost2(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c61322713.cost1(e, tp, eg, ep, ev, re, r, rp, 0) and c:IsAbleToRemove()
  end
