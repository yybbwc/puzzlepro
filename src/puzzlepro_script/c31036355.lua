...
function c31036355.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c31036355.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c31036355.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_CONTROL, nil, 0, 0, 0)
end
function c31036355.activate(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not Duel.IsExistingMatchingCard(c31036355.filter, tp, LOCATION_MZONE, 0, 1, nil) or not Duel.IsExistingMatchingCard(c31036355.filter, tp, 0, LOCATION_MZONE, 1, nil) then
    return
  end
