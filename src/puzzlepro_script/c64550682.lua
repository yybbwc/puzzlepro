...
function c64550682.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c64550682.spfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c64550682.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c64550682.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c64550682.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c64550682.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c64550682.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
