...
function c75252099.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and c == Duel.GetAttackTarget() and c:IsPreviousControler(tp) and c:GetBattlePosition() == POS_FACEUP_ATTACK
end
...
function c75252099.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingMatchingCard(c75252099.filter, tp, LOCATION_DECK, 0, 2, nil, e, tp)
  end
