...
function c69680031.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (ph == PHASE_MAIN1 or ph == PHASE_MAIN2) and Duel.IsExistingMatchingCard(c69680031.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
end
...
function c69680031.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsExistingMatchingCard(c69680031.ofilter, tp, LOCATION_MZONE, 0, 1, c) and Duel.IsExistingMatchingCard(aux.NegateMonsterFilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(69680031, 2)) then
    Duel.BreakEffect()
...
function c69680031.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69680031.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
