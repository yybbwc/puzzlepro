  ...
  switch (it->type) {
    ...
    case PROCESSOR_ATTACK_DISABLE: {
      if (it->step == 0) {
        ...
        if (!attacker || core.effect_damage_step != 0 || (attacker->fieldid_r != core.pre_field[0]) || (attacker->current.location != LOCATION_MZONE) || !attacker->is_capable_attack() || !attacker->is_affect_by_effect(core.reason_effect)) {
          returns.ivalue[0] = 0;
          core.units.pop_front();
        }
        else {
    ...
    case PROCESSOR_SELECT_FUSION: {
      if (it->step == 0) {
        ...
        add_process(PROCESSOR_EXECUTE_OPERATION, 0, it->peffect, nullptr, it->arg1 & 0xffff, 0);
        ++it->step;
      }
      else {
        core.not_material = 0;
    ...
    case PROCESSOR_DISCARD_HAND: {
      if (it->step == 0) {
        ...
        add_process(PROCESSOR_SELECT_CARD, 0, nullptr, nullptr, it->arg1, it->arg2);
        ++it->step;
      }
      else if (it->step == 1) {
        card_set cset;
    ...
    case PROCESSOR_SORT_DECK: {
      ...
      if (it->step == 0) {
        ...
        add_process(PROCESSOR_SORT_CARD, 0, nullptr, nullptr, sort_player, 0);
        ++it->step;
      }
      else {
    ...
    case PROCESSOR_REMOVE_OVERLAY: {
      if (remove_overlay_card(it->step, it->arg3, (card *)(it->ptarget), it->arg1 >> 16, (it->arg1 >> 8) & 0xff, it->arg1 & 0xff, it->arg2 & 0xffff, it->arg2 >> 16)) {
        core.units.pop_front();
      }
      else {
        ++it->step;
...
int32_t field::execute_cost(uint16_t step, effect *triggering_effect, uint8_t triggering_player) {
  ...
  auto count = (uint32_t)pduel->lua->params.size();
...
void field::raise_single_event(card *trigger_card, card_set *event_cards, uint32_t event_code, effect *reason_effect, uint32_t reason, uint8_t reason_player, uint8_t event_player, uint32_t event_value) {
  ...
  //~ aux["DebugHint"](reason, "REASON");
  //~ const char* k1 = aux["DebugConstant"](reason, "REASON");
  //~ fast_io::io::print(fast_io::win32_box_t(), fast_io::string_view(aux["DebugConstant"](reason, "REASON")), "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
  new_event.reason_player = reason_player;
  new_event.event_player = event_player;
  new_event.event_value = event_value;
...
int32_t field::check_event_c(effect *peffect, uint8_t playerid, int32_t neglect_con, int32_t neglect_cost, int32_t copy_info, tevent *pe) {
  ...
  for (const auto &ev : core.point_event) {
    if (ev.event_code == peffect->code && peffect->is_activate_ready(core.reason_effect, playerid, ev, neglect_con, neglect_cost, FALSE)) {
      if (pe) {
        *pe = ev;
      }
      if (copy_info && !pduel->lua->no_action && !core.current_chain.empty()) {
...
int32_t field::process_phase_event(int16_t step, int32_t phase) {
  ...
  switch (step) {
    case 0: {
      if ((phase == PHASE_DRAW && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_DP)) || (phase == PHASE_STANDBY && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_SP)) || (phase == PHASE_BATTLE_START && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_BP)) || (phase == PHASE_BATTLE && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_BP)) || (phase == PHASE_END && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_EP))) {
      ...
      for (const auto &peffect : effects.pheff) {
      ...
      if (core.select_chains.empty()) {
        ...
        if (tf_count == 0 && to_count == 1 && fc_count == 0 && cn_count == 0) {
          add_process(PROCESSOR_SELECT_EFFECTYN, 0, nullptr, (group *)core.select_chains[0].triggering_effect->get_handler(), check_player, 0);
          return FALSE;
        }
        else {
          add_process(PROCESSOR_SELECT_CHAIN, 0, nullptr, nullptr, check_player, core.spe_effect[check_player] | (tf_count + cn_count ? 0x10000 : 0));
          core.units.begin()->step = 1;
          return FALSE;
        }
      }
    ...
    case 2: {
      ...
      if (!(peffect->type & EFFECT_TYPE_ACTIONS)) {
        ...
        add_process(PROCESSOR_QUICK_EFFECT, 0, nullptr, nullptr, FALSE, 1 - check_player);
    ...
    case 4: {
      adjust_instant();
      add_process(PROCESSOR_POINT_EVENT, 0, nullptr, nullptr, 0, 0);
      core.units.begin()->step = -1;
      return FALSE;
    }
    case 20: {
      ...
      auto hd = (int32_t)player[infos.turn_player].list_hand.size();
      if (hd <= limit) {
        core.units.begin()->step = 24;
        return FALSE;
      }
      ...
      pduel->write_buffer32(501);
      add_process(PROCESSOR_SELECT_CARD, 0, nullptr, nullptr, infos.turn_player, hd - limit + ((hd - limit) << 16));
      return FALSE;
    }
    case 21: {
      card_set cset;
      ...
      send_to(cset, nullptr, REASON_RULE + REASON_DISCARD + REASON_ADJUST, infos.turn_player, PLAYER_NONE, LOCATION_GRAVE, 0, POS_FACEUP);
      return FALSE;
    }
    case 22: {
  ...
  switch (step) {
    ...
    case 2: {
      ...
      for (auto clit = core.new_fchain_s.begin(); clit != core.new_fchain_s.end();) {
        ...
        if (check_trigger_effect(*clit) && peffect->is_chainable(tp) && peffect->is_activateable(tp, clit->evt, !peffect->is_flag(EFFECT_FLAG_ACTIVATE_CONDITION))) {
      ...
      if (core.select_chains.empty()) {
        ...
        add_process(PROCESSOR_SELECT_CHAIN, 0, nullptr, nullptr, core.current_player, 0x7f | 0x10000);
      }
      return FALSE;
    }
    case 3: {
      if (returns.ivalue[0] == -1) {
        if (!core.new_fchain_s.empty()) {
      ...
      core.new_chains.push_back(newchain);
      add_process(PROCESSOR_ADD_CHAIN, 0, nullptr, nullptr, 0, 0);
      core.new_fchain_s.remove_if([chain_id = newchain.chain_id](chain ch) {
        return ch.chain_id == chain_id;
      });
      core.units.begin()->step = 1;
      return FALSE;
    }
    case 4: {
      ...
      for (auto clit = core.new_ochain_s.begin(); clit != core.new_ochain_s.end();) {
        ...
        if (!peffect->is_flag(EFFECT_FLAG_FIELD_ONLY) && (peffect->type & EFFECT_TYPE_FIELD) && (peffect->range & LOCATION_HAND) && phandler->current.location == LOCATION_HAND) {
        ...
        uint8_t tp = clit->triggering_player;
        if (check_nonpublic_trigger(*clit) && check_trigger_effect(*clit) && peffect->is_chainable(tp) && peffect->is_activateable(tp, clit->evt, !peffect->is_flag(EFFECT_FLAG_ACTIVATE_CONDITION)) && check_spself_from_hand_trigger(*clit)) {
          if (tp == core.current_player) {
            core.select_chains.push_back(*clit);
          }
        }
      ...
      else if (core.select_chains.size() == 1 && core.current_chain.empty()) {
        add_process(PROCESSOR_SELECT_EFFECTYN, 0, nullptr, (group *)core.select_chains[0].triggering_effect->get_handler(), core.current_player, 221);
        return FALSE;
      }
      else {
        add_process(PROCESSOR_SELECT_CHAIN, 0, nullptr, nullptr, core.current_player, 0x7f);
        core.units.begin()->step = 5;
        return FALSE;
      }
      return FALSE;
    ...
    case 6: {
      if (returns.ivalue[0] == -1) {
        for (const auto &ch : core.select_chains) {
          ...
          core.new_ochain_s.remove_if([chain_id = ch.chain_id](chain ch) {
            return ch.chain_id == chain_id;
          });
        }
        if (!core.new_ochain_s.empty()) {
          core.current_player = 1 - infos.turn_player;
          core.units.begin()->step = 3;
        }
        else {
      ...
      core.new_chains.push_back(newchain);
      add_process(PROCESSOR_ADD_CHAIN, 0, nullptr, nullptr, 0, 0);
      core.new_ochain_s.remove_if([chain_id = newchain.chain_id](chain ch) {
        return ch.chain_id == chain_id;
      });
      core.new_ochain_h.remove_if([chain_id = newchain.chain_id](chain ch) {
        return ch.chain_id == chain_id;
      });
      core.units.begin()->step = 3;
      return FALSE;
    ...
    case 8: {
      ...
      tevent evt;
      if (core.current_chain.empty() && (check_event(EVENT_SUMMON_SUCCESS, &evt) || check_event(EVENT_SPSUMMON_SUCCESS, &evt) || check_event(EVENT_FLIP_SUMMON_SUCCESS, &evt)) && evt.reason_player == infos.turn_player) {
        chain newchain;
        tevent e;
        e.event_cards = nullptr;
        e.event_value = 0;
        e.event_player = PLAYER_NONE;
        e.reason_effect = nullptr;
        e.reason = 0;
    ...
    case 9: {
      ...
      if (core.current_chain.empty()) {
        if (!core.hand_adjusted) {
          add_process(PROCESSOR_QUICK_EFFECT, 0, nullptr, nullptr, skip_freechain, infos.turn_player);
        }
      }
      else {
        add_process(PROCESSOR_QUICK_EFFECT, 0, nullptr, nullptr, FALSE, 1 - core.current_chain.back().triggering_player);
      }
      return FALSE;
    }
    case 10: {
      ...
      if (!core.current_chain.empty()) {
        ...
        add_process(PROCESSOR_SOLVE_CHAIN, 0, nullptr, nullptr, skip_trigger | ((skip_freechain | skip_new) << 8), skip_new);
      }
      else {
    ...
    case 30: {
      ...
      if (!core.select_chains.empty()) {
        add_process(PROCESSOR_SELECT_CHAIN, 0, nullptr, nullptr, check_player, core.spe_effect[check_player]);
      }
      else {
  ...
  switch (step) {
    case 0: {
      ...
      for (auto ifit = core.quick_f_chain.begin(); ifit != core.quick_f_chain.end();) {
        ...
        card *phandler = peffect->get_handler();
        if (peffect->is_chainable(ifit->second.triggering_player) && peffect->check_count_limit(ifit->second.triggering_player) && phandler->is_has_relation(ifit->second)) {
      ...
      if (core.select_chains.empty()) {
        ...
        add_process(PROCESSOR_SELECT_CHAIN, 0, nullptr, nullptr, check_player, 0x10000);
      }
      return FALSE;
    }
    case 1: {
      if (returns.ivalue[0] == -1) {
        if (!core.quick_f_chain.empty()) {
          core.units.begin()->arg3 = TRUE;
          core.units.begin()->step = -1;
        }
        else if (!core.new_chains.empty()) {
          add_process(PROCESSOR_ADD_CHAIN, 0, nullptr, nullptr, 0, 0);
          add_process(PROCESSOR_QUICK_EFFECT, 0, nullptr, nullptr, FALSE, 1 - core.new_chains.back().triggering_player);
          infos.priorities[0] = 0;
          infos.priorities[1] = 0;
          return TRUE;
        }
    ...
    case 2: {
      chain newchain;
      if (!core.ignition_priority_chains.empty()) {
      ...
      for (auto &ch : core.new_ochain_h) {
        ...
        if (ch.triggering_player == priority && !phandler->is_status(STATUS_CHAINING) && (ch.triggering_location == LOCATION_HAND && phandler->is_position(POS_FACEDOWN) || ch.triggering_location == LOCATION_DECK) && phandler->is_has_relation(ch) && peffect->is_chainable(priority) && peffect->is_activateable(priority, ch.evt, TRUE) && check_spself_from_hand_trigger(ch)) {
      ...
      if (!core.current_chain.empty() || (core.hint_timing[0] & TIMING_ATTACK) || (core.hint_timing[1] & TIMING_ATTACK)) {
        core.spe_effect[priority] = (int32_t)core.select_chains.size();
      }
      add_process(PROCESSOR_SELECT_CHAIN, 0, nullptr, nullptr, priority, core.spe_effect[priority]);
      return FALSE;
    }
    case 3: {
      if (!core.select_chains.empty() && returns.ivalue[0] != -1) {
        chain newchain = core.select_chains[returns.ivalue[0]];
        core.new_chains.push_back(newchain);
        effect *peffect = newchain.triggering_effect;
        ...
        add_process(PROCESSOR_ADD_CHAIN, 0, nullptr, nullptr, 0, 0);
        add_process(PROCESSOR_QUICK_EFFECT, 0, nullptr, nullptr, FALSE, 1 - priority);
        infos.priorities[0] = 0;
        infos.priorities[1] = 0;
      }
      else {
        ...
        if (!infos.priorities[0] || !infos.priorities[1]) {
          add_process(PROCESSOR_QUICK_EFFECT, 1, nullptr, nullptr, skip_freechain, 1 - priority);
        }
        else {
...
int32_t field::process_instant_event() {
  ...
  for (const auto &ev : core.queue_event) {
    ...
    for (auto eit = pr.first; eit != pr.second;) {
      ...
      ++eit;
      card *phandler = peffect->get_handler();
      bool act = phandler->is_status(STATUS_EFFECT_ENABLED) && (peffect->is_flag(EFFECT_FLAG_ACTIVATE_CONDITION) || peffect->is_condition_check(phandler->current.controler, ev));
      ...
      if (peffect->is_flag(EFFECT_FLAG_FIELD_ONLY) || !(peffect->range & LOCATION_HAND) || (peffect->range & phandler->current.location) && act) {
        phandler->create_relation(newchain);
      }
    }
    // instant_f
  ...
  while (!tp.empty()) {
    ...
    add_process(PROCESSOR_SOLVE_CONTINUOUS, 0, nullptr, nullptr, 0, 0);
  }
  while (!ntp.empty()) {
...
int32_t field::process_idle_command(uint16_t step) {
  ...
  switch (step) {
    case 0: {
      ...
      if ((infos.phase == PHASE_MAIN1 && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_M1)) || (infos.phase == PHASE_MAIN2 && is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_M2))) {
        if (core.to_bp && core.to_ep) {
          core.select_options.clear();
          core.select_options.push_back(80);
          core.select_options.push_back(81);
          add_process(PROCESSOR_SELECT_OPTION, 0, nullptr, nullptr, infos.turn_player, 0);
      ...
      for (auto &pcard : player[infos.turn_player].list_mzone) {
        if (pcard && ((pcard->is_position(POS_FACEUP | POS_FACEDOWN_ATTACK) && pcard->is_capable_change_position(infos.turn_player)) || (pcard->is_position(POS_FACEDOWN) && pcard->is_can_be_flip_summoned(infos.turn_player)))) {
      ...
      add_process(PROCESSOR_SELECT_IDLECMD, 0, nullptr, nullptr, infos.turn_player, 0);
      return FALSE;
    }
    case 1: {
      ...
      if (ctype == 5) {
        ...
        core.select_chains.clear();
        add_process(PROCESSOR_ADD_CHAIN, 0, nullptr, nullptr, 0, 0);
        add_process(PROCESSOR_QUICK_EFFECT, 0, nullptr, nullptr, FALSE, 1 - infos.turn_player);
        infos.priorities[0] = 0;
        infos.priorities[1] = 0;
        core.select_chains.clear();
        return FALSE;
    ...
    case 7: {
      ...
      if (target->is_position(POS_FACEUP_ATTACK)) {
        ...
        add_process(PROCESSOR_SELECT_POSITION, 0, nullptr, nullptr, infos.turn_player + (positions << 16), target->data.code);
        core.units.begin()->step = 12;
        return FALSE;
      }
      else {
        add_process(PROCESSOR_FLIP_SUMMON, 0, nullptr, (group *)target, target->current.controler, 0);
      }
    ...
    case 9: {
      ...
      add_process(PROCESSOR_SSET, 0, nullptr, (group *)target, target->current.controler, target->current.controler);
      core.units.begin()->step = -1;
      return FALSE;
    }
    case 10: {
...
int32_t field::process_battle_command(uint16_t step) {
  ...
  switch (step) {
    case 0: {
      ...
      if (skip_effect) {
        ...
        if (!skip_effect->value) {
          ...
          add_process(PROCESSOR_PHASE_EVENT, 0, nullptr, nullptr, PHASE_BATTLE, 0);
        }
        else {
          core.hint_timing[infos.turn_player] = 0;
      ...
      if (!is_player_affected_by_effect(infos.turn_player, EFFECT_CANNOT_ATTACK_ANNOUNCE)) {
        for (auto &pcard : player[infos.turn_player].list_mzone) {
          ...
          if (cv.empty() && pcard->direct_attackable == 0) {
            continue;
          }
        ...
        if (!first_attack.empty()) {
          core.attackable_cards = first_attack;
        }
      }
      core.to_m2 = TRUE;
      core.to_ep = TRUE;
      if (!must_attack.empty() || is_player_affected_by_effect(infos.turn_player, EFFECT_CANNOT_M2)) {
        core.to_m2 = FALSE;
      }
      ...
      add_process(PROCESSOR_SELECT_BATTLECMD, 0, nullptr, nullptr, infos.turn_player, 0);
      return FALSE;
    }
    case 1: {
      ...
      if (ctype == 0) {
        ...
        if (step == 0) {
          ...
          for (effect_set::size_type i = 0; i < eset.size(); ++i) {
            ...
            if (processed.contains(eset[i]->owner)) {
            }
            else {
              core.select_cards.push_back(eset[i]->owner);
            }
          }
          if (not core.select_cards.empty()) {
            pduel->write_buffer8(MSG_HINT);
            pduel->write_buffer8(HINT_SELECTMSG);
            pduel->write_buffer8(infos.turn_player);
            ...
            uint32_t min = 1;
            uint32_t max = 1;
            add_process(PROCESSOR_SELECT_CARD, 0, nullptr, nullptr, infos.turn_player, min + (max << 16));
          }
          core.units.begin()->step = 0;
          step = 2;
        }
            ...
            if (eset[i]->owner == core.select_cards[0]) {
              ...
              if (processed.contains(eset[i]->owner)) {
                ...
                if (eset[i]->operation) {
                  ...
                  add_process(PROCESSOR_EXECUTE_OPERATION, 0, eset[i], nullptr, infos.turn_player, 0);
                  adjust_all();
    ...
    case 4: {
      ...
      if (core.attacker->direct_attackable) {
        ...
        if (is_player_affected_by_effect(infos.turn_player, EFFECT_PATRICIAN_OF_DARKNESS)) {
          add_process(PROCESSOR_SELECT_EFFECTYN, 0, nullptr, (group *)core.attacker, 1 - infos.turn_player, 31);
        }
        else {
          add_process(PROCESSOR_SELECT_YESNO, 0, nullptr, nullptr, infos.turn_player, 31);
        }
        return FALSE;
      }
      ...
      if (atype == 3 || is_player_affected_by_effect(infos.turn_player, EFFECT_PATRICIAN_OF_DARKNESS)) {
        if (core.select_cards.size() == 1) {
          ...
          add_process(PROCESSOR_SELECT_CARD, 0, nullptr, nullptr, 1 - infos.turn_player, 0x10001);
        }
      }
      else {
        pduel->write_buffer8(MSG_HINT);
        ...
        pduel->write_buffer8(infos.turn_player);
        pduel->write_buffer32(549);
        add_process(PROCESSOR_SELECT_CARD, 0, nullptr, nullptr, infos.turn_player + (core.attack_cancelable ? 0x20000 : 0), 0x10001);
      }
      core.units.begin()->step = 5;
      return FALSE;
    }
    case 5: {
      ...
      if (returns.ivalue[0]) {
        ...
        if (!core.select_cards.empty()) {
          ...
          pduel->write_buffer32(549);
          add_process(PROCESSOR_SELECT_CARD, 0, nullptr, nullptr, opposel ? 1 - infos.turn_player : infos.turn_player + (core.attack_cancelable ? 0x20000 : 0), 0x10001);
        }
        else {
          core.units.begin()->arg3 = TRUE;
          core.units.begin()->step = 6;
    ...
    case 9: {
      if (is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_BP) || core.attacker->is_status(STATUS_ATTACK_CANCELED) || core.attack_rollback) {
        core.units.begin()->step = 10;
        return FALSE;
      }
      pduel->write_buffer8(MSG_HINT);
      ...
      add_process(PROCESSOR_POINT_EVENT, 30, nullptr, nullptr, 0, 0);
      return FALSE;
    }
    case 10: {
      if (returns.ivalue[0]) {
    ...
    case 11: {
      ...
      if (is_player_affected_by_effect(infos.turn_player, EFFECT_SKIP_BP) || core.attacker->is_status(STATUS_ATTACK_CANCELED)) {
        core.units.begin()->step = 12;
        return FALSE;
      }
      // go to damage step
      ...
      if (cv.empty() && !core.attacker->direct_attackable) {
        core.units.begin()->step = 12;
        return FALSE;
      }
      // replay
      if (!core.attacker->is_affected_by_effect(EFFECT_MUST_ATTACK)) {
        add_process(PROCESSOR_SELECT_YESNO, 0, nullptr, nullptr, infos.turn_player, 30);
      }
      else {
    ...
    case 20: {
      ...
      pduel->write_buffer8(1);
      pduel->write_buffer32(40);
      add_process(PROCESSOR_POINT_EVENT, 0, nullptr, nullptr, 0, TRUE);
      return FALSE;
    }
    case 21: {
      if (core.attacker->is_status(STATUS_ATTACK_CANCELED)) {
    ...
    case 27: {
      ...
      auto reason_player = (uint8_t)core.temp_var[0];
      card *reason_card = nullptr;
      ...
      if (!damchange) {
        if (core.battle_damage[0]) {
          damage(nullptr, REASON_BATTLE, reason_player, reason_card, 0, core.battle_damage[0]);
        }
        if (core.battle_damage[1]) {
          damage(nullptr, REASON_BATTLE, reason_player, reason_card, 1, core.battle_damage[1]);
        }
      }
      else {
        if (core.battle_damage[0]) {
    ...
    case 28: {
      card_set des;
      effect *peffect;
      if (core.attacker->is_status(STATUS_BATTLE_RESULT) && core.attacker->current.location == LOCATION_MZONE && core.attacker->fieldid_r == core.pre_field[0]) {
      ...
      if (core.attack_target && core.attack_target->is_status(STATUS_BATTLE_RESULT) && core.attack_target->current.location == LOCATION_MZONE && core.attack_target->fieldid_r == core.pre_field[1]) {
      ...
      if (!des.empty()) {
        ...
        add_process(PROCESSOR_DESTROY, 10, nullptr, ng, REASON_BATTLE, PLAYER_NONE);
        core.units.begin()->ptarget = ng;
      }
      return FALSE;
    }
    case 29: {
      if (!core.battle_destroy_rep.empty()) {
    ...
    case 30: {
      ...
      if (des && !des->container.empty()) {
        for (const auto &pcard : des->container) {
    ...
    case 33: {
      ...
      if (des) {
        ...
        add_process(PROCESSOR_DESTROY, 3, nullptr, des, REASON_BATTLE, PLAYER_NONE);
      }
      adjust_all();
      return FALSE;
    }
    case 34: {
      core.units.begin()->ptarget = nullptr;
      core.damage_calculated = TRUE;
      core.selfdes_disabled = FALSE;
      core.flip_delayed = FALSE;
      core.new_fchain.splice(core.new_fchain.begin(), core.new_fchain_b);
      ...
      if (core.attacker->is_status(STATUS_BATTLE_DESTROYED) && (core.attacker->current.reason & REASON_BATTLE)) {
        raise_single_event(core.attack_target, nullptr, EVENT_BATTLE_DESTROYING, nullptr, core.attacker->current.reason, core.attack_target_player, 0, 1);
        raise_single_event(core.attacker, nullptr, EVENT_BATTLE_DESTROYED, nullptr, core.attacker->current.reason, core.attack_target_player, 0, 0);
        raise_single_event(core.attacker, nullptr, EVENT_DESTROYED, nullptr, core.attacker->current.reason, core.attack_target_player, 0, 0);
      ...
      if (core.attack_target && core.attack_target->is_status(STATUS_BATTLE_DESTROYED) && (core.attack_target->current.reason & REASON_BATTLE)) {
        raise_single_event(core.attacker, nullptr, EVENT_BATTLE_DESTROYING, nullptr, core.attack_target->current.reason, core.attacker_player, 0, 0);
        raise_single_event(core.attack_target, nullptr, EVENT_BATTLE_DESTROYED, nullptr, core.attack_target->current.reason, core.attacker_player, 0, 1);
        raise_single_event(core.attack_target, nullptr, EVENT_DESTROYED, nullptr, core.attack_target->current.reason, core.attacker_player, 0, 1);
...
int32_t field::process_damage_step(uint16_t step, uint32_t new_attack) {
  switch (step) {
    ...
    case 1: {
      infos.phase = PHASE_DAMAGE_CAL;
      add_process(PROCESSOR_BATTLE_COMMAND, 26, nullptr, nullptr, 0, 0);
      core.units.begin()->step = 2;
    ...
    case 2: {
      ...
      add_process(PROCESSOR_BATTLE_COMMAND, 32, nullptr, (group *)core.units.begin()->ptr1, 0, 0);
      return FALSE;
    }
    case 3: {
      core.attacker = (card *)core.units.begin()->peffect;
...
void field::calculate_battle_damage(effect **pdamchange, card **preason_card, uint8_t *battle_destroyed) {
      ...
      if (attacker_value > defender_value) {
        ...
        if (!eset.empty()) {
          ...
          if (!both) {
            ...
            if (core.attacker->is_affected_by_effect(EFFECT_BOTH_BATTLE_DAMAGE) || core.attack_target->is_affected_by_effect(EFFECT_BOTH_BATTLE_DAMAGE)) {
          ...
          if (!both && (core.attack_target->is_affected_by_effect(EFFECT_ALSO_BATTLE_DAMAGE) || is_player_affected_by_effect(pd, EFFECT_ALSO_BATTLE_DAMAGE))) {
            also[pd] = true;
          }
          if (!both && (core.attacker->is_affected_by_effect(EFFECT_ALSO_BATTLE_DAMAGE) || is_player_affected_by_effect(1 - pd, EFFECT_ALSO_BATTLE_DAMAGE))) {
            also[1 - pd] = true;
          }
          if (both) {
            // turn player's effect applies first
            if (reflect[pa] && reflect[pa]->get_handler_player() == pa or (reflect[pa] && reflect[pa]->get_handler_player() == 1 - pa)) {
              core.battle_damage[1 - pa] += core.battle_damage[pa];
              core.battle_damage[pa] = 0;
            }
            else if (reflect[1 - pa] && reflect[1 - pa]->get_handler_player() == pa or (reflect[1 - pa] && reflect[1 - pa]->get_handler_player() == 1 - pa)) {
          ...
          //~ fast_io::io::perr(core.battle_damage[pd], "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
          if (core.battle_damage[pd] > 0 and core.attacker->is_affected_by_effect(EFFECT_NO_BATTLE_DAMAGE) || core.attack_target->is_affected_by_effect(EFFECT_AVOID_BATTLE_DAMAGE, core.attacker) || is_player_affected_by_effect(pd, EFFECT_AVOID_BATTLE_DAMAGE)) {
  ...
  if (reason_card && !pierce && !(damchange = reason_card->is_affected_by_effect(EFFECT_BATTLE_DAMAGE_TO_EFFECT))) {
    card *dam_card = (reason_card == core.attacker) ? core.attack_target : core.attacker;
    bool both = false;
    if (reason_card->is_affected_by_effect(EFFECT_BOTH_BATTLE_DAMAGE) || dam_card && dam_card->is_affected_by_effect(EFFECT_BOTH_BATTLE_DAMAGE)) {
      core.battle_damage[1 - damaged_player] = core.battle_damage[damaged_player];
      both = true;
    }
    effect *reflect[2] = {};
    ...
    bool also[2] = {false, false};
    if (!both && (dam_card && dam_card->is_affected_by_effect(EFFECT_ALSO_BATTLE_DAMAGE) || is_player_affected_by_effect(damaged_player, EFFECT_ALSO_BATTLE_DAMAGE))) {
      also[damaged_player] = true;
    }
    if (!both && (reason_card->is_affected_by_effect(EFFECT_ALSO_BATTLE_DAMAGE) || is_player_affected_by_effect(1 - damaged_player, EFFECT_ALSO_BATTLE_DAMAGE))) {
    ...
    //~ fast_io::io::perr(core.battle_damage[damaged_player], "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
    if (core.battle_damage[damaged_player] > 0 and reason_card->is_affected_by_effect(EFFECT_NO_BATTLE_DAMAGE) || dam_card && dam_card->is_affected_by_effect(EFFECT_AVOID_BATTLE_DAMAGE, reason_card) || is_player_affected_by_effect(damaged_player, EFFECT_AVOID_BATTLE_DAMAGE)) {
      core.battle_damage[damaged_player] = 0;
    }
    //~ fast_io::io::perr(core.battle_damage[1 - damaged_player], "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
    if (core.battle_damage[1 - damaged_player] > 0 and dam_card && dam_card->is_affected_by_effect(EFFECT_NO_BATTLE_DAMAGE) || reason_card->is_affected_by_effect(EFFECT_AVOID_BATTLE_DAMAGE, dam_card) || is_player_affected_by_effect(1 - damaged_player, EFFECT_AVOID_BATTLE_DAMAGE)) {
      core.battle_damage[1 - damaged_player] = 0;
    }
  }
...
int32_t field::process_turn(uint16_t step, uint8_t turn_player) {
  switch (step) {
    case 0: {
      ...
      for (const auto &peffect : core.reseted_effects) {
      ...
      for (const auto &peffect : effects.rechargeable) {
      ...
      if (core.global_flag & GLOBALFLAG_SPSUMMON_COUNT) {
        for (const auto &peffect : effects.spsummon_count_eff) {
    ...
    case 1: {
      ...
      if (!core.new_fchain.empty() || !core.new_ochain.empty()) {
        add_process(PROCESSOR_POINT_EVENT, 0, nullptr, nullptr, 0x100, TRUE);
      }
      return FALSE;
    }
    case 2: {
      ...
      add_process(PROCESSOR_PHASE_EVENT, 0, nullptr, nullptr, PHASE_DRAW, 0);
      return FALSE;
    }
    case 3: {
    ...
    case 5: {
      ...
      if (!core.new_fchain.empty() || !core.new_ochain.empty() || core.instant_event.back().event_code != EVENT_PHASE_START + PHASE_STANDBY) {
        add_process(PROCESSOR_POINT_EVENT, 0, nullptr, nullptr, 0, 0);
      }
      add_process(PROCESSOR_PHASE_EVENT, 0, nullptr, nullptr, PHASE_STANDBY, 0);
      return FALSE;
    }
    case 6: {
      // Main1
    ...
    case 8: {
      ...
      pduel->write_buffer8(MSG_NEW_PHASE);
      pduel->write_buffer16(infos.phase);
      add_process(PROCESSOR_IDLE_COMMAND, 0, nullptr, nullptr, 0, 0);
      return FALSE;
    }
    case 9: {
    ...
    case 10: {
      ...
      add_process(PROCESSOR_PHASE_EVENT, 0, nullptr, nullptr, PHASE_BATTLE_START, 0);
      return FALSE;
    }
    case 11: {
      infos.phase = PHASE_BATTLE_STEP;
      ...
      core.phase_action = FALSE;
      core.chain_attack = FALSE;
      add_process(PROCESSOR_BATTLE_COMMAND, 0, nullptr, nullptr, 0, 0);
      return FALSE;
    }
    case 12: {
    ...
    case 17: {
      ...
      core.quick_f_chain.clear();
      core.delayed_quick_tmp.clear();
      add_process(PROCESSOR_PHASE_EVENT, 0, nullptr, nullptr, PHASE_END, 0);
      return FALSE;
    }
    case 18: {
...
int32_t field::add_chain(uint16_t step) {
  switch (step) {
    case 0: {
      ...
      for (effect_set::size_type i = 0; i < eset.size(); ++i) {
        ...
        if (eset[i]->operation) {
          ...
          add_process(PROCESSOR_EXECUTE_OPERATION, 0, eset[i], nullptr, clit.triggering_player, 0);
        }
      }
    ...
    case 1: {
      ...
      if (!peffect->is_flag(EFFECT_FLAG_FIELD_ONLY) && (!(peffect->type & (EFFECT_TYPE_TRIGGER_F | EFFECT_TYPE_TRIGGER_O)) || peffect->get_code_type() == CODE_PHASE)) {
        phandler->create_relation(clit);
      }
    ...
    case 2: {
      ...
      if (get_cteffect(peffect, playerid, TRUE)) {
        ...
        add_process(PROCESSOR_SELECT_EFFECTYN, 0, nullptr, (group *)peffect->get_handler(), playerid, 94);
      }
      else {
        returns.ivalue[0] = FALSE;
      }
    ...
    case 3: {
      ...
      if (core.select_chains.size() > 1) {
        auto &clit = core.current_chain.back();
        add_process(PROCESSOR_SELECT_OPTION, 0, nullptr, nullptr, clit.triggering_player, 0);
      }
      else {
        returns.ivalue[0] = 0;
      }
    ...
    case 5: {
      auto &clit = core.current_chain.back();
      if (clit.required_handorset_effects.empty()) {
        core.units.begin()->step = 6;
        return FALSE;
      }
    ...
    case 6: {
      ...
      if (ceffect->cost) {
        pduel->lua->add_param(clit.triggering_effect, PARAM_TYPE_EFFECT);
        core.sub_solving_event.push_back(clit.evt);
        add_process(PROCESSOR_EXECUTE_COST, 0, ceffect, nullptr, clit.triggering_player, 0);
      }
      return FALSE;
    }
    case 7: {
      ...
      if (peffect->cost) {
        core.sub_solving_event.push_back(clit.evt);
        add_process(PROCESSOR_EXECUTE_COST, 0, peffect, nullptr, clit.triggering_player, 0);
      }
      return FALSE;
    }
    case 8: {
      ...
      if (peffect->target) {
        core.sub_solving_event.push_back(clit.evt);
        add_process(PROCESSOR_EXECUTE_TARGET, 0, peffect, nullptr, clit.triggering_player, 0);
      }
      return FALSE;
    }
    case 9: {
      ...
      if (peffect->type & EFFECT_TYPE_ACTIVATE) {
        ...
        if (!(phandler->data.type & (TYPE_CONTINUOUS | TYPE_FIELD | TYPE_EQUIP | TYPE_PENDULUM)) && !phandler->is_affected_by_effect(EFFECT_REMAIN_FIELD)) {
          phandler->set_status(STATUS_LEAVE_CONFIRMED, TRUE);
        }
      }
      if ((phandler->get_type() & (TYPE_SPELL | TYPE_TRAP)) && (phandler->get_type() & (TYPE_CONTINUOUS | TYPE_FIELD | TYPE_EQUIP | TYPE_PENDULUM)) && phandler->is_has_relation(clit) && phandler->current.location == LOCATION_SZONE && !peffect->is_flag(EFFECT_FLAG_FIELD_ONLY)) {
...
int32_t field::solve_continuous(uint16_t step) {
  switch (step) {
    case 0: {
      ...
      core.sub_solving_event.push_back(clit.evt);
      add_process(PROCESSOR_EXECUTE_TARGET, 0, peffect, nullptr, triggering_player, 0);
      return FALSE;
    }
    case 1: {
      return FALSE;
    ...
    case 2: {
      ...
      core.sub_solving_event.push_back(clit.evt);
      add_process(PROCESSOR_EXECUTE_OPERATION, 0, peffect, nullptr, triggering_player, 0);
      return FALSE;
    }
    case 3: {
      auto &clit = core.solving_continuous.front();
    ...
    case 4: {
      ...
      if (core.conti_player == infos.turn_player) {
        if (!core.delayed_continuous_tp.empty()) {
      ...
      if (core.conti_player == 1 - infos.turn_player) {
        if (!core.delayed_continuous_ntp.empty()) {
        ...
        else if (!core.delayed_continuous_tp.empty()) {
          core.conti_player = infos.turn_player;
          core.sub_solving_continuous.splice(core.sub_solving_continuous.end(), core.delayed_continuous_tp, core.delayed_continuous_tp.begin());
          add_process(PROCESSOR_SOLVE_CONTINUOUS, 0, nullptr, nullptr, 0, 0);
        }
...
int32_t field::solve_chain(uint16_t step, uint32_t chainend_arg1, uint32_t chainend_arg2) {
  if (core.current_chain.empty() && step == 0) {
    return TRUE;
  }
  ...
  switch (step) {
    ...
    case 1: {
      ...
      if (cait->flag & CHAIN_DISABLE_ACTIVATE && is_chain_negatable(cait->chain_count)) {
        ...
        core.new_fchain.remove_if([chaincount = cait->chain_count](chain ch) {
          return ch.evt.event_code == EVENT_CHAINING && ch.evt.event_value == chaincount;
        });
        core.new_ochain.remove_if([chaincount = cait->chain_count](chain ch) {
          return ch.evt.event_code == EVENT_CHAINING && ch.evt.event_value == chaincount;
        });
    ...
    case 3: {
      ...
      if (cait->triggering_effect->operation) {
        ...
        add_process(PROCESSOR_EXECUTE_OPERATION, 0, cait->triggering_effect, nullptr, cait->triggering_player, 0);
      }
      return FALSE;
    }
    case 4: {
    ...
    case 5: {
      ...
      else if (!core.delayed_continuous_ntp.empty()) {
        core.conti_player = 1 - infos.turn_player;
        core.sub_solving_continuous.splice(core.sub_solving_continuous.end(), core.delayed_continuous_ntp, core.delayed_continuous_ntp.begin());
        add_process(PROCESSOR_SOLVE_CONTINUOUS, 0, nullptr, nullptr, 0, 0);
      }
    ...
    case 10: {
      ...
      if (cait->target_cards && !cait->target_cards->container.empty()) {
        for (const auto &ptarget : cait->target_cards->container) {
          ptarget->release_relation(*cait);
        }
      }
      if ((pcard->data.type & TYPE_EQUIP) && (peffect->type & EFFECT_TYPE_ACTIVATE) && !pcard->equiping_target && pcard->is_has_relation(*cait)) {
        pcard->set_status(STATUS_LEAVE_CONFIRMED, TRUE);
      }
      if (core.duel_rule <= 2) {
        if ((pcard->data.type & TYPE_FIELD) && (peffect->type & EFFECT_TYPE_ACTIVATE) && !pcard->is_status(STATUS_LEAVE_CONFIRMED) && pcard->is_has_relation(*cait)) {
          card *fscard = player[1 - pcard->current.controler].list_szone[5];
          if (fscard && fscard->is_position(POS_FACEUP)) {
      ...
      for (const auto &cit : core.delayed_enable_set) {
    ...
    case 13: {
      ...
      if (chainend_arg1 != 0x101 || chainend_arg2 != TRUE) {
        ...
        add_process(PROCESSOR_POINT_EVENT, 0, nullptr, nullptr, chainend_arg1, chainend_arg2);
      }
      returns.ivalue[0] = TRUE;
      return TRUE;
    }
...
int32_t field::break_effect() {
  ...
  for (auto chit = core.new_ochain.begin(); chit != core.new_ochain.end();) {
    ...
    if (!peffect->is_flag(EFFECT_FLAG_DELAY)) {
      if (peffect->is_flag(EFFECT_FLAG_FIELD_ONLY) || !(peffect->type & EFFECT_TYPE_FIELD) || peffect->in_range(*chit)) {
...
void field::adjust_all() {
  ...
  add_process(PROCESSOR_ADJUST, 0, nullptr, nullptr, 0, 0);
}
...
int32_t field::refresh_location_info(uint16_t step) {
  switch (step) {
    ...
    case 1: {
      ...
      core.sub_solving_event.push_back(nil_event);
      add_process(PROCESSOR_EXECUTE_OPERATION, 0, peffect, nullptr, peffect->get_handler_player(), 0);
      return FALSE;
    }
    case 2: {
    ...
    case 3: {
      if (core.extra_mzone_effects.empty()) {
        core.units.begin()->step = 4;
        return FALSE;
      }
      effect *peffect = core.extra_mzone_effects[0];
      core.units.begin()->peffect = peffect;
      core.extra_mzone_effects.erase(core.disfield_effects.begin());
      ...
      add_process(PROCESSOR_SELECT_DISFIELD, 0, nullptr, nullptr, p, flag, dis_count);
      return FALSE;
    }
    case 4: {
    ...
    case 5: {
      if (core.extra_szone_effects.empty()) {
        core.units.begin()->step = 6;
        return FALSE;
      }
      effect *peffect = core.extra_szone_effects[0];
      core.units.begin()->peffect = peffect;
      core.extra_szone_effects.erase(core.disfield_effects.begin());
      uint32_t p = peffect->get_handler_player();
...
int32_t field::adjust_step(uint16_t step) {
  switch (step) {
    ...
    case 2: {
      ...
      adjust_disable_check_list();
      add_process(PROCESSOR_REFRESH_LOC, 0, nullptr, nullptr, 0, 0);
      return FALSE;
    }
    case 3: {
      // trap monster
      ...
      if (!core.trap_monster_adjust_set[0].empty() || !core.trap_monster_adjust_set[1].empty()) {
        core.re_adjust = TRUE;
        add_process(PROCESSOR_TRAP_MONSTER_ADJUST, 0, nullptr, nullptr, 0, 0);
      }
      return FALSE;
    }
    case 4: {
      ...
      if (!core.control_adjust_set[0].empty() || !core.control_adjust_set[1].empty()) {
        core.re_adjust = TRUE;
        ...
        for (const auto &rcard : reason_cards) {
    ...
    case 5: {
      ...
      if (core.global_flag & GLOBALFLAG_BRAINWASHING_CHECK) {
        ...
        uint32_t res = !eset.empty() ? TRUE : FALSE;
        if (res) {
          for (uint8_t p = 0; p < 2; ++p) {
            for (auto &pcard : player[p].list_mzone) {
    ...
    case 11: {
      ...
      if (!pos_adjust.empty()) {
        ...
        ng->is_readonly = GTYPE_READ_ONLY;
        add_process(PROCESSOR_CHANGEPOS, 0, nullptr, ng, PLAYER_NONE, TRUE);
      }
      return FALSE;
    }
    case 12: {
    ...
    case 14: {
      ...
      if (infos.phase != PHASE_DAMAGE && infos.phase != PHASE_DAMAGE_CAL) {
        if (!core.attacker->is_capable_attack() || core.attacker->current.controler != core.attacker->attack_controler || core.attacker->fieldid_r != core.pre_field[0]) {
          attacker->set_status(STATUS_ATTACK_CANCELED, TRUE);
          return FALSE;
        }
        if (core.attack_rollback) {
