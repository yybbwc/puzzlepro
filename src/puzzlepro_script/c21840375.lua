...
function c21840375.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c21840375.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c21840375.filter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
