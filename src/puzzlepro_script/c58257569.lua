...
function c58257569.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c58257569.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c58257569.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58257569.thfilter, tp, LOCATION_GRAVE + LOCATION_DECK, 0, 1, nil)
  end
