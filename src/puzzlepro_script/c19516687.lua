...
function s.g2htg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and s.g2hfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.g2hfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function s.g2hop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc, nil, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_HAND) and tc:IsType(TYPE_SPELL + TYPE_TRAP) then
    Duel.ShuffleHand(tp)
