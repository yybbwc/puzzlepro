...
function c60080151.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc:IsOnField() and tc:IsAbleToRemove()
  end
...
function c60080151.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetTurnCount() ~= e:GetLabel() and Duel.GetTurnPlayer() ~= tp and tc:GetFlagEffect(60080151) ~= 0 and tc:GetReasonEffect():GetHandler() == e:GetHandler()
end
