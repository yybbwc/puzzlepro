...
function c86962245.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c86962245.filter, tp, 0, LOCATION_SZONE, 1, nil) and Duel.IsExistingTarget(c86962245.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c86962245.spfilter(c, tp)
  return c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_SZONE) and c:IsPreviousControler(1 - tp) and c:GetPreviousSequence() < 5
end
