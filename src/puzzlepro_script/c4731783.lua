...
function s.actarget(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = c:IsAbleToRemove() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  if chk == 0 then
    return b1 or b2
  end
...
function s.acoperation(e, tp, eg, ep, ev, re, r, rp)
  ...
  elseif op == 2 then
    ...
    if c:IsRelateToEffect(e) and Duel.Remove(c, 0, REASON_EFFECT + REASON_TEMPORARY) ~= 0 then
      ...
      if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) then
