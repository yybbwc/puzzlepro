...
function c54965929.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c54965929.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return c:IsDestructable() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c54965929.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c54965929.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.Destroy(c, REASON_EFFECT) ~= 0 then
    ...
    if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and tc:IsCanAddCounter(0x1, 1) then
      tc:AddCounter(0x1, 1)
    end
  end
end
...
function c54965929.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x1, 2, REASON_COST)
  end
