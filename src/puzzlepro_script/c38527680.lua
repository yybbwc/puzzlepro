...
function s.tgfilter(c, tp)
  return c:IsType(TYPE_NORMAL + TYPE_UNION) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, c, c:GetOriginalCodeRule(), c:GetAttack())
end
...
function s.sunthfilter(c, e, tp, minc, maxc)
  ...
  if c:IsHasEffect(EFFECT_SUMMON_PROC, c:GetControler()) then
    ...
    if not Duel.CheckTribute(c, minc, maxc) then
      return false
    end
  end
...
function s.otcon(e, c, minc)
  ...
  return minc <= 2 and Duel.IsExistingMatchingCard(s.otfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(s.otfilter2, tp, 0, LOCATION_ONFIELD, 1, nil, e)
end
function s.ottg(e, c)
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.thfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, tp, LOCATION_DECK)
end
