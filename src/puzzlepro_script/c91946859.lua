...
function c91946859.cfilter(c, e, tp)
  return c:GetType() & 0x81 == 0x81 and c:IsSetCard(0x2093) and Duel.IsExistingMatchingCard(c91946859.spfilter, tp, LOCATION_HAND, 0, 1, c, e, tp) and Duel.GetMZoneCount(tp, c) > 0
end
...
function c91946859.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroupEx(tp, c91946859.cfilter, 1, REASON_COST, true, nil, e, tp)
  end
...
function c91946859.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91946859.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
