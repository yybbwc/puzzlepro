...
function c55742055.target1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55742055.filter1, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c55742055.target2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c55742055.filter2, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c55742055.operation2(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if not tc or Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) == 0 then
    return
  end
...
function c55742055.target3(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c55742055.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c55742055.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
