...
function c99266988.tgfilter(c, tp)
  return c:IsAttribute(ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c99266988.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetAttribute())
end
...
function c99266988.tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c99266988.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c99266988.tdfilter, tp, LOCATION_REMOVED, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 1)
  end
