...
function s.thdfilter(c)
  return c:IsAbleToHand() and not c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevelBelow(4) and c:IsRace(RACE_CYBERS)
end
function s.thndfilter(c, att)
  return c:IsAbleToHand() and not c:IsAttribute(att) and c:IsSetCard(0x135)
end
function s.cfilter(c, e, tp)
  return c:IsRace(RACE_CYBERSE) and not c:IsPublic() and (c:IsAttribute(ATTRIBUTE_DARK) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.thdfilter, tp, LOCATION_DECK, 0, 1, nil) or not c:IsAttribute(ATTRIBUTE_DARK) and c:IsAbleToDeck() and Duel.IsExistingMatchingCard(s.thndfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetAttribute()))
end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    if tc:IsAttribute(ATTRIBUTE_DARK) then
      if Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsExistingMatchingCard(s.thdfilter, tp, LOCATION_DECK, 0, 1, nil) then
      ...
      if Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_DECK) and Duel.IsExistingMatchingCard(s.thndfilter, tp, LOCATION_DECK, 0, 1, nil, att) then
        Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
