...
function c90500169.filter(c, e, tp)
  if c:IsFacedown() or not c:IsSetCard(0x41) or not c:IsAbleToDeck() then
    return false
  end
  local op = c:GetOwner()
  local locct = Duel.GetLocationCount(op, LOCATION_MZONE)
  local cp = c:GetControler()
  if op == cp and locct <= -1 then
    return false
  end
  if op ~= cp and locct <= 0 then
    return false
  end
...
function c90500169.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c90500169.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c90500169.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e, tp)
  end
...
function c90500169.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(op, LOCATION_MZONE) <= 0 then
    return
  end
  local class = _G["c" .. code]
  if class == nil or class.lvdn == nil then
    return
  end
