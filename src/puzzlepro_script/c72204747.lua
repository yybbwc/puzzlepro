...
function c72204747.filter1(c, e, sp)
  ...
  return lv > 0 and c:IsSetCard(0xc008) and Duel.IsExistingMatchingCard(c72204747.filter2, sp, LOCATION_DECK, 0, 1, nil, lv, e, sp)
end
...
function c72204747.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c72204747.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c72204747.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
