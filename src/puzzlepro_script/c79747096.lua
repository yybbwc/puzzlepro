...
function c79747096.spcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == tp and e:GetHandler():GetFlagEffect(79747096) > 0 and e:GetLabelObject():GetLabel() ~= Duel.GetTurnCount()
end
...
function c79747096.spop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsEnvironment(41418852, tp, LOCATION_FZONE) and Duel.IsExistingMatchingCard(c79747096.damfilter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil) then
    Duel.BreakEffect()
