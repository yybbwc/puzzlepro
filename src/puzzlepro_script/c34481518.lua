...
function s.egfilter(c, tp)
  if not c:IsPreviousControler(1 - tp) or not c:IsLocation(LOCATION_GRAVE) then
    return false
  end
  ...
  if bc:IsRelateToBattle() then
    ...
    return bc:GetPreviousPosition() & POS_FACEUP > 0 and bc:GetPreviousLocation() & LOCATION_MZONE == LOCATION_MZONE and bc:IsPreviousControler(tp) and bc:GetPreviousTypeOnField() & TYPE_MONSTER == TYPE_MONSTER and c:GetPreviousRaceOnField() & RACE_DINOSAUR == RACE_DINOSAUR
  end
end
