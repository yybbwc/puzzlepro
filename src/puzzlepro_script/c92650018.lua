...
function c92650018.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c92650018.filter(chkc, tp)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c92650018.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
