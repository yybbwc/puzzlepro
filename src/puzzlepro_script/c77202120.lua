...
function c77202120.cfilter(c, tp, se)
  return c:IsRace(RACE_DRAGON) and c:IsType(TYPE_SYNCHRO) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousControler(tp) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function c77202120.spfilter(c, e, tp)
  return c:IsRace(RACE_DRAGON) and c:IsType(TYPE_SYNCHRO) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousControler(tp) and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c77202120.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and eg:IsExists(c77202120.spfilter, 1, nil, e, tp)
  end
