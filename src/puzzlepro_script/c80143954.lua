...
function c80143954.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c80143954.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c80143954.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c80143954.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsSetCard(0x103)
end
...
function c80143954.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x103) and c:IsOnField() and c:IsControler(tp) and c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE)
end
function c80143954.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c80143954.repfilter, 1, nil, tp)
  end
