...
function c58699500.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and (c:IsReason(REASON_BATTLE) or rp == 1 - tp) and Duel.IsExistingMatchingCard(c58699500.repfilter, tp, LOCATION_ONFIELD, 0, 1, c, e)
  end
...
function c58699500.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c58699500.filter(chkc)
  end
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.IsExistingMatchingCard(c58699500.cfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingTarget(c58699500.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
  e:SetLabel(0)
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
