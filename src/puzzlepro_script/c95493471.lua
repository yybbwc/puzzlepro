...
function c95493471.cfilter(c, lg, tp)
  return c:IsSetCard(0xdc) and c:IsType(TYPE_XYZ) and c:IsSummonLocation(LOCATION_EXTRA) and lg:IsContains(c) and not Duel.IsExistingMatchingCard(c95493471.drfilter, tp, LOCATION_MZONE, 0, 1, c, c:GetCode())
end
...
function c95493471.cfilter2(c, tp, zone)
  ...
  return c:IsPreviousLocation(LOCATION_MZONE) and bit.extract(zone, seq) ~= 0 and c:IsType(TYPE_XYZ) and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)
end
...
function c95493471.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c95493471.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, e:GetLabel())
  end
