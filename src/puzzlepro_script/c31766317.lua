...
function c31766317.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:GetCount() == 1 and dc ~= e:GetHandler() and dc:IsFaceup() and dc:IsLocation(LOCATION_MZONE) and dc:IsRace(RACE_FIEND) and dc:IsAttribute(ATTRIBUTE_DARK) and not dc:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(c31766317.rfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c31766317.filter(c, e, tp)
  return c:IsRace(RACE_FIEND) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevelAbove(6) and not c:IsCode(31766317) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c31766317.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c31766317.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c31766317.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
