...
function c45653036.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c45653036.cfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45653036.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c45653036.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) then
    ...
    if b1 and b2 then
      opt = Duel.SelectOption(tp, aux.Stringid(45653036, 0), aux.Stringid(45653036, 1))
    elseif b1 then
      opt = Duel.SelectOption(tp, aux.Stringid(45653036, 0))
    elseif b2 then
      opt = Duel.SelectOption(tp, aux.Stringid(45653036, 1)) + 1
    else
      opt = 2
    end
    if opt == 0 then
