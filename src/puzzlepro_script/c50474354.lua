...
function c50474354.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetCustomActivityCount(50474354, tp, ACTIVITY_CHAIN) == 0 and c:IsAbleToGraveAsCost()
  end
...
function c50474354.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c50474354.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
