...
function c21888494.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_HAND, 0, 2, e:GetHandler(), TYPE_SPELL + TYPE_TRAP) and Duel.IsExistingMatchingCard(c21888494.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
end
function c21888494.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g1:GetCount() == 0 or g2:GetCount() < 2 then
    return
  end
