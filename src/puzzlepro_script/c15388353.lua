...
function s.tdfilter(c, tp)
  return c:IsSetCard(0x197) and c:IsAbleToDeck() and Duel.IsExistingTarget(Card.IsAbleToDeck, tp, LOCATION_GRAVE, 0, 1, c)
end
function s.drtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(s.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TODECK)
...
function s.drop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g == 0 or aux.PlaceCardsOnDeckBottom(tp, g) == 0 then
    return
  end
  Duel.BreakEffect()
