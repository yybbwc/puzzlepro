...
function c64398890.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c64398890.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c64398890.desfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil)
  end
...
function c64398890.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(c64398890.cfilter, tp, LOCATION_MZONE, 0, 1, nil) then
    return
  end
...
function c64398890.repfilter(c, e)
  return c:IsFaceup() and c:IsSetCard(0x103d) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
end
function c64398890.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and c:IsOnField() and c:IsFaceup() and Duel.IsExistingMatchingCard(c64398890.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
