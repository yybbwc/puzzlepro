...
function c983995.condition(e, tp, eg, ep, ev, re, r, rp)
  if rp == tp or not Duel.IsChainNegatable(ev) then
    return false
  end
...
function c983995.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bit.band(r, REASON_DESTROY) ~= 0 and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN)
end
