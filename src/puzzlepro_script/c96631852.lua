...
function c96631852.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c96631852.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c96631852.defcon(e)
  ...
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) >= 2 and not Duel.IsExistingMatchingCard(Card.IsAttackPos, tp, LOCATION_MZONE, 0, 1, nil)
end
