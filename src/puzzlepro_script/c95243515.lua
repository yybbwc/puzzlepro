...
function c95243515.filter1(c, tp)
  return c:IsType(TYPE_EFFECT) and aux.NegateEffectMonsterFilter(c) and Duel.IsExistingTarget(c95243515.filter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c, c)
end
function c95243515.filter2(c, tc)
  return c:IsType(TYPE_EFFECT) and aux.NegateEffectMonsterFilter(c) and not c:IsRace(tc:GetRace()) and not c:IsAttribute(tc:GetAttribute())
end
function c95243515.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c95243515.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
