...
function c18205590.filter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x9c) and c:IsAbleToDeck() and Duel.IsExistingMatchingCard(c18205590.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c18205590.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c18205590.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c18205590.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
