...
function s.costcheck(c, ec, e, tp)
  return Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, Group.FromCards(c, ec)) or Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil, e, tp, c)
end
...
function s.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local cost = 0
  if b1 or b2 then
    cost = aux.SelectFromOptions(tp, { b1, aux.Stringid(id, 1), 1 }, { b2, aux.Stringid(id, 2), 2 })
  end
  if cost == 1 then
...
function s.spfilter(c, e, tp, rc)
  return c:IsFaceupEx() and c:IsRace(RACE_SPELLCASTER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not c:IsLocation(LOCATION_EXTRA) and Duel.GetMZoneCount(tp, rc) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, rc, c) > 0)
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b1 = (Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil, e, tp, nil) or b0) and (Duel.GetFlagEffect(tp, id) == 0 or not e:IsCostChecked())
  local b2 = Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler()) and (Duel.GetFlagEffect(tp, id + o) == 0 or not e:IsCostChecked())
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
  if b1 or b2 then
    op = aux.SelectFromOptions(tp, { b1, aux.Stringid(id, 3), 1 }, { b2, aux.Stringid(id, 4), 2 })
  end
  e:SetLabel(op)
  if op == 1 then
