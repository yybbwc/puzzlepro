...
function c47556396.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c47556396.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47556396.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c47556396.spcon(e, tp, eg, ep, ev, re, r, rp)
  return eg:IsExists(c47556396.cfilter, 1, nil, tp) and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
end
function c47556396.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c47556396.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(47556396) == 0
  end
