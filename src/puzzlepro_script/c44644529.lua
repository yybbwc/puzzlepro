...
function c44644529.spfilter(c, e, tp, lv, rc)
  if not (c:IsSetCard(0x14f) and c:IsLevel(lv - 1, lv + 1) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)) then
    return false
  end
...
function c44644529.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c44644529.desfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44644529.desfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c44644529.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c44644529.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44644529.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
