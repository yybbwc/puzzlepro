...
function c327051.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c327051.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c327051.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c327051.repfilter(c, tp)
  return c:IsLocation(LOCATION_SZONE) and c:GetSequence() < 5 and c:IsControler(tp) and c:IsFaceup() and c:IsSetCard(0x150) and c:GetReasonPlayer() == 1 - tp and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c327051.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c327051.repfilter, 1, nil, tp)
  end
