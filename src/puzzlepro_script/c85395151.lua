...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and s.crbfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.crbfilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
