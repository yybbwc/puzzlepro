...
function c96704974.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c96704974.filter(chkc)
  end
  ...
  if chk == 0 then
    return ct > 0 and Duel.IsExistingTarget(c96704974.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
