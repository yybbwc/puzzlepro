...
function c33578406.filter(c, tp)
  return c:IsSetCard(0x60) and not c:IsCode(33578406) and c:IsType(TYPE_EQUIP) and c:CheckUniqueOnField(tp) and not c:IsForbidden() and Duel.IsExistingMatchingCard(c33578406.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, c)
end
...
function c33578406.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > -1 and Duel.IsExistingMatchingCard(c33578406.filter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
