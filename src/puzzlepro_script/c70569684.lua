...
function c70569684.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetCount() >= 2 and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function c70569684.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsSSetable()
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DESTROY)
...
function c70569684.setop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) > 0 and c:IsRelateToEffect(e) and Duel.SSet(tp, c) > 0 then
    local e1 = Effect.CreateEffect(c)
