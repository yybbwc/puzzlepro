...
function c99634927.atkcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c99634927.atkfilter1, 1, nil, tp)
  end
...
function c99634927.atktg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c99634927.atkfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c99634927.atkfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c99634927.atktg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99634927.atkfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
...
function c99634927.atktg3(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc ~= c and chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c99634927.atkfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c99634927.atkfilter2, tp, LOCATION_MZONE, 0, 1, c)
  end
