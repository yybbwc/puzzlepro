...
function c96661780.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c96661780.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c96661780.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c96661780.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96661780.thfilter, tp, LOCATION_DECK + LOCATION_REMOVED, 0, 1, nil)
  end
