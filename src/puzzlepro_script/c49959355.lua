...
function c49959355.lvtg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c49959355.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0 and Duel.IsExistingTarget(c49959355.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c49959355.lvtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c49959355.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c49959355.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
  Duel.SelectTarget(tp, c49959355.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, 1, nil)
