...
function c52665542.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52665542.costfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c52665542.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c52665542.tgfilter(chkc) and chkc ~= cc
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c52665542.tgfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c52665542.dfilter(c, tp)
  return c:IsFaceup() and c:IsOnField() and c:IsSetCard(0x38) and c:IsControler(tp) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
...
function c52665542.value(e, c)
  return c:IsFaceup() and c:IsOnField() and c:IsSetCard(0x38) and c:IsControler(e:GetHandlerPlayer()) and c:IsReason(REASON_EFFECT)
end
