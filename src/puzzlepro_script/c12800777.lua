...
function c12800777.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c12800777.spfilter, tp, LOCATION_GRAVE, 0, 1, nil)
end
...
function c12800777.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c12800777.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c12800777.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
