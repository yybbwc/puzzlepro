...
function s.cfilter(c)
  return c:IsAttackBelow(2000) and c:GetTextAttack() >= 0 and c:IsAbleToRemoveAsCost() and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and (Duel.IsPlayerCanDraw(c:GetControler(), 1) or c:GetOriginalRace() ~= RACE_REPTILE) and Duel.IsExistingTarget(s.atkcheck, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c, c:GetBaseAttack())
end
...
function s.actg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsFaceup() and chkc:IsLocation(LOCATION_MZONE) and chkc:GetAttack() ~= atk
  end
  if chk == 0 then
