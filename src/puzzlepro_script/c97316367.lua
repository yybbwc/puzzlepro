...
function c97316367.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c97316367.cfilter, 1, nil, tp)
  end
...
function c97316367.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c97316367.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 2) and Duel.IsExistingTarget(c97316367.tdfilter, tp, LOCATION_GRAVE, 0, 5, e:GetHandler())
  end
