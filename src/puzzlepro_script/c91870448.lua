...
function c91870448.spcon(e, c)
  ...
  return (Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_ONFIELD, 0, nil, TYPE_SPELL + TYPE_TRAP) < Duel.GetMatchingGroupCount(Card.IsType, tp, 0, LOCATION_ONFIELD, nil, TYPE_SPELL + TYPE_TRAP)) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
end
function c91870448.confilter(c)
  return c:IsFaceup() and c:IsCode(77585513)
end
function c91870448.plcon(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c91870448.confilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
    return false
  end
...
function c91870448.pltg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_SZONE, tp) > 0
  end
end
