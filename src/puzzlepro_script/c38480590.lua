...
function c38480590.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c38480590.filter(chkc, tid)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c38480590.filter, tp, LOCATION_GRAVE, 0, 1, nil, tid)
  end
