...
function c54455435.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and c:IsPreviousControler(tp) and c == Duel.GetAttackTarget() and Duel.GetAttacker():IsControler(1 - tp)
end
...
function c54455435.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c54455435.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
