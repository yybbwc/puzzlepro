...
function c46576366.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c46576366.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c46576366.ovtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c46576366.ovfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46576366.ovfilter, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanOverlay()
  end
