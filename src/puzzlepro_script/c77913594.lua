...
function c77913594.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77913594.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, tp, LOCATION_DECK)
end
function c77913594.spfilter(c, sc)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x172) and aux.IsCodeListed(sc, c:GetCode()) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function c77913594.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if res and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and tc:IsType(TYPE_MONSTER) and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c77913594.spfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil, tc) and Duel.SelectYesNo(tp, aux.Stringid(77913594, 1)) then
    Duel.BreakEffect()
