...
function c12215894.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp) and Duel.IsExistingMatchingCard(c12215894.filter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c12215894.cfilter(c)
  return c:IsSetCard(0xab) and c:IsAbleToRemoveAsCost() and not c:IsCode(12215894) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup())
end
...
function c12215894.target2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToDeck, tp, 0, LOCATION_HAND + LOCATION_ONFIELD + LOCATION_GRAVE, 1, nil)
  end
