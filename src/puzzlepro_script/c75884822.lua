...
function c75884822.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if c75884822.thcon(e, tp, eg, ep, ev, re, r, rp) and c75884822.thtg(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.SelectYesNo(tp, 94) then
...
function c75884822.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 75884822) == 0 and Duel.IsExistingMatchingCard(c75884822.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c75884822.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and ((c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and c:IsType(TYPE_FUSION)) or c:IsSetCard(0x111)) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c75884822.desfilter(c, e, tp)
  return c:IsControler(tp) and (c:IsFaceup() or not c:IsOnField()) and c:IsSetCard(0xf) and c:IsAbleToRemove() and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED) and not c:IsImmuneToEffect(e)
end
function c75884822.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c75884822.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c75884822.desfilter, tp, LOCATION_ONFIELD + LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c75884822.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c75884822.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
