...
function c38383368.cfilter(c)
  if not (c:IsReason(REASON_BATTLE) and c:GetPreviousSequence() >= 5) then
    return false
  end
...
function c38383368.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not eg:IsContains(c) and tc and Duel.GetLocationCount(tc:GetControler(), LOCATION_MZONE, tp) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE, tc:GetControler())
  end
...
function c38383368.drcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetSummonType() == SUMMON_TYPE_SPECIAL + SUMMON_VALUE_SELF
end
