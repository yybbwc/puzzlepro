...
function c55262310.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c55262310.filter(chkc, tp)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c55262310.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function c55262310.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsSetCard(0x15b) or c:IsControler(1 - e:GetHandlerPlayer())
end
...
function c55262310.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local ec = e:GetHandler():GetEquipTarget()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and chkc:IsAbleToDeck()
  end
  if chk == 0 then
    return ec and ec:IsCanChangePosition() and Duel.IsExistingTarget(Card.IsAbleToDeck, tp, 0, LOCATION_GRAVE, 1, nil)
  end
...
function c55262310.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return ec and ec:IsLevelAbove(1) and ec:IsCanChangePosition()
  end
  Duel.SetOperationInfo(0, CATEGORY_POSITION, ec, 1, 0, 0)
end
