...
function s.cfilter(c, e, tp)
  return c:IsRace(RACE_FISH) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function s.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsSetCard(0x11b8) and not c:IsStatus(STATUS_BATTLE_DESTROYED) and re:IsActiveType(TYPE_SPELL + TYPE_TRAP) and Duel.IsChainDisablable(ev)
end
