...
function c29303524.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c29303524.ctfilter(chkc, atk)
  end
  if chk == 0 then
    return c:IsAbleToHand() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0 and Duel.IsExistingTarget(c29303524.ctfilter, tp, 0, LOCATION_MZONE, 1, nil, atk)
  end
