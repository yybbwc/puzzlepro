...
function c42237854.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c42237854.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c42237854.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 42237854, 0, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 4, RACE_MACHINE, ATTRIBUTE_EARTH)
  end
...
function c42237854.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 42237854, 0, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 4, RACE_MACHINE, ATTRIBUTE_EARTH) then
    return
  end
