...
function s.ctcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() ~= tp and aux.dscon(e, tp, eg, ep, ev, re, r, rp) and (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE)
end
function s.ctcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanAddCounter(0x67, 1) and Duel.IsExistingMatchingCard(Card.IsDiscardable, tp, LOCATION_HAND, 0, 1, nil)
  end
  Duel.DiscardHand(tp, Card.IsDiscardable, 1, 1, REASON_COST + REASON_DISCARD)
end
function s.cttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMatchingGroup(Card.IsFaceup, tp, LOCATION_MZONE, 0, e:GetHandler()):GetSum(Card.GetAttack) > 0
  end
end
function s.ctop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFaceup() and c:IsRelateToEffect(e) and #g > 0 then
    ...
    c:RegisterEffect(e1)
    Duel.BreakEffect()
    if c:IsCanAddCounter(0x67, 1) then
      c:AddCounter(0x67, 1)
    end
  end
  local fid = 0
