...
function c49936169.filter(c)
  return c:IsSetCard(0xb2, 0x107) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function c49936169.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and c49936169.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c49936169.filter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c49936169.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoHand(tc, nil, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if Duel.Recover(tp, d, REASON_EFFECT) <= 0 then
      return
    end
    local tg = Duel.GetMatchingGroup(c49936169.cfilter, tp, LOCATION_HAND, 0, nil)
    if #tg <= 0 or not Duel.IsPlayerCanDraw(tp) or not Duel.SelectYesNo(tp, aux.Stringid(49936169, 0)) then
      return
    end
    Duel.BreakEffect()
