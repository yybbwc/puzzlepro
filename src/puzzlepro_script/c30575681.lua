...
function c30575681.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30575681.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c30575681.eqfilter1(c)
  return c:IsSetCard(0x207a) and c:GetEquipTarget() and Duel.IsExistingTarget(c30575681.eqfilter2, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c:GetEquipTarget(), c)
end
...
function c30575681.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c30575681.eqfilter1, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil)
  end
