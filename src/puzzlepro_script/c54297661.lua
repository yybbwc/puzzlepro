...
function c54297661.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local ct = -ft + 1
  if chk == 0 then
    return ft > -2 and rg:GetCount() > 1 and (ft > 0 or rg:IsExists(c54297661.mzfilter, ct, nil, tp))
  end
  local g = nil
  if ft > 0 then
...
function c54297661.filter(c, tp)
  return c:IsFaceup() and Duel.IsPlayerCanSpecialSummonMonster(tp, 54297661, 0, TYPES_EFFECT_TRAP_MONSTER, c:GetAttack(), c:GetDefense(), 4, RACE_SPELLCASTER, ATTRIBUTE_DARK, POS_FACEUP_ATTACK)
end
function c54297661.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c54297661.filter(chkc, tp)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > -2 and Duel.IsExistingTarget(c54297661.filter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
...
function c54297661.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup()) then
    return
  end
  local atk = tc:GetAttack()
  local def = tc:GetDefense()
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 54297661, 0, TYPES_EFFECT_TRAP_MONSTER, atk, def, 4, RACE_SPELLCASTER, ATTRIBUTE_DARK, POS_FACEUP_ATTACK) then
    return
  end
