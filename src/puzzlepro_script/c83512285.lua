...
function c83512285.costfilter(c, tp)
  return (c:IsControler(tp) or c:IsFaceup()) and c:IsAttackAbove(2000) and not c:IsCode(93717133) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingTarget(c83512285.rfilter, tp, 0, LOCATION_MZONE, 1, c)
end
function c83512285.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c83512285.costfilter, 1, nil, tp)
  end
...
function c83512285.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c83512285.rfilter(chkc)
  end
  local res = e:GetLabel() == 1 or Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  if chk == 0 then
    e:SetLabel(0)
    return res and Duel.IsExistingTarget(c83512285.rfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c83512285.spfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
