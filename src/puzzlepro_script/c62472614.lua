...
function c62472614.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() and c62472614.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62472614.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
