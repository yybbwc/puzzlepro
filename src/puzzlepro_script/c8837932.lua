...
function c8837932.spfilter(c, e, tp, tid)
  return c:IsReason(REASON_DESTROY) and c:IsType(TYPE_MONSTER) and c:GetTurnID() == tid and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp) and Duel.IsCanAddCounter(tp, 0x1038, 1, c)
end
function c8837932.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local tid = Duel.GetTurnCount()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c8837932.spfilter(chkc, e, tp, tid)
  end
  local ft = Duel.GetLocationCount(1 - tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > 0 and Duel.IsExistingTarget(c8837932.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp, tid)
  end
...
function c8837932.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local ft = Duel.GetLocationCount(1 - tp, LOCATION_MZONE)
  if ft <= 0 or not c:IsRelateToEffect(e) then
    return
  end
  local sg = Duel.GetChainInfo(0, CHAININFO_TARGET_CARDS):Filter(Card.IsRelateToEffect, nil, e)
  if sg:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    return
  end
...
function c8837932.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if re:IsActiveType(TYPE_MONSTER) and rp == 1 - tp and Duel.IsExistingMatchingCard(c8837932.dfilter, tp, 0, LOCATION_MZONE, 1, nil, g) then
    Duel.NegateEffect(ev)
  end
end
function c8837932.descon(e, tp, eg, ep, ev, re, r, rp)
  local g = e:GetHandler():GetCardTarget()
  return eg:FilterCount(c8837932.dfilter, nil, g) > 0 and not Duel.IsExistingMatchingCard(c8837932.dfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil, g)
end
