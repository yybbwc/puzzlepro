...
function c82361809.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c82361809.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c82361809.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    ...
    if Duel.IsPlayerCanDraw(tp, 1) and Duel.GetMatchingGroupCount(Card.IsDefensePos, tp, LOCATION_MZONE, LOCATION_MZONE, nil) >= 3 and Duel.SelectYesNo(tp, aux.Stringid(82361809, 1)) then
      Duel.BreakEffect()
      Duel.ShuffleDeck(tp)
