...
function c11609969.scop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or c:GetLeftScale() == 1 then
    return
  end
  local scl = 2
  if c:GetLeftScale() == 2 then
    scl = 1
  end
...
function c11609969.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c11609969.filter1(chkc) and chkc ~= e:GetHandler()
  end
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(11609969, 0), aux.Stringid(11609969, 1))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(11609969, 0))
  else
    op = Duel.SelectOption(tp, aux.Stringid(11609969, 1)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
