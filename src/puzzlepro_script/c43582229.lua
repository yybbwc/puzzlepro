...
function c43582229.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c43582229.filter(chkc)
  end
  if chk == 0 then
    return true
  end
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c43582229.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(43582229, 0)) then
...
function c43582229.efilter(e, te)
  ...
  return te:IsActiveType(TYPE_MONSTER) and te:IsActivated() and te:GetOwnerPlayer() == 1 - e:GetHandlerPlayer() and te:GetActivateLocation() == LOCATION_MZONE and tc:IsSummonLocation(LOCATION_EXTRA)
end
