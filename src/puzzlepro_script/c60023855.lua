...
function c60023855.cfilter(c, tp)
  return c:IsLevelAbove(1) and c:IsAttribute(ATTRIBUTE_DARK) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c60023855.lvfilter, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c60023855.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c60023855.cfilter, 1, nil, tp)
  end
...
function c60023855.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c60023855.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
...
function c60023855.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c60023855.ovfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c60023855.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c60023855.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c60023855.spfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
