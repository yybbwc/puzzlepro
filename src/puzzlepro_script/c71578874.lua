...
function c71578874.filter(c)
  return c:IsFaceup() and not c:IsAttack(c:GetDefense()) and c:IsDefenseAbove(0) and c:IsSummonType(SUMMON_TYPE_SPECIAL)
end
function c71578874.adtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c71578874.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71578874.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c71578874.swtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c71578874.swfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71578874.swfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
