...
function c93595154.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c93595154.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93595154.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
