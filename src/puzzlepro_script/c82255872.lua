...
function c82255872.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 82255873, 0x137, TYPES_TOKEN_MONSTER, 500, 500, 1, RACE_BEASTWARRIOR, ATTRIBUTE_WIND)
  end
...
function c82255872.repfilter(c)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsReason(REASON_BATTLE) and c:IsSetCard(0x137) and not c:IsReason(REASON_REPLACE)
end
function c82255872.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c82255872.repfilter, 1, nil)
  end
