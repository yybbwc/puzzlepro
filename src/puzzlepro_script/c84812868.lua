...
function c84812868.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c84812868.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c84812868.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c84812868.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c84812868.filter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84812868.filter, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler()) and Duel.IsExistingTarget(c84812868.filter2, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
