...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (t1 or t2)
  end
  local lv = 0
  if t1 and t2 then
...
function s.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  local lv = e:GetLabel()
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, id + o, 0x1a2, TYPES_TOKEN_MONSTER, 0, 0, lv, RACE_PYRO, ATTRIBUTE_DARK) then
    return
  end
