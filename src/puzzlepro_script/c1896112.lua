...
function c1896112.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c1896112.cfilter, tp, LOCATION_MZONE, 0, 1, nil, ft)
  end
...
function c1896112.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c1896112.filter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp) and Duel.IsExistingTarget(c1896112.filter, 1 - tp, LOCATION_REMOVED, 0, 1, nil, e, 1 - tp)
  end
