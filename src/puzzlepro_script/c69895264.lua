...
function c69895264.tdfilter(c)
  return (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsSetCard(0x17e) and not c:IsCode(69895264) and c:IsAbleToDeck()
end
function c69895264.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c69895264.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69895264.tdfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 2, nil)
  end
...
function c69895264.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg:GetCount() > 0 then
    ...
    if ct > 0 and Duel.IsExistingMatchingCard(c69895264.demfilter, tp, LOCATION_MZONE, 0, 1, nil) and sg:GetClassCount(Card.GetCode) >= ct and ft >= ct and Duel.SelectYesNo(tp, aux.Stringid(69895264, 0)) then
