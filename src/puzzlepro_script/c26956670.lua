...
function c26956670.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c26956670.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c26956670.filter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c26956670.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsRace(RACE_ROCK) and Duel.SpecialSummonStep(tc, 0, tp, tp, false, false, POS_FACEUP) then
    c:SetCardTarget(tc)
  end
