...
function c39468724.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c39468724.tgfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.CheckReleaseGroupEx(tp, c39468724.filter, 1, REASON_EFFECT, true, e:GetHandler())
  end
...
function c39468724.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroupEx(tp, nil, 1, REASON_COST, true, nil, tp)
  end
