...
function c17787975.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c17787975.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17787975.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c17787975.damtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc and Duel.GetAttackTarget() == tc and at and at:IsControler(1 - tp) and at:GetAttack() > tc:GetAttack() and c:GetFlagEffect(17787975) == 0
  end
