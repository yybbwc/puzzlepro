...
function s.rmtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.rmfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.rmfilter2, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function s.rmop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) and tc:IsRace(RACE_FISH) and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_REMOVED) and tc:GetLevel() > 0 and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, tc:GetLevel()) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
