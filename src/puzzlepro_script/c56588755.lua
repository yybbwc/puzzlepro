...
function c56588755.filter(c, tp)
  ...
  return c:IsFaceup() and ctype ~= 0 and c:IsAbleToExtra() and Duel.IsExistingTarget(c56588755.filter2, tp, 0, LOCATION_MZONE, 1, nil, ctype)
end
...
function c56588755.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c56588755.filter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil, tp)
  end
