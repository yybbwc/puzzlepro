...
function c50546208.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c50546208.thfilter1(chkc, tp)
  end
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and Duel.IsExistingTarget(c50546208.thfilter1, tp, LOCATION_ONFIELD, 0, 1, nil, tp)
  end
...
function c50546208.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c50546208.thfilter2, tp, LOCATION_DECK, 0, 1, nil)
  end
