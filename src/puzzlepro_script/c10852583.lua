...
function c10852583.mvop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local tc = eg:GetFirst()
  if not c:IsRelateToEffect(e) or c:IsControler(1 - tp) or not tc:IsRelateToEffect(e) or tc:IsControler(tp) then
    return
  end
  ...
  seq2 = 4 - seq2
  if (seq1 > seq2 and Duel.CheckLocation(tp, LOCATION_MZONE, seq1 - 1)) or (seq1 < seq2 and Duel.CheckLocation(tp, LOCATION_MZONE, seq1 + 1)) then
    local nseq = 0
    if seq1 > seq2 and Duel.CheckLocation(tp, LOCATION_MZONE, seq1 - 1) then
      nseq = seq1 - 1
    else
      nseq = seq1 + 1
    end
