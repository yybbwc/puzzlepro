...
function c67098114.discon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.GetTurnPlayer() == tp and ep ~= tp and bit.band(Duel.GetCurrentPhase(), 0x38) ~= 0 and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
...
function c67098114.regop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if rp == 1 - tp and c:IsPreviousControler(tp) and c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_ONFIELD) and bit.band(pos, POS_FACEUP) ~= 0 then
...
function c67098114.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67098114.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c67098114.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c67098114.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67098114.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
