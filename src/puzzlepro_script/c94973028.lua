...
function c94973028.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsType, 2, nil, TYPE_TOKEN)
  end
...
function c94973028.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c94973028.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c94973028.desfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
