...
function c16780318.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16780318.exfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingMatchingCard(c16780318.spfilter, tp, LOCATION_GRAVE, 0, 4, nil, e, tp) and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function c16780318.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg and Duel.SendtoDeck(tg, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tg:IsLocation(LOCATION_EXTRA) then
    if Duel.GetLocationCount(tp, LOCATION_MZONE) < 4 or Duel.IsPlayerAffectedByEffect(tp, 59822133) then
      return
    end
      ...
      if Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
        ...
        if Duel.Draw(p, d, REASON_EFFECT) ~= 0 then
          ...
          if tc:IsType(TYPE_MONSTER) and tc:IsSetCard(0xe6) then
            if tc:IsCanBeSpecialSummoned(e, 0, tp, true, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(16780318, 0)) then
...
function c16780318.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16780318.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
