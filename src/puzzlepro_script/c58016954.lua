...
function c58016954.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_PZONE) and c58016954.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58016954.cfilter, tp, LOCATION_PZONE, 0, 1, nil) and Duel.IsExistingTarget(c58016954.filter, tp, LOCATION_PZONE, LOCATION_PZONE, 1, nil)
  end
...
function c58016954.operation(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c58016954.cfilter, tp, LOCATION_PZONE, 0, 1, nil) then
    return
  end
