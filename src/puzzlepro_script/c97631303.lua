...
function c97631303.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97631303.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c97631303.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabel() == 1 and c:IsRelateToEffect(e) and Duel.SendtoGrave(c, REASON_EFFECT) > 0 and c:IsLocation(LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c97631303.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(97631303, 2)) then
    Duel.BreakEffect()
...
function c97631303.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97631303.drfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil)
  end
  local ct = 1
