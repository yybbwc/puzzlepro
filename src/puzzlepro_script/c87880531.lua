...
function c87880531.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c87880531.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsAbleToEnterBP() and Duel.IsExistingTarget(c87880531.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c87880531.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not bc or not bc:IsStatus(STATUS_BATTLE_DESTROYED) then
    return
  end
