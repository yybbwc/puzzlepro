...
function c32224143.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c32224143.filter(chkc, atk)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c32224143.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, atk)
  end
