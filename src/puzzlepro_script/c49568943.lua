...
function c49568943.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c49568943.setfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
...
function c49568943.spfilter(c, e, tp)
  ...
  return c:IsFaceup() and c:GetSequence() <= 4 and c:GetOriginalType() & TYPE_MONSTER ~= 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone)
end
function c49568943.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c49568943.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c49568943.spfilter, tp, LOCATION_SZONE, 0, 1, nil, e, tp)
  end
