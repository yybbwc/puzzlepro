...
function s.confilter(c)
  return c:IsFaceup() and not c:IsType(TYPE_TOKEN) and (c:IsType(TYPE_NORMAL) or c:IsSetCard(0x1b9) and c:IsLevelAbove(5))
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and s.nbfilter(chkc) and c ~= chkc
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.nbfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, c)
  end
