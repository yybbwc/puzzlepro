...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.sfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function s.rtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.rfilter(chkc)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingTarget(s.rfilter, tp, LOCATION_MZONE, 0, 1, nil) and c:IsAbleToHand()
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function s.rop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc and tc:IsRelateToEffect(e) and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) > 0) then
    return
  end
  local c = e:GetHandler()
