...
function c28957126.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c28957126.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c28957126.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c28957126.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.bfgcost(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.IsExistingMatchingCard(c28957126.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c28957126.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28957126.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
