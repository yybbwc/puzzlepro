...
function s.cfilter(c, tp)
  return c:IsFaceupEx() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and (c:IsRace(RACE_FIEND) or c:IsAllTypes(TYPE_FUSION + TYPE_MONSTER))
end
...
function s.indtg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return s.indfilter(chkc, eg, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.indfilter, tp, LOCATION_MZONE, 0, 1, nil, eg, tp)
  end
...
function s.indtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tg:IsOnField() and tg:IsCanBeEffectTarget(e)
  end
...
function s.effectfilter(e, ct)
  ...
  return tc and tc == te:GetHandler() and bit.band(loc, LOCATION_MZONE) ~= 0 and tc:GetFlagEffect(id) ~= 0
end
