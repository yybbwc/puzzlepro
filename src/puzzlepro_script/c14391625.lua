...
function c14391625.retfilter(c, e)
  return c:IsSetCard(0x198) and c:IsType(TYPE_MONSTER) and c:IsFaceupEx() and c:IsAbleToDeck() and c:IsCanBeEffectTarget(e)
end
function c14391625.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE + LOCATION_REMOVED) and c14391625.retfilter(chkc, e)
  end
  local c = e:GetHandler()
  local g = Duel.GetMatchingGroup(c14391625.retfilter, tp, LOCATION_MZONE + LOCATION_GRAVE + LOCATION_REMOVED, 0, nil, e)
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:CheckSubGroup(aux.mzctcheck, 1, #g, tp)
  end
