...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsControler(tp) and chkc:IsFaceup()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0
  end
...
function s.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.DiscardHand(tp, nil, 1, 1, REASON_EFFECT + REASON_DISCARD) ~= 0 and tc:IsRelateToEffect(e) and tc:IsFaceup() then
    local e1 = Effect.CreateEffect(c)
