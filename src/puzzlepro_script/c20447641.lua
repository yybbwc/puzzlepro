...
function c20447641.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 20447641, 0xaa, TYPES_EFFECT_TRAP_MONSTER, 1800, 1000, 4, RACE_MACHINE, ATTRIBUTE_EARTH)
  end
...
function c20447641.ttcon(e, c, minc)
  ...
  return minc <= 3 and Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c20447641.ttfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
