...
function c39581190.tdcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c == Duel.GetAttacker() and d and d:IsPosition(POS_FACEDOWN_DEFENSE) and Duel.IsExistingMatchingCard(c39581190.cfilter, tp, LOCATION_MZONE, 0, 1, c)
end
function c39581190.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local d = Duel.GetAttackTarget()
  if chk == 0 then
    return d:IsAbleToDeck()
  end
...
function c39581190.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c39581190.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
