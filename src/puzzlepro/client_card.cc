...
namespace ygo {
  ...
  void ClientCard::UpdateInfo(unsigned char *buf) {
    ...
    if (flag & QUERY_POSITION) {
      ...
      if ((location & (LOCATION_EXTRA | LOCATION_REMOVED)) && (irr::u8)pdata != position) {
        position = pdata;
