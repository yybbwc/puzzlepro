...
function c75775867.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c75775867.cfilter, 1, nil)
  end
...
function c75775867.repfilter(c, e)
  return c:IsFaceup() and c:IsSetCard(0x26) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
end
function c75775867.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE) and c:IsDefensePos() and Duel.IsExistingMatchingCard(c75775867.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
