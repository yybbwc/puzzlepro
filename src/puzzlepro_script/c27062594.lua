...
function c27062594.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 27062594, 0, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_LIGHT)
  end
...
function c27062594.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not Duel.IsPlayerCanSpecialSummonMonster(tp, 27062594, 0, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_LIGHT) then
    return
  end
...
function c27062594.damcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27062594.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
