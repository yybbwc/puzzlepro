...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.smfilter), tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp, code) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
...
function s.gctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and s.gcfilter1(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.gcfilter1, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil, tp)
  end
