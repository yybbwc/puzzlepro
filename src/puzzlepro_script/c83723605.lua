...
function c83723605.texfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_LINK) and c:IsAttribute(ATTRIBUTE_WATER) and c:IsAbleToExtra() and Duel.IsExistingMatchingCard(c83723605.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
end
function c83723605.spfilter(c, e, tp, rc)
  return c:IsSetCard(0x12b) and c:IsType(TYPE_LINK) and c:IsLink(rc:GetLink()) and not c:IsCode(rc:GetCode()) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_LINK, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, rc, c) > 0
end
function c83723605.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c83723605.texfilter(chkc, e, tp)
  end
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_LMATERIAL) and Duel.IsExistingTarget(c83723605.texfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c83723605.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc, nil, 0, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_EXTRA) and aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_LMATERIAL) then
