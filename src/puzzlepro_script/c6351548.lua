...
function c6351548.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6351548.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c6351548.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c6351548.tdfilter(chkc)
  end
  if chk == 0 then
    return true
  end
