...
function c65196094.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  if chk == 0 then
    return Duel.GetCustomActivityCount(65196094, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c65196094.cfilter(c, e, tp, g, maxc)
  return c:IsFaceup() and c:IsType(TYPE_SYNCHRO) and c:IsAbleToExtraAsCost() and g:CheckWithSumEqual(Card.GetLevel, c:GetLevel(), 1, maxc)
end
...
function c65196094.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local maxc = ft + 1
  if maxc > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    maxc = 1
  end
  if chk == 0 then
...
function c65196094.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local ct = g:GetCount()
  if ct == 0 or (ct > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) or ct > Duel.GetLocationCount(tp, LOCATION_MZONE) then
    return
  end
