...
function c48686504.costfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_PLANT) and Duel.GetMZoneCount(tp, c, tp) > 0
end
function c48686504.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c48686504.costfilter, 1, nil, tp)
  end
...
function c48686504.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c48686504.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
