...
function c90448279.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 90448279) > 0 and Duel.GetFlagEffect(tp, 90448280) == 0
  end
...
function c90448279.ovtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c90448279.ofilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil)
  end
end
