...
function c71095768.fselect(g, tp)
  return g:GetClassCount(Card.GetLocation) == g:GetCount() and g:GetClassCount(Card.GetLevel) == 1 and Duel.IsExistingMatchingCard(Card.IsXyzSummonable, tp, LOCATION_EXTRA, 0, 1, nil, g, 2, 2)
end
function c71095768.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and g:CheckSubGroup(c71095768.fselect, 2, 2, tp)
  end
...
function c71095768.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 1 or g:GetCount() == 0 then
    return
  end
...
function c71095768.descon(e, tp, eg, ep, ev, re, r, rp)
  if not (Duel.GetChainInfo(ev, CHAININFO_CHAIN_ID) == c71095768[0] and c71095768[1] == LOCATION_MZONE and re:IsActiveType(TYPE_XYZ)) then
    return false
  end
...
function c71095768.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and chkc:IsType(TYPE_SPELL + TYPE_TRAP)
  end
  if chk == 0 then
