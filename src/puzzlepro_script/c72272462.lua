...
function c72272462.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72272462.atkfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
end
...
function c72272462.thfilter(c, e, tp, check)
  return (c:IsSetCard(0x164) and c:IsType(TYPE_MONSTER) or c:IsCode(68468459)) and (c:IsAbleToHand() or check and c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
function c72272462.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local check = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72272462.thfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, check)
  end
end
