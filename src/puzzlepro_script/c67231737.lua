...
function c67231737.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not Duel.IsExistingMatchingCard(c67231737.oathfilter, tp, LOCATION_MZONE, 0, 1, a)
  end
  local e1 = Effect.CreateEffect(c)
...
function c67231737.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_MZONE, 0, e:GetHandler(), TYPE_LINK) > 0
  end
end
