...
function c42055234.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c42055234.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c42055234.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c42055234.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToHand() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c42055234.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
