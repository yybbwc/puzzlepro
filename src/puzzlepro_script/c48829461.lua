...
function c48829461.uqfilter(c)
  if Duel.IsPlayerAffectedByEffect(c:GetControler(), 75223115) and Duel.IsExistingMatchingCard(c48829461.sfilter, c:GetControler(), LOCATION_MZONE, LOCATION_MZONE, 1, nil) then
...
function c48829461.spfilter(c, e, tp)
  return c:IsCode(74509280) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c48829461.thfilter, tp, LOCATION_DECK, 0, 1, c)
end
...
function c48829461.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c48829461.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
