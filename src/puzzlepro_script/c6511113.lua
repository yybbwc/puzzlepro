...
function c6511113.filter(c)
  return c:GetType() == TYPE_TRAP and c:IsSetCard(0x4c, 0x89) and c:IsAbleToGraveAsCost() and c:CheckActivateEffect(false, true, false) ~= nil
end
function c6511113.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_COST) and Duel.IsExistingMatchingCard(c6511113.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
  e:SetLabel(0)
