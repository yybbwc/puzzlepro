...
function c61204971.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c61204971.filter(chkc, e:GetHandler():GetAttack())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61204971.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e:GetHandler():GetAttack())
  end
...
function c61204971.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:GetBaseAttack() < c:GetAttack() then
