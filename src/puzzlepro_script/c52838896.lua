...
function c52838896.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(52838896, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c52838896.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c52838896.filter(chkc, e, tp)
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  local ct = Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE)
  if chk == 0 then
    return ft > 0 and ct > 0 and Duel.IsExistingTarget(c52838896.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
