...
function c38030232.tkfilter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_EFFECT) and Duel.IsPlayerCanSpecialSummonMonster(tp, 38030233, 0x12c, TYPES_TOKEN_MONSTER, c:GetBaseAttack(), 0, 4, RACE_WYRM, ATTRIBUTE_LIGHT)
end
function c38030232.tktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c38030232.tkfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c38030232.tkfilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
