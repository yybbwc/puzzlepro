...
function c42901635.spcostfilter(c)
  return (c:IsLocation(LOCATION_HAND + LOCATION_GRAVE) or c:IsFaceup()) and c:IsAbleToRemoveAsCost() and c:IsCode(42023223, 79418928, 15502037)
end
...
function c42901635.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c42901635.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function c42901635.spfilter(c, e, tp)
  return c:IsFaceup() and c:IsCode(42023223, 79418928, 15502037) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsCanBeEffectTarget(e)
end
function c42901635.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and g:CheckSubGroupEach(c42901635.spchecks)
  end
