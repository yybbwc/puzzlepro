...
function c21924381.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c21924381.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c21924381.filter, tp, LOCATION_MZONE, 0, 1, Duel.GetAttackTarget())
  end
...
function c21924381.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) and a:IsRelateToBattle() and a:IsAttackable() and a:IsRelateToEffect(e) and not a:IsImmuneToEffect(e) then
