...
function c42925441.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  c:SetTurnCounter(0)
  c:RegisterEffect(e1)
  if c42925441.effcost(e, tp, eg, ep, ev, re, r, rp, 0) and c42925441.efftg(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.SelectYesNo(tp, 94) then
...
function c42925441.effcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(42925441) == 0
  end
...
function c42925441.costfilter1(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x55, 0x7b) and Duel.GetMZoneCount(tp, c) > 0 and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c42925441.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp, c)
end
function c42925441.spfilter1(c, e, tp, cc)
  return c:IsSetCard(0x55) and c:IsType(TYPE_MONSTER) and not c:IsOriginalCodeRule(cc:GetOriginalCodeRule()) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function c42925441.costfilter3(c, e, tp)
  return c:IsFaceup() and c:IsCode(93717133) and Duel.GetMZoneCount(tp, c) > 0 and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c42925441.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, c)
end
...
function c42925441.efftg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = Duel.IsExistingMatchingCard(c42925441.costfilter2, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c42925441.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  if chk == 0 then
    return b1 or b2
  end
