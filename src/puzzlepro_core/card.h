...
public:
  ...
  bool is_extra_deck_monster() const {
    return !!(data.type & TYPES_EXTRA_DECK);
  }
  ...
  template <typename T> void filter_effect_container(const effect_container &container, uint32_t code, effect_filter f, T &eset);
