...
function c97091969.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c97091969.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97091969.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c97091969.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c97091969.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97091969.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
...
function c97091969.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x111) and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c97091969.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return not c:IsStatus(STATUS_DESTROY_CONFIRMED) and c:IsAbleToGrave() and eg:IsExists(c97091969.repfilter, 1, nil, tp)
  end
