...
function c13662809.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c13662809.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
...
function c13662809.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN) and Duel.IsExistingMatchingCard(c13662809.filter2, tp, LOCATION_EXTRA, 0, 1, nil)
end
...
function c13662809.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c13662809.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
