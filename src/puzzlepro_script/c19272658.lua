...
function c19272658.ovtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c19272658.ovfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19272658.ovfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(Card.IsType, tp, 0, LOCATION_GRAVE, 1, nil, TYPE_MONSTER)
  end
...
function c19272658.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c19272658.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19272658.posfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
