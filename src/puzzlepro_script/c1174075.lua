...
function c1174075.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c1174075.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c1174075.discon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev) and ep == 1 - tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsExistingMatchingCard(c1174075.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
