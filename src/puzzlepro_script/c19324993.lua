...
function c19324993.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(19324993, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c19324993.ffilter(c, tp)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x8) and Duel.IsExistingMatchingCard(c19324993.thfilter, tp, LOCATION_DECK, 0, 1, nil, c)
end
...
function c19324993.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19324993.ffilter, tp, LOCATION_EXTRA, 0, 1, nil, tp)
  end
end
