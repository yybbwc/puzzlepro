...
function c75560629.eqlimit(e, c)
  return not c:IsCode(83812099) or c:IsDisabled() or not c:GetEquipGroup():IsExists(Card.IsCode, 1, e:GetHandler(), 75560629)
end
function c75560629.filter(c)
  return c:IsFaceup() and (not c:IsCode(83812099) or c:IsDisabled() or not c:GetEquipGroup():IsExists(Card.IsCode, 1, nil, 75560629))
end
function c75560629.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c75560629.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c75560629.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
