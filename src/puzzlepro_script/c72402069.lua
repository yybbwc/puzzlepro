...
function c72402069.desfilter1(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_SYNCHRO) and c:IsSetCard(0x10af) and Duel.IsExistingMatchingCard(c72402069.desfilter2, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttack())
end
...
function c72402069.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c72402069.desfilter1(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c72402069.desfilter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c72402069.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() then
    ...
    if oc > 0 then
      Duel.Damage(1 - tp, oc * 1000, REASON_EFFECT)
    end
  end
end
...
function c72402069.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not eg:IsContains(e:GetHandler()) and Duel.IsExistingMatchingCard(c72402069.disfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
end
