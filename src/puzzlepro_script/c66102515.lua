...
function s.cfilter(c, tp)
  return c:IsSummonLocation(LOCATION_EXTRA) and c:IsFaceup() and c:IsRace(RACE_CYBERSE) and c:IsControler(tp)
end
function s.tfilter(c, tp)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsRace(RACE_CYBERSE) and c:IsControler(tp)
end
function s.discon(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) then
    return false
  end
  if not Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) then
    return false
  end
