...
function c62941499.seqfilter(c, seq, tp)
  ...
  if seq >= 5 and loc == LOCATION_SZONE then
    return false
  end
  if seq == 7 and loc == LOCATION_MZONE then
    return false
  end
  local cseq = c:GetSequence()
  local cloc = c:GetLocation()
  if cloc == LOCATION_SZONE and cseq >= 5 then
    return false
  end
  if cloc == LOCATION_MZONE and cseq >= 5 and loc == LOCATION_MZONE and (seq == 1 and cseq == 5 or seq == 3 and cseq == 6 or seq == cseq) then
    return true
  end
...
function c62941499.seqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 1, REASON_EFFECT) and Duel.IsExistingMatchingCard(c62941499.desfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
  local filter = 0
  for i = 0, 15 do
...
function c62941499.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ct <= 0 or not c:CheckRemoveOverlayCard(tp, 1, REASON_EFFECT) then
    return
  end
...
function c62941499.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetTurnPlayer() == 1 - tp and (ph == PHASE_MAIN1 or (ph >= PHASE_BATTLE_START and ph <= PHASE_BATTLE) or ph == PHASE_MAIN2)
end
