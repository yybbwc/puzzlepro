...
function c28868394.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28868394.costfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c28868394.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c28868394.setfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c28868394.setfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
