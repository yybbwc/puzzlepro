...
function c87800375.spfilter(c, e, tp)
  return c:IsSetCard(0x107a) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and not Duel.IsExistingMatchingCard(c87800375.eqfilter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil, c:GetCode())
end
function c87800375.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c87800375.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
