...
function c83094004.seqfilter(c, e, tp)
  ...
  if not (c:IsFaceup() and g:IsContains(c) and c:IsCanAddCounter(0x105c, 1)) then
    return false
  end
  for i = 0, 4 do
...
function c83094004.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c83094004.seqfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0) > 0 and Duel.GetLocationCount(1 - tp, LOCATION_MZONE, PLAYER_NONE, 0) > 0 and Duel.IsExistingTarget(c83094004.seqfilter, tp, 0, LOCATION_MZONE, 1, nil, e, tp)
  end
...
function c83094004.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or c:IsControler(1 - tp) or c:IsImmuneToEffect(e) or not tc:IsRelateToEffect(e) or tc:IsControler(tp) or tc:IsImmuneToEffect(e) then
    return
  end
  local filter = 0
  for i = 0, 4 do
