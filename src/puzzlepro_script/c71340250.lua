...
function c71340250.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c71340250.costfilter, 1, nil, tp, ft)
  end
...
function c71340250.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c71340250.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c71340250.spfilter(c, e, tp, lv)
  return c:IsSetCard(0x16) and c:IsType(TYPE_FUSION) and c:IsLevelBelow(lv) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c71340250.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c71340250.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, cg:GetCount())
  end
...
function c71340250.sfilter(c, e, tp, lv)
  return c:IsSetCard(0x16) and c:IsType(TYPE_FUSION) and c:IsLevel(lv) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
