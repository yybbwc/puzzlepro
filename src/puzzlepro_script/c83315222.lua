...
function c83315222.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(83315222, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c83315222.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReleasable() and c83315222.cost(e, tp, eg, ep, ev, re, r, rp, 0)
  end
...
function c83315222.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c83315222.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c83315222.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and c83315222.cost(e, tp, eg, ep, ev, re, r, rp, 0)
  end
...
function c83315222.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c83315222.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
