...
function c44133040.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c44133040.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44133040.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c44133040.setcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 1000) and Duel.IsExistingMatchingCard(c44133040.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
