...
function c58004362.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(58004362, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c58004362.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c58004362.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c58004362.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c58004362.cfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xc008) and Duel.IsExistingMatchingCard(c58004362.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c58004362.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    e:SetLabel(0)
    return c58004362.cost(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.CheckReleaseGroup(tp, c58004362.cfilter, 1, nil, tp)
  end
