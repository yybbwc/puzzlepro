...
function c19153590.cfilter(c)
  return c:IsType(TYPE_MONSTER) and c:IsRace(RACE_DRAGON) and c:IsAbleToGraveAsCost() and (c:IsAttribute(ATTRIBUTE_WIND) or c:IsLevelAbove(7))
end
function c19153590.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19153590.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c19153590.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19153590.tgfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
...
function c19153590.checkop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 19153590) ~= 0 then
    return
  end
