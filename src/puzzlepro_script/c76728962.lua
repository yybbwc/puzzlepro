...
function c76728962.filter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and bit.band(c:GetPreviousRaceOnField(), RACE_MACHINE) ~= 0 and bit.band(c:GetPreviousAttributeOnField(), ATTRIBUTE_DARK) ~= 0 and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c76728962.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c76728962.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
