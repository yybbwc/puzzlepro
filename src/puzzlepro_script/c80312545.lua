...
function c80312545.filter(c)
  return (c:IsCode(6007213, 32491822, 69890967) or ((aux.IsCodeListed(c, 6007213) or aux.IsCodeListed(c, 32491822) or aux.IsCodeListed(c, 69890967)) and c:IsType(TYPE_MONSTER))) and c:IsAbleToHand()
end
function c80312545.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80312545.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c80312545.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c80312545.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c80312545.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80312545.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
