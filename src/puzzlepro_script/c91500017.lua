...
function c91500017.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91500017.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c91500017.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and c:IsCode(15259703) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c91500017.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c91500017.repfilter, 1, nil, tp)
  end
