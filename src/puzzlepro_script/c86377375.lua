...
function c86377375.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86377375.thfilter1, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c86377375.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return ((Duel.CheckReleaseGroup(tp, Card.IsAttribute, 1, nil, ATTRIBUTE_DARK) and Duel.IsPlayerCanDraw(tp, 1)) or (Duel.CheckReleaseGroup(tp, Card.IsAttribute, 1, c, ATTRIBUTE_DARK) and Duel.IsExistingMatchingCard(c86377375.thfilter2, tp, LOCATION_DECK, 0, 1, nil)))
  end
  local sg = nil
...
function c86377375.drop(e, tp, eg, ep, ev, re, r, rp)
  if e:GetLabelObject() ~= e:GetHandler() and Duel.IsExistingMatchingCard(c86377375.thfilter2, tp, LOCATION_DECK, 0, 1, nil) and (not Duel.IsPlayerCanDraw(tp, 1) or Duel.SelectYesNo(tp, aux.Stringid(86377375, 2))) then
