...
function c95697223.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c95697223.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c95697223.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetMatchingGroupCount(Card.IsStatus, tp, 0, LOCATION_MZONE, nil, STATUS_SPSUMMON_TURN) > 0
  end
