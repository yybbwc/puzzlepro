...
function c84851250.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == Duel.GetMatchingGroupCount(c84851250.ntfilter, tp, LOCATION_MZONE, 0, nil)
end
...
function c84851250.damcon(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c84851250.damfilter, tp, LOCATION_MZONE, 0, 1, nil) then
    return false
  end
