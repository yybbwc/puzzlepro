...
function s.stgt(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and s.sfilter(chkc, e, tp)
  end
  if chk == 0 then
...
function s.atkfilter1(c, e, tp)
  return c:IsSetCard(0x119) and c:IsFaceup() and c:GetBaseAttack() > 0 and Duel.IsExistingTarget(s.atkfilter2, tp, LOCATION_GRAVE, 0, 1, c, c:GetCode())
end
...
function s.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.atkfilter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function s.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(tp) and sc:IsRelateToEffect(e) and Duel.SendtoDeck(sc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 then
