...
function c58543073.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58543073.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c58543073.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c58543073.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 58543073, 0x2016, TYPES_NORMAL_TRAP_MONSTER + TYPE_TUNER, 0, 0, 1, RACE_MACHINE, ATTRIBUTE_WIND) and Duel.IsExistingTarget(c58543073.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c58543073.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and not tc:IsImmuneToEffect(e) then
    ...
    if c:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 58543073, 0x2016, TYPES_NORMAL_TRAP_MONSTER + TYPE_TUNER, 0, 0, 1, RACE_MACHINE, ATTRIBUTE_WIND) ~= 0 then
