...
function c97729135.mvtg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c97729135.mvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97729135.mvfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE, PLAYER_NONE, 0, zone) > 0
  end
...
function c97729135.mvop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or tc:IsControler(tp) or Duel.GetLocationCount(1 - tp, LOCATION_MZONE, PLAYER_NONE, 0, zone) <= 0 then
    return
  end
...
function c97729135.mvtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c97729135.mvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97729135.mvfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0, zone) > 0
  end
  Duel.Hint(HINT_SELECTMSG, tp, aux.Stringid(97729135, 2))
...
function c97729135.mvop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = Duel.GetFirstTarget()
  if not tc:IsRelateToEffect(e) or tc:IsControler(1 - tp) or Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0, zone) <= 0 then
    return
  end
