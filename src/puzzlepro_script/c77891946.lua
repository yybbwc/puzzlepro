...
function c77891946.spfilter1(c, e, tp)
  return c:IsSetCard(0x172) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c77891946.spfilter2, tp, LOCATION_DECK, 0, 1, c, e, tp, c)
end
function c77891946.spfilter2(c, e, tp, ec)
  return c:IsSetCard(0x172) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and aux.IsCodeListed(ec, c:GetCode())
end
function c77891946.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingMatchingCard(c77891946.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp) and not Duel.IsPlayerAffectedByEffect(tp, 59822133)
  end
