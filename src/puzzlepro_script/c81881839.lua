...
function c81881839.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return g:CheckSubGroup(c81881839.fselect, 2, 2)
  end
...
function c81881839.cfilter(c)
  return c:IsFaceup() and c:IsSetCard(0xe5) and Duel.IsExistingMatchingCard(c81881839.namefilter, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c, c:GetCode())
end
function c81881839.nametg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c81881839.cfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81881839.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
