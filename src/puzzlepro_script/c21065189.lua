...
function c21065189.spfilter(c, e, tp, seq)
  if not (c:IsType(TYPE_LINK) and c:IsRace(RACE_CYBERSE) and c:IsLink(4)) then
    return false
  end
...
function c21065189.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c21065189.spfilter(chkc, e, tp, seq)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c21065189.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, seq)
  end
