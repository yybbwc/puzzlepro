...
function c52083044.cfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x126) and not c:IsCode(52083044) and c:IsAbleToHand() and Duel.GetMZoneCount(tp, c) > 0
end
function c52083044.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c52083044.cfilter(chkc, tp)
  end
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c52083044.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c52083044.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = Duel.GetFirstTarget()
  if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) and c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    local e1 = Effect.CreateEffect(c)
