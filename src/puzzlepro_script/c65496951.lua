...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:IsHasType(EFFECT_TYPE_ACTIVATE) or Duel.IsExistingMatchingCard(s.pfilter, tp, LOCATION_PZONE, 0, 1, nil)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp, op)
  if op == nil then
    if not Duel.IsExistingMatchingCard(s.pfilter, tp, LOCATION_PZONE, 0, 1, nil) then
      return
    end
  ...
  if op == 1 then
    if Duel.Recover(tp, 500, REASON_EFFECT) < 1 then
      return
    end
