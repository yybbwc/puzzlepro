...
function c3828844.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c3828844.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c3828844.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c3828844.spfilter(c, e, tp)
  if not (c:IsRankAbove(5) and c:IsRace(RACE_PLANT) and c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)) then
    return false
  end
...
function c3828844.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c3828844.spfilter, tp, LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c3828844.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    if not c:IsRelateToEffect(e) or not c:IsCanOverlay() or not aux.NecroValleyFilter()(c) then
      return
    end
    if c:IsLocation(LOCATION_HAND + LOCATION_DECK) or (not c:IsLocation(LOCATION_GRAVE) and c:IsFacedown()) then
      return
    end
