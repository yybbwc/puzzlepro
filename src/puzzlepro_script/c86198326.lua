...
function c86198326.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c86198326.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86198326.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_EQUIP)
  local g = Duel.SelectTarget(tp, c86198326.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, 1, nil)
  local opt = 0
  if g:GetFirst():IsDefenseAbove(0) then
    opt = Duel.SelectOption(tp, aux.Stringid(86198326, 0), aux.Stringid(86198326, 1))
  end
  e:SetLabel(opt)
