...
function c71331215.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanAddCounter(tp, 0x5a, 3, e:GetHandler())
  end
end
...
function c71331215.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and tc:IsLocation(LOCATION_GRAVE) and tc:IsReason(REASON_BATTLE) and at:IsRelateToBattle() and at:IsControler(tp) and at:IsSetCard(0x15f)
end
function c71331215.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x5a, 1, REASON_EFFECT) and Duel.IsPlayerCanDraw(tp, 1)
  end
