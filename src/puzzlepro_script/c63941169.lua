...
function s.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, gc)
  end
...
function s.cfilter2(c, tp, se)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:GetPreviousAttributeOnField() & ATTRIBUTE_WATER ~= 0 and c:IsPreviousLocation(LOCATION_MZONE) and (se == nil or c:GetReasonEffect() ~= se)
end
