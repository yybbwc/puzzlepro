...
function c86801871.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 86801872, 0, TYPES_TOKEN_MONSTER, 1200, 1200, 3, RACE_REPTILE, ATTRIBUTE_EARTH) then
    return
  end
...
function c86801871.damcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if eg:IsContains(tok) then
    ...
    if not tok:IsLocation(LOCATION_MZONE) then
      e:Reset()
    end
    return false
  end
