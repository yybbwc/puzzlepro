...
function c10032958.effop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ct > 0 and c:IsFaceup() and c:IsRelateToEffect(e) then
    ...
    if opt1 == 0 then
      opt2 = Duel.SelectOption(tp, aux.Stringid(10032958, 2), aux.Stringid(10032958, 3)) + 1
    elseif opt1 == 2 then
      opt2 = Duel.SelectOption(tp, aux.Stringid(10032958, 1), aux.Stringid(10032958, 2))
    else
    ...
    if opt1 ~= 0 and opt2 ~= 0 then
      opt3 = Duel.SelectOption(tp, aux.Stringid(10032958, 1))
    elseif opt1 ~= 1 and opt2 ~= 1 then
      opt3 = Duel.SelectOption(tp, aux.Stringid(10032958, 2)) + 1
    else
      opt3 = Duel.SelectOption(tp, aux.Stringid(10032958, 3)) + 2
    end
