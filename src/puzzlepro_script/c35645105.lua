...
function c35645105.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c35645105.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c35645105.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc ~= c and chkc:IsLocation(LOCATION_MZONE) and c35645105.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c35645105.lvfilter, tp, LOCATION_MZONE, 0, 1, c)
  end
