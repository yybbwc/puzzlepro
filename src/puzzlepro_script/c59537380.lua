...
function c59537380.spfilter(c, e, tp, zone)
  return c:IsRace(RACE_DRAGON) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c59537380.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local zone = aux.GetMultiLinkedZone(tp)
  if chk == 0 then
    return zone ~= 0 and Duel.IsExistingMatchingCard(c59537380.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, zone)
  end
