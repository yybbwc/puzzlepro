...
function c83888009.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetCount() > 0 and g:FilterCount(c83888009.filter, nil) == g:GetCount() and Duel.GetMZoneCount(tp, g) > 0
  end
...
function c83888009.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c83888009.spfilter(chkc, e, tp)
  end
  if chk == 0 then
...
function c83888009.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 and g:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(83888009, 0)) then
    Duel.BreakEffect()
