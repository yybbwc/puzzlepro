...
function s.dfilter(c, tp)
  return c:IsAttribute(ATTRIBUTE_WATER) and c:IsDiscardable() and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetLevel())
end
...
function s.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and Duel.IsExistingMatchingCard(s.dfilter, tp, LOCATION_HAND, 0, 1, c, tp)
  end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, e:GetLabel())
  end
