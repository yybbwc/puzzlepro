...
function c31772684.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_DESTROY) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp)
end
...
function c31772684.mtcon(e, tp, eg, ep, ev, re, r, rp)
  return r == REASON_RITUAL and e:GetHandler():IsPreviousLocation(LOCATION_MZONE) and eg:IsExists(Card.IsSetCard, 1, nil, 0x106)
end
...
function c31772684.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c31772684.rmfilter(chkc) and chkc:IsControler(1 - tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c31772684.rmfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
