...
function c72648577.filter(c)
  return c:IsSetCard(0xaf) and c:IsAbleToDeck() and (not c:IsLocation(LOCATION_MZONE) or c:IsFaceup()) and (c:IsType(TYPE_MONSTER) or c:IsLocation(LOCATION_PZONE))
end
...
function c72648577.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72648577.filter, tp, LOCATION_MZONE + LOCATION_PZONE + LOCATION_GRAVE + LOCATION_HAND, 0, 3, nil)
  end
