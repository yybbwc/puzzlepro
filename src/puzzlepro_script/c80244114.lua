...
function c80244114.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c80244114.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c80244114.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c80244114.indtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c80244114.indfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c80244114.indfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c80244114.addown(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():GetFlagEffect(80244114) == 0 then
    return
  end
