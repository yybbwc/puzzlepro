...
function c28762303.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c28762303.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and e:GetHandler():IsAbleToGrave() and Duel.IsExistingTarget(c28762303.spfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c28762303.cfilter(c, tp)
  return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsAttribute(ATTRIBUTE_WATER) and c:IsReason(REASON_BATTLE + REASON_EFFECT)
end
...
function c28762303.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c28762303.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
