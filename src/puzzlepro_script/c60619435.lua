...
function c60619435.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c60619435.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c60619435.costfilter(c, e, tp)
  return c:IsRace(RACE_INSECT) and not c:IsPublic() and Duel.IsExistingMatchingCard(c60619435.sfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp, c:GetAttack())
end
function c60619435.sfilter(c, e, tp, atk)
  return c:IsFaceup() and c:IsRace(RACE_INSECT) and c:IsCanBeEffectTarget(e) and c:GetAttack() < atk and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function c60619435.ofilter(c, e, tp)
  return c:IsFaceup() and c:IsCanBeEffectTarget(e) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function c60619435.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return e:IsCostChecked() and #mg > 0 and #emg > 0
  end
