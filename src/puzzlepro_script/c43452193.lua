...
function c43452193.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local at = Duel.GetAttackTarget()
  if chk == 0 then
    return a:IsOnField() and a:IsAbleToChangeControler() and Duel.GetMZoneCount(1 - tp, a, 1 - tp, LOCATION_REASON_CONTROL) > 0 and at:IsOnField() and at:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, at, tp, LOCATION_REASON_CONTROL) > 0
  end
