...
function c75886890.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c75886890.eqtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c75886890.filter2(chkc, ec)
  end
  if chk == 0 then
    return ec and Duel.IsExistingTarget(c75886890.filter2, tp, LOCATION_GRAVE, 0, 1, nil, ec)
  end
