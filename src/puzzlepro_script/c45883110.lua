...
function c45883110.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c45883110.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c45883110.spfilter(c, e, tp)
  return aux.IsCodeOrListed(c, 68468459) and not c:IsCode(45883110) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c45883110.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c45883110.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c45883110.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
