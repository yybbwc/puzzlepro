...
function c98904974.condition(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c98904974.cfilter, tp, LOCATION_MZONE, 0, 1, nil) then
    return false
  end
...
function c98904974.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg, REASON_EFFECT) ~= 0 and Duel.SelectYesNo(tp, aux.Stringid(98904974, 0)) then
    Duel.BreakEffect()
