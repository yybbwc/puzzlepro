...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 and Duel.IsExistingMatchingCard(s.smfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, code) and Duel.SelectYesNo(tp, aux.Stringid(id, 0)) then
...
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and s.rmfilter1(chkc, tp)
  end
  if chk == 0 then
