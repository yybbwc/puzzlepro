...
function c43040603.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c43040603.cfilter, 1, nil, tp)
  end
...
function c43040603.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    e:SetLabel(0)
    return res and Duel.IsPlayerCanSpecialSummon(tp) and not Duel.IsPlayerAffectedByEffect(tp, 63060238) and not Duel.IsPlayerAffectedByEffect(tp, 97148796) and Duel.IsExistingMatchingCard(Card.IsSummonableCard, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsPlayerCanDiscardDeck(tp, 1)
  end
