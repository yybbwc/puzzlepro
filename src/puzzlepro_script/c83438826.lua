...
function c83438826.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c83438826.eqfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83438826.eqfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c83438826.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c83438826.desfilter(chkc)
  end
  local eq = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return eq and eq:IsAttackAbove(500) and Duel.IsExistingTarget(c83438826.desfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c83438826.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or eq:IsImmuneToEffect(e) or not eq:IsAttackAbove(500) then
    return
  end
  local e1 = Effect.CreateEffect(c)
...
function c83438826.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c83438826.eqfilter2(chkc)
  end
  if chk == 0 then
    return e:GetHandler():IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c83438826.eqfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_EQUIP)
...
function c83438826.operation2(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local tc = Duel.GetFirstTarget()
  if tc and c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(tp) and c83438826.eqlimit(nil, tc) and c:CheckUniqueOnField(tp) then
    Duel.Equip(tp, c, tc)
  end
end
