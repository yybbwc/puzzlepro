...
function c20934852.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c20934852.thfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c20934852.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetTurnPlayer() == 1 - tp and bit.band(loc, LOCATION_SZONE) ~= 0 and re:IsActiveType(TYPE_SPELL + TYPE_TRAP) and Duel.IsChainDisablable(ev)
end
...
function c20934852.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c20934852.spfilter, tp, LOCATION_SZONE, 0, 1, nil, e, tp, e:GetHandler())
  end
