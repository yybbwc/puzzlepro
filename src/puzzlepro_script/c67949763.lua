...
function c67949763.filter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and Duel.IsPlayerCanSpecialSummonMonster(tp, 67949764, 0x87, TYPES_TOKEN_MONSTER, -2, 0, 1, RACE_FIEND, ATTRIBUTE_DARK)
end
function c67949763.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c67949763.filter(chkc, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and Duel.IsExistingTarget(c67949763.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c67949763.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 then
    ...
    if not Duel.IsPlayerCanSpecialSummonMonster(tp, 67949764, 0x87, TYPES_TOKEN_MONSTER, -2, 0, 1, RACE_FIEND, ATTRIBUTE_DARK) then
      return
    end
    if cr then
