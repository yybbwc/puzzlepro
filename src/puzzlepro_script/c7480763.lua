...
function c7480763.sttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7480763.stfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
end
...
function c7480763.descon(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetTurnPlayer() ~= 1 - tp or Duel.GetTurnCount() == e:GetValue() then
    return false
  end
...
function c7480763.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c7480763.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
