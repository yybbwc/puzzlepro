...
function c20191720.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c20191720.spfilter(chkc)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c20191720.spfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c20191720.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c20191720.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c20191720.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
