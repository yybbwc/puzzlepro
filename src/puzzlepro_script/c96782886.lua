...
function c96782886.costfilter(c, ft, tp)
  return c:IsRace(RACE_PSYCHO) and not c:IsCode(96782886) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup())
end
function c96782886.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 800) and ft > -1 and Duel.CheckReleaseGroup(tp, c96782886.costfilter, 1, nil, ft, tp)
  end
...
function c96782886.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96782886.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
