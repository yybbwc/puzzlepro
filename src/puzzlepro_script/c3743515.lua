...
function s.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dg:GetCount() > 0 then
    ...
    if Duel.Destroy(dg, REASON_EFFECT) > 0 and Duel.IsExistingMatchingCard(Card.IsCanChangePosition, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
...
function s.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.Destroy(c, REASON_EFFECT) ~= 0 then
    if not a or not a:IsRelateToBattle() then
      return
    end
