...
function c6353603.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6353603.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c6353603.filter1(c)
  return c:IsFaceup() and c:IsSetCard(0x7c) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsAbleToGraveAsCost() and Duel.IsExistingTarget(aux.TRUE, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
function c6353603.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6353603.filter1, tp, LOCATION_ONFIELD, 0, 1, nil) or (Duel.IsPlayerAffectedByEffect(tp, 46241344) and Duel.IsExistingTarget(aux.TRUE, 0, LOCATION_MZONE, LOCATION_MZONE, 1, nil))
  end
  if Duel.IsExistingMatchingCard(c6353603.filter1, tp, LOCATION_ONFIELD, 0, 1, nil) and (not Duel.IsPlayerAffectedByEffect(tp, 46241344) or not Duel.SelectYesNo(tp, aux.Stringid(46241344, 0))) then
