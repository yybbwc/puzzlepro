...
function c88875132.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_EARTH) and c:IsRace(RACE_MACHINE) and not c:IsCode(88875132) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c88875132.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c88875132.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88875132.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
...
function c88875132.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(88875132, tp, ACTIVITY_ATTACK) == 0
  end
...
function c88875132.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local lv = c:GetLevel()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c88875132.lvfilter(chkc, lv)
  end
  if chk == 0 then
    return lv > 0 and Duel.IsExistingTarget(c88875132.lvfilter, tp, LOCATION_MZONE, 0, 1, c, lv)
  end
