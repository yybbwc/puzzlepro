...
function c55125728.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c55125728.ctfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c55125728.ctfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c55125728.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c55125728.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c55125728.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c55125728.discon(e, tp, eg, ep, ev, re, r, rp)
  return rp == 1 - tp and Duel.IsChainDisablable(ev) and re:IsActiveType(TYPE_MONSTER) and Duel.GetMatchingGroupCount(c55125728.disfilter, tp, LOCATION_MZONE, 0, nil) >= 5 and e:GetHandler():GetFlagEffect(55125728) <= 0
end
