...
function c63193879.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(63193879, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c63193879.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c63193879.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c63193879.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
  ...
  if g:GetFirst():IsLevel(1) then
    op = Duel.SelectOption(tp, aux.Stringid(63193879, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(63193879, 1), aux.Stringid(63193879, 2))
  end
  e:SetLabel(op)
end
