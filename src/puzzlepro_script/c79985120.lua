...
function c79985120.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c79985120.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c79985120.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    ...
    if dam < 0 or tc:IsFacedown() then
      dam = 0
    end
...
function c79985120.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c79985120.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp) and e:GetHandler():IsCanOverlay()
  end
