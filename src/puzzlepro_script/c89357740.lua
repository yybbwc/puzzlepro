...
function s.tgfilter(c, e, tp)
  ...
  return c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsControler(tp) and Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_REMOVED, 0, 1, nil, c, tp, exct)
end
function s.cfilter(c, ec, tp, exclude_modern_count)
  return c:IsFaceup() and c:IsType(TYPE_UNION) and c:CheckUniqueOnField(tp) and not c:IsForbidden() and c:CheckUnionTarget(ec) and aux.CheckUnionEquip(c, ec, exclude_modern_count)
end
function s.retg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, id, 0, TYPE_EFFECT + TYPE_MONSTER, 2100, 1000, 5, RACE_MACHINE, ATTRIBUTE_LIGHT) and Duel.IsExistingMatchingCard(s.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
