...
function c79724755.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and ct > 0 and dg:FilterCount(Card.IsAbleToRemove, nil) == ct
  end
...
function c79724755.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c79724755.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c79724755.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c79724755.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c79724755.cfilter2, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil) and Duel.IsExistingMatchingCard(c79724755.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
end
