...
function c27642961.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  elseif re then
    ...
    return eg:IsExists(c27642961.cfilter, 1, nil, tp) and rc and rc:IsSetCard(0xee) and rc:IsControler(tp) and re:IsActiveType(TYPE_MONSTER)
  end
  return false
end
...
function c27642961.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c27642961.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
