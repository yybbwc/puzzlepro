...
function c2461031.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c2461031.filter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil)
  end
...
function c2461031.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c2461031.lvfilter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():GetEquipTarget() and Duel.IsExistingTarget(c2461031.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
