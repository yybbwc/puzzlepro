...
function c82134632.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100, 0)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c82134632.rmfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
  local ct = 0
  local min = 1
  while true do
...
function c82134632.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    ...
    e:SetLabel(0, ct)
    return Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingMatchingCard(c82134632.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
