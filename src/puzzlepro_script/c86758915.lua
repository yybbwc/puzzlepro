...
function c86758915.cfilter(c, tp)
  return (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsSetCard(0x117) and not c:IsCode(86758915) and Duel.GetMZoneCount(tp, c) > 0 and c:IsAbleToGraveAsCost()
end
function c86758915.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86758915.cfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil, tp)
  end
...
function c86758915.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToHand() and Duel.IsExistingMatchingCard(c86758915.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
