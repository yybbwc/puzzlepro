...
function c13382806.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c13382806.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingTarget(c13382806.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c13382806.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.IsExistingMatchingCard(c13382806.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp, tc:GetAttribute()) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(13382806, 0)) then
    Duel.BreakEffect()
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
