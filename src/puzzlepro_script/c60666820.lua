...
function c60666820.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c60666820.cfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c60666820.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c60666820.filter(chkc, ft)
  end
  if chk == 0 then
    return ft > -1 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c60666820.filter, tp, LOCATION_MZONE, 0, 1, nil, ft)
  end
