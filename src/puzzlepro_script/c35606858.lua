...
function c35606858.mtop(e, tp, eg, ep, ev, re, r, rp)
  if (Duel.CheckLPCost(tp, 500) or Duel.IsPlayerAffectedByEffect(tp, 94585852)) and Duel.SelectEffectYesNo(tp, e:GetHandler(), aux.Stringid(35606858, 2)) then
    if not Duel.IsPlayerAffectedByEffect(tp, 94585852) or not Duel.SelectEffectYesNo(tp, e:GetHandler(), aux.Stringid(94585852, 1)) then
...
function c35606858.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(35606858) == 0
  end
...
function c35606858.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c35606858.tgfilter, tp, LOCATION_DECK, 0, 1, nil, ev)
  end
...
function c35606858.dccost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:GetFlagEffect(35606859) == 0
  end
  c:RegisterFlagEffect(35606859, RESET_CHAIN, 0, 1)
end
function c35606858.cfilter(c, e, tp)
  return c:IsRace(RACE_FIEND) and c:IsType(TYPE_MONSTER) and c:IsLocation(LOCATION_GRAVE) and c:IsControler(tp) and (c:IsAbleToHand() or c:IsAbleToDeck() or Duel.GetLocationCount(tp, LOCATION_MZONE) > 0)
end
function c35606858.dctg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummon(tp) and eg:IsExists(c35606858.cfilter, 1, nil, e, tp)
  end
