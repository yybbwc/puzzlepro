...
function c43959432.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c43959432.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c43959432.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsPlayerCanSpecialSummonMonster(tp, 43959432, 0, TYPES_EFFECT_TRAP_MONSTER, 1000, 1000, 4, RACE_ROCK, ATTRIBUTE_EARTH)
  end
...
function c43959432.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 43959432, 0, TYPES_EFFECT_TRAP_MONSTER, 1000, 1000, 4, RACE_ROCK, ATTRIBUTE_EARTH) then
    return
  end
...
function c43959432.atkval(e, c)
  ...
  if not tc or tc:GetEquipTarget() ~= c then
    return 0
  end
