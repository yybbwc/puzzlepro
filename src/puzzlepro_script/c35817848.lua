...
function c35817848.sctg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsSynchroSummonable, tp, LOCATION_EXTRA, 0, 1, nil, nil)
  end
...
function c35817848.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c35817848.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c35817848.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
