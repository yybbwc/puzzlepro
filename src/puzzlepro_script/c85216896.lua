...
function c85216896.cfilter(c, tp, zone)
  ...
  return c:IsPreviousControler(tp) and bit.extract(zone, seq) ~= 0 and c:IsSummonLocation(LOCATION_EXTRA) and c:IsPreviousLocation(LOCATION_MZONE) and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)
end
...
function c85216896.thfilter(c)
  return (c:IsLocation(LOCATION_GRAVE) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM))) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c85216896.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85216896.thfilter, tp, LOCATION_GRAVE + LOCATION_EXTRA, 0, 1, nil)
  end
...
function c85216896.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c85216896.rmfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) and Duel.IsExistingTarget(c85216896.rmfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
