...
function s.mgfilter(c, e, tp, link)
  return c:IsControler(tp) and c:IsLocation(LOCATION_GRAVE) and bit.band(c:GetReason(), 0x10000008) == 0x10000008 and c:GetReasonCard() == link and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoDeck(tc, nil, SEQ_DECKTOP, REASON_EFFECT) ~= 0 then
    ...
    if sumtype == SUMMON_TYPE_LINK and tc:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and mg:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 0)) then
