...
function c41141943.hspcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and not Duel.IsExistingMatchingCard(c41141943.filter, tp, LOCATION_GRAVE, 0, 1, nil)
end
...
function c41141943.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingMatchingCard(c41141943.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
