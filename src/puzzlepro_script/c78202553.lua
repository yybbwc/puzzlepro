...
function c78202553.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c78202553.tgfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 2)
  end
...
function c78202553.spcon(e, tp, eg, ep, ev, re, r, rp)
  return eg:IsExists(c78202553.cfilter, 1, nil, tp) and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c78202553.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(78202553) == 0
  end
