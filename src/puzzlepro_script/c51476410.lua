...
function c51476410.spfilter3(c, e, tp, chkf, rc)
  if not c:IsType(TYPE_FUSION) or not c:IsAbleToExtra() then
    return false
  end
  if Duel.GetLocationCountFromEx(tp, tp, rc, TYPE_FUSION) <= 0 then
    return false
  end
...
function c51476410.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanRemove(tp) and Duel.IsExistingMatchingCard(c51476410.spfilter3, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, chkf, e:GetHandler())
  end
...
function c51476410.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsRace, tp, LOCATION_GRAVE, 0, 1, nil, RACE_CYBERSE) and Duel.IsExistingMatchingCard(c51476410.atkfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
end
