...
function c21249921.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c21249921.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c21249921.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c21249921.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:GetEquipGroup():IsExists(c21249921.atkfilter, 1, nil, e:GetHandlerPlayer())
  end
