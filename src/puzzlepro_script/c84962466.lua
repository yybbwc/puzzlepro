...
function c84962466.cfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_BEAST + RACE_BEASTWARRIOR) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c84962466.filter, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c84962466.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84962466.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c84962466.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c84962466.filter(chkc)
  end
  if chk == 0 then
