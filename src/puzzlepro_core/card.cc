...
int32_t card::get_infos(byte *buf, uint32_t query_flag, int32_t use_cache) {
  ...
  if (query_flag & QUERY_TARGET_CARD) {
    ...
    for (const auto &pcard : effect_target_cards) {
...
uint32_t card::get_level() {
  if ((data.type & (TYPE_XYZ | TYPE_LINK)) || (status & STATUS_NO_LEVEL) || (!(data.type & TYPE_MONSTER) && !(get_type() & TYPE_MONSTER) && !is_affected_by_effect(EFFECT_PRE_MONSTER))) {
    return 0;
  }
...
uint32_t card::get_fusion_attribute(uint8_t playerid) {
  ...
  if (effects.empty() || pduel->game_field->core.not_material) {
...
uint32_t card::get_linked_zone() {
  ...
  if ((s == 0 && is_link_marker(LINK_MARKER_TOP_RIGHT)) || (s == 1 && is_link_marker(LINK_MARKER_TOP)) || (s == 2 && is_link_marker(LINK_MARKER_TOP_LEFT))) {
    zones |= (1u << 5) | (1u << (16 + 6));
  }
  if ((s == 2 && is_link_marker(LINK_MARKER_TOP_RIGHT)) || (s == 3 && is_link_marker(LINK_MARKER_TOP)) || (s == 4 && is_link_marker(LINK_MARKER_TOP_LEFT))) {
    zones |= (1u << 6) | (1u << (16 + 5));
  }
  if (s == 5) {
...
void card::xyz_overlay(const card_set &materials) {
  ...
  for (auto &pcard : cv) {
    ...
    if (pcard->previous.location == LOCATION_GRAVE) {
      ...
      pduel->game_field->raise_single_event(pcard, nullptr, EVENT_LEAVE_GRAVE, pduel->game_field->core.reason_effect, pcard->current.reason, pduel->game_field->core.reason_player, 0, 0);
    }
    else if (pcard->previous.location == LOCATION_DECK || pcard->previous.location == LOCATION_EXTRA) {
      leave_deck.insert(pcard);
      pduel->game_field->raise_single_event(pcard, nullptr, EVENT_LEAVE_DECK, pduel->game_field->core.reason_effect, pcard->current.reason, pduel->game_field->core.reason_player, 0, 0);
    }
    pduel->write_buffer32(pcard->get_info_location());
    pduel->write_buffer32(pcard->current.reason);
  }
  if (!leave_grave.empty() || !leave_deck.empty()) {
    if (!leave_grave.empty()) {
  ...
  if (!des.empty()) {
    pduel->game_field->destroy(des, nullptr, REASON_LOST_TARGET + REASON_RULE, PLAYER_NONE);
  }
  else {
...
int32_t card::add_effect(effect *peffect) {
  ...
  if (peffect->type & EFFECT_TYPE_SINGLE && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE) && peffect->owner == this && get_status(STATUS_DISABLED) && (peffect->reset_flag & RESET_DISABLE)) {
    return 0;
  }
  ...
  if (peffect->get_code_type() == CODE_PHASE && peffect->code & (PHASE_DRAW | PHASE_STANDBY | PHASE_END) && peffect->type & (EFFECT_TYPE_TRIGGER_O | EFFECT_TYPE_TRIGGER_F) && !peffect->is_flag(EFFECT_FLAG_COUNT_LIMIT)) {
  ...
  if (peffect->type & EFFECT_TYPE_SINGLE) {
    ...
    if (peffect->code == EFFECT_SET_ATTACK_FINAL && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
      for (auto it = single_effect.begin(); it != single_effect.end();) {
        ...
        if ((code == EFFECT_UPDATE_ATTACK || code == EFFECT_SET_ATTACK || code == EFFECT_SET_ATTACK_FINAL) && !rm->second->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
          remove_effect(rm->second);
        }
      }
    }
    ...
    if (peffect->code == EFFECT_SET_DEFENSE_FINAL && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
      for (auto it = single_effect.begin(); it != single_effect.end();) {
        auto rm = it++;
        auto code = rm->second->code;
        if ((code == EFFECT_UPDATE_DEFENSE || code == EFFECT_SET_DEFENSE || code == EFFECT_SET_DEFENSE_FINAL) && !rm->second->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
          remove_effect(rm->second);
        }
      }
    }
    ...
    if (peffect->code == EFFECT_SET_BASE_ATTACK_FINAL && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
      for (auto it = single_effect.begin(); it != single_effect.end();) {
        auto rm = it++;
        auto code = rm->second->code;
        if ((code == EFFECT_SET_BASE_ATTACK || code == EFFECT_SET_BASE_ATTACK_FINAL || code == EFFECT_SET_ATTACK || code == EFFECT_SET_ATTACK_FINAL) && !rm->second->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
          remove_effect(rm->second);
        }
      }
    }
    ...
    if (peffect->code == EFFECT_SET_BASE_DEFENSE_FINAL && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
      for (auto it = single_effect.begin(); it != single_effect.end();) {
        auto rm = it++;
        auto code = rm->second->code;
        if ((code == EFFECT_SET_BASE_DEFENSE || code == EFFECT_SET_BASE_DEFENSE_FINAL || code == EFFECT_SET_DEFENSE || code == EFFECT_SET_DEFENSE_FINAL) && !rm->second->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
          remove_effect(rm->second);
        }
      }
    }
  ...
  if (current.controler != PLAYER_NONE && !check_target.empty()) {
    if (peffect->is_disable_related()) {
      for (const auto &target : check_target) {
  ...
  if (peffect->type & EFFECT_TYPE_SINGLE && peffect->code == EFFECT_UPDATE_LEVEL && !peffect->is_flag(EFFECT_FLAG_SINGLE_RANGE)) {
    ...
    if (val > 0) {
      pduel->game_field->raise_single_event(this, nullptr, EVENT_LEVEL_UP, peffect, 0, 0, 0, val);
      pduel->game_field->process_single_event();
    }
  }
  return peffect->id;
...
int32_t card::copy_effect(uint32_t code, uint32_t reset, int32_t count) {
  ...
  for (const auto &peffect : pduel->uncopy) {
...
void card::reset(uint32_t id, uint32_t reset_type) {
  ...
  if (reset_type == RESET_EVENT) {
    ...
    if (id & (RESET_TODECK | RESET_TOHAND | RESET_TOGRAVE | RESET_REMOVE | RESET_TEMP_REMOVE | RESET_OVERLAY | RESET_MSCHANGE)) {
      clear_relate_effect();
    }
    if (id & (RESET_TODECK | RESET_TOHAND | RESET_TOGRAVE | RESET_REMOVE | RESET_TEMP_REMOVE | RESET_OVERLAY | RESET_MSCHANGE | RESET_LEAVE | RESET_TOFIELD)) {
    ...
    if (id & (RESET_TODECK | RESET_TOHAND | RESET_TOGRAVE | RESET_REMOVE | RESET_TEMP_REMOVE | RESET_OVERLAY | RESET_MSCHANGE | RESET_LEAVE | RESET_TOFIELD | RESET_TURN_SET)) {
      battled_cards.clear();
      reset_effect_count();
    ...
    if (id & (RESET_TODECK | RESET_TOHAND | RESET_TOGRAVE | RESET_REMOVE | RESET_TEMP_REMOVE | RESET_OVERLAY | RESET_MSCHANGE | RESET_TOFIELD | RESET_TURN_SET)) {
      counters.clear();
    }
    if (id & (RESET_TODECK | RESET_TOHAND | RESET_TOGRAVE | RESET_REMOVE | RESET_TEMP_REMOVE | RESET_LEAVE | RESET_TOFIELD | RESET_TURN_SET | RESET_CONTROL)) {
...
int32_t card::is_has_relation(effect *peffect) {
  for (const auto &it : relate_effect) {
    if (it.first == peffect) {
      return TRUE;
    }
  }
...
int32_t card::leave_field_redirect(uint32_t reason) {
  ...
  for (effect_set::size_type i = 0; i < es.size(); ++i) {
    ...
    if (((redirect & LOCATION_HAND) && !is_affected_by_effect(EFFECT_CANNOT_TO_HAND) && pduel->game_field->is_player_can_send_to_hand(es[i]->get_handler_player(), this)) or ((redirect & LOCATION_DECK) && !is_affected_by_effect(EFFECT_CANNOT_TO_DECK) && pduel->game_field->is_player_can_send_to_deck(es[i]->get_handler_player(), this))) {
      redirects |= redirect;
    }
...
int32_t card::add_counter(uint8_t playerid, uint16_t countertype, uint16_t count, uint8_t singly) {
  ...
  pduel->game_field->raise_single_event(this, nullptr, EVENT_ADD_COUNTER + countertype, pduel->game_field->core.reason_effect, REASON_EFFECT, playerid, playerid, pcount);
  pduel->game_field->process_single_event();
  return TRUE;
}
...
void card::set_material(card_set *materials) {
  ...
  for (const auto &pcard : material_cards) {
...
void card::set_special_summon_status(effect *peffect) {
  if ((peffect->code == EFFECT_SPSUMMON_PROC || peffect->code == EFFECT_SPSUMMON_PROC_G) && peffect->is_flag(EFFECT_FLAG_CANNOT_DISABLE) && peffect->is_flag(EFFECT_FLAG_UNCOPYABLE)) {
    spsummon.code = 0;
    spsummon.code2 = 0;
    spsummon.type = 0;
    spsummon.level = 0;
...
template <typename T> void card::filter_effect_container(const effect_container &container, uint32_t code, effect_filter f, T &eset) {
  ...
  for (auto &it : indexer) {
    ...
    if (peffect->is_disable_related()) {
      if (peffect->type & EFFECT_TYPE_FIELD) {
        ...
        for (const auto &target : effect_target_cards) {
          pduel->game_field->add_to_disable_check_list(target);
        }
      }
...
int32_t card::filter_summon_procedure(uint8_t playerid, effect_set *peset, uint8_t ignore_count, uint8_t min_tribute, uint32_t zone) {
  ...
  if (!ignore_count && !pduel->game_field->core.extra_summon[playerid] && pduel->game_field->core.summon_count[playerid] >= pduel->game_field->get_summon_count_limit(playerid)) {
    ...
    for (int32_t i = 0; i < extra_count.size(); ++i) {
      ...
      int32_t new_min = !retval.empty() ? static_cast<int32_t>(retval[0]) : 0;
    ...
    return pduel->game_field->check_tribute(this, min, max, nullptr, current.controler, zone);
  }
  return FALSE;
}
...
int32_t card::filter_set_procedure(uint8_t playerid, effect_set *peset, uint8_t ignore_count, uint8_t min_tribute, uint32_t zone) {
  ...
  if (!ignore_count && !pduel->game_field->core.extra_summon[playerid] && pduel->game_field->core.summon_count[playerid] >= pduel->game_field->get_summon_count_limit(playerid)) {
    ...
    return pduel->game_field->check_tribute(this, min, max, nullptr, current.controler, zone, 0xff00ff, POS_FACEDOWN_DEFENSE);
  }
  return FALSE;
}
...
void card::filter_spsummon_procedure(uint8_t playerid, effect_set *peset, uint32_t summon_type, material_info info) {
  ...
  for (const auto &peffect : proc_set) {
    uint8_t toplayer{};
    uint8_t topos{};
    ...
    if (peffect->is_available() && peffect->check_count_limit(playerid) && is_spsummonable(peffect, info) && ((topos & POS_FACEDOWN) || !pduel->game_field->check_unique_onfield(this, toplayer, LOCATION_MZONE))) {
      effect *sumeffect = pduel->game_field->core.reason_effect;
      if (!sumeffect) {
        sumeffect = peffect;
      }
      uint32_t sumtype = peffect->get_value(this);
      if ((!summon_type || summon_type == sumtype) && pduel->game_field->is_player_can_spsummon(sumeffect, sumtype, topos, playerid, toplayer, this)) {
...
void card::get_unique_target(card_set *cset, int32_t controler, card *icard) {
  ...
  for (int32_t p = 0; p < 2; ++p) {
    ...
    if (unique_location & LOCATION_MZONE) {
      for (const auto &pcard : player.list_mzone) {
        if (pcard && (pcard != icard) && pcard->is_position(POS_FACEUP) && !pcard->get_status(STATUS_SPSUMMON_STEP) && check_unique_code(pcard)) {
          cset->insert(pcard);
        }
      }
    }
    if (unique_location & LOCATION_SZONE) {
      for (const auto &pcard : player.list_szone) {
...
int32_t card::is_can_be_summoned(uint8_t playerid, uint8_t ignore_count, effect *peffect, uint8_t min_tribute, uint32_t zone) {
  ...
  if (!ignore_count && (pduel->game_field->core.extra_summon[playerid] || !is_affected_by_effect(EFFECT_EXTRA_SUMMON_COUNT)) && pduel->game_field->core.summon_count[playerid] >= pduel->game_field->get_summon_count_limit(playerid)) {
    return FALSE;
  }
  ...
  if (current.location == LOCATION_MZONE) {
    if (is_position(POS_FACEDOWN) || !is_affected_by_effect(EFFECT_DUAL_SUMMONABLE) || is_affected_by_effect(EFFECT_DUAL_STATUS) || !pduel->game_field->is_player_can_summon(SUMMON_TYPE_DUAL, playerid, this, playerid) || is_affected_by_effect(EFFECT_CANNOT_SUMMON)) {
      pduel->game_field->restore_lp_cost();
      return FALSE;
    }
  }
...
int32_t card::is_can_be_special_summoned(effect *reason_effect, uint32_t sumtype, uint8_t sumpos, uint8_t sumplayer, uint8_t toplayer, uint8_t nocheck, uint8_t nolimit, uint32_t zone) {
  ...
  if (is_affected_by_effect(EFFECT_REVIVE_LIMIT) && !is_status(STATUS_PROC_COMPLETE)) {
    if ((!nolimit && (current.location & (LOCATION_GRAVE | LOCATION_REMOVED | LOCATION_SZONE))) || (!nocheck && !nolimit && (current.location & (LOCATION_DECK | LOCATION_HAND)))) {
      return FALSE;
    }
...
int32_t card::is_setable_mzone(uint8_t playerid, uint8_t ignore_count, effect *peffect, uint8_t min_tribute, uint32_t zone) {
  ...
  if (!ignore_count && (pduel->game_field->core.extra_summon[playerid] || !is_affected_by_effect(EFFECT_EXTRA_SET_COUNT)) && pduel->game_field->core.summon_count[playerid] >= pduel->game_field->get_summon_count_limit(playerid)) {
    return FALSE;
  }
  pduel->game_field->save_lp_cost();
...
int32_t card::is_control_can_be_changed(int32_t ignore_mzone, uint32_t zone) {
  ...
  if (pduel->game_field->core.duel_rule <= 4 && (get_type() & TYPE_TRAPMONSTER) && pduel->game_field->get_useable_count(this, 1 - current.controler, LOCATION_SZONE, current.controler, LOCATION_REASON_CONTROL) <= 0) {
    return FALSE;
  }
  if (is_affected_by_effect(EFFECT_CANNOT_CHANGE_CONTROL)) {
    return FALSE;
