...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.rfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function s.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:AddCounter(0x66, 1) then
    return
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_CONFIRM)
...
function s.plpop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if val > 0 and Duel.GetLP(1 - tp) >= val then
    Duel.PayLPCost(1 - tp, val)
  end
end
