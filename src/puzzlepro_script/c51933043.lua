...
function c51933043.sscon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) > 0 and not Duel.IsExistingMatchingCard(c51933043.ssfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c51933043.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c51933043.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51933043.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
