...
function c74153887.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c74153887.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74153887.filter, tp, LOCATION_DECK, 0, 1, nil) or Duel.IsExistingTarget(c74153887.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  local op = 0
  if Duel.IsExistingMatchingCard(c74153887.filter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingTarget(c74153887.filter, tp, LOCATION_GRAVE, 0, 1, nil) then
