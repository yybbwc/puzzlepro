...
function c57348141.filter1(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x38) and Duel.IsExistingMatchingCard(c57348141.filter2, tp, LOCATION_MZONE, 0, 1, c, c:GetRace(), c:GetCode())
end
...
function c57348141.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c57348141.cfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c57348141.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc ~= e:GetHandler() and chkc:IsAbleToRemove()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToRemove, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 2, e:GetHandler())
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function c57348141.spcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsPreviousLocation(LOCATION_DECK) and re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsSetCard(0x38) and bit.band(r, REASON_EFFECT) ~= 0
end
...
function c57348141.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c57348141.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
