...
function c61912252.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c61912252.cfilter, 1, nil, tp)
  end
...
function c61912252.spfilter2(c, e, tp)
  return c:IsLevelBelow(5) and c:IsSetCard(0x127) and not c:IsCode(61912252) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c61912252.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c61912252.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c61912252.spfilter2, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
