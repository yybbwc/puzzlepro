...
function c88409165.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c88409165.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c88409165.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c88409165.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88409165.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
...
function c88409165.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) then
    if tc:IsExtraDeckMonster() or Duel.SelectOption(tp, aux.Stringid(88409165, 1), aux.Stringid(88409165, 2)) == 0 then
