...
function c57511992.tgfilter(c, e, tp, ec, spchk)
  return c:IsAttribute(ATTRIBUTE_WATER) and (c:IsLevelAbove(1) and ec:IsLevelAbove(1) and (not c:IsLevel(ec:GetLevel()) or not c:IsCode(ec:GetCode())) or spchk and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE))
end
function c57511992.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c57511992.tgfilter(chkc, e, tp, c, spchk)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57511992.tgfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c, spchk)
  end
...
function c57511992.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  local b1 = tc:IsRelateToEffect(e) and c:IsRelateToEffect(e) and c:IsFaceup() and c:IsLevelAbove(1) and tc:IsLevelAbove(1) and (not c:IsLevel(tc:GetLevel()) or not c:IsCode(tc:GetCode()))
  local b2 = tc:IsRelateToEffect(e) and Duel.IsEnvironment(22702055) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and aux.NecroValleyFilter()(tc) and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
  if b1 or b2 then
    local s
    if b1 and b2 then
...
function c57511992.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c57511992.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57511992.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
