...
function c61488417.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c61488417.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c61488417.cfilter(c, tp)
  return c:IsSetCard(0x9e) and c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp)
end
