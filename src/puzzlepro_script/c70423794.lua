...
function c70423794.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c70423794.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70423794.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c70423794.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c70423794.desfilter(chkc)
  end
  if chk == 0 then
