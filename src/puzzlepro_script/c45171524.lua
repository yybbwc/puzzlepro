...
function s.costfilter(c, tp)
  return (c:IsControler(tp) or c:IsFaceup()) and c:IsRace(RACE_REPTILE)
end
function s.filter(c)
  return c:IsSetCard(0x1c3) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function s.spfilter(c, e, tp)
  return c:IsSetCard(0x1c3) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if e:IsCostChecked() and Duel.CheckReleaseGroupEx(tp, s.costfilter, 1, REASON_COST, true, nil, tp) and Duel.SelectYesNo(tp, aux.Stringid(id, 1)) then
