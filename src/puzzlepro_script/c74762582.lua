...
function c74762582.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc ~= c and chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c74762582.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c74762582.filter, tp, LOCATION_MZONE, 0, 1, c)
  end
...
function c74762582.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c74762582.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c74762582.posfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c74762582.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_POSCHANGE)
...
function c74762582.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsFaceup() and tc1:IsRelateToEffect(e) and Duel.ChangePosition(tc1, POS_FACEDOWN_DEFENSE) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
