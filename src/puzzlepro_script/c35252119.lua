...
function c35252119.indfilter2(c, tc)
  return c:GetOriginalRace() & tc:GetOriginalRace() ~= 0 and c:GetOriginalAttribute() & tc:GetOriginalAttribute() ~= 0 and not c:IsCode(tc:GetCode())
end
...
function c35252119.fselect(g)
  return g:GetClassCount(Card.GetOriginalRace) == 1 and g:GetClassCount(Card.GetOriginalAttribute) == 1 and g:GetClassCount(Card.GetCode) > 1
end
...
function c35252119.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:CheckSubGroup(c35252119.fselect, 2, 2)
  end
