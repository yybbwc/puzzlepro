...
function c23812568.filter2(c, e, tp)
  return c:IsCanBeEffectTarget(e) and Duel.IsExistingMatchingCard(c23812568.spfilter, tp, LOCATION_DECK, 0, 2, nil, e, tp, c:GetCode())
end
...
function c23812568.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c23812568.cfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 2, e:GetHandler())
  end
...
function c23812568.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and #mg > 0
  end
  local g = mg
  if #mg > 1 then
