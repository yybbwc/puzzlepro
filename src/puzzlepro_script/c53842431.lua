...
function c53842431.thfilter(c)
  return c:IsSetCard(0x10d) and c:IsLevelBelow(7) and c:IsType(TYPE_EFFECT) and c:IsAbleToHand()
end
function c53842431.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsDestructable() and Duel.IsExistingMatchingCard(c53842431.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c53842431.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x1, 4, REASON_COST)
  end
...
function c53842431.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c53842431.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c53842431.rmfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
