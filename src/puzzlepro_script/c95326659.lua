...
function c95326659.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c95326659.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c95326659.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(c95326659.cfilter, tp, LOCATION_SZONE, 0, 2, nil) then
    return
  end
