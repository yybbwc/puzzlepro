...
function s.eqcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():CheckUniqueOnField(tp) and Duel.IsExistingMatchingCard(Card.IsSetCard, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), 0x18d)
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ec and ec:IsAbleToHand() and c:IsAbleToHand()
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local ec = c:GetEquipTarget()
  if not c:IsRelateToEffect(e) or not ec:IsRelateToEffect(e) then
    return
  end
