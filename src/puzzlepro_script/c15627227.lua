...
function c15627227.spfilter1(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_LINK) and Duel.IsExistingMatchingCard(c15627227.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetLinkedZone(tp))
end
...
function c15627227.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c15627227.spfilter1(chkc, e, tp) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c15627227.spfilter1, tp, LOCATION_MZONE, 0, 1, c, e, tp)
  end
...
function c15627227.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c15627227.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c15627227.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
