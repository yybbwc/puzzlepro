...
function c25494711.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c25494711.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c25494711.repfilter(c, e)
  return c:IsFaceup() and c:IsSetCard(0xab) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
end
function c25494711.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(c25494711.repfilter, tp, LOCATION_ONFIELD, 0, 1, c, e)
  end
