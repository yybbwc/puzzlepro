...
function c61496006.spcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) > 0 and not Duel.IsExistingMatchingCard(c61496006.spfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c61496006.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c61496006.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c61496006.damcon(e)
  ...
  if a and d and a:IsControler(tp) and a:IsSetCard(0x48) and a:IsStatus(STATUS_OPPO_BATTLE) and Duel.GetFlagEffect(tp, 61496006) == 0 then
