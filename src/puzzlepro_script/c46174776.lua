...
function s.spfilter(c, e, tp)
  return not c:IsCode(id) and c:IsRace(RACE_ILLUSION) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function s.descon(e, tp, eg, ep, ev, re, r, rp)
  return not re:GetHandler():IsCode(id) and re:GetHandler():IsSetCard(0x1bc, 0x19e) and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED)
end
