...
function c74191528.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74191528.filter, tp, LOCATION_DECK, 0, 5, nil) and Duel.IsExistingMatchingCard(c74191528.filter, 1 - tp, LOCATION_DECK, 0, 5, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, PLAYER_ALL, LOCATION_DECK)
end
function c74191528.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFieldGroupCount(tp, LOCATION_DECK, 0) < 5 or Duel.GetFieldGroupCount(1 - tp, LOCATION_DECK, 0) < 5 or not Duel.IsExistingMatchingCard(c74191528.filter, tp, LOCATION_DECK, 0, 1, nil) or not Duel.IsExistingMatchingCard(c74191528.filter, 1 - tp, LOCATION_DECK, 0, 1, nil) then
    return
  end
