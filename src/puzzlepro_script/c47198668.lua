...
function c47198668.atktg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c47198668.filter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47198668.filter1, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c47198668.filter2(c, e, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsType(TYPE_MONSTER) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and c:IsCanBeEffectTarget(e)
end
...
function c47198668.atktg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c47198668.filter2(chkc, e, tp)
  end
  if chk == 0 then
    return eg:IsExists(c47198668.filter2, 1, nil, e, tp)
  end
...
function c47198668.efilter(e, re, rp)
  if not re:IsActiveType(TYPE_SPELL + TYPE_TRAP) then
    return false
  end
