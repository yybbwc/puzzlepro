...
function c52601736.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c52601736.filter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil)
  end
...
function c52601736.dacon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.IsAbleToEnterBP() and tc and tc:IsAttackable() and tc:GetEffectCount(EFFECT_DIRECT_ATTACK) == 0
end
