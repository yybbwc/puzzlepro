...
function c65976795.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsFacedown()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsFacedown, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, ct, nil)
  end
...
function c65976795.spcon(e, tp, eg, ep, ev, re, r, rp)
  return eg:IsExists(c65976795.cfilter, 1, nil, tp) and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c65976795.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(65976795) == 0
  end
