...
function c51611041.costfilter(c, tp)
  return c:IsType(TYPE_EFFECT) and c:IsFaceup() and Duel.GetMZoneCount(tp, c) >= 2 and Duel.IsPlayerCanSpecialSummonMonster(tp, 51611042, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, c:GetOriginalRace(), c:GetOriginalAttribute())
end
function c51611041.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c51611041.costfilter, 1, nil, tp)
  end
...
function c51611041.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsPlayerCanSpecialSummonCount(tp, 2)
  end
...
function c51611041.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 51611042, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, race, attr) then
    Duel.BreakEffect()
