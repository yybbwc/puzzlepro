...
function c86613346.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c86613346.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c86613346.thfilter(c)
  return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsLevel(7) and c:IsAbleToHand() and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function c86613346.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_MZONE) and chkc:IsControler(tp) and c86613346.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86613346.thfilter, tp, LOCATION_GRAVE + LOCATION_MZONE, 0, 1, e:GetHandler())
  end
