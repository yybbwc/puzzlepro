...
function c25573054.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    if ft == 0 then
      if not Duel.IsExistingMatchingCard(c25573054.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) then
        return false
      end
    else
      if not Duel.IsExistingMatchingCard(c25573054.tgfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
        return false
      end
    end
