...
function c41306080.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsRace, 1, c, RACE_CYBERSE)
  end
...
function c41306080.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41306080.thfilter1, tp, LOCATION_DECK, 0, 1, nil) and not c:IsLevel(4) and c:IsLevelAbove(1)
  end
