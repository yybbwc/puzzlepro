...
function c60992364.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c60992364.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c60992364.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c60992364.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c60992364.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c60992364.attg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eqc and eqc:GetAttackedCount() == 1 and eqc:GetEffectCount(EFFECT_EXTRA_ATTACK) == 0
  end
