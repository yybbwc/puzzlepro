...
function c86157908.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local at = Duel.GetAttacker()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c86157908.atkfilter(chkc) and chkc ~= at
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86157908.atkfilter, tp, LOCATION_MZONE, 0, 1, at)
  end
...
function c86157908.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local at = Duel.GetAttacker()
  if at:IsFaceup() and at:IsRelateToBattle() and at:IsAttackable() and not at:IsStatus(STATUS_ATTACK_CANCELED) and tc:IsFaceup() and tc:IsRelateToEffect(e) then
...
function c86157908.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c86157908.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86157908.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
