...
function s.rmtg(e, c)
  return (c:IsLocation(LOCATION_OVERLAY) or c:IsPreviousLocation(LOCATION_OVERLAY)) and c:IsReason(REASON_COST + REASON_SPSUMMON + REASON_EFFECT)
end
...
function s.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.mattgfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 3, nil, c)
  end
