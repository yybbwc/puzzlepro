...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (Duel.IsExistingMatchingCard(s.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp) or Duel.IsExistingMatchingCard(s.spfilter2, tp, LOCATION_GRAVE, 0, 1, nil, e, tp))
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.IsEnvironment(73206827, tp, LOCATION_FZONE) then
    ...
    if b1 and b2 then
      res = aux.SelectFromOptions(tp, { b1, 60, 1 }, { b2, 61, 0 })
    end
  else
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
