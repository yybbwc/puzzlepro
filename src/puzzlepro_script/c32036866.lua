...
function c32036866.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c32036866.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c32036866.target2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32036866.filter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, tp)
  end
end
