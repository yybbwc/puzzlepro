...
function c4179255.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c4179255.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4179255.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c4179255.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsPosition(POS_FACEUP_ATTACK) and Duel.IsExistingMatchingCard(nil, tp, LOCATION_FZONE, LOCATION_FZONE, 1, nil) then
