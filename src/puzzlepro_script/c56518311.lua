...
function c56518311.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56518311.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56518311.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return g:CheckSubGroup(c56518311.fselect, 2, 2, tp) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c56518311.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c56518311.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c56518311.rmfilter, tp, 0, LOCATION_MZONE + LOCATION_GRAVE, 1, nil)
  end
