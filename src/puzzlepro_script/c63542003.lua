...
function c63542003.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63542003.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c63542003.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c63542003.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c63542003.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.IsExistingMatchingCard(c63542003.thfilter, tp, LOCATION_DECK, 0, 1, nil) then
    Duel.BreakEffect()
...
function c63542003.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToDeck, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, e:GetHandler())
  end
  local ct = 5
  if not Duel.IsExistingMatchingCard(c63542003.filter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil) then
    ct = 3
  end
