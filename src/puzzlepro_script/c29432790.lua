...
function c29432790.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(29432790, tp, ACTIVITY_SUMMON) == 0 and Duel.GetCustomActivityCount(29432790, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c29432790.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29432790.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp) and g:GetCount() > 0 and Duel.GetMZoneCount(tp, g) > 0
  end
