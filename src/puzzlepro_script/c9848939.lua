...
function c9848939.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsType, c:GetControler(), LOCATION_GRAVE, 0, 2, nil, TYPE_NORMAL)
end
...
function c9848939.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9848939.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
