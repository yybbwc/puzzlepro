...
function c25518020.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c25518020.filter(chkc, e, tp, e:GetLabel())
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c25518020.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, e:GetHandler():GetCounter(0x1d))
  end
