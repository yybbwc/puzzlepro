...
function c89310929.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToHand, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
...
function c89310929.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFacedown() or not c:IsRelateToEffect(e) or not tc:IsLocation(LOCATION_HAND) then
    return
  end
  local e1 = Effect.CreateEffect(c)
