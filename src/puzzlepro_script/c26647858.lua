...
function c26647858.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c26647858.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c26647858.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c26647858.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsAttackBelow(1500) and c:IsRace(RACE_WARRIOR)
end
