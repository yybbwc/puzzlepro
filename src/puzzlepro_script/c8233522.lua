...
function c8233522.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and c8233522.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8233522.filter, tp, 0, LOCATION_GRAVE, 1, nil)
  end
