...
function c5852388.desfilter(c, e, tp)
  return c:IsFaceupEx() and c:IsRace(RACE_DINOSAUR) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c5852388.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, c, e, tp)
end
...
function c5852388.dsptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5852388.desfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, e, tp)
  end
