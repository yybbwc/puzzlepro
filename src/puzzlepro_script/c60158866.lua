...
function c60158866.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c60158866.cfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60158866.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c60158866.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if s == 1 then
    if tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
      ...
      if og:GetCount() > 0 and Duel.SendtoGrave(og, REASON_EFFECT) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c60158866.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(60158866, 2)) then
