...
function c85590798.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85590798.cfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingMatchingCard(c85590798.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c85590798.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_DRAGON) and c:IsLevel(7) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c85590798.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c85590798.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
