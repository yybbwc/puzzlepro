...
function c28126717.rmcfilter(c, tp)
  return c:IsLevel(1) and c:IsSetCard(0x16d) and c:IsSummonable(true, nil) and not c:IsPublic() and Duel.IsExistingMatchingCard(c28126717.rmfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c28126717.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28126717.rmcfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c28126717.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28126717.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
