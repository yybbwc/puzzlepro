...
function c96433300.rfilter(c, tp)
  return c:IsRace(RACE_REPTILE) and c:IsType(TYPE_EFFECT) and (c:IsControler(tp) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c) > 0
end
function c96433300.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c96433300.rfilter, 1, nil, tp)
  end
...
function c96433300.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and c96433300.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c96433300.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp)
  end
