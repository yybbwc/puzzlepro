...
function c58272005.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c58272005.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c58272005.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c58272005.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsRace(RACE_DINOSAUR)
end
function c58272005.atcon(e, tp, eg, ep, ev, re, r, rp)
  local ec = e:GetHandler():GetEquipTarget()
  if not eg:IsContains(ec) then
    return false
  end
