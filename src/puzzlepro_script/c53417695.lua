...
function c53417695.desfilter(c, tp, ec)
  return c:IsFaceup() and c:IsSetCard(0x130) and Duel.IsExistingTarget(nil, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, Group.FromCards(c, ec))
end
function c53417695.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c53417695.desfilter, tp, LOCATION_MZONE, 0, 1, nil, tp, c)
  end
...
function c53417695.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c53417695.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
