...
function c75719089.filter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x107a) and Duel.IsExistingMatchingCard(c75719089.eqfilter, tp, LOCATION_DECK, 0, 1, nil, c, tp)
end
...
function c75719089.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c75719089.filter(chkc, tp)
  end
  local ft = 0
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > ft and Duel.IsExistingTarget(c75719089.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
