...
function c41442341.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c41442341.filter(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c41442341.filter, tp, LOCATION_GRAVE, 0, 2, nil, e, tp)
  end
...
function c41442341.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if fg:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    return
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
  if fg:GetCount() > 1 and ft == 1 then
    fg = fg:Select(tp, 1, 1, nil)
  end
