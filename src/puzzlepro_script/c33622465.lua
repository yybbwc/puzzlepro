...
function c33622465.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c33622465.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c33622465.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c33622465.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 33622465, 0, TYPES_NORMAL_TRAP_MONSTER, 1200, 400, 3, RACE_WARRIOR, ATTRIBUTE_EARTH)
  end
