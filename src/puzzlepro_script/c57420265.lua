...
function s.costfilter(c, ec, e, tp)
  if not c:IsAttribute(ATTRIBUTE_WATER) or c:IsPublic() then
    return false
  end
...
function s.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsPublic() and Duel.IsExistingMatchingCard(s.costfilter, tp, LOCATION_HAND, 0, 1, c, c, e, tp)
  end
...
function s.discon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsLocation(LOCATION_GRAVE) and r == REASON_SYNCHRO and e:GetHandler():GetReasonCard():IsAttribute(ATTRIBUTE_WATER)
end
