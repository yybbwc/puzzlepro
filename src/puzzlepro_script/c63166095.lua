...
function c63166095.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63166095.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c63166095.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if Duel.IsPlayerCanDraw(tp, 1) and Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_GRAVE, 0, nil, TYPE_SPELL) >= 3 and Duel.SelectYesNo(tp, aux.Stringid(63166095, 0)) then
      Duel.BreakEffect()
      Duel.ShuffleDeck(tp)
