...
function c28265983.destg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c28265983.desfilter1(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c28265983.descost2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckLPCost(tp, lp, true)
  end
...
function c28265983.destg2(e, tp, eg, ep, ev, re, r, rp, chk)
  local lp = Duel.GetLP(tp) - Duel.GetLP(1 - tp)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28265983.desfilter2, tp, 0, LOCATION_MZONE, 1, nil, lp)
  end
