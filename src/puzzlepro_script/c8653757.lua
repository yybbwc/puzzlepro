...
function c8653757.filter(c, tp)
  return c:IsType(TYPE_CONTINUOUS) and c:IsSetCard(0x55, 0x7b) and (c:IsAbleToHand() or (Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and not c:IsForbidden() and c:CheckUniqueOnField(tp)))
end
function c8653757.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c8653757.filter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
...
function c8653757.etcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetTurnPlayer() ~= tp and c:IsPreviousControler(tp) and c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN) and Duel.IsExistingMatchingCard(c8653757.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
