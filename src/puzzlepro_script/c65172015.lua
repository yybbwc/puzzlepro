...
function c65172015.fselect(g, tp, sc)
  return aux.gfcheck(g, Card.IsOriginalCodeRule, 1561110, 91998119) and Duel.GetLocationCountFromEx(tp, tp, g, sc) > 0
end
...
function c65172015.discon(e, tp, eg, ep, ev, re, r, rp)
  return ep == 1 - tp and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
end
...
function c65172015.spfilter2(c, e, tp)
  return c:IsFaceup() and c:IsCode(1561110) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c65172015.spfilter3, tp, LOCATION_REMOVED, 0, 1, c, e, tp)
end
...
function c65172015.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    ...
    return ft > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingTarget(c65172015.spfilter2, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
