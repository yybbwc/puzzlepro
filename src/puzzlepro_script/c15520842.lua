...
function c15520842.filter(c, tp)
  return (Duel.IsExistingMatchingCard(c15520842.geffilter, tp, LOCATION_ONFIELD, 0, 1, nil) or (c:IsFaceup() and c:IsType(TYPE_XYZ))) and c:IsControlerCanBeChanged()
end
function c15520842.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c15520842.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c15520842.filter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
