...
function c23116808.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c23116808.spfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c23116808.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c23116808.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c23116808.tkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 23116809, 0, TYPES_TOKEN_MONSTER, 100, 100, 1, RACE_PYRO, ATTRIBUTE_FIRE, POS_FACEUP_DEFENSE) then
    return
  end
...
function c23116808.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsAttribute, 1, e:GetHandler(), ATTRIBUTE_FIRE)
  end
