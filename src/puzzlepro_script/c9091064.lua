...
function s.spcon(e, tp, eg, ep, ev, re, r, rp)
  return eg:IsExists(Card.IsLocation, 1, nil, LOCATION_HAND) and r & REASON_COST > 0 and re:IsActivated() and re:GetHandler():IsSetCard(0x195) and rp == tp
end
...
function s.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ct = e:GetLabel()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_HAND, 0, ct, nil)
  end
...
function s.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and Duel.GetFlagEffect(tp, id) == 0
  end
  Duel.RegisterFlagEffect(tp, id, RESET_CHAIN, 0, 1)
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_DECK, 0, 1, nil) and Duel.GetFlagEffect(tp, id) == 0
  end
  Duel.RegisterFlagEffect(tp, id, RESET_CHAIN, 0, 1)
