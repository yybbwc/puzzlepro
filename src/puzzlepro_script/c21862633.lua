...
function c21862633.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return a and at and at:GetAttack() > 0 and (a:IsType(TYPE_NORMAL) and a:IsLevelAbove(5) or a:GetFlagEffect(21862633) > 0 and (a:IsSummonType(SUMMON_TYPE_RITUAL) or a:IsSummonType(SUMMON_TYPE_FUSION) or a:IsSummonType(SUMMON_TYPE_SYNCHRO) or a:IsSummonType(SUMMON_TYPE_XYZ)))
end
function c21862633.atkop(e, tp, eg, ep, ev, re, r, rp)
  local a, at = Duel.GetBattleMonster(tp)
  if not a or not at or not a:IsRelateToBattle() or a:IsFacedown() or not at:IsRelateToBattle() or at:IsFacedown() then
    return
  end
