...
function c83039729.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c83039729.spfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c83039729.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c83039729.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83039729.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
