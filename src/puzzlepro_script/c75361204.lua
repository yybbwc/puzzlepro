...
function c75361204.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c75361204.spcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():GetEquipTarget() and Duel.GetTurnPlayer() == tp and (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2)
end
...
function c75361204.spfilter1(c, e, tp)
  return c:IsSetCard(0xd1) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c75361204.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c75361204.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c75361204.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c75361204.spop(e, tp, eg, ep, ev, re, r, rp)
  local ec = e:GetLabelObject()
  if ec:IsRelateToEffect(e) and ec:IsFaceup() and Duel.Destroy(ec, REASON_EFFECT) ~= 0 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 then
