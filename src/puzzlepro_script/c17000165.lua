...
function c17000165.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c17000165.desfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and Duel.IsExistingTarget(c17000165.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c17000165.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c17000165.ctfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17000165.ctfilter, tp, 0, LOCATION_MZONE, 1, nil, tp) and Duel.IsPlayerCanSpecialSummonMonster(tp, 21179144, 0x3c, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_REPTILE, ATTRIBUTE_EARTH)
  end
...
function c17000165.ctop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.GetControl(tc, tp) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 21179144, 0x3c, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_REPTILE, ATTRIBUTE_EARTH) then
    Duel.BreakEffect()
