...
function c7602840.filter(c, e, tp)
  return c:IsAttackBelow(1500) and c:IsRace(RACE_MACHINE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_ATTACK)
end
function c7602840.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c7602840.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
