...
function c8785161.spfilter1(c, e, tp)
  local lv = c:GetLevel()
  return c:IsLevelBelow(5) and c:IsSetCard(0xba) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c8785161.spfilter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, c, e, tp, 6 - lv)
end
...
function c8785161.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c8785161.spfilter1, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
