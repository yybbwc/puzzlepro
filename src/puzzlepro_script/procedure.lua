...
function Auxiliary.SynMixCondition(f1, f2, f3, f4, minct, maxct, gc)
  return function(e, c, smat, mg1, min, max)
    ...
    if min then
      ...
      if min - exct > minc then
        minc = min - exct
      end
      if max - exct < maxc then
        maxc = max - exct
      end
      if minc > maxc then
        return false
...
function Auxiliary.SynMixTarget(f1, f2, f3, f4, minct, maxct, gc)
  return function(e, tp, eg, ep, ev, re, r, rp, chk, c, smat, mg1, min, max)
    ...
    if f2 then
      ...
      if g:IsContains(c2) then
        goto SynMixTargetSelectStart
      end
      g:AddCard(c2)
      if f3 then
        Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SMATERIAL)
    ...
    for i = 0, maxc - 1 do
      ...
      if g:IsContains(c4) or g4:IsContains(c4) then
        goto SynMixTargetSelectStart
      end
      g4:AddCard(c4)
    end
...
function Auxiliary.SynMixFilter2(c, f2, f3, f4, minc, maxc, syncard, mg, smat, c1, gc, mgchk)
  if f2 then
    return f2(c, syncard, c1) and (mg:IsExists(Auxiliary.SynMixFilter3, 1, Group.FromCards(c1, c), f3, f4, minc, maxc, syncard, mg, smat, c1, c, gc, mgchk) or minc == 0 and Auxiliary.SynMixFilter4(c, nil, 1, 1, syncard, mg, smat, c1, nil, nil, gc, mgchk))
  else
...
function Auxiliary.SynMixFilter3(c, f3, f4, minc, maxc, syncard, mg, smat, c1, c2, gc, mgchk)
  if f3 then
    return f3(c, syncard, c1, c2) and (mg:IsExists(Auxiliary.SynMixFilter4, 1, Group.FromCards(c1, c2, c), f4, minc, maxc, syncard, mg, smat, c1, c2, c, gc, mgchk) or minc == 0 and Auxiliary.SynMixFilter4(c, nil, 1, 1, syncard, mg, smat, c1, c2, nil, gc, mgchk))
  else
...
function Auxiliary.SynMixFilter4(c, f4, minc, maxc, syncard, mg1, smat, c1, c2, c3, gc, mgchk)
  if f4 and not f4(c, syncard, c1, c2, c3) then
    return false
  end
...
function Auxiliary.SynMixCheck(mg, sg1, minc, maxc, syncard, smat, gc, mgchk)
  ...
  if minc <= 0 and Auxiliary.SynMixCheckGoal(tp, sg1, 0, 0, syncard, sg, smat, gc, mgchk) then
    return true
  end
  if maxc == 0 then
    return false
...
function Auxiliary.SynMixCheckRecursive(c, tp, sg, mg, ct, minc, maxc, syncard, sg1, smat, gc, mgchk)
  sg:AddCard(c)
  ct = ct + 1
  local res = Auxiliary.SynMixCheckGoal(tp, sg, minc, ct, syncard, sg1, smat, gc, mgchk) or (ct < maxc and mg:IsExists(Auxiliary.SynMixCheckRecursive, 1, sg, tp, sg, mg, ct, minc, maxc, syncard, sg1, smat, gc, mgchk))
  sg:RemoveCard(c)
  ct = ct - 1
  return res
end
...
function Auxiliary.SynMixHandCheck(g, tp, syncard)
  ...
  if hct > 0 then
    ...
    for c in Auxiliary.Next(g) do
      ...
      if he then
        found = true
        if hf and hg:IsExists(Auxiliary.SynLimitFilter, 1, c, hf, he, syncard) then
          return false
        end
...
function Auxiliary.SynMixCheckGoal(tp, sg, minc, ct, syncard, sg1, smat, gc, mgchk)
  ...
  if Duel.IsPlayerAffectedByEffect(tp, 8173184) and not g:IsExists(Auxiliary.Tuner(Card.IsSetCard, 0x2), 1, nil, syncard) then
    return false
  end
  if not g:CheckWithSumEqual(Card.GetSynchroLevel, syncard:GetLevel(), g:GetCount(), g:GetCount(), syncard) and (not g:IsExists(Card.IsHasEffect, 1, nil, 89818984) or not g:CheckWithSumEqual(Auxiliary.GetSynchroLevelFlowerCardian, syncard:GetLevel(), g:GetCount(), g:GetCount(), syncard)) then
    return false
  end
  if not (mgchk or Auxiliary.SynMixHandCheck(g, tp, syncard)) then
    return false
  end
  for c in Auxiliary.Next(g) do
    ...
    if le then
      ...
      if lf and g:IsExists(Auxiliary.SynLimitFilter, 1, c, lf, le, syncard) then
        return false
      end
...
function Auxiliary.TuneMagicianCheckX(c, sg, ecode)
  ...
  for _, te in pairs(eset) do
    if sg:IsExists(Auxiliary.TuneMagicianFilter, 1, c, te) then
      return true
    end
  end
  return false
...
function Auxiliary.XyzAlterFilter(c, alterf, xyzc, e, tp, alterop)
  return alterf(c, e, tp, xyzc) and c:IsCanBeXyzMaterial(xyzc) and Duel.GetLocationCountFromEx(tp, tp, c, xyzc) > 0 and not c:IsHasEffect(EFFECT_XYZ_MIN_COUNT) and Auxiliary.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and (not alterop or alterop(e, tp, 0, c))
end
---Xyz monster, lv k*n
---@param c Card
...
function Auxiliary.AddFusionProcMix(fcard, sub, insf, ...)
  if fcard:IsStatus(STATUS_COPYING_EFFECT) then
    return
  end
  local val = { ... }
  local fun = {}
  ...
  for i = 1, #val do
    if type(val[i]) == "function" then
      fun[i] = function(c, fc, subm, mg, sg)
        return val[i](c, fc, subm, mg, sg) and not c:IsHasEffect(6205579)
      end
    elseif type(val[i]) == "table" then
      fun[i] = function(c, fc, subm, mg, sg)
        for _, fcode in ipairs(val[i]) do
          if type(fcode) == "function" then
            if fcode(c, fc, subm, mg, sg) and not c:IsHasEffect(6205579) then
              return true
            end
          elseif type(fcode) == "number" then
            if c:IsFusionCode(fcode) or (subm and c:CheckFusionSubstitute(fc)) then
              return true
            end
          end
        end
      ...
      for _, fcode in ipairs(val[i]) do
        if type(fcode) == "number" then
          mat[fcode] = true
        end
      end
    elseif type(val[i]) == "number" then
      fun[i] = function(c, fc, subm)
        return c:IsFusionCode(val[i]) or (subm and c:CheckFusionSubstitute(fc))
      end
      mat[val[i]] = true
    end
  end
...
function Auxiliary.FCheckMixGoal(sg, tp, fc, sub, chkfnf, ...)
  ...
  if not not_fusion and sg:IsExists(Auxiliary.TuneMagicianCheckX, 1, nil, sg, EFFECT_TUNE_MAGICIAN_F) then
    return false
  end
  ...
  local g = Group.CreateGroup()
  return sg:IsExists(Auxiliary.FCheckMix, 1, nil, sg, g, fc, sub, ...) and (chkf == PLAYER_NONE or Duel.GetLocationCountFromEx(tp, tp, sg, fc) > 0) and (not Auxiliary.FCheckAdditional or Auxiliary.FCheckAdditional(tp, sg, fc)) and (not Auxiliary.FGoalCheckAdditional or Auxiliary.FGoalCheckAdditional(tp, sg, fc))
end
...
function Auxiliary.AddFusionProcMixRep(fcard, sub, insf, fun1, minc, maxc, ...)
  ...
  for i = 1, #val do
    if type(val[i]) == "function" then
      fun[i] = function(c, fc, subm)
        return val[i](c, fc, subm) and not c:IsHasEffect(6205579)
      end
    elseif type(val[i]) == "table" then
      fun[i] = function(c, fc, subm)
        for _, fcode in ipairs(val[i]) do
          if type(fcode) == "function" then
            if fcode(c, fc, subm) and not c:IsHasEffect(6205579) then
              return true
            end
          elseif type(fcode) == "number" then
            if c:IsFusionCode(fcode) or (subm and c:CheckFusionSubstitute(fc)) then
...
function Auxiliary.FCheckMixRepGoalCheck(tp, sg, fc, chkfnf)
  ...
  if Auxiliary.FGoalCheckAdditional and not Auxiliary.FGoalCheckAdditional(tp, sg, fc) then
    return false
  end
  return true
end
function Auxiliary.FCheckMixRepGoal(tp, sg, fc, sub, chkfnf, fun1, minc, maxc, ...)
  local chkf = chkfnf & 0xff
  if sg:GetCount() < minc + #{ ... } or sg:GetCount() > maxc + #{ ... } then
    return false
  end
  if not (chkf == PLAYER_NONE or Duel.GetLocationCountFromEx(tp, tp, sg, fc) > 0) then
    return false
  end
  if Auxiliary.FCheckAdditional and not Auxiliary.FCheckAdditional(tp, sg, fc) then
    return false
  end
  if not Auxiliary.FCheckMixRepGoalCheck(tp, sg, fc, chkfnf) then
    return false
  end
  local g = Group.CreateGroup()
...
function Auxiliary.FCheckSelectMixRep(tp, mg, sg, g, fc, sub, chkfnf, fun1, minc, maxc, ...)
  local chkf = chkfnf & 0xff
  if Auxiliary.FCheckAdditional and not Auxiliary.FCheckAdditional(tp, g, fc) then
    return false
  end
  if chkf == PLAYER_NONE or Duel.GetLocationCountFromEx(tp, tp, g, fc) > 0 then
    if minc <= 0 and #{ ... } == 0 and Auxiliary.FCheckMixRepGoalCheck(tp, g, fc, chkfnf) then
      return true
    end
...
function Auxiliary.FCheckSelectMixRepM(c, tp, ...)
  return c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and Auxiliary.FCheckMixRepTemplate(c, Auxiliary.FCheckSelectMixRep, tp, ...)
end
...
function Auxiliary.FShaddollSpFilter1(c, fc, tp, mg, exg, attr, chkf)
  return mg:IsExists(Auxiliary.FShaddollSpFilter2, 1, c, fc, tp, c, attr, chkf) or (exg and exg:IsExists(Auxiliary.FShaddollSpFilter2, 1, c, fc, tp, c, attr, chkf))
end
function Auxiliary.FShaddollSpFilter2(c, fc, tp, mc, attr, chkf)
  local sg = Group.FromCards(c, mc)
  if sg:IsExists(Auxiliary.TuneMagicianCheckX, 1, nil, sg, EFFECT_TUNE_MAGICIAN_F) then
    return false
  end
  if not Auxiliary.MustMaterialCheck(sg, tp, EFFECT_MUST_BE_FMATERIAL) then
    return false
  end
  if Auxiliary.FCheckAdditional and not Auxiliary.FCheckAdditional(tp, sg, fc) or Auxiliary.FGoalCheckAdditional and not Auxiliary.FGoalCheckAdditional(tp, sg, fc) then
    return false
  end
  return ((Auxiliary.FShaddollFilter1(c) and Auxiliary.FShaddollFilter2(mc, attr)) or (Auxiliary.FShaddollFilter2(c, attr) and Auxiliary.FShaddollFilter1(mc))) and (chkf == PLAYER_NONE or Duel.GetLocationCountFromEx(tp, tp, sg, fc) > 0)
end
function Auxiliary.FShaddollCondition(attr)
  return function(e, g, gc, chkf)
    if g == nil then
      return Auxiliary.MustMaterialCheck(nil, e:GetHandlerPlayer(), EFFECT_MUST_BE_FMATERIAL)
    end
    local c = e:GetHandler()
...
function Auxiliary.FShaddollOperation(attr)
  return function(e, tp, eg, ep, ev, re, r, rp, gc, chkf)
    ...
    if exg and exg:IsExists(Auxiliary.FShaddollSpFilter2, 1, nil, c, tp, g:GetFirst(), attr, chkf) and (mg:GetCount() == 0 or (exg:GetCount() > 0 and Duel.SelectYesNo(tp, Auxiliary.Stringid(81788994, 0)))) then
...
function Auxiliary.AddFusionEffectProcUltimate(c, params)
  ...
  if params.mat_location == nil then
    params.mat_location = LOCATION_HAND + LOCATION_MZONE
  end
  if params.mat_filter == nil then
    params.mat_filter = Auxiliary.FMaterialFilter
  end
  if params.mat_operation == nil then
    params.mat_operation = Auxiliary.FMaterialToGrave
  end
  if params.grave_filter == nil then
    params.grave_filter = Auxiliary.FMaterialRemoveFilter
  end
  if params.grave_operation == nil then
    params.grave_operation = Auxiliary.FMaterialRemove
  end
  if params.removed_operation == nil then
    params.removed_operation = params.mat_operation
  end
  if params.deck_filter == nil then
    params.deck_filter = Auxiliary.FMaterialToGraveFilter
  end
  if params.deck_operation == nil then
    params.deck_operation = params.mat_operation
  end
  if params.extra_filter == nil then
    params.extra_filter = Auxiliary.FMaterialToGraveFilter
  end
  if params.extra_operation == nil then
    params.extra_operation = params.mat_operation
  end
  if params.spsummon_nocheck == nil then
    params.spsummon_nocheck = false
  end
...
function Auxiliary.FusionEffectUltimateFilter(c, e, tp, mg, chkf, params)
  ...
  if not c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, params.spsummon_nocheck, false) then
    return false
  end
  if params.filter1 and not params.filter1(c) then
    return false
  end
  if params.filter2 and not params.filter2(c) then
    return false
  end
  if params.get_fcheck then
    Auxiliary.FCheckAdditional = params.get_fcheck(c, e, tp)
  end
...
function Auxiliary.FusionEffectUltimateTarget(params)
  return function(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
      ...
      if params.get_extra_mat then
        mgex = params.get_extra_mat(e, tp, eg, ep, ev, re, r, rp)
      end
      mgall:Merge(mgex)
      mgbase:Merge(mgex)
      local mg1 = mgall:Filter(Auxiliary.FusionEffectUltimateMatFilter, nil, e, tp, params.mat_filter):Filter(Auxiliary.FusionEffectUltimateMatLocFilter, nil, e, tp, LOCATION_GRAVE, params.grave_filter):Filter(Auxiliary.FusionEffectUltimateMatLocFilter, nil, e, tp, LOCATION_REMOVED, params.removed_filter):Filter(Auxiliary.FusionEffectUltimateMatLocFilter, nil, e, tp, LOCATION_DECK, params.deck_filter):Filter(Auxiliary.FusionEffectUltimateMatLocFilter, nil, e, tp, LOCATION_EXTRA, params.extra_filter)
      if gc then
        mg1:AddCard(gc)
      end
    ...
    if params.mat_operation == Auxiliary.FMaterialRemove or params.grave_operation == Auxiliary.FMaterialRemove or params.deck_operation == Auxiliary.FMaterialRemove then
      Duel.SetOperationInfo(0, CATEGORY_REMOVE, nil, 1, tp, params.mat_location)
      e:SetCategory(e:GetCategory() | CATEGORY_REMOVE)
    end
    if params.mat_operation == Auxiliary.FMaterialToDeck or params.grave_operation == Auxiliary.FMaterialToDeck or params.deck_operation == Auxiliary.FMaterialToDeck then
...
function Auxiliary.FusionEffectUltimateOperation(params)
  return function(e, tp, eg, ep, ev, re, r, rp)
    ...
    if params.include_this_card then
      gc = e:GetHandler()
      if gc:IsOnField() and gc:IsFacedown() or not gc:IsRelateToEffect(e) or gc:IsImmuneToEffect(e) then
        return
      end
    end
    local mgall, mgbase = Duel.GetFusionMaterial(tp, params.mat_location)
    ...
    if sg1:GetCount() > 0 or (sg2 ~= nil and sg2:GetCount() > 0) then
      ...
      if params.get_fcheck then
        Auxiliary.FCheckAdditional = params.get_fcheck(tc, e, tp)
      end
      if sg1:IsContains(tc) and (sg2 == nil or not sg2:IsContains(tc) or ce and not Duel.SelectYesNo(tp, ce:GetDescription())) then
        local mat1 = Duel.SelectFusionMaterial(tp, tc, mg1, gc, chkf)
        tc:SetMaterial(mat1)
        if params.mat_operation2 then
          params.mat_operation2(e, tp, mat1)
        end
      ...
      if params.foperation then
        params.foperation(e, tp, tc)
      end
    end
    Auxiliary.FCheckAdditional = nil
    Auxiliary.GCheckAdditional = nil
    Auxiliary.FGoalCheckAdditional = nil
    Auxiliary.FMaterialBase = nil
    if params.foperation2 then
      params.foperation2(e, tp, eg, ep, ev, re, r, rp)
    end
  end
end
...
function Auxiliary.RitualCheck(g, tp, c, lv, greater_or_equal)
  return Auxiliary["RitualCheck" .. greater_or_equal](g, c, lv) and Duel.GetMZoneCount(tp, g, tp) > 0 and (not c.mat_group_check or c.mat_group_check(g, tp)) and (not Auxiliary.RCheckAdditional or Auxiliary.RCheckAdditional(tp, g, c))
end
...
function Auxiliary.RitualUltimateFilter(c, filter, e, tp, m1, m2, level_function, greater_or_equal, chk)
  if bit.band(c:GetType(), 0x81) ~= 0x81 or (filter and not filter(c, e, tp, chk)) or not c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_RITUAL, tp, false, true) then
    return false
  end
...
function Auxiliary.RitualUltimateTarget(filter, level_function, greater_or_equal, summon_location, grave_filter, mat_filter, extra_target)
  return function(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
      ...
      if mat_filter then
        mg = mg:Filter(mat_filter, nil, e, tp, true)
      end
      local exg = nil
...
function Auxiliary.RitualUltimateOperation(filter, level_function, greater_or_equal, summon_location, grave_filter, mat_filter, extra_operation)
  return function(e, tp, eg, ep, ev, re, r, rp)
    ...
    local mg = Duel.GetRitualMaterial(tp)
    if mat_filter then
      mg = mg:Filter(mat_filter, nil, e, tp)
    end
    local exg = nil
    if grave_filter then
      exg = Duel.GetMatchingGroup(Auxiliary.RitualExtraFilter, tp, LOCATION_GRAVE, 0, nil, grave_filter)
...
function Auxiliary.PConditionExtraFilter(c, e, tp, lscale, rscale, eset)
  for _, te in ipairs(eset) do
    if Auxiliary.PConditionExtraFilterSpecific(c, e, tp, lscale, rscale, te) then
      return true
    end
  end
  return false
...
function Auxiliary.PConditionFilter(c, e, tp, lscale, rscale, eset)
  ...
  return (c:IsLocation(LOCATION_HAND) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM))) and lv > lscale and lv < rscale and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_PENDULUM, tp, bool, bool) and not c:IsForbidden() and (Auxiliary.PendulumChecklist & (0x1 << tp) == 0 or Auxiliary.PConditionExtraFilter(c, e, tp, lscale, rscale, eset))
end
function Auxiliary.PendCondition(e, c, og)
  ...
  if Duel.GetLocationCountFromEx(tp, tp, nil, TYPE_PENDULUM) > 0 then
...
function Auxiliary.LExtraFilter(c, f, lc, tp)
  if c:IsOnField() and c:IsFacedown() then
    return false
  end
  if not c:IsCanBeLinkMaterial(lc) or f and not f(c) then
    return false
  end
...
function Auxiliary.LCheckGoal(sg, tp, lc, gf, lmat)
  return sg:CheckWithSumEqual(Auxiliary.GetLinkCount, lc:GetLink(), #sg, #sg) and Duel.GetLocationCountFromEx(tp, tp, sg, lc) > 0 and (not gf or gf(sg, lc, tp)) and not sg:IsExists(Auxiliary.LUncompatibilityFilter, 1, nil, sg, lc, tp) and (not lmat or sg:IsContains(lmat))
end
...
function Auxiliary.LinkCondition(f, minct, maxct, gf)
  return function(e, c, og, lmat, min, max)
    ...
    if lmat ~= nil then
      if not Auxiliary.LConditionFilter(lmat, f, c, e) then
        return false
      end
      mg:AddCard(lmat)
    end
...
function Auxiliary.MustMaterialCheck(v, tp, code)
  ...
  if not v then
    ...
    if code == EFFECT_MUST_BE_SMATERIAL and Duel.IsPlayerAffectedByEffect(tp, 8173184) then
      return false
    end
    return #g == 0
  end
...
function Auxiliary.DarkFusionLimit(e, se, sp, st)
  return se:GetHandler():IsCode(94820406) or st == SUMMON_VALUE_DARK_FUSION or (Duel.IsPlayerAffectedByEffect(sp, 72043279) and st & SUMMON_TYPE_FUSION == SUMMON_TYPE_FUSION)
end
--sp_summon condition for Fossil
function Auxiliary.FossilFusionLimit(e, se, sp, st)
  return st == SUMMON_VALUE_FOSSIL_FUSION or se:GetHandler():IsCode(59419719) or not e:GetHandler():IsLocation(LOCATION_EXTRA)
end
