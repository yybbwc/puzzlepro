...
function c3055837.condition(e, tp, eg, ep, ev, re, r, rp)
  if re:IsActiveType(TYPE_SPELL + TYPE_TRAP) and not re:IsHasType(EFFECT_TYPE_ACTIVATE) then
    return false
  end
  ...
  if cp ~= PLAYER_ALL then
    return Duel.IsPlayerAffectedByEffect(cp, EFFECT_REVERSE_RECOVER)
  else
    return Duel.IsPlayerAffectedByEffect(0, EFFECT_REVERSE_RECOVER) or Duel.IsPlayerAffectedByEffect(1, EFFECT_REVERSE_RECOVER)
  end
end
...
function c3055837.refcon(e, re, val, r, rp, rc)
  ...
  if cc == 0 or bit.band(r, REASON_EFFECT) == 0 then
    return
  end
