...
function c7445307.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local ct = -ft + 1
  if chk == 0 then
    return ft > -2 and rg:GetCount() > 1 and (ft > 0 or rg:IsExists(c7445307.mzfilter, ct, nil, tp))
  end
  local g = nil
  if ft > 0 then
...
function c7445307.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7445307.rmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e:GetHandler():GetAttack())
  end
