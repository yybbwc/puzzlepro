...
function c66015185.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c66015185.filter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c66015185.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, zone)
  end
