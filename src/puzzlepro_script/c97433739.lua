...
function c97433739.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(97433739, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c97433739.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and g:GetClassCount(Card.GetLevel) >= 3
  end
