...
function c55557574.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55557574.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil) or (g:GetClassCount(Card.GetCode) == 3 and Duel.IsExistingMatchingCard(c55557574.thfilter, tp, LOCATION_DECK, 0, 1, nil))
  end
end
function c55557574.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if check1 and Duel.IsExistingMatchingCard(c55557574.thfilter, tp, LOCATION_DECK, 0, 1, nil) and (not check2 or Duel.SelectYesNo(tp, aux.Stringid(55557574, 1))) then
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
    local g = Duel.SelectMatchingCard(tp, c55557574.thfilter, tp, LOCATION_DECK, 0, 1, 1, nil)
    Duel.SendtoHand(g, nil, REASON_EFFECT)
    Duel.ConfirmCards(1 - tp, g)
    if Duel.IsExistingMatchingCard(c55557574.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(55557574, 2)) then
      Duel.BreakEffect()
      Duel.ShuffleHand(tp)
