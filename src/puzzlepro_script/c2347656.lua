...
function c2347656.sttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c2347656.stfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c2347656.stfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c2347656.actcon(e)
  ...
  return not e:GetHandler():IsStatus(STATUS_EFFECT_ENABLED) and not Duel.IsExistingMatchingCard(c2347656.actfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
function c2347656.cfilter(c)
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsReason(REASON_EFFECT)
end
function c2347656.descon(e, tp, eg, ep, ev, re, r, rp)
  return re and rp == tp and re:IsActiveType(TYPE_TRAP) and re:GetHandler():GetOriginalType() == TYPE_TRAP and eg:IsExists(c2347656.cfilter, 1, nil)
end
function c2347656.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_ONFIELD + LOCATION_HAND) > 0
  end
