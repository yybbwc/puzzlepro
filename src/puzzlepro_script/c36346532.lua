...
function c36346532.cfilter(c, tp)
  return c:GetSequence() < 5 and c:IsFacedown() and c:IsAbleToGrave() and Duel.GetSZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c36346532.setfilter, tp, LOCATION_DECK, 0, 1, nil, c, tp)
end
function c36346532.setfilter(c, mc, tp)
  if not (c:IsSetCard(0xd4) and c:IsType(TYPE_TRAP)) then
    return false
  end
...
function c36346532.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c36346532.cfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c36346532.cfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil, tp)
  end
...
function c36346532.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:IsExists(c36346532.repfilter, 1, c, tp) and c:IsAbleToRemove() and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
  end
