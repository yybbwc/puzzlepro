...
function c44335251.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44335251.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c44335251.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c44335251.desfilter(chkc, tp) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44335251.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c, tp) and Duel.IsExistingMatchingCard(c44335251.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
