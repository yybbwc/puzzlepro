...
function c74115234.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c74115234.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c74115234.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c74115234.recon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(bc)
  return e:GetHandler():GetEquipTarget() == eg:GetFirst() and ec:IsControler(tp) and bc:IsLocation(LOCATION_GRAVE) and bc:IsType(TYPE_MONSTER) and bc:IsReason(REASON_BATTLE)
end
