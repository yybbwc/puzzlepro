...
function c60406591.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and (ct == 1 or not Duel.IsPlayerAffectedByEffect(tp, 59822133)) and Duel.GetLocationCount(tp, LOCATION_MZONE) >= ct and Duel.IsPlayerCanSpecialSummonMonster(tp, 60406592, 0, TYPES_TOKEN_MONSTER, 100, 100, 1, RACE_PLANT, ATTRIBUTE_DARK)
  end
...
function c60406591.activate(e, tp, eg, ep, ev, re, r, rp)
  local ct = c60406591[1]
  if (ct > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) or Duel.GetLocationCount(tp, LOCATION_MZONE) < ct or not Duel.IsPlayerCanSpecialSummonMonster(tp, 60406592, 0, TYPES_TOKEN_MONSTER, 100, 100, 1, RACE_PLANT, ATTRIBUTE_DARK) then
    return
  end
  for i = 1, ct do
