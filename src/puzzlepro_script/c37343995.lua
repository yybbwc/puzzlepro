...
function c37343995.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(37343995, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c37343995.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c37343995.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c37343995.xyztg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c37343995.xyzfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
