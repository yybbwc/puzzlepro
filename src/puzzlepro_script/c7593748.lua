...
function c7593748.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c7593748.filter(chkc)
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE, 1 - tp, LOCATION_REASON_CONTROL)
  if chk == 0 then
    return ft > 0 and Duel.IsExistingTarget(c7593748.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c7593748.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7593748.lvfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
end
