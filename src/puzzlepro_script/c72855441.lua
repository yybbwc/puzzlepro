...
function c72855441.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72855441.spfilter1, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c72855441.gvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c72855441.gvfilter(chkc, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c72855441.gvfilter, tp, LOCATION_MZONE, 0, 1, nil, ft) and Duel.IsExistingMatchingCard(c72855441.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c72855441.gvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    if Duel.SendtoGrave(tc, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72855441.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp) then
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
