...
function c84808313.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsRace, 1, nil, RACE_DINOSAUR)
  end
...
function c84808313.ntcon(e, c, minc)
  ...
  return e:GetHandler():GetType() == TYPE_SPELL and minc == 0 and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0
end
