...
function c39943352.xyztg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c39943352.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c39943352.filter, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanOverlay()
  end
...
function c39943352.penop1(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not c:IsRelateToEffect(e) or not Duel.MoveToField(c, tp, tp, LOCATION_PZONE, POS_FACEUP, true) then
    return
  end
  if Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c39943352.pmfilter), tp, LOCATION_GRAVE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(39943352, 3)) then
