...
function c75922381.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c75922381.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c75922381.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c75922381.cfilter, 1, e:GetHandler())
  end
...
function c75922381.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateEffect(ev) and rc:IsRelateToEffect(re) and rc:IsDestructable() and (e:GetLabelObject():IsRank(2) or e:GetLabelObject():IsLink(2)) and Duel.SelectYesNo(tp, aux.Stringid(75922381, 2)) then
    Duel.BreakEffect()
