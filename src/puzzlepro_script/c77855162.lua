...
function c77855162.spfilter(c, e, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_PENDULUM) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP) and (c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)
end
function c77855162.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77855162.spfilter, tp, LOCATION_HAND + LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c77855162.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(bc)
  return aux.dsercon(e, tp, eg, ep, ev, re, r, rp) and bc and bc:IsRelateToBattle() and bc:IsOnField() and c:IsStatus(STATUS_OPPO_BATTLE)
end
function c77855162.descon2(e, tp, eg, ep, ev, re, r, rp)
  local tc, bc = Duel.GetBattleMonster(tp)
  e:SetLabelObject(bc)
  return tc and bc and tc:IsStatus(STATUS_OPPO_BATTLE) and bc:IsOnField() and bc:IsRelateToBattle() and tc:IsFaceup() and tc:IsType(TYPE_PENDULUM)
end
