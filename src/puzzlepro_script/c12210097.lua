...
function s.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.tdfilter, tp, LOCATION_GRAVE, 0, 4, nil) and (Duel.GetFieldGroup(tp, 0, LOCATION_MZONE) == 0 or Duel.IsPlayerCanRelease(1 - tp))
  end
...
function s.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) then
    ...
    if og:IsExists(Card.IsLocation, 1, nil, LOCATION_DECK + LOCATION_EXTRA) and Duel.CheckReleaseGroupEx(1 - tp, nil, 1, REASON_RULE, false, nil) then
