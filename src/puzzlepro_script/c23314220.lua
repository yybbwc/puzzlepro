...
function c23314220.filter(c)
  return (((c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsSetCard(0x106e)) or (c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsRace(RACE_SPELLCASTER))) and not c:IsCode(23314220) and c:IsAbleToGrave()
end
function c23314220.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 2) and Duel.IsExistingMatchingCard(c23314220.filter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
