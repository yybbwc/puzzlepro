...
function c38572779.immtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 38572779) == 0
  end
end
...
function c38572779.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    return c:IsAbleToRemoveAsCost() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c38572779.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, cg:GetCount())
  end
  local cg = Duel.GetMatchingGroup(c38572779.cfilter, tp, LOCATION_GRAVE, 0, nil)
