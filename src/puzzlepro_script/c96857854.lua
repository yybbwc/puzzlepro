...
function c96857854.filter(c, e, tp)
  return c:IsSetCard(0x80) and (c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp))
end
function c96857854.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return (not Duel.IsPlayerAffectedByEffect(tp, 59822133) or ct == 1) and ft >= ct and Duel.IsExistingMatchingCard(c96857854.filter, tp, LOCATION_DECK + LOCATION_HAND, 0, ct, nil, e, tp)
  end
...
function c96857854.operation(e, tp, eg, ep, ev, re, r, rp)
  local ct = eg:GetCount()
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) and ct > 1 then
    return
  end
...
function c96857854.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96857854.spfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c96857854.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 96857854, 0, TYPES_NORMAL_TRAP_MONSTER, 0, 1000, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE, 1 - tp)
  end
...
function c96857854.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.IsPlayerCanSpecialSummonMonster(tp, 96857854, 0, TYPES_NORMAL_TRAP_MONSTER, 0, 1000, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE, 1 - tp) then
