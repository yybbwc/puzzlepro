...
function c77656797.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c77656797.eqfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, c, tp)
  end
...
function c77656797.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc then
    ...
    if Duel.IsExistingMatchingCard(c77656797.eqfilter2, tp, LOCATION_DECK, 0, 1, nil, tp) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(77656797, 2)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_EQUIP)
