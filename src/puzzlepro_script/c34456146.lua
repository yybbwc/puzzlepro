...
function c34456146.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c34456146.costfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c34456146.ctcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c34456146.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c34456146.cttg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsControlerCanBeChanged() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c34456146.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c34456146.ctop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not c:IsRelateToEffect(e) or Duel.GetControl(c, 1 - tp) == 0 or Duel.GetMZoneCount(tp, c) == 0 then
    return
  end
...
function c34456146.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c34456146.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c34456146.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
