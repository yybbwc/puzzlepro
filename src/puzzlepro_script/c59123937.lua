...
function c59123937.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsSetCard, 1, nil, 0xaf)
  end
...
function c59123937.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c59123937.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c59123937.filter2(c, e, tp, m, f, gc, chkf)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x10af) and c:IsLevelAbove(8) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, gc, chkf) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
