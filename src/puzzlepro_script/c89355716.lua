...
function c89355716.eftg(e, c)
  ...
  return c:IsType(TYPE_EFFECT) and c:IsSetCard(0x109) and seq < 5 and math.abs(e:GetHandler():GetSequence() - seq) <= 1
end
function c89355716.spcfilter(c, e, tp, ft)
  return c:IsReleasableByEffect() and (ft > 0 or c:GetSequence() < 5) and Duel.IsExistingMatchingCard(c89355716.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetCode())
end
...
function c89355716.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c89355716.spcfilter(chkc, e, tp, ft)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c89355716.spcfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), e, tp, ft)
  end
...
function c89355716.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsControler(tp) and tc:IsRelateToEffect(e) and Duel.Release(tc, REASON_EFFECT) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
