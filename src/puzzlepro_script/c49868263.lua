...
function c49868263.disop2(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():IsRelateToEffect(re) and re:IsActiveType(TYPE_SPELL) and re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
...
function c49868263.distg2(e, c)
  return c:GetCardTargetCount() > 0 and c:IsType(TYPE_SPELL) and c:GetCardTarget():IsContains(e:GetHandler())
end
