...
function c87990236.costfilter(c, tp)
  return c:IsSetCard(0xef) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function c87990236.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c87990236.costfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, tp)
  end
