...
function c19891310.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  if bt and bt:IsControler(tp) then
    return bt:IsRace(RACE_MACHINE)
  end
...
function c19891310.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c19891310.thfilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19891310.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
