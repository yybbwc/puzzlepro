...
function c39041729.filter(c, e, tp)
  ...
  local lv = c:GetLevel()
  return lv > 0 and c:IsRace(RACE_DINOSAUR) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and rg:CheckWithSumEqual(Card.GetLevel, lv, 2, 99)
end
...
function c39041729.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c39041729.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  local g = Duel.GetMatchingGroup(c39041729.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, nil, e, tp)
  local lvt = {}
  local pc = 1
  for i = 2, 12 do
    if g:IsExists(c39041729.spfilter, 1, nil, e, tp, i) then
      lvt[pc] = i
      pc = pc + 1
    end
  end
