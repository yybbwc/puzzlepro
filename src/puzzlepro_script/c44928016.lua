...
function c44928016.cfilter(c, ft)
  return c:IsRace(RACE_PLANT) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and not c:IsCode(44928016) and c:IsAbleToGraveAsCost() and (ft > 0 or c:GetSequence() < 5)
end
function c44928016.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c44928016.cfilter, tp, loc, 0, 1, nil, ft)
  end
