...
function s.spfilter(c, e, tp)
  return aux.IsCodeListed(c, 89812483) and c:IsType(TYPE_MONSTER) and not c:IsSummonableCard() and c:IsCanBeSpecialSummoned(e, 0, tp, true, true)
end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummon(tc, 0, tp, tp, true, true, POS_FACEUP) ~= 0 and e:GetLabel() == 100 and c:IsOnField() and c:IsRelateToEffect(e) and Duel.SelectYesNo(tp, aux.Stringid(id, 1)) then
    Duel.BreakEffect()
...
function s.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGrave() and not c:IsStatus(STATUS_DESTROY_CONFIRMED) and tg and tg:IsReason(REASON_BATTLE + REASON_EFFECT)
  end
