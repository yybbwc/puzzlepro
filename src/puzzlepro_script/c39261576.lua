...
function c39261576.filter2(c, e, tp, m, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x1047) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, nil, chkf)
end
...
function c39261576.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsSetCard(0x1047) and mat:IsContains(chkc)
  end
  if chk == 0 then
    return mat:IsExists(Card.IsSetCard, 1, nil, 0x1047)
  end
...
function c39261576.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not sc:IsRelateToEffect(e) or sc:IsFacedown() then
    return
  end
