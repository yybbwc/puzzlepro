...
function c69540484.spfilter(c, e, tp)
  return c:IsSetCard(0x189) and not c:IsType(TYPE_XYZ) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c69540484.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c69540484.spfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c69540484.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsAbleToRemove(tp, POS_FACEDOWN)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToRemove, tp, 0, LOCATION_GRAVE, 3, nil, tp, POS_FACEDOWN) and Duel.IsExistingMatchingCard(c69540484.rmcfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
