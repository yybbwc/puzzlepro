...
function c75730490.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c75730490.costfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c75730490.spfilter(c, e, tp)
  return not c:IsCode(75730490) and c:IsRace(RACE_FIEND) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c75730490.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c75730490.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c75730490.drcon(e, tp, eg, ep, ev, re, r, rp)
  return re and rp == tp and re:IsActiveType(TYPE_TRAP) and re:GetHandler():GetOriginalType() == TYPE_TRAP and eg:IsExists(c75730490.cfilter, 1, nil)
end
