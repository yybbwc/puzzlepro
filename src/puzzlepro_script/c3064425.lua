...
function c3064425.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c3064425.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c3064425.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c3064425.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return ep ~= tp and Duel.IsChainNegatable(ev) and ph > PHASE_MAIN1 and ph < PHASE_MAIN2 and Duel.IsExistingMatchingCard(c3064425.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
