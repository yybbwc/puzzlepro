...
function c86805855.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and c86805855.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86805855.filter, tp, 0, LOCATION_GRAVE, 1, nil)
  end
