...
function c14772491.filter(c, e, tp)
  return c:IsFaceup() and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c14772491.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetControler())
end
function c14772491.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c14772491.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c14772491.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e, tp)
  end
...
function c14772491.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsRelateToEffect(e) then
    if Duel.GetLocationCount(cp, LOCATION_MZONE) <= 0 then
      return
    end
...
function c14772491.rcon(e)
  return e:GetOwner():IsHasCardTarget(e:GetHandler()) and e:GetOwner():IsHasCardTarget(e:GetLabelObject())
end
function c14772491.thcon(e, tp, eg, ep, ev, re, r, rp)
  local tc = e:GetLabelObject()
  return tc and tc:IsOnField() and tc:IsFaceup() and tc:IsSetCard(0x1f) and e:GetHandler():IsHasCardTarget(tc)
end
