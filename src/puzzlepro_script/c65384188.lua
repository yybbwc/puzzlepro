...
function c65384188.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c65384188.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c65384188.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c65384188.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsRelateToEffect(e) and tc1:IsFaceup() and tc2:IsRelateToEffect(e) and tc2:IsFaceup() then
    ...
    if tc1:IsDefensePos() or tc2:IsDefensePos() or a ~= 2 then
      return
    end
    Duel.BreakEffect()
