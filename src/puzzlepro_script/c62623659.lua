...
function c62623659.filter(c)
  return ((c:IsSetCard(0x107e, 0x207e) and c:IsType(TYPE_MONSTER)) or (c:IsSetCard(0x7e) and c:IsType(TYPE_SPELL + TYPE_TRAP)) or (c:IsSetCard(0x95, 0x15e) and c:IsType(TYPE_SPELL))) and c:IsAbleToHand()
end
...
function c62623659.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62623659.filter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c62623659.tdfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
