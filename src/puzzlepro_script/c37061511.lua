...
function c37061511.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c37061511.thfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c37061511.tetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and aux.nzatk(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c37061511.tefilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(aux.nzatk, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c37061511.teop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsFaceup() and tc:IsRelateToEffect(e) then
...
function c37061511.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:GetFlagEffect(37061511) == 0
  end
