...
function c71978434.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and Duel.IsExistingMatchingCard(c71978434.costfilter, tp, LOCATION_HAND, 0, 1, c)
  end
...
function c71978434.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c71978434.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71978434.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c71978434.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    if tc:IsExtraDeckMonster() or Duel.SelectOption(tp, aux.Stringid(71978434, 2), aux.Stringid(71978434, 3)) == 0 then
