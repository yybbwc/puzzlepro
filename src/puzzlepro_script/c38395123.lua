...
function c38395123.cfilter(c, tp)
  return c:IsSetCard(0xf) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c38395123.filter1, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, c, c:GetCode()) and Duel.IsExistingMatchingCard(c38395123.filter2, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, c)
end
...
function c38395123.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c38395123.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c38395123.tdfilter, tp, LOCATION_REMOVED, 0, 3, nil)
  end
