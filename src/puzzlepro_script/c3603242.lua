...
function c3603242.cfilter(c, tp)
  return c:IsRace(RACE_MACHINE) and c:IsAbleToGraveAsCost() and Duel.IsExistingTarget(c3603242.dfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetLevel())
end
...
function c3603242.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c3603242.cfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c3603242.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c3603242.dfilter(chkc, e:GetLabel())
  end
  if chk == 0 then
    return true
  end
