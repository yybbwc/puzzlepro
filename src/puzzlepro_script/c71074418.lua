...
function c71074418.thfilter(c, tp)
  return c:IsAbleToHand() and (c:IsControler(1 - tp) and c:IsOnField() or c:IsLocation(LOCATION_GRAVE) and c:IsSetCard(0x128) and c:IsType(TYPE_SPELL))
end
function c71074418.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return c71074418.thfilter(chkc, tp)
  end
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c71074418.thfilter, tp, LOCATION_GRAVE, LOCATION_ONFIELD, 1, nil, tp)
  end
