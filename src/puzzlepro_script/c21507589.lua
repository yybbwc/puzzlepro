...
function c21507589.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c21507589.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c21507589.filter, tp, LOCATION_REMOVED, 0, 2, nil)
  end
