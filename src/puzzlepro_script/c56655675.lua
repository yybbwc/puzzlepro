...
function c56655675.cfilter(c)
  return c:IsFusionSetCard(0x40b5, 0x10b5, 0x20b5) and c:IsAbleToRemoveAsCost()
end
...
function c56655675.discon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():GetFlagEffect(56655675) ~= 0 and re ~= e and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
end
...
function c56655675.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56655675.filter, tp, LOCATION_HAND, 0, 1, nil)
  end
