...
function c29669359.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_COST) and not e:GetHandler():IsDirectAttacked()
  end
...
function c29669359.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c29669359.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29669359.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
