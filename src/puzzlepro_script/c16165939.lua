...
function c16165939.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousLocation(LOCATION_SZONE) and c:IsPreviousControler(tp) and rp == 1 - tp and c:IsReason(REASON_EFFECT)
end
...
function c16165939.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c16165939.spfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
