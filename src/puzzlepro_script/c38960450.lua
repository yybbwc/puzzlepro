...
function c38960450.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c38960450.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c38960450.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if tc:CheckUniqueOnField(tp) and not tc:IsForbidden() and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and g2:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(38960450, 0)) then
      Duel.BreakEffect()
