...
function c58988903.filter1(c, e, tp)
  local rk = c:GetRank()
  return c:IsFaceup() and c:IsSetCard(0xba) and c:IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c58988903.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c, rk + 2) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL)
end
...
function c58988903.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c58988903.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c58988903.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c58988903.cfilter(c, e, tp)
  return c:IsSetCard(0xba) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c58988903.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
...
function c58988903.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c58988903.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c58988903.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c58988903.spfilter(chkc, e, tp)
  end
  if chk == 0 then
