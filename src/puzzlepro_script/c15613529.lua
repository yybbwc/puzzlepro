...
function c15613529.spcon1(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc:IsControler(1 - tp) and loc == LOCATION_MZONE and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainDisablable(ev) and Duel.IsExistingMatchingCard(c15613529.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c15613529.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c15613529.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c15613529.spfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
