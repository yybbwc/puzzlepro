...
function c79400597.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if Duel.IsExistingMatchingCard(c79400597.costfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 1) and Duel.SelectYesNo(tp, aux.Stringid(79400597, 0)) then
...
function c79400597.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return d ~= nil and d:IsAbleToRemove() and a:IsControler(tp) and a:IsSetCard(0x129) and a:GetEquipGroup() and a:GetEquipGroup():IsExists(Card.IsCode, 1, nil, 44133040)
  end
  e:SetLabelObject(d)
...
function c79400597.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c79400597.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c79400597.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
