...
function c56166150.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56166150.cfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56166150.spfilter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x38) and not c:IsCode(56166150) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c56166150.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c56166150.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c56166150.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c56166150.ddcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return re:IsActiveType(TYPE_MONSTER) and rc ~= c and rc:IsSetCard(0x38) and rc:IsControler(tp)
end
