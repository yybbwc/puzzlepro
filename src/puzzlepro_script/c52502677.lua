...
function c52502677.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52502677.costfilter, tp, LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil, c)
  end
...
function c52502677.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c52502677.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
