...
function c25343280.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c25343280.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c25343280.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c25343280.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) and g:GetCount() > 1 then
    return
  end
