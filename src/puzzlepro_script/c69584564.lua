...
function c69584564.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c69584564.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c69584564.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c69584564.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and c69584564.filter(tc) and Duel.Destroy(tc, REASON_EFFECT) ~= 0 then
    ...
    if dc:IsRace(RACE_PLANT) and Duel.IsPlayerCanDraw(tp, 1) and Duel.SelectYesNo(tp, aux.Stringid(69584564, 0)) then
      Duel.BreakEffect()
