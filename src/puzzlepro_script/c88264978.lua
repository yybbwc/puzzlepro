...
function c88264978.spfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_DRAGON) and c:IsAbleToRemoveAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c88264978.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c88264978.filter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, e, tp)
  end
