...
function s.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(s.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function s.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft <= 0 or not c:IsRelateToEffect(e) or c:IsFacedown() then
    return
  end
...
function s.tdop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:IsRelateToEffect(e) and Duel.SendtoDeck(c, nil, SEQ_DECKTOP, REASON_EFFECT) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
