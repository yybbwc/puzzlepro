...
function c18859369.tgfilter(c, tp)
  return c:IsAttribute(ATTRIBUTE_DARK + ATTRIBUTE_LIGHT) and c:IsDiscardable() and Duel.IsExistingMatchingCard(c18859369.tgfilter2, tp, LOCATION_DECK, 0, 1, nil, c:GetAttribute())
end
function c18859369.tgfilter2(c, attr)
  return c:IsAbleToGrave() and c:IsAttribute(ATTRIBUTE_DARK + ATTRIBUTE_LIGHT) and not c:IsAttribute(attr)
end
