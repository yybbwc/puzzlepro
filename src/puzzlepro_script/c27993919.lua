...
function c27993919.cfilter(c, e, tp)
  return (c:IsControler(tp) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c) > 0 and c:IsSetCard(0x162) and c:IsType(TYPE_PENDULUM) and Duel.IsExistingMatchingCard(c27993919.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCurrentScale())
end
function c27993919.spfilter(c, e, tp, sc)
  return c:IsSetCard(0x162) and c:IsType(TYPE_MONSTER) and not c:IsCode(27993919) and math.abs(c:GetCurrentScale() - sc) == 2 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c27993919.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroupEx(tp, c27993919.cfilter, 1, REASON_EFFECT, false, nil, e, tp)
  end
...
function c27993919.actcon2(e)
  ...
  return a and a:IsControler(tp) and a:IsSetCard(0x162) and Duel.IsExistingMatchingCard(c27993919.pfilter, tp, LOCATION_PZONE, 0, 1, nil)
end
