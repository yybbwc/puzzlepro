...
function c71817640.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return re:IsActiveType(TYPE_MONSTER) and rp == tp and rc:IsSetCard(0xc7) and rc:IsRace(RACE_DRAGON) and rc:IsLocation(LOCATION_MZONE) and re:GetActivateLocation() == LOCATION_MZONE
end
...
function c71817640.tsfilter(c, e, tp)
  if not (c:IsSetCard(0xc7, 0xda) and c:IsType(TYPE_MONSTER)) then
    return false
  end
...
function c71817640.tstg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c71817640.tsfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
