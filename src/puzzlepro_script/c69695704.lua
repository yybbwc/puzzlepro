...
function c69695704.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c69695704.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69695704.filter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c69695704.desop(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c69695704.cfilter, tp, LOCATION_MZONE, 0, 3, nil) then
    return
  end
