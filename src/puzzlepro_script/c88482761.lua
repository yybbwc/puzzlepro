...
function c88482761.rmfilter1(c, e, tp)
  return c:IsSetCard(0x2016) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c88482761.rmfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetOriginalLevel())
end
function c88482761.rmfilter2(c, e, tp, lv)
  return c:IsSetCard(0x2016) and c:IsType(TYPE_TUNER) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c88482761.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetOriginalLevel() + lv)
end
...
function c88482761.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c88482761.rmfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88482761.rmfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c88482761.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c88482761.atkfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c88482761.atkfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c88482761.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() == 2 then
    local c1 = g:GetFirst()
    local c2 = g:GetNext()
    if c1 ~= e:GetLabelObject() then
      c1, c2 = c2, c1
    end
    if c1:IsControler(1 - tp) and c1:IsPosition(POS_FACEUP_ATTACK) and not c1:IsImmuneToEffect(e) and c2:IsControler(tp) then
