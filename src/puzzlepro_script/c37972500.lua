...
function c37972500.thfilter(c, odevity)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:GetCurrentScale() % 2 == 1 - odevity and c:IsAbleToHand()
end
function c37972500.chkcon(g, tp, odevity)
  return g:IsExists(c37972500.chkfilter, 1, nil, odevity) and Duel.IsExistingMatchingCard(c37972500.thfilter, tp, LOCATION_EXTRA, 0, 1, nil, odevity)
end
function c37972500.tetg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c37972500.chkcon(g, tp, 0) or c37972500.chkcon(g, tp, 1)
  end
...
function c37972500.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c37972500.tgfilter(chkc, tp, g)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37972500.tgfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp, g)
  end
