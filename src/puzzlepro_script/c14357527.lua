...
function c14357527.sprcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c14357527.sprfilter, tp, LOCATION_REMOVED, 0, 3, nil)
end
...
function c14357527.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2) and c:IsReason(REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
...
function c14357527.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c14357527.atkfilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c14357527.atkfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
