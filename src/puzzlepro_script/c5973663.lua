...
function c5973663.cost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x18, 1, REASON_COST)
  end
...
function c5973663.tg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c5973663.filter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5973663.filter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c5973663.cost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x18, 2, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c5973663.cost3(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x18, 3, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c5973663.tg3(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c5973663.filter3(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c5973663.filter3, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
