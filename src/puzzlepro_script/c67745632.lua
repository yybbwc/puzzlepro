...
function c67745632.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c67745632.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67745632.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c67745632.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc, nil, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_HAND) then
    if e:GetLabel() > 0 and Duel.GetFieldGroupCount(tp, LOCATION_HAND + LOCATION_ONFIELD, 0) > 0 and Duel.GetFieldGroupCount(1 - tp, LOCATION_ONFIELD, 0) > 0 and Duel.SelectYesNo(tp, aux.Stringid(67745632, 2)) then
      Duel.BreakEffect()
...
function c67745632.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67745632.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c67745632.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    ...
    if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and g:FilterCount(Card.IsLocation, nil, LOCATION_DECK) > 0 and c:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(67745632, 3)) then
      Duel.BreakEffect()
