...
function c40441990.sumcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c40441990.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c40441990.sumop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 40441990) ~= 0 then
    return
  end
...
function c40441990.tetg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return bc and bc:IsType(TYPE_LINK) and bc:IsAbleToExtra()
  end
