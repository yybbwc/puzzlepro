...
function s.damval2(c)
  if c:GetPreviousLocation() ~= LOCATION_MZONE then
    return
  end
  return math.max(0, c:GetTextAttack())
end
...
function s.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local g = Duel.GetMatchingGroup(aux.TRUE, tp, 0, LOCATION_ONFIELD, nil)
  if Duel.Destroy(g, REASON_EFFECT) > 0 and c:IsRelateToEffect(e) and c:GetOverlayGroup():Filter(s.xfilter, nil):GetClassCount(Card.GetCode) >= 3 then
