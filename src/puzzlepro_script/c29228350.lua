...
function c29228350.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c29228350.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29228350.filter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil, tp) and Duel.IsExistingMatchingCard(aux.TRUE, tp, 0, LOCATION_MZONE, 1, nil)
  end
