...
function c59789370.tcfilter(c, tp)
  local lv = c:GetLevel()
  return c:IsFaceup() and lv > 0 and aux.AtkEqualsDef(c) and c:IsRace(RACE_MACHINE) and Duel.IsExistingMatchingCard(c59789370.tgfilter, tp, LOCATION_DECK, 0, 1, nil, lv)
end
function c59789370.tgfilter(c, lv)
  return aux.AtkEqualsDef(c) and c:GetLevel() < lv and c:IsRace(RACE_MACHINE) and c:IsAbleToGrave()
end
function c59789370.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c59789370.tcfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c59789370.tcfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
