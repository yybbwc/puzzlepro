...
function c78199891.filter(c, tp)
  return c:IsFaceup() and Duel.IsExistingMatchingCard(c78199891.eqfilter, tp, LOCATION_DECK, 0, 1, nil, tp, c)
end
...
function c78199891.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return c78199891.filter(chkc, tp)
  end
  if chk == 0 then
...
function c78199891.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c78199891.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c78199891.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
