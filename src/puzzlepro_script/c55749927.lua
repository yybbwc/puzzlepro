...
function c55749927.rmfilter(c)
  return (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup() or c:GetEquipTarget()) and (c:GetType() & (TYPE_EQUIP + TYPE_SPELL)) == TYPE_EQUIP + TYPE_SPELL and c:IsAbleToRemove()
end
function c55749927.desfilter(c, tp, g)
  ...
  return c:IsFaceup() and c:IsSetCard(0x148) and (not g or ft > 0 and g:CheckWithSumEqual(Card.GetLevel, 9, 1, ft))
end
...
function c55749927.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  elseif b1 then
    ...
    op = Duel.SelectOption(tp, aux.Stringid(55749927, 1)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
...
function c55749927.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabel() == 0 then
    ...
    if g:GetCount() > 0 and Duel.Remove(g, POS_FACEUP, REASON_EFFECT) ~= 0 and Duel.IsExistingMatchingCard(aux.TRUE, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, exc) and Duel.SelectYesNo(tp, aux.Stringid(55749927, 2)) then
      Duel.BreakEffect()
