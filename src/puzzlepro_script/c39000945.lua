...
function c39000945.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) >= 2 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsCanAddCounter(tp, 0x1, 1, c) and Duel.IsExistingMatchingCard(c39000945.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c39000945.rthtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c39000945.rthfilter, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler()) and Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c39000945.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c39000945.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
