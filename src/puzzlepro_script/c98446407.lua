...
function c98446407.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and c:IsReleasable() and Duel.CheckReleaseGroup(tp, nil, 1, c) and (ft > 0 or Duel.CheckReleaseGroup(tp, c98446407.mzfilter, 1, c, tp))
  end
  local rg = nil
...
function c98446407.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98446407.filter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
