...
function c38275183.condition1(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tg and tg:GetCount() == 1 and tg:GetFirst():IsLocation(LOCATION_MZONE) and re:IsActiveType(TYPE_SPELL) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
...
function c38275183.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c38275183.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
