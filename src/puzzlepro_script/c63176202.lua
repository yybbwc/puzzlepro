...
function c63176202.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c63176202.spfilter, tp, LOCATION_MZONE, 0, 2, nil)
end
...
function c63176202.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and c:IsOnField() and c:IsFaceup() and Duel.IsExistingMatchingCard(c63176202.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
...
function c63176202.count(e, tp, eg, ep, ev, re, r, rp)
  if ep == tp or not re:IsHasType(EFFECT_TYPE_ACTIVATE) then
    return
  end
