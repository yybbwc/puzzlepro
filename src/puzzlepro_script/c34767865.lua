...
function c34767865.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c34767865.cfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
...
function c34767865.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 34767866, 0, TYPES_TOKEN_MONSTER, 0, 0, 4, RACE_CYBERSE, ATTRIBUTE_LIGHT)
  end
...
function c34767865.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 34767866, 0, TYPES_TOKEN_MONSTER, 0, 0, 4, RACE_CYBERSE, ATTRIBUTE_LIGHT) then
    return
  end
  for i = 1, 2 do
