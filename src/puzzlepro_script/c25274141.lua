...
function c25274141.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(25274141, tp, ACTIVITY_ATTACK) == 0
  end
...
function c25274141.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingMatchingCard(c25274141.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c25274141.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c25274141.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(c25274141.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp) or not Duel.IsExistingMatchingCard(c25274141.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp) then
    return
  end
...
function c25274141.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c25274141.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
