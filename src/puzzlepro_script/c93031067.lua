...
function c93031067.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c93031067.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c93031067.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c93031067.setfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93031067.setfilter, tp, LOCATION_SZONE, 0, 1, nil) and e:GetHandler():IsSSetable()
  end
