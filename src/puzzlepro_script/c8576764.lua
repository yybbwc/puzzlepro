...
function c8576764.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c8576764.spfilter(chkc, e, tp, lv)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c8576764.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, lv)
  end
...
function c8576764.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rc:IsRelateToEffect(e) and rc:IsFaceup() and Duel.GetFieldGroupCount(tp, 0, LOCATION_HAND) > 0
  end
