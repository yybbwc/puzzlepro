...
function s.thfilter(c, att)
  return c:IsAttack(2400) and c:IsDefense(1000) and c:IsAttribute(att) and c:IsAbleToHand()
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return s.tgfilter(chkc, tp) and chkc:IsControler(1 - tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.tgfilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc or not tc:IsRelateToChain() or not tc:IsType(TYPE_MONSTER) or not tc:IsFaceup() then
    return
  end
