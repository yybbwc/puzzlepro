...
function c22819092.filter(c)
  return c:IsAbleToHand() and c:IsLevelBelow(7) and (aux.IsCodeListed(c, 22702055) or (c:IsType(TYPE_NORMAL) and c:IsAttribute(ATTRIBUTE_WATER)))
end
function c22819092.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22819092.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c22819092.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    Duel.ShuffleHand(tp)
    if Duel.IsEnvironment(22702055) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c22819092.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(22819092, 0)) then
      Duel.BreakEffect()
