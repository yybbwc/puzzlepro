...
function c77672444.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c77672444.filter, 1, nil, tp)
  end
...
function c77672444.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 77672445, 0, TYPES_TOKEN_MONSTER, 200, 200, 1, RACE_MACHINE, ATTRIBUTE_EARTH, POS_FACEUP_ATTACK)
  end
...
function c77672444.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 77672445, 0, TYPES_TOKEN_MONSTER, 200, 200, 1, RACE_MACHINE, ATTRIBUTE_EARTH, POS_FACEUP_ATTACK) then
    return
  end
  for i = 1, 2 do
