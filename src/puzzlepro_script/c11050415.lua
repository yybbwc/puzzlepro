...
function c11050415.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c11050415.filter, tp, 0x13, 0, 1, nil, e, tp)
  end
...
function c11050415.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if ft <= 0 or (Duel.IsPlayerAffectedByEffect(tp, 59822133) and ft > 1) then
      return
    end
    if Duel.IsPlayerCanSpecialSummonMonster(tp, 18027139, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_BEAST, ATTRIBUTE_EARTH) and Duel.SelectYesNo(tp, aux.Stringid(11050415, 0)) then
      Duel.BreakEffect()
