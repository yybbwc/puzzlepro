...
function c25845518.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c25845518.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c25845518.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c25845518.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c25845518.filter, tp, LOCATION_SZONE, 0, 1, nil, e, tp)
  end
