...
function c55343303.thfilter(c, tp)
  return c:IsSetCard(0x16c) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not Duel.IsExistingMatchingCard(c55343303.cfilter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil, c:GetCode())
end
function c55343303.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55343303.thfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
