...
function c11264180.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c11264180.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c11264180.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c11264180.dfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
