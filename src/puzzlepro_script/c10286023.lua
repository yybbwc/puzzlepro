...
function c10286023.dttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c10286023.texfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c10286023.texfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, c) and c:IsAbleToDeck()
  end
