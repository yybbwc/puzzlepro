...
function c25793414.cptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsType(TYPE_MONSTER)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsType, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, TYPE_MONSTER)
  end
...
function c25793414.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingTarget(c25793414.spfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.IsExistingTarget(c25793414.spfilter2, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
