...
function c97854941.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c97854941.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97854941.tgfilter, tp, LOCATION_ONFIELD, 0, 1, nil, tp) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c97854941.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c97854941.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
