...
function c13361027.cfilter(c, tp)
  return c:IsSetCard(0x29) and c:IsRace(RACE_DRAGON) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
function c13361027.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c13361027.cfilter, 1, nil, tp)
  end
...
function c13361027.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c13361027.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13361027.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
