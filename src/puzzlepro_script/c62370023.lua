...
function c62370023.filter(c, tp)
  return c:IsFaceup() and c:GetFlagEffect(62370023) == 0 and Duel.IsExistingMatchingCard(c62370023.atkfilter, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c62370023.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c62370023.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62370023.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
