...
function c63956833.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 2000) and Duel.GetCustomActivityCount(63956833, tp, ACTIVITY_SUMMON) == 0 and Duel.GetCustomActivityCount(63956833, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c63956833.filter1(c, e, tp)
  return c:IsSetCard(0x55) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and Duel.IsExistingMatchingCard(c63956833.filter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetLevel())
end
...
function c63956833.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c63956833.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c63956833.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
