...
function c82962242.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c82962242.eqfilter(chkc, e:GetHandler():GetAttack())
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c82962242.eqfilter, tp, 0, LOCATION_MZONE, 1, nil, e:GetHandler():GetAttack())
  end
...
function c82962242.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or not tc:IsType(TYPE_MONSTER) then
    return
  end
