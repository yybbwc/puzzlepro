...
function c43940008.csop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sc1 or sc2 then
    local p = 0
    if (not sc2) or sc1 and sc1:GetTextAttack() > sc2:GetTextAttack() then
      p = tp
    elseif (not sc1) or sc1:GetTextAttack() < sc2:GetTextAttack() then
      p = 1 - tp
    else
      p = PLAYER_ALL
    end
    ...
    if (p == tp or p == PLAYER_ALL) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsCanBeSpecialSummoned, tp, LOCATION_HAND, 0, 1, nil, e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(43940008, 2)) then
    ...
    if (p == 1 - tp or p == PLAYER_ALL) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsCanBeSpecialSummoned, 1 - tp, LOCATION_HAND, 0, 1, nil, e, 0, 1 - tp, false, false) and Duel.SelectYesNo(1 - tp, aux.Stringid(43940008, 2)) then
...
function c43940008.descon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return Duel.GetTurnCount() == e:GetLabel() and c:IsOnField() and c:IsFaceup() and c:GetFlagEffect(43940008) > 0
end
