...
function c71521025.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c71521025.rmfilter1, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(c71521025.rmfilter1, tp, 0, LOCATION_GRAVE, 1, nil)
  end
...
function c71521025.rmfilter2(c, e, tp)
  ...
  return c:IsType(TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ + TYPE_LINK) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c71521025.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp, type)
end
...
function c71521025.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c71521025.rmfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingTarget(c71521025.rmfilter2, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
