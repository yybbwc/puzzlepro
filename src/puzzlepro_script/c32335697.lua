...
function c32335697.rmfilter(c, tp)
  return c:IsRace(RACE_ZOMBIE) and c:IsLevelAbove(1) and c:IsAbleToRemove() and Duel.IsPlayerCanSpecialSummonMonster(tp, 32335698, nil, TYPES_TOKEN_MONSTER, 0, 0, c:GetLevel(), RACE_ZOMBIE, ATTRIBUTE_DARK)
end
function c32335697.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingMatchingCard(c32335697.rmfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, tp) and not Duel.IsPlayerAffectedByEffect(tp, 59822133)
  end
...
function c32335697.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_REMOVED) and Duel.IsPlayerCanSpecialSummonMonster(tp, 32335698, nil, TYPES_TOKEN_MONSTER, 0, 0, tc:GetLevel(), RACE_ZOMBIE, ATTRIBUTE_DARK) then
    for i = 1, 2 do
...
function c32335697.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsSSetable() and Duel.IsExistingMatchingCard(c32335697.setfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
