...
function c57135971.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return c57135971.filter(chkc) and chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c57135971.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c57135971.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsSetCard(0x3b)
end
...
function c57135971.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c57135971.desfilter(chkc) and chkc ~= tc
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57135971.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler():GetEquipTarget())
  end
