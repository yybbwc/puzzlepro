...
function c77710579.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77710579.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c77710579.spfilter(c, e, tp)
  return c:IsSetCard(0x3013) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and not Duel.IsExistingMatchingCard(c77710579.filter, tp, LOCATION_MZONE, 0, 1, nil, c:GetAttribute())
end
function c77710579.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c77710579.spfilter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
