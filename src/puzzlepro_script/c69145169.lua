...
function c69145169.costfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsType(TYPE_EFFECT) and (c:IsFaceup() or c:IsControler(tp)) and Duel.IsExistingTarget(c69145169.filter, tp, 0, LOCATION_MZONE, 1, c)
end
function c69145169.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 69145170) == 0 and Duel.CheckReleaseGroup(tp, c69145169.costfilter, 1, nil, tp)
  end
...
function c69145169.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c69145169.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69145169.filter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c69145169.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
