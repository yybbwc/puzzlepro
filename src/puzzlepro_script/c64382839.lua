...
function c64382839.cfilter(c, tp)
  return c:IsPreviousControler(1 - tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_TOKEN)
end
...
function c64382839.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 64382840, 0, TYPES_TOKEN_MONSTER, 500, 500, 2, RACE_FIEND, ATTRIBUTE_DARK) then
    return
  end
...
function c64382839.spfilter(c, tp)
  return c:IsCode(64382840) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
