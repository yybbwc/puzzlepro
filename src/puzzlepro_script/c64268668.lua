...
function c64268668.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE) and ((a and a:IsControler(tp) and a:IsFaceup() and a:IsCode(64268668)) or (d and d:IsControler(tp) and d:IsFaceup() and d:IsCode(64268668)))
end
...
function c64268668.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    if a:IsControler(tp) then
      return a:IsCanBeEffectTarget(e)
    else
      return d:IsCanBeEffectTarget(e)
    end
  end
  if a:IsControler(tp) then
    return Duel.SetTargetCard(a)
  else
    return Duel.SetTargetCard(d)
  end
end
