...
function c79868386.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(aux.NegateEffectMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c79868386.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if Duel.Destroy(tc, REASON_EFFECT) ~= 0 then
      ...
      if #sg > 0 and Duel.GetTurnPlayer() == tp and (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2) and Duel.SelectYesNo(tp, aux.Stringid(79868386, 0)) then
        Duel.BreakEffect()
