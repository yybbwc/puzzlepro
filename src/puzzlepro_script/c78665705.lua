...
function c78665705.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c78665705.filter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c78665705.gaintg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 78665706) == 0
  end
end
function c78665705.gainop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 78665706) ~= 0 then
    return
  end
