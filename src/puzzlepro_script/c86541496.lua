...
function c86541496.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 86541496) == 0 and g:GetCount() > 1 and g:GetCount() == g:FilterCount(Card.IsAbleToRemoveAsCost, nil)
  end
...
function c86541496.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86541496.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
