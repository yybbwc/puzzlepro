...
function c94130731.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94130731.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c94130731.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    ...
    if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and g:FilterCount(Card.IsLocation, nil, LOCATION_DECK) > 0 and c:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(94130731, 1)) then
      Duel.BreakEffect()
