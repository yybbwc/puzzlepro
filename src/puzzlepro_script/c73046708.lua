...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    ...
    return ft > 0 and Duel.IsExistingTarget(s.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(s.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function s.catg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and tc:IsChainAttackable()
  end
