...
function c57288064.filter(c, tp)
  ...
  return c:IsFaceup() and c:IsRace(RACE_ZOMBIE) and atk >= 0 and c:IsAbleToDeck() and Duel.IsExistingMatchingCard(c57288064.posfilter, tp, 0, LOCATION_MZONE, 1, nil, atk)
end
...
function c57288064.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c57288064.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57288064.filter, tp, LOCATION_REMOVED, 0, 1, nil, tp)
  end
