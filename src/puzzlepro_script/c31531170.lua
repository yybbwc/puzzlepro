...
function c31531170.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return (aux.PendulumChecklist & (0x1 << tp) == 0 or Duel.IsPlayerAffectedByEffect(tp, EFFECT_EXTRA_PENDULUM_SUMMON)) and Duel.IsExistingTarget(nil, tp, 0, LOCATION_PZONE, 2, nil)
  end
...
function c31531170.pendcon(e, c, og)
  ...
  if aux.PendulumChecklist & (0x1 << tp) ~= 0 and #eset == 0 then
    return false
  end
  local rpz = Duel.GetFieldCard(1 - tp, LOCATION_PZONE, 1)
  if rpz == nil or rpz:GetFieldID() ~= c:GetFlagEffectLabel(31531170) then
    return false
  end
