...
function c67647362.cfilter(c, e, tp)
  return (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and Duel.IsExistingMatchingCard(c67647362.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetAttribute())
end
...
function c67647362.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c67647362.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c67647362.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c67647362.cfilter, tp, 0, LOCATION_MZONE + LOCATION_GRAVE, 1, nil, e, tp) and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function c67647362.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sc and Duel.SpecialSummon(sc, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and sc:IsFaceup() and tgc:IsRelateToEffect(e) and tgc:IsControler(tp) and tgc:IsFaceup() and tgc:IsType(TYPE_EFFECT) then
    if not Duel.Equip(tp, tgc, sc, false) then
      return
    end
    --equip limit
    local e1 = Effect.CreateEffect(c)
