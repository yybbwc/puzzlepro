...
function c47882774.actcon(e, tp, eg, ep, ev, re, r, rp)
  return ep == 1 - tp and Duel.IsChainDisablable(ev) and Duel.IsExistingMatchingCard(c47882774.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and ((re:IsActiveType(TYPE_MONSTER) and Duel.GetChainInfo(ev, CHAININFO_TRIGGERING_LOCATION) == LOCATION_MZONE) or (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE))))
end
