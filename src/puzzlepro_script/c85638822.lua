...
function c85638822.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanAddCounter(tp, 0x46, 3, e:GetHandler())
  end
...
function c85638822.rccon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rc:IsRelateToBattle() and rc:IsStatus(STATUS_OPPO_BATTLE) and rc:IsFaceup() and rc:IsSetCard(0xfc) and rc:IsControler(tp)
end
...
function c85638822.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c85638822.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
