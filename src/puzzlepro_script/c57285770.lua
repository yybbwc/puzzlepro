...
function c57285770.costfilter(c)
  return (c:IsSetCard(0x179) or c:IsCode(21887075)) and not c:IsCode(57285770) and c:IsAbleToGraveAsCost() and ((c:IsFaceup() and c:GetSequence() < 5) or not c:IsLocation(LOCATION_SZONE))
end
function c57285770.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c57285770.costfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil)
  end
