...
function s.thfilter(c)
  return c:IsFaceupEx() and c:IsRace(RACE_CYBERSE) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and s.thfilter(chkc)
  end
  if chk == 0 then
...
function s.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.lvfilter(chkc, c:GetLevel()) and chkc ~= c
  end
  if chk == 0 then
    return c:IsLevelAbove(1) and Duel.IsExistingTarget(s.lvfilter, tp, LOCATION_MZONE, 0, 1, c, c:GetLevel())
  end
