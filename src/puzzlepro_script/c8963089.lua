...
function s.rfilter(c)
  return c:IsFaceupEx() and c:IsAbleToRemoveAsCost() and c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT)
end
function s.gcheck(g)
  return (#g == 2 and g:FilterCount(Card.IsLocation, nil, LOCATION_GRAVE) == 2 or #g == 1 and g:FilterCount(Card.IsLocation, nil, LOCATION_MZONE) == 1)
end
function s.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local g = Duel.GetMatchingGroup(s.rfilter, tp, LOCATION_GRAVE + LOCATION_MZONE, 0, nil)
  if chk == 0 then
    return g:CheckSubGroup(s.gcheck, 1, 2)
  end
