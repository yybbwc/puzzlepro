...
function c92919429.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c92919429.lvfilter, tp, LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil)
  end
...
function c92919429.lvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoGrave(tc, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_GRAVE) and tc:GetLevel() > 0 and c:IsFaceup() and c:IsRelateToEffect(e) then
    local e1 = Effect.CreateEffect(c)
...
function c92919429.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c92919429.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
