...
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return g:IsContains(chkc) and e:GetHandler():IsAbleToRemove()
  end
  if chk == 0 then
    return e:IsCostChecked() and #g > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) >= 0 and Duel.IsExistingTarget(s.spfilter1, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function s.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if rc:IsRelateToEffect(e) and rc:IsControler(1 - tp) and rc:IsType(TYPE_MONSTER) and c:IsRelateToEffect(e) and c:IsAbleToRemove() and rc:IsAbleToRemove() then
