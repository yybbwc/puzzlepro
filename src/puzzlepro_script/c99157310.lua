...
function c99157310.condition(e, tp, eg, ep, ev, re, r, rp)
  return ep == tp and Duel.GetAttacker():IsControler(1 - tp) and Duel.GetMatchingGroup(c99157310.cfilter1, tp, LOCATION_GRAVE, 0, nil):GetClassCount(Card.GetCode) >= 3
end
...
function c99157310.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tg:IsOnField() and Duel.IsExistingMatchingCard(c99157310.filter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c99157310.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and g:GetClassCount(Card.GetCode) > 2
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
...
function c99157310.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft <= 0 or g:GetCount() == 0 or (g:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) then
    return
  end
