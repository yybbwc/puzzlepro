...
function c81767888.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    e:SetLabel(0)
    return ft > 0 and Duel.IsExistingMatchingCard(c81767888.costfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsPlayerCanSpecialSummonMonster(tp, 81767889, 0, TYPES_TOKEN_MONSTER, 2500, 2000, 8, RACE_DRAGON, ATTRIBUTE_DARK)
  end
  e:SetLabel(0)
...
function c81767888.spop(e, tp, eg, ep, ev, re, r, rp)
  local ct = e:GetLabel()
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if ct > ft or not Duel.IsPlayerCanSpecialSummonMonster(tp, 81767889, 0, TYPES_TOKEN_MONSTER, 2500, 2000, 8, RACE_DRAGON, ATTRIBUTE_DARK) then
    return
  end
...
function c81767888.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c81767888.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81767888.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
