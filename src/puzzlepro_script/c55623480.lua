...
function c55623480.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c55623480.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c55623480.posfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c55623480.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return sg:GetCount() >= 7 and (ft > 0 or sg:IsExists(c55623480.mainfilter, ct, nil))
  end
  local g = nil
  if ft <= 0 then
