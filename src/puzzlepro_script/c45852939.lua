...
function s.xatktg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsHasEffect(EFFECT_EXTRA_ATTACK_MONSTER)
  end
end
...
function s.tgfilter(c, e, tp)
  return c:IsCanBeEffectTarget(e) and c:IsRankBelow(4) and (c:IsCanOverlay() or c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
