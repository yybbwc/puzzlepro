...
function c81019803.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c81019803.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81019803.thfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c81019803.spfilter(c, e, tp)
  return c:IsSetCard(0x146) and c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c81019803.tdop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:IsRelateToEffect(e) and Duel.SendtoDeck(c, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and c:IsLocation(LOCATION_EXTRA) and Duel.IsExistingMatchingCard(c81019803.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(81019803, 2)) then
    Duel.BreakEffect()
