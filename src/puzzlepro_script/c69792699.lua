...
function c69792699.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c69792699.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69792699.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c69792699.rthtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69792699.rthfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c69792699.cfilter(c, tp)
  return c:IsPreviousControler(1 - tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEUP) and c:IsReason(REASON_EFFECT) and c:IsLocation(LOCATION_HAND + LOCATION_REMOVED)
end
