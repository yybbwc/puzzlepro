...
function c95471006.cfilter(c, tp)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_UNION) and c:IsPreviousControler(tp) and c:IsFaceup()
end
...
function c95471006.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c95471006.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c95471006.sprcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95471006.sprcfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c95471006.sprtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95471006.sprfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
  local max = 2
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) or Duel.GetMZoneCount(tp) < 2 then
    max = 1
  end
...
function c95471006.sprop(e, tp, eg, ep, ev, re, r, rp)
  local max = 2
  if Duel.GetMZoneCount(tp) < 1 then
    return
  end
