...
function c12467005.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(12467005, 1), aux.Stringid(12467005, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(12467005, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(12467005, 2)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
