...
function c85808813.filter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x149) and c:IsType(TYPE_FUSION) and c:IsReleasableByEffect() and Duel.IsExistingMatchingCard(c85808813.ffilter, tp, LOCATION_EXTRA, 0, 1, nil, c:GetOriginalLevel(), e, tp, c)
end
function c85808813.ffilter(c, lv, e, tp, tc)
  return c:IsSetCard(0x149) and c:IsType(TYPE_FUSION) and c:GetOriginalLevel() == lv + 2 and c:IsCanBeSpecialSummoned(e, SUMMON_VALUE_FOSSIL_FUSION, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, tc, c) > 0
end
...
function c85808813.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and chkc:IsReleasableByEffect() and chkc:IsFaceup() and chkc:IsSetCard(0x149) and chkc:IsType(TYPE_FUSION)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85808813.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c85808813.cfilter(c, e, tp)
  return c:IsSetCard(0x149) and c:IsType(TYPE_FUSION) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c85808813.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
...
function c85808813.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c85808813.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c85808813.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c85808813.spfilter(chkc, e, tp)
  end
  if chk == 0 then
