...
function c40177746.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c40177746.cfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c40177746.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c40177746.thop(e, tp, eg, ep, ev, re, r, rp)
  local dg = Duel.GetMatchingGroup(c40177746.filter, tp, LOCATION_DECK, 0, nil)
  local ct = e:GetLabel()
  if dg:GetClassCount(Card.GetCode) < ct then
    return
  end
