...
function c96637156.movop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or c:GetSequence() >= 5 then
    return
  end
  ...
  if p ~= tp and not c:IsControlerCanBeChanged(true) or Duel.GetMZoneCount(p, tc, tp, LOCATION_REASON_CONTROL, zone) <= 0 or tc and (not tc:IsCanOverlay(p) or tc:IsImmuneToEffect(e)) then
  ...
  if winflag and c:GetOverlayCount() > 6 then
    Duel.Win(tp, WIN_REASON_MUSOMARU)
  end
end
