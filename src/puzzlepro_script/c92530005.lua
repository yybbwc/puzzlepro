...
function s.tgfilter(c)
  return c:IsFaceup() and c:IsSetCard(0xc6) and c:IsType(TYPE_PENDULUM) and c:IsAbleToGrave() and c:GetLeftScale() > 0
end
...
function s.pop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoGrave(tc, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_GRAVE) and Duel.SelectYesNo(tp, aux.Stringid(id, 8)) then
    Duel.BreakEffect()
...
function s.tg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    local sel = 0
    if Duel.IsExistingMatchingCard(s.schfilter, tp, LOCATION_DECK, 0, 1, nil) then
      sel = sel + 1
    end
    if Duel.IsExistingMatchingCard(s.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
      sel = sel + 2
    end
    e:SetLabel(sel)
    return sel ~= 0
...
function s.spcon2(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetAttacker():IsControler(1 - tp) and Duel.GetAttackTarget() == nil and Duel.GetAttacker():IsSummonType(SUMMON_TYPE_SPECIAL)
end
