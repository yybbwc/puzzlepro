...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b1 = #g1 > 1 and Duel.IsPlayerCanRemove(tp) and g1:IsExists(Card.IsAbleToRemove, 1, nil, tp, POS_FACEDOWN, REASON_RULE)
  local b2 = #g2 > 1 and Duel.IsPlayerCanRemove(1 - tp) and g2:IsExists(Card.IsAbleToRemove, 1, nil, 1 - tp, POS_FACEDOWN, REASON_RULE)
  if chk == 0 then
    return b1 or b2
  end
  local g3 = g1 + g2
...
function s.thfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x189) and c:IsAbleToHand() and c:GetOwner() == tp
end
function s.xfilter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x189) and c:GetOverlayGroup():IsExists(s.thfilter, 1, nil, tp)
end
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and s.xfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.xfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and #mg > 0 then
    ...
    if Duel.SendtoHand(bc, nil, REASON_EFFECT) > 0 and bc:IsLocation(LOCATION_HAND) then
      Duel.ConfirmCards(1 - tp, bc)
      Duel.ShuffleHand(tp)
      if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and bc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
        Duel.BreakEffect()
