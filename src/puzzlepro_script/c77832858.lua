...
function s.tcheck(g, tp)
  return g:IsExists(s.tfilter, 1, nil, tp) and g:IsExists(Card.IsControler, 1, nil, 1 - tp) and Duel.GetMZoneCount(tp, g) > 0
end
...
function s.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 and Duel.Remove(g, POS_FACEUP, REASON_EFFECT) > 0 and Duel.Damage(1 - tp, 1000, REASON_EFFECT) > 0 then
    ...
    if e:GetLabel() > 0 and #rg > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
      ...
      Duel.BreakEffect()
      if Duel.Remove(rc, POS_FACEUP, REASON_EFFECT) > 0 and rc:IsAttribute(ATTRIBUTE_DARK + ATTRIBUTE_FIRE) and rc:GetOriginalType() & TYPE_MONSTER > 0 and rc:GetOriginalLevel() > 0 then
