...
function c16958382.spcon(e, c)
  ...
  local b1 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c16958382.spfilter, tp, LOCATION_EXTRA, 0, 1, nil)
...
function c16958382.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16958382.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c16958382.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16958382.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function c16958382.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, tc) > 0 and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(16958382, 1)) then
    Duel.BreakEffect()
