...
function c60461077.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 or not Duel.IsExistingMatchingCard(c60461077.cfilter, tp, LOCATION_MZONE, 0, 1, nil))
end
...
function c60461077.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c60461077.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
