...
function c15555120.cfilter(c, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsSetCard(0x2016) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c15555120.filter, tp, LOCATION_MZONE, 0, 1, nil, lv)
end
function c15555120.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c15555120.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c15555120.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c15555120.filter(chkc, e:GetLabel())
  end
  if chk == 0 then
    return true
  end
