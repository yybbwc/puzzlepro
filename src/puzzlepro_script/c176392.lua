...
function c176392.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:IsExists(c176392.rfilter, 1, c) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
  end
...
function c176392.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 176393, 0x1d, TYPES_TOKEN_MONSTER, 1800, 1800, 4, RACE_ROCK, ATTRIBUTE_EARTH)
  end
...
function c176392.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 176393, 0x1d, TYPES_TOKEN_MONSTER, 1800, 1800, 4, RACE_ROCK, ATTRIBUTE_EARTH) then
    return
  end
