...
function c60806437.filter(c, e, tp)
  return c:IsAttackBelow(1500) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_ATTACK)
end
function c60806437.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c60806437.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
