...
function c64765016.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c64765016.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c64765016.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 64765016, 0xd4, TYPES_NORMAL_TRAP_MONSTER, 1200, 0, 2, RACE_AQUA, ATTRIBUTE_WATER)
  end
