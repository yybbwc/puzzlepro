...
function c11270236.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c11270236.rmfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c11270236.rmfilter1, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil)
  end
