...
function c45950291.filter2(c, e, tp, mc, rk, rc, att, code)
  ...
  return c:IsRank(rk) and c:IsRace(rc) and c:IsAttribute(att) and mc:IsCanBeXyzMaterial(c) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c45950291.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c45950291.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45950291.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c45950291.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.IsPlayerCanNormalDraw(tp) and Duel.GetActivityCount(tp, ACTIVITY_SPSUMMON) == 0
  end
