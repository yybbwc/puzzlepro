...
function c71549257.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local lv = c:GetLevel()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c71549257.lvfilter(chkc, lv)
  end
  if chk == 0 then
    return lv > 0 and Duel.IsExistingTarget(c71549257.lvfilter, tp, LOCATION_MZONE, 0, 1, c, lv)
  end
