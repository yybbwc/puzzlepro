...
function c4709881.cfilter(c, g, tp, zone)
  return c:IsSetCard(0xfd) and g:IsContains(c) and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_TOFIELD, zone) > 0
end
function c4709881.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  local lg = e:GetHandler():GetLinkedGroup()
  local zone = e:GetHandler():GetLinkedZone(tp)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c4709881.cfilter, 1, nil, lg, tp, zone)
  end
...
function c4709881.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and chkc ~= cc and c4709881.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4709881.spfilter1, tp, LOCATION_GRAVE, 0, 1, cc, e, tp)
  end
...
function c4709881.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c4709881.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
