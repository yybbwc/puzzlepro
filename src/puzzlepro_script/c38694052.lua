...
function c38694052.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 2, REASON_COST) and c:GetAttackAnnouncedCount() == 0
  end
  local e1 = Effect.CreateEffect(c)
...
function c38694052.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() then
    ...
    if og:IsExists(c38694052.checkfilter, 1, nil) and #g > 0 and Duel.SelectYesNo(tp, aux.Stringid(38694052, 1)) then
      local star = 0
      if tc:IsType(TYPE_XYZ) then
        star = tc:GetOriginalRank()
      else
        star = tc:GetOriginalLevel()
      end
