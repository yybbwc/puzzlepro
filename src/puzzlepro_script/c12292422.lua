...
function c12292422.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c12292422.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c12292422.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    if not Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_HAND, 0, 1, nil, TYPE_MONSTER) then
      return false
    end
    local mc = Duel.GetFieldGroupCount(tp, 0, LOCATION_DECK)
    local g = Duel.GetFieldGroup(tp, 0, LOCATION_HAND)
    return mc > 0 or g and g:IsExists(c12292422.tgfilter0, 1, nil)
  end
