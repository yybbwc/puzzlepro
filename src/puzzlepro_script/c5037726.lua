...
function c5037726.rfilter(c, ft, tp)
  return c:IsAttribute(ATTRIBUTE_LIGHT) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup())
end
function c5037726.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c5037726.rfilter, 1, nil, ft, tp)
  end
...
function c5037726.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c5037726.filter(chkc, e, tp)
  end
  if chk == 0 then
    if e:GetLabel() == 1 then
      ...
      return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c5037726.filter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil, e, tp)
    end
  end
  e:SetLabel(0)
