...
function c81260679.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81260679.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c81260679.filter(c, tp)
  return c:IsType(TYPE_EQUIP) and c:CheckUniqueOnField(tp) and not c:IsForbidden() and Duel.GetMatchingGroupCount(c81260679.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, nil, c) > 0
end
...
function c81260679.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c81260679.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c81260679.filter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
