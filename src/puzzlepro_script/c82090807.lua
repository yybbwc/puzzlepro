...
function c82090807.sprcon1(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c82090807.twofilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
end
function c82090807.sprcon2(e, c)
  ...
  local tp = c:GetControler()
  return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c82090807.twofilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
end
...
function c82090807.dhtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c82090807.xfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82090807.xfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
