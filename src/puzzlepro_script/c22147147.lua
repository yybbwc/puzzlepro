...
function c22147147.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c22147147.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22147147.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c22147147.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c22147147.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
