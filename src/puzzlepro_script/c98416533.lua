...
function c98416533.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SendtoHand(c, nil, REASON_EFFECT) ~= 0 and c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and g:GetClassCount(Card.GetCode) > 1 and Duel.SelectYesNo(tp, aux.Stringid(98416533, 2)) then
    Duel.BreakEffect()
