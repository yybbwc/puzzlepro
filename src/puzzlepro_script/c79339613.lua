...
function c79339613.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsFaceup, 2, g)
  end
...
function c79339613.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c79339613.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c79339613.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoHand(g, nil, REASON_EFFECT) > 0 and g:GetFirst():IsLocation(LOCATION_HAND) then
    ...
    if Duel.IsExistingMatchingCard(c79339613.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(79339613, 0)) then
      Duel.BreakEffect()
      Duel.ShuffleHand(tp)
