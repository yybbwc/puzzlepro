...
function c77297908.cfilter2(c, tp)
  return c:IsSetCard(0x20ec) and c:IsType(TYPE_SPELL) and not c:IsPublic() and Duel.IsExistingMatchingCard(c77297908.cfilter3, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c77297908.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77297908.cfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingMatchingCard(c77297908.cfilter2, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c77297908.chcon(e, tp, eg, ep, ev, re, r, rp)
  return ep == 1 - tp and re:IsActiveType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(c77297908.confilter, tp, LOCATION_MZONE, 0, 1, nil)
end
