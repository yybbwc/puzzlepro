...
function c44874522.valcheck(e, c)
  ...
  local tc = g:GetFirst()
  while tc do
    if tc:IsSetCard(0x7) and tc:IsType(TYPE_MONSTER) then
      flag = bit.bor(flag, 0x1)
    end
    if tc:IsSetCard(0x51) and tc:IsType(TYPE_MONSTER) then
...
function c44874522.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44874522.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
