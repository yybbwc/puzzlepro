...
function c66084673.atkcon(e, tp, eg, ep, ev, re, r, rp)
  return (Duel.GetAttacker():IsControler(tp) and Duel.GetAttacker():IsSetCard(0x79)) or (Duel.GetAttackTarget() and Duel.GetAttackTarget():IsControler(tp) and Duel.GetAttackTarget():IsSetCard(0x79))
end
...
function c66084673.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(66084673) == 0 and Duel.IsExistingMatchingCard(c66084673.filter1, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.IsExistingMatchingCard(c66084673.filter2, tp, LOCATION_HAND, 0, 1, nil)
  end
