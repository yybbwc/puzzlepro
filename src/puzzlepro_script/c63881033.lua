...
function c63881033.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63881033.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c63881033.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsType(TYPE_XYZ) and c:IsRace(RACE_MACHINE) and c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE)
end
function c63881033.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c63881033.repfilter, 1, nil, tp)
  end
...
function c63881033.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63881033.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
