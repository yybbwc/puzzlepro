...
function c81558967.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    if ct == 0 or Duel.GetFieldGroupCount(tp, LOCATION_DECK, 0) < ct then
      return false
    end
...
function c81558967.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c81558967.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
