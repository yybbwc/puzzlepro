...
function c61264008.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_FZONE) and c61264008.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61264008.filter, tp, LOCATION_FZONE, LOCATION_FZONE, 1, nil)
  end
...
function c61264008.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    ...
    if Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) ~= 0 then
      ...
      if Duel.MoveToField(tc2, 1 - ttp, 1 - ttp, LOCATION_FZONE, POS_FACEUP, true) ~= 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c61264008.filter2), 1 - ttp, LOCATION_GRAVE, 0, 1, nil, code) and Duel.SelectYesNo(tp, aux.Stringid(61264008, 0)) then
        Duel.BreakEffect()
