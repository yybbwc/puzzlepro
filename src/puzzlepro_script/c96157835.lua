...
function c96157835.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c96157835.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c96157835.spfilter2(c, e, tp)
  return c:IsSetCard(0xba) and c:IsType(TYPE_XYZ) and not c:IsCode(96157835) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c96157835.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96157835.spfilter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp) and e:GetHandler():IsCanOverlay()
  end
