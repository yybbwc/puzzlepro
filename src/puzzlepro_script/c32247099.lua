...
function c32247099.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32247099.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c32247099.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c32247099.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c32247099.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c32247099.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) ~= 0 and not Duel.GetAttacker():IsImmuneToEffect(e) then
    Duel.BreakEffect()
