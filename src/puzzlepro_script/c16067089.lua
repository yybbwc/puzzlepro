...
function c16067089.cfilter(c, tp)
  return c:IsCode(72677437) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c16067089.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c16067089.filter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
