...
function c97803170.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 97803170) == 0
  end
  Duel.RegisterFlagEffect(tp, 97803170, RESET_PHASE + PHASE_END, 0, 1)
end
function c97803170.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return c97803170.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  end
  if chk == 0 then
    return true
  end
  local b1 = c97803170.cost(e, tp, eg, ep, ev, re, r, rp, 0) and c97803170.sptg(e, tp, eg, ep, ev, re, r, rp, 0)
...
function c97803170.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c97803170.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c97803170.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c97803170.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
