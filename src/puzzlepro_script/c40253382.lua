...
function c40253382.cfilter(c, tp)
  ...
  return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and rc:IsSetCard(0x27) and rc:IsControler(tp) and rc:IsRelateToBattle()
end
...
function c40253382.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c40253382.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c40253382.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
