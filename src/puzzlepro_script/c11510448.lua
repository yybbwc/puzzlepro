...
function c11510448.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 11510448) == 0
  end
...
function c11510448.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c11510448.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c11510448.filter2, tp, LOCATION_GRAVE, 0, 1, nil)
  end
