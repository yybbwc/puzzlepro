...
function c86016245.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) == 0 and eg:GetCount() == 1 and tc:IsLocation(LOCATION_GRAVE) and tc:IsReason(REASON_BATTLE) and bc:IsRelateToBattle() and bc:IsControler(tp) and bc:IsLevelBelow(2)
end
...
function c86016245.drop(e, tp, eg, ep, ev, re, r, rp)
  if not e:GetHandler():IsRelateToEffect(e) or Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) ~= 0 then
    return
  end
