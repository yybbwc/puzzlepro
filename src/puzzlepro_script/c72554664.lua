...
function s.spfilter1(c, e, tp)
  return c:IsCode(68468459) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and s.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE + LOCATION_GRAVE, LOCATION_MZONE + LOCATION_GRAVE, 1, nil, tp)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_EXTRA) then
    ...
    if not Duel.IsPlayerAffectedByEffect(tp, 59822133) and #g1 > 0 and #g2 > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 0)) then
      Duel.BreakEffect()
