...
function c73422829.cfilter(c, tp)
  return c:IsNonAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_MONSTER) and c:IsSetCard(0xdc) and c:IsAbleToGraveAsCost() and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c) > 0
end
function c73422829.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73422829.cfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, e:GetHandler(), tp)
  end
...
function c73422829.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
  end
...
function c73422829.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73422829.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c73422829.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73422829.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
