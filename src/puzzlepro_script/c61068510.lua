...
function c61068510.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(1 - tp) and c61068510.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61068510.filter, tp, 0, LOCATION_SZONE, 1, nil)
  end
