...
function c3567660.filter(c, tp)
  if not (c:IsFaceup() and c:IsType(TYPE_LINK) and c:GetSequence() >= 5) then
    return false
  end
...
function c3567660.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c3567660.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c3567660.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c3567660.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc:IsRelateToEffect(e) and tc:IsControler(tp)) then
    return
  end
