...
function c8660395.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c8660395.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8660395.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c8660395.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c8660395.desfilter, tp, LOCATION_SZONE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsType, tp, 0, LOCATION_ONFIELD, 1, nil, TYPE_SPELL + TYPE_TRAP)
  end
