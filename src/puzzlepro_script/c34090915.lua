...
function s.cfilter(c, e)
  return c:IsFaceup() and c:IsAttribute(ATTRIBUTE_DARK + ATTRIBUTE_LIGHT) and c:IsAbleToDeck() and c:IsCanBeEffectTarget(e) and c:IsLocation(LOCATION_REMOVED)
end
function s.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return eg:IsExists(s.cfilter, 1, nil, e) and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetFlagEffect(tp, id) == 0 and Duel.IsExistingTarget(s.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
