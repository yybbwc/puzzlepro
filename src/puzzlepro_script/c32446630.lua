...
function c32446630.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 32446631, 0, TYPES_TOKEN_MONSTER, -2, -2, 1, RACE_FIEND, ATTRIBUTE_DARK)
  end
...
function c32446630.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 32446631, 0, TYPES_TOKEN_MONSTER, -2, -2, 1, RACE_FIEND, ATTRIBUTE_DARK) then
    return
  end
