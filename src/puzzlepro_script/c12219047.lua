...
function c12219047.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c12219047.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c12219047.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c12219047.condition2(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():GetOverlayGroup():IsExists(Card.IsSetCard, 1, nil, 0x10db) and aux.dscon(e, tp, eg, ep, ev, re, r, rp)
end
