...
function c36591747.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c36591747.filter(chkc, tp)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c36591747.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function c36591747.sttg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return ec and ec:IsCanChangePosition() and Duel.IsExistingMatchingCard(c36591747.stfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c36591747.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return ec and ec:IsAbleToRemove(tp)
  end
