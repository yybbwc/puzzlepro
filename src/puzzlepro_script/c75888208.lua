...
function c75888208.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c75888208.spfilter1, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c75888208.spcon2(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (ph == PHASE_MAIN1 or (ph >= PHASE_BATTLE_START and ph <= PHASE_BATTLE) or ph == PHASE_MAIN2) and Duel.IsEnvironment(74665651, PLAYER_ALL, LOCATION_FZONE)
end
...
function c75888208.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c75888208.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
