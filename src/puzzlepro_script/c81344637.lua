...
function c81344637.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c81344637.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c81344637.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81344637.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
