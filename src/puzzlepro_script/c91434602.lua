...
function c91434602.spcfilter(c, tp)
  return c:IsSetCard(0x44) and (not c:IsLocation(LOCATION_MZONE) or c:IsFaceup()) and c:IsAbleToRemoveAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c91434602.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if check then
    return e:GetHandler():GetFlagEffect(91434602) < 2
  else
    return e:GetHandler():GetFlagEffect(91434602) < 1
  end
end
...
function c91434602.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91434602.costfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
