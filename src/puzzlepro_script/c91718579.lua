...
function c91718579.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c91718579.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91718579.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c91718579.posop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsDefensePos() or not tc:IsRelateToEffect(e) then
    return
  end
