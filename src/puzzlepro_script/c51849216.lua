...
function c51849216.condition(e, tp, eg, ep, ev, re, r, rp)
  return ep == 1 - tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev) and Duel.IsExistingMatchingCard(c51849216.filter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c51849216.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Remove(eg, POS_FACEUP, REASON_EFFECT) ~= 0 and eg:GetFirst():IsLocation(LOCATION_REMOVED) then
    ...
    if Duel.IsExistingMatchingCard(c51849216.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and ct > 0 and Duel.SelectYesNo(tp, aux.Stringid(51849216, 0)) then
      Duel.BreakEffect()
