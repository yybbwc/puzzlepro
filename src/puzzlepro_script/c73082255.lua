...
function c73082255.eval(e, re, rp)
  ...
  return re:IsActiveType(TYPE_MONSTER) and re:GetActivateLocation() == LOCATION_MZONE and rc:IsSummonType(SUMMON_TYPE_SPECIAL) and (not rc:IsSummonLocation(LOCATION_GRAVE) or (rc:GetOriginalType() & TYPE_TRAP ~= 0))
end
...
function c73082255.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDiscardDeck(tp, 4) and Duel.IsPlayerCanDiscardDeck(1 - tp, 4)
  end
