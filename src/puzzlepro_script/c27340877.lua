...
function c27340877.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c27340877.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c27340877.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 2) and Duel.IsPlayerCanDraw(1 - tp, 2)
  end
