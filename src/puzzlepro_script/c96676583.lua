...
function s.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.rmfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function s.drop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local rc = g:GetFirst()
  if rc and Duel.Remove(rc, POS_FACEUP, REASON_EFFECT) ~= 0 and rc:IsLocation(LOCATION_REMOVED) and Duel.IsPlayerCanDraw(tp, 2) and Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
    Duel.BreakEffect()
...
function s.immtg(e, c)
  ...
  return c:IsFaceup() and c:IsType(TYPE_LINK) and c:IsSetCard(0x1bf) and lg and lg:IsContains(e:GetHandler())
end
