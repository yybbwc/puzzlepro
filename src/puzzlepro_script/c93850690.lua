...
function c93850690.spfilter(c, e, tp, check)
  return c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and ((check and c:IsRace(RACE_WYRM)) or c:IsSetCard(0x16b))
end
...
function c93850690.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c93850690.spfilter(chkc, e, tp, check)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c93850690.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, check)
  end
...
function c93850690.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c93850690.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93850690.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
