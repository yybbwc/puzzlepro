...
function c8279188.target1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if Duel.CheckEvent(EVENT_ATTACK_ANNOUNCE) and tp ~= Duel.GetTurnPlayer() and tg:IsOnField() and tg:IsCanBeEffectTarget(e) and Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) ~= 0 and Duel.SelectYesNo(tp, aux.Stringid(8279188, 0)) then
...
function c8279188.target2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_CHAINING) and Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) ~= 0 and tg:IsOnField() and tg:IsCanBeEffectTarget(e)
  end
  Duel.SetTargetCard(tg)
end
