...
function c4483989.atktg1(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  e:SetLabel(0)
  local at = Duel.GetAttackTarget()
  if Duel.CheckEvent(EVENT_ATTACK_ANNOUNCE) and tp ~= Duel.GetTurnPlayer() and at and at:IsPosition(POS_FACEUP_DEFENSE) then
    e:SetLabel(1)
...
function c4483989.atkop(e, tp, eg, ep, ev, re, r, rp)
  if e:GetLabel() == 0 or not e:GetHandler():IsRelateToEffect(e) then
    return
  end
