...
function c89899996.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c89899996.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and not Duel.IsExistingMatchingCard(c89899996.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_HAND)
end
function c89899996.operation(e, tp, eg, ep, ev, re, r, rp)
  if Duel.IsExistingMatchingCard(c89899996.cfilter, tp, LOCATION_MZONE, 0, 1, nil) or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 then
    return
  end
