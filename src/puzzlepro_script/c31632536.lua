...
function c31632536.caop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetHandler() == a and d and d:IsDefensePos() then
    e:SetLabel(1)
  else
    e:SetLabel(0)
  end
...
function c31632536.caop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabelObject():GetLabel() == 1 and c:IsRelateToBattle() and c:IsChainAttackable() and Duel.GetMatchingGroupCount(Card.IsSetCard, tp, LOCATION_GRAVE, 0, nil, 0x39) >= 2 then
    Duel.ChainAttack()
  end
end
