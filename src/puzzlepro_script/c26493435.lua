...
function c26493435.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x30, 2, REASON_COST)
  end
  e:GetHandler():RemoveCounter(tp, 0x30, 2, REASON_COST)
end
function c26493435.filter(c, e, tp)
  return c:IsSetCard(0x8f, 0x54, 0x59, 0x82) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c26493435.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c26493435.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
