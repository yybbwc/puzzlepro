...
function c368382.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return e:GetHandler():GetFlagEffect(368382) == 0 and re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) and g and g:IsExists(c368382.tfilter, 1, e:GetHandler(), tp) and Duel.IsChainDisablable(ev) and not Duel.IsChainDisabled(ev)
end
...
function c368382.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and not Duel.IsExistingMatchingCard(c368382.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and tg:IsExists(Card.IsControler, 1, nil, 1 - tp)
end
