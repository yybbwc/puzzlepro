...
function c28066831.flagop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsLocation(LOCATION_GRAVE) or bit.band(c:GetReason(), REASON_BATTLE) == 0 then
    return
  end
  --special summon
  local e1 = Effect.CreateEffect(c)
...
function c28066831.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28066831.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c28066831.filter(c, e, tp)
  return c:IsDefenseBelow(1500) and c:IsRace(RACE_PSYCHO) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c28066831.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c28066831.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
