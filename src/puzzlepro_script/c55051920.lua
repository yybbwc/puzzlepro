...
function c55051920.costfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c55051920.tgfilter, tp, LOCATION_ONFIELD, 0, 1, c)
end
...
function c55051920.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55051920.costfilter, tp, LOCATION_GRAVE + LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c55051920.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c55051920.tgfilter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c55051920.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xfe, 0x11b) and c:IsControler(tp) and c:IsOnField() and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c55051920.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c55051920.repfilter, 1, e:GetHandler(), tp)
  end
