...
function s.cfilter(c)
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsLocation(LOCATION_MZONE) and (c:GetPreviousSequence() ~= c:GetSequence() or c:GetPreviousControler() ~= c:GetControler())
end
...
function s.mvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsStatus(STATUS_EFFECT_ENABLED) and c:IsAbleToGraveAsCost()
  end
...
function s.mvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and s.mfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.mfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
