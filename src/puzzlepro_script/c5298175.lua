...
function c5298175.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c5298175.rmfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5298175.rmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
  end
