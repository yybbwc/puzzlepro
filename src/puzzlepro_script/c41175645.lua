...
function c41175645.spfilter(c, tp)
  return c:IsRace(RACE_SPELLCASTER) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c41175645.condition(e, tp, eg, ep, ev, re, r, rp)
  return re:IsActiveType(TYPE_SPELL) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev) and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED)
end
...
function c41175645.spcon2(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (c:IsReason(REASON_BATTLE) or (rp == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousControler(tp))) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
function c41175645.filter(c, e, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xe8) and not c:IsCode(41175645) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false)
end
function c41175645.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c41175645.filter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
