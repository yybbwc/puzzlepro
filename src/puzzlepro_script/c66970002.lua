...
function c66970002.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c66970002.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c66970002.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c66970002.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) then
    ...
    if atk < 0 or tc:IsFacedown() then
      atk = 0
    end
