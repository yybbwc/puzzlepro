...
function c89238128.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c89238128.atkfilter(chkc, lg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c89238128.atkfilter, tp, LOCATION_MZONE, 0, 1, nil, lg)
  end
...
function c89238128.extg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local lg = e:GetHandler():GetLinkedGroup()
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c89238128.exfilter(chkc, lg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c89238128.exfilter, tp, LOCATION_MZONE, 0, 1, nil, lg)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
