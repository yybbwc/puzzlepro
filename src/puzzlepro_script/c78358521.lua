...
function c78358521.rmfilter(c, tp)
  return c:IsSetCard(0x53) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c78358521.filter, tp, LOCATION_GRAVE, 0, 1, c)
end
...
function c78358521.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c78358521.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c78358521.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c78358521.filter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c78358521.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c78358521.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
...
function c78358521.decop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if Duel.GetFlagEffect(tp, 78358522) ~= 0 then
    return
  end
  local e1 = Effect.CreateEffect(c)
