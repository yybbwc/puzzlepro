...
function c49221191.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_COST) and Duel.IsExistingMatchingCard(c49221191.rfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c49221191.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c49221191.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c49221191.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
