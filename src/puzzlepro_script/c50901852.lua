...
function c50901852.spfilter(c, e, tp)
  return aux.AtkEqualsDef(c) and c:IsRace(RACE_MACHINE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c50901852.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c50901852.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c50901852.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c50901852.cfilter(chkc, atk)
  end
  if chk == 0 then
    return g:GetCount() > 0 and Duel.IsExistingTarget(c50901852.cfilter, tp, LOCATION_MZONE, 0, 1, nil, atk)
  end
