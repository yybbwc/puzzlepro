...
function c46005939.spfilter1(c, e, tp)
  return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c46005939.spfilter2, tp, LOCATION_PZONE, 0, 1, c, e, tp, c)
end
...
function c46005939.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c46005939.spfilter1, tp, LOCATION_PZONE, 0, 1, nil, e, tp)
  end
