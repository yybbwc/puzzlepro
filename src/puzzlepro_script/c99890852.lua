...
function c99890852.filter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xfb) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
function c99890852.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c99890852.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
