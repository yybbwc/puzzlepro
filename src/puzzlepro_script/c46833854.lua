...
function c46833854.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(c46833854.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c46833854.tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c46833854.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46833854.filter, tp, 0, LOCATION_ONFIELD, 2, nil)
  end
