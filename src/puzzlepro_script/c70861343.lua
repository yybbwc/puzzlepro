...
function c70861343.cfilter(c, e, tp, ft)
  local lv = c:GetLevel()
  return c:IsFaceup() and lv > 0 and c:IsSetCard(0x2b) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and Duel.IsExistingMatchingCard(c70861343.filter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, lv + 3, e, tp)
end
...
function c70861343.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c70861343.cfilter, 1, nil, e, tp, ft)
  end
