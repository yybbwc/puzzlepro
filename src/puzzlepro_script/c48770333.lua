...
function c48770333.spcon2(e, c)
  ...
  local tp = c:GetControler()
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c48770333.cfilter, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function c48770333.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 1, 0x37, 3, REASON_COST)
  end
