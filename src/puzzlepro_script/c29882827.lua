...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if Duel.IsExistingMatchingCard(s.exfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
...
function s.cfilter(c, e, tp)
  return c:IsSetCard(0x1c0) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetOriginalRace())
end
function s.spfilter(c, e, tp, race)
  return c:IsSetCard(0x1c0) and c:GetOriginalRace() == race and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function s.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetLabel() == 100 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
