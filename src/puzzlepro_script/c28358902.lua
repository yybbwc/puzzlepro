...
function c28358902.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c28358902.filter(chkc, tid)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c28358902.filter, tp, LOCATION_GRAVE, 0, 1, nil, tid)
  end
