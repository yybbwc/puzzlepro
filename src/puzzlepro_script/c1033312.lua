...
function c1033312.tgfilter(c, tp)
  return c:IsLevelBelow(4) and c:IsType(TYPE_NORMAL) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(c1033312.thfilter, tp, LOCATION_DECK, 0, 1, c, c:GetCode())
end
...
function c1033312.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c1033312.tgfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
