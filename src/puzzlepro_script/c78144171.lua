...
function c78144171.ovfilter(c)
  if Duel.GetMatchingGroupCount(Card.IsAttribute, c:GetControler(), LOCATION_GRAVE, 0, nil, ATTRIBUTE_DARK) ~= 5 then
    return false
  end
...
function c78144171.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 78144171) == 0
  end
...
function c78144171.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_GRAVE, 0, 1, nil)
  end
