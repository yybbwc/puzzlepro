...
function c57470761.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c57470761.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c57470761.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c57470761.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsRace(RACE_DRAGON)
end
