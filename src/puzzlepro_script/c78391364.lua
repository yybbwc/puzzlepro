...
function c78391364.spcon(e, c)
  ...
  return Duel.GetFieldGroupCount(c:GetControler(), LOCATION_MZONE, 0) == 0 and Duel.GetFieldGroupCount(c:GetControler(), 0, LOCATION_MZONE) > 1 and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0
end
...
function c78391364.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c78391364.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c78391364.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
