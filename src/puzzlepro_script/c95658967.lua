...
function c95658967.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95658967.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
  Duel.DiscardHand(tp, c95658967.cfilter, 1, 1, REASON_COST + REASON_DISCARD)
end
function c95658967.thfilter(c)
  return ((c:IsType(TYPE_MONSTER) and c:IsAttribute(ATTRIBUTE_LIGHT)) or c:IsType(TYPE_SPELL)) and c:IsType(TYPE_RITUAL) and c:IsAbleToHand()
end
function c95658967.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95658967.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c95658967.spfilter(c, e, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_FAIRY) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c95658967.cfilter2, tp, LOCATION_GRAVE, 0, lv, nil)
end
...
function c95658967.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c95658967.spfilter2(chkc, e, tp, e:GetLabel())
  end
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c95658967.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  e:SetLabel(0)
