...
function c57108202.filtera(c, tp)
  local lv = c:GetLevel()
  return c:IsSetCard(0x26) and lv > 0 and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c57108202.filter, tp, LOCATION_DECK, 0, 1, nil, lv)
end
function c57108202.tga(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c57108202.filtera(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57108202.filtera, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c57108202.filterd(c, tp)
  local lv = c:GetLevel()
  return c:IsSetCard(0x26) and lv > 0 and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(c57108202.filter, tp, LOCATION_GRAVE, 0, 1, nil, lv)
end
function c57108202.tgd(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c57108202.filterd, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
