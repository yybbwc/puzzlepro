...
function c27483935.costfilter(c, tp)
  return c:IsCode(83104731) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c27483935.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, c)
end
function c27483935.scost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27483935.costfilter, tp, LOCATION_DECK + LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil, tp)
  end
...
function c27483935.stg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27483935.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
...
function c27483935.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c27483935.filter1, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c27483935.pfilter(c, tp)
  return c:IsType(TYPE_CONTINUOUS) and c:IsType(TYPE_TRAP) and c:IsSetCard(0x7) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
function c27483935.tftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c27483935.pfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
end
