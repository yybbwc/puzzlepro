...
function c92067220.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(nil, tp, LOCATION_HAND, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c92067220.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c92067220.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
  ...
  if Duel.DiscardHand(tp, nil, 1, 1, REASON_EFFECT + REASON_DISCARD, nil) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
    ...
    if g1:GetCount() > 0 and Duel.SpecialSummon(g1, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
      ...
      if g2:GetCount() > 0 then
        ...
        if Duel.IsExistingMatchingCard(Card.IsSummonable, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, true, nil) and Duel.SelectYesNo(tp, aux.Stringid(92067220, 1)) then
          Duel.BreakEffect()
          Duel.ShuffleHand(tp)
...
function c92067220.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92067220.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c92067220.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil) and e:GetHandler():IsAbleToHand()
  end
