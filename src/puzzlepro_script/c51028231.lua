...
function c51028231.filter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)) and c:IsSetCard(0x10ec) and not c:IsReason(REASON_REPLACE)
end
function c51028231.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return eg:IsExists(c51028231.filter, 1, nil, tp) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
  end
...
function c51028231.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c51028231.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c51028231.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c51028231.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51028231.desfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
