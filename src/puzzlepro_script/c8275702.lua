...
function c8275702.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c8275702.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8275702.filter, tp, LOCATION_SZONE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
