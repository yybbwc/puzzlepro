...
function c5969957.spcfilter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsSummonLocation(LOCATION_EXTRA) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c5969957.tgfilter(c, e, tp)
  return c5969957.spcfilter(c, tp) and c:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and c:IsCanBeEffectTarget(e) and Duel.IsExistingMatchingCard(c5969957.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp, c)
end
function c5969957.spfilter(c, e, tp, tc)
  return c:IsAttribute(ATTRIBUTE_DARK) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and not c:IsOriginalCodeRule(tc:GetOriginalCodeRule())
end
