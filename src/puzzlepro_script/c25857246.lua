...
function c25857246.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsDiscardable() and Duel.IsExistingMatchingCard(c25857246.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c25857246.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.CheckReleaseGroupEx(tp, nil, 1, REASON_EFFECT, true, nil)
  end
