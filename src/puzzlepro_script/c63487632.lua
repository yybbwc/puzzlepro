...
function c63487632.spfilter(c, tp)
  return c:GetEquipGroup():IsExists(Card.IsSetCard, 1, nil, 0x29) and c:IsAbleToRemoveAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c63487632.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c63487632.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c63487632.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c63487632.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return g:IsContains(chkc) and c63487632.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:IsExists(c63487632.spfilter2, 1, nil, e, tp)
  end
