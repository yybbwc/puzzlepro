...
function c45221020.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return ct > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c45221020.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp, ct)
  end
  ...
  local pc = 1
  for i = 1, 3 do
    if tg:IsExists(c45221020.sfilter, 1, nil, i, e, tp) then
      lvt[pc] = i
      pc = pc + 1
    end
  end
