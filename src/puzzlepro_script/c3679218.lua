...
function c3679218.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c3679218.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c3679218.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and e:GetLabel() == 1 and Duel.IsPlayerCanDraw(tp, 1) and Duel.SelectYesNo(tp, aux.Stringid(3679218, 1)) then
    Duel.BreakEffect()
    Duel.ShuffleDeck(tp)
