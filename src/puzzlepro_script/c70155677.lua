...
function c70155677.mvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(70155677, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c70155677.mvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c70155677.mvfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, tp)
  end
end
function c70155677.mvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g == 0 or Duel.GetLocationCount(tp, LOCATION_SZONE) <= 0 then
    return
  end
