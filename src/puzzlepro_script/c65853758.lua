...
function s.cfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_BEAST) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_SYNCHRO) and c:IsSummonType(SUMMON_TYPE_SYNCHRO) and c:IsSummonPlayer(tp)
end
...
function s.ctfilter2(c)
  local tp = c:GetControler()
  return c:IsFaceup() and c:IsSetCard(0x1ca) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function s.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.ctfilter1, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(s.ctfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
