...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > ft and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil)
  end
end
...
function s.cfilter(c, tp, se)
  return c:IsFaceup() and c:IsSetCard(0x1a2) and c:IsType(TYPE_SYNCHRO) and c:IsControler(tp) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function s.pttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(s.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOFIELD)
