...
function s.filter(c, e, tp, mg)
  ...
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x1bc) and c:CheckFusionMaterial() and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and mg:CheckSubGroup(s.gcheck, ct, ct, tp, c)
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_FMATERIAL) and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, g)
  end
...
function s.tdfilter(c)
  return c:IsFaceupEx() and c:IsType(TYPE_MONSTER) and c:IsAbleToDeck() and c:IsSetCard(0x1bc)
end
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and s.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.tdfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil, tp) and c:IsAbleToHand()
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and aux.NecroValleyFilter()(tc) and tc:IsType(TYPE_MONSTER) and Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_DECK + LOCATION_EXTRA) and c:IsRelateToEffect(e) then
