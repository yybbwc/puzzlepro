...
function c68159562.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.NegateAnyFilter, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingTarget(c68159562.cfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c68159562.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c68159562.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68159562.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c68159562.matfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
