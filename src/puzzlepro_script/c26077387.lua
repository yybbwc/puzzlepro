...
function c26077387.spfilter1(c, e, tp, ec)
  return c:IsSetCard(0x1115) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, ec, c, 0x60) > 0
end
function c26077387.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c26077387.spfilter1, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, e:GetHandler())
  end
...
function c26077387.cfilter(c, tp, rp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousTypeOnField(), TYPE_LINK) ~= 0 and c:IsPreviousSetCard(0x1115) and (c:IsReason(REASON_BATTLE) or (rp == 1 - tp and c:IsReason(REASON_EFFECT)))
end
