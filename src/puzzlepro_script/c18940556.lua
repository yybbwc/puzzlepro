...
function c18940556.sprcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c18940556.sprfilter, tp, LOCATION_GRAVE, 0, 2, nil)
end
...
function c18940556.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, TYPE_MONSTER) and Duel.IsExistingMatchingCard(c18940556.posfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c18940556.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetAttacker() == e:GetHandler() and d and d:IsDefensePos()
  end
