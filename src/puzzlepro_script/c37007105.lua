...
function c37007105.filter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_LINK) and Duel.IsExistingMatchingCard(c37007105.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil, c:GetLink())
end
...
function c37007105.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c37007105.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37007105.filter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
...
function c37007105.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local rc = g:GetFirst()
  if rc and Duel.Remove(rc, 0, REASON_EFFECT) ~= 0 and rc:IsLocation(LOCATION_REMOVED) and Duel.Destroy(tc, REASON_EFFECT) ~= 0 and bit.band(rc:GetOriginalRace(), RACE_CYBERSE) > 0 then
