...
function c73575650.tgfilter1(c, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(c73575650.thfilter1, tp, LOCATION_DECK, 0, 1, nil, c:GetAttribute(), c:GetCode())
end
...
function c73575650.tgfilter2(c, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_FLIP) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(c73575650.thfilter2, tp, LOCATION_DECK, 0, 1, nil, c:GetAttribute(), c:GetOriginalLevel())
end
