...
function c18210764.tefilter(c)
  return (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsType(TYPE_PENDULUM) and not c:IsCode(18210764)
end
function c18210764.tetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c18210764.tefilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c18210764.tefilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
...
function c18210764.spfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:IsLevel(1) and not c:IsCode(18210764) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c18210764.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c18210764.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
