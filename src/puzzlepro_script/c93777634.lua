...
function s.spfilter(c, e, tp)
  return c:IsSetCard(0x48) and c:IsType(TYPE_XYZ) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
...
function s.spop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) > 0 and c:IsRelateToEffect(e) and #g > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
    Duel.BreakEffect()
    Duel.Overlay(tc, g)
  end
end
