...
function c30929786.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30929786.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c30929786.setcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if Duel.IsExistingMatchingCard(c30929786.filter2, tp, LOCATION_DECK, 0, 1, nil) then
    ft = ft + 1
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30929786.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) or (Duel.IsPlayerAffectedByEffect(tp, 46241344) and ft > 0)
  end
  if Duel.IsExistingMatchingCard(c30929786.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and (not Duel.IsPlayerAffectedByEffect(tp, 46241344) or ft <= 0 or not Duel.SelectYesNo(tp, aux.Stringid(46241344, 0))) then
...
function c30929786.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30929786.filter, tp, LOCATION_DECK, 0, 1, nil, true)
  end
end
