...
function c65877963.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0 and Duel.IsExistingMatchingCard(c65877963.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c65877963.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c65877963.cfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
