...
function c86154370.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86154370.gyfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c86154370.spcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO) and Duel.GetTurnPlayer() ~= tp and (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2)
end
...
function c86154370.spfilter(c, e, tp, mc)
  return c:IsLevel(7) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c86154370.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86154370.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, e:GetHandler())
  end
...
function c86154370.gcheck(g, ft1, ft2)
  return aux.dncheck(g) and #g <= ft1 and g:FilterCount(c86154370.exfilter, nil) <= ft2
end
