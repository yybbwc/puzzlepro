...
function c9156135.addct(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c9156135.filter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c9156135.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c9156135.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
