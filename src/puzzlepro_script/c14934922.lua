...
function c14934922.costfilter(c, mc, tp)
  return c:IsSetCard(0x119) and c:IsType(TYPE_MONSTER) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsAbleToGraveAsCost() and Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, Group.FromCards(c, mc))
end
...
function c14934922.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return e:GetLabel() == 0 and chkc:IsOnField() and chkc ~= e:GetHandler() or e:GetLabel() == 1 and chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c14934922.filter(chkc)
  end
  local b1 = Duel.IsExistingMatchingCard(c14934922.costfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, e:GetHandler(), tp)
  local b2 = Duel.IsExistingTarget(c14934922.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(aux.TRUE, tp, 0, LOCATION_ONFIELD, 1, nil)
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
