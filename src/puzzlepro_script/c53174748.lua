...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    ...
    return ft > 0 and Duel.IsExistingTarget(s.filter, tp, 0, LOCATION_MZONE, 1, nil, tp) and Duel.IsExistingMatchingCard(s.eqfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 then
    ...
    if sc and Duel.Equip(tp, tc, sc) then
      ...
      if Duel.IsExistingMatchingCard(s.dafilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingMatchingCard(s.dafilter2, tp, LOCATION_SZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
        Duel.BreakEffect()
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
