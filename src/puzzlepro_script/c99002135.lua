...
function c99002135.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99002135.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c99002135.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c99002135.spfilter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, e, tp)
  end
