...
function c47077697.cfilter(c, tp)
  return c:IsRace(RACE_REPTILE) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
function c47077697.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c47077697.cfilter, 1, nil, tp)
  end
...
function c47077697.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c47077697.rfilter, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_RELEASE)
...
function c47077697.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c47077697.adfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
end
