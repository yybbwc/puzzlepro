...
function c94145683.filter1(c, e, tp, ft)
  local lv = c:GetLevel()
  return lv > 0 and c:IsFaceup() and c:IsRace(RACE_WINDBEAST) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c94145683.filter2, tp, LOCATION_DECK, 0, 1, nil, lv, e, tp)
end
