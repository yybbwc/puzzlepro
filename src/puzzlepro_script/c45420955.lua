...
function c45420955.negtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and aux.NegateEffectMonsterFilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c45420955.dcfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c45420955.econ(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return eg:IsExists(c45420955.cfilter, 1, nil, tp) and (c:GetFlagEffect(45420955) == 0 or bit.band(c:GetFlagEffectLabel(45420955), 0x7) ~= 0x7 or not c:IsHasEffect(EFFECT_INDESTRUCTABLE_BATTLE) or not c:IsHasEffect(EFFECT_INDESTRUCTABLE_EFFECT) or not c:IsHasEffect(EFFECT_CANNOT_BE_EFFECT_TARGET))
end
