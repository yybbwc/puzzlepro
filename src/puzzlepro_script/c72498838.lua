...
function c72498838.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    local sel = 0
    if Duel.IsExistingMatchingCard(c72498838.thfilter, tp, LOCATION_DECK, 0, 1, nil) then
      sel = sel + 1
    end
    if Duel.IsExistingMatchingCard(c72498838.rtfilter, tp, LOCATION_REMOVED, 0, 1, nil) then
      sel = sel + 2
    end
    e:SetLabel(sel)
    return sel ~= 0
...
function c72498838.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c72498838.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c72498838.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c72498838.eqfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
