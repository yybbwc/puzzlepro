...
function s.spfilter(c, e, tp)
  return not c:IsCode(id) and (c:IsLevel(4) or c:IsRank(4)) and c:IsRace(RACE_PSYCHO) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function s.codetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.codefilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.codefilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
  end
