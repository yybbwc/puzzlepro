...
function c38848158.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and chkc ~= c and c38848158.desfilter1(chkc)
  end
  if chk == 0 then
    ...
    e:SetLabel(loc)
    return Duel.IsExistingTarget(c38848158.desfilter1, tp, loc, 0, 1, c) and Duel.IsExistingMatchingCard(c38848158.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
