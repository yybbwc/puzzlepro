...
function c70980824.spfilter(c, e, tp)
  if not (c:IsCode(52687916) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)) then
    return false
  end
...
function c70980824.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c70980824.spfilter, tp, LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
