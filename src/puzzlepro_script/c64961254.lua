...
function c64961254.thcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) > 0 and not Duel.IsExistingMatchingCard(c64961254.thcfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c64961254.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 and c64961254.thcon(e, tp, eg, ep, ev, re, r, rp) and Duel.SelectYesNo(tp, aux.Stringid(64961254, 0)) then
...
function c64961254.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if opt == 0 and tc:IsType(TYPE_MONSTER) and tc:IsCanBeSpecialSummoned(e, 0, 1 - tp, false, false, POS_FACEDOWN_DEFENSE, 1 - tp) then
