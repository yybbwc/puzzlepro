...
function s.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED) and tg and tg:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
  end
...
function s.ffilter(c, e, tp, qc)
  return c:IsType(TYPE_FUSION) and (c:IsCode(45231177) or aux.IsCodeListed(c, 45231177)) and c:CheckFusionMaterial() and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, qc, c) > 0
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  local qc = c:GetEquipTarget()
  if chk == 0 then
    return qc and aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_FMATERIAL) and Duel.IsExistingMatchingCard(s.ffilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, qc) and c:GetControler() == qc:GetControler() and qc:IsType(TYPE_FUSION) and qc:IsAbleToGrave() and c:IsAbleToGrave()
  end
