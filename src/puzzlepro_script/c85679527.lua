...
function c85679527.sprop(e, tp, eg, ep, ev, re, r, rp, c)
  ...
  if not rc:IsType(TYPE_MONSTER) or lv <= 0 then
    return
  end
  local e1 = Effect.CreateEffect(c)
...
function c85679527.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c85679527.costfilter, 1, nil, tp)
  end
...
function c85679527.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85679527.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
