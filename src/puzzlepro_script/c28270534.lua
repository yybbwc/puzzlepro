...
function c28270534.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c28270534.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c28270534.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c28270534.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x135) and c:IsAttackAbove(2300) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c28270534.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c28270534.repfilter, 1, nil, tp)
  end
