...
function c84274024.valcheck(e, c)
  ...
  if c:IsType(TYPE_FUSION) and g:IsExists(Card.IsFusionType, 1, nil, TYPE_PENDULUM) or c:IsType(TYPE_SYNCHRO) and g:IsExists(Card.IsSynchroType, 1, nil, TYPE_PENDULUM) or c:IsType(TYPE_XYZ) and g:IsExists(Card.IsXyzType, 1, nil, TYPE_PENDULUM) then
...
function c84274024.effcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return e:GetHandler():IsStatus(STATUS_EFFECT_ENABLED) and eg:GetCount() == 1 and ec:IsSummonPlayer(tp) and ec:IsFaceup() and ec:IsType(e:GetLabel()) and ec:GetFlagEffect(84274024) ~= 0
end
...
function c84274024.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c84274024.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, eg:GetFirst():GetOriginalLevel()) and Duel.GetFlagEffect(tp, 84274024) == 0
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_DECK)
end
function c84274024.spop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 84274024) ~= 0 then
    return
  end
...
function c84274024.thtg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84274024.thfilter1, tp, LOCATION_DECK, 0, 1, nil) and Duel.GetFlagEffect(tp, 84274025) == 0
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, tp, LOCATION_DECK)
end
function c84274024.thop1(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 84274025) ~= 0 then
    return
  end
...
function c84274024.thtg2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84274024.thfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, ft, eg:GetFirst():GetRank()) and Duel.GetFlagEffect(tp, 84274026) == 0
  end
end
function c84274024.thop2(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 84274026) ~= 0 or eg:GetFirst():IsFacedown() then
    return
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
