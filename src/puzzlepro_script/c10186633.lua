...
function c10186633.tdfilter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x3008, 0x1f) and c:IsType(TYPE_MONSTER) and c:IsAbleToDeck() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c10186633.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c10186633.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c10186633.tdfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c10186633.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_DECK + LOCATION_EXTRA) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
...
function c10186633.drop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sg then
    ...
    if Duel.SendtoDeck(sg, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 then
      ...
      if not og:IsExists(Card.IsLocation, 1, nil, LOCATION_DECK + LOCATION_EXTRA) then
        return
      end
      Duel.ShuffleDeck(tp)
      Duel.BreakEffect()
