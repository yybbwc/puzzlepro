...
function c3429238.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0 and e:GetHandler():IsAbleToRemove()
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c3429238.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(3429238) ~= 0
  end
...
function c3429238.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if aux.NecroValleyNegateCheck(tg) then
    return
  end
  local c = e:GetHandler()
