...
function c58990631.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58990631.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c58990631.filter(c, tp, ep)
  return c:IsFaceup() and c:IsAttackAbove(1000) and ep ~= tp
end
function c58990631.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local tc = eg:GetFirst()
  if chk == 0 then
    return c58990631.filter(tc, tp, ep)
  end
...
function c58990631.target2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c58990631.filter2, 1, nil, tp)
  end
...
function c58990631.filter3(c, e, tp)
  return c:IsFaceup() and c:IsAttackAbove(1000) and c:IsSummonPlayer(1 - tp) and c:IsRelateToEffect(e)
end
