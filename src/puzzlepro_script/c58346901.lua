...
function c58346901.cfilter(c, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and (c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_FIRE) or (c:GetType() & (TYPE_EQUIP + TYPE_SPELL)) == TYPE_EQUIP + TYPE_SPELL) and c:IsAbleToGraveAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
function c58346901.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58346901.cfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, e:GetHandler(), tp)
  end
...
function c58346901.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c58346901.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c58346901.eqfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
