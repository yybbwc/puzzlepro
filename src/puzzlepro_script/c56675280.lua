...
function c56675280.sctg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetLeftScale() ~= 7
  end
end
function c56675280.scop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not c:IsRelateToEffect(e) or c:GetLeftScale() == 7 then
    return
  end
  local e1 = Effect.CreateEffect(c)
...
function c56675280.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c56675280.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
