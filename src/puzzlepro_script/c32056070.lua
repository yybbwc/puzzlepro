...
function c32056070.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 and not eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_ALL, 0) then
    return false
  end
  ...
  local b3 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 11738490, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_DARK)
  ...
  local att = 0
  if b1 and eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_EARTH, used) then
    att = att | ATTRIBUTE_EARTH
  end
  if b1 and eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_WATER, used) then
    att = att | ATTRIBUTE_WATER
  end
  if b2 and eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_WIND, used) then
    att = att | ATTRIBUTE_WIND
  end
  if b2 and eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_LIGHT, used) then
    att = att | ATTRIBUTE_LIGHT
  end
  if b3 and eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_FIRE, used) then
    att = att | ATTRIBUTE_FIRE
  end
  if b3 and eg:IsExists(c32056070.filter, 1, nil, ATTRIBUTE_DARK, used) then
...
function c32056070.attrop3(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 11738490, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_DARK) then
    return
  end
