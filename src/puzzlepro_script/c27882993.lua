...
function c27882993.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c27882993.spfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c27882993.toktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 27882994, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FISH, ATTRIBUTE_WATER)
  end
...
function c27882993.tokop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 27882994, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FISH, ATTRIBUTE_WATER) then
    return
  end
  repeat
