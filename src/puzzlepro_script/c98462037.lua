...
function c98462037.desop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) > 0 and c:IsRelateToEffect(e) and Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_GRAVE, 0, nil, TYPE_SPELL) <= 3 then
    Duel.BreakEffect()
...
function c98462037.descost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98462037.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
