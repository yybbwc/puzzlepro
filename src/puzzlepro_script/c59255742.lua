...
function c59255742.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToBattle() or c:IsFacedown() then
    return false
  end
...
function c59255742.spcon2(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and rc:IsControler(tp) and rc:GetOwner() ~= tp and tc:IsType(TYPE_MONSTER) and tc:IsReason(REASON_BATTLE) and tc:IsLocation(LOCATION_GRAVE)
end
function c59255742.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local bc = e:GetHandler():GetBattleTarget()
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and bc:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c59255742.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c59255742.costfilter, 1, nil, tp, g)
  end
...
function c59255742.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local g = eg:Filter(c59255742.filter, nil, tp)
  if chk == 0 then
    return g:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) >= g:GetCount() - 1
  end
...
function c59255742.cttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c59255742.ctfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
