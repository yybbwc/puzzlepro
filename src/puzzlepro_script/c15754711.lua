...
function c15754711.filter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x133) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c15754711.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetCode())
end
...
function c15754711.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c15754711.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c15754711.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
