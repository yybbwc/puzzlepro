...
function c66401502.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetFlagEffect(tp, 66401502) == 0
  end
...
function c66401502.indescost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c66401502.indescfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c66401502.indestg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 66401502) == 0
  end
  Duel.RegisterFlagEffect(tp, 66401502, RESET_CHAIN, 0, 1)
end
...
function c66401502.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and Duel.GetFlagEffect(tp, 66401502) == 0
  end
  Duel.RegisterFlagEffect(tp, 66401502, RESET_CHAIN, 0, 1)
