...
function c191749.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c191749.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, 74825788) and Duel.IsExistingMatchingCard(c191749.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, 213326) and Duel.IsExistingMatchingCard(c191749.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, 37318031) and Duel.IsExistingMatchingCard(c191749.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, 63703130)
  end
...
function c191749.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c191749.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
