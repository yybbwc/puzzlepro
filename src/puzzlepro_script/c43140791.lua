...
function c43140791.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(43140791, tp, ACTIVITY_SUMMON) == 0 and Duel.GetCustomActivityCount(43140791, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c43140791.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 43140792, 0x3e, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_INSECT, ATTRIBUTE_EARTH)
  end
...
function c43140791.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 43140792, 0x3e, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_INSECT, ATTRIBUTE_EARTH) then
    for i = 1, 2 do
