...
function c95134948.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 2, REASON_COST) and Duel.GetCustomActivityCount(95134948, tp, ACTIVITY_SPSUMMON) == 0 and Duel.GetFlagEffect(tp, 95134949) == 0 and (Duel.GetFlagEffect(tp, 95134948) == 0 or c:GetFlagEffect(95134948) ~= 0)
  end
  --splimit
  local e1 = Effect.CreateEffect(c)
...
function c95134948.spfilter(c, e, tp)
  ...
  return no and no >= 1 and no <= 100 and c:IsSetCard(0x48) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c95134948.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c95134948.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
