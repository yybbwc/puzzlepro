...
function c10712320.filter1(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x30) and c:GetEquipTarget() and Duel.IsExistingTarget(c10712320.filter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c:GetEquipTarget(), c)
end
...
function c10712320.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c10712320.filter1, tp, LOCATION_SZONE, 0, 1, nil, tp)
  end
...
function c10712320.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not eqc:IsRelateToEffect(e) then
    return
  end
