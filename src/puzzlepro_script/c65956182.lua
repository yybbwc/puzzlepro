...
function c65956182.filter2(c, e, tp, mg1, dm, f, chkf)
  ...
  return c:IsType(TYPE_FUSION) and c:IsRace(RACE_FIEND) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(mg, nil, chkf)
end
...
function c65956182.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToHand() and Duel.IsExistingMatchingCard(c65956182.thfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
