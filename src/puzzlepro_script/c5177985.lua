...
function c5177985.cfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xd9) and Duel.IsExistingMatchingCard(c5177985.cfilter2, tp, LOCATION_MZONE, 0, 1, nil, c:GetCode())
end
...
function c5177985.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c5177985.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5177985.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
