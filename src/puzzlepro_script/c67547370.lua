...
function c67547370.cfilter(c, tp)
  return c:IsFaceup() and c:GetCounter(0x1039) == 10 and Duel.GetMZoneCount(tp, c) > 0
end
...
function c67547370.counttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c67547370.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67547370.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
