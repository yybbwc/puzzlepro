...
function c93437091.thfilter(c)
  return (((aux.IsCodeListed(c, 89631139) or aux.IsCodeListed(c, 23995346)) and not c:IsCode(93437091) and c:IsType(TYPE_SPELL + TYPE_TRAP)) or (c:IsSetCard(0xdd) and c:IsType(TYPE_MONSTER))) and c:IsAbleToHand()
end
function c93437091.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c93437091.thfilter, tp, LOCATION_DECK, 0, 3, nil)
  end
