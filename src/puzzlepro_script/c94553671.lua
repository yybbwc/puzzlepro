...
function c94553671.srtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c94553671.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c94553671.tdfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil) and Duel.IsExistingMatchingCard(c94553671.srfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c94553671.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c94553671.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c94553671.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
