...
function c78990927.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local lv = c:GetLevel()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c78990927.thfilter(chkc, lv)
  end
  if chk == 0 then
    return c:IsLevelAbove(1) and c:IsRelateToEffect(e) and Duel.IsExistingTarget(c78990927.thfilter, tp, LOCATION_GRAVE, 0, 1, nil, lv)
  end
