...
function c32696942.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_COST) and re:IsActivated() and re:IsActiveType(TYPE_MONSTER) and c:IsPreviousLocation(LOCATION_OVERLAY)
end
...
function c32696942.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c32696942.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
