...
function c62953041.hspcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c62953041.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
function c62953041.spreg(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if bit.band(r, 0x41) ~= 0x41 or not c:IsPreviousPosition(POS_FACEUP) or not c:IsPreviousLocation(LOCATION_ONFIELD) then
    return
  end
