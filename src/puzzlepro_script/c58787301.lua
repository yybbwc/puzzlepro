...
function c58787301.cfilter(c, e, tp)
  return c:IsFaceup() and c:GetLevel() > 0 and Duel.IsExistingMatchingCard(c58787301.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetLevel())
end
...
function c58787301.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c58787301.cfilter, tp, 0, LOCATION_MZONE, 1, nil, e, tp)
  end
