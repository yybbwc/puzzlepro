...
function c21830679.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 21830680, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_WIND)
  end
...
function c21830679.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and c:IsFaceup() and not c:IsImmuneToEffect(e) then
    ...
    c:RegisterEffect(e1)
    if not c:IsHasEffect(EFFECT_REVERSE_UPDATE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 21830680, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_CYBERSE, ATTRIBUTE_WIND) then
