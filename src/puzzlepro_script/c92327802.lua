...
function c92327802.damval(e, re, val, r, rp, rc)
  ...
  if bit.band(r, REASON_EFFECT) ~= 0 and rp == 1 - e:GetOwnerPlayer() and c:IsCanAddCounter(0x43, 1) and c:GetFlagEffect(92327802) == 0 then
    c:AddCounter(0x43, 1)
...
function c92327802.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x43, 1, REASON_COST)
  end
...
function c92327802.tg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c92327802.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c92327802.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
