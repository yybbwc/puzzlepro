...
function c45283341.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c45283341.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c45283341.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c45283341.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c45283341.ctfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45283341.ctfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c45283341.ctop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.GetControl(tc, tp) ~= 0 then
    ...
    if g:GetCount() > 0 then
      atk = g:GetSum(Card.GetBaseAttack)
    end
    local e1 = Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_SINGLE)
