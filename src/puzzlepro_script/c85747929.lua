...
function c85747929.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85747929.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c85747929.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.IsInGroup, tp, LOCATION_MZONE, 0, 1, nil, g)
  end
  local sg
...
function c85747929.lvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() then
    ...
    if tc:GetLevel() == 4 then
      lv = 8
    elseif tc:GetLevel() == 8 then
      lv = 4
    else
      lv = Duel.AnnounceNumber(tp, 4, 8)
    end
