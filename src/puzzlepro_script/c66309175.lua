...
function c66309175.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c66309175.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE + LOCATION_EXTRA, 0, 1, nil)
  end
...
function c66309175.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c66309175.relfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c66309175.relfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp) and Duel.IsExistingMatchingCard(c66309175.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c66309175.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) and Duel.Release(tc, REASON_EFFECT) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
