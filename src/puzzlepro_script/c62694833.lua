...
function c62694833.setfilter1(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x7c) and c:IsType(TYPE_SPELL + TYPE_TRAP) and Duel.IsExistingMatchingCard(c62694833.setfilter2, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c62694833.settg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c62694833.setfilter1(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62694833.setfilter1, tp, LOCATION_ONFIELD, 0, 1, nil, tp)
  end
...
function c62694833.setfilter3(c, tp)
  return c:IsSetCard(0x7c) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsSSetable() and not Duel.IsExistingMatchingCard(Card.IsCode, tp, LOCATION_GRAVE, 0, 1, nil, c:GetCode())
end
function c62694833.settg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62694833.setfilter3, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
