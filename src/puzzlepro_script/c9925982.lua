...
function c9925982.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 9925983, 0, TYPES_TOKEN_MONSTER, 0, 800, 2, RACE_WINDBEAST, ATTRIBUTE_DARK)
  end
...
function c9925982.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 9925983, 0, TYPES_TOKEN_MONSTER, 0, 800, 2, RACE_WINDBEAST, ATTRIBUTE_DARK) then
    for i = 1, 2 do
