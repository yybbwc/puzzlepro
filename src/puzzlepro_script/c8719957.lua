...
function c8719957.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c8719957.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8719957.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c8719957.negcon(e, tp, eg, ep, ev, re, r, rp)
  return rp == 1 - tp and Duel.GetChainInfo(ev, CHAININFO_TRIGGERING_LOCATION) == LOCATION_MZONE and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainDisablable(ev)
end
