...
function c91654806.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91654806.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c91654806.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    e:SetLabel(0)
    return res and Duel.IsExistingMatchingCard(c91654806.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c91654806.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToGrave() and Duel.IsPlayerCanDraw(tp, 1)
  end
...
function c91654806.drop(e, tp, eg, ep, ev, re, r, rp)
  if not eg:IsExists(Card.IsLocation, 1, nil, LOCATION_HAND) then
    return
  end
