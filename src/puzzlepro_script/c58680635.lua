...
function c58680635.lvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and c:IsFaceup() and not c:IsLevel(4) then
    ...
    c:RegisterEffect(e1)
    if c:IsLevel(4) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c58680635.thfilter), tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(58680635, 2)) then
      Duel.BreakEffect()
...
function c58680635.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToGrave, tp, LOCATION_ONFIELD, 0, 1, c) and Duel.IsPlayerCanDraw(tp, 1)
  end
