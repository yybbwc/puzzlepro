...
function c52566270.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52566270.costfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c52566270.spfilter(c, e, tp)
  return c:IsSetCard(0x2066, 0xe9) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and not Duel.IsExistingMatchingCard(c52566270.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil, c)
end
function c52566270.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  c:RegisterEffect(e1)
  if Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c52566270.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(52566270, 1)) then
    Duel.BreakEffect()
