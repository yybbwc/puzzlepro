...
function c84521924.spfilter(c, e, tp, zone)
  return c:IsSetCard(0x162) and c:GetCurrentScale() % 2 == 1 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, tp, zone)
end
...
function c84521924.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE, tp, LOCATION_REASON_TOFIELD, zone) <= 0 or #g == 0 then
    return
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
  local sg = g:Select(tp, 1, 1, nil)
  if #sg == 0 or Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP, zone) == 0 then
    return
  end
  if Duel.NegateActivation(ev) and Duel.IsExistingMatchingCard(c84521924.tefilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(84521924, 1)) then
    Duel.BreakEffect()
