...
function c53451824.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c53451824.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -e:GetLabel() + 1 and Duel.IsExistingTarget(c53451824.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
