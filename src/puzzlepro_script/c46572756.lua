...
function c46572756.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c46572756.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46572756.filter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
