...
function c33503878.filter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x10ec) and c:IsType(TYPE_PENDULUM) and Duel.IsExistingMatchingCard(c33503878.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c33503878.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c33503878.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c33503878.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c33503878.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc:IsRelateToEffect(e) and tc:IsFaceup()) then
    return
  end
  ...
  if g:GetCount() > 0 then
    ...
    if (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1)) and not tc:IsForbidden() and Duel.SelectOption(tp, aux.Stringid(33503878, 2), aux.Stringid(33503878, 3)) == 0 then
      Duel.BreakEffect()
...
function c33503878.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN) and Duel.IsExistingMatchingCard(c33503878.cfilter, tp, LOCATION_EXTRA, 0, 1, nil)
end
