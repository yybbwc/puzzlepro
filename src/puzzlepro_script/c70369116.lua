...
function c70369116.atttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c70369116.attfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70369116.attfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c70369116.cpfilter(c)
  return (c:GetType() == TYPE_SPELL or c:IsType(TYPE_QUICKPLAY)) and c:IsSetCard(0x46) and c:IsAbleToGraveAsCost() and c:CheckActivateEffect(true, true, false) ~= nil
end
