...
function c84792926.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c84792926.dfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c84792926.repfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c84792926.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c84792926.thfilter(chkc)
  end
  if chk == 0 then
    return eg:IsExists(c84792926.filter, 1, nil) and Duel.IsExistingTarget(c84792926.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
