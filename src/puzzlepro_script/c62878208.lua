...
function c62878208.filter1(c, e, tp)
  ...
  return ec and ec:IsControler(tp) and ec:IsPosition(POS_FACEUP_ATTACK) and Duel.IsExistingTarget(c62878208.filter2, tp, 0, LOCATION_MZONE, 1, nil, c)
end
...
function c62878208.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c62878208.filter1, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil, e, tp)
  end
...
function c62878208.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if eqc and eq:IsRelateToEffect(e) then
    if not Duel.Equip(tp, eq, tc) then
      return
    end
    Duel.BreakEffect()
    local a = eqc
