...
function c55888045.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bit.band(loc, LOCATION_ONFIELD) ~= 0 and not e:GetHandler():IsStatus(STATUS_CHAINING) and e:GetHandler():GetOverlayGroup():IsExists(Card.IsSetCard, 1, nil, 0x48)
end
...
function c55888045.negop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or Duel.GetCurrentChain() ~= ev + 1 or not c:RemoveOverlayCard(tp, 1, 1, REASON_EFFECT) then
    return
  end
