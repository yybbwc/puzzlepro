...
function c93953933.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() >= 2 and g:GetClassCount(Card.GetCode) == g:GetCount() then
    ...
    Duel.SortDecktop(tp, tp, dg:GetCount())
  end
end
...
function c93953933.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c93953933.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93953933.tdfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
