...
function s.cfilter(c)
  return c:IsFaceup() and c:GetFlagEffect(id) ~= 0 and c:IsReason(REASON_COST) and c:IsPreviousLocation(LOCATION_ONFIELD + LOCATION_HAND)
end
...
function s.actg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local b1 = Duel.IsExistingMatchingCard(s.disfiter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and g:IsExists(Card.IsType, 1, nil, TYPE_MONSTER)
  local b2 = Duel.IsPlayerCanDraw(tp, 2) and g:IsExists(Card.IsType, 1, nil, TYPE_SPELL)
  local b3 = Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and g:IsExists(Card.IsType, 1, nil, TYPE_TRAP)
  if chk == 0 then
...
function s.acop(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local g = eg:Filter(s.cfilter, 1, nil)
  local b1 = Duel.IsExistingMatchingCard(s.disfiter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and label & TYPE_MONSTER ~= 0
  local b2 = Duel.IsPlayerCanDraw(tp) and label & TYPE_SPELL ~= 0
  local b3 = Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and label & TYPE_TRAP ~= 0
  if not b1 and not b2 and not b3 then
    return
  end
  local op = aux.SelectFromOptions(tp, { b1, aux.Stringid(id, 2), 1 }, { b2, aux.Stringid(id, 3), 2 }, { b3, aux.Stringid(id, 4), 3 })
  if op == 1 then
