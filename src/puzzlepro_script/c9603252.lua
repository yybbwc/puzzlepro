...
function c9603252.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (re:IsActiveType(TYPE_MONSTER) and race & RACE_SPELLCASTER > 0 and not re:IsActiveType(TYPE_FUSION)) or re:IsActiveType(TYPE_SPELL)
end
...
function c9603252.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c9603252.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c9603252.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9603252.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
