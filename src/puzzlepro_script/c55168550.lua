...
function c55168550.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55168550.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c55168550.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c55168550.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c55168550.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c55168550.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e)) then
    return
  end
