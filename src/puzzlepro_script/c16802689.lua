...
function c16802689.hspfilter(c, tp)
  return c:IsSetCard(0xe6) and c:IsLevel(12) and not c:IsCode(16802689) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c16802689.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) ~= 0 then
    ...
    if tc:IsType(TYPE_MONSTER) and tc:IsSetCard(0xe6) then
      if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(16802689, 1)) then
