...
function c5288597.cfilter(c, e, tp)
  ...
  return bit.band(c:GetOriginalType(), TYPE_MONSTER) ~= 0 and lv > 0 and c:IsFaceup() and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c5288597.spfilter, tp, LOCATION_DECK, 0, 1, nil, lv + 1, rc, att, e, tp)
end
...
function c5288597.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c5288597.cfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
  e:SetLabel(0)
