...
function c82422049.filter2(c, e, tp)
  return c:IsSetCard(0x10) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c82422049.filter1, tp, LOCATION_GRAVE, 0, 2, c)
end
function c82422049.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c82422049.filter2, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
