...
function c10678778.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c10678778.rmfilter, tp, 0, LOCATION_EXTRA, 1, nil)
  end
...
function c10678778.filter(c, tp)
  ...
  return c:IsFaceup() and ctype ~= 0 and c:IsAbleToExtra() and Duel.IsExistingMatchingCard(c10678778.filter2, tp, 0, LOCATION_MZONE, 1, nil, ctype)
end
...
function c10678778.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(1 - tp) and c10678778.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c10678778.filter, tp, 0, LOCATION_REMOVED, 1, nil, tp)
  end
