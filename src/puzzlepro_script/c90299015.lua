...
function c90299015.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90299015.costfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
...
function c90299015.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90299015.filter1, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c90299015.filter2, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c90299015.sumcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.CheckLPCost(tp, 500)
  end
...
function c90299015.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90299015.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
