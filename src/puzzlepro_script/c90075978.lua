...
function c90075978.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_REPTILE) and c:IsLevelAbove(7) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function c90075978.setop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if dt == 0 or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 then
      return
    end
