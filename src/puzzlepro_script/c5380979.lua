...
function c5380979.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c5380979.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c5380979.setcon(e, tp, eg, ep, ev, re, r, rp)
  return eg:IsExists(c5380979.cfilter, 1, nil) and not eg:IsContains(e:GetHandler()) and rp == tp and re:IsActiveType(TYPE_TRAP) and re:GetHandler():GetOriginalType() == TYPE_TRAP and aux.exccon(e)
end
