...
function c60643553.cfilter(c, sp)
  return c:IsType(TYPE_EFFECT) and c:IsSummonPlayer(sp) and c:IsFaceup()
end
function c60643553.lpcon1(e, tp, eg, ep, ev, re, r, rp)
  local ph = Duel.GetCurrentPhase()
  return eg:IsExists(c60643553.cfilter, 1, nil, 1 - tp) and (ph == PHASE_MAIN1 or ph == PHASE_MAIN2 or (ph >= PHASE_BATTLE_START and ph <= PHASE_BATTLE)) and not Duel.IsChainSolving()
end
function c60643553.lpop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Recover(tp, rnum, REASON_EFFECT) < 1 then
    return
  end
...
function c60643553.regcon(e, tp, eg, ep, ev, re, r, rp)
  local ph = Duel.GetCurrentPhase()
  return eg:IsExists(c60643553.cfilter, 1, nil, 1 - tp) and (ph == PHASE_MAIN1 or ph == PHASE_MAIN2 or (ph >= PHASE_BATTLE_START and ph <= PHASE_BATTLE)) and Duel.IsChainSolving()
end
