...
function c47929865.thcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsLocation(LOCATION_GRAVE) and r == REASON_SYNCHRO and e:GetHandler():GetReasonCard():IsRace(RACE_SPELLCASTER)
end
...
function c47929865.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c47929865.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47929865.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
