...
function c71791814.efftg(e, tp, eg, ep, ev, re, r, rp, chk)
  local b1 = Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c71791814.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  local b2 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetMatchingGroupCount(Card.IsType, tp, 0, LOCATION_ONFIELD, nil, TYPE_SPELL + TYPE_TRAP) > 0 and Duel.IsExistingMatchingCard(c71791814.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(71791814, 0), aux.Stringid(71791814, 1))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(71791814, 0))
  else
    op = Duel.SelectOption(tp, aux.Stringid(71791814, 1)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
...
function c71791814.effop(e, tp, eg, ep, ev, re, r, rp)
  if e:GetLabel() == 0 then
    ...
    if ft <= 0 or ct <= 0 or g:GetCount() == 0 then
      return
    end
