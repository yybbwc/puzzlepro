...
function c40493210.cfilter(c, tp)
  return not c:IsType(TYPE_TOKEN) and (c:IsType(TYPE_NORMAL) or c:IsSetCard(0x165)) and not c:IsStatus(STATUS_BATTLE_DESTROYED) and Duel.GetMZoneCount(tp, c) > 0
end
function c40493210.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c40493210.cfilter, 1, nil, tp)
  end
  local g = Duel.SelectReleaseGroup(tp, c40493210.cfilter, 1, 1, nil, tp)
  Duel.Release(g, REASON_COST)
end
function c40493210.spfilter(c, e, tp)
  return (c:IsType(TYPE_NORMAL) or c:IsSetCard(0x165)) and c:IsLevelBelow(8) and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
...
function c40493210.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c40493210.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
