...
function c48372950.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.IsExistingMatchingCard(c48372950.ntfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c48372950.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c48372950.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return e:GetHandler():IsAbleToHand() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c48372950.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
