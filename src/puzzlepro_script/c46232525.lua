...
function c46232525.spfilter(c, e, tp, race, mc)
  return c:IsType(TYPE_FUSION) and c.material_race and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and race == c.material_race and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c46232525.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c46232525.tgfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, e, tp)
  end
