...
function c52467217.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52467217.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c52467217.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c52467217.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c52467217.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c52467217.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
