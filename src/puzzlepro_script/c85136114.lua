...
function c85136114.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c85136114.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c85136114.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c85136114.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c85136114.thfilter1, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) and Duel.IsExistingTarget(c85136114.thfilter2, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
