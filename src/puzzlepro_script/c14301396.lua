...
function s.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 2, REASON_COST) and c:GetFlagEffect(id) == 0
  end
...
function s.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.eqfilter(chkc) and e:GetHandler() ~= chkc
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(s.eqfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function s.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.tdfilter, tp, LOCATION_GRAVE, 0, 3, e:GetHandler())
  end
  Duel.SetOperationInfo(0, CATEGORY_TODECK, nil, 3, tp, LOCATION_GRAVE)
end
function s.tdop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetMatchingGroupCount(s.tdfilter, tp, LOCATION_GRAVE, 0, nil) < 3 then
    return
  end
