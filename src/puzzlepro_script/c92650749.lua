...
function s.cost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x25, 1, REASON_COST)
  end
...
function s.tg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function s.cost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x25, 2, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function s.tg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return true and Duel.IsExistingMatchingCard(s.filter2, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function s.cost3(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x25, 3, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function s.cost4(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x25, 4, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function s.cost5(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x25, 5, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function s.tg5(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return true and Duel.IsExistingMatchingCard(s.filter5, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
