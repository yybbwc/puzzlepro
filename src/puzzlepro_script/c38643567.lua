...
function c38643567.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c38643567.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c38643567.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c38643567.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsSetCard(0x56)
end
function c38643567.ngcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg:GetCount() ~= 1 or not tc:IsLocation(LOCATION_MZONE) or not tc:IsSetCard(0x56) then
    return false
  end
