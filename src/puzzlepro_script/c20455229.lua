...
function c20455229.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(20455229, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c20455229.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c20455229.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c20455229.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c20455229.repfilter, 1, nil, tp)
  end
