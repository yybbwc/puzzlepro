...
function c66407907.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c66407907.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c66407907.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c66407907.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c66407907.cfilter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c66407907.spfilter1(c, e, tp)
  return c:IsType(TYPE_LINK) and c:IsRace(RACE_PLANT) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c66407907.spnfilter, tp, LOCATION_GRAVE, 0, 1, c, c:GetCode())
end
...
function c66407907.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c66407907.spfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
