...
function c81344070.cfilter(c, tp)
  return c:IsAbleToGraveAsCost() and c:IsSetCard(0x13f) and c:GetOriginalType() & TYPE_MONSTER ~= 0 and (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and Duel.GetMZoneCount(tp, c) > 0
end
function c81344070.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81344070.cfilter, tp, LOCATION_ONFIELD + LOCATION_HAND, 0, 1, e:GetHandler(), tp)
  end
