...
function s.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.rfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, c)
  end
...
function s.thfilter(c, e, tp)
  if not (c:IsSetCard(0x171) and c:IsType(TYPE_MONSTER) and not c:IsLevel(8)) then
    return false
  end
...
function s.poscon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return p == 1 - tp and LOCATION_ONFIELD & loc ~= 0 and re:IsActiveType(TYPE_MONSTER) and atk >= 2500 and rc:IsRelateToEffect(re)
end
function s.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  local rc = re:GetHandler()
  if chk == 0 then
    return rc:IsCanTurnSet()
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
