...
function c75660578.costfilter(c, e, tp)
  return (c:IsSetCard(0x134) or c:IsRace(RACE_ZOMBIE)) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingTarget(c75660578.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetCode())
end
...
function c75660578.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c75660578.costfilter, 1, nil, e, tp)
  end
...
function c75660578.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c75660578.spfilter(chkc, e, tp, code)
  end
  if chk == 0 then
    return true
  end
