...
function c51369889.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    if e:GetLabel() == 100 then
      e:SetLabel(0)
      return Duel.IsExistingMatchingCard(c51369889.rmfilter, tp, LOCATION_MZONE, 0, 1, tp) and Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
    else
      return false
    end
  end
...
function c51369889.cfilter(c, e, tp)
  return (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)) and c:IsPreviousPosition(POS_FACEUP) and c:IsType(TYPE_XYZ) and c:IsRace(RACE_MACHINE) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp)
end
...
function c51369889.damcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c51369889.damfilter, tp, LOCATION_GRAVE, 0, 1, c)
  end
