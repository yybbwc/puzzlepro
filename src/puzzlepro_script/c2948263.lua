...
function c2948263.spfilter(c, tp)
  return c:IsSetCard(0x59) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c2948263.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainDisablable(ev) and tgp ~= tp and re:IsActiveType(TYPE_MONSTER) and loc == LOCATION_MZONE
end
