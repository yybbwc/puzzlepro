...
function c26570480.retcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsReason(REASON_DESTROY) and e:GetHandler():GetReasonPlayer() == 1 - tp and e:GetHandler():IsPreviousControler(tp)
end
...
function c26570480.cfilter(c, tp, ec)
  ...
  if c == ec then
    return ((np == POS_FACEUP_DEFENSE and pp == POS_FACEUP_ATTACK) or (np == POS_FACEUP_ATTACK and pp == POS_FACEUP_DEFENSE)) and c:IsControler(tp) and c:IsSetCard(0x71)
  else
    return ((np == POS_FACEUP_DEFENSE and pp == POS_FACEUP_ATTACK) or (np == POS_FACEUP_ATTACK and pp & POS_DEFENSE ~= 0)) and c:IsControler(tp) and c:IsSetCard(0x71)
  end
end
...
function c26570480.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c26570480.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c26570480.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
