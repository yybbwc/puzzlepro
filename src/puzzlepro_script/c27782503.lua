...
function c27782503.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return e:GetHandler() == Duel.GetAttacker() and d and d:IsFacedown() and d:IsDefensePos() and Duel.IsExistingMatchingCard(c27782503.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c27782503.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and c:IsOnField() and c:IsFaceup() and Duel.IsExistingMatchingCard(c27782503.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
