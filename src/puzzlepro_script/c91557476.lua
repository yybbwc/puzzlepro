...
function c91557476.cfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:IsReason(REASON_EFFECT) and bit.band(c:GetPreviousTypeOnField(), TYPE_LINK) ~= 0 and c:IsPreviousSetCard(0x2158) and c:IsPreviousLocation(LOCATION_MZONE)
end
...
function c91557476.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c91557476.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91557476.atkfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c91557476.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE, tp, LOCATION_REASON_TOFIELD, zone) > 0 and bc:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
