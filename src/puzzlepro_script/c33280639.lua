...
function c33280639.cfilter1(c, tp)
  return c:IsSetCard(0xd6) and not c:IsCode(33280639) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c33280639.cfilter2, tp, LOCATION_DECK, 0, 1, c)
end
...
function c33280639.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c33280639.cfilter1, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
...
function c33280639.filter(c, e, tp)
  return c:IsCode(11790356) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_GRAVE) and Duel.GetMZoneCount(tp) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)
end
function c33280639.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c33280639.filter, tp, LOCATION_GRAVE + LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
