...
function c11375683.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c11375683.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c11375683.thfilter(c, tp)
  return c:IsCode(59490397) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c11375683.tgfilter, tp, LOCATION_DECK, 0, 1, c)
end
...
function c11375683.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c11375683.thfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
