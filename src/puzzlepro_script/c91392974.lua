...
function c91392974.cfilter(c, tp)
  return c:IsPreviousSetCard(0x8) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
function c91392974.regcon(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c91392974.cfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c91392974.cfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c91392974.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c91392974.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c91392974.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.IsExistingMatchingCard(c91392974.atkfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(91392974, 1)) then
