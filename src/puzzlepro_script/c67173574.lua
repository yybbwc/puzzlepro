...
function c67173574.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 2, REASON_EFFECT) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
  end
...
function c67173574.damtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsRelateToEffect(e) and e:GetHandler():IsFaceup()
  end
...
function c67173574.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c67173574.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67173574.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
