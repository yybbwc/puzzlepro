...
function c7496001.actfilter(c, tp)
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:IsReason(REASON_EFFECT) and c:GetPreviousTypeOnField() & TYPE_XYZ ~= 0
end
...
function c7496001.actg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c7496001.cfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c7496001.cfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c7496001.aftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7496001.affilter, tp, LOCATION_GRAVE + LOCATION_DECK, 0, 1, nil, tp)
  end
end
