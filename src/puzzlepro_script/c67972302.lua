...
function c67972302.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67972302.sumfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c67972302.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c67972302.thfilter(chkc)
  end
  if chk == 0 then
    return true
  end
