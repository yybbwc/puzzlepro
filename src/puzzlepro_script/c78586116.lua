...
function c78586116.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c78586116.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c78586116.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c78586116.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsSetCard(0x26)
end
function c78586116.attg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local eq = e:GetHandler():GetEquipTarget()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() and chkc ~= eq
  end
  if chk == 0 then
    return eq and Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_MZONE, LOCATION_MZONE, 1, eq)
  end
