...
function c35100834.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 35100834, 0, TYPES_EFFECT_TRAP_MONSTER, 0, 2000, 4, RACE_MACHINE, ATTRIBUTE_EARTH)
  end
...
function c35100834.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 35100834, 0, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 4, RACE_MACHINE, ATTRIBUTE_EARTH) then
    return
  end
