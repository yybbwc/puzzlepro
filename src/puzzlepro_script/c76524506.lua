...
function c76524506.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_FZONE) and c76524506.desfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c76524506.desfilter, tp, LOCATION_FZONE, LOCATION_FZONE, 1, nil, tp)
  end
...
function c76524506.scop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not c:IsControler(tp) or not c:IsRelateToEffect(e) or c:IsFacedown() then
    return
  end
