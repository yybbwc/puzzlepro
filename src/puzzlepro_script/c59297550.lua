...
function c59297550.spcon(e, tp, eg, ep, ev, re, r, rp)
  return re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsSetCard(0x58) and not re:GetHandler():IsCode(59297550)
end
...
function c59297550.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsStatus(STATUS_CHAINING) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c59297550.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
