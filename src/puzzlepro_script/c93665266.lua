...
function c93665266.scfilter1(c, e, tp, mc)
  ...
  return not c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c93665266.scfilter2, tp, LOCATION_EXTRA, 0, 1, nil, mg)
end
...
function c93665266.sctg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c93665266.scfilter1, tp, LOCATION_HAND, 0, 1, nil, e, tp, e:GetHandler())
  end
