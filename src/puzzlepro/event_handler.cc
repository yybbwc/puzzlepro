...
namespace ygo {
    ...
    switch (event.EventType) {
      case irr::EET_GUI_EVENT: {
        ...
        switch (event.GUIEvent.EventType) {
          case irr::gui::EGET_BUTTON_CLICKED: {
            ...
            switch (id) {
              ...
              case BUTTON_CMD_SHUFFLE: {
                ...
                //~ if (id == BUTTON_CMD_ACTIVATE) {
                //~ continue;
                //~ }
                //~ }
                //~ else {
                //~ if (id == BUTTON_CMD_RESET) {
                //~ continue;
                //~ }
                //~ }
                ...
                switch (mainGame->dInfo.curMsg) {
                  ...
                  case MSG_SELECT_CHAIN: {
                    ...
                    if (list_command == COMMAND_SPSUMMON) {
                      ...
                      int index = 0;
                      while (spsummonable_cards[index] != command_card) {
                        index++;
                      }
                      ...
                      for (size_t i = 0; i < activatable_cards.size(); ++i) {
                        if (activatable_cards[i] == command_card) {
                          if (activatable_descs[i].second == EDESC_OPERATION) {
                            ...
                            if (list_command == COMMAND_OPERATION) {
                              continue;
                            }
                          }
                          select_options.push_back(activatable_descs[i].first);
                  ...
                  case MSG_SELECT_CARD: {
                    ...
                    if (command_card->is_selected) {
                      ...
                      int i = 0;
                      while (selected_cards[i] != command_card) {
                        i++;
                      }
                      ...
                      if (command_card->controler) {
                        ...
                        mainGame->stCardPos[id - BUTTON_CARD_0]->setBackgroundColor(0xffffffff);
                      }
                    }
                    else {
          ...
          case irr::gui::EGET_MENU_ITEM_SELECTED: {
            switch (mainGame->cmMenu->getItemCommandId(mainGame->cmMenu->getSelectedItem())) {
              case BUTTON_CMD_ACTIVATE:
              case BUTTON_CMD_RESET: {
                HideMenu();
                ShowCancelOrFinishButton(0);
                ...
                //~ else if (command_id == BUTTON_CMD_SUMMON) {
              case BUTTON_CMD_SUMMON: {
                HideMenu();
                if (!menu_card) {
                  break;
                ...
                //~ else if (command_id == BUTTON_CMD_SPSUMMON) {
              case BUTTON_CMD_SPSUMMON: {
                HideMenu();
                if (!list_command) {
                  if (!menu_card) {
                  ...
                  switch (command_location) {
                    ...
                    case LOCATION_EXTRA: {
                      ...
                      //~ fast_io::io::print(fast_io::win32_box_t(), selectable_cards.size(), "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
                      break;
                    }
                  }
                  list_command = COMMAND_SPSUMMON;
                ...
                //~ else if (command_id == BUTTON_CMD_MSET) {
              case BUTTON_CMD_MSET: {
                HideMenu();
                if (!menu_card) {
                  break;
                ...
                //~ else if (command_id == BUTTON_CMD_SSET) {
              case BUTTON_CMD_SSET: {
                HideMenu();
                if (!menu_card) {
                  break;
                ...
                //~ else if (command_id == BUTTON_CMD_REPOS) {
              case BUTTON_CMD_REPOS: {
                HideMenu();
                if (!menu_card) {
                  break;
                ...
                //~ else if (command_id == BUTTON_CMD_ATTACK) {
              case BUTTON_CMD_ATTACK: {
                HideMenu();
                if (!menu_card) {
                  break;
                ...
                //~ else if (command_id == BUTTON_CMD_SHOWLIST) {
              case BUTTON_CMD_SHOWLIST: {
                HideMenu();
                selectable_cards.clear();
                wchar_t formatBuffer[2048];
            ...
            switch (id) {
              ...
              case SCROLL_CARD_SELECT: {
                ...
                for (int i = 0; i < 5; ++i) {
                  ...
                  mainGame->stCardPos[i]->setOverrideColor(0xff333333);
                  // image
                  ...
                  if (mainGame->dInfo.curMsg == MSG_SORT_CARD) {
                    ...
                    if (conti_selecting) {
                      ...
                      myswprintf(formatBuffer, L"%ls[%d](%d)", dataManager.FormatLocation(selectable_cards[i + pos]->overlayTarget->location, selectable_cards[i + pos]->overlayTarget->sequence), selectable_cards[i + pos]->overlayTarget->sequence + 1, selectable_cards[i + pos]->sequence + 1);
                    }
                    else {
                      myswprintf(formatBuffer, L"%ls[%d]", dataManager.FormatLocation(selectable_cards[i + pos]->location, selectable_cards[i + pos]->sequence), selectable_cards[i + pos]->sequence + 1);
                    }
                  }
                  mainGame->stCardPos[i]->setText(formatBuffer);
                  // color
              ...
              case SCROLL_CARD_DISPLAY: {
                ...
                for (int i = 0; i < 5; ++i) {
                  ...
                  mainGame->stCardPos[i]->setOverrideColor(0xff000000);
                  ...
                  if (display_cards[i + pos]->location == LOCATION_OVERLAY) {
                    myswprintf(formatBuffer, L"%ls[%d](%d)", dataManager.FormatLocation(display_cards[i + pos]->overlayTarget->location, display_cards[i + pos]->overlayTarget->sequence), display_cards[i + pos]->overlayTarget->sequence + 1, display_cards[i + pos]->sequence + 1);
                  }
                  else {
                    myswprintf(formatBuffer, L"%ls[%d]", dataManager.FormatLocation(display_cards[i + pos]->location, display_cards[i + pos]->sequence), display_cards[i + pos]->sequence + 1);
                  }
                  ...
                  else if (display_cards[i + pos]->location == LOCATION_EXTRA || display_cards[i + pos]->location == LOCATION_REMOVED or display_cards[i + pos]->location == LOCATION_DECK) {
          ...
          case irr::gui::EGET_ELEMENT_HOVERED: {
            ...
            if (id == BUTTON_POS_AU or id == BUTTON_POS_AD or id == BUTTON_POS_DU or id == BUTTON_POS_DD) {
              mainGame->ShowCardInfo(mainGame->code_btnPS[id]);
            }
            break;
          }
          case irr::gui::EGET_ELEMENT_LEFT: {
            if (id >= BUTTON_CARD_0 && id <= BUTTON_CARD_4 && mainGame->stCardListTip->isVisible()) {
              int pos = mainGame->scrCardList->getPos() / 10;
              //~ fast_io::io::print(fast_io::win32_box_t(), selectable_cards.size(), "\n", id - BUTTON_CARD_0 + pos, "\n", mainGame->xScale, "\n", pos, "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
              if (selectable_cards.size() > id - BUTTON_CARD_0 + pos) {
                ClientCard *mcard = selectable_cards[id - BUTTON_CARD_0 + pos];
          ...
          case irr::gui::EGET_COMBO_BOX_CHANGED: {
            if (id == wOptions_combo_box_option_id) {
              //~ fast_io::io::perr(mainGame->wOptions_combo_box_option->getSelected(), "\n");
              selected_option = mainGame->wOptions_combo_box_option->getSelected();
            }
            break;
          }
          default:
          ...
          case irr::EMIE_LMOUSE_LEFT_UP: {
            ...
            if (mainGame->dInfo.isReplay) {
              ...
              switch (hovered_location) {
                case LOCATION_DECK: {
                  if (deck[hovered_controler].empty()) {
                    break;
                  }
                ...
                case LOCATION_MZONE: {
                  if (!clicked_card || clicked_card->overlayed.empty()) {
                    break;
                  }
                ...
                case LOCATION_GRAVE: {
                  if (grave[hovered_controler].empty()) {
                    break;
                  }
                ...
                case LOCATION_REMOVED: {
                  if (remove[hovered_controler].empty()) {
                    break;
                  }
                ...
                case LOCATION_EXTRA: {
                  if (extra[hovered_controler].empty()) {
                    break;
                  }
            ...
            //~ fast_io::io::print(fast_io::win32_box_t(), mainGame->dInfo.curMsg, "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
            switch (mainGame->dInfo.curMsg) {
              case MSG_WAITING: {
                switch (hovered_location) {
                  case LOCATION_MZONE:
              ...
              case MSG_SELECT_CHAIN: {
                switch (hovered_location) {
                  ...
                  case LOCATION_SZONE: {
                    ...
                    if (!clicked_card->overlayed.empty()) {
                      command_flag |= COMMAND_LIST;
                    }
                    list_command = 0;
                    ShowMenu(command_flag, x, y);
              ...
              case MSG_SELECT_CARD: {
                ...
                if (clicked_card->is_selected) {
                  ...
                  int i = 0;
                  while (selected_cards[i] != clicked_card) {
                    i++;
                  }
                  selected_cards.erase(selected_cards.begin() + i);
                }
                ...
                if ((int)selected_cards.size() >= select_max) {
                  ...
                  select_ready = false;
                  if (select_cancelable && selected_cards.empty()) {
                    ShowCancelOrFinishButton(1);
                  }
                  else {
                    ShowCancelOrFinishButton(0);
              ...
              case MSG_SELECT_UNSELECT_CARD: {
                ...
                clicked_card->is_selected = !clicked_card->is_selected;
                selected_cards.push_back(clicked_card);
                if (!selected_cards.empty()) {
                  ShowCancelOrFinishButton(0);
                  SetResponseSelectedCards();
          ...
          case irr::EMIE_MOUSE_MOVED: {
                ...
                if (mcard->code) {
                  ...
                  if (mcard->location & 0xe) {
                    ...
                    if (mcard->type & TYPE_MONSTER) {
                      if (mcard->alias && std::wcscmp(dataManager.GetName(mcard->code), dataManager.GetName(mcard->alias)) != 0) {
                    ...
                    mainGame->stTip->setRelativePosition(irr::core::recti(mousepos.X - 10 - dtip.Width, mousepos.Y - 10 - dtip.Height, mousepos.X - 10, mousepos.Y - 10));
            ...
            if (mplayer != hovered_player) {
              if (mplayer >= 0) {
                ...
                should_show_tip = true;
                irr::core::dimension2d<unsigned int> dtip = mainGame->guiFont->getDimension(str.c_str()) + irr::core::dimension2d<unsigned int>(10, 10);
                mainGame->stTip->setRelativePosition(irr::core::recti(mousepos.X - 10 - dtip.Width, mousepos.Y + 10, mousepos.X - 10, mousepos.Y + 10 + dtip.Height));
                mainGame->stTip->setText(str.c_str());
              }
      ...
      case irr::EET_KEY_INPUT_EVENT: {
        switch (event.KeyInput.Key) {
          ...
          case irr::KEY_F7:
          case irr::KEY_F8: {
            if (!event.KeyInput.PressedDown && !mainGame->dInfo.isReplay && mainGame->dInfo.player_type != 7 && mainGame->dInfo.isStarted && !mainGame->wCardDisplay->isVisible() && !mainGame->HasFocus(irr::gui::EGUIET_EDIT_BOX)) {
              int loc_id = 0;
              display_cards.clear();
              switch (event.KeyInput.Key) {
                case irr::KEY_F1:
        ...
        switch (event.GUIEvent.EventType) {
          ...
          case irr::gui::EGET_COMBO_BOX_CHANGED: {
            switch (id) {
              ...
              case chain_timing_combo_box_id: {
                soundManager.PlaySoundEffect(SOUND_BUTTON);
                int64_t k1 = mainGame->chain_timing_combo_box->getSelected();
                if (k1 == 0) {
                  mainGame->ignore_chain = false;
                  mainGame->always_chain = false;
                  mainGame->chain_when_avail = false;
      ...
      case irr::EET_MOUSE_INPUT_EVENT: {
        switch (event.MouseInput.Event) {
          case irr::EMIE_LMOUSE_LEFT_UP: {
            ...
            switch (id) {
              case TEXT_DATA_INFO: {
                clip::set_text(mainGame->currentCardName.c_str());
                soundManager.PlaySoundEffect(SOUND_COMMON_FEEDBACK);
                return true;
                break;
              }
              default: {
  ...
  void ClientField::GetHoverField(int x, int y) {
    ...
    if (sfRect.isPointInside(pos)) {
      ...
      else if (boardx >= matManager.vFieldContiAct[0].X && boardx <= matManager.vFieldContiAct[1].X && boardy >= matManager.vFieldContiAct[0].Y && boardy <= matManager.vFieldContiAct[2].Y) {
        hovered_controler = 0;
  ...
  void ClientField::ShowMenu(int flag, int x, int y) {
    ...
    mainGame->cmMenu->removeAllItems();
    menu_card = clicked_card;
    if (flag & COMMAND_ACTIVATE) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1150), BUTTON_CMD_ACTIVATE);
    }
    if (flag & COMMAND_SUMMON) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1151), BUTTON_CMD_SUMMON);
    }
    if (flag & COMMAND_SPSUMMON) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1152), BUTTON_CMD_SPSUMMON);
    }
    if (flag & COMMAND_MSET) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1153), BUTTON_CMD_MSET);
    }
    if (flag & COMMAND_SSET) {
      if (!(clicked_card->type & TYPE_MONSTER)) {
        mainGame->cmMenu->addItem(dataManager.GetSysString(1153), BUTTON_CMD_SSET);
      }
      else {
        mainGame->cmMenu->addItem(dataManager.GetSysString(1159), BUTTON_CMD_SSET);
      }
    }
    if (flag & COMMAND_REPOS) {
      if (clicked_card->position & POS_FACEDOWN) {
        mainGame->cmMenu->addItem(dataManager.GetSysString(1154), BUTTON_CMD_REPOS);
      }
      else if (clicked_card->position & POS_ATTACK) {
        mainGame->cmMenu->addItem(dataManager.GetSysString(1155), BUTTON_CMD_REPOS);
      }
      else {
        mainGame->cmMenu->addItem(dataManager.GetSysString(1156), BUTTON_CMD_REPOS);
      }
    }
    if (flag & COMMAND_ATTACK) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1157), BUTTON_CMD_ATTACK);
    }
    if (flag & COMMAND_LIST) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1158), BUTTON_CMD_SHOWLIST);
    }
    if (flag & COMMAND_OPERATION) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1161), BUTTON_CMD_ACTIVATE);
    }
    if (flag & COMMAND_RESET) {
      mainGame->cmMenu->addItem(dataManager.GetSysString(1162), BUTTON_CMD_RESET);
    }
    mainGame->btnBP->setEnabled(false);
    mainGame->btnM2->setEnabled(false);
    mainGame->btnEP->setEnabled(false);
    mainGame->cmMenu->setRelativePosition(irr::core::position2d<irr::s32>(x * mainGame->xScale - gui_config["menu_width_width"].cast<double>().value() * mainGame->cmMenu->getRelativePosition().getWidth(), y * mainGame->yScale - gui_config["menu_width_height"].cast<double>().value() * mainGame->cmMenu->getRelativePosition().getWidth()));
    mainGame->PopupElement(mainGame->cmMenu);
  }

  void ClientField::HideMenu() {
    mainGame->cmMenu->setVisible(false);
  ...
  void ClientField::ShowCardInfoInList(ClientCard *pcard, irr::gui::IGUIElement *element, irr::gui::IGUIElement *parent) {
    ...
    if (pcard->location != LOCATION_DECK) {
      ...
      if ((pcard->status & STATUS_PROC_COMPLETE) && (pcard->type & (TYPE_RITUAL | TYPE_FUSION | TYPE_SYNCHRO | TYPE_XYZ | TYPE_LINK | TYPE_SPSUMMON))) {
    ...
    if (!str.empty()) {
      ...
      mainGame->stCardListTip->setRelativePosition(irr::core::rect<irr::s32>(irr::core::position2d<irr::s32>(element->getRelativePosition().UpperLeftCorner.X, element->getRelativePosition().UpperLeftCorner.Y), irr::core::dimension2d<irr::s32>(mainGame->stCardListTip->getTextWidth() + 5, mainGame->stCardListTip->getTextHeight() + 5)));
      mainGame->stCardListTip->setVisible(true);
    }
  }
