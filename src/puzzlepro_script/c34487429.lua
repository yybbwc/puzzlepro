...
function c34487429.discon(e, tp, eg, ep, ev, re, r, rp)
  return re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev) and Duel.IsExistingMatchingCard(Card.IsSetCard, tp, LOCATION_SZONE, 0, 3, nil, 0x1034)
end
...
function c34487429.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c34487429.filter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c34487429.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c34487429.filter3(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c34487429.filter3, tp, LOCATION_SZONE, 0, 1, nil, e, tp)
  end
...
function c34487429.rdcon(e, tp, eg, ep, ev, re, r, rp)
  return ep == tp and e:GetHandler():GetFlagEffect(34487429) == 0 and Duel.IsExistingMatchingCard(c34487429.filter1, tp, LOCATION_SZONE, 0, 2, nil)
end
