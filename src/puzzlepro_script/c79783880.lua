...
function c79783880.spfilter1(c, e, tp)
  return c:IsFaceup() and c:IsRace(RACE_ZOMBIE) and c:IsSetCard(0xd9) and not c:IsCode(79783880) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and Duel.IsExistingTarget(c79783880.spfilter2, tp, LOCATION_REMOVED, 0, 1, c, e, tp)
end
...
function c79783880.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetMZoneCount(tp, e:GetHandler()) > 1 and Duel.IsExistingTarget(c79783880.spfilter1, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
