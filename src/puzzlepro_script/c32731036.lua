...
function s.cfilter(c, tp)
  return c:IsLevelAbove(6) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsRace(RACE_DRAGON) and Duel.GetMZoneCount(tp, c) > 0
end
...
function s.pfilter(c, tp)
  return c:IsType(TYPE_CONTINUOUS) and c:IsSetCard(0x15d) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
