...
function c52628687.cfilter(c, e, tp)
  return c:IsRace(RACE_SPELLCASTER) and c:GetLevel() > 0 and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c52628687.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
...
function c52628687.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c52628687.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c52628687.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c52628687.cffilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
