...
function c28770951.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c28770951.matfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c28770951.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanOverlay()
  end
...
function c28770951.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c28770951.filter(chkc, e)
  end
  local g = Duel.GetMatchingGroup(c28770951.filter, tp, LOCATION_GRAVE, 0, nil, e)
  if chk == 0 then
    return g:GetClassCount(Card.GetCode) >= 5 and Duel.IsPlayerCanDraw(tp, 2)
  end
