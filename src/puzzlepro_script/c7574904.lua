...
function c7574904.rmcon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and ep == 1 - tp and re:IsActiveType(TYPE_MONSTER)
end
...
function c7574904.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7574904.rmcostfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil, e, tp)
  end
...
function c7574904.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c7574904.thtgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c7574904.thtgfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
