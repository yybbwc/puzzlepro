...
function c13317419.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c13317419.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c13317419.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c13317419.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsPosition(POS_FACEUP_ATTACK) and c:IsAttackAbove(800)
end
...
function c13317419.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c13317419.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13317419.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c13317419.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = Duel.GetFirstTarget()
  if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) ~= 0 and ec and ec:IsFaceup() and ec:IsLocation(LOCATION_MZONE) then
    Duel.BreakEffect()
