...
function c58807980.hspcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c58807980.cfilter, tp, LOCATION_HAND, 0, 1, nil)
end
...
function c58807980.filter(c, e, tp)
  return c:IsLocation(LOCATION_GRAVE) and c:IsType(TYPE_TUNER) and c:IsRace(RACE_BEAST) and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c58807980.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local mg = e:GetHandler():GetReasonCard():GetMaterial()
  if chkc then
    return mg:IsContains(chkc) and c58807980.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and mg:IsExists(c58807980.filter, 1, nil, e, tp)
  end
