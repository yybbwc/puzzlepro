...
function c61318483.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(61318483) == 0
  end
...
function c61318483.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local at = Duel.GetAttacker()
  if chk == 0 then
    return at:IsCanTurnSet() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE)
  end
