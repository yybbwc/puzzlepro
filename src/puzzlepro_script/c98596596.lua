...
function c98596596.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 23116809, 0, TYPES_TOKEN_MONSTER, 100, 100, 1, RACE_PYRO, ATTRIBUTE_FIRE)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOKEN, nil, 1, 0, 0)
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, 0, 0)
end
function c98596596.spop1(e, tp, eg, ep, ev, re, r, rp)
  if not e:GetHandler():IsRelateToEffect(e) or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 then
    return
  end
...
function c98596596.cfilter2(c, tp)
  return c:IsReason(REASON_EFFECT) and c:IsLocation(LOCATION_GRAVE) and c:IsPreviousLocation(LOCATION_HAND + LOCATION_MZONE) and c:IsPreviousControler(tp) and c:GetReasonPlayer() == 1 - tp
end
...
function c98596596.spop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not e:GetHandler():IsRelateToEffect(e) or ct <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 23116809, 0, TYPES_TOKEN_MONSTER, 100, 100, 1, RACE_PYRO, ATTRIBUTE_FIRE) then
    return
  end
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    ct = 1
