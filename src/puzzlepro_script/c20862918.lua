...
function c20862918.costfilter1(c, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c20862918.costfilter2, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, c, c:GetOriginalRace(), c:GetOriginalAttribute(), c:GetCode(), tp)
end
function c20862918.costfilter2(c, race, att, code, tp)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and c:GetOriginalRace() == race and c:GetOriginalAttribute() == att and not c:IsCode(code) and Duel.IsExistingMatchingCard(c20862918.thfilter, tp, LOCATION_DECK, 0, 1, nil, race, att, code, c:GetCode())
end
function c20862918.thfilter(c, race, att, code1, code2)
  return c:GetOriginalRace() == race and c:GetOriginalAttribute() == att and not c:IsCode(code1, code2) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
