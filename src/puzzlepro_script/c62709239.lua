...
function c62709239.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.IsExistingTarget(aux.TRUE, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c62709239.spfilter1(c, e, tp)
  return c:GetLevel() > 0 and c:IsSetCard(0x10db) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c62709239.spfilter2, tp, LOCATION_GRAVE, 0, 1, c, c:GetLevel(), e, tp)
end
...
function c62709239.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c62709239.spfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
