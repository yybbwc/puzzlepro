...
function c54498517.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckRemoveOverlayCard(tp, 1, 0, 1, REASON_EFFECT) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c54498517.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c54498517.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.RemoveOverlayCard(tp, 1, 0, 1, 1, REASON_EFFECT) ~= 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:GetCount() > 0 then
