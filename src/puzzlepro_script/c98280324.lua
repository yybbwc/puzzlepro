...
function c98280324.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c98280324.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c98280324.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c98280324.cfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), ft)
  end
...
function c98280324.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98280324.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
