...
function c25451652.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c25451652.ctfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c25451652.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c25451652.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanDiscardDeck(tp, ct)
  end
