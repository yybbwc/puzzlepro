...
function c84815190.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c84815190.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingTarget(c84815190.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and e:GetHandler():IsAbleToExtra()
  end
