...
function c42427230.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rg:GetCount() == rg:FilterCount(Card.IsReleasable, nil) and rg:GetCount() ~= 0 and Duel.GetMZoneCount(tp, rg) > 0
  end
...
function c42427230.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonMonster(tp, 42427231, 0, TYPES_TOKEN_MONSTER, 200, 200, 1, RACE_MACHINE, ATTRIBUTE_FIRE)
  end
...
function c42427230.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c42427230.desfilter(chkc)
  end
