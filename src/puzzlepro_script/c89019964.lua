...
function c89019964.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c89019964.tgfilter, tp, 0, LOCATION_MZONE, 1, nil, tp, e:GetHandler())
  end
...
function c89019964.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFaceup() and c:IsRelateToEffect(e) and c:IsControler(tp) and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) and not tc:IsImmuneToEffect(e) then
...
function c89019964.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c89019964.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c89019964.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
