...
function c92043888.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c92043888.confilter, tp, LOCATION_PZONE, 0, 1, nil) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and rp == 1 - tp
end
...
function c92043888.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c92043888.tdfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c92043888.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_DECK) and Duel.IsExistingMatchingCard(c92043888.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
    Duel.BreakEffect()
