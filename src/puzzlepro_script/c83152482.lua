...
function c83152482.cfilter(c, tp)
  return c:IsFaceup() and Duel.IsExistingMatchingCard(c83152482.eqfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, c:GetOriginalAttribute(), c:GetOriginalRace(), tp)
end
function c83152482.eqfilter(c, att, race, tp)
  return c:IsType(TYPE_MONSTER) and (c:GetOriginalAttribute() == att or c:GetOriginalRace() == race) and c:CheckUniqueOnField(tp) and not c:IsForbidden()
end
function c83152482.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c83152482.cfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83152482.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0
  end
