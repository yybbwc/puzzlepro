...
function s.rmfilter1(c)
  return c:IsFaceup() and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(s.rmfilter2, 1 - c:GetControler(), LOCATION_GRAVE, 0, 1, nil, c:GetRace(), c:GetAttribute())
end
...
function s.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(aux.NegateMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil) or Duel.IsExistingMatchingCard(s.rmfilter1, tp, 0, LOCATION_MZONE, 1, nil)
  end
