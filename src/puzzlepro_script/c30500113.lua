...
function c30500113.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c30500113.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30500113.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c30500113.cfilter2(c, tp)
  return c:IsSetCard(0xba) and c:IsReason(REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c30500113.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30500113.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
