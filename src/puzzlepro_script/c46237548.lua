...
function c46237548.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.GetMatchingGroupCount(Card.IsType, c:GetControler(), LOCATION_GRAVE, 0, nil, TYPE_NORMAL) == 3 and Duel.IsExistingMatchingCard(c46237548.spcfilter, c:GetControler(), LOCATION_GRAVE, 0, 2, nil)
end
...
function c46237548.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c46237548.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c46237548.tgfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
