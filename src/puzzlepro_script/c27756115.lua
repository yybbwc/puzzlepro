...
function c27756115.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c27756115.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c27756115.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c27756115.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsLocation(LOCATION_MZONE) and c:IsFacedown() then
    return
  end
...
function c27756115.cfilter(c, e, tp)
  return c:IsControler(tp) and c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_DEFENSE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_ATTACK)
end
function c27756115.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and eg:IsExists(c27756115.cfilter, 1, nil, e, tp)
  end
