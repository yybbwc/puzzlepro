...
function c45313724.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and (c:IsSetCard(0xb) or c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevel(8) and c:IsType(TYPE_SYNCHRO)) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and c:GetReasonPlayer() == 1 - tp
end
...
function c45313724.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:IsExists(c45313724.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c45313724.desfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
