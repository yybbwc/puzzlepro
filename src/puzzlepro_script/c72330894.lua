...
function c72330894.desfilter(c, e)
  return c:IsFaceup() and c:IsSetCard(0x12d) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
end
function c72330894.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReason(REASON_BATTLE) and not c:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(c72330894.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil, e)
  end
...
function c72330894.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return ct > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72330894.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp, ct)
  end
