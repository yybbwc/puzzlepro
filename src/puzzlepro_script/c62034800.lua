...
function c62034800.spfilter(c, e, tp)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_EARTH) and not c:IsCode(62034800) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c62034800.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c62034800.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c62034800.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc ~= c and chkc:IsLocation(LOCATION_MZONE) and c62034800.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62034800.lvfilter, tp, LOCATION_MZONE, 0, 1, c)
  end
