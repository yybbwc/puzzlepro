...
function c39317553.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 39317553) == 0
  end
...
function c39317553.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c39317553.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingTarget(c39317553.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
