...
function c11449436.indesop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.DiscardHand(tp, nil, 1, 1, REASON_EFFECT + REASON_DISCARD) > 0 and a and a:IsRelateToBattle() and a:IsControler(tp) then
    local e1 = Effect.CreateEffect(c)
...
function c11449436.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c11449436.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c11449436.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  e:SetLabelObject(tc)
  if chk == 0 then
    return tc and tc:IsControler(1 - tp) and Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0
  end
  Duel.SetOperationInfo(0, CATEGORY_HANDES, nil, 0, tp, 1)
