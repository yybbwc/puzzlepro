...
function c92781606.spfilter1(c, e, tp)
  return c:IsSetCard(0x104) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE) and Duel.IsExistingTarget(c92781606.spfilter2, tp, LOCATION_GRAVE, 0, 1, c, c:GetCode(), e, tp)
end
...
function c92781606.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c92781606.spfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
