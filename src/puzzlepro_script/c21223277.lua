...
function c21223277.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c21223277.filter(chkc, e, tp, e:GetHandler())
  end
  if chk == 0 then
...
function c21223277.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21223277.cfilter, tp, LOCATION_SZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c21223277.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
