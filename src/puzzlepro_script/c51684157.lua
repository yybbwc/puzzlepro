...
function c51684157.filter(c, e, tp, check)
  return c:IsRace(RACE_WYRM) and (Duel.IsExistingMatchingCard(c51684157.thfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp) or (check and not c:IsType(TYPE_EFFECT) and Duel.IsExistingMatchingCard(c51684157.thfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, true) and Duel.GetMZoneCount(tp, c) > 0))
end
...
function c51684157.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100, 0)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c51684157.filter, 1, nil, e, tp, true)
  end
...
function c51684157.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c51684157.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, tp, LOCATION_DECK)
end
