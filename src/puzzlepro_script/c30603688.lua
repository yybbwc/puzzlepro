...
function c30603688.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(nil, tp, LOCATION_HAND, 0, 1, c)
end
...
function c30603688.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30603688.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c30603688.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(c)
  return c and c ~= e:GetHandler() and c:IsRace(RACE_SPELLCASTER) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsRelateToBattle()
end
