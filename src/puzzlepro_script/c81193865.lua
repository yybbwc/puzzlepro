...
function c81193865.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c81193865.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81193865.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c81193865.cacost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81193865.cafilter, tp, LOCATION_GRAVE, 0, 1, nil) and (c81193865[tp] == 0 or c:GetEquipTarget():GetFlagEffect(81193865) ~= 0)
  end
  local e1 = Effect.CreateEffect(c)
...
function c81193865.catg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and e:GetHandler():GetEquipTarget():IsChainAttackable(0, true)
  end
end
