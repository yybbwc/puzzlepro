...
function c51728779.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsDiscardable() and Duel.IsExistingMatchingCard(c51728779.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c51728779.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c51728779.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51728779.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c51728779.cptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c51728779.cpfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c51728779.cpop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not te:GetHandler():IsRelateToEffect(e) then
    return
  end
