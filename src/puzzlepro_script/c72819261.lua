...
function c72819261.costfilter(c, tp)
  return c:IsSetCard(0x400d) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c72819261.filter, tp, LOCATION_DECK, 0, 1, c)
end
function c72819261.sgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72819261.costfilter, tp, loc, 0, 1, nil, tp)
  end
...
function c72819261.sgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72819261.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
