...
function c84417082.target1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c84417082.filter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler())
  end
...
function c84417082.cost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, 5, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
