...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and s.disfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_MZONE + LOCATION_HAND, 0, 1, nil) and Duel.IsExistingTarget(s.disfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sg:GetCount() > 0 and Duel.Remove(sg, POS_FACEUP, REASON_EFFECT) ~= 0 and sg:IsExists(Card.IsLocation, 1, nil, LOCATION_REMOVED) and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsType(TYPE_MONSTER) and tc:IsCanBeDisabledByEffect(e) then
...
function s.rmfilter(c, tp)
  return c:IsSetCard(0x1bf) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetType() & (TYPE_SPELL + TYPE_TRAP + TYPE_MONSTER))
end
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
