...
function c50696588.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 50696588) == 0
  end
end
function c50696588.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE, PLAYER_NONE, 0) > 0
  end
...
function c50696588.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.CheckLocation(1 - tp, LOCATION_MZONE, seq) or Duel.GetFlagEffect(tp, 50696588) ~= 0 then
    return
  end
