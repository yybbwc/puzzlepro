...
function c16691074.spfilter2(c, e, tp)
  return c:IsSetCard(0x99) and not c:IsCode(16691074) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c16691074.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16691074.spfilter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
