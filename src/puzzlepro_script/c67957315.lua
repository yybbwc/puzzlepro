...
function c67957315.adcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == tp and (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE) and (e:GetHandler() == Duel.GetAttacker() or e:GetHandler() == Duel.GetAttackTarget())
end
...
function c67957315.adcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67957315.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
