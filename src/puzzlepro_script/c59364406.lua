...
function c59364406.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c59364406.filter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(59364406) == 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c59364406.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c59364406.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(59364406) == 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, true, false, POS_FACEUP_ATTACK)
  end
...
function c59364406.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c59364406.desfilter(chkc)
  end
  if chk == 0 then
    return true
  end
