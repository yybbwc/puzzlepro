...
function c98715423.thfilter(c)
  return (c:IsSetCard(0x2e) and c:IsType(TYPE_MONSTER) or c:IsRace(RACE_FAIRY) and c:IsAttribute(ATTRIBUTE_EARTH)) and c:IsAbleToHand()
end
function c98715423.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98715423.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c98715423.cfcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == 1 - tp and Duel.GetFieldGroupCount(1 - tp, LOCATION_DECK, 0) > 0 and Duel.GetDrawCount(1 - tp) > 0 and e:GetHandler():IsStatus(STATUS_EFFECT_ENABLED)
end
