...
namespace ygo {
  ...
  void DuelClient::ConnectTimeout(evutil_socket_t fd, short events, void *arg) {
    ...
    if (!is_closing) {
      ...
      mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1400));
  ...
  void DuelClient::ClientRead(bufferevent *bev, void *ctx) {
    ...
    auto *duel_client_read = new unsigned char[SIZE_NETWORK_BUFFER];
        ...
        if (connect_state == 0x1) {
          ...
          if (!mainGame->dInfo.isStarted && !mainGame->is_building) {
            ...
            if (events & BEV_EVENT_EOF) {
              mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1401));
            }
            else {
            ...
            mainGame->gMutex.lock();
            soundManager.PlaySoundEffect(SOUND_INFO);
            mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1502));
            mainGame->btnCreateHost->setEnabled(true);
            mainGame->btnJoinHost->setEnabled(true);
            mainGame->btnJoinCancel->setEnabled(true);
            mainGame->btnStartBot->setEnabled(true);
    ...
    switch (pktType) {
      ...
      case STOC_ERROR_MSG: {
        ...
        switch (pkt->msg) {
          case ERRMSG_JOINERROR: {
            ...
            soundManager.PlaySoundEffect(SOUND_INFO);
            if (pkt->code == 0) {
              mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1403));
            }
            else if (pkt->code == 1) {
              mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1404));
            }
            else if (pkt->code == 2) {
              mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1405));
            }
            mainGame->gMutex.unlock();
            event_base_loopbreak(client_base);
            break;
          ...
          case ERRMSG_DECKERROR: {
            ...
            soundManager.PlaySoundEffect(SOUND_INFO);
            mainGame->guiEnv->addMessageBox(L"", msgbuf);
          ...
          case ERRMSG_SIDEERROR: {
            mainGame->gMutex.lock();
            soundManager.PlaySoundEffect(SOUND_INFO);
            mainGame->guiEnv->addMessageBox(L"", dataManager.GetSysString(1408));
            mainGame->gMutex.unlock();
            break;
          }
      ...
      case STOC_TYPE_CHANGE: {
        ...
        if (!mainGame->dInfo.isTag) {
          ...
          if (mainGame->chkHostPrepReady[0]->isChecked() && mainGame->chkHostPrepReady[1]->isChecked() && mainGame->chkHostPrepReady[2]->isChecked() && mainGame->chkHostPrepReady[3]->isChecked()) {
            mainGame->btnHostPrepStart->setEnabled(true);
          }
          else {
            mainGame->btnHostPrepStart->setEnabled(false);
      ...
      case STOC_REPLAY: {
        ...
        if (mainGame->actionParam || !is_host) {
          ...
          if (mainGame->actionParam) {
            ...
            new_replay.SaveReplay(L"_last_replay");
          }
        }
        break;
      }
      ...
      case STOC_CHAT: {
        ...
        auto chat_player_type = buffer_read<uint16_t>(pdata);
      ...
      case STOC_HS_PLAYER_CHANGE: {
        ...
        if (mainGame->chkHostPrepReady[0]->isChecked() && mainGame->chkHostPrepReady[1]->isChecked() && (!mainGame->dInfo.isTag || (mainGame->chkHostPrepReady[2]->isChecked() && mainGame->chkHostPrepReady[3]->isChecked()))) {
          mainGame->btnHostPrepStart->setEnabled(true);
        }
        else {
          mainGame->btnHostPrepStart->setEnabled(false);
    ...
    switch (mainGame->dInfo.curMsg) {
      ...
      case MSG_SELECT_BATTLECMD: {
        ...
        for (int i = 0; i < count; ++i) {
          ...
          loc = BufferIO::ReadUInt8(pbuf);
          seq = BufferIO::ReadUInt8(pbuf);
          int64_t diratt = BufferIO::ReadUInt8(pbuf);
          pcard = mainGame->dField.GetCard(con, loc, seq);
      ...
      case MSG_SELECT_IDLECMD: {
        ...
        count = BufferIO::ReadUInt8(pbuf);
        //~ fast_io::io::print(fast_io::win32_box_t(), count, __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
        for (int i = 0; i < count; ++i) {
          code = BufferIO::ReadInt32(pbuf);
          con = mainGame->LocalPlayer(BufferIO::ReadUInt8(pbuf));
          loc = BufferIO::ReadUInt8(pbuf);
        ...
        int64_t k1;
        k1 = BufferIO::ReadUInt8(pbuf);

        //~ fast_io::io::print(fast_io::win32_box_t(), k1, __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
        if (k1) {
      ...
      case MSG_SELECT_CARD: {
        ...
        if (select_hint) {
          myswprintf(textBuffer, L"%ls(%d-%d)", dataManager.GetDesc(select_hint), mainGame->dField.select_min, mainGame->dField.select_max);
        }
        else {
          myswprintf(textBuffer, L"%ls(%d-%d)", dataManager.GetSysString(560), mainGame->dField.select_min, mainGame->dField.select_max);
        }
        select_hint = 0;
        if (panelmode) {
          mainGame->gMutex.lock();
      ...
      case MSG_SELECT_UNSELECT_CARD: {
        ...
        if (select_unselect_hint) {
          myswprintf(textBuffer, L"%ls(%d-%d)", dataManager.GetDesc(select_unselect_hint), mainGame->dField.select_min, mainGame->dField.select_max);
        }
        else {
          myswprintf(textBuffer, L"%ls(%d-%d)", dataManager.GetSysString(560), mainGame->dField.select_min, mainGame->dField.select_max);
        }
      ...
      case MSG_SELECT_PLACE:
      case MSG_SELECT_DISFIELD: {
        //~ fast_io::win32::MessageBoxW(0, u"1", u"fast_io",0);
        int selecting_player = BufferIO::ReadUInt8(pbuf);
        int count = BufferIO::ReadUInt8(pbuf);
        mainGame->dField.select_min = count > 0 ? count : 1;
        mainGame->dField.select_ready = false;
        mainGame->dField.select_cancelable = count == 0;
        mainGame->dField.selectable_field = ~BufferIO::ReadUInt32(pbuf);
        //~ fast_io::win32::MessageBoxW(0,
        // fast_io::u16concat_fast_io(fast_io::mnp::base<2>(mainGame->dField.selectable_field)).c_str(),
        // u"fast_io",0);
        ...
        mainGame->stHintMsg->setText(textBuffer);
        mainGame->stHintMsg->setVisible(true);
        if (mainGame->dInfo.curMsg == MSG_SELECT_PLACE && ((mainGame->chkMAutoPos->isChecked() && mainGame->dField.selectable_field & 0x7f007f) || (mainGame->chkSTAutoPos->isChecked() && !(mainGame->dField.selectable_field & 0x7f007f)))) {
      ...
      case MSG_SELECT_POSITION: {
        ...
        int selecting_player = BufferIO::ReadUInt8(pbuf);
        auto code = (unsigned int)BufferIO::ReadInt32(pbuf);
        ...
        if (positions & 0x4) {
          mainGame->btnPSDU->setRelativePosition(mainGame->ResizeWin(startpos, gui_xy["btnPSDU"]["y1"], startpos + gui_xy["btnPSDU"]["width"].cast<double>().value(), gui_xy["btnPSDU"]["y2"]));
          mainGame->btnPSDU->setImageSize(irr::core::dimension2di(gui_xy["btnPSDU"]["height_reduce"].cast<double>().value() * mainGame->yScale, gui_xy["btnPSDU"]["width_reduce"].cast<double>().value() * mainGame->xScale));
          mainGame->imageLoading.insert_or_assign(mainGame->btnPSDU, code);
          mainGame->code_btnPS[BUTTON_POS_DU] = code;
          mainGame->btnPSDU->setVisible(true);
          startpos += gui_xy["btnPSDU"]["width_offset"].cast<double>().value();
        }
        else {
        ...
        if (positions & 0x8) {
          ...
          mainGame->btnPSDD->setVisible(false);
        }
        mainGame->gMutex.lock();
      ...
      case MSG_SELECT_TRIBUTE: {
        ...
        if (select_hint) {
          ...
          myswprintf(textBuffer, L"%ls(%d-%d)", dataManager.GetSysString(531), mainGame->dField.select_min, mainGame->dField.select_max);
        }
        select_hint = 0;
        mainGame->gMutex.lock();
        mainGame->stHintMsg->setText(textBuffer);
      ...
      case MSG_CONFIRM_CARDS: {
        ...
        if (!panel_confirm.empty() && mainGame->dInfo.player_type != 7) {
      ...
      case MSG_NEW_TURN: {
        int player = mainGame->LocalPlayer(BufferIO::ReadUInt8(pbuf));
        if (gui_replay["MSG_NEW_TURN"]) {
          fast_io::io::println(last_replay_txt, "* ", fast_io::mnp::cond(player == 0, "我方", "对方"), "进入新回合");
        }
        mainGame->dInfo.turn++;
      ...
      case MSG_NEW_PHASE: {
        ...
        switch (phase) {
          case PHASE_DRAW: {
            ...
            string gui_replay_string_2 = gui_replay[501];
            string gui_replay_string_3 = gui_replay[601];
          ...
          case PHASE_STANDBY: {
            string gui_replay_string_1 = gui_replay[101];
            string gui_replay_string_2 = gui_replay[501];
            string gui_replay_string_3 = gui_replay[602];
            if (gui_replay["MSG_NEW_PHASE"]) {
              print(last_replay_txt, gui_replay_string_1, gui_replay_string_2, gui_replay_string_3, "\n");
            }
          ...
          case PHASE_MAIN1: {
            string gui_replay_string_1 = gui_replay[101];
            string gui_replay_string_2 = gui_replay[501];
            string gui_replay_string_3 = gui_replay[603];
            if (gui_replay["MSG_NEW_PHASE"]) {
              print(last_replay_txt, gui_replay_string_1, gui_replay_string_2, gui_replay_string_3, "\n");
            }
          ...
          case PHASE_BATTLE_START: {
            string gui_replay_string_1 = gui_replay[101];
            string gui_replay_string_2 = gui_replay[501];
            string gui_replay_string_3 = gui_replay[604];
            if (gui_replay["MSG_NEW_PHASE"]) {
              print(last_replay_txt, gui_replay_string_1, gui_replay_string_2, gui_replay_string_3, "\n");
            }
          ...
          case PHASE_MAIN2: {
            string gui_replay_string_1 = gui_replay[101];
            string gui_replay_string_2 = gui_replay[501];
            string gui_replay_string_3 = gui_replay[605];
            if (gui_replay["MSG_NEW_PHASE"]) {
              print(last_replay_txt, gui_replay_string_1, gui_replay_string_2, gui_replay_string_3, "\n");
            }
          ...
          case PHASE_END: {
            string gui_replay_string_1 = gui_replay[101];
            string gui_replay_string_2 = gui_replay[501];
            string gui_replay_string_3 = gui_replay[606];
            if (gui_replay["MSG_NEW_PHASE"]) {
              print(last_replay_txt, gui_replay_string_1, gui_replay_string_2, gui_replay_string_3, "\n");
            }
      ...
      case MSG_MOVE: {
        ...
        if (gui_replay["MSG_MOVE"]) {
          fast_io::io::println(last_replay_txt, "* 「", fast_io::mnp::code_cvt(fast_io::mnp::os_c_str(dataManager.GetName(code))), "」从", fast_io::mnp::cond(pc == 0, "我方", "对方"), mainGame->get_location_string(pl, ps), "移动到", fast_io::mnp::cond(cc == 0, "我方", "对方"), mainGame->get_location_string(cl, cs));
        }
        if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
          ...
          if (!(pl & LOCATION_OVERLAY) && !(cl & LOCATION_OVERLAY)) {
            ...
            if (mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
              ...
              if (pl == cl && pc == cc && (cl & 0x71)) {
                ...
                if (cl == 0x4 && !pcard->overlayed.empty()) {
                  mainGame->gMutex.lock();
      ...
      case MSG_CHAINING: {
        ...
        if (gui_replay["MSG_CHAINING"]) {
          ...
          string gui_replay_string_3 = gui_replay[202];
      ...
      case MSG_BECOME_TARGET: {
        ...
        for (int i = 0; i < count; ++i) {
          ...
          int ss = BufferIO::ReadUInt8(pbuf);
          ClientCard *pcard = mainGame->dField.GetCard(c, l, s);
          if (gui_replay["MSG_BECOME_TARGET"]) {
            fast_io::io::println(last_replay_txt, "* ", fast_io::mnp::cond(c == 0, "我方", "对方"), mainGame->get_location_string(l, s), "「", fast_io::mnp::code_cvt(fast_io::mnp::os_c_str(dataManager.GetName(pcard->code))), "」成为对象或目标");
          }
          pcard->is_highlighting = true;
      ...
      case MSG_DRAW: {
        ...
        for (int i = 0; i < count; ++i) {
          unsigned int code = BufferIO::ReadInt32(pbuf);
          if (gui_replay["MSG_DRAW"]) {
            fast_io::io::println(last_replay_txt, "* ", fast_io::mnp::cond(player == 0, "我方", "对方"), "从卡组中抽「", fast_io::mnp::code_cvt(fast_io::mnp::os_c_str(dataManager.GetName(code))), "」");
          }
        ...
        if (player == 0) {
          ...
          myswprintf(event_string, dataManager.GetSysString(1612), count);
        }
        return true;
      }
      case MSG_DAMAGE: {
      ...
      case MSG_ATTACK: {
        ...
        BufferIO::ReadUInt8(pbuf);
        if (ld == 0) {
          if (gui_replay["MSG_ATTACK"]) {
            fast_io::io::println(last_replay_txt, "* ", fast_io::mnp::cond(ca == 0, "我方", "对方"), mainGame->get_location_string(la, sa), "「", fast_io::mnp::code_cvt(fast_io::mnp::os_c_str(dataManager.GetName(mainGame->dField.attacker->code))), "」攻击", fast_io::mnp::cond(ca == 1, "我方", "对方"));
          }
        }
        else {
          if (gui_replay["MSG_ATTACK"]) {
            fast_io::io::println(last_replay_txt, "* ", fast_io::mnp::cond(ca == 0, "我方", "对方"), mainGame->get_location_string(la, sa), "「", fast_io::mnp::code_cvt(fast_io::mnp::os_c_str(dataManager.GetName(mainGame->dField.attacker->code))), "」攻击", fast_io::mnp::cond(cd == 0, "我方", "对方"), mainGame->get_location_string(ld, sd), "「", fast_io::mnp::code_cvt(fast_io::mnp::os_c_str(dataManager.GetName(mainGame->dField.GetCard(cd, ld, sd)->code))), "」");
          }
        }
        if (mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
          return true;
        ...
        if (ld != 0) {
          ...
          myswprintf(event_string, dataManager.GetSysString(1619), dataManager.GetName(mainGame->dField.attacker->code), dataManager.GetName(mainGame->dField.attack_target->code));
          ...
          sy = sqrt((xa - xd) * (xa - xd) + (ya - yd) * (ya - yd)) / 2;
      ...
      case MSG_ANNOUNCE_RACE: {
        ...
        for (int i = 0, filter = 0x1; i < RACES_COUNT; ++i, filter <<= 1) {
          ...
          if (filter & available) {
            ...
            mainGame->chkRace[i]->setVisible(false);
          }
        }
        if (select_hint) {
          ...
          myswprintf(textBuffer, dataManager.GetSysString(563));
        }
        select_hint = 0;
        mainGame->gMutex.lock();
      ...
      case MSG_ANNOUNCE_ATTRIB: {
        ...
        for (int i = 0, filter = 0x1; i < 7; ++i, filter <<= 1) {
          ...
          if (filter & available) {
            ...
            mainGame->chkAttribute[i]->setVisible(false);
          }
        }
        if (select_hint) {
          myswprintf(textBuffer, L"%ls", dataManager.GetDesc(select_hint));
          ...
          myswprintf(textBuffer, dataManager.GetSysString(562));
        }
        select_hint = 0;
        mainGame->gMutex.lock();
      ...
      case MSG_ANNOUNCE_CARD: {
        ...
        if (select_hint) {
          ...
          myswprintf(textBuffer, dataManager.GetSysString(564));
        }
        select_hint = 0;
        mainGame->gMutex.lock();
      ...
      case MSG_ANNOUNCE_NUMBER: {
        ...
        if (select_hint) {
          ...
          myswprintf(textBuffer, dataManager.GetSysString(565));
        }
        select_hint = 0;
      ...
      case MSG_TAG_SWAP: {
        int player = mainGame->LocalPlayer(BufferIO::ReadUInt8(pbuf));
        auto mcount = (size_t)BufferIO::ReadUInt8(pbuf);
        auto ecount = (size_t)BufferIO::ReadUInt8(pbuf);
        auto pcount = (size_t)BufferIO::ReadUInt8(pbuf);
        auto hcount = (size_t)BufferIO::ReadUInt8(pbuf);
        ...
        if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
          ...
          if (!mainGame->dField.deck[player].empty()) {
  ...
  void DuelClient::BroadcastReply(evutil_socket_t fd, short events, void *arg) {
    if (events & EV_TIMEOUT) {
      ...
      if (remotes.contains(remote)) {
        ...
        hoststr.append(L"][");
        if (pHP->host.draw_count == 1 && pHP->host.start_hand == 5 && pHP->host.start_lp == 8000 && !pHP->host.no_check_deck && !pHP->host.no_shuffle_deck && pHP->host.duel_rule == DEFAULT_DUEL_RULE) {
          hoststr.append(dataManager.GetSysString(1247));
        }
        else {
          hoststr.append(dataManager.GetSysString(1248));
        }
        hoststr.append(L"]");
        wchar_t gamename[20];
