...
function c76136345.filter(c, e, tp)
  return c:IsLevel(4) and c:IsAttackAbove(1800) and c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c76136345.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c76136345.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c76136345.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c76136345.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
