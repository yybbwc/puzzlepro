...
function c55838342.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 55838342, 0, TYPES_EFFECT_TRAP_MONSTER, 300, 2100, 2, RACE_INSECT, ATTRIBUTE_EARTH)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, e:GetHandler(), 1, 0, 0)
end
function c55838342.activate(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 55838342, 0, TYPES_EFFECT_TRAP_MONSTER, 300, 2100, 2, RACE_INSECT, ATTRIBUTE_EARTH) then
    return
  end
...
function c55838342.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55838342.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
