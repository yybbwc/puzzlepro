...
function c94431029.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMatchingGroupCount(Card.IsFacedown, tp, LOCATION_EXTRA, 0, nil) > 0 and Duel.GetMatchingGroupCount(Card.IsFacedown, tp, 0, LOCATION_EXTRA, nil) > 0
  end
end
function c94431029.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if res then
    if Duel.SendtoGrave(tc2, REASON_EFFECT) ~= 0 and tc2:IsLocation(LOCATION_GRAVE) and tc1:IsCanBeSpecialSummoned(e, 0, tp, false, false) then
    ...
    if Duel.SendtoGrave(tc1, REASON_EFFECT) ~= 0 and tc1:IsLocation(LOCATION_GRAVE) and tc2:IsCanBeSpecialSummoned(e, 0, 1 - tp, false, false, POS_FACEUP, 1 - tp) then
