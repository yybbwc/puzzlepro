...
function c80611581.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80611581.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c80611581.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if Duel.IsExistingMatchingCard(Card.IsSummonable, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, true, nil) and Duel.SelectYesNo(tp, aux.Stringid(80611581, 2)) then
      Duel.BreakEffect()
      Duel.ShuffleHand(tp)
...
function c80611581.limval(e, re, rp)
  ...
  return rc:IsLocation(LOCATION_MZONE) and re:IsActiveType(TYPE_MONSTER) and rc:IsAttackPos() and rc:IsSummonType(SUMMON_TYPE_SPECIAL)
end
...
function c80611581.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return bc and Duel.IsExistingMatchingCard(Card.IsAbleToRemoveAsCost, tp, LOCATION_HAND, 0, 1, nil) and c:GetFlagEffect(80611581) == 0
  end
