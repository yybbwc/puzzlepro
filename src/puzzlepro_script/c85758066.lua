...
function c85758066.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c85758066.filter(chkc, tp, ft)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > ft and Duel.IsExistingTarget(c85758066.filter, tp, 0, LOCATION_GRAVE, 1, nil, tp, ft)
  end
...
function c85758066.costfilter(c)
  return c:IsCode(83764718) and c:IsAbleToGraveAsCost() and (c:IsLocation(LOCATION_HAND) or (c:IsLocation(LOCATION_SZONE) and c:IsFacedown()))
end
function c85758066.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c85758066.costfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil)
  end
...
function c85758066.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c85758066.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c85758066.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) > 0 and Duel.GetTurnPlayer() == 1 - tp then
