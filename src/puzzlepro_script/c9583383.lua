...
function c9583383.rfilter(c, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c9583383.tfilter, tp, LOCATION_MZONE, 0, 1, nil, lv)
end
...
function c9583383.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9583383.rfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
