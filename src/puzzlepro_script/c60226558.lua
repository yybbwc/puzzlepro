...
function c60226558.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c60226558.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60226558.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c60226558.filter2(c, e, tp, m, ec, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x9d) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, ec, chkf)
end
...
function c60226558.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ec:IsControler(1 - tp) or ec:IsImmuneToEffect(e) then
    return
  end
  local chkf = tp
