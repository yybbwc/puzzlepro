...
function c68054593.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c68054593.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c68054593.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c68054593.atkcon(e)
  return (Duel.GetCurrentPhase() == PHASE_DAMAGE or Duel.GetCurrentPhase() == PHASE_DAMAGE_CAL) and Duel.IsExistingMatchingCard(c68054593.atkfilter, e:GetHandlerPlayer(), 0, LOCATION_MZONE, 1, nil)
end
