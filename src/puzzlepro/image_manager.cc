...
#pragma omp for schedule(dynamic)
      for (irr::s32 dy = 0; dy < destDim.Height; dy++) {
        for (irr::s32 dx = 0; dx < destDim.Width; dx++) {
          ...
          if (area > 0) {
            npxl.set((irr::u32)(aa / area + 0.5), (irr::u32)(ra / area + 0.5), (irr::u32)(ga / area + 0.5), (irr::u32)(ba / area + 0.5));
          }
          else {
            npxl.set(0);
          }
  ...
  irr::video::ITexture *ImageManager::GetBigPicture(int code, irr::f32 width, irr::f32 height) {
    if (code == 0) {
      return tUnknown;
    }
    if (tBigPicture != nullptr) {
