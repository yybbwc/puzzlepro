...
function c71098407.condition(e, tp, eg, ep, ev, re, r, rp)
  return rp == 1 - tp and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainDisablable(ev) and re:GetHandler():IsLocation(LOCATION_MZONE) and re:GetHandler():IsPosition(POS_FACEUP_ATTACK) and re:GetHandler():IsCanChangePosition()
end
