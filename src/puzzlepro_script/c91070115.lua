...
function c91070115.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 0, 1, 0x100e, 1, REASON_COST)
  end
...
function c91070115.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c91070115.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91070115.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
