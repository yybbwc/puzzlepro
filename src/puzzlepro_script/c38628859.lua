...
function c38628859.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c38628859.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c38628859.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) ~= 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) and Duel.GetLP(tp) <= 2000 then
    Duel.BreakEffect()
...
function c38628859.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c38628859.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c38628859.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c38628859.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
