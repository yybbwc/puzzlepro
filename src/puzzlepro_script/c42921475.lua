...
function c42921475.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c42921475.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c42921475.chcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c42921475.cfilter, 1, e:GetHandler())
  end
...
function c42921475.chtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c42921475.filter, rp, 0, LOCATION_MZONE, 1, nil)
  end
end
