...
function c80681226.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80681226.costfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
...
function c80681226.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c80681226.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c80681226.thfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(c80681226.dfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c80681226.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 and Duel.SendtoGrave(g, REASON_EFFECT) > 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) and tc:IsRelateToEffect(e) then
