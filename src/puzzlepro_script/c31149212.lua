...
function s.lvtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsLevelAbove(0) and not c:IsLevel(7) and tg:FilterCount(Card.IsAbleToRemove, nil, tp, POS_FACEDOWN) == 3
  end
...
function s.lvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Remove(tg, POS_FACEDOWN, REASON_EFFECT) > 0 and c:IsFaceup() and c:IsRelateToChain() then
    local e1 = Effect.CreateEffect(c)
