...
function c88093706.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return a:IsControler(tp) and a:IsRace(RACE_CYBERSE) or d and d:IsControler(tp) and d:IsRace(RACE_CYBERSE)
end
function c88093706.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:GetFlagEffect(88093706) == 0
  end
...
function c88093706.eftg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rc:IsRelateToEffect(e) and Duel.IsAbleToEnterBP()
  end
...
function c88093706.efop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not rc:IsRelateToChain() then
    return
  end
  local e1 = Effect.CreateEffect(c)
