...
function c24094258.tetg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c24094258.tefilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c24094258.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and chkc:IsFaceup() and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_ONFIELD, 0, 1, c) and Duel.IsExistingMatchingCard(c24094258.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
