...
function c35486099.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c35486099.filter(chkc)
  end
  if chk == 0 then
    if not Duel.IsExistingTarget(c35486099.filter, tp, LOCATION_GRAVE, 0, 1, nil) then
      return false
    end
