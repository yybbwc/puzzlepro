...
function c19580308.cfilter(c, ft)
  return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsSetCard(0xaf, 0xae) and not c:IsCode(19580308) and c:IsAbleToGraveAsCost() and (ft > 0 or c:GetSequence() < 5)
end
function c19580308.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c19580308.cfilter, tp, loc, 0, 1, nil, ft)
  end
