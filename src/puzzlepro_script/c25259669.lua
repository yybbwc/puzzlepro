...
function c25259669.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c25259669.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c25259669.sumop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
    ...
    if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) > 0 and c:IsRelateToEffect(e) and c:IsPosition(POS_FACEUP_ATTACK) then
      Duel.BreakEffect()
