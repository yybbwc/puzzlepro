...
function c7548747.eqfilter(c, check)
  return (c:IsFaceup() or not c:IsLocation(LOCATION_MZONE)) and c:IsSetCard(0x150) and c:IsType(TYPE_MONSTER) and not c:IsLevel(4) or check and c:IsLocation(LOCATION_EXTRA) and not c:IsSetCard(0x150) and c:IsType(TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ + TYPE_LINK)
end
...
function c7548747.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c7548747.filter(chkc, tp, check)
  end
  ...
  if chk == 0 then
    return ft > 0 and Duel.IsExistingTarget(c7548747.filter, tp, LOCATION_MZONE, 0, 1, nil, tp, check)
  end
