static int my_lua_panic(lua_State *L) {
  const char *errmsg = lua_tostring(L, -1); // 获取错误信息
  ...
  return 0; // 返回后 Lua 会调用 exit(EXIT_FAILURE)
}
...
interpreter::interpreter(duel *pd) : coroutines(256) {
  ...
  load_script("./PuzzlePro_script/constant.lua");
  load_script("./PuzzlePro_script/utility.lua");
  load_script("./PuzzlePro_script/procedure.lua");
  lua_atpanic(lua_state, my_lua_panic);
}
...
int32_t interpreter::load_script(const char *script_name) {
  ...
  if (error) {
    fast_io::io::print(fast_io::win32_box_t(), fast_io::mnp::os_c_str(script_name), "\n", fast_io::mnp::os_c_str(lua_tostring(current_state, -1)), "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
    //~ my_lua_panic(current_state);
...
int32_t interpreter::load_card_script(uint32_t code) {
  ...
  if (lua_isnil(current_state, -1)) {
    ...
    if (is_load_script(cdata)) {
      ...
      sprintf(script_name, "./PuzzlePro_script/c%d.lua", code);
...
int32_t interpreter::check_filter(lua_State *L, card *pcard, int32_t findex, int32_t extraargs) {
  ...
  //~ fast_io::io::print(fast_io::win32_box_t(), lua_gettop(L), "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
