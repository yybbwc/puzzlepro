...
function c63612442.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsSetCard, 1, e:GetHandler(), 0x100d)
  end
...
function c63612442.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(63612442) == 0 or c:GetFlagEffect(63612443) == 0
  end
  ...
  elseif t1 == 0 then
    op = Duel.SelectOption(tp, aux.Stringid(63612442, 1))
  else
    Duel.SelectOption(tp, aux.Stringid(63612442, 2))
    op = 1
  end
  e:SetLabel(op)
  if op == 0 then
    c:RegisterFlagEffect(63612442, RESET_EVENT + RESETS_STANDARD + RESET_PHASE + PHASE_END, 0, 1)
  else
    c:RegisterFlagEffect(63612443, RESET_EVENT + RESETS_STANDARD + RESET_PHASE + PHASE_END, 0, 1)
  end
end
