...
function c24694698.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c24694698.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c24694698.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c24694698.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c24694698.filter, tp, LOCATION_GRAVE, 0, 2, nil) and Duel.IsPlayerCanDraw(tp, 1)
  end
