...
function c23379054.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(23379054, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c23379054.filter1(c, e, tp, lv, mc)
  return c:IsType(TYPE_FUSION) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c23379054.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, lv + c:GetLevel(), mc)
end
function c23379054.filter2(c, e, tp, lv, mc)
  return c:IsLevel(lv) and c:IsRace(RACE_DRAGON) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c23379054.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c23379054.filter1(chkc, e, tp, c:GetLevel(), c)
  end
  if chk == 0 then
    return c:IsAbleToRemove() and Duel.IsExistingTarget(c23379054.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetLevel(), c)
  end
