...
function c53490455.rectg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetAttacker():IsOnField() and Duel.GetAttacker():IsCanBeEffectTarget(e) and Duel.GetAttackTarget():IsOnField() and Duel.GetAttackTarget():IsCanBeEffectTarget(e)
  end
...
function c53490455.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and tc:IsLocation(LOCATION_GRAVE) and tc:IsReason(REASON_BATTLE) and bc:IsRelateToBattle() and bc:IsControler(tp) and bc:IsSetCard(0x119)
end
