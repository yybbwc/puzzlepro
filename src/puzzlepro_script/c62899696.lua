...
function c62899696.filter(tc, c, tp)
  if tc:IsFacedown() or not tc:IsCanBeSynchroMaterial() then
    return false
  end
...
function c62899696.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c62899696.filter(chkc, e:GetHandler(), tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62899696.filter, tp, 0, LOCATION_MZONE, 1, nil, e:GetHandler(), tp)
  end
