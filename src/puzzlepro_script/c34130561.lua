...
function c34130561.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 or Duel.IsExistingMatchingCard(c34130561.ntfilter, tp, 0, LOCATION_MZONE, 1, nil))
end
...
function c34130561.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c34130561.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
