...
function c27036706.cfilter(c, tp)
  return c:IsAbleToHandAsCost() and Duel.GetMZoneCount(tp, c) > 0 and (c:IsSetCard(0x159) and not c:IsCode(27036706) or c:IsRace(RACE_DRAGON) and c:IsLevel(7) and c:IsAttribute(ATTRIBUTE_LIGHT))
end
function c27036706.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27036706.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c27036706.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c27036706.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
