...
function c46261704.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c46261704.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c46261704.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 and tc:IsLocation(LOCATION_MZONE) then
    ...
    if a:IsAttackable() and not a:IsImmuneToEffect(e) then
      ...
      if Duel.ChangeAttackTarget(tc) and mg:GetCount() > 0 and c:IsRelateToChain() and c:IsLocation(LOCATION_GRAVE) and aux.NecroValleyFilter()(c) and c:IsCanOverlay() and Duel.SelectYesNo(tp, aux.Stringid(46261704, 2)) then
        Duel.BreakEffect()
...
function c46261704.repfilter(c, tp)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsSetCard(0x55, 0x7b) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)) and not c:IsReason(REASON_REPLACE)
end
function c46261704.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c46261704.repfilter, 1, nil, tp)
  end
