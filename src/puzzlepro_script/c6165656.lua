...
function c6165656.splimit(e, se, sp, st)
  return se:GetHandler():IsSetCard(0x95) and se:GetHandler():IsType(TYPE_SPELL) and se:IsHasProperty(EFFECT_FLAG_CARD_TARGET)
end
...
function c6165656.winop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetTurnPlayer() == tp and Duel.GetCurrentPhase() == PHASE_END and Duel.GetLP(1 - tp) <= 2000 and e:GetHandler():GetOverlayCount() == 0 then
