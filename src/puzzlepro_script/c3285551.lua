...
function c3285551.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(3285551, tp, ACTIVITY_CHAIN) == 0
  end
...
function c3285551.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 3285552, 0, TYPES_TOKEN_MONSTER, 2000, 2000, 4, RACE_FAIRY, ATTRIBUTE_EARTH)
  end
...
function c3285551.operation(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 3285552, 0, TYPES_TOKEN_MONSTER, 2000, 2000, 4, RACE_FAIRY, ATTRIBUTE_EARTH) then
    return
  end
  ...
  if not Duel.IsExistingMatchingCard(c3285551.cfilter, tp, LOCATION_SZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and g:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(3285551, 0)) then
    Duel.BreakEffect()
