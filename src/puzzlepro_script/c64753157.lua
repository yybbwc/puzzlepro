...
function c64753157.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not d or a:GetControler() == d:GetControler() or d:IsFacedown() or a:IsFacedown() then
    return
  end
  if a:IsControler(tp) and a:IsCode(41091257) then
    e:SetLabelObject(d)
  elseif d:IsControler(tp) and d:IsCode(41091257) then
    e:SetLabelObject(a)
  else
    return false
  end
...
function c64753157.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c64753157.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c64753157.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c64753157.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
