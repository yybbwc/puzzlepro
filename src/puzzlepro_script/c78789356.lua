...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsPlayerCanSpecialSummonMonster(tp, id + o, 0x161, TYPES_TOKEN_MONSTER, 0, 0, 2, RACE_REPTILE, ATTRIBUTE_DARK)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ct <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, id + o, 0x161, TYPES_TOKEN_MONSTER, 0, 0, 2, RACE_REPTILE, ATTRIBUTE_DARK) then
    return
  end
  repeat
...
function s.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and s.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function s.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc and tc:IsRelateToEffect(e)) or Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) == 0 or not tc:IsLocation(LOCATION_DECK + LOCATION_EXTRA) then
    return
  end
  Duel.ShuffleDeck(tp)
