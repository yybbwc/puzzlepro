...
function c28355718.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 28355719, 0, TYPES_TOKEN_MONSTER, 1000, 1000, 4, RACE_WARRIOR, ATTRIBUTE_DARK)
  end
...
function c28355718.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 28355719, 0, TYPES_TOKEN_MONSTER, 1000, 1000, 4, RACE_WARRIOR, ATTRIBUTE_DARK) then
    return
  end
  for i = 1, 2 do
