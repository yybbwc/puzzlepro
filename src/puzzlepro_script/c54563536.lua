...
function c54563536.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c54563536.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c54563536.atkfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c54563536.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rc:IsRelateToBattle() and rc:IsStatus(STATUS_OPPO_BATTLE) and rc:IsFaceup() and rc:IsSetCard(0x36) and rc:IsControler(tp)
end
