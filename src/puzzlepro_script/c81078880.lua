...
function c81078880.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c81078880.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c81078880.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c81078880.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81078880.atkfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
