...
function c99011763.ctcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c99011763.cfilter, 1, nil)
  end
...
function c99011763.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsCanAddCounter(0x1049, 1)
  end
  if chk == 0 then
    return true
  end
...
function c99011763.ctop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) then
    ...
    if tc:GetFlagEffect(99011764) ~= 0 then
      return
    end
...
function c99011763.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and c:IsCanRemoveCounter(tp, 0x1049, 1, REASON_EFFECT)
  end
  return true
end
