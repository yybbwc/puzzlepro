...
function c76833149.ovtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c76833149.ovfilter, tp, LOCATION_MZONE + LOCATION_HAND, 0, 1, e:GetHandler())
  end
end
...
function c76833149.damcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:GetOverlayCount() >= 5 and bc and bc:IsPosition(POS_FACEUP_ATTACK) and (Duel.GetAttacker() == c or Duel.GetAttackTarget() == c)
end
