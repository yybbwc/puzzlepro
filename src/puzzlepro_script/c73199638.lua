...
function c73199638.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c73199638.filter(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and Duel.IsExistingTarget(c73199638.filter, tp, LOCATION_GRAVE, 0, 3, nil, e, tp)
  end
...
function c73199638.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:FilterCount(c73199638.rfilter, nil, e, tp) ~= 3 then
    return
  end
