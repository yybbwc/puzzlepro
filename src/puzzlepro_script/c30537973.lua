...
function c30537973.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and chkc ~= c and c30537973.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30537973.desfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, c)
  end
...
function c30537973.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(30537973) ~= 0 and Duel.IsExistingMatchingCard(c30537973.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c30537973.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30537973.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
