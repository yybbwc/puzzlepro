...
function s.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  e:SetLabel(100)
  if chk == 0 then
    return c:IsReleasable() and Duel.CheckReleaseGroupEx(tp, s.cfilter, 1, REASON_COST, true, c, tp)
  end
...
function s.thfilter(c, e, tp, el)
  if not (c:IsLevel(7) and c:IsRace(RACE_AQUA + RACE_FISH + RACE_SEASERPENT)) then
    return false
  end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, e:GetLabel())
  end
end
...
function s.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return aux.bfgcost(e, tp, eg, ep, ev, re, r, rp, chk) and Duel.IsExistingMatchingCard(s.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
