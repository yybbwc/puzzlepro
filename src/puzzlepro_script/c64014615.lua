...
function c64014615.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c64014615.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c64014615.filter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 3, nil)
  end
...
function c64014615.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:IsExists(c64014615.sfilter, 1, nil, tp) then
    Duel.ShuffleDeck(tp)
  end
  if g:IsExists(c64014615.sfilter, 1, nil, 1 - tp) then
