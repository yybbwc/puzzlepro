...
function c2602411.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c2602411.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c2602411.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c2602411.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c2602411.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c2602411.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
