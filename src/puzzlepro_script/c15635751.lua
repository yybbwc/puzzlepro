...
function s.filter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x107a) and c:IsControler(tp) and c:IsOnField() and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
...
function s.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:IsExists(s.filter, 1, nil, tp) and #g > 0
  end
...
function s.rptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsAbleToRemove() and Duel.IsExistingMatchingCard(s.pfilter, tp, LOCATION_DECK + LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, tp)
  end
...
function s.sfilter(c, e, tp)
  ...
  elseif c:IsSetCard(0x207a) then
...
function s.rpop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if Duel.Remove(c, POS_FACEUP, REASON_EFFECT + REASON_TEMPORARY) == 0 or not c:IsLocation(LOCATION_REMOVED) then
    return
  end
  ...
  if not (tc and Duel.MoveToField(tc, tp, tp, LOCATION_FZONE, POS_FACEUP, true)) then
    return
  end
