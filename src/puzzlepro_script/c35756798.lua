...
function c35756798.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c35756798.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c35756798.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c35756798.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() then
    ...
    if num > 0 and Duel.IsExistingMatchingCard(c35756798.atkfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(35756798, 1)) then
