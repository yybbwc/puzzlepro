...
function c7150545.tgfilter(c, e, tp)
  return c:IsFaceup() and c:IsAttribute(ATTRIBUTE_WATER) and c:IsLevelAbove(1) and Duel.IsExistingMatchingCard(c7150545.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetLevel(), c:GetCode())
end
...
function c7150545.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c7150545.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c7150545.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
