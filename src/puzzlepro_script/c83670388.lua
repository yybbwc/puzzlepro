...
function c83670388.actcon(e)
  return Duel.IsExistingMatchingCard(c83670388.actfilter, 0, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.IsEnvironment(7142724)
end
function c83670388.aclimit(e, re, tp)
  local rc = re:GetHandler()
  return re:IsActiveType(TYPE_MONSTER) and rc:IsLocation(LOCATION_MZONE) and rc:IsStatus(STATUS_SUMMON_TURN + STATUS_FLIP_SUMMON_TURN + STATUS_SPSUMMON_TURN)
end
...
function c83670388.damtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c83670388.filter(chkc, e, tp)
  end
  if chk == 0 then
    return eg:IsExists(c83670388.damfilter, 1, nil, e)
  end
  local g = eg
  if #eg > 1 then
