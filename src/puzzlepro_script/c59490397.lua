...
function c59490397.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c59490397.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c59490397.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c59490397.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsType(TYPE_LINK) and c:IsSetCard(0x10b)
end
...
function c59490397.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  local tc = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return tc:GetLinkedGroupCount() > 0
  end
...
function c59490397.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not tc then
    return
  end
