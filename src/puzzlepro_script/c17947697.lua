...
function s.pcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToGraveAsCost() and e:GetHandler():IsAbleToGraveAsCost()
  end
...
function s.ptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(s.pfilter, tp, LOCATION_DECK + LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, tp)
  end
end
...
function s.cfilter(c, tp, se)
  return c:IsFaceup() and c:IsSummonPlayer(tp) and c:IsCode(89631139) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not aux.NecroValleyFilter()(c) then
    return
  end
...
function s.spfilter(c, e, tp)
  return (c:IsCode(89631139) or c:IsLevel(1) and c:IsType(TYPE_TUNER) and c:IsAttribute(ATTRIBUTE_LIGHT)) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
