...
function c82529174.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c82529174.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82529174.filter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
