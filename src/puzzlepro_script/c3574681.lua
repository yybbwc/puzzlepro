...
function c3574681.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > 1 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:CheckSubGroup(c3574681.gcheck, 2, 2, e, tp)
  end
...
function c3574681.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft > 1 then
    ...
    local bc = sg:GetNext()
    if Duel.MoveToField(ac, tp, tp, LOCATION_SZONE, POS_FACEUP, true) and Duel.MoveToField(bc, tp, tp, LOCATION_SZONE, POS_FACEUP, true) then
      local e1 = Effect.CreateEffect(c)
