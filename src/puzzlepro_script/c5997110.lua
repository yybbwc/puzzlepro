...
function s.gcheck(g)
  return (Auxiliary.SameValueCheck(g, Card.GetRace) or Auxiliary.SameValueCheck(g, Card.GetAttribute)) and g:IsExists(s.atkfilter, 1, nil, g)
end
...
function s.thfilter(c)
  return c:IsSetCard(0xaf) and c:IsType(TYPE_MONSTER) and (c:IsAttack(0) or c:IsDefense(0)) and c:IsAbleToHand()
end
