...
function c75498415.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(c:GetControler(), LOCATION_MZONE, 0) == 0 and Duel.GetFieldGroupCount(c:GetControler(), 0, LOCATION_MZONE) > 0
end
...
function c75498415.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c75498415.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c75498415.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
