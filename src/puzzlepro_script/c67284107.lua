...
function c67284107.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 67284108, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_ZOMBIE, ATTRIBUTE_DARK)
  end
...
function c67284107.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 67284108, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_ZOMBIE, ATTRIBUTE_DARK) then
    return
  end
  repeat
