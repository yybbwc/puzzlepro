...
function c78785392.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(78785392, tp, ACTIVITY_SUMMON) == 0 and Duel.GetCustomActivityCount(78785392, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c78785392.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 3 and Duel.GetMatchingGroup(c78785392.filter, tp, LOCATION_DECK, 0, nil, e, tp):GetClassCount(Card.GetCode) > 3
  end
...
function c78785392.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerAffectedByEffect(tp, 59822133) and ct > 3 and g:GetClassCount(Card.GetCode) > 3 then
