...
function c94415058.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if eg:GetCount() == 1 and rp == 1 - tp and tc:IsReason(REASON_EFFECT) and tc:IsPreviousControler(tp) and tc:IsPreviousLocation(LOCATION_MZONE) and tc:IsPreviousPosition(POS_FACEUP) and tc:IsType(TYPE_PENDULUM) and tc:IsControler(tp) then
...
function c94415058.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c94415058.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp, e:GetLabel())
  end
