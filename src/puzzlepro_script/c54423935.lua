...
function c54423935.filter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0xba) and Duel.IsExistingMatchingCard(c54423935.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c54423935.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c54423935.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
