...
function c66752837.cfilter(c)
  return c:IsRace(RACE_DRAGON) and c:GetPreviousRaceOnField() == RACE_DRAGON and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsCode(66752837)
end
...
function c66752837.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and e:GetLabel() == 1 and g:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(66752837, 0)) then
    Duel.BreakEffect()
