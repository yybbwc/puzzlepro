...
function c58092907.spfilter(c, tp, rp)
  return (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and rp == 1 - tp)) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsSummonLocation(LOCATION_EXTRA) and bit.band(c:GetPreviousTypeOnField(), TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ) ~= 0 and c:IsPreviousPosition(POS_FACEUP)
end
...
function c58092907.spfilter2(c, e, tp)
  if c:IsLocation(LOCATION_HAND + LOCATION_DECK) or (not c:IsLocation(LOCATION_GRAVE) and c:IsFacedown()) or not c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or c:IsControler(1 - tp) then
    return false
  end
...
function c58092907.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c58092907.spfilter2(ec, e, tp)
  end
...
function c58092907.metg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58092907.mefilter1, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) or (Duel.IsExistingMatchingCard(c58092907.mefilter2, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_DECK, 0, 1, nil, TYPE_PENDULUM))
  end
...
function c58092907.meop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local b4 = Duel.IsExistingMatchingCard(c58092907.mefilter2, tp, LOCATION_MZONE, 0, 1, aux.ExceptThisCard(e)) and Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_DECK, 0, 1, nil, TYPE_PENDULUM)
  if b1 then
    local e1 = Effect.CreateEffect(c)
