...
function c50903514.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c50903514.filter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():IsAttackAbove(600) and Duel.IsExistingTarget(c50903514.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c50903514.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFacedown() or not c:IsRelateToEffect(e) or c:GetAttack() < 600 or tc:IsFacedown() or not tc:IsRelateToEffect(e) then
    return
  end
  local e1 = Effect.CreateEffect(c)
...
function c50903514.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c50903514.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c50903514.spfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
