...
function c65503206.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(65503206) ~= 0
  end
...
function c65503206.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c65503206.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c65503206.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
