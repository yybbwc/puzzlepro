...
function c11881272.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c11881272.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c11881272.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c11881272.acttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c11881272.actfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, tp)
  end
...
function c11881272.actop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabel() == 1 then
    Duel.RegisterFlagEffect(tp, 11881272, RESET_CHAIN, 0, 1)
  end
