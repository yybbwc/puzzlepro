...
function c8437145.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c8437145.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c8437145.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c8437145.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0xbb) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c8437145.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c8437145.repfilter, 1, nil, tp)
  end
