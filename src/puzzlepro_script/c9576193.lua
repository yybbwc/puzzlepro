...
function c9576193.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c9576193.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c9576193.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsFacedown() or not tc1:IsRelateToEffect(e) then
    return
  end
  if not tc2:IsRelateToEffect(e) or not tc2:IsLevel(tc1:GetLevel()) or Duel.Remove(tc2, POS_FACEUP, REASON_EFFECT) == 0 then
    return
  end
  Duel.BreakEffect()
