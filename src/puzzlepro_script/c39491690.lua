...
function s.cfilter(c)
  return c:IsFaceupEx() and c:IsRace(RACE_FIEND + RACE_ILLUSION + RACE_SPELLCASTER) and c:IsType(TYPE_FUSION + TYPE_SYNCHRO)
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return ct > 0 and Duel.IsExistingTarget(Card.IsAbleToHand, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
