...
function c28645123.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c28645123.spfilter(chkc, e, tp) and not eg:IsContains(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c28645123.spfilter, tp, LOCATION_GRAVE, 0, 1, eg, e, tp)
  end
