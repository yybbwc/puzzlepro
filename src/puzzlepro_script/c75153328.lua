...
function c75153328.filter1(c)
  ...
  return c:IsFaceup() and not c:IsType(TYPE_LINK) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function c75153328.filter2(c)
  local tp = c:GetControler()
  return c:IsFaceup() and c:IsType(TYPE_LINK) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function c75153328.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c75153328.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c75153328.filter2, tp, 0, LOCATION_MZONE, 1, nil)
  end
