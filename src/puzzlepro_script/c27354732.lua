...
function c27354732.spfilter(c, e, tp)
  return c:IsSetCard(0x10d) and c:IsFaceup() and c:IsType(TYPE_PENDULUM) and not c:IsCode(27354732) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c27354732.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsDestructable() and Duel.IsExistingMatchingCard(c27354732.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
