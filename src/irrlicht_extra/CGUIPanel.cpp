  ...
  namespace gui {
    ...
    bool CGUIPanel::OnEvent(const SEvent &event) {
      ...
      if (event.EventType == EET_MOUSE_INPUT_EVENT && event.MouseInput.Event == EMIE_MOUSE_WHEEL) {
        ...
        if (event.EventType == EET_GUI_EVENT && event.GUIEvent.EventType == EGET_SCROLL_BAR_CHANGED && (event.GUIEvent.Caller == HScrollBar || event.GUIEvent.Caller == VScrollBar)) {
          moveInnerPane();
    ...
    void CGUIPanel::resizeInnerPane() {
      ...
      for (it = InnerPane->getChildren().begin(); it != InnerPane->getChildren().end(); ++it) {
      ...
      // scrollbars
      if (HScrollBarMode != ESBM_ALWAYS_INVISIBLE && (totalRect.getWidth() > outerRect.getWidth() || HScrollBarMode == ESBM_ALWAYS_VISIBLE)) {
      ...
      if (VScrollBarMode != ESBM_ALWAYS_INVISIBLE && (totalRect.getHeight() > outerRect.getHeight() || VScrollBarMode == ESBM_ALWAYS_VISIBLE)) {
