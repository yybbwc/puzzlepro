...
function c78225596.chkfilter(c, tp, g)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsSetCard(0x14b) and c:IsType(TYPE_LINK) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not g:IsContains(c)
end
...
function c78225596.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(ac)
  return ac:IsControler(tp) and ac:IsFaceup() and ac:IsType(TYPE_LINK) and ac:IsSetCard(0x14b) and g:IsContains(ac) and ac:IsRelateToBattle() and bc:IsControler(1 - tp)
end
