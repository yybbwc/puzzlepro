...
function c97946536.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97946536.tgcostfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
...
function c97946536.opfilter(c, e, tp, spchk, eqchk)
  return c:IsSetCard(0x56) and c:IsType(TYPE_MONSTER) and (spchk and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or eqchk and c:CheckUniqueOnField(tp) and not c:IsForbidden())
end
...
function c97946536.optg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    local eqchk = Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c97946536.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
...
function c97946536.opop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc then
    if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and spchk and (not eqchk or Duel.SelectOption(tp, 1152, aux.Stringid(97946536, 2)) == 0) then
      ...
      if sc then
        if not Duel.Equip(tp, tc, sc) then
          return
        end
        --equip limit
...
function c97946536.eqfilter(c, tp)
  return c:IsSetCard(0x56) and c:IsType(TYPE_EQUIP) and c:CheckUniqueOnField(tp) and not c:IsForbidden() and Duel.IsExistingMatchingCard(c97946536.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, c)
end
...
function c97946536.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c97946536.eqfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
