...
function c60110982.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local g = Duel.GetMatchingGroup(Card.IsAbleToRemove, tp, LOCATION_ONFIELD, 0, nil)
  if #g > 0 and Duel.Remove(g, POS_FACEUP, REASON_EFFECT) ~= 0 and g:IsExists(Card.IsLocation, 1, nil, LOCATION_REMOVED) and Duel.IsExistingMatchingCard(c60110982.spfilter, p, LOCATION_EXTRA, 0, 1, nil, e, p) and Duel.SelectYesNo(p, aux.Stringid(60110982, 2)) then
    Duel.BreakEffect()
