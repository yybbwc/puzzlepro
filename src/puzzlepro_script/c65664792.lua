...
function c65664792.spfilter1(c, e, tp)
  return c65664792.spfilter(c, e, tp) and Duel.IsExistingMatchingCard(c65664792.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, c)
end
function c65664792.spfilter2(c, e, tp, c1)
  return c65664792.spfilter(c, e, tp) and not c:IsOriginalCodeRule(c1:GetOriginalCodeRule()) and Duel.IsExistingMatchingCard(c65664792.spfilter3, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c1, c)
end
function c65664792.spfilter3(c, e, tp, c1, c2)
  return c65664792.spfilter(c, e, tp) and not c:IsOriginalCodeRule(c1:GetOriginalCodeRule()) and not c:IsOriginalCodeRule(c2:GetOriginalCodeRule())
end
function c65664792.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c65664792.tdfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c65664792.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #dg > 0 and Duel.SendtoDeck(dg, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) >= 3 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingMatchingCard(c65664792.spfilter1, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(65664792, 2)) then
