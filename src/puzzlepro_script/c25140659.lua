...
function c25140659.filter1(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x11e) and c:IsAbleToHand() and Duel.IsExistingTarget(c25140659.filter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
...
function c25140659.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c25140659.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c25140659.tdcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c25140659.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
