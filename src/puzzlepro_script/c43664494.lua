...
function c43664494.tkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c43664494.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
  Duel.DiscardHand(tp, c43664494.cfilter, 1, 1, REASON_COST + REASON_DISCARD)
end
function c43664494.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 43664495, 0x120, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_PYRO, ATTRIBUTE_FIRE, POS_FACEUP)
  end
...
function c43664494.tkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 43664495, 0x120, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_PYRO, ATTRIBUTE_FIRE, POS_FACEUP) then
    return
  end
...
function c43664494.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c43664494.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c43664494.tdfilter, tp, LOCATION_GRAVE, 0, 3, nil)
  end
