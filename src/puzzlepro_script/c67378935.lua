...
function c67378935.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(67378935, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c67378935.cfilter(c, e, tp)
  return c:IsFaceup() and c:IsLevelAbove(1) and Duel.IsExistingMatchingCard(c67378935.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetLevel())
end
...
function c67378935.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c67378935.cfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c67378935.cfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c67378935.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c67378935.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67378935.thfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
