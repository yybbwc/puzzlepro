...
function c66574418.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c66574418.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c66574418.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c66574418.tdfilter(chkc)
  end
  if chk == 0 then
    return e:GetHandler():IsAbleToHand() and Duel.IsExistingTarget(c66574418.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
