...
function c80839052.spfilter(c, tp)
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and (c:GetPreviousRaceOnField() & RACE_ROCK) > 0 and c:IsRace(RACE_ROCK)
end
...
function c80839052.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80839052.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
