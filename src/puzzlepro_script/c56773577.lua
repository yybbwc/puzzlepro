...
function c56773577.atkcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c56773577.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.IsEnvironment(47355498)
end
function c56773577.accon(e, tp, eg, ep, ev, re, r, rp)
  return (Duel.GetCurrentPhase() == PHASE_MAIN1 or Duel.GetCurrentPhase() == PHASE_MAIN2) and not Duel.IsExistingMatchingCard(aux.TRUE, tp, LOCATION_FZONE, 0, 1, nil)
end
...
function c56773577.actg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56773577.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, tp)
  end
end
...
function c56773577.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56773577.setfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
