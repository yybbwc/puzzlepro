...
function c62645025.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 62645025, 0x10db, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 2, RACE_WARRIOR, ATTRIBUTE_DARK, POS_FACEUP_ATTACK)
  end
...
function c62645025.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not Duel.IsPlayerCanSpecialSummonMonster(tp, 62645025, 0x10db, TYPES_EFFECT_TRAP_MONSTER, 0, 0, 2, RACE_WARRIOR, ATTRIBUTE_DARK, POS_FACEUP_ATTACK) then
    return
  end
...
function c62645025.cfilter(c)
  if c:IsFacedown() or not c:IsAbleToGraveAsCost() then
    return false
  end
  return (c:IsSetCard(0x10db) and c:IsType(TYPE_MONSTER)) or (c:IsSetCard(0xdb) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsType(TYPE_CONTINUOUS))
end
function c62645025.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c62645025.cfilter, tp, LOCATION_ONFIELD, 0, 1, c)
  end
