...
function c42429678.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c42429678.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c42429678.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return mg:IsContains(chkc) and c42429678.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and mg:IsExists(c42429678.spfilter, 1, nil, e, tp)
  end
