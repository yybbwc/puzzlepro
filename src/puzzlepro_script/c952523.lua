...
function c952523.setcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.GetFlagEffect(tp, 952523) == 0 and rp == tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_TRAP) and rc:GetType() == TYPE_TRAP and rc:IsRelateToEffect(re) and rc:IsCanTurnSet() and rc:IsStatus(STATUS_LEAVE_CONFIRMED) and rc:IsSetCard(0x4c, 0x89)
end
...
function c952523.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c952523.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c952523.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
