...
function c96699830.cfilter(c)
  return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsAbleToRemoveAsCost()
end
function c96699830.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetCount() > 0 and Duel.GetActivityCount(tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c96699830.spfilter(c, e, tp)
  return c:IsLevelAbove(6) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false)
end
function c96699830.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c96699830.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
