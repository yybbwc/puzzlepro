...
function c85250352.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85250352.costfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c85250352.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c85250352.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c85250352.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c85250352.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c85250352.disfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingTarget(c85250352.desfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
