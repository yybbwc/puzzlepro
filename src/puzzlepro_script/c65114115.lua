...
function s.cfilter(c, tp)
  return c:IsFaceup() and Duel.IsExistingMatchingCard(s.eqfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, c:GetCode(), c:GetRace(), tp)
end
function s.eqfilter(c, code, race, tp)
  return not c:IsCode(code) and c:IsType(TYPE_MONSTER) and c:IsLevel(7) and c:IsRace(race) and c:CheckUniqueOnField(tp) and not c:IsForbidden()
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and Duel.GetLocationCount(tp, LOCATION_SZONE) > ct
  end
