...
function c94243005.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c94243005.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c94243005.spfilter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil, e, tp)
  end
...
function c94243005.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabel(ct)
  return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousControler(tp) and ct > 0 and rp == 1 - tp and bit.band(r, REASON_EFFECT) ~= 0
end
...
function c94243005.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94243005.thfilter, tp, LOCATION_DECK, 0, 1, nil, e:GetLabel())
  end
