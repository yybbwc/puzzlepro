...
function c69313735.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c69313735.filter(chkc)
  end
  if chk == 0 then
    if e:GetLabel() == 0 then
      return Duel.IsExistingTarget(c69313735.filter, tp, LOCATION_MZONE, 0, 1, nil)
    end
    e:SetLabel(0)
