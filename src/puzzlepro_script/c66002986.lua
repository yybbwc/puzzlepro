...
function s.rmfilter(c)
  return c:IsFaceup() and c:IsType(TYPE_SYNCHRO) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsAbleToRemove()
end
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.rmfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
