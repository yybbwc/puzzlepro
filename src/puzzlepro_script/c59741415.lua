...
function c59741415.desfilter(c, e, tp)
  return c:IsFaceup() and c:IsRace(RACE_MACHINE) and Duel.IsExistingMatchingCard(c59741415.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c59741415.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c59741415.desfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c59741415.desfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c59741415.drtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c59741415.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingTarget(c59741415.tdfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 3, nil)
  end
