...
function c92932860.scfilter1(c, e, tp, mc)
  ...
  return c:IsLevel(1) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c92932860.scfilter2, tp, LOCATION_EXTRA, 0, 1, nil, mg)
end
...
function c92932860.sctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92932860.scfilter1(chkc, e, tp, c)
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c92932860.scfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c)
  end
