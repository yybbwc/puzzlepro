...
function c53535814.regop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ep == 1 - tp and re:IsActivated() and Duel.IsExistingMatchingCard(c53535814.cfilter, tp, LOCATION_MZONE, 0, 1, c) then
...
function c53535814.llpop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if ep == 1 - tp and c:GetFlagEffect(53535814) > 0 and Duel.IsExistingMatchingCard(c53535814.cfilter, tp, LOCATION_MZONE, 0, 1, c) then
...
function c53535814.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c53535814.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsAbleToDeck, tp, 0, LOCATION_GRAVE, 1, nil)
  end
...
function c53535814.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c53535814.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c53535814.posfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
