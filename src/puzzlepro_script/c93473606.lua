...
function c93473606.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c93473606.filter(chkc, tp)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c93473606.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function c93473606.swtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return ec and ec:IsDefenseAbove(0)
  end
end
