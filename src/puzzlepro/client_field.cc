...
namespace ygo {
  ...
  void ClientField::ShowSelectCard(bool buttonok, bool chain) {
    ...
    if (selectable_cards.size() <= 5) {
      startpos = gui_xy["btnCardSelect"]["x1"].cast<double>().value() + gui_xy["btnCardSelect"]["width_offset"].cast<double>().value() * (5 - selectable_cards.size()) / 2;
    ...
    for (int i = 0; i < ct; ++i) {
      ...
      // image
      mainGame->btnCardSelect[i]->setImageSize(irr::core::dimension2di(gui_xy["btnCardSelect"]["width_reduce"].cast<double>().value() * mainGame->xScale, gui_xy["btnCardSelect"]["height_reduce"].cast<double>().value() * mainGame->yScale));
      if (selectable_cards[i]->code) {
        mainGame->imageLoading.insert_or_assign(mainGame->btnCardSelect[i], selectable_cards[i]->code);
      }
      ...
      if (mainGame->dInfo.curMsg != MSG_SORT_CARD) {
        ...
        if (conti_selecting) {
          ...
          myswprintf(formatBuffer, L"%ls[%d](%d)", dataManager.FormatLocation(selectable_cards[i]->overlayTarget->location, selectable_cards[i]->overlayTarget->sequence), selectable_cards[i]->overlayTarget->sequence + 1, selectable_cards[i]->sequence + 1);
        }
        else {
          myswprintf(formatBuffer, L"%ls[%d]", dataManager.FormatLocation(selectable_cards[i]->location, selectable_cards[i]->sequence), selectable_cards[i]->sequence + 1);
        }
      ...
      mainGame->stCardPos[i]->setRelativePosition(mainGame->ResizeWin(startpos + i * gui_xy["stCardPos"]["width_offset"].cast<int32_t>().value(), gui_xy["stCardPos"]["y1"], startpos + gui_xy["stCardPos"]["width"].cast<int32_t>().value() + i * gui_xy["stCardPos"]["width_offset"].cast<int32_t>().value(), gui_xy["stCardPos"]["y2"]));
    }
    if (selectable_cards.size() <= 5) {
  ...
  void ClientField::ShowLocationCard() {
    ...
    for (int i = 0; i < ct; ++i) {
      ...
      if (display_cards[i]->location == LOCATION_OVERLAY) {
        myswprintf(formatBuffer, L"%ls[%d](%d)", dataManager.FormatLocation(display_cards[i]->overlayTarget->location, display_cards[i]->overlayTarget->sequence), display_cards[i]->overlayTarget->sequence + 1, display_cards[i]->sequence + 1);
      }
      else {
        myswprintf(formatBuffer, L"%ls[%d]", dataManager.FormatLocation(display_cards[i]->location, display_cards[i]->sequence), display_cards[i]->sequence + 1);
      }
  ...
  void ClientField::ShowSelectOption(int select_hint) {
    ...
    for (int64_t i = 0; i < mainGame->btnOption_count; ++i) {
      if (i < count) {
        const wchar_t *option = dataManager.GetDesc(select_options[i]);
        mainGame->wOptions_combo_box_option->addItem(option);
      }
    }
    mainGame->wOptions_combo_box_option->setVisible(true);
    //~ mainGame->wOptions_combo_box_option->setSelected(0);
  ...
  bool ClientField::ShowSelectSum(bool panelmode) {
    ...
    if (select_ready && (selectsum_cards.empty() || selectable_cards.empty())) {
  ...
  bool ClientField::CheckSelectSum() {
    ...
    for (int i = 0; i < (int)selected_cards.size(); ++i) {
      selected_cards[i]->is_selectable = i >= must_select_count;
  ...
  bool ClientField::check_min(const std::set<ClientCard *> &left, std::set<ClientCard *>::const_iterator index, int min, int max) {
    ...
    ++index;
    return (min > m && check_min(left, index, min - m, max - m)) || check_min(left, index, min, max);
  }
  ...
  void ClientField::check_sel_sum_t(const std::set<ClientCard *> &left, int acc) {
    ...
    for (auto sit = left.begin(); sit != left.end(); ++sit) {
      if (selectsum_cards.contains(*sit)) {
        continue;
      }
      ...
      int l1 = l & 0xffff;
      int l2 = l >> 16;
      if (check_sum(testlist.begin(), testlist.end(), acc - l1, count) || (l2 > 0 && check_sum(testlist.begin(), testlist.end(), acc - l2, count))) {
  ...
  bool ClientField::check_sum(std::set<ClientCard *>::const_iterator index, std::set<ClientCard *>::const_iterator end, int acc, int count) {
    ...
    ++index;
    return (acc > l1 && check_sum(index, end, acc - l1, count + 1)) || (l2 > 0 && acc > l2 && check_sum(index, end, acc - l2, count + 1)) || check_sum(index, end, acc, count);
  }
  ...
  void ClientField::check_sel_sum_trib_t(const std::set<ClientCard *> &left, int acc) {
    for (auto sit = left.begin(); sit != left.end(); ++sit) {
      ...
      int l1 = l & 0xffff;
      int l2 = l >> 16;
      if (check_sum_trib(testlist.begin(), testlist.end(), acc + l1) || (l2 > 0 && check_sum_trib(testlist.begin(), testlist.end(), acc + l2))) {
        selectsum_cards.insert(*sit);
      }
    }
  }
  ...
  bool ClientField::check_sum_trib(std::set<ClientCard *>::const_iterator index, std::set<ClientCard *>::const_iterator end, int acc) {
    ...
    ++index;
    return check_sum_trib(index, end, acc + l1) || check_sum_trib(index, end, acc + l2) || check_sum_trib(index, end, acc);
  }
  ...
  static bool is_declarable(const CardData &cd, const std::vector<unsigned int> &opcode) {
    ...
    return !cd.alias || second_code.contains(cd.code);
  }
