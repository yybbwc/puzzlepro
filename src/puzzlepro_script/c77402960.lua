...
function c77402960.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77402960.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c77402960.fselect(g, ft1, ft2, ect, ft)
  return aux.dncheck(g) and #g <= ft and #g <= ect and g:FilterCount(c77402960.exfilter1, nil) <= ft1 and g:FilterCount(c77402960.exfilter2, nil) <= ft2
end
...
function c77402960.countval(e, re, tp)
  if c77402960.getsummoncount(tp) > e:GetLabel() then
    return 0
  else
    return 1
  end
