...
function c68450517.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c68450517.filter(chkc, e:GetHandler():GetAttribute())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68450517.filter, tp, 0, LOCATION_MZONE, 1, nil, e:GetHandler():GetAttribute())
  end
