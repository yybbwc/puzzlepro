...
function c22011689.eqcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsRelateToBattle() and c:IsStatus(STATUS_OPPO_BATTLE) and bc:IsType(TYPE_MONSTER) and (bc:IsLocation(LOCATION_GRAVE) or bc:IsFaceup() and bc:IsLocation(LOCATION_EXTRA + LOCATION_REMOVED))
end
...
function c22011689.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c22011689.desfilter(chkc, c)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22011689.desfilter, tp, LOCATION_SZONE, 0, 1, nil, c)
  end
