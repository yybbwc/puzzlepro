...
function c57736667.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c57736667.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57736667.thfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c57736667.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c57736667.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c57736667.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
