...
function c31423101.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c31423101.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c31423101.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c31423101.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c31423101.thfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
