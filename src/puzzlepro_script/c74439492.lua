...
function c74439492.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c74439492.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c74439492.filter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c74439492.filter, tp, LOCATION_MZONE, 0, 2, nil)
  end
