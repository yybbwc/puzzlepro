...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return gc > 0 and Duel.IsExistingTarget(Card.IsFaceup, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function s.xyzfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsRank(4) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.mtfilter), tp, LOCATION_GRAVE, 0, 1, nil, e)
end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg:GetCount() > 0 and Duel.Destroy(tg, REASON_EFFECT) ~= 0 and Duel.IsExistingMatchingCard(s.xyzfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
    Duel.BreakEffect()
...
function s.xyztg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.xyzfilter2, tp, LOCATION_EXTRA, 0, 1, nil, mg)
  end
