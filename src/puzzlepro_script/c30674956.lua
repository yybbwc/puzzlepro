...
function c30674956.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c30674956.spfilter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c30674956.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp, zone)
  end
...
function c30674956.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c30674956.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
