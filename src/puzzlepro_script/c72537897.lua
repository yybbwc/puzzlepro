...
function c72537897.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and g:GetClassCount(Card.GetCode) >= 3
  end
...
function c72537897.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local g = Duel.GetMatchingGroup(c72537897.spfilter, tp, LOCATION_DECK, 0, nil, e, tp)
  if not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and g:GetClassCount(Card.GetCode) >= 3 then
