...
function s.bstg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsControler(tp) and s.bfilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.bfilter, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
...
function s.bsop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg:GetCount() > 0 and Duel.SendtoHand(tg, nil, REASON_EFFECT) > 0 and tg:IsExists(Card.IsLocation, 1, nil, LOCATION_HAND + LOCATION_EXTRA) then
