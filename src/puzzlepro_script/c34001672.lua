...
function c34001672.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c34001672.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c34001672.xmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return (chkc:IsLocation(LOCATION_MZONE)) and (chkc ~= c) and (c34001672.xmfilter(chkc))
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c34001672.xmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
  end
...
function c34001672.mttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c34001672.mtfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
