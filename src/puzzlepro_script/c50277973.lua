...
function c50277973.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(rac)
    return e:IsCostChecked() and rac ~= 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
...
function c50277973.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 50277973, 0, TYPES_NORMAL_TRAP_MONSTER, 1800, 1000, 4, rac, att) then
    return
  end
