...
function c55034079.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsCanAddCounter(tp, 0x48, 4, c)
  end
end
...
function c55034079.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanRemoveCounter(tp, 0x48, 1, REASON_EFFECT) and Duel.GetCustomActivityCount(55034079, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c55034079.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c55034079.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c55034079.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
