...
function c24425055.filter1(c, tp)
  return c:IsDiscardable() and ((c24425055.filter2(c) and c:IsAbleToGraveAsCost()) or Duel.IsExistingMatchingCard(c24425055.filter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, c))
end
...
function c24425055.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c24425055.filter1, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c24425055.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return ct > 0 and (e:IsCostChecked() or Duel.IsExistingMatchingCard(c24425055.filter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil))
  end
end
