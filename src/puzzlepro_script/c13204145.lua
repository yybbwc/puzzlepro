  ...
  if Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and g:GetClassCount(Card.GetCode) >= 2 then
    ...
    if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE, 1 - tp) then
      ...
      if cg:GetFirst():IsAbleToHand() then
        ...
        if Duel.IsExistingMatchingCard(s.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
...
function s.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and chkc:IsFacedown()
  end
  if chk == 0 then
