...
function c97870394.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsControler(tp) and c97870394.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97870394.filter, tp, LOCATION_ONFIELD, 0, 1, nil, tp) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c97870394.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97870394.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
