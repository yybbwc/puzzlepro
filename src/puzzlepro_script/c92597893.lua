...
function c92597893.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c92597893.posfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c92597893.posfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c) and c:IsAbleToHand()
  end
