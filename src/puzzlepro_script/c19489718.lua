...
function c19489718.srtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19489718.srfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c19489718.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  e:SetLabelObject(tc)
  return tc and tc:IsControler(1 - tp) and tc:IsType(TYPE_MONSTER) and aux.NegateMonsterFilter(tc) and Duel.IsExistingMatchingCard(c19489718.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, tc:GetAttribute())
end
function c19489718.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  local tc = e:GetLabelObject()
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp) and Duel.IsExistingMatchingCard(Card.IsAbleToDeck, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c19489718.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToBattle() and tc:IsControler(1 - tp) and tc:IsCanBeDisabledByEffect(e) then
    local e1 = Effect.CreateEffect(c)
