...
function c74627016.tkcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsLocation(LOCATION_GRAVE) and r == REASON_SYNCHRO and e:GetHandler():GetReasonCard():IsSetCard(0x27)
end
function c74627016.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 74627017, 0x27, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_MACHINE, ATTRIBUTE_EARTH, POS_FACEUP_ATTACK)
  end
...
function c74627016.tkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 74627017, 0x27, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_MACHINE, ATTRIBUTE_EARTH, POS_FACEUP_ATTACK) then
    return
  end
