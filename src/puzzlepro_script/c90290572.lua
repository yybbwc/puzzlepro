...
function c90290572.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90290572.tgfilter, tp, LOCATION_DECK, 0, 1, nil) or b and Duel.IsExistingMatchingCard(c90290572.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
end
...
function c90290572.costfilter(c, tp)
  return c:IsRace(RACE_FAIRY) and (c:IsFaceup() or c:IsControler(tp)) and Duel.IsExistingTarget(aux.TRUE, tp, 0, LOCATION_ONFIELD, 1, c)
end
function c90290572.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c90290572.costfilter, 1, nil, tp)
  end
