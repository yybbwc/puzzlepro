...
function c74018812.stcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c74018812.costfilter, tp, LOCATION_HAND, 0, 1, c, tp)
  end
...
function c74018812.sttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74018812.stfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
end
...
function c74018812.spcon(e, tp, eg, ep, ev, re, r, rp)
  return re and rp == tp and re:IsActiveType(TYPE_TRAP) and re:GetHandler():GetOriginalType() == TYPE_TRAP and eg:IsExists(c74018812.cfilter, 1, nil) and not eg:IsContains(e:GetHandler())
end
