...
function c81599449.filter(c, tp)
  return c:IsType(TYPE_DUAL) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(c81599449.thfilter, tp, LOCATION_DECK, 0, 1, c)
end
...
function c81599449.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81599449.filter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
