...
function c88000953.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c88000953.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88000953.atkfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
...
function c88000953.costfilter(c, tp, g)
  return g:IsContains(c) and Duel.IsExistingMatchingCard(c88000953.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, c, Group.FromCards(c))
end
...
function c88000953.fselect(g, tp)
  return Duel.IsExistingMatchingCard(c88000953.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, g:GetCount(), g, g) and Duel.CheckReleaseGroup(tp, aux.IsInGroup, #g, nil, g)
end
function c88000953.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  local lg = e:GetHandler():GetLinkedGroup()
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c88000953.costfilter, 1, nil, tp, lg)
  end
