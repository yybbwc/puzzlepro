...
function c69700783.tgfilter1(c, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsFaceup() and Duel.IsExistingMatchingCard(c69700783.tgfilter2, tp, LOCATION_DECK, 0, 1, nil, lv)
end
...
function c69700783.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local b2 = Duel.IsExistingMatchingCard(c69700783.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, tp) and Duel.IsExistingTarget(c69700783.spfilter2, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
...
function c69700783.activate(e, tp, eg, ep, ev, re, r, rp)
  if e:GetLabel() == 0 then
    ...
    if tg:GetCount() > 0 and Duel.SendtoGrave(tg, REASON_EFFECT) ~= 0 and tg:GetFirst():IsLocation(LOCATION_GRAVE) and tc:IsRelateToEffect(e) then
