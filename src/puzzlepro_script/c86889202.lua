...
function c86889202.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetActivityCount(tp, ACTIVITY_SPSUMMON) == 0 and Duel.IsExistingMatchingCard(c86889202.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c86889202.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c86889202.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
