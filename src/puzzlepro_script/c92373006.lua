...
function c92373006.sprcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c92373006.sprfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c92373006.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c92373006.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -2 and Duel.IsExistingTarget(c92373006.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c92373006.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c92373006.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not rc or rc:IsFacedown() or not rc:IsRelateToEffect(e) then
    return
  end
