...
function c16605586.tdfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xc008) and (not c:IsLocation(LOCATION_REMOVED) or c:IsFaceup()) and (not c:IsLocation(LOCATION_DECK) or Duel.GetFieldGroupCount(tp, LOCATION_DECK, 0) > 1)
end
...
function c16605586.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c16605586.tdfilter), tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, tp) or Duel.IsExistingMatchingCard(c16605586.tdfilter, tp, LOCATION_DECK, 0, 1, nil, tp) and Duel.SelectYesNo(tp, aux.Stringid(16605586, 3)) then
