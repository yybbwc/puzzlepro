...
function c4417407.poscost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsType, 1, nil, TYPE_TOKEN)
  end
...
function c4417407.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c4417407.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4417407.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
