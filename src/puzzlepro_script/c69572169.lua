...
function c69572169.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and rg:GetClassCount(Card.GetCode) >= 3
  end
...
function c69572169.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c69572169.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69572169.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
