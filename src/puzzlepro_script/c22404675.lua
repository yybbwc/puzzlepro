...
function c22404675.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) then
    if Duel.SpecialSummonStep(c, 0, tp, tp, false, false, POS_FACEUP) then
      if Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 22404676, 0, TYPES_TOKEN_MONSTER, 800, 1000, 1, RACE_THUNDER, ATTRIBUTE_LIGHT, POS_FACEUP_DEFENSE) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) then
...
function c22404675.sumop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 22404675) ~= 0 then
    return
  end
