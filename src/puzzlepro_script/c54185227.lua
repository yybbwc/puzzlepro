...
function c54185227.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c54185227.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c54185227.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function c54185227.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c54185227.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsExistingTarget(c54185227.spfilter, tp, LOCATION_SZONE, 0, 2, nil, e, tp)
  end
