...
function c93138457.filter(c, e, tp)
  return (c:IsSetCard(0x27) or (c:IsLevelBelow(4) and c:IsType(TYPE_TUNER))) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function c93138457.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateAttack() and #g > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(93138457, 0)) then
    Duel.BreakEffect()
