...
function c10424147.spfilter1(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x10dc) and Duel.IsExistingMatchingCard(c10424147.spfilter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c, c:GetAttribute()) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL)
end
function c10424147.spfilter2(c, e, tp, mc, att)
  return c:IsType(TYPE_XYZ) and c:IsSetCard(0x20dc) and c:IsAttribute(att) and mc:IsCanBeXyzMaterial(c) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c10424147.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c10424147.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c10424147.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c10424147.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return g:GetClassCount(Card.GetCode) >= 3 and Duel.IsExistingMatchingCard(c10424147.spfilter4, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
