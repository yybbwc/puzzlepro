...
function c44161893.cfilter(c, tp, rp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:GetPreviousTypeOnField() & TYPE_LINK ~= 0 and c:IsPreviousSetCard(0x119) and rp == 1 - tp and c:IsReason(REASON_EFFECT)
end
...
function c44161893.fselect(g, tp, c)
  return g:IsContains(c) and Duel.IsExistingMatchingCard(c44161893.xyzfilter, tp, LOCATION_EXTRA, 0, 1, nil, g)
end
...
function c44161893.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:CheckSubGroup(c44161893.fselect, 2, 2, tp, e:GetHandler())
  end
