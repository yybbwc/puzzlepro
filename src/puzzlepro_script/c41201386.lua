...
function c41201386.filter1(c, e, tp)
  local rk = c:GetRank()
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c41201386.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c, rk + 1) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL)
end
...
function c41201386.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c41201386.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41201386.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c41201386.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c41201386.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c41201386.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c41201386.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41201386.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
