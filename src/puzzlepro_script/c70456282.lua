...
function c70456282.spfilter(c, tp)
  return c:IsSetCard(0x33) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c70456282.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c70456282.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70456282.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
