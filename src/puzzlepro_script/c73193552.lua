...
function c73193552.regcon(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c73193552.cfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c73193552.cfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c73193552.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73193552.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c73193552.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return a and d and (a:IsFaceup() and a:IsType(TYPE_SYNCHRO) or d:IsFaceup() and d:IsType(TYPE_SYNCHRO))
  end
...
function c73193552.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and c:IsReason(REASON_EFFECT) and c:IsFaceup() and Duel.IsExistingMatchingCard(c73193552.repfilter, tp, LOCATION_MZONE, 0, 1, nil, e)
  end
