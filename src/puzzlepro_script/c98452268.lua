...
function c98452268.spfilter(c, e, tp, mc)
  return c:IsSetCard(0x13b, 0x10db) and mc:IsCanBeXyzMaterial(c) and c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and not c:IsCode(98452268) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c98452268.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_XMATERIAL) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c98452268.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
...
function c98452268.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) then
    ...
    if tc then
      ...
      if Duel.SpecialSummon(tc, SUMMON_TYPE_XYZ, tp, tp, false, false, POS_FACEUP) ~= 0 then
        ...
        if Duel.IsExistingMatchingCard(c98452268.matfilter, tp, LOCATION_PZONE, 0, 1, nil, e) and Duel.SelectYesNo(tp, aux.Stringid(98452268, 3)) then
          Duel.BreakEffect()
