...
function c37313338.filter(c, e, tp)
  return c:IsSetCard(0x182) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)))
end
function c37313338.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c37313338.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37313338.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c37313338.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c37313338.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
