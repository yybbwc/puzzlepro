...
function c90488465.tdfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsCanBeEffectTarget(e) and (c:IsAbleToDeck() or c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
...
function c90488465.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return dg:CheckSubGroup(c90488465.fselect, 3, 3, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
