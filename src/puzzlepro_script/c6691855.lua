...
function c6691855.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c6691855.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c6691855.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
