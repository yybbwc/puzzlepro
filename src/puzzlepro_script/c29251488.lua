...
function s.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return c:GetFlagEffect(id) == 0 and Duel.IsExistingTarget(s.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function s.rsptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chk == 0 then
    return e:IsCostChecked() and c:GetFlagEffect(id) == 0 and Duel.IsExistingMatchingCard(s.rfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
  c:RegisterFlagEffect(id, RESET_CHAIN, 0, 1)
...
function s.pzcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsReason(REASON_RELEASE) and c:IsLocation(LOCATION_EXTRA) and c:IsFaceup()
end
