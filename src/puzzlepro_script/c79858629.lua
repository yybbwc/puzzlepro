...
function c79858629.effcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c79858629.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c79858629.effop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) > 0 and Duel.IsExistingMatchingCard(c79858629.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
    Duel.BreakEffect()
...
function c79858629.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c79858629.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
