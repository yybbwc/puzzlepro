...
function c49575521.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c49575521.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c49575521.tdfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
...
function c49575521.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToHand() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c49575521.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
