...
function c43383478.checkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if p1 then
    Duel.RegisterFlagEffect(0, 43383478, RESET_PHASE + PHASE_END, 0, 1)
  end
  if p2 then
    Duel.RegisterFlagEffect(1, 43383478, RESET_PHASE + PHASE_END, 0, 1)
  end
end
...
function c43383478.filter1(c, e, tp)
  return c:IsSetCard(0xba) and c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c43383478.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c, c:GetRank() + 1)
end
...
function c43383478.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c43383478.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c43383478.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
