...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tg:GetCount() > 0 then
    ...
    if g:GetCount() > 0 then
      local lv = aux.SelectFromOptions(tp, { g:IsExists(s.lvfilter, 1, nil, 3), aux.Stringid(id, 2), 3 }, { g:IsExists(s.lvfilter, 1, nil, 5), aux.Stringid(id, 3), 5 }, { true, aux.Stringid(id, 4), 0 })
      if lv == 0 then
        return
      end
      Duel.BreakEffect()
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and s.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.spfilter, tp, LOCATION_SZONE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  end
