...
function c27204311.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.Release(g, REASON_EFFECT) ~= 0 then
    ...
    if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 then
      ...
      if Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 27204312, 0, TYPES_TOKEN_MONSTER, atk, def, 11, RACE_ROCK, ATTRIBUTE_LIGHT, POS_FACEUP, 1 - tp) then
        Duel.BreakEffect()
