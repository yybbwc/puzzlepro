...
function c70122149.rftg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return d ~= nil and d:IsControler(tp) and d:IsSetCard(0x129)
  end
end
...
function c70122149.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c70122149.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70122149.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
