...
function c67314110.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67314110.setfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c67314110.eqfilter1(c, tp)
  return c:IsSetCard(0x15c) and c:IsType(TYPE_TRAP) and c:IsFaceup() and c:GetEquipTarget() and Duel.IsExistingMatchingCard(c67314110.eqfilter2, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c:GetEquipTarget(), tp)
end
...
function c67314110.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c67314110.eqfilter1(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67314110.eqfilter1, tp, LOCATION_SZONE, 0, 1, nil, tp)
  end
