...
function c19036557.costfilter(c, tp)
  return c:IsSetCard(0x400d) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c19036557.thfilter, tp, LOCATION_DECK, 0, 1, c)
end
function c19036557.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19036557.costfilter, tp, loc, 0, 1, nil, tp)
  end
...
function c19036557.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19036557.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
