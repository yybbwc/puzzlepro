...
function c46412900.sprfilter(c)
  return c:IsFaceupEx() and c:IsAbleToRemoveAsCost() and (c:IsRace(RACE_PYRO) and c:IsType(TYPE_MONSTER) or c:IsSetCard(0xb9))
end
function c46412900.gcheck(g, tp)
  return Duel.GetMZoneCount(tp, g) > 0 and (#g == 3 and g:FilterCount(Card.IsRace, nil, RACE_PYRO) == 3 or #g == 1 and g:FilterCount(Card.IsSetCard, nil, 0xb9) == 1)
end
...
function c46412900.damtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c46412900.damfilter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil)
  end
...
function c46412900.damop(e, tp, eg, ep, ev, re, r, rp)
  local val = Duel.GetMatchingGroupCount(c46412900.damfilter, tp, LOCATION_REMOVED, LOCATION_REMOVED, nil) * 500
  if Duel.Damage(1 - tp, val, REASON_EFFECT) ~= 0 and Duel.IsExistingMatchingCard(c46412900.scfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(46412900, 2)) then
    Duel.BreakEffect()
