...
function c59312550.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and c:IsPreviousPosition(POS_FACEUP_ATTACK) and c:IsPreviousControler(tp)
end
...
function c59312550.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c59312550.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
