...
function c24673894.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tc:IsLocation(LOCATION_MZONE) and tc:IsAttackPos() and tc:IsCanChangePosition() and tc:IsCanBeEffectTarget(e)
  end
...
function c24673894.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.NegateAttack() and Duel.ChangePosition(tc, POS_FACEUP_DEFENSE) ~= 0 then
    ...
    if op == 0 then
      Duel.Recover(1 - tp, math.ceil(val), REASON_EFFECT)
    else
      Duel.Damage(tp, math.floor(val), REASON_EFFECT)
    end
  end
end
