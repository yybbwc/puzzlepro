...
function c46593546.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and c46593546.sptgfilter(chkc)
  end
  local c = e:GetHandler()
  local tc = Duel.GetFirstMatchingCard(nil, tp, LOCATION_PZONE, 0, c)
  if chk == 0 then
    return tc and Duel.GetMZoneCount(tp) > 0 and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c46593546.sptgfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c46593546.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.GetMZoneCount(tp) > 0 and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 and fc:IsRelateToEffect(e) then
    Duel.MoveToField(fc, tp, tp, LOCATION_PZONE, POS_FACEUP, true)
  end
end
function c46593546.ovlcon(e, tp, eg, ep, ev, re, r, rp)
  local rc = re:GetHandler()
  return rc:IsControler(1 - tp) and rc:GetOriginalType() & TYPE_MONSTER ~= 0 and (re:GetActivateLocation() & LOCATION_ONFIELD ~= 0 or re:IsHasType(EFFECT_TYPE_ACTIVATE))
end
...
function c46593546.ovltg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local rc = re:GetHandler()
  if chk == 0 then
    return c:IsType(TYPE_XYZ) and (c:CheckRemoveOverlayCard(tp, 2, REASON_EFFECT) or Duel.IsExistingMatchingCard(c46593546.ovltgfilter, tp, LOCATION_ONFIELD, 0, 1, nil)) and rc:IsRelateToEffect(re) and rc:IsCanBeXyzMaterial(c)
  end
end
function c46593546.ovlop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if opt2 and not opt1 then
    result = 1
  end
  if opt1 and opt2 then
    result = Duel.SelectOption(tp, aux.Stringid(46593546, 5), aux.Stringid(46593546, 6))
  end
  if result == 0 then
  ...
  if result > 0 and c:IsRelateToEffect(e) and rc:IsRelateToEffect(re) and rc:IsControler(1 - tp) and not rc:IsImmuneToEffect(e) then
