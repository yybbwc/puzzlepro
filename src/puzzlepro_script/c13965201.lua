...
function c13965201.costfilter(c, e, tp)
  return c:IsDiscardable() and c:IsRace(RACE_ZOMBIE) and Duel.IsExistingMatchingCard(c13965201.spfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetCode())
end
...
function c13965201.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c13965201.costfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
