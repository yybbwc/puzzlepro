...
function c62895219.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c62895219.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c62895219.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c62895219.filter2(c, e, tp, m, f, gc, chkf)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x9b) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, gc, chkf)
end
