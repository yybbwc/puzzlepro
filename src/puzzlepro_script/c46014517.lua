...
function s.mttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and s.mtfilter(chkc)
  end
  if chk == 0 then
    return ct > 0 and e:GetHandler():IsType(TYPE_XYZ) and Duel.CheckRemoveOverlayCard(tp, 1, 0, 1, REASON_EFFECT) and Duel.IsExistingTarget(s.mtfilter, tp, 0, LOCATION_MZONE, 1, e:GetHandler())
  end
...
function s.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return ct > 0 and Duel.CheckRemoveOverlayCard(tp, 1, 0, 3, REASON_EFFECT) and Duel.IsExistingTarget(Card.IsAbleToGrave, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function s.tgop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.RemoveOverlayCard(tp, 1, 0, 3, 3, REASON_EFFECT) == 0 then
    return
  end
  local g = Duel.GetChainInfo(0, CHAININFO_TARGET_CARDS)
