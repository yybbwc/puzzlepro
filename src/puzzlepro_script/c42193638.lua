...
function s.activate(e, tp, eg, ep, ev, re, r, rp, op)
  ...
  if op == 1 then
    if Duel.Recover(tp, 500, REASON_EFFECT) < 1 or not Duel.IsExistingMatchingCard(Card.IsAbleToHand, tp, LOCATION_DECK, 0, 1, nil) or not Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
      return
    end
