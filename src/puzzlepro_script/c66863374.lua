...
function c66863374.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c66863374.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c66863374.tdfilter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x1019) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c66863374.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c)
end
function c66863374.spfilter(c, e, tp, tc)
  return c:IsSetCard(0x1019) and not c:IsOriginalCodeRule(tc:GetOriginalCodeRule()) and c:IsCanBeSpecialSummoned(e, SUMMON_VALUE_GLADIATOR, tp, false, false)
end
function c66863374.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c66863374.tdfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c66863374.tdfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c66863374.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
