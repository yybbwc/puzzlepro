...
function c55465441.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c55465441.filter2, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c55465441.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c55465441.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc2:IsRelateToEffect(e) and Duel.SpecialSummon(tc2, 0, tp, 1 - tp, false, false, POS_FACEUP_DEFENSE) ~= 0 and tc1:IsFaceup() and tc1:IsRelateToEffect(e) then
