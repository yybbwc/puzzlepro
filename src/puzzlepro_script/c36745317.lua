...
function c36745317.tgcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and (fe or b2)
  end
...
function c36745317.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c36745317.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c36745317.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_GRAVE) then
    ...
    if sg:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(36745317, 0)) then
      Duel.BreakEffect()
...
function c36745317.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c36745317.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c36745317.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
