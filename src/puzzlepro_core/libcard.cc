...
int32_t scriptlib::card_is_fusion_code(lua_State *L) {
  ...
  effect_set eset;
  pcard->filter_effect(EFFECT_ADD_FUSION_CODE, &eset);
  if (eset.empty() || pcard->pduel->game_field->core.not_material) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    auto tcode = (uint32_t)lua_tointeger(L, i + 2);
...
int32_t scriptlib::card_is_set_card(lua_State *L) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    auto set_code = (uint32_t)lua_tointeger(L, i + 2);
...
int32_t scriptlib::card_is_link_marker(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto dir = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_get_column_zone(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto loc = (int32_t)lua_tointeger(L, 2);
  int32_t cp = pcard->current.controler;
...
int32_t scriptlib::card_is_type(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto ttype = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_level(lua_State *L) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    auto tlevel = (uint32_t)lua_tointeger(L, i + 2);
    if (level == tlevel) {
      result = TRUE;
      break;
    }
...
int32_t scriptlib::card_is_rank(lua_State *L) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    auto trank = (uint32_t)lua_tointeger(L, i + 2);
    if (rank == trank) {
      result = TRUE;
      break;
    }
...
int32_t scriptlib::card_is_link(lua_State *L) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    auto tlink = (uint32_t)lua_tointeger(L, i + 2);
    if (link == tlink) {
      result = TRUE;
      break;
    }
...
int32_t scriptlib::card_is_attack(lua_State *L) {
  ...
  if (!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER)) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      auto tatk = (uint32_t)lua_tointeger(L, i + 2);
      if (atk == tatk) {
        result = TRUE;
        break;
      }
...
int32_t scriptlib::card_is_defense(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  if ((pcard->data.type & TYPE_LINK) || (!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER))) {
    lua_pushboolean(L, 0);
  }
  else {
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      auto tdef = (uint32_t)lua_tointeger(L, i + 2);
      if (def == tdef) {
        result = TRUE;
        break;
      }
...
int32_t scriptlib::card_is_race(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto trace = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_attribute(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto tattrib = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_reason(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  int32_t treason = (uint32_t)lua_tointeger(L, 2);
  //~ fast_io::io::print(fast_io::win32_box_t(), fast_io::mnp::base<16>(pcard->current.reason), "\n", __FILE__, "\n", __LINE__, "\n", __PRETTY_FUNCTION__);
...
int32_t scriptlib::card_is_all_reasons(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto treason = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_summon_location(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto tloc = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_summon_player(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto con = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_status(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto tstatus = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_set_turn_counter(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto ct = (int32_t)lua_tointeger(L, 2);
  pcard->count_turn(ct);
  return 0;
}
...
//~ int32_t scriptlib::card_check_equip_target(lua_State *L) {
  ...
  //~ if ((pcard->is_affected_by_effect(EFFECT_EQUIP_LIMIT, target) && ((!pcard->is_affected_by_effect(EFFECT_OLDUNION_STATUS) || target->get_union_count() == 0) && (!pcard->is_affected_by_effect(EFFECT_UNION_STATUS) || target->get_old_union_count() == 0))))) {
    //~ lua_pushboolean(L, 1);
  //~ }
  //~ else {
    //~ lua_pushboolean(L, 0);
...
int32_t scriptlib::card_check_equip_target(lua_State *L) {  
  ...
  // 添加唯一性检查  
  if (pcard->pduel->game_field->check_unique_onfield(pcard, target->current.controler, LOCATION_SZONE, target)) {  
    lua_pushboolean(L, 0);  
    return 1;  
  }  
...
int32_t scriptlib::card_check_union_target(lua_State *L) {
  ...
  card *pcard = *(card **)lua_touserdata(L, 1);
  card *target = *(card **)lua_touserdata(L, 2);
  if (pcard->is_affected_by_effect(EFFECT_UNION_LIMIT, target) && ((!pcard->is_affected_by_effect(EFFECT_OLDUNION_STATUS) || target->get_union_count() == 0) && (!pcard->is_affected_by_effect(EFFECT_UNION_STATUS) || target->get_old_union_count() == 0))) {
    lua_pushboolean(L, 1);
  }
  else {
    lua_pushboolean(L, 0);
...
int32_t scriptlib::card_check_remove_overlay_card(lua_State *L) {
  ...
  auto reason = (int32_t)lua_tointeger(L, 4);
  duel *pduel = pcard->pduel;
...
int32_t scriptlib::card_remove_overlay_card(lua_State *L) {
  ...
  auto min = (int32_t)lua_tointeger(L, 3);
  auto max = (int32_t)lua_tointeger(L, 4);
...
int32_t scriptlib::card_is_direct_attacked(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  if (pcard->attacked_cards.findcard(nullptr)) {
    lua_pushboolean(L, 1);
  }
  else {
    lua_pushboolean(L, 0);
...
int32_t scriptlib::card_get_first_card_target(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  if (!pcard->effect_target_cards.empty()) {
...
int32_t scriptlib::card_check_activate_effect(lua_State *L) {
  ...
  for (auto eit = pcard->field_effect.begin(); eit != pcard->field_effect.end();) {
    ...
    ++eit;
    if ((peffect->type & EFFECT_TYPE_ACTIVATE) && pduel->game_field->check_event_c(peffect, pduel->game_field->core.reason_player, neglect_con, neglect_cost, copy_info, &pe)) {
...
int32_t scriptlib::card_register_effect(lua_State *L) {
  ...
  for (const auto &entry : category_checklist) {
...
int32_t scriptlib::card_reset_effect(lua_State *L) {
  ...
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto code = (uint32_t)lua_tointeger(L, 2);
  auto type = (uint32_t)lua_tointeger(L, 3);
...
int32_t scriptlib::card_get_flag_effect_label(lua_State *L) {
  ...
  for (; rg.first != rg.second; ++rg.first, ++count) {
    lua_pushinteger(L, !rg.first->second->label.empty() ? rg.first->second->label[0] : 0);
  }
  if (!count) {
    lua_pushnil(L);
    return 1;
...
int32_t scriptlib::card_is_can_be_special_summoned(lua_State *L) {
  ...
  card *pcard = *(card **)lua_touserdata(L, 1);
  effect *peffect = *(effect **)lua_touserdata(L, 2);
  auto sumtype = (uint32_t)lua_tointeger(L, 3);
  auto sumplayer = (uint32_t)lua_tointeger(L, 4);
...
int32_t scriptlib::card_is_can_be_placed_on_field(lua_State *L) {
  ...
  if (pcard->is_status(STATUS_FORBIDDEN) || pcard->pduel->game_field->check_unique_onfield(pcard, toplayer, tolocation, nullptr)) {
    lua_pushboolean(L, 0);
  }
  else {
    lua_pushboolean(L, 1);
...
int32_t scriptlib::card_is_discardable(lua_State *L) {
  ...
  if ((reason != REASON_COST || !pcard->is_affected_by_effect(EFFECT_CANNOT_USE_AS_COST)) && pcard->pduel->game_field->is_player_can_discard_hand(p, pcard, pe, reason)) {
    lua_pushboolean(L, 1);
  }
  else {
    lua_pushboolean(L, 0);
...
int32_t scriptlib::card_is_location(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto loc = (uint32_t)lua_tointeger(L, 2);
  if (pcard->is_treated_as_not_on_field()) {
    lua_pushboolean(L, 0);
  }
...
int32_t scriptlib::card_is_level_below(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto lvl = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_rank_below(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto rnk = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_link_below(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto lnk = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_is_attack_below(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto atk = (int32_t)lua_tointeger(L, 2);
  if (!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER)) {
    lua_pushboolean(L, 0);
  }
  else {
...
int32_t scriptlib::card_is_defense_below(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto def = (int32_t)lua_tointeger(L, 2);
  if ((pcard->data.type & TYPE_LINK) || (!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER))) {
    lua_pushboolean(L, 0);
  }
  else {
...
int32_t scriptlib::card_is_has_level(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  if ((pcard->data.type & (TYPE_XYZ | TYPE_LINK)) || (pcard->status & STATUS_NO_LEVEL) || (!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER))) {
    lua_pushboolean(L, 0);
  }
  else {
    lua_pushboolean(L, 1);
...
int32_t scriptlib::card_add_counter(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto countertype = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::card_remove_counter(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto rplayer = (uint32_t)lua_tointeger(L, 2);
  auto countertype = (uint32_t)lua_tointeger(L, 3);
...
int32_t scriptlib::card_set_counter_limit(lua_State *L) {
  ...
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto countertype = (int32_t)lua_tointeger(L, 2);
  auto limit = (int32_t)lua_tointeger(L, 3);
  effect *peffect = pcard->pduel->new_effect();
  peffect->owner = pcard;
  peffect->type = EFFECT_TYPE_SINGLE;
...
int32_t scriptlib::card_add_monster_attribute(lua_State *L) {
  check_param_count(L, 2);
  check_param(L, PARAM_TYPE_CARD, 1);
  auto type = (int32_t)lua_tointeger(L, 2);
  auto attribute = (int32_t)lua_tointeger(L, 3);
  auto race = (int32_t)lua_tointeger(L, 4);
  auto level = (int32_t)lua_tointeger(L, 5);
  auto atk = (int32_t)lua_tointeger(L, 6);
  auto def = (int32_t)lua_tointeger(L, 7);
  card *pcard = *(card **)lua_touserdata(L, 1);
  duel *pduel = pcard->pduel;
...
int32_t scriptlib::card_set_hint(lua_State *L) {
  ...
  card *pcard = *(card **)lua_touserdata(L, 1);
  duel *pduel = pcard->pduel;
  auto type = (uint32_t)lua_tointeger(L, 2);
  auto value = (uint32_t)lua_tointeger(L, 3);
...
int32_t scriptlib::card_check_unique_onfield(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_CARD, 1);
  card *pcard = *(card **)lua_touserdata(L, 1);
  auto check_player = (uint32_t)lua_tointeger(L, 2);
