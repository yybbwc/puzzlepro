...
function c13760677.cfilter(c, e, tp)
  return c:IsRace(RACE_ZOMBIE) and c:IsSummonPlayer(tp) and c:IsSummonType(SUMMON_TYPE_PENDULUM) and (not e or c:IsRelateToEffect(e))
end
...
function c13760677.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c13760677.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13760677.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
