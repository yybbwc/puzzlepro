...
function c51474037.filter1(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_REPTILE) and Duel.IsExistingTarget(c51474037.filter2, tp, 0, LOCATION_MZONE, 1, nil, c:IsCanTurnSet())
end
...
function c51474037.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c51474037.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
