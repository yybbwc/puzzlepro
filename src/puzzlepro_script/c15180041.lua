...
function c15180041.spfilter(c, tp)
  return c:IsRace(RACE_WARRIOR) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c15180041.filter(c, e, tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xe7) and not c:IsCode(15180041) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false)
end
function c15180041.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c15180041.filter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
