...
function c40044918.tg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    local sel = 0
    if ct > 0 and Duel.IsExistingMatchingCard(c40044918.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) then
      sel = sel + 1
    end
    if Duel.IsExistingMatchingCard(c40044918.schfilter, tp, LOCATION_DECK, 0, 1, nil) then
      sel = sel + 2
    end
    e:SetLabel(sel)
    return sel ~= 0
