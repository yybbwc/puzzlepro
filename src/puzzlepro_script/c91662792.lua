...
function c91662792.filter(c, e, tp)
  return c:IsLevelBelow(4) and c:IsSetCard(0x10) and not c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c91662792.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c91662792.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
