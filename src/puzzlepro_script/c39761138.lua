...
function c39761138.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsRelateToBattle() and c:GetBattleTarget():IsType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(Card.IsSetCard, tp, LOCATION_GRAVE, 0, 3, nil, 0x2c)
end
function c39761138.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if Duel.IsExistingMatchingCard(Card.IsType, tp, 0, LOCATION_GRAVE, 1, nil, TYPE_SPELL) then
    e:SetLabel(0)
  else
    e:SetLabel(1)
...
function c39761138.rmop(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(Card.IsSetCard, tp, LOCATION_GRAVE, 0, 3, nil, 0x2c) then
    return
  end
