...
function c22198672.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c22198672.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22198672.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c22198672.chfilter1(c)
  return c:IsType(TYPE_LINK) and c:GetSequence() < 5 and Duel.IsExistingMatchingCard(c22198672.chfilter2, c:GetControler(), LOCATION_MZONE, 0, 1, c)
end
...
function c22198672.chtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22198672.chfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
end
