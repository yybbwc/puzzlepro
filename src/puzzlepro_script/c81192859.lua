...
function c81192859.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (ac and ac:IsFaceup() and ac:IsSetCard(0x171)) then
    return false
  end
...
function c81192859.limcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rp == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_SZONE) and Duel.GetMatchingGroupCount(c81192859.cfilter, tp, LOCATION_MZONE, 0, nil) > 0
end
