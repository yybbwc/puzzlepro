...
function c63413494.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c63413494.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c63413494.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp, eg)
  end
