...
function c35149085.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c35149085.filter(chkc, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c35149085.filter, tp, LOCATION_MZONE, 0, 1, nil, ft)
  end
