...
function c4357063.costfilter(c, e, tp)
  return c:IsSetCard(0x70) and c:IsAbleToRemoveAsCost() and c:GetLevel() > 0 and Duel.IsExistingTarget(c4357063.spfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp, c:GetLevel())
end
...
function c4357063.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4357063.spfilter(chkc, e, tp, e:GetLabel())
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c4357063.costfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
