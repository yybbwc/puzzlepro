...
function c23323812.filter(c, tp)
  return c:IsPreviousControler(tp) and c:IsPreviousPosition(POS_FACEUP) and c == Duel.GetAttackTarget() and c:IsRace(RACE_PSYCHO)
end
...
function c23323812.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return at:IsControler(1 - tp) and at:IsRelateToBattle() and at:IsCanBeEffectTarget(e)
  end
