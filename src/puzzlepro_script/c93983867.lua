...
function c93983867.cfilter(c, tp)
  return c:IsSetCard(0xc6) and c:IsReason(REASON_DESTROY) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp)
end
...
function c93983867.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsControlerCanBeChanged, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c93983867.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
