...
function c76076738.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c76076738.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c76076738.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c76076738.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local ec = c:GetPreviousEquipTarget()
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and ec and ec:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
