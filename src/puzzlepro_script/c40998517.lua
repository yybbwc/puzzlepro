...
function c40998517.costfilter(c, tp)
  return (c:IsSetCard(0x134) or c:IsRace(RACE_WARRIOR)) and Duel.IsExistingTarget(nil, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
function c40998517.fselect(g, tp)
  return Duel.IsExistingTarget(nil, tp, LOCATION_MZONE, LOCATION_MZONE, g:GetCount(), g) and Duel.CheckReleaseGroup(tp, aux.IsInGroup, #g, nil, g)
end
function c40998517.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c40998517.costfilter, 1, nil, tp)
  end
