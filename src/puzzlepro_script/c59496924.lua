...
function c59496924.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and c:IsSetCard(0xd2) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)) and not c:IsReason(REASON_REPLACE)
end
function c59496924.desfilter(c, e, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and c:IsSetCard(0xd2) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
end
function c59496924.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c59496924.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c59496924.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil, e, tp)
  end
...
function c59496924.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and e:GetHandler():IsLocation(LOCATION_GRAVE)
  end
...
function c59496924.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c59496924.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
