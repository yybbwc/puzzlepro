...
function c71166481.xtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c71166481.xfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71166481.xfilter, tp, LOCATION_MZONE, 0, 1, c) and c:IsCanOverlay()
  end
