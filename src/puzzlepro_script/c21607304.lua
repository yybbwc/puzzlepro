...
function c21607304.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c21607304.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c21607304.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c21607304.spcon(e, tp, eg, ep, ev, re, r, rp)
  return eg:IsExists(c21607304.cfilter, 1, nil, tp) and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
end
function c21607304.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and not Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
  end
...
function c21607304.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(21607304) == 0
  end
