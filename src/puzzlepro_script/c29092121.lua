...
function c29092121.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c29092121.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29092121.lvfilter, tp, LOCATION_MZONE, 0, 2, nil)
  end
...
function c29092121.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29092121.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
