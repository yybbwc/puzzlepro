...
function c91969909.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c91969909.rmfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91969909.rmfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c91969909.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (e:GetHandler():IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFacedown()) then
    return
  end
