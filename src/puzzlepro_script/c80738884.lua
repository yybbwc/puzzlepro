...
function c80738884.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tg and tg:IsExists(c80738884.cfilter2, 1, nil, tp) and Duel.IsChainNegatable(ev) and Duel.IsExistingMatchingCard(c80738884.cfilter1, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c80738884.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg, REASON_EFFECT) > 0 and Duel.IsExistingMatchingCard(c80738884.dmgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(80738884, 1)) then
    Duel.BreakEffect()
