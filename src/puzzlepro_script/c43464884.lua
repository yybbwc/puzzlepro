...
function c43464884.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(43464884, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c43464884.cgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c43464884.cgfilter(chkc, c)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c43464884.cgfilter, tp, LOCATION_GRAVE, 0, 1, nil, c)
  end
...
function c43464884.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c43464884.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, e:GetHandler())
  end
