...
function s.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateActivation(ev) then
    if Duel.GetMatchingGroupCount(Card.IsOnField, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, nil) > 0 and c:CheckRemoveOverlayCard(tp, 1, REASON_EFFECT) and Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
      Duel.BreakEffect()
