...
function c28427869.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, EFFECT_AVOID_BATTLE_DAMAGE) and Duel.GetBattleDamage(tp) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c28427869.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c28427869.lvfilter(chkc, lv)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c28427869.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, lv)
  end
