...
function c44311445.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c44311445.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44311445.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c44311445.cfilter(c, tp)
  return c:IsSetCard(0x71) and c:IsLocation(LOCATION_DECK) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_GRAVE)
end
...
function c44311445.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c44311445.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
