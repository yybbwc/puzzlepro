...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and rg:CheckSubGroup(s.fselect, 1, 2)
  end
...
function s.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.atkfilter, tp, 0, LOCATION_MZONE, ev, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
