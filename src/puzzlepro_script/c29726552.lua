...
function c29726552.spcon2(e, c)
  ...
  local tp = c:GetControler()
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c29726552.cfilter, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function c29726552.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c29726552.filter, 1, nil, 1 - tp) and not eg:IsContains(e:GetHandler())
  end
