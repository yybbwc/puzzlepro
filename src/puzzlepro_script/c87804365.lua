...
function c87804365.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return ft > 0 and Duel.IsPlayerCanSpecialSummonCount(tp, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and (b1 or b2)
  end
  local op = 0
  if b1 and b2 then
...
function c87804365.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  Duel.AdjustAll()
  if og:FilterCount(Card.IsLocation, nil, LOCATION_MZONE) < g:GetCount() then
    return
  end
  if op == 0 then
