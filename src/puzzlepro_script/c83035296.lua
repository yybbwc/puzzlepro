...
function c83035296.thfilter1(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_SPELL) and Duel.IsExistingMatchingCard(c83035296.thfilter2, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c83035296.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c83035296.thfilter1(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83035296.thfilter1, tp, LOCATION_REMOVED, 0, 1, nil, tp)
  end
