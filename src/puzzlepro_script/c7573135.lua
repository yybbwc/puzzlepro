...
function c7573135.hsptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c7573135.hspfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c7573135.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c7573135.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
