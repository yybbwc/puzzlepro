...
function c24175232.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c24175232.desfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c24175232.spr(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsReason(REASON_DESTROY)) then
    return
  end
