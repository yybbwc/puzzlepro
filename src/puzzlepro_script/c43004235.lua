...
function c43004235.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, i + 1)
  end
...
function c43004235.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, i + 1, REASON_EFFECT) == 0 then
    return
  end
  if i > 0 then
    Duel.ShuffleHand(tp)
...
function c43004235.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and not c:IsForbidden() and c:CheckUniqueOnField(tp, LOCATION_SZONE) and Duel.IsExistingMatchingCard(c43004235.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, g)
  end
...
function c43004235.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or c:IsForbidden() or not c:CheckUniqueOnField(tp, LOCATION_SZONE) then
    return
  end
  local g = eg:Filter(c43004235.exfilter, nil, tp)
