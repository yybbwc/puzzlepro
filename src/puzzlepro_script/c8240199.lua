...
function c8240199.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c8240199.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c8240199.gvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c8240199.gvfilter(chkc, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c8240199.gvfilter, tp, LOCATION_MZONE, 0, 1, nil, ft) and Duel.IsExistingMatchingCard(c8240199.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c8240199.gvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    if Duel.SendtoGrave(tc, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c8240199.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp) then
