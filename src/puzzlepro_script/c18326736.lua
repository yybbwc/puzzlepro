...
function c18326736.filter(c, e, tp, rk, mc)
  return c:IsRank(rk) and not c:IsSetCard(0x48) and e:GetHandler():IsCanBeXyzMaterial(c) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c18326736.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c18326736.filter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetRank() + 1, c)
  end
...
function c18326736.skipcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 7, REASON_COST)
  end
...
function c18326736.mttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c18326736.mtfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
end
