...
function c74752631.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c74752631.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c74752631.cfilter(c, ec)
  if c:IsLocation(LOCATION_MZONE) then
    ...
    return c:IsPreviousSetCard(0x103d) and c:IsPreviousPosition(POS_FACEUP) and bit.extract(ec:GetLinkedZone(c:GetPreviousControler()), c:GetPreviousSequence()) ~= 0
  end
end
