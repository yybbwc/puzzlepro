...
function c6021033.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c6021033.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c6021033.filter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function c6021033.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsRace(RACE_ZOMBIE) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP_ATTACK) ~= 0 then
