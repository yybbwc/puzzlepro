...
function c98864751.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c98864751.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98864751.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c98864751.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c98864751[tp] == 0 or c:GetEquipTarget():GetFlagEffect(98864751) ~= 0
  end
  local e1 = Effect.CreateEffect(c)
