...
function c23792058.tdfilter(c)
  return c:IsAttribute(ATTRIBUTE_WIND) and c:IsType(TYPE_SYNCHRO) and c:IsAbleToExtra() and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup())
end
function c23792058.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c23792058.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c23792058.tdfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
...
function c23792058.spfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:IsSetCard(0x2016) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c23792058.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c23792058.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
