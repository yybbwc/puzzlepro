...
function c68829754.spfilter(c, e, tp, check)
  return c:IsRace(RACE_ZOMBIE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and (check or c:IsSetCard(0x1142))
end
function c68829754.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c68829754.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp, chk1)
  end
...
function c68829754.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c68829754.stfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
