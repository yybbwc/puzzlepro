...
function s.rmfilter(c)
  return c:IsAttack(1500) and c:IsDefense(2100) and c:IsAbleToRemove() and c:IsFaceupEx()
end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #sg > 0 and Duel.Remove(sg, POS_FACEUP, REASON_EFFECT) > 0 and tc:IsRelateToEffect(e) and tc:IsFaceup() then
    local e1 = Effect.CreateEffect(c)
...
function s.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
