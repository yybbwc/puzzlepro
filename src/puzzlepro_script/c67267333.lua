...
function c67267333.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67267333.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c67267333.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c67267333.rmfilter(chkc) and chkc:IsControler(1 - tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67267333.rmfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
