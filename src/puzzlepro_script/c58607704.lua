...
function c58607704.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc ~= e:GetLabelObject() and chkc:IsOnField() and c58607704.filter(chkc, ev)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c58607704.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetLabelObject(), ev)
  end
