...
function c51987571.condition(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler() == Duel.GetAttacker() and e:GetHandler():IsLocation(LOCATION_GRAVE) and e:GetHandler():IsReason(REASON_BATTLE)
end
...
function c51987571.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 51987572, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_ROCK, ATTRIBUTE_EARTH) then
    return
  end
  for i = 1, 2 do
