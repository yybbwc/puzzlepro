...
function c82913020.srtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c82913020.srfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c82913020.mcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsSetCard(0x53) and c:IsType(TYPE_XYZ) and c:IsRelateToBattle() and bc and bc:IsFaceup() and bc:IsAttribute(ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) and bc:IsRelateToBattle()
end
function c82913020.mtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local bc = e:GetHandler():GetBattleTarget()
  if chk == 0 then
    return bc and bc:IsRelateToBattle() and bc:IsAbleToRemove()
  end
