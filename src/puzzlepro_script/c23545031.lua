...
function c23545031.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c23545031.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c23545031.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousControler(tp) and (c:IsReason(REASON_EFFECT) and rp == 1 - tp or c:IsReason(REASON_BATTLE) and Duel.GetAttacker():IsControler(1 - tp))
end
...
function c23545031.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c23545031.spcfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c23545031.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c23545031.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c23545031.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
