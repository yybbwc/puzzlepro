...
function c44968459.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return ct1 > ct2 and ph >= PHASE_BATTLE_START and ph <= PHASE_BATTLE and Duel.IsExistingMatchingCard(c44968459.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
function c44968459.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct1 > 0 and Duel.IsPlayerCanDraw(tp, ct1) and ct2 > 0 and Duel.IsPlayerCanDraw(1 - tp, ct2)
  end
...
function c44968459.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44968459.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
