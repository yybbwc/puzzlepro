...
function c23979249.cfilter(c, tp)
  return c:IsSetCard(0x88) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE + LOCATION_GRAVE)
end
...
function c23979249.regfilter(c, tp)
  return c:IsSetCard(0x88) and c:IsControler(tp) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_DECK) and not c:IsReason(REASON_DRAW)
end
