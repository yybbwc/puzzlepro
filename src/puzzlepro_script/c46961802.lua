...
function c46961802.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c46961802.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c46961802.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc2 = g:GetNext()
  if tc1:IsFacedown() or tc2:IsFacedown() or not tc1:IsRelateToEffect(e) or not tc2:IsRelateToEffect(e) or not tc1:IsAttack(tc2:GetAttack()) then
    return
  end
