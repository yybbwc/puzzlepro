...
function c51786039.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and not c:IsPublic() and Duel.IsExistingMatchingCard(c51786039.spfilter, tp, LOCATION_HAND, 0, 1, c, e, tp)
  end
...
function c51786039.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) then
...
function c51786039.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c51786039.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51786039.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
