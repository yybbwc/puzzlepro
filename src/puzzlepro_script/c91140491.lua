...
function c91140491.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x53, 1, REASON_COST)
  end
...
function c91140491.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c91140491.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
