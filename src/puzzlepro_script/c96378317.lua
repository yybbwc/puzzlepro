...
function c96378317.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsController(tp) and c96378317.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c96378317.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c96378317.replaceft(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_BEAST + RACE_BEASTWARRIOR + RACE_WINDBEAST) and c:IsLinkState() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE)
end
function c96378317.replacetg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c96378317.replaceft, 1, nil, tp)
  end
