...
function c82977464.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  zone = zone & 0x1f
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c82977464.spfilter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c82977464.spfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp, zone)
  end
...
function c82977464.mvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c82977464.mvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82977464.mvfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0) > 0
  end
...
function c82977464.mvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = Duel.GetFirstTarget()
  if not tc:IsRelateToEffect(e) or tc:IsImmuneToEffect(e) or not tc:IsControler(tp) then
    return
  end
