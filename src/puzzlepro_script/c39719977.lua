...
function c39719977.filter(c, tp)
  local tpe = c:GetType()
  return c:IsFaceup() and bit.band(tpe, TYPE_NORMAL) ~= 0 and bit.band(tpe, TYPE_TOKEN) == 0 and Duel.IsExistingMatchingCard(c39719977.filter2, tp, LOCATION_MZONE, 0, 2, c, c:GetCode())
end
...
function c39719977.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsExistingMatchingCard(c39719977.filter, tp, LOCATION_MZONE, 0, 3, nil, tp)
  end
end
