...
function c59170782.tgcon1(e, tp, eg, ep, ev, re, r, rp)
  if rp == tp or not re:IsActiveType(TYPE_SPELL + TYPE_TRAP) or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
    return false
  end
  ...
  if tc == c or tc:IsControler(1 - tp) or tc:IsFacedown() or not tc:IsLocation(LOCATION_MZONE) or not tc:IsSetCard(0x74) then
    return false
  end
...
function c59170782.tgcon2(e, tp, eg, ep, ev, re, r, rp)
  if tp == Duel.GetTurnPlayer() or e:GetHandler():IsStatus(STATUS_CHAINING) then
    return false
  end
...
function c59170782.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c59170782.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c59170782.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
