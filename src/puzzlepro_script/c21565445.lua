...
function c21565445.cfilter(c, tp)
  return c:IsSetCard(0x77) and c:IsType(TYPE_MONSTER) and not c:IsCode(21565445) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c21565445.filter, tp, LOCATION_DECK, 0, 1, c)
end
function c21565445.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21565445.cfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
...
function c21565445.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c21565445.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return true
  end
