...
function c17810268.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x1019, 2, REASON_COST)
  end
...
function c17810268.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c17810268.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17810268.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
