...
function c40680521.sttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c40680521.stfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c40680521.stfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c40680521.pencon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsSummonType(SUMMON_TYPE_SPECIAL) and rp == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsFaceup()
end
