...
function c18491580.filter(c, e, tp)
  return c:IsSetCard(0x3b) and c:IsLevelBelow(7) and not c:IsCode(18491580) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c18491580.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c18491580.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c18491580.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
