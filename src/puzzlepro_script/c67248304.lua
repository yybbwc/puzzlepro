...
function c67248304.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.ChangePosition(g, POS_FACEDOWN_DEFENSE) ~= 0 then
    ...
    if rg:GetCount() > 0 then
      ...
      if setg1:GetCount() > 0 or setg2:GetCount() > 0 then
        Duel.BreakEffect()
      end
      if setg1:GetCount() > 0 then
        Duel.SSet(turnp, setg1, turnp, false)
      end
      if setg2:GetCount() > 0 then
        Duel.SSet(1 - turnp, setg2, 1 - turnp, false)
      end
    end
  end
end
