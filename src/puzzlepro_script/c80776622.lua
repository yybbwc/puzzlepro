...
function c80776622.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local ct = 1
  if Duel.IsExistingMatchingCard(c80776622.pfilter, tp, LOCATION_PZONE, 0, 1, nil) then
    ct = 2
  end
...
function c80776622.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (rc:IsRelateToEffect(re) and rc:IsLocation(LOCATION_MZONE) and re:IsActiveType(TYPE_MONSTER)) then
    return false
  end
