...
function c40371092.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsAttribute, 1, e:GetHandler(), ATTRIBUTE_LIGHT)
  end
...
function c40371092.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c40371092.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c40371092.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c40371092.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
