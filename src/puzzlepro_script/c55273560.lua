...
function c55273560.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c55273560.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c55273560.checkop(e, tp, eg, ep, ev, re, r, rp)
  if eg:IsExists(c55273560.checkfilter, 1, nil, 0) then
    Duel.RegisterFlagEffect(0, 55273560, RESET_PHASE + PHASE_END, 0, 1)
  end
  if eg:IsExists(c55273560.checkfilter, 1, nil, 1) then
    Duel.RegisterFlagEffect(1, 55273560, RESET_PHASE + PHASE_END, 0, 1)
  end
end
