...
function c61844784.addct(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c61844784.filter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61844784.filter, tp, LOCATION_SZONE, 0, 1, e:GetHandler())
  end
...
function c61844784.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return bit.band(r, REASON_RULE) == 0 and e:GetHandler():GetCounter(0x102a) > 0
  end
  return true
end
