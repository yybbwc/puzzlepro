...
function c86827882.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c86827882.filter(chkc)
  end
  local exc = nil
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c86827882.filter, tp, LOCATION_MZONE, 0, 1, exc)
  end
