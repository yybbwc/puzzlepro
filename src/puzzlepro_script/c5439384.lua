...
function c5439384.filter1(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsDefense(200) and c:IsType(TYPE_TUNER) and c:IsAbleToRemove() and Duel.IsExistingTarget(c5439384.filter2, tp, LOCATION_GRAVE, 0, 1, c, e, tp, c:GetLevel())
end
function c5439384.filter2(c, e, tp, lv)
  return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsDefense(200) and not c:IsType(TYPE_TUNER) and c:IsLevelAbove(0) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c5439384.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetLevel() + lv)
end
function c5439384.spfilter(c, e, tp, lv)
  return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsType(TYPE_SYNCHRO) and c:IsLevel(lv) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c5439384.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c5439384.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
