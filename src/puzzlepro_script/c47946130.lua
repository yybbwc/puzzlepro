...
function c47946130.immval(e, te)
  return te:GetOwner() ~= e:GetHandler() and te:IsActiveType(TYPE_MONSTER) and te:GetOwnerPlayer() ~= e:GetHandlerPlayer() and te:GetOwner():GetAttack() <= e:GetHandler():GetAttack() and te:IsActivated()
end
...
function c47946130.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if rp == tp or c:IsStatus(STATUS_BATTLE_DESTROYED) then
    return false
  end
