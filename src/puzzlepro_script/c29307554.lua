...
function c29307554.checkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  while tc do
    if tc:IsPreviousLocation(LOCATION_MZONE) and tc:IsPreviousPosition(POS_DEFENSE) then
      if tc:GetReasonPlayer() == 0 and tc:GetPreviousControler() == 1 then
        p1 = true
      end
      if tc:GetReasonPlayer() == 1 and tc:GetPreviousControler() == 0 then
        p2 = true
      end
    end
    tc = eg:GetNext()
  ...
  if p1 then
    Duel.RegisterFlagEffect(0, 29307554, RESET_PHASE + PHASE_END, 0, 1)
  end
  if p2 then
    Duel.RegisterFlagEffect(1, 29307554, RESET_PHASE + PHASE_END, 0, 1)
  end
end
...
function c29307554.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c29307554.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29307554.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
