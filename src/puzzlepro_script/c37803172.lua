...
function c37803172.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(c37803172.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c37803172.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c37803172.filter, tp, LOCATION_DECK, 0, 2, nil, e, tp)
  end
