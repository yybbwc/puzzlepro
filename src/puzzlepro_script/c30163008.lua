...
function c30163008.cfilter(c, e, tp, lg, zone)
  return c:IsFaceup() and lg:IsContains(c) and Duel.IsExistingMatchingCard(c30163008.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetRace(), zone)
end
...
function c30163008.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c30163008.chkfilter(chkc, e, tp, lg, e:GetLabel())
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE, tp, LOCATION_REASON_TOFIELD, zone) > 0 and Duel.IsExistingTarget(c30163008.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e, tp, lg, zone)
  end
...
function c30163008.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not tc:IsRelateToEffect(e) or tc:IsFacedown() then
    return
  end
  local zone = c:GetLinkedZone(tp)
