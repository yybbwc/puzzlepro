...
function c91677585.filter(c)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:GetBattledGroupCount() == 1 and c:GetEffectCount(EFFECT_EXTRA_ATTACK) == 0
end
function c91677585.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c91677585.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91677585.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
