...
function c44553392.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Destroy(tc, REASON_EFFECT) > 0 then
    ...
    if tc:IsOriginalCodeRule(65815684) and ft > 0 and #sg > 0 and Duel.SelectYesNo(tp, aux.Stringid(44553392, 0)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
      local sg1 = sg:Select(tp, 1, 1, nil)
      Duel.SpecialSummon(sg1, 0, tp, tp, false, false, POS_FACEUP)
    elseif not tc:IsOriginalCodeRule(65815684) and #ag > 0 and Duel.SelectYesNo(tp, aux.Stringid(44553392, 1)) then
      Duel.BreakEffect()
