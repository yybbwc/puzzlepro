...
function c35960413.lvfilter(c, tp)
  return c:IsFaceup() and c:IsLevelAbove(1) and c:IsRace(RACE_FAIRY) and c:IsAttribute(ATTRIBUTE_LIGHT) and Duel.IsExistingTarget(c35960413.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c, c:GetLevel())
end
...
function c35960413.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c35960413.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
