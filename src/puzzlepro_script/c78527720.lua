...
function c78527720.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c78527720.filter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c78527720.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsEnvironment(72283691, PLAYER_ALL, LOCATION_FZONE) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c78527720.efilter, tp, LOCATION_DECK, 0, 1, nil, c) and c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SelectYesNo(tp, aux.Stringid(78527720, 1)) then
    Duel.BreakEffect()
...
function c78527720.eqfilter(c, tc, tp)
  return c:IsCode(9677699) and tc:GetEquipGroup():IsContains(c) and Duel.IsExistingTarget(c78527720.eqtfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, tc, c)
end
...
function c78527720.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingTarget(c78527720.eqfilter, tp, LOCATION_SZONE, 0, 1, nil, c, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_EQUIP)
...
function c78527720.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  local sc = g:GetNext()
  if tc:IsFacedown() or not tc:IsRelateToEffect(e) or sc:IsFacedown() or not sc:IsRelateToEffect(e) then
    return
  end
