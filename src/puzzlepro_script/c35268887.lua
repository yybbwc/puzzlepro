...
function c35268887.tokentg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and (ct == 1 or not Duel.IsPlayerAffectedByEffect(tp, 59822133)) and Duel.GetLocationCount(tp, LOCATION_MZONE) >= ct and Duel.IsPlayerCanSpecialSummonMonster(tp, 35268888, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE)
  end
...
function c35268887.tokenop(e, tp, eg, ep, ev, re, r, rp)
  local ct = c35268887[tp]
  if (ct > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) or Duel.GetLocationCount(tp, LOCATION_MZONE) < ct or not Duel.IsPlayerCanSpecialSummonMonster(tp, 35268888, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE) then
    return
  end
  for i = 1, ct do
