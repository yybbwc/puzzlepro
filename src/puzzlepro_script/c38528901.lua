...
function c38528901.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c38528901.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c38528901.desfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c38528901.sfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
