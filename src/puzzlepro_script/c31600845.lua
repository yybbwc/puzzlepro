...
function c31600845.dhtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc ~= c and c31600845.xfilter(chkc)
  end
  if chk == 0 then
    return c:IsAbleToDeck() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingTarget(c31600845.xfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c) and Duel.IsExistingMatchingCard(c31600845.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c31600845.dhop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or Duel.SendtoDeck(c, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) == 0 or c:GetLocation() ~= LOCATION_DECK then
    return
  end
  ...
  if Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP) == 0 or not tc:IsRelateToEffect(e) or tc:IsFacedown() then
    return
  end
  Duel.BreakEffect()
  local e1 = Effect.CreateEffect(c)
