...
function c98022050.desfilter(c, tp, solve)
  return c:IsRace(RACE_DINOSAUR) and not c:IsCode(98022050) and (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and (solve or Duel.IsExistingMatchingCard(c98022050.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetOriginalLevel()))
end
...
function c98022050.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98022050.desfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, tp)
  end
