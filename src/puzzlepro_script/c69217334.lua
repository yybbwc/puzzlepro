...
function c69217334.lvfilter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_RITUAL) and Duel.IsExistingMatchingCard(c69217334.lvcfilter, tp, LOCATION_HAND, 0, 1, nil, c)
end
...
function c69217334.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c69217334.lvfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c69217334.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
