...
function c44478599.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c44478599.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c44478599.mttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c44478599.cfilter(chkc, lg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44478599.cfilter, tp, LOCATION_MZONE, 0, 1, nil, lg) and Duel.GetMatchingGroupCount(c44478599.valfilter, tp, LOCATION_MZONE, 0, nil) > 1
  end
