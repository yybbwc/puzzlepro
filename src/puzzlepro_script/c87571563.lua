...
function c87571563.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c87571563.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return true
  end
  if Duel.IsExistingTarget(c87571563.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(87571563, 1)) then
...
function c87571563.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    if aux.NecroValleyNegateCheck(tc) then
      return
    end
    if not aux.NecroValleyFilter()(tc) then
      return
    end
    if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not tc:IsAbleToHand() or Duel.SelectOption(tp, 1190, 1152) == 1) then
...
function c87571563.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c87571563.seqfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c87571563.seqfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0) > 0
  end
...
function c87571563.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = Duel.GetFirstTarget()
  if not c:IsRelateToEffect(e) or not tc:IsRelateToEffect(e) or tc:IsControler(1 - tp) or tc:IsImmuneToEffect(e) or Duel.GetLocationCount(tp, LOCATION_MZONE, PLAYER_NONE, 0) <= 0 then
    return
  end
