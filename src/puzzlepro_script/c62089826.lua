...
function c62089826.filter2(c, tp)
  return aux.IsCodeListed(c, 89631139) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsSSetable() and not Duel.IsExistingMatchingCard(c62089826.cfilter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil, c:GetCode())
end
function c62089826.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local b1 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c62089826.filter1, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
