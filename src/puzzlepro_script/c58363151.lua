...
function c58363151.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c58363151.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c58363151.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp) and Duel.GetMZoneCount(tp) > 0
  end
