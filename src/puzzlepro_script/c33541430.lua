...
function c33541430.drop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) ~= 0 then
    ...
    if tc:IsType(TYPE_MONSTER) and tc:IsSetCard(0xe6) then
      if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(33541430, 1)) and Duel.SpecialSummonStep(tc, 0, tp, tp, true, false, POS_FACEUP) then
        local e1 = Effect.CreateEffect(e:GetHandler())
...
function c33541430.syncheck(c, g, mg, tp, lv, syncard, minc, maxc)
  g:AddCard(c)
  local ct = g:GetCount()
  local res = c33541430.syngoal(g, tp, lv, syncard, minc, ct) or (ct < maxc and mg:IsExists(c33541430.syncheck, 1, g, g, mg, tp, lv, syncard, minc, maxc))
  g:RemoveCard(c)
  return res
end
function c33541430.syngoal(g, tp, lv, syncard, minc, ct)
  return ct >= minc and Duel.GetLocationCountFromEx(tp, tp, g, syncard) > 0 and (g:CheckWithSumEqual(Card.GetSynchroLevel, lv, ct, ct, syncard) or g:CheckWithSumEqual(c33541430.cardiansynlevel, lv, ct, ct, syncard)) and aux.MustMaterialCheck(g, tp, EFFECT_MUST_BE_SMATERIAL)
end
function c33541430.syntg(e, syncard, f, min, max)
  ...
  if lv <= c:GetLevel() and lv <= c33541430.cardiansynlevel(c) then
    return false
  end
