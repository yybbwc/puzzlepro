...
function c84740193.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c84740193.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c84740193.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c84740193.atkcon(e)
  ...
  return Duel.GetCurrentPhase() == PHASE_DAMAGE_CAL and bc and ((bc:IsAttackPos() and bc:IsAttackAbove(2500)) or (bc:IsDefensePos() and bc:IsDefenseAbove(2500)))
end
