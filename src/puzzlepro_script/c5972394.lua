...
function c5972394.filter(c, e, tp)
  return c:IsLevelBelow(2) and c:IsPosition(POS_FACEUP_DEFENSE) and Duel.IsExistingMatchingCard(c5972394.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c5972394.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c5972394.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c5972394.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
