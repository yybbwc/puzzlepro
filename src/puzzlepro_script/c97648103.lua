...
function c97648103.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c97648103.thfilter(chkc, e)
  end
...
function c97648103.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsReason(REASON_BATTLE) and c:IsType(TYPE_LINK) and c:IsLinkState()
end
function c97648103.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c97648103.repfilter, 1, nil, tp)
  end
