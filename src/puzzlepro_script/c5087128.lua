...
function c5087128.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDestructable() and g:GetCount() > 0
  end
...
function c5087128.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.Destroy(c, REASON_EFFECT) > 0 then
    ...
    if seq < 5 then
      dg = Duel.GetMatchingGroup(c5087128.desfilter2, tp, LOCATION_MZONE, LOCATION_MZONE, nil, seq, tc:GetControler())
    end
...
function c5087128.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c5087128.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
