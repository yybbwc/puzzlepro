...
function c66938505.scfilter1(c, e, tp, mc)
  ...
  return not c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c66938505.scfilter2, tp, LOCATION_EXTRA, 0, 1, nil, mg)
end
...
function c66938505.sctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c66938505.scfilter1(chkc, e, tp, e:GetHandler())
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c66938505.scfilter1, tp, LOCATION_REMOVED, 0, 1, nil, e, tp, e:GetHandler())
  end
