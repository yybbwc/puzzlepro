...
function c24269961.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c24269961.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c24269961.cfilter(c, tp, se)
  return c:GetPreviousControler() == tp and c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousLocation(LOCATION_ONFIELD) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function c24269961.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c24269961.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp)
  end
