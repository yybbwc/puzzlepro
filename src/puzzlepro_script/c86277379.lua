...
function c86277379.regcon2(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c86277379.cfilter2, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c86277379.cfilter2, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c86277379.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86277379.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c86277379.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoGrave(tc, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c86277379.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(86277379, 2)) then
    Duel.BreakEffect()
