...
function c5352328.effop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) > 0 and Duel.IsExistingMatchingCard(c5352328.cfilter1, tp, LOCATION_ONFIELD, 0, 1, nil) then
    Duel.BreakEffect()
...
function c5352328.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c5352328.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
