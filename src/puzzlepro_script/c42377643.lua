...
function c42377643.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c42377643.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c42377643.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    ...
    if tc:IsCode(24639891) then
      if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(42377643, 2)) then
        Duel.BreakEffect()
...
function c42377643.lvltg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c42377643.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c42377643.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c42377643.lvlop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.IsExistingMatchingCard(c42377643.thfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(42377643, 5)) then
    Duel.BreakEffect()
