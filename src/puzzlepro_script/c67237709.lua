...
function c67237709.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c67237709.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67237709.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function c67237709.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp) and Duel.IsExistingMatchingCard(c67237709.drfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c67237709.thtg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67237709.thfilter2, tp, LOCATION_DECK, 0, 1, nil)
  end
