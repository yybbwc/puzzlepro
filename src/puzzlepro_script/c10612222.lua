...
function c10612222.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and chkc:IsFaceup() and chkc:IsAbleToHand()
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c10612222.cfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingTarget(c10612222.tgfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
  local ct = 1
  if Duel.IsExistingTarget(c10612222.tgfilter, tp, 0, LOCATION_ONFIELD, 2, nil) then
    ct = 2
  end
