...
function c48214588.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  if chk == 0 then
    return Duel.GetCustomActivityCount(48214588, tp, ACTIVITY_ATTACK) == 0
  end
...
function c48214588.cfilter(c, e, tp)
  local lv = c:GetLevel()
  return lv > 0 and (c:IsControler(tp) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c48214588.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, lv, e, tp)
end
