...
function c3909436.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c3909436.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c3909436.thfilter(c, tp)
  return bit.band(c:GetType(), TYPE_RITUAL + TYPE_MONSTER) == TYPE_RITUAL + TYPE_MONSTER and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c3909436.tgfilter, tp, LOCATION_DECK, 0, 1, c)
end
...
function c3909436.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c3909436.thfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
