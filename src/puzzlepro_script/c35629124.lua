...
function c35629124.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetActivityCount(tp, ACTIVITY_BATTLE_PHASE) == 0 and Duel.GetCustomActivityCount(35629124, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c35629124.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c35629124.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
