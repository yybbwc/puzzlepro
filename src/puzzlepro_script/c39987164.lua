...
function c39987164.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c39987164.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c39987164.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c39987164.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dt == c then
    dt = Duel.GetAttackTarget()
  end
  if not dt or dt:IsFacedown() then
    return false
  end
  e:SetLabelObject(dt)
