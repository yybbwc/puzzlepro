...
function c82744076.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(82744076, tp, ACTIVITY_ATTACK) == 0
  end
...
function c82744076.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c82744076.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82744076.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
