...
function c75952542.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c75952542.setfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
...
function c75952542.stfilter(c, tp)
  ...
  return seq <= 4 and c:IsType(TYPE_EFFECT) and c:IsFaceup() and Duel.IsExistingMatchingCard(c75952542.cfilter, tp, LOCATION_MZONE, 0, 1, nil, seq)
end
...
function c75952542.sttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c75952542.stfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c75952542.stfilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
...
function c75952542.stop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) and not tc:IsImmuneToEffect(e)) then
    return
  end
