...
function c41803903.thfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:IsSetCard(0x10ec) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c41803903.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
end
function c41803903.spfilter(c, e, tp, hc)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:IsSetCard(0x10ec) and not c:IsOriginalCodeRule(hc:GetOriginalCodeRule()) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and Duel.GetLocationCountFromEx(tp, tp, hc, c) > 0
end
function c41803903.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41803903.thfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c41803903.setcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN) and Duel.IsExistingMatchingCard(c41803903.filter2, tp, LOCATION_EXTRA, 0, 1, nil)
end
...
function c41803903.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41803903.setfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
