...
function c98875863.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98875863.lvcfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c98875863.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetMZoneCount(tp, e:GetHandler()) > 2 and Duel.IsPlayerCanSpecialSummonMonster(tp, 98875864, 0x186, TYPES_TOKEN_MONSTER, 0, 0, 3, RACE_CYBERSE, ATTRIBUTE_EARTH, POS_FACEUP_DEFENSE)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 3, 0, 0)
  Duel.SetOperationInfo(0, CATEGORY_TOKEN, nil, 3, 0, 0)
end
function c98875863.tkop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 2 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonMonster(tp, 98875864, 0x186, TYPES_TOKEN_MONSTER, 0, 0, 3, RACE_CYBERSE, ATTRIBUTE_EARTH, POS_FACEUP_DEFENSE) then
    local ct = 3
    while ct > 0 do
