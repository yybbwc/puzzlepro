...
function c42956963.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 2 and Duel.IsPlayerCanSpecialSummonMonster(tp, 42956964, 0x45, TYPES_TOKEN_MONSTER, 2000, 2000, 6, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_ATTACK, 1 - tp)
  end
...
function c42956963.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 42956964, 0x45, TYPES_TOKEN_MONSTER, 2000, 2000, 6, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_ATTACK, 1 - tp) then
    return
  end
  for i = 1, 3 do
