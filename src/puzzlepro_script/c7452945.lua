...
function c7452945.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c7452945.eqfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c7452945.eqfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c7452945.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c7452945.eqfilter2(chkc)
  end
  if chk == 0 then
    return e:GetHandler():IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c7452945.eqfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_EQUIP)
...
function c7452945.operation2(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local tc = Duel.GetFirstTarget()
  if tc and c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(tp) and tc:IsSetCard(0x107a) and c7452945.eqlimit(nil, tc) and c:CheckUniqueOnField(tp) then
    Duel.Equip(tp, c, tc)
  end
end
