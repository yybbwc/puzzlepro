...
function c38250531.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c38250531.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c38250531.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function c38250531.spreg(e, tp, eg, ep, ev, re, r, rp)
  ...
  if rp == 1 - tp and c:IsReason(REASON_DESTROY) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) then
