...
function c50186558.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c50186558.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c50186558.atkfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c50186558.cfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c50186558.repfilter(c, tp)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsRace(RACE_DRAGON) and c:IsControler(tp) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
...
function c50186558.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:GetCount() == 1 and eg:IsExists(c50186558.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c50186558.tgfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
