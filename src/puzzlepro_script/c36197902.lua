...
function c36197902.cfilter(c, e)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xfe) and c:GetBaseAttack() > 0 and c:IsAbleToRemoveAsCost() and (c:IsFaceup() or not c:IsLocation(LOCATION_MZONE)) and Duel.IsExistingMatchingCard(c36197902.filter, 0, LOCATION_MZONE, LOCATION_MZONE, 1, c, e)
end
...
function c36197902.cfilter2(c, tp)
  return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and bit.band(c:GetPreviousTypeOnField(), TYPE_LINK) ~= 0 and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)
end
...
function c36197902.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c36197902.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
