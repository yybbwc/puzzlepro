...
function c15381421.cfilter(c)
  return c:IsRace(RACE_DRAGON) and c:GetOriginalLevel() > 0 and c:IsAbleToGraveAsCost() and (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
end
...
function c15381421.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.IsExistingMatchingCard(c15381421.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp, rg)
  end
  local g = Duel.GetMatchingGroup(c15381421.filter, tp, LOCATION_DECK, 0, nil, e, tp, rg)
  local lvt = {}
  local pc = 1
  for i = 1, 12 do
    if g:IsExists(c15381421.thfilter, 1, nil, i) then
      lvt[pc] = i
      pc = pc + 1
    end
  end
...
function c15381421.thtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c15381421.thfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c15381421.thfilter2, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
