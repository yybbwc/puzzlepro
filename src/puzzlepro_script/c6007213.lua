...
function c6007213.spfilter(c, check)
  return c:IsAbleToGraveAsCost() and c:IsType(TYPE_TRAP) and (c:IsFaceup() or check and c:IsFacedown())
end
...
function c6007213.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(1 - tp) and c6007213.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c6007213.desfilter, tp, 0, LOCATION_SZONE, 1, nil)
  end
