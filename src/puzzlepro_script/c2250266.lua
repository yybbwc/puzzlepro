...
function c2250266.tgd2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c2250266.filter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c2250266.opd2(e, tp, eg, ep, ev, re, r, rp)
  ...
  local c = e:GetHandler()
  if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsAttackPos() and Duel.ChangePosition(tc, POS_FACEUP_DEFENSE) ~= 0 and c:IsRelateToBattle() then
