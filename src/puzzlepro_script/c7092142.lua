...
function c7092142.cfilter(c, tp)
  return c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsPreviousPosition(POS_FACEUP) and c:IsAttribute(ATTRIBUTE_WATER) and bit.band(c:GetPreviousAttributeOnField(), ATTRIBUTE_WATER) ~= 0
end
...
function c7092142.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return ct > 0 and (ct == 1 or not Duel.IsPlayerAffectedByEffect(tp, 59822133)) and Duel.GetLocationCount(tp, LOCATION_MZONE) >= ct
  end
