...
function c11327848.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c11327848.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(c11327848.tgfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c11327848.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1 and tc2 and tc2:IsFaceup() then
    ...
    if not Duel.Equip(tp, tc1, tc2, false) then
      return
    end
    local e1 = Effect.CreateEffect(c)
