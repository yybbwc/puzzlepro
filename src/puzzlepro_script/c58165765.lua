...
function c58165765.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58165765.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c58165765.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c58165765.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c58165765.filter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c58165765.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and tc:IsRelateToEffect(e) and tc:IsFacedown() then
    ...
    if tc:IsAbleToDeck() then
      if tc:IsExtraDeckMonster() or Duel.SelectOption(tp, aux.Stringid(58165765, 1), aux.Stringid(58165765, 2)) == 0 then
