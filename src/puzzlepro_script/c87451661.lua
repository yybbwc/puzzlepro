...
function s.spfilter(c, e, tp)
  return c:IsCode(70902743) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc then
    ...
    if Duel.SpecialSummon(tc, SUMMON_TYPE_SYNCHRO, tp, tp, false, false, POS_FACEUP) > 0 then
      ...
      if dg:GetCount() > 0 and e:GetLabel() > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 1)) then
        Duel.BreakEffect()
