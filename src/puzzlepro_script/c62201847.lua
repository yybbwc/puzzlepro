...
function c62201847.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62201847.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c62201847.spcostfilter(c, e, tp, tc)
  ...
  return c:IsAbleToRemoveAsCost() and c62201847.spcostexcheck(c, e, tp) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and Duel.GetMZoneCount(tp, tg) > 0
end
function c62201847.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    e:SetLabel(100)
    return Duel.IsExistingMatchingCard(c62201847.spcostfilter, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, e:GetHandler(), e, tp, e:GetHandler()) and e:GetHandler():IsReleasable()
  end
...
function c62201847.spopfilter(c, e, tp, typ)
  return (((typ & TYPE_MONSTER) > 0 and c:IsCode(34695290)) or ((typ & TYPE_SPELL) > 0 and c:IsCode(61089209)) or ((typ & TYPE_TRAP) > 0 and c:IsCode(7574904))) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
