...
function c19535693.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19535693.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c19535693.atktg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c19535693.atkfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19535693.atkfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
