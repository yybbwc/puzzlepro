...
function c90812044.filter(c, e, tp, rk)
  return c:IsType(TYPE_XYZ) and c:GetRank() < rk and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c90812044.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return rg:IsContains(chkc)
  end
  if chk == 0 then
    return g:GetCount() >= 2 and rg:IsExists(Card.IsCanBeEffectTarget, 1, nil, e) and Duel.IsExistingMatchingCard(c90812044.filter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, rk)
  end
