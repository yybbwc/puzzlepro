...
function c19280589.tktg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 19280590, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT)
  end
...
function c19280589.tkcost2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19280589.tkcsfilter, tp, LOCATION_HAND, 0, 1, c) and c:IsAbleToRemoveAsCost()
  end
...
function c19280589.tktg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsPlayerCanSpecialSummonMonster(tp, 19280590, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT)
  end
...
function c19280589.tkop2(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonMonster(tp, 19280590, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FAIRY, ATTRIBUTE_LIGHT) then
    for i = 1, 2 do
      local token = Duel.CreateToken(tp, 19280590)
      Duel.SpecialSummon(token, 0, tp, tp, false, false, POS_FACEUP)
    end
