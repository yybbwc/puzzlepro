...
function c29143457.filter2(c, e, tp, m, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsRace(RACE_BEASTWARRIOR) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, nil, chkf)
end
...
function c29143457.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c29143457.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29143457.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
