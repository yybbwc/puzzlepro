...
function c76382116.spfilter(c, e, tp, ...)
  return (c:IsSetCard(0x134) or c:IsRace(RACE_MACHINE)) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and not c:IsCode(...)
end
function c76382116.costfilter1(c, e, tp)
  return (c:IsSetCard(0x134) or c:IsRace(RACE_MACHINE)) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c76382116.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetCode())
end
function c76382116.costfilter2(c, e, tp)
  return (c:IsSetCard(0x134) or c:IsRace(RACE_MACHINE)) and Duel.IsExistingMatchingCard(c76382116.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetCode())
end
...
function c76382116.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c76382116.costfilter1, 1, nil, e, tp)
  end
