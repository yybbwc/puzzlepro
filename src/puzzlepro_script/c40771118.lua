...
function c40771118.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c40771118.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c40771118.thfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
...
function c40771118.plcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and c:IsStatus(STATUS_EFFECT_ENABLED)
  end
  Duel.SendtoGrave(c, REASON_COST)
end
function c40771118.plfilter(c, tp, mc)
  if not c:IsSetCard(0x1c) then
    return false
  end
  if Duel.IsPlayerAffectedByEffect(tp, 16625614) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, c:GetCode(), 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP, tp, SUMMON_VALUE_DARK_SANCTUARY) then
    return true
  end
...
function c40771118.pltg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c40771118.plfilter, tp, LOCATION_HAND + LOCATION_GRAVE + LOCATION_DECK, 0, 1, nil, tp, e:GetHandler())
  end
end
function c40771118.plop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.IsPlayerAffectedByEffect(tp, 16625614) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, tc:GetCode(), 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP, tp, SUMMON_VALUE_DARK_SANCTUARY) and Duel.SelectYesNo(tp, aux.Stringid(16625614, 0)) then
