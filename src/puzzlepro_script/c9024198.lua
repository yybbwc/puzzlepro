...
function c9024198.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckLocation(tp, LOCATION_PZONE, 0) and Duel.CheckLocation(tp, LOCATION_PZONE, 1) and g:GetClassCount(Card.GetCode) >= 2
  end
...
function c9024198.setop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.CheckLocation(tp, LOCATION_PZONE, 0) or not Duel.CheckLocation(tp, LOCATION_PZONE, 1) then
    return
  end
...
function c9024198.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousControler(tp) and c:IsSummonType(SUMMON_TYPE_LINK) and c:IsPreviousLocation(LOCATION_MZONE) and (c:IsReason(REASON_EFFECT) and rp == 1 - tp or c:IsReason(REASON_BATTLE) and Duel.GetAttacker():IsControler(1 - tp))
end
function c9024198.spfilter(c, e, tp)
  return c:IsSetCard(0xaf) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and (c:IsLocation(LOCATION_GRAVE) and Duel.GetMZoneCount(tp) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)
end
function c9024198.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9024198.spfilter, tp, LOCATION_GRAVE + LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
