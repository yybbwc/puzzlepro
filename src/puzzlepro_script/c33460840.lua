...
function c33460840.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c33460840.filter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e:GetHandler())
  end
...
function c33460840.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if not (tc and Duel.Equip(tp, tc, c)) then
    return
  end
...
function c33460840.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReleasable() and Duel.CheckReleaseGroup(tp, nil, 1, c)
  end
...
function c33460840.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c33460840.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -2 and Duel.IsExistingTarget(c33460840.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
