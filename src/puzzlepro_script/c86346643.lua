...
function c86346643.tdcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86346643.cfilter1, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c86346643.tdcost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86346643.cfilter2, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
...
function c86346643.tdtg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86346643.filter2, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c86346643.tdtg3(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToDeck, tp, 0, LOCATION_GRAVE, 1, nil)
  end
