...
function c85651167.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c85651167.filter1, 1, nil, e:GetHandler())
  end
...
function c85651167.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Destroy(g, REASON_EFFECT) ~= 0 and sg:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(85651167, 2)) then
    Duel.BreakEffect()
...
function c85651167.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:IsExists(c85651167.spcostfilter, 1, nil, tp)
  end
...
function c85651167.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c85651167.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c85651167.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
