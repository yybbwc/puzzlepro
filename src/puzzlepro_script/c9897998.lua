...
function c9897998.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsRelateToBattle() and ((a == c and d:IsLocation(LOCATION_GRAVE) and d:IsType(TYPE_MONSTER)) or (d == c and a:IsLocation(LOCATION_GRAVE) and a:IsType(TYPE_MONSTER)))
end
...
function c9897998.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c9897998.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
