...
function c57473560.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c57473560.tgfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, 32274490) and Duel.IsExistingMatchingCard(c57473560.tgfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, 40991587)
  end
...
function c57473560.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c57473560.cfilter, tp, LOCATION_GRAVE, 0, 2, e:GetHandler())
  end
...
function c57473560.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c57473560.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
