...
function c76067258.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and chkc ~= c and c76067258.filter(chkc, c)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c76067258.filter, tp, LOCATION_ONFIELD, 0, 1, c, c)
  end
...
function c76067258.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFaceup() and c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
    c:SetCardTarget(tc)
...
function c76067258.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:GetCardTarget():IsExists(c76067258.repfilter, 1, nil, tp) and c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
  end
