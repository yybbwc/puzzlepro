...
function c9715126.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) ~= 0 and not Duel.IsExistingMatchingCard(c9715126.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
function c9715126.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsPosition, tp, 0, LOCATION_MZONE, 1, nil, POS_ATTACK)
  end
