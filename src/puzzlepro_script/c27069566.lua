...
function c27069566.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c27069566.rmfilter, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingTarget(Card.IsAbleToRemove, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
...
function c27069566.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousControler(tp) and c:IsSummonType(SUMMON_TYPE_XYZ) and c:IsPreviousLocation(LOCATION_MZONE) and (c:IsReason(REASON_EFFECT) and rp == 1 - tp or c:IsReason(REASON_BATTLE) and Duel.GetAttacker():IsControler(1 - tp))
end
...
function c27069566.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return Duel.GetLocationCount(tp, LOCATION_MZONE) >= 2 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and g:CheckSubGroup(c27069566.fselect, 2, 2)
  end
