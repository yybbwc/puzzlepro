...
function c70238111.filter(c, e, tp)
  return c:IsRace(RACE_CYBERSE) and c:IsLink(3) and (c:IsFaceup() or not c:IsLocation(LOCATION_MZONE)) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c70238111.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
end
...
function c70238111.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_MZONE) and chkc:IsControler(tp) and c70238111.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70238111.filter, tp, LOCATION_GRAVE + LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c70238111.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c70238111.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c70238111.spfilter2, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
