...
function c94392192.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94392192.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c94392192.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and g:GetFirst():IsAbleToRemove(tp, POS_FACEDOWN)
  end
...
function c94392192.rmtg2(e, tp, eg, ep, ev, re, r, rp, chk)
  local g = Duel.GetDecktopGroup(1 - tp, 1)
  if chk == 0 then
    return rp == 1 - tp and #g > 0 and g:GetFirst():IsAbleToRemove(tp, POS_FACEDOWN)
  end
  Duel.SetOperationInfo(0, CATEGORY_REMOVE, nil, 1, 1 - tp, LOCATION_DECK)
end
