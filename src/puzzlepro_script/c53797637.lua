...
function c53797637.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and Duel.IsExistingMatchingCard(c53797637.costfilter, tp, LOCATION_HAND, 0, 1, c)
  end
...
function c53797637.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c53797637.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
