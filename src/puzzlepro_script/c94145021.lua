...
function c94145021.regcon(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetCurrentPhase() == PHASE_DRAW or Duel.GetCurrentPhase() == 0 then
    return false
  end
  local v = 0
  if eg:IsExists(c94145021.cfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c94145021.cfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
