...
function s.cfilter(c, re)
  if not c:IsType(TYPE_SPELL + TYPE_TRAP) or not c:IsReason(REASON_COST) then
    return false
  end
  if c:IsPreviousLocation(LOCATION_ONFIELD) and c:GetPreviousTypeOnField() & (TYPE_SPELL + TYPE_TRAP) == 0 then
    return false
  end
  local recode = re:GetCode()
  return re:IsActivated() or recode == EFFECT_TRAP_ACT_IN_HAND or recode == EFFECT_TRAP_ACT_IN_SET_TURN or recode == EFFECT_QP_ACT_IN_NTPHAND or recode == EFFECT_QP_ACT_IN_SET_TURN
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.tgfilter, tp, LOCATION_DECK + LOCATION_HAND + LOCATION_EXTRA, 0, 1, nil)
  end
...
function s.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(s.rmfilter, tp, LOCATION_GRAVE, 0, 2, c)
  end
