...
function c22007085.filter(c)
  return c:IsAbleToHand() and (c:IsSetCard(0x159) and c:IsType(TYPE_MONSTER) or c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsLevel(7))
end
function c22007085.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22007085.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c22007085.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    Duel.ShuffleHand(tp)
    if Duel.IsExistingMatchingCard(c22007085.cfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 and Duel.IsExistingMatchingCard(c22007085.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(22007085, 0)) then
      Duel.BreakEffect()
