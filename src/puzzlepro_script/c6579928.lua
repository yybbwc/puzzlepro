...
function c6579928.thfilter(c, tp)
  return c:IsSetCard(0x103d) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not Duel.IsExistingMatchingCard(c6579928.filter, tp, LOCATION_MZONE, 0, 1, nil, c:GetAttribute())
end
function c6579928.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6579928.thfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
...
function c6579928.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c6579928.repfilter, 1, nil, tp) and eg:GetCount() == 1
  end
