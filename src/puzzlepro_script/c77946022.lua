...
function s.cfilter(c, tp)
  return c:IsRace(RACE_WYRM) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingTarget(nil, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
end
...
function s.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc, exc)
  if chk == 0 then
    return e:GetHandler():IsAbleToGrave() and Duel.IsExistingMatchingCard(s.setfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
