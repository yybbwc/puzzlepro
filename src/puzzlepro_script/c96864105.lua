...
function c96864105.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:CheckRemoveOverlayCard(tp, 1, REASON_COST) and c:GetFlagEffect(96864105) == 0
  end
...
function c96864105.atkop(e, tp, eg, ep, ev, re, r, rp)
  local a = Duel.GetAttacker()
  local d = Duel.GetAttackTarget()
  if not a:IsRelateToBattle() or a:IsFacedown() or not d:IsRelateToBattle() or d:IsFacedown() then
    return
  end
