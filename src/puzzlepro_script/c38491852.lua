...
function s.thfilter(c, tp)
  return c:IsSetCard(0x1a3) and c:IsType(TYPE_PENDULUM) and (c:IsAbleToExtra() or c:IsAbleToHand())
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = g:CheckSubGroup(s.Group, 2, 2) and Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  if chk == 0 then
    return b1 or b2
  end
end
