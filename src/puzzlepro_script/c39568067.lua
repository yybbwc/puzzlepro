...
function c39568067.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c39568067.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c39568067.eqfilter(c, equip, tp)
  return c:IsType(TYPE_EQUIP) and aux.IsCodeListed(c, 3285552) and (c:IsAbleToHand() or equip and c:CheckUniqueOnField(tp) and not c:IsForbidden())
end
...
function c39568067.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    local equip = Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c39568067.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
...
function c39568067.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if g:GetCount() > 0 then
    if equip and tc:CheckUniqueOnField(tp) and not tc:IsForbidden() and (not tc:IsAbleToHand() or Duel.SelectOption(tp, 1190, aux.Stringid(39568067, 2)) == 1) then
