...
function c25908748.cfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c25908748.srfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetOriginalRace())
end
...
function c25908748.limcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rp == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_SZONE) and Duel.GetTurnPlayer() == 1 - tp and aux.bpcon(e, tp, eg, ep, ev, re, r, rp)
end
