...
function c96462121.tgfilter(c, e, tp)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x127) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c96462121.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c:GetRank() + 2, c)
end
function c96462121.spfilter(c, e, tp, rank, mc)
  return c:IsRace(RACE_MACHINE) and c:IsType(TYPE_XYZ) and c:IsRank(rank) and mc:IsCanBeXyzMaterial(c, tp) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c96462121.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c96462121.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c96462121.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c96462121.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsControler(tp) and tc:IsFaceup() and aux.MustMaterialCheck(tc, tp, EFFECT_MUST_BE_XMATERIAL) and not tc:IsImmuneToEffect(e) then
