...
function c63468625.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c63468625.spfilter, c:GetControler(), LOCATION_HAND, 0, 3, c)
end
...
function c63468625.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c63468625.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c63468625.eqfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c63468625.damcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetEquipGroup():IsExists(c63468625.dcfilter, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
