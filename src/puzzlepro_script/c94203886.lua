...
function c94203886.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c94203886.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c94203886.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c94203886.lvfilter(chkc, e:GetHandler():GetLevel())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c94203886.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, e:GetHandler():GetLevel())
  end
