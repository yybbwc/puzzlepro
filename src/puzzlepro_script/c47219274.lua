...
function c47219274.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (c == Duel.GetAttacker() or c == Duel.GetAttackTarget()) and c:GetSequence() > 4
end
function c47219274.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetDecktopGroup(1 - tp, 2):FilterCount(Card.IsAbleToRemove, nil) == 2
  end
...
function c47219274.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c47219274.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c47219274.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
