...
function c89558743.same_check(c, mc)
  local flag = 0
  if c:GetRace() == mc:GetRace() then
    flag = flag + 1
  end
  if c:GetAttribute() == mc:GetAttribute() then
    flag = flag + 1
  end
  if c:GetLevel() == mc:GetLevel() then
    flag = flag + 1
  end
  if c:GetTextAttack() == mc:GetTextAttack() then
    flag = flag + 1
  end
  if c:GetTextDefense() == mc:GetTextDefense() then
    flag = flag + 1
  end
  return flag == 1
end
function c89558743.filter1(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToRemove(tp, POS_FACEDOWN) and not c:IsPublic() and Duel.IsExistingMatchingCard(c89558743.filter2, tp, LOCATION_DECK, 0, 1, nil, tp, c)
end
function c89558743.filter2(c, tp, mc)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToRemove(tp, POS_FACEDOWN) and c89558743.same_check(c, mc) and Duel.IsExistingMatchingCard(c89558743.filter3, tp, LOCATION_DECK, 0, 1, nil, c)
end
...
function c89558743.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c89558743.filter1, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
