...
function c22933016.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c22933016.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c22933016.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c22933016.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(Card.IsAbleToRemoveAsCost, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c22933016.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c22933016.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22933016.thfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, e:GetHandler())
  end
