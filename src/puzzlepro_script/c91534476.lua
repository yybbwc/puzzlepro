...
function c91534476.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91534476.cfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
...
function c91534476.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and chkc:IsControler(tp) and c91534476.thfilter(chkc) and chkc ~= e:GetLabelObject()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c91534476.thfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil)
  end
