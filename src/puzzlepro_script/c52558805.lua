...
function c52558805.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c52558805.filter(chkc, tp)
  end
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_EFFECT) and Duel.IsExistingTarget(c52558805.filter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
...
function c52558805.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or tc:GetOverlayCount() == 0 then
    return
  end
