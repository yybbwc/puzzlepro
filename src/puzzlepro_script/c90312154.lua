...
function c90312154.filter(c, ft)
  return aux.IsCodeOrListed(c, 56433456) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsSSetable() and (ft == nil or ft > 0 or c:IsType(TYPE_FIELD))
end
...
function c90312154.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90312154.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
