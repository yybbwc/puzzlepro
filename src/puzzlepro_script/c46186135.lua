...
function s.fselect(g, tp, sc)
  return g:GetClassCount(Card.GetCode) == 2 and Duel.GetLocationCountFromEx(tp, tp, g, sc) > 0 and g:IsExists(Card.IsLocation, 1, nil, LOCATION_MZONE) and g:IsExists(Card.IsLocation, 1, nil, LOCATION_GRAVE)
end
...
function s.thfilter1(c, tp)
  return c:IsFaceup() and c:IsCode(19959563) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(s.thfilter2, tp, LOCATION_REMOVED, 0, 1, c)
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.thfilter1, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoHand(g1, nil, REASON_EFFECT) == 2 then
    ...
    if tg:FilterCount(s.spfilter, nil, e, tp) == 2 and Duel.GetLocationCount(tp, LOCATION_MZONE) >= 2 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
      Duel.BreakEffect()
