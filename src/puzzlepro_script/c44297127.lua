...
function c44297127.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44297127.tgfilter, tp, LOCATION_DECK, 0, 1, nil) and (not draw or Duel.IsPlayerCanDraw(tp, 1))
  end
...
function c44297127.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) ~= 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) and Duel.IsExistingMatchingCard(Card.IsCode, tp, LOCATION_GRAVE, 0, 1, nil, 59419719) then
    Duel.BreakEffect()
