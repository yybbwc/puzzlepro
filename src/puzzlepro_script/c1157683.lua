...
function c1157683.eqfilter(c, tp)
  return c:IsSetCard(0x4093) and c:IsFaceup() and c:IsType(TYPE_EFFECT) and Duel.IsExistingMatchingCard(c1157683.eqfilter2, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
end
...
function c1157683.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c1157683.eqfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 1157683) == 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c1157683.eqfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function c1157683.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    ...
    local ec = g:GetFirst()
    if not ec or not Duel.Equip(tp, ec, tc) then
      return
    end
    local e1 = Effect.CreateEffect(c)
...
function c1157683.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c1157683.cfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c1157683.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 1157684) == 0 and Duel.IsExistingMatchingCard(aux.TRUE, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
