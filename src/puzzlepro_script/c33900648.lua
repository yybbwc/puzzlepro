...
function c33900648.darkcon1(e)
  local tp = e:GetHandlerPlayer()
  return c33900648.attributechk(tp) & ATTRIBUTE_DARK ~= 0 and Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) >= 2
end
function c33900648.darkcon2(e)
  local tp = e:GetHandlerPlayer()
  return c33900648.attributechk(1 - tp) & ATTRIBUTE_DARK ~= 0 and Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) >= 2
end
...
function c33900648.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c33900648.desfilter, turnp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c33900648.hdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local turnp = Duel.GetTurnPlayer()
  if chk == 0 then
    return Duel.GetFieldGroupCount(turnp, LOCATION_HAND, 0) > 0
  end
