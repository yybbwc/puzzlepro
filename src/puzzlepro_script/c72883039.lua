...
function c72883039.actcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72883039.acfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c72883039.acttg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if c72883039.spcon(e, tp, eg, ep, ev, re, r, rp) and c72883039.spcost(e, tp, eg, ep, ev, re, r, rp, 0) and c72883039.sptg(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.SelectYesNo(tp, 94) then
...
function c72883039.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(72883039) == 0
  end
...
function c72883039.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72883039.spfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
