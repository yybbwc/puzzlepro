...
function c18458255.repfilter(c, tp)
  return not c:IsReason(REASON_REPLACE) and c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsSetCard(0x191) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp))
end
function c18458255.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c18458255.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c18458255.reprmfilter, tp, LOCATION_EXTRA, 0, 1, nil, tp)
  end
