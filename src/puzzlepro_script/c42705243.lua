...
function c42705243.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c42705243.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c42705243.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummonStep(tc, 0, tp, tp, false, false, POS_FACEUP) then
    ...
    if Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c42705243.eqfilter), tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, tp, tc) and Duel.SelectYesNo(tp, aux.Stringid(42705243, 0)) then
      Duel.BreakEffect()
