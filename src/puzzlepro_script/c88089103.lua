...
function c88089103.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c88089103.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c88089103.filter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
