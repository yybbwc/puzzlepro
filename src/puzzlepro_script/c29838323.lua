...
function c29838323.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and c29838323.desfilter(chkc)
  end
  if chk == 0 then
    ...
    e:SetLabel(loc)
    return Duel.IsExistingTarget(c29838323.desfilter, tp, loc, 0, 1, nil) and Duel.IsExistingMatchingCard(c29838323.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c29838323.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29838323.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
