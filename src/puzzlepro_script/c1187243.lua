...
function c1187243.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c1187243.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c1187243.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c1187243.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c1187243.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp, ec:GetRace())
  end
...
function c1187243.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c1187243.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_DECK)
end
...
function c1187243.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c1187243.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c1187243.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c1187243.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
