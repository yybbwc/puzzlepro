...
function c43413875.hspfilter(c, tp)
  return c:IsSetCard(0xe6) and c:IsLevel(8) and not c:IsCode(43413875) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c43413875.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) ~= 0 then
    ...
    if tc:IsType(TYPE_MONSTER) and tc:IsSetCard(0xe6) then
      if tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(43413875, 1)) then
