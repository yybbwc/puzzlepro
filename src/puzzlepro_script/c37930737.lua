...
function c37930737.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_EARTH) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsCanBeEffectTarget(e)
end
...
function c37930737.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and g:CheckSubGroup(c37930737.fselect, 2, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133)
  end
...
function c37930737.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 1 or g:GetCount() ~= 2 or Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    return
  end
  local tc = g:GetFirst()
...
function c37930737.repfilter(c, tp)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsSetCard(0x186) and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)) and not c:IsReason(REASON_REPLACE)
end
function c37930737.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c37930737.repfilter, 1, nil, tp)
  end
