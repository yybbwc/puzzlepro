...
function s.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if g:GetFirst():IsSetCard(0x1b1) then
    e:SetLabel(100)
  end
...
function s.negtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local l = e:GetLabel()
  if chkc then
    return chkc:IsOnField() and s.negfilter(chkc) and c ~= chkc
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.negfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, c)
  end
...
function s.negop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsCanBeDisabledByEffect(e) then
    ...
    if l == 100 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
