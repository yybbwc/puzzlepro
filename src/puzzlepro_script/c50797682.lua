...
function c50797682.tgfilter(c, e, tp)
  if c:GetOriginalType() & TYPE_XYZ == 0 or c:IsFacedown() then
    return false
  end
...
function c50797682.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_ONFIELD + LOCATION_GRAVE) and c50797682.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c50797682.tgfilter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c50797682.activate(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsPlayerCanSpecialSummonCount(tp, 2) then
    return
  end
  ...
  Duel.AdjustAll()
  if g1:FilterCount(Card.IsLocation, nil, LOCATION_MZONE) < 2 then
    return
  end
