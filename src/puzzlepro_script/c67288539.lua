...
function c67288539.rmfilter(c, e, tp, check)
  return c:IsFaceup() and c:IsAttackAbove(3000) and c:IsAttribute(ATTRIBUTE_DARK) and (check or not c:IsImmuneToEffect(e) and Duel.IsExistingMatchingCard(c67288539.spfilter, tp, LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil, e, tp, c))
end
function c67288539.spfilter(c, e, tp, tc)
  if not (c:IsSetCard(0x16e) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)) then
    return false
  end
...
function c67288539.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67288539.rmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, e, tp)
  end
