...
function c87091930.descfilter(c, tp)
  return c:IsSummonType(SUMMON_TYPE_XYZ) and c:IsSummonPlayer(tp) and c:GetMaterial():IsExists(c87091930.descfilter2, 1, nil)
end
...
function c87091930.ssetcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsPreviousPosition(POS_FACEUP) and e:GetHandler():IsPreviousLocation(LOCATION_SZONE) and e:GetHandler():IsReason(REASON_EFFECT)
end
...
function c87091930.ssettg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c87091930.ssetfilter, tp, LOCATION_GRAVE + LOCATION_DECK, 0, 1, nil)
  end
end
