...
int32_t scriptlib::effect_reset(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_EFFECT, 1);
  effect *peffect = *(effect **)lua_touserdata(L, 1);
  if (peffect->owner == nullptr || peffect->handler == nullptr) {
    return 0;
  }
...
int32_t scriptlib::effect_set_description(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_EFFECT, 1);
  effect *peffect = *(effect **)lua_touserdata(L, 1);
  auto v = (uint32_t)lua_tointeger(L, 2);
...
int32_t scriptlib::effect_set_absolute_range(lua_State *L) {
  ...
  auto s = (uint32_t)lua_tointeger(L, 3);
  auto o = (uint32_t)lua_tointeger(L, 4);
  if (playerid == 0) {
    peffect->s_range = s;
    peffect->o_range = o;
  }
...
int32_t scriptlib::effect_set_reset(lua_State *L) {
  ...
  effect *peffect = *(effect **)lua_touserdata(L, 1);
  auto v = (uint32_t)lua_tointeger(L, 2);
  auto c = (int32_t)lua_tointeger(L, 3);
  if (c == 0) {
    c = 1;
  }
...
int32_t scriptlib::effect_set_hint_timing(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_EFFECT, 1);
  effect *peffect = *(effect **)lua_touserdata(L, 1);
  auto vs = (uint32_t)lua_tointeger(L, 2);
  uint32_t vo = vs;
  if (lua_gettop(L) >= 3) {
...
int32_t scriptlib::effect_set_owner_player(lua_State *L) {
  ...
  check_param(L, PARAM_TYPE_EFFECT, 1);
  effect *peffect = *(effect **)lua_touserdata(L, 1);
  auto p = (int32_t)lua_tointeger(L, 2);
  if (p != 0 && p != 1) {
    return 0;
  }
...
int32_t scriptlib::effect_set_activate_location(lua_State *L) {
  check_param_count(L, 1);
  check_param(L, PARAM_TYPE_EFFECT, 1);
  effect *peffect = *(effect **)lua_touserdata(L, 1);
  auto active_location = (int32_t)lua_tointeger(L, 2);
  peffect->active_location = active_location;
  return 0;
}
...
static const struct luaL_Reg effectlib[] = {
  ...
  {"SetActivateLocation", scriptlib::effect_set_activate_location},
