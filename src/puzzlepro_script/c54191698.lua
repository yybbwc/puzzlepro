...
function c54191698.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c54191698.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and Duel.IsExistingTarget(c54191698.spfilter1, tp, 0, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function c54191698.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c54191698.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c54191698.spfilter2, tp, 0, LOCATION_MZONE, 1, nil, e, tp)
  end
