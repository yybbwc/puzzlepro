...
function c36016907.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c36016907.spfilter1, tp, LOCATION_HAND, 0, 1, c)
  end
...
function c36016907.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c36016907.spfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c36016907.spfilter2, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp) and c:IsAbleToRemove()
  end
