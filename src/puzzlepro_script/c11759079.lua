...
function c11759079.spfilter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousSetCard(0x14f) and c:GetPreviousCodeOnField() ~= 11759079 and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c11759079.desfilter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x14f) and Duel.IsExistingMatchingCard(c11759079.sffilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
end
function c11759079.sffilter(c, e, tp, tc)
  return c:IsSetCard(0x14f) and c:IsType(TYPE_FUSION) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, tc, c) > 0
end
function c11759079.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsExistingMatchingCard(c11759079.desfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(11759079, 2)) then
    Duel.BreakEffect()
...
function c11759079.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c11759079.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
