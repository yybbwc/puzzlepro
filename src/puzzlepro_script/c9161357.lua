...
function c9161357.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c9161357.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c9161357.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c9161357.lpcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetCurrentPhase() == PHASE_MAIN1 and eqg:IsExists(c9161357.lpfilter, 1, nil, tp) and c:CheckRemoveOverlayCard(tp, 1, REASON_COST)
  end
