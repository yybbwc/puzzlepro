...
function c55277252.tg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c55277252.filter, tp, LOCATION_HAND, 0, 1, e:GetHandler()) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c55277252.op(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SendtoGrave(tc, REASON_EFFECT + REASON_DISCARD) > 0 and tc:IsLocation(LOCATION_GRAVE) and c:IsRelateToEffect(e) then
...
function c55277252.tg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c55277252.filter2, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
