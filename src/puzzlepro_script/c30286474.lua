...
function c30286474.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    if e:GetLabel() == 1 then
      e:SetLabel(0)
      return Duel.IsExistingMatchingCard(c30286474.costfilter, tp, LOCATION_MZONE, 0, 1, nil, lg) and Duel.IsExistingTarget(aux.NegateAnyFilter, tp, 0, LOCATION_ONFIELD, 1, nil)
    else
      return false
    end
  end
