...
function c82044279.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return re:IsActiveType(TYPE_MONSTER) and rc ~= c and level >= 5 and loc == LOCATION_MZONE and not c:IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev)
end
function c82044279.condition2(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  local c = e:GetHandler()
  return re:IsActiveType(TYPE_MONSTER) and tc:IsFaceup() and tc:IsLevelAbove(5) and tc:IsLocation(LOCATION_MZONE) and not c:IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev)
end
...
function c82044279.atkcon(e, tp, eg, ep, ev, re, r, rp)
  return bit.band(r, REASON_EFFECT) ~= 0 and re and re:GetOwner() == e:GetHandler() and eg:IsExists(Card.IsType, 1, nil, TYPE_MONSTER)
end
