...
function c87685879.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsFaceup() and chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE)
  end
  if chk == 0 then
...
function c87685879.operation(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsExistingMatchingCard(c87685879.cfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) then
    return
  end
