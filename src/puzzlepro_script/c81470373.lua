...
function c81470373.sumcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81470373.cfilter, tp, LOCATION_HAND, 0, 1, c)
  end
...
function c81470373.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    if Duel.GetLocationCount(tp, LOCATION_SZONE) <= 0 or not Duel.IsExistingMatchingCard(c81470373.acfilter, tp, LOCATION_DECK, 0, 1, nil, tp) then
      return false
    end
...
function c81470373.sumop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.MoveToField(tc, tp, tp, LOCATION_SZONE, POS_FACEUP, true) ~= 0 then
    ...
    if c:IsSummonable(true, se) and (not c:IsAbleToGrave() or Duel.SelectOption(tp, 1151, 1191) == 0) then
      Duel.BreakEffect()
