...
function c72971064.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and aux.IsInGroup(chkc, g)
  end
  if chk == 0 then
...
function c72971064.retg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c72971064.filter2(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c72971064.filter2, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
