...
function c71413901.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x1, 1, REASON_COST)
  end
...
function c71413901.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c71413901.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71413901.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
