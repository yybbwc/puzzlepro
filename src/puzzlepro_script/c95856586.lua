...
function c95856586.tgfilter(c, e, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsType(TYPE_XYZ) and c:IsLocation(LOCATION_MZONE) and c:IsCanBeEffectTarget(e)
end
...
function c95856586.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c95856586.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
    return eg:IsExists(c95856586.tgfilter, 1, nil, e, tp) and Duel.IsExistingMatchingCard(c95856586.matfilter, tp, LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil)
  end
  local g = eg
  if #eg > 1 then
...
function c95856586.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, LOCATION_DECK, 0) > 1 and Duel.IsExistingMatchingCard(c95856586.tdfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
