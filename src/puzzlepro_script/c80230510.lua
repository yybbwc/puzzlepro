...
function c80230510.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 80230510) == 0
  end
end
...
function c80230510.otcon(e, c, minc)
  ...
  return minc <= 2 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c80230510.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(c80230510.rmfilter, tp, 0, LOCATION_GRAVE, 1, nil)
end
