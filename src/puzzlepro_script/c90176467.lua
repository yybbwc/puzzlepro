...
function c90176467.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c90176467.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c90176467.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoHand(g, nil, REASON_EFFECT) > 0 and g:GetFirst():IsLocation(LOCATION_HAND) then
    ...
    if c:IsRelateToEffect(e) and c:IsFaceup() and not c:IsAttribute(ATTRIBUTE_LIGHT) and Duel.SelectYesNo(tp, aux.Stringid(90176467, 2)) then
      Duel.BreakEffect()
      local e1 = Effect.CreateEffect(c)
...
function c90176467.costfilter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x131) and c:IsLevelAbove(0) and Duel.IsExistingMatchingCard(c90176467.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetLevel()) and Duel.GetMZoneCount(tp, c) > 0
end
function c90176467.spfilter(c, e, tp, lv)
  return c:IsSetCard(0x131) and not c:IsLevel(lv) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and (aux.IsCodeListed(c, 74665651) and Duel.IsExistingMatchingCard(c90176467.spthfilter, tp, LOCATION_DECK, 0, 1, nil, 74665651) or aux.IsCodeListed(c, 1050355) and Duel.IsExistingMatchingCard(c90176467.spthfilter, tp, LOCATION_DECK, 0, 1, nil, 1050355))
end
