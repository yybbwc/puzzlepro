...
function c63717421.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c63717421.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return true
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if Duel.IsExistingTarget(c63717421.spfilter, tp, LOCATION_SZONE, 0, 1, nil, e, tp) and ft > 0 and Duel.SelectYesNo(tp, aux.Stringid(63717421, 1)) then
...
function c63717421.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63717421.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
