...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return re:GetHandler():IsCanAddCounter(0x1065, 1) and c:GetFlagEffect(id) == 0
  end
...
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.filter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return c:IsAbleToRemove() and Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
