...
function c96156729.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(96156729, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c96156729.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c96156729.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingTarget(c96156729.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
