...
function c3072808.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c3072808.filter(chkc, e, tp)
  end
  if chk == 0 then
    return true
  end
...
function c3072808.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc or not tc:IsRelateToEffect(e) or not tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) then
    return
  end
  local e1 = Effect.CreateEffect(c)
