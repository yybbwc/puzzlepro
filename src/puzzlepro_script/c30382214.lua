...
function c30382214.drawcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsSetCard, 1, nil, 0xe6)
  end
...
function c30382214.sfilter(c, e, tp)
  return c:IsSetCard(0xe6) and c:IsType(TYPE_MONSTER) and not c:IsCode(30382214) and (c:IsAbleToHand() or (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)))
end
  ...
  if Duel.Draw(p, d, REASON_EFFECT) ~= 0 then
    ...
    if tc:IsType(TYPE_MONSTER) and tc:IsSetCard(0xe6) then
      ...
      if sel == 0 then
        ...
        if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and sc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not sc:IsAbleToHand() or Duel.SelectOption(tp, 1190, 1152) == 1) then
