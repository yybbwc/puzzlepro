...
function c94599451.ctfilter(c, tp)
  return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:IsType(TYPE_PENDULUM) and c:IsPreviousSetCard(0x10d) and c:IsReason(REASON_BATTLE + REASON_EFFECT)
end
...
function c94599451.thfilter1(c, tp)
  local lv = c:GetLevel()
  return (c:IsLocation(LOCATION_DECK) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM))) and lv > 0 and c:IsCanHaveCounter(0x1) and Duel.IsCanRemoveCounter(tp, 1, 0, 0x1, lv, REASON_COST) and c:IsAbleToHand()
end
function c94599451.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94599451.thfilter1, tp, LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil, tp)
  end
...
function c94599451.thfilter2(c, lv)
  return (c:IsLocation(LOCATION_DECK) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM))) and c:IsCanHaveCounter(0x1) and c:IsLevel(lv) and c:IsAbleToHand()
end
...
function c94599451.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsReason(REASON_EFFECT) and c:IsCanRemoveCounter(tp, 0x1, 1, REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
  end
