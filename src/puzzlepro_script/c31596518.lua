...
function s.filter(c)
  return (c:IsSetCard(0xbe) and c:IsType(TYPE_SPELL + TYPE_TRAP) or c:IsType(TYPE_MONSTER) and c:IsAttack(2800) and c:IsDefense(1000)) and c:IsAbleToHand()
end
...
function s.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc:IsSummonPlayer(tp) and tc:IsSummonType(SUMMON_TYPE_ADVANCE) and tc:IsAttackAbove(2400) and tc:IsDefense(1000)
end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if not c:IsRelateToChain() or not aux.NecroValleyFilter()(c) then
    return
  end
