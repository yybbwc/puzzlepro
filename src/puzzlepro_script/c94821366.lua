...
function c94821366.eqfilter1(c, e, tp)
  return c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsCanBeEffectTarget(e) and c:IsControler(1 - tp) and c:IsSummonPlayer(1 - tp) and Duel.IsExistingMatchingCard(c94821366.eqfilter2, tp, LOCATION_DECK, 0, 1, nil)
end
...
function c94821366.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c94821366.eqfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and eg:IsExists(c94821366.eqfilter1, 1, nil, e, tp)
  end
  local g = eg:Clone()
  if #eg > 1 then
...
function c94821366.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFacedown() or not tc:IsRelateToEffect(e) or tc:IsControler(tp) then
    return
  end
...
function c94821366.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94821366.costfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
