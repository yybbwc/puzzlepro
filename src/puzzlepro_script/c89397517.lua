...
function c89397517.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 2000) and Duel.CheckReleaseGroup(tp, c89397517.costfilter, 1, nil, tp)
  end
...
function c89397517.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    e:SetLabel(0)
    return res and Duel.IsExistingMatchingCard(c89397517.rmfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(c89397517.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE + LOCATION_DECK, 0, 1, nil, e, tp)
  end
