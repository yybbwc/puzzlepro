...
function s.tntg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.tnfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.tnfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function s.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.eqfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and c:CheckUniqueOnField(tp) and Duel.IsExistingTarget(s.eqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
  end
