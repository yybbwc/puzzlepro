...
function c5338223.cfilter(c, ft, tp)
  return c:IsSetCard(0x304e) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup())
end
function c5338223.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c5338223.cfilter, 1, nil, ft, tp)
  end
...
function c5338223.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    if e:GetLabel() == 1 then
      ...
      return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c5338223.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
    end
  end
  e:SetLabel(0)
