...
function c69488544.spcon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and aux.IsDualState(e) and rp == 1 - tp and re:IsActiveType(TYPE_SPELL) and re:IsHasType(EFFECT_TYPE_ACTIVATE)
end
...
function c69488544.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c69488544.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c69488544.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c69488544.negcon(e, tp, eg, ep, ev, re, r, rp)
  if not aux.IsDualState(e) or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
    return false
  end
  local g = Duel.GetChainInfo(ev, CHAININFO_TARGET_CARDS)
  if not g or not g:IsExists(Card.IsLocation, 1, nil, LOCATION_MZONE) then
    return false
  end
...
function c69488544.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69488544.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
