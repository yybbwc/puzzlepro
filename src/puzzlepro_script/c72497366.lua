...
function c72497366.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:GetCount() == 1 and tc:IsFaceup() and tc:IsLocation(LOCATION_MZONE) and tc:IsRace(RACE_FIEND) and tc:IsLevelAbove(5) and tc:IsSummonType(SUMMON_TYPE_NORMAL) and tc:IsReason(REASON_EFFECT + REASON_BATTLE) and not tc:IsReason(REASON_REPLACE)
  end
