...
function c52253888.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and chkc:IsAttackPos()
  end
  if chk == 0 then
...
function c52253888.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (c:IsRelateToEffect(e) and c:IsFaceup() and c:IsControler(tp)) then
    return
  end
