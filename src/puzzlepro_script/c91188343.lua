...
function c91188343.tg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    if not Duel.IsEnvironment(56433456) then
      return Duel.IsExistingMatchingCard(c91188343.filter1, tp, LOCATION_DECK, 0, 1, nil)
    end
...
function c91188343.op(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsEnvironment(56433456) then
    ...
    g = Duel.SelectMatchingCard(tp, c91188343.filter2, tp, LOCATION_DECK, 0, 1, 1, nil)
  end
