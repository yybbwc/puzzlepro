...
function c82489470.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) and Duel.NegateAttack() and Duel.IsExistingMatchingCard(c82489470.atkfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(82489470, 0)) then
...
function c82489470.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c82489470.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, c, e, tp)
  end
