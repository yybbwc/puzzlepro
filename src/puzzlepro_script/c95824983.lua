...
function c95824983.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95824983.cfilter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c95824983.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c95824983.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c95824983.discon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsEnvironment(22702055) and ep == 1 - tp and re:IsActiveType(TYPE_SPELL + TYPE_MONSTER) and Duel.IsChainDisablable(ev)
end
...
function c95824983.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateEffect(ev) and c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SelectYesNo(tp, aux.Stringid(95824983, 2)) then
    Duel.BreakEffect()
    local e1 = Effect.CreateEffect(c)
