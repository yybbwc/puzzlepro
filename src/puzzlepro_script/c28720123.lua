...
function c28720123.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_PZONE) and chkc ~= c and c28720123.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c28720123.spfilter, tp, LOCATION_PZONE, 0, 1, c, e, tp)
  end
...
function c28720123.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c28720123.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
