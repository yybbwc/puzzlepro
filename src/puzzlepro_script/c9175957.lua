...
function c9175957.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local spct = 0
  if Duel.CheckRemoveOverlayCard(tp, LOCATION_MZONE, 0, 1, REASON_COST) and Duel.SelectYesNo(tp, aux.Stringid(9175957, 1)) then
...
function c9175957.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9175957.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c9175957.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    local spct = e:GetLabel()
    if spct > 0 and ct >= spct and ft >= spct and Duel.SelectYesNo(tp, aux.Stringid(9175957, 2)) then
      Duel.BreakEffect()
...
function c9175957.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c9175957.matfilter(chkc, tp)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsAbleToHand() and Duel.IsExistingTarget(c9175957.matfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function c9175957.matop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:RemoveOverlayCard(tp, 1, 1, REASON_EFFECT) > 0 and c:IsRelateToEffect(e) then
