...
function c2116237.costfilter(c, e, tp)
  return c:IsCode(56099748) and c:IsFaceup() and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c2116237.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
end
function c2116237.spfilter(c, e, tp, sc)
  return c:IsSetCard(0x1a0) and c:IsAttack(3000) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and Duel.GetLocationCountFromEx(tp, tp, sc, c) > 0
end
function c2116237.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c2116237.costfilter, tp, LOCATION_ONFIELD, 0, 1, nil, e, tp)
  end
...
function c2116237.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:IsCostChecked() or Duel.IsExistingMatchingCard(c2116237.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, nil)
  end
