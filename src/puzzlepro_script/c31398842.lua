...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.tgfilter, tp, LOCATION_DECK, 0, 1, nil, b)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.RemoveOverlayCard(tp, 1, 1, 1, 1, REASON_EFFECT) ~= 0 then
    ...
    local tc = g:GetFirst()
    if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and tc:IsType(TYPE_XYZ) and tc:GetOwner() == tp and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and tc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and not tc:IsHasEffect(EFFECT_NECRO_VALLEY) and Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
      Duel.BreakEffect()
