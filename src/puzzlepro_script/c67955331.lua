...
function c67955331.filter(c, e, tp)
  ...
  return lv > 1 and c:IsFaceup() and c:IsSetCard(0x12e) and Duel.IsExistingMatchingCard(c67955331.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp, lv)
end
...
function c67955331.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c67955331.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c67955331.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
