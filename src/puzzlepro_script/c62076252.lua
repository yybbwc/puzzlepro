...
function s.tdfilter(c, tp, ec)
  ...
  return c:IsFaceupEx() and c:IsSetCard(0x1019) and c:IsType(TYPE_MONSTER) and c:IsAbleToDeckOrExtraAsCost() and Duel.GetMZoneCount(tp, g) > 0
end
function s.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(s.tdfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, e:GetHandler(), tp, e:GetHandler())
  end
