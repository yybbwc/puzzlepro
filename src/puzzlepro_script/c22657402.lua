...
function c22657402.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c22657402.spfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function c22657402.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c22657402.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22657402.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
