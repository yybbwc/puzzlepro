...
function c7850740.cfilter1(c, e, tp, se)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsControler(tp) and c:IsCanBeEffectTarget(e) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function c7850740.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c7850740.tgfilter(chkc, tp, eg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c7850740.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, tp, eg) and e:GetHandler():IsCanOverlay()
  end
