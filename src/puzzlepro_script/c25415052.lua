...
function c25415052.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c25415052.tdfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c25415052.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoDeck(sg, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 then
    ...
    if sg2:IsExists(c25415052.cfilter, 1, nil, tp) then
      Duel.ShuffleDeck(tp)
    end
    if sg2:IsExists(c25415052.cfilter, 1, nil, 1 - tp) then
      Duel.ShuffleDeck(1 - tp)
    end
    if not sg2:IsExists(Card.IsLocation, 1, nil, LOCATION_DECK + LOCATION_EXTRA) then
      return
    end
    local tg = Duel.GetMatchingGroup(c25415052.spfilter, tp, LOCATION_DECK, 0, nil, e, tp)
    if tg:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(25415052, 1)) then
      Duel.BreakEffect()
...
function c25415052.rettg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    if e:IsHasType(EFFECT_TYPE_TRIGGER_F) then
      ...
      return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonMonster(tp, 25415053, 0, TYPES_TOKEN_MONSTER, 1500, 1500, 4, RACE_WINDBEAST, ATTRIBUTE_WIND)
    end
  end
...
function c25415052.retop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SendtoHand(c, nil, REASON_EFFECT) ~= 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonMonster(tp, 25415053, 0, TYPES_TOKEN_MONSTER, 1500, 1500, 4, RACE_WINDBEAST, ATTRIBUTE_WIND) then
    for i = 1, 2 do
