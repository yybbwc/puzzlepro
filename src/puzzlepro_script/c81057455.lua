...
function c81057455.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c81057455.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsCanAddCounter(tp, 0x37, 1, e:GetHandler()) and Duel.IsExistingTarget(c81057455.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
