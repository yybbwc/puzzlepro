...
function c85497611.cfilter(c, tp)
  return c:IsAttribute(ATTRIBUTE_DARK) and Duel.IsExistingMatchingCard(c85497611.filter1, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_MZONE, 0, 1, c, tp, c)
end
...
function c85497611.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c85497611.cfilter, 1, nil, tp)
  end
...
function c85497611.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c85497611.spfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c85497611.thop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
    ...
    if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsExistingMatchingCard(c85497611.thfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(85497611, 2)) then
      Duel.BreakEffect()
