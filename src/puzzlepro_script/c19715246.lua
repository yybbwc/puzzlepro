...
function s.eqfilter(c, e, tp, chk)
  return c:IsCode(70095154) and (c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not chk and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 or chk and Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1) or Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and c:CheckUniqueOnField(tp) and not c:IsForbidden())
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(s.eqfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, true)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToChain() and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    ...
    if tc then
      ...
      if b1 and b2 then
        op = aux.SelectFromOptions(tp, { b1, 1152, 1 }, { b2, 1068, 2 })
      end
      Duel.BreakEffect()
      if op == 1 then
