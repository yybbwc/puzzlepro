...
function c88554436.spdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c88554436.desfilter(chkc, tp)
  end
  if chk == 0 then
    return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c88554436.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DESTROY)
