...
function c90951921.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(90951921, tp, ACTIVITY_SPSUMMON) == 0
  end
  --oath effects
...
function c90951921.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c90951921.filter(chkc)
  end
  if chk == 0 then
    if not Duel.IsPlayerCanDiscardDeck(tp, 2) or not Duel.IsExistingTarget(c90951921.filter, tp, LOCATION_GRAVE, 0, 1, nil) then
      return false
    end
