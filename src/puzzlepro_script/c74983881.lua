...
function c74983881.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 74983882, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_PLANT, ATTRIBUTE_EARTH)
  end
...
function c74983881.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 74983882, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_PLANT, ATTRIBUTE_EARTH) then
    return
  end
