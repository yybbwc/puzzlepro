...
function c88332693.filter1(c, e, tp)
  return c:IsType(TYPE_SYNCHRO) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c88332693.filter2, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetCode())
end
...
function c88332693.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c88332693.setfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
end
