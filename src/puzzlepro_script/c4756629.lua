...
function c4756629.rmfilter(c, tp)
  return c:IsSetCard(0xa) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c4756629.filter, tp, LOCATION_GRAVE, 0, 1, c)
end
...
function c4756629.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4756629.rmfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c4756629.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c4756629.filter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c4756629.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4756629.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
...
function c4756629.decop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  if Duel.GetFlagEffect(tp, 4756630) ~= 0 then
    return
  end
  local e1 = Effect.CreateEffect(c)
