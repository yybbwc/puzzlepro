...
function s.operation(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, 0, LOCATION_DECK, 1, nil, e, 1 - tp) and Duel.SelectYesNo(1 - tp, aux.Stringid(id, 2)) then
...
function s.sspfilter(c, tp, e)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) < Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP)
end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local toplayer = aux.SelectFromOptions(tp, { b1, aux.Stringid(id, 3), tp }, { b2, aux.Stringid(id, 4), 1 - tp })
  if toplayer == tp then
