...
function c65124425.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c65124425.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c65124425.thfilter(c, tc)
  return c:IsSetCard(0x12c) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not c:IsCode(tc:GetCode())
end
function c65124425.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SendtoGrave(tc, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_GRAVE) and Duel.IsExistingMatchingCard(c65124425.thcfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c65124425.thfilter, tp, LOCATION_DECK, 0, 1, nil, tc) and Duel.SelectYesNo(tp, aux.Stringid(65124425, 0)) then
