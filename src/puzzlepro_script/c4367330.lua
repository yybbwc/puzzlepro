...
function c4367330.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4367330.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4367330.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c4367330.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:GetOriginalRace() == RACE_BEASTWARRIOR and not c:IsStatus(STATUS_BATTLE_DESTROYED) and ep == 1 - tp and re:IsActiveType(TYPE_SPELL) and Duel.IsChainNegatable(ev) and re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) and Duel.GetChainInfo(ev, CHAININFO_TARGET_CARDS):IsContains(c)
end
