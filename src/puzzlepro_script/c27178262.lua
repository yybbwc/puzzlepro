...
function c27178262.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c27178262.costfilter, tp, LOCATION_MZONE, 0, 1, nil, ft)
  end
...
function c27178262.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c27178262.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c27178262.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
