...
function c21454943.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  if phase ~= PHASE_DAMAGE or Duel.IsDamageCalculated() then
    return false
  end
...
function c21454943.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckLPCost(tp, 100, true) and e:GetHandler():GetFlagEffect(21454943) == 0 and (bc:IsAttackAbove(100) or bc:IsDefenseAbove(100))
  end
  ...
  if lp >= 500 then
    alp = Duel.AnnounceNumber(tp, 100, 200, 300, 400, 500)
  elseif lp >= 400 then
    alp = Duel.AnnounceNumber(tp, 100, 200, 300, 400)
  elseif lp >= 300 then
    alp = Duel.AnnounceNumber(tp, 100, 200, 300)
  elseif lp >= 200 then
    alp = Duel.AnnounceNumber(tp, 100, 200)
  end
...
function c21454943.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc or not tc:IsRelateToEffect(e) or not tc:IsControler(1 - tp) then
    return
  end
  local e1 = Effect.CreateEffect(c)
