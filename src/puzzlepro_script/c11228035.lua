...
function c11228035.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsType(TYPE_XYZ)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsType, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, TYPE_XYZ) and Duel.IsExistingMatchingCard(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c11228035.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and tc:IsRelateToEffect(e) then
    ...
    if tc:IsAbleToDeck() and aux.NecroValleyFilter()(tc) and Duel.SelectYesNo(tp, aux.Stringid(11228035, 0)) then
      Duel.BreakEffect()
