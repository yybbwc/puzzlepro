...
function c95772051.spfilter(c, ft)
  return c:IsFaceup() and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_NORMAL) and c:IsAbleToGraveAsCost() and (ft > 0 or c:GetSequence() < 5)
end
function c95772051.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > -1 and Duel.IsExistingMatchingCard(c95772051.spfilter, tp, LOCATION_MZONE, 0, 1, nil, ft)
  end
...
function c95772051.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsSetCard, 1, nil, 0x107a)
  end
...
function c95772051.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95772051.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
