...
function c92868896.eqcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c92868896.cfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c92868896.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c92868896.eqfilter, tp, LOCATION_DECK, 0, 1, nil, e:GetHandler())
  end
...
function c92868896.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and c:IsSetCard(0x29) and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT)) and not c:IsReason(REASON_REPLACE)
end
function c92868896.desfilter(c, e)
  return c:IsLocation(LOCATION_SZONE) and c:IsSetCard(0x29) and e:GetHandler():GetEquipGroup():IsContains(c) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
end
function c92868896.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c92868896.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c92868896.desfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil, e)
  end
