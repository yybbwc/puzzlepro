...
function c80764541.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c80764541.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c80764541.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c80764541.desop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.IsChainDisablable(0) and Duel.CheckLPCost(1 - tp, 1000) and Duel.SelectYesNo(1 - tp, aux.Stringid(80764541, 1)) then
