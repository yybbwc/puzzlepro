...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD) and (chkc:IsControler(tp) or b) and chkc:IsCanTurnSet()
  end
  local loc2 = 0
  if b then
    loc2 = LOCATION_MZONE
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsCanTurnSet, tp, LOCATION_MZONE, loc2, 1, nil) and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function s.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or Duel.ChangePosition(tc, POS_FACEDOWN_DEFENSE) < 1 then
    return
  end
