...
function c54529134.filter1(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x119) and c:IsType(TYPE_LINK) and Duel.IsExistingMatchingCard(c54529134.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_LMATERIAL)
end
function c54529134.filter2(c, e, tp, mc)
  return c:IsSetCard(0x119) and c:IsType(TYPE_LINK) and c:IsCode(mc:GetCode()) and mc:IsCanBeLinkMaterial(c) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_LINK, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function c54529134.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c54529134.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c54529134.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c54529134.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not aux.MustMaterialCheck(tc, tp, EFFECT_MUST_BE_LMATERIAL) then
    return
  end
