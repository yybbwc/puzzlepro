...
function c43863925.efftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c43863925.effspfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c43863925.effop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SpecialSummon(g, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and Duel.IsExistingMatchingCard(c43863925.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
    Duel.BreakEffect()
...
function c43863925.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c43863925.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
