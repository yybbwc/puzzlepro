...
function c13046291.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c13046291.costfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(Card.IsDiscardable, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c13046291.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c13046291.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c13046291.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c13046291.matfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13046291.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c13046291.matfilter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil)
  end
