...
function c87475570.filter(c)
  return ((c:IsSetCard(0x1034, 0x2034) and c:IsType(TYPE_MONSTER)) or (c:IsSetCard(0x34) and c:IsType(TYPE_SPELL + TYPE_TRAP))) and c:IsAbleToHand()
end
function c87475570.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c87475570.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
