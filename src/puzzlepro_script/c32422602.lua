...
function c32422602.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bit.band(r, REASON_EFFECT) ~= 0 and re:GetHandler():IsSetCard(0x137) and c:IsPreviousLocation(LOCATION_DECK) and c:IsPreviousControler(tp)
end
...
function c32422602.negcon2(e, tp, eg, ep, ev, re, r, rp)
  return re:IsActiveType(TYPE_MONSTER) and Duel.IsChainNegatable(ev) and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsExistingMatchingCard(c32422602.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function c32422602.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32422602.costfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
