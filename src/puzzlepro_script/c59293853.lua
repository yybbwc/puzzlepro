...
function s.filter(c, e, tp)
  return c:IsSetCard(0x1b7) and (c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 or c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE, 1 - tp) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0)
end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    local op = aux.SelectFromOptions(tp, { s1, aux.Stringid(id, 2), tp }, { s2, aux.Stringid(id, 3), 1 - tp })
    local sumpos = op == tp and POS_FACEUP or POS_FACEDOWN_DEFENCE
    if Duel.SpecialSummon(tc, 0, tp, op, false, false, sumpos) == 0 then
      return
    end
    if op == 1 - tp then
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and s.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.thfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
