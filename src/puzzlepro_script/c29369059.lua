...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local b1 = (Duel.GetFlagEffect(tp, id) == 0 or not e:IsCostChecked()) and Duel.IsExistingTarget(s.thfilter1, tp, LOCATION_MZONE, 0, 2, nil) and Duel.IsExistingTarget(Card.IsAbleToHand, tp, 0, LOCATION_ONFIELD, 2, nil)
  ...
  local op = aux.SelectFromOptions(tp, { b1, aux.Stringid(id, 1), 1 }, { b2, aux.Stringid(id, 2), 2 }, { b3, aux.Stringid(id, 3), 3 })
  e:SetLabel(op)
  if op == 1 then
...
function s.tffilter(c, tp)
  return c:IsSetCard(0x1ca) and c:IsAllTypes(TYPE_FIELD + TYPE_SPELL) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
