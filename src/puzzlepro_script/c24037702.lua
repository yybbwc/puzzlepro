...
function c24037702.filter(c, tp)
  if not c:IsType(TYPE_MONSTER) or not c:IsSetCard(0x38) or c:GetOriginalLevel() <= 0 or not c:IsAbleToHand() then
    return false
  end
...
function c24037702.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c24037702.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c24037702.filter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
