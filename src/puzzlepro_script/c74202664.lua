...
function c74202664.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummon(tp) and not Duel.IsPlayerAffectedByEffect(tp, 63060238) or g:IsExists(Card.IsAbleToHand, 1, nil))
  end
end
function c74202664.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsType(TYPE_MONSTER) and (tc:IsLevel(1) or tc:IsLevel(8)) then
    ...
    if not tc:IsAbleToHand() and (not tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) or ft <= 0) then
      return
    end
