...
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.rmfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function s.rmtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.rmfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.rmfilter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function s.rmcon3(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not g or not g:IsContains(c) then
    return false
  end
  return true
end
