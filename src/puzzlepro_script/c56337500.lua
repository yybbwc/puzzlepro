...
function c56337500.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(56337500, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c56337500.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_COST) and c56337500.cost(e, tp, eg, ep, ev, re, r, rp, 0)
  end
...
function c56337500.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c56337500.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c56337500.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56337500.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp and c:IsPreviousControler(tp))) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsSummonType(SUMMON_TYPE_XYZ)
end
