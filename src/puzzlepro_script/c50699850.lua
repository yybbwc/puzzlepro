...
function c50699850.tdfilter(c, e)
  return (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:GetType() == TYPE_TRAP and c:IsAbleToDeck() and c:IsCanBeEffectTarget(e)
end
...
function c50699850.sttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c50699850.stfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c50699850.stfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
