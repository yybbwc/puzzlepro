...
function s.spcon(e, tp)
  return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION) and e:GetHandler():IsStatus(STATUS_SPSUMMON_TURN)
end
function s.spfilter(c, e, tp)
  return c:IsSetCard(0x1bc) and c:IsLevelBelow(9) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_DECK) and Duel.GetMZoneCount(tp) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)
end
