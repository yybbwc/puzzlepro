...
function c85696777.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0x64) and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT)) and not c:IsReason(REASON_REPLACE)
end
function c85696777.desfilter(c, e, tp)
  return c:IsControler(tp) and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
end
function c85696777.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c85696777.repfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c85696777.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil, e, tp)
  end
...
function c85696777.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c85696777.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85696777.thfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
