...
function s.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(id, tp, ACTIVITY_SPSUMMON) == 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local lv = e:GetLabel()
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local lv = e:GetLabel()
  if not c:IsRelateToEffect(e) or Duel.IsPlayerAffectedByEffect(tp, 59822133) or not c:IsCanBeSpecialSummoned(e, 0, tp, false, false) then
    return
  end
