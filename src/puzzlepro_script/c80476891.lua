...
function c80476891.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tc:IsType(TYPE_DUAL) and tc ~= e:GetHandler()
  end
...
function c80476891.repfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsType(TYPE_DUAL) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and c:GetFlagEffect(80476891) == 0
end
function c80476891.desfilter(c, e, tp)
  return c:IsControler(tp) and c:IsOnField() and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
end
function c80476891.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80476891.desfilter, tp, LOCATION_ONFIELD, 0, 1, nil, e, tp) and eg:IsExists(c80476891.repfilter, 1, nil, tp)
  end
