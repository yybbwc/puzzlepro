...
function c83546647.spfilter(c, e, tp)
  return c:IsLevel(1) and not c:IsPublic() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c83546647.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode())
end
...
function c83546647.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(1)
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingMatchingCard(c83546647.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c83546647.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc1:IsRelateToEffect(e) or not tc1:IsCanBeSpecialSummoned(e, 0, tp, false, false) then
    return
  end
