...
function c9418365.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c9418365.spfilter, tp, LOCATION_GRAVE, 0, 1, nil)
end
...
function c9418365.hdcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:GetSummonType() == SUMMON_TYPE_SPECIAL + SUMMON_VALUE_SELF and rp == 1 - tp and c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp)
end
