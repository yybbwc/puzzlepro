...
function c41463181.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c41463181.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c41463181.tdfilter, tp, 0, LOCATION_SZONE, 1, nil)
  end
...
function c41463181.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c41463181.atkfilter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function c41463181.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFacedown() or not tc:IsRelateToEffect(e) or not sc:IsRelateToEffect(e) then
    return
  end
