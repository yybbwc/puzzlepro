...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if e:GetType() & EFFECT_TYPE_QUICK_O > 0 then
    loc1, loc2 = LOCATION_GRAVE, 0
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, loc1, loc2, 1, nil)
  end
...
function s.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (tc:IsRelateToEffect(e) and tc:GetType() == TYPE_TRAP) then
    return
  end
