...
function c61968753.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c61968753.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c61968753.filter2, tp, 0, LOCATION_MZONE, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c61968753.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c61968753.posop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() == 2 and Duel.ChangePosition(g, POS_FACEUP_DEFENSE) ~= 0 then
    ...
    if not tc:IsImmuneToEffect(e) and tc:IsReleasableByEffect() then
      Duel.BreakEffect()
      if Duel.Release(tc, REASON_EFFECT) == 0 or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 then
        return
      end
