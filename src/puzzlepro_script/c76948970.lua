...
function s.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.CheckReleaseGroupEx(tp, s.spfilter, 1, REASON_SPSUMMON, true, c) and Duel.IsExistingMatchingCard(s.spfilter2, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetMatchingGroupCount(Card.IsDiscardable, tp, LOCATION_HAND, 0, nil, REASON_EFFECT) > 0 and c:IsAbleToHand()
  end
