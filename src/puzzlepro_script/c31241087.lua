...
function c31241087.dsptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:CheckSubGroup(aux.mzctcheck, 2, 2, tp) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c31241087.dspop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local g = Duel.GetMatchingGroup(c31241087.desfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, c)
  if #g == 0 or not g:CheckSubGroup(aux.mzctcheck, 2, 2, tp) then
    return
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DESTROY)
  local dg = g:SelectSubGroup(tp, aux.mzctcheck, false, 2, 2, tp)
  Duel.HintSelection(dg)
  if Duel.Destroy(dg, REASON_EFFECT) == 0 or not c:IsRelateToEffect(e) then
    return
  end
...
function c31241087.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c31241087.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c31241087.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c31241087.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c31241087.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 then
    local c = e:GetHandler()
    if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and g:FilterCount(Card.IsLocation, nil, LOCATION_DECK) > 0 and c:IsRelateToEffect(e) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.SelectYesNo(tp, aux.Stringid(31241087, 3)) then
      Duel.BreakEffect()
      Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP)
    end
  end
