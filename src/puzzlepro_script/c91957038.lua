...
function c91957038.nsfilter(c, tp)
  return c:IsRace(RACE_SPELLCASTER) and c:IsAttack(1850) and not c:IsPublic() and c:IsSummonable(true, nil) and not Duel.IsExistingMatchingCard(c91957038.cfilter, tp, LOCATION_MZONE, 0, 1, nil, c:GetCode())
end
function c91957038.nscost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c91957038.nsfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c91957038.damcon(e)
  return e:GetHandler():GetFlagEffect(91957038) == 0 and Duel.IsExistingMatchingCard(c91957038.damfilter, e:GetHandlerPlayer(), LOCATION_MZONE, 0, 1, nil)
end
