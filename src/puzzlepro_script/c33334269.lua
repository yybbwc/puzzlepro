...
function c33334269.tgfilter1(c, tp)
  return ((c:IsSetCard(0xaf) and c:IsType(TYPE_MONSTER) and not c:IsCode(33334269)) or c:IsSetCard(0xae)) and Duel.IsExistingMatchingCard(c33334269.tgfilter2, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c33334269.tgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and chkc:IsCode(e:GetLabel())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c33334269.tgfilter1, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c33334269.rtgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c33334269.rtgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c33334269.rtgfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
