...
function c3972721.checkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local p2 = false
  while tc do
    if tc:IsType(TYPE_SYNCHRO) and tc:IsPreviousLocation(LOCATION_MZONE) and ((tc:IsReason(REASON_BATTLE) and bit.band(tc:GetBattlePosition(), POS_FACEUP) ~= 0) or (not tc:IsReason(REASON_BATTLE) and tc:IsPreviousPosition(POS_FACEUP))) and tc:GetPreviousControler() ~= tc:GetReasonPlayer() then
  ...
  if p1 then
    Duel.RegisterFlagEffect(0, 3972721, RESET_PHASE + PHASE_END, 0, 1)
  end
  if p2 then
    Duel.RegisterFlagEffect(1, 3972721, RESET_PHASE + PHASE_END, 0, 1)
  end
end
