...
function c32615065.atkcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE and aux.dscon(e, tp, eg, ep, ev, re, r, rp)
end
...
function c32615065.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32615065.atkfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
end
...
function c32615065.atkcost2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(32615065) == 0
  end
...
function c32615065.spcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsSummonType(SUMMON_TYPE_FUSION) and c:IsReason(REASON_DESTROY) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE)
end
function c32615065.spfilter(c, e, tp)
  return c:IsCode(96220350) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial() and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c32615065.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_FMATERIAL) and Duel.IsExistingMatchingCard(c32615065.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
