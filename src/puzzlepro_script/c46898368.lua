...
function c46898368.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(46898368, tp, ACTIVITY_SUMMON) == 0 and Duel.GetCustomActivityCount(46898368, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c46898368.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c46898368.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46898368.filter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
