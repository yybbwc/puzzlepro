...
function c70204022.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c70204022.posfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c70204022.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (Duel.IsExistingMatchingCard(c70204022.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp) or b and Duel.IsExistingMatchingCard(c70204022.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp))
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_DECK)
end
function c70204022.spop(e, tp, eg, ep, ev, re, r, rp)
  local b = Duel.IsEnvironment(75304793, tp, LOCATION_FZONE)
  if b and Duel.IsExistingMatchingCard(c70204022.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp) and (not Duel.IsExistingMatchingCard(c70204022.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp) or Duel.SelectYesNo(tp, aux.Stringid(70204022, 3))) then
