...
function c45115956.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c45115956.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c45115956.atttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c45115956.attfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45115956.attfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c45115956.attop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
    ...
    if e:GetLabel() == 1 - tp and tc:IsControler(1 - tp) and tc:IsControlerCanBeChanged() and Duel.SelectYesNo(tp, aux.Stringid(45115956, 2)) then
      Duel.BreakEffect()
