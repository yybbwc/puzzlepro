...
function c59464593.spfilter(c, tp)
  return c:IsCode(73879377) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup())
end
...
function c59464593.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c59464593.dfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
