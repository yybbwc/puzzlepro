...
function c99913726.rfilter(c, tp)
  return c:GetCounter(0x1041) > 0 and c:IsReleasable(REASON_SPSUMMON) and Duel.GetMZoneCount(tp, c) > 0
end
...
function c99913726.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99913726.cfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
