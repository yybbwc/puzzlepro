...
function c59160188.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 59160189, 0, TYPES_TOKEN_MONSTER, 1000, 1000, 3, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE, p) then
    return
  end
  local ft = Duel.GetLocationCount(p, LOCATION_MZONE)
  ct = math.min(ct, ft)
  if ct > 1 and Duel.IsPlayerAffectedByEffect(p, 59822133) then
    ct = 1
  end
  for i = 1, ct do
