...
function c6471156.cfilter(c, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsSetCard(0x1083) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c6471156.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, lv)
end
...
function c6471156.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c6471156.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c6471156.cfilter2(c, tp)
  return c:IsSetCard(0x1083) and c:IsReason(REASON_DESTROY) and (c:IsReason(REASON_BATTLE) or c:GetReasonPlayer() == 1 - tp and c:IsReason(REASON_DESTROY)) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c6471156.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c6471156.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c6471156.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
