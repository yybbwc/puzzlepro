...
function c62803464.filter(c, sc, tp)
  if not (c:IsAttribute(ATTRIBUTE_EARTH) and c:IsDiscardable()) then
    return false
  end
...
function c62803464.srfilter(c, race)
  return c:IsAttribute(ATTRIBUTE_EARTH) and c:IsAttackBelow(1850) and c:IsAbleToHand() and c:GetOriginalRace() & race > 0
end
function c62803464.srcost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(100)
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62803464.filter, tp, LOCATION_HAND, 0, 1, c, c, tp) and c:IsDiscardable()
  end
...
function c62803464.cfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousAttributeOnField(), ATTRIBUTE_EARTH) ~= 0
end
