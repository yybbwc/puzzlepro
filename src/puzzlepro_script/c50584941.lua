...
function c50584941.cfilter(c, tp)
  ...
  return c:IsSetCard(0x1045) and c:IsType(TYPE_SYNCHRO) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c50584941.filter, tp, LOCATION_MZONE, 0, 1, nil, code)
end
...
function c50584941.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c50584941.filter(chkc, e:GetLabel())
  end
  if chk == 0 then
