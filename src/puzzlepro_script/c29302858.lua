...
function c29302858.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29302858.thfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.GetFlagEffect(tp, 29302858) == 0
  end
...
function c29302858.indescost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29302858.indescfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c29302858.indestg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 29302858) == 0
  end
  Duel.RegisterFlagEffect(tp, 29302858, RESET_CHAIN, 0, 1)
end
...
function c29302858.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and Duel.GetFlagEffect(tp, 29302858) == 0
  end
  Duel.RegisterFlagEffect(tp, 29302858, RESET_CHAIN, 0, 1)
