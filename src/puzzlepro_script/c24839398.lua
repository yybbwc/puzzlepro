...
function s.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return (e:GetHandler():IsLocation(LOCATION_SZONE) or Duel.GetLocationCount(tp, LOCATION_SZONE) > 0) and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
end
...
function s.repfilter(c, tp)
  return c:IsControler(tp) and c:IsOnField() and c:IsType(TYPE_TRAP) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and c:IsFaceup()
end
function s.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return eg:IsExists(s.repfilter, 1, nil, tp) and c:IsAbleToRemove() and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
  end
