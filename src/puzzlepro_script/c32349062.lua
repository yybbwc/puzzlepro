...
function c32349062.distg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c32349062.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c32349062.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c32349062.distg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c32349062.disfilter(chkc, g)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c32349062.disfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, g)
  end
