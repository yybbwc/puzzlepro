...
function c41232647.spfilter1(c, e, tp)
  local lv = c:GetLevel()
  return lv > 0 and c:IsFaceup() and c:IsSetCard(0x133) and Duel.IsExistingMatchingCard(c41232647.spfilter2, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp, lv)
end
...
function c41232647.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c41232647.spfilter1(chkc, e, tp) and chkc ~= c
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c41232647.spfilter1, tp, LOCATION_MZONE, 0, 1, c, e, tp)
  end
...
function c41232647.cfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousRaceOnField(), RACE_DRAGON) ~= 0 and c:IsControler(tp)
end
