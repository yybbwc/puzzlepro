...
function c54451023.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c54451023.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c54451023.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c54451023.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsRace(RACE_PLANT)
end
function c54451023.spcon(e, tp, eg, ep, ev, re, r, rp)
  return bit.band(e:GetHandler():GetReason(), REASON_EFFECT + REASON_DESTROY) == REASON_EFFECT + REASON_DESTROY and e:GetHandler():GetEquipTarget()
end
...
function c54451023.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c54451023.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c54451023.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
