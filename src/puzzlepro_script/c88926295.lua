...
function c88926295.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c88926295.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c88926295.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c88926295.negcon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and rp == 1 - tp and Duel.IsChainNegatable(ev) and re:IsActiveType(TYPE_MONSTER)
end
...
function c88926295.negtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return (not rc:IsRelateToEffect(re) or rc:IsAbleToDeck()) and Duel.IsExistingMatchingCard(c88926295.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
