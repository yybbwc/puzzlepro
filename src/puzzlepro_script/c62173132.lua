...
function s.thfilter(c, e, tp)
  return c:IsFaceup() and c:IsRace(RACE_ILLUSION + RACE_SPELLCASTER) and (c:IsAbleToHand() or c:IsAbleToExtra()) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp)
end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and s.thfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.thfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
