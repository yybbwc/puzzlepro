...
function c29246354.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c29246354.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c29246354.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsEnvironment(42015635) then
    return
  end
