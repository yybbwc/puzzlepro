...
function c95245544.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(c95245544.costfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil, tp)
  end
...
function c95245544.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c95245544.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c95245544.drtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanRemove(tp) and Duel.IsPlayerCanDraw(tp, 1)
  end
