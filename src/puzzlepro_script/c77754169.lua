...
function c77754169.eqfilter(c, e, tp)
  return c:IsRace(RACE_INSECT) and c:IsCanBeEffectTarget(e) and c:CheckUniqueOnField(tp) and Duel.IsExistingMatchingCard(Card.IsCode, tp, LOCATION_GRAVE, 0, 2, c, c:GetCode())
end
...
function c77754169.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and g:CheckSubGroup(c77754169.fselect, 1, ft) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c77754169.tgfilter(c, tp)
  return c:GetFlagEffect(77754169) ~= 0 and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c77754169.desfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetTextAttack())
end
...
function c77754169.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetEquipGroup():IsExists(c77754169.tgfilter, 1, nil, tp)
  end
