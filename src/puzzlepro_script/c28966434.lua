...
function c28966434.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c28966434.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c28966434.filter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c28966434.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c1:IsLevelBelow(3) or c1:IsFacedown() or not c1:IsRelateToEffect(e) then
    return
  end
