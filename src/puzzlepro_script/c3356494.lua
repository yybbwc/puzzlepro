...
function c3356494.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c3356494.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c3356494.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c3356494.cfilter2(c, tp)
  return c:IsSetCard(0x55) and c:IsDiscardable() and Duel.IsExistingMatchingCard(c3356494.cfilter3, tp, LOCATION_HAND, 0, 1, c)
end
...
function c3356494.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsSummonType, tp, 0, LOCATION_MZONE, 1, nil, SUMMON_TYPE_SPECIAL)
  end
