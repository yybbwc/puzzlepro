...
function c32671443.costfilter(c, e, tp)
  return c:IsSetCard(0xc008) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c32671443.thfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
function c32671443.thfilter(c, e, tp)
  if not (c:IsSetCard(0xc008) and c:IsType(TYPE_MONSTER)) then
    return false
  end
...
function c32671443.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c32671443.costfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c32671443.atkfilter1(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xc008) and Duel.IsExistingTarget(c32671443.atkfilter2, tp, LOCATION_MZONE, 0, 1, c, c)
end
...
function c32671443.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c32671443.atkfilter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
