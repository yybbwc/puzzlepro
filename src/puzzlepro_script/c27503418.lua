...
function c27503418.filter1(c, e, tp, tc)
  ...
  return rlv > 0 and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_SYNCHRO, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, tc, c) > 0 and Duel.IsExistingMatchingCard(c27503418.filter2, tp, LOCATION_GRAVE, 0, 1, nil, tp, rlv)
end
...
function c27503418.activate(e, tp, eg, ep, ev, re, r, rp)
  local tc = Duel.GetAttackTarget()
  if Duel.NegateAttack() and tc:IsRelateToEffect(e) and tc:IsAbleToRemove() and not tc:IsImmuneToEffect(e) and aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_SMATERIAL) and Duel.IsExistingMatchingCard(c27503418.filter1, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, tc) and Duel.SelectYesNo(tp, aux.Stringid(27503418, 0)) then
    Duel.BreakEffect()
