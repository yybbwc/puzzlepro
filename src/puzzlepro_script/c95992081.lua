...
function c95992081.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c95992081.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c95992081.filter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil, e, tp)
  end
