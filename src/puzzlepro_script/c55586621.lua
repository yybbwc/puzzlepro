...
function c55586621.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return eg:GetCount() == 1 and rc:IsControler(tp) and rc:IsSetCard(0x31) and tc:IsType(TYPE_MONSTER) and tc:IsReason(REASON_BATTLE) and tc:IsLocation(LOCATION_GRAVE)
end
...
function c55586621.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c55586621.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c55586621.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
