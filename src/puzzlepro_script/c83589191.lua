...
function c83589191.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c83589191.filter(chkc, 1 - tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83589191.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, 1 - tp)
  end
...
function c83589191.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsType(TYPE_MONSTER) and not tc:IsImmuneToEffect(e) and not tc:IsAttack(Duel.GetLP(1 - tp)) then
    local e1 = Effect.CreateEffect(c)
...
function c83589191.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c83589191.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83589191.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
...
function c83589191.atkop(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local tc = Duel.GetFirstTarget()
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsType(TYPE_MONSTER) and not tc:IsImmuneToEffect(e) and not tc:IsAttack(Duel.GetLP(tp)) then
    local e1 = Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_SINGLE)
    e1:SetCode(EFFECT_SET_ATTACK_FINAL)
