...
function c1683982.cfilter2(c, tp)
  return not c:IsReason(REASON_BATTLE) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousRaceOnField() & RACE_REPTILE ~= 0 and c:IsRace(RACE_REPTILE) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c1683982.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c1683982.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c1683982.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c1683982.filter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
