...
function c20960340.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetAttacker():IsControler(1 - tp) and Duel.GetAttackTarget() == nil and Duel.GetAttacker():IsAttackAbove(Duel.GetLP(tp))
end
function c20960340.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local atk = Duel.GetLP(tp)
  if chk == 0 then
    return e:IsCostChecked() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 20960340, 0, TYPES_EFFECT_TRAP_MONSTER, atk, 0, 4, RACE_WARRIOR, ATTRIBUTE_LIGHT, POS_FACEUP_ATTACK)
  end
...
function c20960340.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local atk = Duel.GetLP(tp)
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 20960340, 0, TYPES_EFFECT_TRAP_MONSTER, atk, 0, 4, RACE_WARRIOR, ATTRIBUTE_LIGHT, POS_FACEUP_ATTACK) then
    return
  end
