...
function c94141712.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94141712.costfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c94141712.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGrave() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingMatchingCard(c94141712.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c94141712.thcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsReason(REASON_COST) and re:IsActivated() and re:IsActiveType(TYPE_MONSTER) and Duel.GetChainInfo(ev, CHAININFO_TRIGGERING_RACE) & RACE_DRAGON > 0
end
...
function c94141712.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94141712.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
