...
function c75304793.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = Duel.IsCanRemoveCounter(tp, 1, 0, 0x35, 7, REASON_COST) and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_ONFIELD + LOCATION_GRAVE, 1, nil)
  if chk == 0 then
    return ct > 0 and (b1 or b2)
  end
  local op = 0
  if b1 and b2 then
