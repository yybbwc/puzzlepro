...
function c88757791.cfilter(c, tp)
  return c:IsType(TYPE_PENDULUM) and not c:IsPublic() and Duel.IsExistingTarget(c88757791.scfilter, tp, LOCATION_PZONE, 0, 1, nil, c)
end
...
function c88757791.sctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_PZONE) and c88757791.scfilter(chkc, e:GetLabelObject())
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c88757791.cfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
