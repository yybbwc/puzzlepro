...
function c81983656.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c81983656.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c81983656.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c81983656.cbtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c81983656.cbfilter(chkc, at)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81983656.cbfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), at)
  end
...
function c81983656.cecon(e, tp, eg, ep, ev, re, r, rp)
  if e == re or rp == tp or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
    return false
  end
...
function c81983656.cetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c81983656.cefilter(chkc, ev)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c81983656.cefilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), ev)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
