...
function c68688135.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c68688135.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
end
...
function c68688135.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg, REASON_EFFECT) ~= 0 and re:GetHandler():IsType(TYPE_MONSTER) and re:GetHandler():GetBaseAttack() > 0 then
    Duel.BreakEffect()
