...
function c71172240.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c71172240.filter, tp, 0, LOCATION_MZONE, 1, nil) and not Duel.IsExistingMatchingCard(c71172240.filter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c71172240.tdtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c71172240.tdfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler())
  end
