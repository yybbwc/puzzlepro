...
function c43994202.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return #g == 3 and g:FilterCount(c43994202.cfilter, nil) == 3 and g:GetClassCount(Card.GetCode) == 1
end
function c43994202.filter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c43994202.filter2, tp, LOCATION_DECK, 0, 2, c, c:GetCode())
end
...
function c43994202.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c43994202.filter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
