...
function c60004971.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c60004971.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c60004971.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c60004971.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c60004971.eqfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c60004971.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and ec:IsFaceup() then
    ...
    local tc = g:GetFirst()
    if not tc or not Duel.Equip(tp, tc, ec) then
      return
    end
    local e1 = Effect.CreateEffect(c)
