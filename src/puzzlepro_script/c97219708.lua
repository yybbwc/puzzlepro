...
function c97219708.rectg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and not e:GetHandler():IsStatus(STATUS_CHAINING) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(Card.IsType, tp, LOCATION_ONFIELD + LOCATION_GRAVE, 0, 1, nil, TYPE_SPELL + TYPE_TRAP)
  end
...
function c97219708.spfilter(c, e, tp, mc)
  return c:IsType(TYPE_XYZ) and c:IsSetCard(0xba) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
...
function c97219708.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97219708.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, e:GetHandler())
  end
