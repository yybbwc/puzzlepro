...
function c79928401.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c79928401.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c79928401.cfilter(c, tp)
  return c:IsLevelAbove(1) and c:IsRace(RACE_FAIRY) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c79928401.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetLevel())
end
