...
function c96203584.costfilter(c, e, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingTarget(c96203584.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c:GetOriginalAttribute())
end
...
function c96203584.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96203584.costfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c96203584.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c96203584.spfilter(chkc, e, tp, attr)
  end
  if chk == 0 then
    return true
  end
...
function c96203584.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96203584.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
