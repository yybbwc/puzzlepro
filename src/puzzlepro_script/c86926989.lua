...
function c86926989.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c86926989.filter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86926989.filter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp, zone)
  end
