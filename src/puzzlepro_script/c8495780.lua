...
function c8495780.cfcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bit.band(r, REASON_EFFECT) ~= 0 and re:GetHandler():IsAttribute(ATTRIBUTE_WATER) and c:IsPreviousLocation(LOCATION_DECK + LOCATION_GRAVE) and c:IsPreviousControler(tp) and not c:IsPublic()
end
...
function c8495780.spfilter(c, e, tp)
  return c:IsLevelBelow(4) and c:IsAttribute(ATTRIBUTE_WATER) and not c:IsCode(8495780) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c8495780.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c8495780.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c8495780.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
