...
function c55072170.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c55072170.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c55072170.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, tp)
  end
...
function c55072170.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    return c:IsAbleToRemoveAsCost() and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c55072170.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, cg:GetCount())
  end
  local cg = Duel.GetMatchingGroup(c55072170.cfilter, tp, LOCATION_GRAVE, 0, nil)
