...
function c54734082.spfilter(c, e, tp)
  return c:IsSetCard(0x2066) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and not Duel.IsExistingMatchingCard(c54734082.cfilter2, tp, LOCATION_ONFIELD, 0, 1, nil, c:GetCode())
end
function c54734082.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c54734082.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
