...
function c23361526.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(23361526, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c23361526.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = e:GetLabel() == 1 and ct > 0 and mg:FilterCount(c23361526.spfilter, nil, e, tp, c) == ct and Duel.GetLocationCount(tp, LOCATION_MZONE) >= ct and not Duel.IsPlayerAffectedByEffect(tp, 59822133)
  if chk == 0 then
    return b1 or b2
  end
  local off = 1
...
function c23361526.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sel == 0 then
    ...
    if g:GetCount() < mg:GetCount() then
      return
    end
