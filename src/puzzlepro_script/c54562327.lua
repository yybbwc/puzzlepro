...
function c54562327.filter(c, e, tp)
  return not c:IsPublic() and c:IsType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(c54562327.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c)
end
function c54562327.spfilter(c, e, tp, pc)
  return c:IsSetCard(0x195) and c:IsAttribute(pc:GetAttribute()) and not c:IsCode(pc:GetCode()) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c54562327.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c54562327.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c54562327.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or pc == nil then
    return
  end
