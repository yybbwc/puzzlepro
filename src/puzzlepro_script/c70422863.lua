...
function c70422863.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc:IsFaceup() and tc:IsType(TYPE_XYZ) and tc:IsSetCard(0x9c) and tc:IsControler(tp) and tc:IsLocation(LOCATION_MZONE)
end
...
function c70422863.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c70422863.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
