...
function c61606250.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(61606250, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c61606250.thfilter(c, e, tp, code)
  if not (c:IsCode(code) and c:IsType(TYPE_MONSTER)) then
    return false
  end
...
function c61606250.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c61606250.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61606250.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
