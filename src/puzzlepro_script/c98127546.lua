...
function c98127546.exmatcheck(c, lc, tp)
  if not c:IsControler(1 - tp) then
    return false
  end
  ...
  for _, te in pairs(le) do
    ...
    if related and not te:GetHandler():IsCode(98127546) then
      return false
    end
  end
  return true
...
function c98127546.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(aux.NegateMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c98127546.efilter(e, te)
  if te:GetOwnerPlayer() == e:GetHandlerPlayer() or not te:IsActivated() then
    return false
  end
...
function c98127546.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.IsChainNegatable(ev) and rp == 1 - tp and (ex and (dv & LOCATION_GRAVE == LOCATION_GRAVE or g and g:IsExists(c98127546.cfilter, 1, nil)) or re:IsHasCategory(CATEGORY_GRAVE_SPSUMMON))
end
