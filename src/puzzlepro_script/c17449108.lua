...
function c17449108.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and c17449108.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17449108.filter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, e:GetHandler())
  end
