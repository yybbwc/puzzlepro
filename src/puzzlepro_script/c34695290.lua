...
function c34695290.negcon(e, tp, eg, ep, ev, re, r, rp)
  return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and ep == 1 - tp and re:IsActiveType(TYPE_SPELL) and Duel.IsChainNegatable(ev)
end
function c34695290.negcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToRemoveAsCost, tp, LOCATION_HAND + LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c34695290.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c34695290.thtgfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c34695290.thtgfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
