...
function c31320433.mattg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c31320433.matfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
end
...
function c31320433.datg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c31320433.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c31320433.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
