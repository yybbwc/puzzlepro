...
function c29867611.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c29867611.filter(chkc, tp)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c29867611.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, tp)
  end
...
function c29867611.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ec and ec:IsReason(REASON_BATTLE + REASON_EFFECT) and not ec:IsReason(REASON_REPLACE) and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
  end
