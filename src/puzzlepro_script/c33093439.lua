...
function c33093439.cfilter(c)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsAbleToRemoveAsCost() and (not c:IsLocation(LOCATION_MZONE) or c:IsFaceup())
end
function c33093439.spcon(e, c)
  ...
  return Duel.IsExistingMatchingCard(c33093439.cfilter, tp, LOCATION_MZONE, 0, 1, nil) or (Duel.IsExistingMatchingCard(c33093439.cfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0)
end
