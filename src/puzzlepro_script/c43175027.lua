...
function c43175027.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_WATER) and c:IsLevelBelow(4) and not c:IsCode(43175027) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c43175027.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c43175027.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
