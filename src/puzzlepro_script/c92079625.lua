...
function c92079625.cfilter(c, tp)
  return c:IsFaceup() and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c92079625.tgfilter, tp, LOCATION_EXTRA, 0, 1, nil, c:GetAttribute())
end
...
function c92079625.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c92079625.cfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c92079625.cfilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
  end
...
function c92079625.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and Duel.IsPlayerCanDiscardDeck(tp, ct)
  end
