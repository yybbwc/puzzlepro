...
function c80433039.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.Remove(tc, POS_FACEUP, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_REMOVED) and Duel.IsExistingMatchingCard(c80433039.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(80433039, 2)) then
    Duel.BreakEffect()
    Duel.ShuffleHand(tp)
