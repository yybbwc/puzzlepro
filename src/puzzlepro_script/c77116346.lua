...
function c77116346.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77116346.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c77116346.thfilter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_EXTRA) and c:IsSetCard(0xd8) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
