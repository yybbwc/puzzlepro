...
function c4474060.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c4474060.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, 0, LOCATION_ONFIELD) > 0 and Duel.IsExistingTarget(c4474060.atkfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c4474060.cfilter(c, tp)
  return c:IsSetCard(0xee) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c4474060.thfilter, tp, LOCATION_GRAVE, 0, 1, c)
end
...
function c4474060.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c4474060.cfilter, tp, LOCATION_GRAVE, 0, 1, c, tp)
  end
...
function c4474060.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4474060.thfilter(chkc)
  end
  if chk == 0 then
    return true
  end
