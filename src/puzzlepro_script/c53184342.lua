...
function c53184342.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c53184342.spcostfilter, tp, LOCATION_GRAVE, 0, 2, e:GetHandler())
  end
...
function c53184342.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c53184342.disfilter(chkc, eq)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c53184342.disfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil, eq)
  end
...
function c53184342.filter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(1 - tp) and c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and c53184342.chkfilter(c, tp)
end
function c53184342.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  local g = eg:Filter(c53184342.filter, nil, tp)
  if chk == 0 then
    return #g > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) >= #g
  end
