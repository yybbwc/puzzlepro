...
function c37507488.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c37507488.filter(chkc, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c37507488.filter, tp, LOCATION_MZONE, 0, 1, nil, ft) and Duel.IsExistingMatchingCard(c37507488.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
