...
function c5466615.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return aux.NegateEffectMonsterFilter(chkc) and chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE)
  end
  if chk == 0 then
    return true
  end
  if Duel.IsExistingMatchingCard(c5466615.tgfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(5466615, 0)) then
...
function c5466615.drcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rc:IsRelateToBattle() and rc:IsStatus(STATUS_OPPO_BATTLE) and rc:IsControler(tp) and rc:IsFaceup() and rc:IsSetCard(0x157) and rc:IsLevelAbove(8)
end
