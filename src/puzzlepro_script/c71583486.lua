...
function c71583486.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(71583486, 1), aux.Stringid(71583486, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(71583486, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(71583486, 2)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
