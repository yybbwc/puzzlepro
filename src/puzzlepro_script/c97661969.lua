...
function c97661969.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c97661969.spfilter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c97661969.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp, zone)
  end
...
function c97661969.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97661969.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
