...
function c40139997.discon(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) or not Duel.IsChainNegatable(ev) or ep == tp then
    return false
  end
...
function c40139997.spfilter(c, e, tp)
  if not (c:IsCode(73580471) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)) then
    return false
  end
...
function c40139997.disop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.NegateActivation(ev) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c40139997.spfilter), tp, LOCATION_EXTRA + LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(40139997, 3)) then
    Duel.BreakEffect()
