...
function c70156997.cfilter(c, ft, tp)
  return c:IsAttribute(ATTRIBUTE_EARTH) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup())
end
function c70156997.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  e:SetLabel(1)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c70156997.cfilter, 1, nil, ft, tp)
  end
...
function c70156997.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c70156997.filter(chkc, e, tp)
  end
  if chk == 0 then
    if e:GetLabel() == 1 then
      ...
      return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c70156997.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
    end
  end
  local ex = nil
  if e:GetLabel() == 1 then
