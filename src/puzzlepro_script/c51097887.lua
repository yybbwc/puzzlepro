...
function c51097887.spfilter1(c, e, tp)
  return c:IsFaceup() and c:IsRace(RACE_BEAST + RACE_BEASTWARRIOR + RACE_WINDBEAST) and Duel.IsExistingMatchingCard(c51097887.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetAttack(), c:GetRace())
end
...
function c51097887.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c51097887.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingTarget(c51097887.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
