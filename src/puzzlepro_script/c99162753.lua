...
function c99162753.spfilter(c, e, tp, g)
  return g:IsExists(aux.FilterEqualFunction(Card.GetOriginalRace, c:GetOriginalRace()), 1, nil) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_DECK) and Duel.GetMZoneCount(tp, g) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, g, c) > 0)
end
