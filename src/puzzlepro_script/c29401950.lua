...
function c29401950.filter(c, tp, ep)
  return c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:GetAttack() >= 1500 and ep ~= tp and c:IsAbleToRemove()
end
function c29401950.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local tc = eg:GetFirst()
  if chk == 0 then
    return c29401950.filter(tc, tp, ep)
  end
...
function c29401950.filter2(c, tp)
  return c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:GetAttack() >= 1500 and c:IsSummonPlayer(1 - tp) and c:IsAbleToRemove()
end
function c29401950.target2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c29401950.filter2, 1, nil, tp)
  end
...
function c29401950.filter3(c, e, tp)
  return c:IsFaceup() and c:GetAttack() >= 1500 and c:IsSummonPlayer(1 - tp) and c:IsRelateToEffect(e) and c:IsLocation(LOCATION_MZONE)
end
