...
function c80101899.rmfilter(c, tp)
  return c:GetType() == TYPE_TRAP and not c:IsCode(80101899) and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c80101899.setfilter, tp, LOCATION_DECK, 0, 1, c, c:GetCode())
end
...
function c80101899.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c80101899.rmfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
...
function c80101899.aclimit1(e, tp, eg, ep, ev, re, r, rp)
  if ep == 1 - tp or not re:IsHasType(EFFECT_TYPE_ACTIVATE) or not re:IsActiveType(TYPE_TRAP) then
    return
  end
