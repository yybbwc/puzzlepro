...
function s.spfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_DARK) and c:IsRace(RACE_PYRO) and c:IsLevelAbove(8) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE, 1 - tp)
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.tgfilter, tp, 0, LOCATION_ONFIELD, 1, nil, tp) and Duel.IsExistingTarget(s.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1 and Duel.SendtoGrave(tc1, REASON_EFFECT) > 0 and tc1:IsLocation(LOCATION_GRAVE) and tc2 and aux.NecroValleyFilter()(tc2) then
