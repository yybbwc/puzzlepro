...
function c97617181.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c97617181.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c97617181.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c97617181.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return (ph > PHASE_MAIN1 and ph < PHASE_MAIN2) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED) and ec:IsReason(REASON_BATTLE + REASON_EFFECT) and not ec:IsReason(REASON_REPLACE)
  end
