...
function c14882493.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsAttribute, 1, e:GetHandler(), ATTRIBUTE_DARK)
  end
...
function c14882493.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c14882493.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c14882493.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c14882493.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
