...
function c55990317.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c55990317.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c55990317.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return eg:IsExists(c55990317.repfilter, 1, nil, tp) and c:IsAbleToRemove() and not c:IsStatus(STATUS_DESTROY_CONFIRMED + STATUS_BATTLE_DESTROYED)
  end
