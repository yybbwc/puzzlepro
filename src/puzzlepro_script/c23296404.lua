...
function c23296404.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c23296404.desfilter(chkc)
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  local ct = -ft + 1
  if chk == 0 then
    return ct <= 3 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingTarget(c23296404.desfilter, tp, LOCATION_ONFIELD, 0, 3, nil) and (ct <= 0 or Duel.IsExistingTarget(c23296404.desfilter, tp, LOCATION_MZONE, 0, ct, nil))
  end
  local g = nil
  if ct > 0 then
...
function c23296404.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c23296404.thfilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c23296404.thfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c23296404.tdfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
