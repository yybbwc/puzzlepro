...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return ft > 1 and Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsType(TYPE_MONSTER) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 1 and Duel.GetMatchingGroupCount(aux.NecroValleyFilter(s.eqfilter), tp, LOCATION_DECK + LOCATION_HAND + LOCATION_GRAVE, 0, nil, code, tp) > 1 then
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_EQUIP)
