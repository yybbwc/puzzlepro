...
function c92719314.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(c:GetControler(), LOCATION_MZONE, 0) == 0 and Duel.IsExistingMatchingCard(Card.IsAttribute, c:GetControler(), LOCATION_GRAVE, 0, 5, nil, ATTRIBUTE_DARK)
end
function c92719314.costfilter(c, e, tp)
  return c:IsAttribute(ATTRIBUTE_DARK) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c92719314.tgfilter, tp, LOCATION_GRAVE, 0, 1, c, e, tp)
end
...
function c92719314.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92719314.tgfilter(chkc, e, tp)
  end
  if chk == 0 then
