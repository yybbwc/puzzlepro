...
function c63509474.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c63509474.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c63509474.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c63509474.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    local sc = g:GetFirst()
    if spchk and sc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not sc:IsAbleToHand() or Duel.SelectOption(tp, 1190, 1152) == 1) then
