...
function c79072916.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c79072916.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c79072916.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c79072916.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc and Duel.GetAttacker() == ec and tc:IsSummonType(SUMMON_TYPE_SPECIAL) and tc:IsControler(1 - tp)
end
...
function c79072916.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabelObject(tc)
    return tc and tc:IsAbleToGrave() and Duel.GetMZoneCount(tp, tc) > 0 and Duel.IsExistingMatchingCard(c79072916.sptgfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp, tc:GetOriginalAttribute())
  end
