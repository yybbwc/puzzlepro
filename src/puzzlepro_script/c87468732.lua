...
function c87468732.tptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_PZONE) and c87468732.tpfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c87468732.tpfilter, tp, LOCATION_PZONE, 0, 1, nil) and (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1))
  end
