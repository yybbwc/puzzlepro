...
function s.cfilter(c, tp)
  return c:IsFaceup() and c:IsLevelBelow(5) and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetOriginalRace())
end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or not tc:IsFaceup() or not tc:IsType(TYPE_MONSTER) then
    return
  end
