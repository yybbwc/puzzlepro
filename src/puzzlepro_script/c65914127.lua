...
function s.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 4 and g:CheckSubGroup(s.check, 5, 5, tp)
  end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return (e:IsCostChecked() or Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_DECK + LOCATION_EXTRA, 0, 1, nil)) and (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1))
  end
end
...
function s.rstg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_PZONE)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.rfilter, tp, LOCATION_PZONE, LOCATION_PZONE, 1, nil)
  end
