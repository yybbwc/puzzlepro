...
function s.lvcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.lvcfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function s.spfilter(c, e, tp)
  return c:IsSetCard(0x74, 0x77) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsLevelBelow(4) and not c:IsCode(id)
end
