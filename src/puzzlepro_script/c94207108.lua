...
function c94207108.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(94207109) == 0
  end
...
function c94207108.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c94207108.spfilter1, tp, LOCATION_SZONE, 0, 1, nil, e, tp, e:GetHandler())
  end
...
function c94207108.eqop(mc)
  return function(e, tp, eg, ep, ev, re, r, rp)
    local tc = e:GetLabelObject()
    if not Duel.Equip(tp, tc, mc) then
      return
    end
...
function c94207108.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c94207108.spfilter2, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
