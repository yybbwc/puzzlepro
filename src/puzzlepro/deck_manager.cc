...
namespace ygo {
  ...
  void DeckManager::LoadLFListSingle(const char *path) {
    ...
    if (fp) {
      while (std::fgets(linebuf, sizeof linebuf, fp)) {
        ...
        int code = 0;
        int count = -1;
        if (std::sscanf(linebuf, "%10d%*[ ]%1d", &code, &count) != 2) {
          continue;
        }
        if (count < 0 || count > 2) {
          continue;
  ...
  unsigned int DeckManager::CheckDeck(Deck &deck, int lfhash, int rule) {
    ...
    const auto *lflist = GetLFList(lfhash);
    if (!lflist) {
      return 0;
    }
    const auto &list = lflist->content;
  ...
  int DeckManager::LoadDeckFromStream(Deck &deck, std::istringstream &deckStream, bool is_packlist) {
    int32_t ct = 0;
    ...
    while (std::getline(deckStream, linebuf, '\n') && ct < PACK_MAX_SIZE) {
      ...
      //~ errno = 0;
      code = std::stoi(linebuf.c_str(), nullptr, 10);
      if (code >= INT32_MAX) {
        continue;
      }
      cardlist[ct++] = code;
