...
function c72924435.filter(c, e, tp)
  return c:IsRace(RACE_PLANT) and c:IsLevelAbove(5) and not c:IsCode(72924435) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
end
function c72924435.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72924435.filter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, e, tp)
  end
