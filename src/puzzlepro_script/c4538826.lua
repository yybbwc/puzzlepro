...
function c4538826.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c4538826.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4538826.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function c4538826.spcon(e, c)
  ...
  return ((c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0) or (c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)) and g:CheckSubGroup(aux.gfcheck, 2, 2, Card.IsAttribute, ATTRIBUTE_LIGHT, ATTRIBUTE_DARK)
end
...
function c4538826.gytg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetCount() > 0 and og:GetCount() > 0
  end
...
function c4538826.gyop(e, tp, eg, ep, ev, re, r, rp)
  local g = Duel.GetMatchingGroup(c4538826.gyfilter, tp, LOCATION_ONFIELD, 0, nil)
  if g:GetCount() == 0 or Duel.SendtoGrave(g, REASON_EFFECT) == 0 then
    return
  end
