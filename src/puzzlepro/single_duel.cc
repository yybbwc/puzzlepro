...
namespace ygo {
  ...
  SingleDuel::SingleDuel(bool is_match) : match_mode(is_match) {
  }
  ...
  void SingleDuel::JoinGame(DuelPlayer *dp, unsigned char *pdata, bool is_creater) {
    ...
    dp->game = this;
    if (!players[0] && !players[1] && observers.empty()) {
      host_player = dp;
    }
    STOC_JoinGame scjg;
    scjg.info = host_info;
  ...
  void SingleDuel::TPResult(DuelPlayer *dp, unsigned char tp) {
    ...
    last_replay.WriteData(players[0]->name, 40);
    last_replay.WriteData(players[1]->name, 40);
    ...
    last_replay.Write<int32_t>(host_info.start_lp);
    ;
    last_replay.Write<int32_t>(host_info.start_hand);
    ;
    last_replay.Write<int32_t>(host_info.draw_count);
    ;
    last_replay.Write<int32_t>(opt);
    ;
    //~ last_replay.Flush();
    auto load = [&](const std::vector<code_pointer> &deck_container, uint8_t p, uint8_t location) {
      last_replay.Write<int32_t>(deck_container.size());
      ;
      for (auto cit = deck_container.rbegin(); cit != deck_container.rend(); ++cit) {
        ...
        last_replay.Write<int32_t>((*cit)->first);
        ;
      }
    };
  ...
  void SingleDuel::DuelEndProc() {
    if (!match_mode) {
      ...
      if (match_kill || (winc[0] == 2 || (winc[0] == 1 && winc[2] == 2)) || (winc[1] == 2 || (winc[1] == 1 && winc[2] == 2)) || (winc[2] == 3 || (winc[0] == 1 && winc[1] == 1 && winc[2] == 1))) {
        NetServer::SendPacketToPlayer(players[0], STOC_DUEL_END);
        NetServer::ReSendToPlayer(players[1]);
        for (auto oit = observers.begin(); oit != observers.end(); ++oit) {
          NetServer::ReSendToPlayer(*oit);
  ...
  int SingleDuel::Analyze(unsigned char *msgbuffer, unsigned int len) {
    ...
    unsigned char *pbufw = nullptr;
    unsigned char *pbuf = msgbuffer;
    int player = 0;
    int count = 0;
    int type = 0;
  ...
  void SingleDuel::SingleTimer(evutil_socket_t fd, short events, void *arg) {
    auto *sd = static_cast<SingleDuel *>(arg);
    sd->time_elapsed++;
