...
function c15576074.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c15576074.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c15576074.filter2, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingTarget(c15576074.filter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
