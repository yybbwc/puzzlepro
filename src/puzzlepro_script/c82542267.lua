...
function c82542267.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c82542267.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82542267.filter, tp, 0, LOCATION_GRAVE, 1, nil)
  end
