...
function c53503015.cfilter(c, chk, p, chk1, chk2)
  return c:IsFaceup() and c:IsAbleToGraveAsCost() and ((chk == 0 and c:IsRace(RACE_PLANT) == p) or ((c:IsRace(RACE_PLANT) and chk1) or (not c:IsRace(RACE_PLANT) and chk2)))
end
...
function c53503015.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local chk2 = Duel.IsExistingMatchingCard(c53503015.cfilter, tp, LOCATION_MZONE, 0, 1, nil, 0, false) and Duel.IsExistingMatchingCard(c53503015.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  if chk == 0 then
