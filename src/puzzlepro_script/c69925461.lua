...
function s.spfilter(c, e, tp, check)
  return c:IsLevelBelow(6) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_HAND) or (check and not c:IsCode(69925461)))
end
...
function s.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp, check)
  end
