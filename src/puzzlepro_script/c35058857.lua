...
function c35058857.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c35058857.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c35058857.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and c35058857.lvfilter(chkc, lv)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c35058857.lvfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil, lv)
  end
