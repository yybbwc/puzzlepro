...
function c9485511.spfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xb2) and not c:IsCode(9485511) and c:IsAbleToHandAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c9485511.indtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c9485511.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c9485511.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
