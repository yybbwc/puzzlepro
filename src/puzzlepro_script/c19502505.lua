...
function c19502505.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19502505.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c19502505.negcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return g and g:GetCount() == 1 and g:GetFirst():IsControler(tp) and g:GetFirst():IsLocation(LOCATION_MZONE) and re:IsActiveType(TYPE_SPELL) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev)
end
...
function c19502505.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c19502505.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c19502505.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
