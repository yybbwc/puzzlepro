...
function c70546737.filter(c, clv)
  return c:IsSetCard(0x87) and c:IsLevelAbove(1) and not c:IsLevel(clv) and ((c:IsLocation(LOCATION_MZONE) and c:IsFaceup()) or c:IsLocation(LOCATION_GRAVE))
end
function c70546737.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and c70546737.filter(chkc, e:GetHandler():GetLevel())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70546737.filter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, e:GetHandler(), e:GetHandler():GetLevel())
  end
