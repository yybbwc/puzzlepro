...
function c95500396.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c95500396.eqfilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c95500396.eqfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c95500396.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetControler() == tc:GetControler() and tc:IsReleasable() and Duel.GetMZoneCount(tp, tc) > 0
  end
...
function c95500396.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95500396.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
