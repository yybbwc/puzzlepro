...
function c34884015.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(nil, tp, LOCATION_PZONE, 0, 2, nil)
  end
...
function c34884015.expcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x4e, 3, REASON_COST)
  end
  e:GetHandler():RemoveCounter(tp, 0x4e, 3, REASON_COST)
end
function c34884015.exptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 34884015) == 0
  end
end
