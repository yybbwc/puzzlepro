...
function c46384672.cfilter(c, tp)
  ...
  return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and Duel.IsExistingTarget(c46384672.dfilter, tp, 0, LOCATION_MZONE, 1, nil, atk)
end
...
function c46384672.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c46384672.cfilter, tp, LOCATION_HAND, 0, 1, nil, tp)
  end
...
function c46384672.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c46384672.dfilter(chkc, e:GetLabel())
  end
  if chk == 0 then
    return true
  end
...
function c46384672.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c46384672.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
