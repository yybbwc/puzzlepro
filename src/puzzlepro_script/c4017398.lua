...
function c4017398.spfilter(c, e, tp)
  return c:IsSetCard(0x107f, 0x107e, 0x207e) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)))
end
function c4017398.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4017398.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4017398.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c4017398.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4017398.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4017398.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
