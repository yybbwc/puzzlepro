...
function c44256816.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(44256816, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c44256816.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c44256816.filter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil, e, tp)
  end
