...
function c16678947.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc:IsPreviousControler(tp) and tc:IsLocation(LOCATION_GRAVE) and tc:IsRace(RACE_PSYCHO) and bit.band(tc:GetBattlePosition(), POS_FACEUP) ~= 0 and bc:IsRelateToBattle() and bc:IsControler(1 - tp) and bc == Duel.GetAttacker()
end
function c16678947.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return bc:IsCanBeEffectTarget(e)
  end
