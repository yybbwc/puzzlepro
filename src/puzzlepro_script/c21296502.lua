...
function c21296502.dircon(e)
  ...
  return Duel.IsExistingMatchingCard(c21296502.cfilter1, tp, LOCATION_ONFIELD, 0, 1, nil) and not Duel.IsExistingMatchingCard(c21296502.cfilter2, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function c21296502.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21296502.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c21296502.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21296502.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c21296502.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c21296502.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
