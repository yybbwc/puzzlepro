...
function c22653490.tdtg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c22653490.tdfilter1(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c22653490.tdtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c22653490.tdfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22653490.tdfilter2, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TODECK)
