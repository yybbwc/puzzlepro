...
function s.filter(c, e, tp)
  return c:IsSetCard(0x21) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and not c:IsCode(id)
end
...
function s.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and Duel.IsExistingMatchingCard(s.dfilter, tp, LOCATION_GRAVE + LOCATION_MZONE, 0, 1, nil)
  end
