...
function c91530236.filter(c)
  return c:IsSetCard(0xbf) and c:IsType(TYPE_MONSTER) or c:IsSetCard(0x10c0) and c:IsType(TYPE_MONSTER) or c:IsSetCard(0xc0) and c:IsType(TYPE_SPELL + TYPE_TRAP)
end
function c91530236.thfilter(c, e, tp, mft, sft)
  return c91530236.filter(c) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c91530236.setfilter, tp, LOCATION_DECK, 0, 1, c, e, tp, mft, sft, c:GetCode())
end
function c91530236.setfilter(c, e, tp, mft, sft, code)
  return c91530236.filter(c) and not c:IsCode(code) and (sft > 0 and c:IsSSetable(true) or mft > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEDOWN_DEFENSE))
end
