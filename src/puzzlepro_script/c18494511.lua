...
function c18494511.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c18494511.costfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c18494511.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    ...
    if ft > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 18494512, 0x16c, TYPES_TOKEN_MONSTER, 0, 0, 3, RACE_AQUA, ATTRIBUTE_WATER) and Duel.SelectYesNo(tp, aux.Stringid(18494511, 1)) then
      Duel.BreakEffect()
