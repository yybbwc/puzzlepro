...
function c89185742.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and Duel.IsExistingMatchingCard(c89185742.costfilter, tp, LOCATION_HAND, 0, 1, c)
  end
...
function c89185742.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c89185742.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
