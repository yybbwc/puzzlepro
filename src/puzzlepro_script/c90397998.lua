...
function c90397998.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c90397998.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c90397998.desfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c90397998.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsExistingMatchingCard(c90397998.cfilter, tp, LOCATION_MZONE, 0, 1, nil) then
    return
  end
...
function c90397998.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and c:IsOnField() and c:IsFaceup() and Duel.IsExistingMatchingCard(c90397998.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
