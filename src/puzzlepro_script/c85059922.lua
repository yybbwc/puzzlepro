...
function c85059922.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c85059922.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c85059922.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c85059922.remcostfilter(c, tp)
  return c:IsAttribute(ATTRIBUTE_DARK + ATTRIBUTE_LIGHT) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_MZONE, 1, c)
end
...
function c85059922.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c85059922.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c85059922.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
