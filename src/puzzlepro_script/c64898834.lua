...
function c64898834.filter(c, e, tp)
  return c:IsSetCard(0x27) and c:IsLevelBelow(3) and c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c64898834.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c64898834.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
