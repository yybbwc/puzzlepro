...
function c90173539.checkop(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():GetFlagEffect(90173539) ~= 0 then
    return
  end
...
function c90173539.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c90173539.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
