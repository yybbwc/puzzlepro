...
function c9409625.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:GetFlagEffect(9409625) == 0
  end
...
function c9409625.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9409625.drfilter, tp, LOCATION_HAND, 0, 1, nil) and c9409625.cost(e, tp, eg, ep, ev, re, r, rp, 0)
  end
...
function c9409625.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c9409625.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c9409625.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
