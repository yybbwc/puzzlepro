...
function c30829071.eqtg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c30829071.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c30829071.eqfilter1(c, tp)
  return c:IsSetCard(0x15c) and c:IsType(TYPE_TRAP) and c:IsFaceup() and c:GetEquipTarget() and Duel.IsExistingMatchingCard(c30829071.eqfilter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c:GetEquipTarget(), tp)
end
...
function c30829071.eqtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c30829071.eqfilter1(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30829071.eqfilter1, tp, LOCATION_SZONE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
