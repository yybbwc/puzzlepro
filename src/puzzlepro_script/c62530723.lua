...
function c62530723.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c62530723.atkfilter(chkc)
  end
  if chk == 0 then
  ...
  if Duel.GetCurrentPhase() == PHASE_DAMAGE or (Duel.IsExistingTarget(c62530723.atkfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(62530723, 0))) then
...
function c62530723.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rp == 1 - tp and re:IsActiveType(TYPE_SPELL) and loc & LOCATION_SZONE == LOCATION_SZONE and seq <= 4 and Duel.IsExistingMatchingCard(c62530723.cfilter, tp, LOCATION_MZONE, 0, 1, nil, seq)
end
