...
function c58981727.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58981727.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c58981727.tftg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsOnField() and c58981727.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c58981727.tdfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.IsExistingMatchingCard(c58981727.tffilter, tp, LOCATION_DECK, 0, 1, nil)
  end
