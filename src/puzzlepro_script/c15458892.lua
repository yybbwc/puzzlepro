...
function c15458892.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c15458892.cfilter, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function c15458892.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c15458892.thfilter(chkc)
  end
  if chk == 0 then
    return true
  end
