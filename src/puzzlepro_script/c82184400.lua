...
function c82184400.rmfilter(c, tp)
  return c:IsFaceup() and c:IsRace(RACE_FISH) and c:IsAbleToRemove() and Duel.IsExistingTarget(Card.IsAbleToRemove, tp, 0, LOCATION_ONFIELD, 1, c)
end
function c82184400.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c82184400.rmfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), tp)
  end
