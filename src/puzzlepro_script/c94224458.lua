...
function c94224458.spfilter(c, e, tp, check)
  return c:IsRace(RACE_ZOMBIE) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (check or c:IsSetCard(0x1142))
end
function c94224458.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c94224458.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp, chk1)
  end
...
function c94224458.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c94224458.stfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
