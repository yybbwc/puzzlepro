...
function c18189187.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) then
    if Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 and not c:IsAttribute(ATTRIBUTE_DARK) and Duel.SelectYesNo(tp, aux.Stringid(18189187, 1)) then
      Duel.BreakEffect()
      local e1 = Effect.CreateEffect(c)
...
function c18189187.thop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.IsEnvironment(74665651, PLAYER_ALL, LOCATION_FZONE) and Duel.IsExistingMatchingCard(c18189187.thfilter, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(18189187, 2)) then
