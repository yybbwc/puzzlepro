...
function c9349094.ovtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c9349094.ovfilter(chkc, typ)
  end
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and Duel.IsExistingTarget(c9349094.ovfilter, tp, LOCATION_GRAVE, 0, 1, nil, typ)
  end
