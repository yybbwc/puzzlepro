...
function c82112494.pcfilter(c, tp)
  return not c:IsCode(82112494) and c:IsSetCard(0x9a) and c:IsType(TYPE_PENDULUM) and not c:IsForbidden() and c:CheckUniqueOnField(tp, LOCATION_SZONE)
end
function c82112494.pctg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1)) and Duel.IsExistingMatchingCard(c82112494.pcfilter, tp, LOCATION_DECK, 0, 1, nil, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c82112494.pcop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g == 0 or not Duel.MoveToField(g:GetFirst(), tp, tp, LOCATION_PZONE, POS_FACEUP, true) then
    return
  end
...
function c82112494.costfilter(c, e, tp)
  return c:IsType(TYPE_MONSTER) and c:IsDiscardable() and Duel.IsExistingMatchingCard(c82112494.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, c, e, tp)
end
...
function c82112494.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c82112494.costfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c82112494.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c82112494.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
