...
function s.tfilter(c, tp)
  return c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsSetCard(0x188) and c:IsControler(tp)
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.cfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
