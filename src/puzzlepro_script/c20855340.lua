...
function c20855340.condition(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():IsPreviousControler(tp) and e:GetHandler():IsReason(REASON_DESTROY)
end
...
function c20855340.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c20855340.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
