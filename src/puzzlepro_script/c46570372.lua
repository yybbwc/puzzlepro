...
function c46570372.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c46570372.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, Duel.GetTurnCount())) or (Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c46570372.filter, tp, 0, LOCATION_GRAVE, 1, nil, e, 1 - tp, Duel.GetTurnCount()))
  end
