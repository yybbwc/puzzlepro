...
function c58811192.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58811192.cfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
...
function c58811192.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58811192.filter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c58811192.spfilter1(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x10b5) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and Duel.IsExistingTarget(c58811192.spfilter2, tp, LOCATION_REMOVED, 0, 1, c, e, tp)
end
...
function c58811192.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetMZoneCount(tp, e:GetHandler()) > 1 and Duel.IsExistingTarget(c58811192.spfilter1, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
