...
function c52900000.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToHand, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c52900000.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoHand(sg, nil, REASON_EFFECT) ~= 0 then
    ...
    if not sg2:IsExists(Card.IsLocation, 1, nil, LOCATION_HAND) then
      return
    end
    local tg = Duel.GetMatchingGroup(c52900000.spfilter, tp, LOCATION_HAND, 0, nil, e, tp)
    if tg:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(52900000, 1)) then
      Duel.BreakEffect()
