...
function c92998610.cfilter(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c92998610.tgfilter, tp, LOCATION_DECK, 0, 1, nil, c)
end
...
function c92998610.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c92998610.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c92998610.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
