...
function c89907227.tktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 89907228, 0, TYPES_TOKEN_MONSTER, 3300, 3000, 10, RACE_FISH, ATTRIBUTE_WATER, POS_FACEUP_DEFENSE)
  end
...
function c89907227.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_DESTROY) and rp == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and Duel.IsExistingMatchingCard(Card.IsCode, tp, LOCATION_ONFIELD, 0, 1, nil, 89907228)
end
