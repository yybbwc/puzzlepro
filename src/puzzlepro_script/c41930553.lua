...
function c41930553.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c41930553.rfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41930553.cfilter, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingTarget(c41930553.rfilter, tp, 0, LOCATION_GRAVE, 1, nil)
  end
