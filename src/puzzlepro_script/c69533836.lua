...
function s.eqfilter(c, tp)
  return c:IsControler(tp) and c:IsType(TYPE_SYNCHRO) and c:IsRace(RACE_DRAGON)
end
function s.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.eqfilter(chkc, tp) and s.eqlimit(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.eqfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
...
function s.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  local qc = e:GetHandler():GetEquipTarget()
  if chk == 0 then
    return qc and Duel.IsExistingMatchingCard(s.atkfilter, tp, 0, LOCATION_MZONE, 1, nil, qc:GetAttack())
  end
end
...
function s.atktg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and s.rmfilter(chkc)
  end
  if chk == 0 then
