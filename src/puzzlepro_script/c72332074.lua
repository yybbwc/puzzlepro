...
function c72332074.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToDeck, tp, 0, LOCATION_ONFIELD, 1, nil) and Duel.GetFieldGroupCount(tp, 0, LOCATION_EXTRA) > 0 and Duel.IsPlayerCanSpecialSummon(1 - tp)
  end
...
function c72332074.activate(e, tp, eg, ep, ev, re, r, rp)
  local g = Duel.GetMatchingGroup(Card.IsAbleToDeck, tp, 0, LOCATION_ONFIELD, nil)
  if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c72332074.spfilter, 1 - tp, LOCATION_EXTRA, 0, 1, nil, e, 1 - tp) then
    Duel.BreakEffect()
...
function c72332074.actcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c72332074.costfilter, tp, LOCATION_GRAVE, 0, 1, c)
  end
...
function c72332074.acttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72332074.actfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
end
