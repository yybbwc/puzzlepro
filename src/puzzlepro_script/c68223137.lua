...
function c68223137.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c68223137.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c68223137.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
