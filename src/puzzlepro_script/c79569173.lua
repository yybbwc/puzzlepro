...
function c79569173.cfilter(c, tp)
  return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousControler(tp) and c:IsReason(REASON_DESTROY) and c:IsRace(RACE_DINOSAUR)
end
...
function c79569173.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_SZONE, PLAYER_NONE, 0) > 2
  end
...
function c79569173.activate(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(1 - tp, LOCATION_SZONE, PLAYER_NONE, 0) < 3 then
    return
  end
...
function c79569173.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c79569173.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c79569173.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
