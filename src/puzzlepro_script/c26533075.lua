...
function c26533075.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return tg:IsOnField() and tg:IsCanChangePosition() and tg:IsCanBeEffectTarget(e)
  end
...
function c26533075.descon(e, tp, eg, ep, ev, re, r, rp)
  return bit.band(r, REASON_DESTROY) ~= 0 and rp == 1 - tp and re:IsActiveType(TYPE_SPELL + TYPE_TRAP) and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():IsPreviousPosition(POS_FACEDOWN)
end
