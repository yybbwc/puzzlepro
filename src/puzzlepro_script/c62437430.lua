...
function c62437430.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62437430.filter, tp, LOCATION_HAND + LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c62437430.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetLabelObject():GetFlagEffect(62437430) ~= 0 and e:GetLabelObject():GetReasonEffect():GetHandler() == e:GetHandler()
  end
