...
function c89521713.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c89521713.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c89521713.filter, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler())
  end
