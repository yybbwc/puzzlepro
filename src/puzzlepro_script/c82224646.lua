...
function c82224646.tntg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c82224646.tnfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c82224646.tnfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c82224646.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c82224646.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c82224646.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c82224646.scfilter1(c, e, tp, mc)
  ...
  return c:IsCanBeSynchroMaterial() and c:IsCanBeSpecialSummoned(e, SUMMON_VALUE_SYNCHRO_MATERIAL, tp, false, false) and Duel.IsExistingMatchingCard(c82224646.scfilter2, tp, LOCATION_EXTRA, 0, 1, nil, tp, mg)
end
...
function c82224646.sctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_PZONE) and chkc:IsControler(tp) and c82224646.scfilter1(chkc, e, tp, c)
  end
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c82224646.scfilter1, tp, LOCATION_PZONE, 0, 1, nil, e, tp, c)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
...
function c82224646.scop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if mg:FilterCount(Card.IsLocation, nil, LOCATION_MZONE) < 2 then
    return
  end
