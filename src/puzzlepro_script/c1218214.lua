...
function c1218214.drawtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanDraw(tp, 1) and Duel.IsExistingMatchingCard(c1218214.cfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c1218214.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c1218214.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c1218214.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
