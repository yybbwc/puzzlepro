...
function c98935722.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c98935722.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return true
  end
  if Duel.IsExistingTarget(c98935722.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(98935722, 0)) then
...
function c98935722.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rp == 1 - tp and re:IsActiveType(TYPE_MONSTER) and loc == LOCATION_MZONE and Duel.IsExistingMatchingCard(c98935722.cfilter, tp, LOCATION_MZONE, 0, 1, nil, seq)
end
