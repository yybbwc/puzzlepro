...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function s.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsAttribute(ATTRIBUTE_FIRE)
end
...
function s.filter2(c, e, tp, m, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsRace(RACE_WARRIOR + RACE_DRAGON) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, nil, chkf)
end
