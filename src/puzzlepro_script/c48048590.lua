...
function c48048590.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(48048590, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c48048590.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsDiscardable, tp, LOCATION_HAND, 0, 1, nil) and c48048590.cost(e, tp, eg, ep, ev, re, r, rp, 0)
  end
...
function c48048590.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c48048590.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c48048590.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return c48048590.cost(e, tp, eg, ep, ev, re, r, rp, 0) and Duel.IsExistingMatchingCard(c48048590.filter1, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
