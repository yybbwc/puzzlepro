...
function c21007444.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c21007444.filter(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c21007444.filter, tp, LOCATION_GRAVE, 0, 2, nil, e, tp)
  end
...
function c21007444.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if dg:GetCount() > 0 then
    ...
    if og:IsContains(tg1) then
      dam = dam + at1
    end
    if tg2 and og:IsContains(tg2) then
      dam = dam + at2
    end
    if dam ~= 0 then
