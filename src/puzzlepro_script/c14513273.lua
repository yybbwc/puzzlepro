...
function s.rmlimit(e, c, rp, r, re)
  ...
  return c:IsControler(tp) and c:IsOnField() and c:IsCode(13331639) and c:IsFaceup() and r & REASON_EFFECT ~= 0 and re and re:GetOwnerPlayer() ~= tp
end
...
function s.pentg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsDestructable() and Duel.IsExistingMatchingCard(s.penfilter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil)
  end
...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(s.spfilter2, tp, LOCATION_HAND + LOCATION_EXTRA, 0, 1, nil)
  end
