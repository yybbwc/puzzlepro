...
function c19688343.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c19688343.desfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c19688343.spfilter(c, e, tp, code)
  return c:IsSetCard(0x1084) and c:IsType(TYPE_XYZ) and not c:IsCode(code) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c19688343.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc then
    ...
    if Duel.Destroy(tc, REASON_EFFECT) ~= 0 and Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg, REASON_EFFECT) ~= 0 then
      local c = e:GetHandler()
      local g = Duel.GetMatchingGroup(c19688343.spfilter, tp, LOCATION_EXTRA, 0, nil, e, tp, tc:GetCode())
      if g:GetCount() > 0 and c:IsRelateToChain() and c:IsCanOverlay() and Duel.SelectYesNo(tp, aux.Stringid(19688343, 0)) then
        Duel.BreakEffect()
