...
function c54851325.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c54851325.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if hc:IsRelateToEffect(e) and hc:IsControler(tp) and Duel.Destroy(hc, REASON_EFFECT) > 0 and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(1 - tp) and tc:IsCanBeDisabledByEffect(e) then
    ...
    if Duel.IsExistingMatchingCard(c54851325.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(54851325, 2)) then
      Duel.BreakEffect()
