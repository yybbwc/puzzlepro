...
function c54880296.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c54880296.filter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c54880296.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoHand(sg, nil, REASON_EFFECT) > 0 then
    ...
    if ct > 0 and Duel.IsPlayerCanDraw(tp, ct) and Duel.SelectYesNo(tp, aux.Stringid(54880296, 0)) then
      Duel.BreakEffect()
