...
function c26931058.filter1(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_UNION) and Duel.IsExistingMatchingCard(c26931058.filter2, tp, LOCATION_MZONE, 0, 1, c, c)
end
...
function c26931058.eftg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    if e:GetLabel() == 0 then
      return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c26931058.filter1(chkc, tp)
    else
      return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_SZONE) and c26931058.filter3(chkc, e, tp)
    end
  end
  ...
  elseif b1 then
    ...
    op = Duel.SelectOption(tp, aux.Stringid(26931058, 1)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
