...
function c9831539.filter1(c, tp)
  return c:IsLevelAbove(1) and c:IsAttackBelow(1000) and Duel.IsExistingTarget(c9831539.filter2, tp, LOCATION_MZONE, 0, 1, c, c:GetRace())
end
...
function c9831539.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c9831539.filter1, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
