...
function s.tgfilter(c, e, tp, chk)
  return c:IsSetCard(0x12f) and c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsControler(tp) and c:IsCanBeEffectTarget(e) and c:IsAttackAbove(1) and (chk or Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetAttack(), tp))
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and s.tgfilter(chkc, e, tp, true)
  end
...
function s.tfilter(c)
  return c:IsFaceup() and c:IsRace(RACE_INSECT) and not c:IsType(TYPE_TUNER) and c:IsLevelAbove(1)
end
function s.tntg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and s.tfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.tfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
