...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.CheckRemoveOverlayCard(tp, 1, 1, 1, REASON_EFFECT) and Duel.SelectEffectYesNo(tp, e:GetHandler(), aux.Stringid(id, 2)) then
      Duel.BreakEffect()
