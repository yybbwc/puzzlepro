...
function c37675138.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousLocation(LOCATION_GRAVE) and e:GetHandler():IsPreviousControler(tp) and typ & TYPE_MONSTER ~= 0 and race & RACE_ZOMBIE ~= 0
end
...
function c37675138.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsOnField() and c37675138.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c37675138.desfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
