...
function c28827503.spfilter(c, e, tp)
  return c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c28827503.lkfilter, tp, LOCATION_EXTRA, 0, 1, nil, c)
end
function c28827503.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c28827503.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c28827503.cfilter(c, tp)
  return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsType(TYPE_LINK) and c:IsReason(REASON_BATTLE + REASON_EFFECT)
end
