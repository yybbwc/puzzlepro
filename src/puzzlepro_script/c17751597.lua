...
function c17751597.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c17751597.filter1, tp, LOCATION_MZONE, 0, 1, nil) or Duel.IsExistingMatchingCard(c17751597.filter2, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c17751597.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c17751597.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c17751597.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
