...
function c90965652.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b2 = Duel.IsExistingMatchingCard(c90965652.thfilter2, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c90965652.thfilter3, tp, LOCATION_DECK, 0, 1, nil)
  if chk == 0 then
    return b1 or b2
  end
  local off = 1
...
function c90965652.activate(e, tp, eg, ep, ev, re, r, rp)
  if e:GetLabel() == 1 then
    ...
    if Duel.IsExistingMatchingCard(c90965652.thfilter2, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsExistingMatchingCard(c90965652.thfilter3, tp, LOCATION_DECK, 0, 1, nil) then
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
...
function c90965652.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return g:CheckSubGroup(c90965652.tgcheck, 2, 2)
  end
...
function c90965652.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1:IsImmuneToEffect(e) then
    return
  end
