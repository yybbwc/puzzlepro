...
function c48285768.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c48285768.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c48285768.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFaceup() and c:IsRelateToEffect(e) then
    ...
    if ct >= 2 and Duel.IsExistingMatchingCard(c48285768.exfilter, tp, LOCATION_EXTRA, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(48285768, 2)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
