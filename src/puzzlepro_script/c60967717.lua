...
function c60967717.filter(c, e, tp, ft)
  return c:IsSetCard(0x157) and c:IsLevelBelow(4) and (c:IsAbleToHand() or (ft > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)))
end
...
function c60967717.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x157) and c:IsLocation(LOCATION_MZONE) and c:IsLevelAbove(8) and c:IsControler(tp) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c60967717.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c60967717.repfilter, 1, nil, tp)
  end
