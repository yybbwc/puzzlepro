...
function s.lmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and s.filter(chkc, lg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter, tp, 0, LOCATION_MZONE, 1, nil, lg)
  end
...
function s.matval(e, lc, mg, c, tp)
  ...
  if not lc:IsLink(5) or ct:GetFlagEffectLabel(id) ~= fid then
    return false, nil
  end
