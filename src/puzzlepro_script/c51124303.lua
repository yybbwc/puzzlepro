...
function c51124303.spfilter(c, e, tp, mc)
  ...
  return c:IsSetCard(0xb4) and bit.band(c:GetType(), 0x81) == 0x81 and (not c.mat_filter or c.mat_filter(mc, tp)) and (not c.mat_group_check or c.mat_group_check(mg, tp)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_RITUAL, tp, false, true) and mc:IsCanBeRitualMaterial(c)
end
function c51124303.rfilter(c, mc)
  local mlv = mc:GetRitualLevel(c)
  if mlv == mc:GetLevel() then
    return false
  end
  local lv = c:GetLevel()
...
function c51124303.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c51124303.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c51124303.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c51124303.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
