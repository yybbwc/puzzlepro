...
function c77229910.condition(e, tp, eg, ep, ev, re, r, rp)
  if not Duel.IsChainNegatable(ev) or not re:IsHasType(EFFECT_TYPE_ACTIVATE) then
    return false
  end
...
function c77229910.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77229910.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
