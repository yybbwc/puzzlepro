...
function c43266605.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDiscardable() and not c:IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c43266605.repfilter, 1, nil, tp)
  end
...
function c43266605.cfilter(c, tp, se)
  return c:IsSetCard(0xc1) and c:IsType(TYPE_TUNER) and c:IsControler(tp) and (se == nil or c:GetReasonEffect() ~= se)
end
