...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(s.acfilter, tp, LOCATION_DECK, 0, 1, nil, tp) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
end
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 then
    ...
    if tc and Duel.MoveToField(tc, tp, tp, LOCATION_SZONE, POS_FACEUP, true) ~= 0 and c:IsRelateToEffect(e) then
      Duel.BreakEffect()
