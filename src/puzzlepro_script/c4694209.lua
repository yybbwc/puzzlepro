...
function c4694209.addccost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsCanRemoveCounter(tp, 0x1021, 1, REASON_COST)
  end
  e:GetHandler():RemoveCounter(tp, 0x1021, 1, REASON_COST)
end
function c4694209.addct2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and chkc:IsCanAddCounter(0x1021, 1)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsCanAddCounter, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler(), 0x1021, 1)
  end
...
function c4694209.addc2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) then
    ...
    if tc:GetFlagEffect(4694209) ~= 0 then
      return
    end
    local e1 = Effect.CreateEffect(c)
...
function c4694209.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsReason(REASON_RULE) and e:GetHandler():GetCounter(0x1021) > 0
  end
  return true
end
