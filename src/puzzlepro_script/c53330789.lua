...
function c53330789.spfilter(c, e, tp, attr)
  return c:IsSetCard(0x195) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:GetOriginalAttribute() ~= attr
end
function c53330789.filter(c, e, tp)
  return c:IsFaceup() and c:IsSetCard(0x195) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c53330789.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetOriginalAttribute())
end
function c53330789.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c53330789.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingTarget(c53330789.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
