...
function c13364097.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, xg) and g:CheckSubGroup(aux.dncheck, 2, 2)
  end
...
function c13364097.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sg then
    ...
    if Duel.SendtoDeck(sg, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and sg:IsExists(Card.IsLocation, 1, nil, LOCATION_DECK + LOCATION_EXTRA) and tc:IsRelateToEffect(e) then
...
function c13364097.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c13364097.lvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13364097.lvfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
