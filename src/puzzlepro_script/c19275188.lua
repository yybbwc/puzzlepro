...
function c19275188.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c19275188.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c19275188.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.GetTurnPlayer() == 1 - tp then
