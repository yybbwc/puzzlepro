...
function c65357623.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc, race)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c65357623.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c65357623.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c65357623.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() then
    local atk = 1000
    if Duel.SelectOption(tp, aux.Stringid(65357623, 1), aux.Stringid(65357623, 2)) == 1 then
      atk = -1000
    end
...
function c65357623.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsSSetable() and Duel.IsExistingMatchingCard(c65357623.retfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
