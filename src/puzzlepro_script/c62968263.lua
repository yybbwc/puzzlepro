...
function c62968263.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE)
  end
...
function c62968263.spfilter(c, e, tp)
  return c:IsCode(93717133) and (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or (Duel.IsExistingMatchingCard(c62968263.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and c:IsCanOverlay()))
end
...
function c62968263.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c62968263.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
  local bpchk = 0
...
function c62968263.spop2(e, tp, eg, ep, ev, re, r, rp)
  ...
  local res = 0
  if tc then
    if Duel.IsExistingMatchingCard(c62968263.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and tc:IsCanOverlay() and (not tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or Duel.SelectOption(tp, 1152, aux.Stringid(62968263, 2)) == 1) then
