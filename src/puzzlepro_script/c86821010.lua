...
function c86821010.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c86821010.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86821010.filter, tp, LOCATION_SZONE + LOCATION_GRAVE, 0, 1, nil)
  end
