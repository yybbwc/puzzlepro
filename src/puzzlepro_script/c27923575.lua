...
function c27923575.setcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bit.band(r, REASON_DESTROY + REASON_EFFECT) == REASON_DESTROY + REASON_EFFECT and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN)
end
...
function c27923575.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c27923575.setfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c27923575.setfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
