...
function c13923256.tgfilter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_SPELL + TYPE_TRAP) and c:IsType(TYPE_CONTINUOUS) and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(c13923256.thfilter, tp, LOCATION_GRAVE, 0, 1, nil, c:GetCode())
end
...
function c13923256.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c13923256.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13923256.tgfilter, tp, LOCATION_ONFIELD, 0, 1, nil, tp)
  end
