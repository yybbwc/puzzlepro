...
function s.thfilter(c)
  return (c:IsCode(94820406) or aux.IsCodeListed(c, 94820406)) and c:IsAbleToHand() and not c:IsType(TYPE_FUSION)
end
...
function s.damfilter(c, tp)
  if not c:IsPreviousControler(1 - tp) then
    return false
  end
  ...
  if bc:IsRelateToBattle() then
    ...
    return bc:GetPreviousPosition() & POS_FACEUP > 0 and bc:GetPreviousLocation() & LOCATION_MZONE == LOCATION_MZONE and bc:IsPreviousControler(tp) and bc:GetPreviousTypeOnField() & TYPE_MONSTER == TYPE_MONSTER and bc:IsPreviousSetCard(0x8)
  end
end
