...
function c27873305.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c27873305.thfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c27873305.thfilter, tp, LOCATION_ONFIELD, 0, 1, c) and Duel.IsExistingMatchingCard(c27873305.eqfilter, tp, 0, LOCATION_MZONE, 1, c:GetBattleTarget())
  end
...
function c27873305.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if c:IsFaceup() and c:IsRelateToEffect(e) then
      ...
      if not Duel.Equip(tp, ec, c, false) then
        return
      end
      local e1 = Effect.CreateEffect(c)
