...
function c48768179.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanDiscardDeck(1 - tp, 1) or Duel.IsExistingTarget(Card.IsAbleToDeck, tp, 0, LOCATION_MZONE, 1, nil)
  end
  local op = 0
  Duel.Hint(HINT_SELECTMSG, tp, aux.Stringid(48768179, 0))
  if Duel.IsExistingTarget(Card.IsAbleToDeck, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsPlayerCanDiscardDeck(1 - tp, 1) then
