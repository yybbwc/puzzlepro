...
function c46724542.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE and aux.dscon(e, tp, eg, ep, ev, re, r, rp) and e:GetHandler():GetAttackAnnouncedCount() < 1
end
...
function c46724542.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c46724542.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46724542.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
