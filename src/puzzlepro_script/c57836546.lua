...
function c57836546.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c57836546.filter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, Duel.GetTurnCount())
  end
...
function c57836546.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() ~= 2 or Duel.GetLocationCount(tp, LOCATION_MZONE) < 2 then
    return
  end
