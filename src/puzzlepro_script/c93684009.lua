...
function c93684009.filter(c, e, tp)
  return c:IsSetCard(0x1019) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE) and not Duel.IsExistingMatchingCard(c93684009.filter1, tp, LOCATION_MZONE, 0, 1, c, c:GetRace())
end
...
function c93684009.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c93684009.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
