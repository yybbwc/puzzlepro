...
function c4836680.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4836680.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4836680.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c4836680.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4836680.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c4836680.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c4836680.tgfilter(c)
  return c:IsSetCard(0x9f, 0x99) and c:IsType(TYPE_PENDULUM) and (c:IsAbleToExtra() or c:IsAbleToGrave())
end
function c4836680.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4836680.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
