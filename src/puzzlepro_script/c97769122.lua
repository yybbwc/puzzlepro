...
function c97769122.cpfilter(c)
  return c:GetType() == TYPE_SPELL and c:IsSetCard(0x175) and c:IsAbleToGraveAsCost() and c:CheckActivateEffect(true, true, false) ~= nil
end
...
function c97769122.matfilter(sg, tp, g)
  if sg:Filter(aux.IsInGroup, nil, g):GetCount() ~= g:GetCount() then
    return false
  end
...
function c97769122.xyztg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonCount(tp, 2) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and mg:CheckSubGroup(c97769122.fselect, 1, ft, tp)
  end
  Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_DECK + LOCATION_EXTRA)
end
function c97769122.xyzop(e, tp, eg, ep, ev, re, r, rp)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if not Duel.IsPlayerCanSpecialSummonCount(tp, 2) or ft <= 0 then
    return
  end
  if ft > 2 then
    ft = 2
  ...
  Duel.AdjustAll()
  if g:FilterCount(Card.IsLocation, nil, LOCATION_MZONE) < #g then
    return
  end
