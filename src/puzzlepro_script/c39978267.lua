...
function c39978267.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    if e:GetLabel() == 1 then
      return chkc:IsLocation(LOCATION_SZONE) and c39978267.desfilter(chkc)
    else
      return chkc:IsLocation(LOCATION_SZONE) and c39978267.eqfilter(chkc, e:GetHandler())
    end
  end
  if chk == 0 then
    return true
  ...
  local sel = 0
  if Duel.IsExistingMatchingCard(c39978267.desfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil) then
    sel = sel + 1
  end
  if Duel.IsExistingMatchingCard(c39978267.eqfilter, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil, e:GetHandler()) then
    sel = sel + 2
  end
  if sel == 3 then
