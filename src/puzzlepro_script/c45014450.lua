...
function c45014450.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c45014450.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45014450.atkfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0
  end
...
function c45014450.copycost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():GetFlagEffect(45014450) == 0
  end
...
function c45014450.copytg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c45014450.copyfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c45014450.copyfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
...
function c45014450.pentg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c45014450.penfilter, tp, LOCATION_PZONE, 0, 1, nil, e, tp)
  end
