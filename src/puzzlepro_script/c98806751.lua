...
function c98806751.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c98806751.spfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98806751.spfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil, tp) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c98806751.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c98806751.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c98806751.thfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, c) and c:IsAbleToHand()
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
