...
function c50213848.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c50213848.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c50213848.filter, tp, LOCATION_GRAVE, 0, 3, nil, e, tp)
  end
...
function c50213848.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() == 0 or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 then
    return
  end
