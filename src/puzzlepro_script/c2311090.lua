...
function c2311090.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c2311090.filter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c2311090.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c2311090.cfilter, 1, e:GetHandler())
  end
...
function c2311090.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.NegateEffect(ev) and rc:IsRelateToEffect(re) and rc:IsDestructable() and (e:GetLabelObject():IsRank(2) or e:GetLabelObject():IsLink(2)) and Duel.SelectYesNo(tp, aux.Stringid(2311090, 2)) then
    Duel.BreakEffect()
