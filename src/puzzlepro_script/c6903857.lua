...
function c6903857.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsPreviousControler(tp) and c:IsReason(REASON_DESTROY) and (c:IsReason(REASON_DESTROY) and rp == 1 - tp or c:IsReason(REASON_BATTLE) and c == Duel.GetAttackTarget())
end
...
function c6903857.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c6903857.dfilter(chkc)
  end
  if chk == 0 then
    return true
  end
