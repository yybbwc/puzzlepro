...
function c24799107.spfilter(c, e, tp)
  return c:IsLevelBelow(9) and c:IsSetCard(0x133) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c24799107.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c24799107.spfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c24799107.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToExtra() and Duel.IsExistingMatchingCard(c24799107.cfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c)
  end
