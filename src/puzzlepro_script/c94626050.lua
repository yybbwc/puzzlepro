...
function c94626050.cfilter(c)
  return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEUP) and c:IsRace(RACE_FISH + RACE_SEASERPENT + RACE_AQUA)
end
...
function c94626050.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c94626050.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
