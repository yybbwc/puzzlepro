...
function c37629703.stcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(Card.IsDiscardable, tp, LOCATION_HAND, 0, 1, c)
  end
...
function c37629703.sttg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c37629703.stfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil)
  end
end
...
function c37629703.thcon(e, tp, eg, ep, ev, re, r, rp)
  return re and rp == tp and re:IsActiveType(TYPE_TRAP) and re:GetHandler():GetOriginalType() == TYPE_TRAP and eg:IsExists(c37629703.cfilter, 1, nil) and not eg:IsContains(e:GetHandler())
end
