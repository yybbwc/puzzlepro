...
function c14604710.target1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c14604710.spfilter1, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c14604710.tgfilter2(c, e, tp)
  if c:IsFacedown() or not c:IsLevel(9) then
    return false
  end
...
function c14604710.spfilter2(c, e, tp, tc)
  return c:IsLevel(9) and c:GetOriginalRace() ~= tc:GetOriginalRace() and c:GetOriginalAttribute() ~= tc:GetOriginalAttribute() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c14604710.target2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c14604710.tgfilter2(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c14604710.tgfilter2, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c14604710.activate2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerAffectedByEffect(tp, 59822133) and ft > 1 and g:GetClassCount(Card.GetCode) > 1 and tc:IsRelateToEffect(e) and tc:IsFaceup() then
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
