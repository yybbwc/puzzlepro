...
function c9061682.operation(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFieldGroupCount(tp, 0, LOCATION_HAND) > 0 and Duel.IsChainDisablable(0) and Duel.SelectYesNo(1 - tp, aux.Stringid(9061682, 1)) then
...
function c9061682.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and c9061682.filter(chkc, e, tp, eg)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c9061682.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp, eg)
  end
