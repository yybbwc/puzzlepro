...
function c6917479.ctfilter2(c)
  local tp = c:GetControler()
  return c:IsFaceup() and c:IsSetCard(0x8e) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
function c6917479.cttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c6917479.ctfilter1, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c6917479.ctfilter2, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c6917479.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c6917479.spfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp, e:GetHandler(), Duel.GetTurnCount())
  end
