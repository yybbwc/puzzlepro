...
function c10497636.atkcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFlagEffect(tp, 10497636) > 0 and (Duel.GetCurrentPhase() >= PHASE_BATTLE_START and Duel.GetCurrentPhase() <= PHASE_BATTLE) and aux.dscon(e, tp, eg, ep, ev, re, r, rp)
end
...
function c10497636.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c10497636.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
