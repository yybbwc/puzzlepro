...
function c22134079.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and chkc:IsAbleToDeck()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToDeck, tp, 0, LOCATION_GRAVE, 1, nil)
  end
...
function c22134079.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c22134079.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22134079.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
