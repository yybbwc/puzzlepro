...
function c83518674.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoGrave(tc, REASON_DISCARD + REASON_EFFECT) ~= 0 and not tc:IsCode(83518674) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
...
function c83518674.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c83518674.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
