...
function c59560625.target1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return ag:IsExists(c59560625.filter1, 1, at, e)
  end
...
function c59560625.condition2(e, tp, eg, ep, ev, re, r, rp)
  if rp == tp or not re:IsHasType(EFFECT_TYPE_ACTIVATE) or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then
    return false
  end
...
function c59560625.target2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc ~= e:GetLabelObject() and chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c59560625.filter2(chkc, ev)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c59560625.filter2, tp, LOCATION_MZONE, 0, 1, e:GetLabelObject(), ev)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
