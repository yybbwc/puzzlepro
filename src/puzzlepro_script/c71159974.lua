...
function c71159974.ckfilter(c, tp)
  return (c:IsType(TYPE_NORMAL) or c:IsSetCard(0x165) and c:IsType(TYPE_MONSTER)) and Duel.IsExistingMatchingCard(c71159974.desfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttribute())
end
...
function c71159974.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c71159974.ckfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c71159974.ckfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c71159974.drfilter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousControler() == tp and Duel.IsExistingMatchingCard(Card.IsAttribute, tp, 0, LOCATION_GRAVE, 1, nil, c:GetAttribute())
end
