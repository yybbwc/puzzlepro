...
function c53782828.matval(e, lc, mg, c, tp)
  if not lc:IsSetCard(0x156) then
    return false, nil
  end
...
function c53782828.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetMZoneCount(tp, c) > 0 and c:IsAbleToHand() and Duel.IsExistingMatchingCard(c53782828.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c53782828.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoHand(c, nil, REASON_EFFECT) > 0 and c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
