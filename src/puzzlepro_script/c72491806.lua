...
function c72491806.spfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0xb2) and not c:IsCode(72491806) and c:IsAbleToHandAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
...
function c72491806.thfilter(c, e, tp, ft)
  return c:IsFaceup() and c:IsSetCard(0xb2) and c:IsAbleToHand() and (ft > 0 or c:GetSequence() < 5) and Duel.IsExistingMatchingCard(c72491806.spfilter2, tp, LOCATION_HAND, 0, 1, nil, e, tp, c:GetCode())
end
...
function c72491806.tstg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c72491806.thfilter(chkc, e, tp, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c72491806.thfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler(), e, tp, ft)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_RTOHAND)
