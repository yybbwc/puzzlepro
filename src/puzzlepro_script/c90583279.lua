...
function c90583279.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c90583279.spfilter, tp, LOCATION_GRAVE, 0, 1, nil)
end
...
function c90583279.tdfilter(c, tp)
  return c:IsAbleToDeck() and ((c:IsType(TYPE_RITUAL + TYPE_FUSION + TYPE_SYNCHRO) and c:IsLevel(8)) or (c:IsType(TYPE_XYZ) and c:IsRank(8))) and (Duel.IsPlayerCanDraw(tp, 1) or c:IsType(TYPE_SYNCHRO + TYPE_XYZ))
end
function c90583279.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c90583279.tdfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c90583279.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
...
function c90583279.tdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and ((tc:IsType(TYPE_RITUAL) and tc:IsLocation(LOCATION_DECK)) or (tc:IsType(TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ) and tc:IsLocation(LOCATION_EXTRA))) then
    if tc:IsType(TYPE_RITUAL + TYPE_FUSION) then
      Duel.BreakEffect()
      Duel.ShuffleDeck(tp)
      Duel.Draw(tp, 1, REASON_EFFECT)
    end
    if tc:IsType(TYPE_SYNCHRO + TYPE_XYZ) and c:IsFaceup() and c:IsRelateToEffect(e) then
      Duel.BreakEffect()
      local e1 = Effect.CreateEffect(c)
