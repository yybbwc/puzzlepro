...
function c27491571.postg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(27491571) == 0
  end
...
function c27491571.postg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c27491571.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c27491571.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
