...
function c41620959.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41620959.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c41620959.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = g:GetFirst()
  if tc and Duel.SendtoGrave(tc, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_GRAVE) and tc:IsRace(RACE_DRAGON) and tc:IsType(TYPE_NORMAL) and Duel.IsExistingMatchingCard(c41620959.tgfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(41620959, 0)) then
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
