...
function c96381979.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96381979.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c96381979.distg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96381979.dfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
end
...
function c96381979.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96381979.cfilter, tp, LOCATION_ONFIELD, 0, 3, nil) or Duel.IsPlayerAffectedByEffect(tp, 46241344)
  end
  if Duel.IsExistingMatchingCard(c96381979.cfilter, tp, LOCATION_ONFIELD, 0, 3, nil) and (not Duel.IsPlayerAffectedByEffect(tp, 46241344) or not Duel.SelectYesNo(tp, aux.Stringid(46241344, 0))) then
...
function c96381979.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and g:IsExists(c96381979.afilter1, 1, nil, g)
  end
