...
function c9238125.drop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(p, d, REASON_EFFECT) > 0 then
    ...
    if sg:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(9238125, 0)) then
      Duel.BreakEffect()
...
function c9238125.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c9238125.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c9238125.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
