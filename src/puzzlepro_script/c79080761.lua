...
function c79080761.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c79080761.spfilter, tp, LOCATION_GRAVE, 0, 2, nil)
end
...
function c79080761.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousLocation(LOCATION_ONFIELD)
end
