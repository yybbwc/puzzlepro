...
function c63056220.srtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63056220.srfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c63056220.rsop(e, tp, eg, ep, ev, re, r, rp)
  ...
  ::cancel::
  local mg = Duel.GetRitualMaterial(tp)
  if c:IsControler(1 - tp) or not c:IsRelateToEffect(e) or not mg:IsContains(c) then
    return
  end
