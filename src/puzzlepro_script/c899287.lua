...
function c899287.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c899287.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c899287.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c899287.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c899287.desfilter, tp, 0, LOCATION_SZONE, 1, nil)
  end
