...
function c22555834.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c22555834.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c22555834.thfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(nil, tp, LOCATION_HAND, 0, 2, nil)
  end
...
function c22555834.atkcon(e)
  ...
  return Duel.GetCurrentPhase() == PHASE_DAMAGE_CAL and Duel.GetAttackTarget() ~= nil and Duel.GetFieldGroupCount(tp, 0, LOCATION_HAND) > Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0)
end
