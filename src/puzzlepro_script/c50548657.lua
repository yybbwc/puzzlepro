...
function c50548657.costfilter(c, e, tp, g, ft)
  local lv = c:GetLevel()
  return c:IsLevelBelow(4) and c:IsRace(RACE_CYBERSE) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup()) and g:CheckWithSumEqual(Card.GetLevel, lv, 1, ft + 1)
end
...
function c50548657.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c50548657.costfilter, 1, nil, e, tp, g, ft)
  end
