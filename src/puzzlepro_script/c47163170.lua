...
function c47163170.spfilter(c, e, tp)
  return c:IsFaceup() and c:IsLevelBelow(4) and c:IsRace(RACE_BEAST + RACE_BEASTWARRIOR + RACE_WINDBEAST) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c47163170.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and c47163170.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c47163170.spfilter, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c47163170.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c47163170.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
