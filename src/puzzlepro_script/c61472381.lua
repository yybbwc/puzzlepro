...
function c61472381.setcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c61472381.costfilter1, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c61472381.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c61472381.setfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
end
...
function c61472381.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c61472381.costfilter2, tp, LOCATION_ONFIELD, 0, 1, nil) or Duel.IsPlayerAffectedByEffect(tp, 46241344)
  end
  if Duel.IsExistingMatchingCard(c61472381.costfilter2, tp, LOCATION_ONFIELD, 0, 1, nil) and (not Duel.IsPlayerAffectedByEffect(tp, 46241344) or not Duel.SelectYesNo(tp, aux.Stringid(46241344, 0))) then
