...
function c29284413.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29284413.cfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c29284413.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c29284413.tdfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29284413.tdfilter, tp, LOCATION_GRAVE, 0, 1, c) and c:IsAbleToHand()
  end
