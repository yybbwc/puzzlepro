...
function c29884951.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c29884951.rfilter, 1, nil, tp)
  end
...
function c29884951.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return a and d and a:IsFaceup() and a:IsRelateToBattle() and a:IsRace(RACE_WYRM) and d:IsFaceup() and d:IsRelateToBattle() and d:IsSummonLocation(LOCATION_EXTRA)
end
...
function c29884951.remtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD + LOCATION_GRAVE) and c29884951.remfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c29884951.remfilter, tp, LOCATION_ONFIELD + LOCATION_GRAVE, LOCATION_ONFIELD + LOCATION_GRAVE, 1, nil)
  end
