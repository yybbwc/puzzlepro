...
function c879958.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c879958.filter(chkc)
  end
  if chk == 0 then
    return e:IsCostChecked() and Duel.IsExistingTarget(c879958.filter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c879958.eqlimit(e, c)
  return e:GetHandler():GetEquipTarget() == c or c:IsControler(e:GetHandlerPlayer()) and c:IsType(TYPE_LINK)
end
...
function c879958.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c879958.cfilter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
...
function c879958.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c879958.atkfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c879958.atkfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
