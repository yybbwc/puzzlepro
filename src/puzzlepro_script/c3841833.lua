...
function c3841833.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToGraveAsCost() and e:GetHandler():IsDiscardable()
  end
...
function c3841833.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c3841833.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c3841833.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c3841833.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c3841833.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
