...
function c35514096.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c35514096.filter(chkc)
  end
  if chk == 0 then
    return true
  end
  ...
  local t3 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 35514097, 0, TYPES_TOKEN_MONSTER, 1200, 1200, 4, RACE_MACHINE, ATTRIBUTE_LIGHT)
  local op = 0
  if t1 or t2 or t3 then
    ...
    local ct = 1
    if t1 then
      m[ct] = aux.Stringid(35514096, 1)
      n[ct] = 1
      ct = ct + 1
    end
    if t2 then
      m[ct] = aux.Stringid(35514096, 2)
      n[ct] = 2
      ct = ct + 1
    end
    if t3 then
      m[ct] = aux.Stringid(35514096, 3)
      n[ct] = 3
      ct = ct + 1
    end
...
function c35514096.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  elseif e:GetLabel() == 3 then
    if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 35514097, 0, TYPES_TOKEN_MONSTER, 1200, 1200, 4, RACE_MACHINE, ATTRIBUTE_LIGHT) then
      return
    end
