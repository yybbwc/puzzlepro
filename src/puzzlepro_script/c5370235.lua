...
function c5370235.initial_effect(c)
  ...
  if not aux.fus_mat_hack_check then
    ...
    function Duel.GetFusionMaterial(tp, loc)
      if loc == nil then
        loc = LOCATION_HAND + LOCATION_MZONE
      end
...
function c5370235.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5370235.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c5370235.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5370235.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c5370235.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5370235.tgfilter, tp, LOCATION_DECK, 0, 1, nil, tp)
  end
