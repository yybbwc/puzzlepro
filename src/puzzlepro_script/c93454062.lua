...
function c93454062.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c93454062.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c93454062.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft >= 2 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and tg and tg:IsExists(Card.IsControler, 1, nil, 1 - tp) then
    ft = 2
  else
    ft = 1
  end
...
function c93454062.cfilter(c, tp, se)
  return (c:IsSummonPlayer(1 - tp) or c:IsSetCard(0x2a)) and c:IsSummonLocation(LOCATION_EXTRA) and (se == nil or c:GetReasonEffect() ~= se)
end
