...
function c42143067.cfilter(c, e, tp)
  return c:IsRace(RACE_ROCK) and c:IsDiscardable() and Duel.IsExistingMatchingCard(c42143067.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetOriginalAttribute(), c:GetOriginalLevel())
end
...
function c42143067.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    e:SetLabel(0)
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c42143067.cfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
