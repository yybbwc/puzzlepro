...
function c72813401.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72813401.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil)
  end
...
function c72813401.costfilter(c, g)
  return c:IsAttribute(ATTRIBUTE_WIND) and c:IsType(TYPE_SYNCHRO) and c:IsAbleToRemoveAsCost() and g:IsExists(c72813401.thfilter1, 1, nil, g, c:GetLevel())
end
...
function c72813401.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72813401.costfilter, tp, LOCATION_EXTRA, 0, 1, nil, g)
  end
