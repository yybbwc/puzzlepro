...
function c77406972.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0
end
...
function c77406972.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77406972.rmfilter, tp, 0, LOCATION_MZONE, 1, nil, c:GetAttack())
  end
...
function c77406972.thfilter(c)
  return c:IsAbleToHand() and c:IsType(TYPE_MONSTER) and ((c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevel(7) and c:IsRace(RACE_DRAGON)) or (c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevel(6) and c:IsRace(RACE_MACHINE)) or (c:IsAttribute(ATTRIBUTE_WATER) and c:IsLevel(5) and c:IsRace(RACE_WARRIOR)))
end
function c77406972.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c77406972.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
