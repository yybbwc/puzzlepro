...
function c56132807.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c56132807.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c56132807.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c56132807.thfilter(chkc)
  end
  local c = e:GetHandler()
  if chk == 0 then
    return Duel.IsExistingTarget(c56132807.thfilter, tp, LOCATION_GRAVE, 0, 1, c)
  end
