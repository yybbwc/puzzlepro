...
function c6764709.spcon(e, c)
  ...
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or Duel.IsExistingMatchingCard(c6764709.spfilter1, tp, LOCATION_MZONE, 0, 1, nil) then
    return false
  end
...
function c6764709.rdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and e:GetHandler():IsLevelAbove(4)
  end
...
function c6764709.rdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFacedown() or not c:IsRelateToEffect(e) or c:IsLevelBelow(3) then
    return
  end
  local e1 = Effect.CreateEffect(c)
