...
function c67331360.filter(c, e, tp)
  return c:IsType(TYPE_NORMAL) and (c:IsAttack(0) or c:IsDefense(0)) and c:IsCanBeEffectTarget(e) and Duel.IsExistingMatchingCard(c67331360.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetCode(), nil)
end
...
function c67331360.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c67331360.filter(chkc, e, tp)
  end
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return ft > 0 and Duel.IsExistingTarget(c67331360.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
  local ct = math.min(2, ft)
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) or not Duel.IsExistingMatchingCard(c67331360.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) then
    ct = 1
  end
...
function c67331360.mattg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c67331360.matfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c67331360.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
end
