...
function c54374642.spfilter(c, e, tp)
  return c:IsRace(RACE_CYBERSE) and not c:IsType(TYPE_LINK) and c:IsAttack(2300) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c54374642.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  local tc = Duel.GetAttacker()
  if chk == 0 then
    return tc:IsControler(1 - tp) and tc:IsAttackAbove(2300) and Duel.IsExistingMatchingCard(c54374642.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
...
function c54374642.fselect(g, ft1, ft2, ect, ft, atk)
  return #g <= ft and #g <= ect and g:FilterCount(c54374642.exfilter1, nil) <= ft1 and g:FilterCount(c54374642.exfilter2, nil) <= ft2 and g:GetSum(Card.GetAttack) <= atk
end
function c54374642.operation(e, tp, eg, ep, ev, re, r, rp)
  local tc = Duel.GetFirstTarget()
  if not tc:IsRelateToEffect(e) or tc:IsFacedown() or not tc:IsAttackAbove(2300) then
    return
  end
  ...
  if ect == 0 or ft1 == 0 and ft2 == 0 then
    return
  end
