...
function c15291624.spfilter(c, fc, tp)
  return c:IsRace(RACE_THUNDER) and c:IsFusionType(TYPE_EFFECT) and not c:IsFusionType(TYPE_FUSION) and Duel.GetLocationCountFromEx(tp, tp, c, fc) > 0 and c:IsCanBeFusionMaterial(fc, SUMMON_TYPE_SPECIAL)
end
function c15291624.spcon(e, c)
  ...
  return (Duel.GetCustomActivityCount(15291624, tp, ACTIVITY_CHAIN) ~= 0 or Duel.GetCustomActivityCount(15291624, 1 - tp, ACTIVITY_CHAIN) ~= 0) and Duel.CheckReleaseGroupEx(tp, c15291624.spfilter, 1, REASON_SPSUMMON, false, nil, c, tp)
end
...
function c15291624.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsReason(REASON_BATTLE + REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(c15291624.repfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
