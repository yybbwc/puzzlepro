...
function c37663536.spcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:IsSummonType(SUMMON_TYPE_FUSION) and c:GetReasonPlayer() == 1 - tp and c:IsReason(REASON_EFFECT) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c37663536.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c37663536.spfilter, tp, LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
