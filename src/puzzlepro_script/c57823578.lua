...
function c57823578.fselect(g, tp, mc)
  local sg = g:Clone()
  sg:AddCard(mc)
  return g:GetClassCount(Card.GetOriginalAttribute) == g:GetCount() and Duel.IsExistingMatchingCard(Card.IsAbleToHand, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, sg)
end
function c57823578.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToGraveAsCost() and g:CheckSubGroup(c57823578.fselect, 2, 2, tp, c)
  end
