...
function c56252810.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c56252810.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c56252810.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c56252810.filter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingMatchingCard(aux.TRUE, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c56252810.filter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
