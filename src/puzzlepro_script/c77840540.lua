...
function c77840540.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return Duel.IsExistingMatchingCard(c77840540.cfilter, tp, LOCATION_MZONE, loc, 1, nil) and Duel.IsExistingMatchingCard(c77840540.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c77840540.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c77840540.tdfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c77840540.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 1)
  end
