...
function c36768783.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c36768783.spfilter_chkc(chkc, e, tp, e:GetLabel(), lg)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c36768783.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp, zone, lg)
  end
