    ...
    video::SColor color)
{
    ...
    //! constructor
    CGUIImageButton::CGUIImageButton(IGUIEnvironment *environment, IGUIElement *parent, s32 id, core::rect<s32> rectangle, bool noclip) : IGUIButton(environment, parent, id, rectangle), SpriteBank(0), OverrideFont(0), Image(0), PressedImage(0), IsPushButton(false), Pressed(false), UseAlphaChannel(false), DrawBorder(true), ScaleImage(false) {
#ifdef _DEBUG
    ...
    bool CGUIImageButton::OnEvent(const SEvent &event) {
      ...
      switch (event.EventType) {
        case EET_KEY_INPUT_EVENT:
          ...
          else if (!event.KeyInput.PressedDown && Pressed && (event.KeyInput.Key == KEY_RETURN || event.KeyInput.Key == KEY_SPACE)) {
        ...
        case EET_MOUSE_INPUT_EVENT:
          if (event.MouseInput.Event == EMIE_LMOUSE_PRESSED_DOWN) {
            ...
            if ((!IsPushButton && wasPressed && Parent) || (IsPushButton && wasPressed != Pressed)) {
              SEvent newEvent;
              newEvent.EventType = EET_GUI_EVENT;
              newEvent.GUIEvent.Caller = this;
              newEvent.GUIEvent.Element = 0;
