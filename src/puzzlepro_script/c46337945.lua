...
function c46337945.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c46337945.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c46337945.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c46337945.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c46337945.filter, tp, LOCATION_GRAVE, 0, 2, nil)
  end
