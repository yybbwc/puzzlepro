...
function c45974017.pctg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c45974017.pcfilter(chkc)
  end
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(45974017, 1), aux.Stringid(45974017, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(45974017, 1))
  else
    op = Duel.SelectOption(tp, aux.Stringid(45974017, 2)) + 1
  end
  e:SetLabel(op)
  if op == 0 then
