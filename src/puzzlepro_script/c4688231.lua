...
function c4688231.rettg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c4688231.retfilter1, tp, LOCATION_GRAVE, 0, 2, nil) and Duel.IsExistingTarget(c4688231.retfilter2, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
...
function c4688231.spfilter(c, e, tp)
  return c:IsSetCard(0xe1) and c:IsType(TYPE_PENDULUM) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_GRAVE) and Duel.GetMZoneCount(tp) > 0 or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0)
end
function c4688231.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4688231.spfilter, tp, LOCATION_GRAVE + LOCATION_EXTRA, 0, 1, nil, e, tp)
  end
