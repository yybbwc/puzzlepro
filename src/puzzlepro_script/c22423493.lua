...
function c22423493.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.NegateAnyFilter, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler()) and Duel.IsExistingTarget(aux.NegateAnyFilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
...
function c22423493.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c22423493.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
