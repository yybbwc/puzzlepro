...
function c49816630.tetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c49816630.tefilter, tp, LOCATION_GRAVE, 0, 3, e:GetHandler())
  end
...
function c49816630.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c49816630.eqfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c49816630.eqfilter, tp, LOCATION_MZONE, 0, 1, nil) and c:CheckUniqueOnField(tp, LOCATION_SZONE) and not c:IsForbidden()
  end
...
function c49816630.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:CheckUniqueOnField(tp, LOCATION_SZONE) or c:IsForbidden() then
    return
  end
