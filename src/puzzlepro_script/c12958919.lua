...
function c12958919.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 12958920, 0x49, TYPES_TOKEN_MONSTER, 500, 500, 4, RACE_FIEND, ATTRIBUTE_DARK)
  end
...
function c12958919.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 12958920, 0x49, TYPES_TOKEN_MONSTER, 500, 500, 4, RACE_FIEND, ATTRIBUTE_DARK) then
    return
  end
  local ctn = true
  while ft > 0 and ctn do
    ...
    ft = ft - 1
    if ft <= 0 or not Duel.SelectYesNo(tp, aux.Stringid(12958919, 2)) then
      ctn = false
    end
  end
...
function c12958919.damcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 12958919) == 0
  end
