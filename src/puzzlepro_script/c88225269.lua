...
function s.discon(e, tp, eg, ep, ev, re, r, rp)
  return rp == 1 - tp and Duel.IsChainDisablable(ev) and Duel.IsExistingMatchingCard(s.tfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function s.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and c:IsSetCard(0x1047) and g:GetCount() >= 2
  end
  local sg
  if #g == 2 then
    sg = g
