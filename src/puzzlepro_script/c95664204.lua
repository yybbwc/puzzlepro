...
function c95664204.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c95664204.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c95664204.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sg:GetCount() > 0 and Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.IsExistingMatchingCard(c95664204.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c95664204.lvfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(95664204, 2)) then
    Duel.BreakEffect()
    local g = Duel.GetMatchingGroup(c95664204.lvfilter, tp, LOCATION_MZONE, 0, nil)
    local lv = 0
    if g:FilterCount(Card.IsLevel, nil, 5) == #g then
      lv = 4
    end
    if g:FilterCount(Card.IsLevel, nil, 4) == #g then
      lv = 5
    end
    if lv == 0 then
      lv = Duel.AnnounceNumber(tp, 4, 5)
    end
    local lc = g:GetFirst()
    while lc do
...
function c95664204.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c95664204.costfilter, tp, LOCATION_GRAVE, 0, 1, c)
  end
