...
function c41933425.cfilter1(c, e, tp)
  return c:IsSetCard(0x1f) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c41933425.cfilter2, tp, LOCATION_GRAVE, 0, 1, c, e, tp, c)
end
...
function c41933425.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41933425.cfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c41933425.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and g:GetClassCount(Card.GetCode) > 1
  end
...
function c41933425.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.IsPlayerAffectedByEffect(tp, 59822133) or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 1 then
    return
  end
...
function c41933425.confilter(c, tp)
  return c:IsType(TYPE_FUSION) and aux.IsMaterialListCode(c, 89943723) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:IsLocation(LOCATION_EXTRA)
end
...
function c41933425.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c41933425.spfilter2, tp, LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
