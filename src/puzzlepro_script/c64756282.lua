...
function c64756282.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(c64756282.costfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil, tp)
  end
...
function c64756282.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c64756282.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c64756282.cpfilter(c, exc, e, tp, eg, ep, ev, re, r, rp)
  ...
  if not (c:IsSetCard(0x128) and c:IsType(TYPE_SPELL) and c:IsAbleToRemoveAsCost() and te and te:GetOperation()) then
    return false
  end
...
function c64756282.cpcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c64756282.cpfilter, tp, LOCATION_GRAVE, 0, 1, nil, c, e, tp, eg, ep, ev, re, r, rp)
  end
