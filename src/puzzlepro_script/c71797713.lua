...
function c71797713.costfilter(c)
  return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost() and (c:IsSetCard(0x400d) or c:IsLocation(LOCATION_HAND))
end
...
function c71797713.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c71797713.costfilter, tp, loc, 0, 2, e:GetHandler())
  end
  ...
  local flag = 0
  if g:IsExists(c71797713.regfilter, 1, nil, ATTRIBUTE_EARTH + ATTRIBUTE_WIND) then
    flag = bit.bor(flag, 0x1)
  end
  if g:IsExists(c71797713.regfilter, 1, nil, ATTRIBUTE_WATER + ATTRIBUTE_FIRE) then
    flag = bit.bor(flag, 0x2)
  end
  if g:IsExists(c71797713.regfilter, 1, nil, ATTRIBUTE_LIGHT + ATTRIBUTE_DARK) then
    flag = bit.bor(flag, 0x4)
  end
