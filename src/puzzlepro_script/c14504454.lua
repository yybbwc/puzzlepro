...
function c14504454.sscost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c14504454.costfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c14504454.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c14504454.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c14504454.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c14504454.thfilter(c)
  return c:IsRace(RACE_FIEND) and not c:IsCode(14504454) and c:IsAbleToHand() and c:IsAttackAbove(0) and c:IsDefenseAbove(0) and c:GetAttack() + c:GetDefense() == 2000
end
function c14504454.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c14504454.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
