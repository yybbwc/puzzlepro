...
function c6374519.thfilter(c, tp, e)
  return c:IsFaceup() and c:IsSetCard(0x15b) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsCanBeEffectTarget(e) and c:IsAbleToHand()
end
...
function c6374519.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return eg:IsContains(chkc) and c6374519.thfilter(chkc, tp, e)
  end
  if chk == 0 then
    return true
  end
