...
function c39528955.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c39528955.seqfilter(chkc) and chkc ~= c
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c39528955.seqfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, c)
  end
...
function c39528955.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or tc:IsImmuneToEffect(e) or Duel.GetLocationCount(ttp, LOCATION_MZONE, PLAYER_NONE, 0) <= 0 then
    return
  end
  local p1, p2
