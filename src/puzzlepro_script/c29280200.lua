...
function c29280200.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29280200.thfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.GetFlagEffect(tp, 29280200) == 0
  end
...
function c29280200.adcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c29280200.adcfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c29280200.adtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil) and Duel.GetFlagEffect(tp, 29280200) == 0
  end
  Duel.RegisterFlagEffect(tp, 29280200, RESET_CHAIN, 0, 1)
end
...
function c29280200.rthtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return tg:IsExists(Card.IsAbleToHand, 1, nil) and Duel.GetFlagEffect(tp, 29280200) == 0
  end
  Duel.RegisterFlagEffect(tp, 29280200, RESET_CHAIN, 0, 1)
