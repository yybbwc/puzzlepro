...
function c50237654.drop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Draw(tp, 1, REASON_EFFECT) ~= 0 then
    ...
    if dc:IsType(TYPE_SPELL + TYPE_TRAP) and dc:IsSSetable() and Duel.SelectYesNo(tp, aux.Stringid(50237654, 0)) then
      Duel.BreakEffect()
      if Duel.SSet(tp, dc, tp, false) == 0 then
        return
      end
...
function c50237654.spfilter1(c, e, tp)
  return c:IsCode(46986414) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c50237654.spfilter2, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, c, e, tp)
end
...
function c50237654.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c50237654.spfilter1, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and not Duel.IsPlayerAffectedByEffect(tp, 59822133)
  end
