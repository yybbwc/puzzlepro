...
function c68815401.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 500) and Duel.GetActivityCount(tp, ACTIVITY_ATTACK) == 0
  end
...
function c68815401.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c68815401.filter1, tp, LOCATION_SZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(aux.TRUE, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil) and Duel.IsPlayerCanSpecialSummonMonster(tp, 68815402, 0, TYPES_TOKEN_MONSTER, 1000, 1000, 3, RACE_PYRO, ATTRIBUTE_FIRE, POS_FACEUP_ATTACK)
  end
...
function c68815401.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.Destroy(dg1, REASON_EFFECT) > 0 then
    local dg2 = Duel.GetMatchingGroup(aux.TRUE, tp, LOCATION_MZONE, LOCATION_MZONE, nil)
    if Duel.Destroy(dg2, REASON_EFFECT) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 68815402, 0, TYPES_TOKEN_MONSTER, 1000, 1000, 3, RACE_PYRO, ATTRIBUTE_FIRE, POS_FACEUP_ATTACK) then
      Duel.BreakEffect()
