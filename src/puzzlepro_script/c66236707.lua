...
function s.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetTurnPlayer() == tp and Duel.IsMainPhase() and not Duel.CheckPhaseActivity()
end
...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and s.tgfilter(chkc)
  end
  ...
  if chk == 0 then
    return ct > 0 and Duel.IsExistingTarget(s.tgfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
