...
function c31123642.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c31123642.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c31123642.repfilter(c, tp)
  return c:IsFaceup() and c:IsSetCard(0x7f) and c:IsType(TYPE_XYZ) and c:GetOriginalAttribute() == ATTRIBUTE_LIGHT and not c:IsCode(31123642) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE)
end
function c31123642.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsAbleToRemove() and not c:IsStatus(STATUS_DESTROY_CONFIRMED) and eg:IsExists(c31123642.repfilter, 1, nil, tp)
  end
