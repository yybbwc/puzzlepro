...
function c90247311.cfilter(c)
  return ((c:IsType(TYPE_MONSTER) and c:IsSetCard(0x2b)) or c:IsSetCard(0x61)) and (c:IsFaceup() or not c:IsOnField()) and c:IsAbleToGraveAsCost()
end
...
function c90247311.costfilter(c, rg, dg)
  if not (c:IsType(TYPE_MONSTER) and c:IsSetCard(0x2b)) then
    return false
  end
  local a = 0
  if dg:IsContains(c) then
    a = 1
  end
...
function c90247311.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c90247311.filter(chkc)
  end
  if chk == 0 then
