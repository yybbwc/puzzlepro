...
function c9287078.filter1(c, e, tp)
  return c:IsSummonPlayer(1 - tp) and c:IsLocation(LOCATION_MZONE) and c:IsCanBeEffectTarget(e) and Duel.IsExistingTarget(c9287078.filter2, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c9287078.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return eg:IsExists(c9287078.filter1, 1, nil, e, tp)
  end
