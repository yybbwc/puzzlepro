...
function c8836329.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c8836329.thfilter(chkc)
  end
  if chk == 0 then
    return c:IsAbleToHand() and Duel.IsExistingTarget(c8836329.thfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
