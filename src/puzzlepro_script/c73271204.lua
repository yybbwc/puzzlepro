...
function c73271204.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c73271204.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c73271204.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c73271204.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if tg:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(73271204, 2)) then
      Duel.BreakEffect()
