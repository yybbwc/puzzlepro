...
function c61269611.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c61269611.costfilter1, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c61269611.costfilter2(c, e, tp)
  return c:IsRace(RACE_DINOSAUR) and Duel.GetMZoneCount(tp, c) > 0 and Duel.IsExistingTarget(c61269611.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp, c)
end
...
function c61269611.spcost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c61269611.costfilter2, 1, nil, e, tp)
  end
...
function c61269611.sptg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c61269611.spfilter(chkc, e, tp, mc)
  end
  if chk == 0 then
    return true
  end
