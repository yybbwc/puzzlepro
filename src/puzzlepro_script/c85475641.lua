...
function c85475641.filter(c, tp)
  return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE) and c:GetLevel() > 0 and c:IsControler(tp) and c:IsPreviousControler(tp) and not c:IsCode(85475641)
end
...
function c85475641.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) ~= 0 or not c:IsRelateToEffect(e) then
    return
  end
