...
function c2434862.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c2434862.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c2434862.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c2434862.filter, tp, LOCATION_HAND + LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c2434862.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabel() == 1 and sg:GetCount() > 0 and ft > 0 and Duel.SelectYesNo(tp, aux.Stringid(2434862, 2)) then
    Duel.BreakEffect()
