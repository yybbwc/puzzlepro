...
function c21639276.spfilter(c, e, tp)
  return c:IsSetCard(0x189) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
end
function c21639276.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c21639276.spfilter, tp, LOCATION_HAND + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
...
function c21639276.rmcon(e, tp, eg, ep, ev, re, r, rp)
  return rp == 1 - tp and re:IsActiveType(TYPE_TRAP) and e:GetHandler():IsStatus(STATUS_EFFECT_ENABLED) and Duel.IsExistingMatchingCard(c21639276.rmcfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
