...
namespace irr {
  namespace gui {
    ...
    struct SGlyphPixelSpan {
      ...
      SGlyphPixelSpan(int _x, int _y, int _width, int _coverage) : X(_x), Y(_y), Width(_width), Coverage(_coverage) {
      }
    ...
    struct SGUITTGlyph {
      ...
      bool IsLoaded() const {
        return TopLayer.GlyphPage || OutlineLayer.GlyphPage;
      }
    ...
    private:
      struct PagedGlyphTextures {
        PagedGlyphTextures(video::IImage *surface, const irr::core::recti &pageRect) : Surface(surface), PageRect(pageRect) {
          if (Surface)
            Surface->grab();
        }
    ...
    public:
      CGUITTGlyphPage(video::IVideoDriver *driver, const io::path &textureName) : Texture(0), Driver(driver), Name(textureName) {
      }
    ...
    public:
      ...
      bool getAutoHinting() const {
        return UseAutoHinting;
      }
      ...
      void setOutlineColor(video::SColor color) {
        OutlineColor = color;
      }

      video::SColor getOutlineColor() const {
        return OutlineColor;
      }
      ...
      irr::u32 getNumGlyphPages() const {
        return GlyphPages.size();
      }
