...
function c61399402.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c61399402.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c61399402.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c61399402.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c61399402.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return eg:IsExists(c61399402.repfilter, 1, nil, tp) and e:GetHandler():CheckRemoveOverlayCard(tp, 1, REASON_EFFECT)
  end
