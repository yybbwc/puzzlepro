...
function c96570609.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    ...
    return g:GetClassCount(Card.GetCode) > 1 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c96570609.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c96570609.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoGrave(tg1, REASON_EFFECT) ~= 0 and tg1:IsExists(Card.IsLocation, 2, nil, LOCATION_GRAVE) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 then
...
function c96570609.sumcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96570609.cfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c96570609.sumop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsMSetable(true, nil, 1) then
