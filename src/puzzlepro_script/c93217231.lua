...
function c93217231.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c93217231.filter1, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(aux.TRUE, tp, 0, LOCATION_ONFIELD, 2, nil)
  end
...
function c93217231.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sg:GetCount() ~= 3 or sc:IsFacedown() or not sc:IsRace(RACE_REPTILE) or sc:IsControler(1 - tp) then
    return
  end
