...
function c50766506.cfilter(c, e, tp, ft)
  local lv = c:GetLevel()
  return lv > 0 and c:IsSetCard(0x2b) and (ft > 0 or (c:IsControler(tp) and c:GetSequence() < 5)) and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c50766506.spfilter, tp, LOCATION_DECK, 0, 1, nil, lv, e, tp)
end
...
function c50766506.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ft > -1 and Duel.CheckReleaseGroup(tp, c50766506.cfilter, 1, nil, e, tp, ft)
  end
