...
function c93394164.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c93394164.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93394164.filter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c93394164.seqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not ec or ec:IsImmuneToEffect(e) then
    return
  end
  local p = ec:GetControler()
  local zone = bit.band(ec:GetLinkedZone(), 0x1f)
