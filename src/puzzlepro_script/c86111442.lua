...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and s.dfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.dfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or Duel.Destroy(tc, REASON_EFFECT) < 1 then
    return
  end
