...
function c80627281.spfilter1(c, e, tp)
  local lk = c:GetLink()
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsPreviousSetCard(0x12b) and bit.band(c:GetPreviousTypeOnField(), TYPE_LINK) ~= 0 and c:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and c:IsCanBeEffectTarget(e) and lk > 0 and Duel.IsExistingMatchingCard(c80627281.spfilter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, lk)
end
function c80627281.spfilter2(c, e, tp, lk)
  return c:IsSetCard(0x12b) and c:IsType(TYPE_LINK) and c:GetLink() < lk and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_LINK, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, nil, c) > 0
end
function c80627281.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c80627281.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_LMATERIAL) and eg:IsExists(c80627281.spfilter1, 1, nil, e, tp)
  end
...
function c80627281.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsRelateToEffect(e) or not aux.MustMaterialCheck(nil, tp, EFFECT_MUST_BE_LMATERIAL) then
    return
  end
