...
function c9726840.spfilter1(c, e, tp, mc)
  return c:IsSetCard(0x1115) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c, 0x60) > 0
end
function c9726840.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c9726840.tgfilter1, tp, LOCATION_ONFIELD, 0, 1, e:GetHandler(), e, tp)
  end
