...
function c74852810.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c74852810.filter1, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c74852810.filter2, tp, LOCATION_MZONE, 0, 1, nil) and e:GetHandler():IsCanOverlay()
  end
...
function c74852810.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc1:IsControler(1 - tp) or not tc1:IsRelateToEffect(e) then
    return
  end
