...
function s.filter2(c, e, tp, mc)
  return c:IsSetCard(0xac) and mc:IsCanBeXyzMaterial(c) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_XYZ, tp, false, false) and Duel.GetLocationCountFromEx(tp, tp, mc, c) > 0
end
function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return e:GetHandler():IsCanOverlay() and Duel.IsExistingTarget(s.filter, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function s.mattg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.mfilter, tp, LOCATION_MZONE, 0, 1, nil, e, tp) and Duel.IsExistingTarget(s.ofilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
