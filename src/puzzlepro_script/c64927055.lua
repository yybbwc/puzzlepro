...
function c64927055.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c64927055.thfilter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c64927055.thfilter1, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c64927055.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SendtoHand(tc, nil, REASON_EFFECT) > 0 and tc:IsLocation(LOCATION_HAND) then
    ...
    if Duel.IsEnvironment(56433456, PLAYER_ALL, LOCATION_ONFIELD + LOCATION_GRAVE) and #g > 0 and Duel.SelectYesNo(tp, aux.Stringid(64927055, 0)) then
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
