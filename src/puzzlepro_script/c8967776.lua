...
function c8967776.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsType, c:GetControler(), LOCATION_GRAVE, 0, 10, nil, TYPE_MONSTER)
end
function c8967776.filter(c, e, tp)
  return c:IsRace(RACE_FAIRY) and c:IsLevelAbove(8) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c8967776.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c8967776.filter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, e, tp)
  end
