...
function c27407330.spcon(e, c)
  ...
  return Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c27407330.spfilter, c:GetControler(), LOCATION_HAND, 0, 1, c)
end
...
function c27407330.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27407330.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c27407330.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27407330.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
