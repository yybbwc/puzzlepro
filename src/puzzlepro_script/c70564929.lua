...
function c70564929.setcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsSetCard, 1, nil, 0x10ec)
  end
...
function c70564929.settg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c70564929.setfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70564929.setfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c70564929.spcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return c:IsReason(REASON_EFFECT) and rp == 1 - tp and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsPreviousPosition(POS_FACEDOWN) and Duel.IsExistingMatchingCard(c70564929.filter2, tp, LOCATION_EXTRA, 0, 1, nil)
end
...
function c70564929.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c70564929.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
