...
function c63175639.sgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63175639.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c63175639.spcon(e, tp, eg, ep, ev, re, r, rp)
  if e:GetHandler():GetFlagEffect(63175639) == 0 then
    return false
  end
...
function c63175639.spfilter(c, e, tp)
  return c:IsRace(RACE_SPELLCASTER) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevelAbove(5) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c63175639.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > -1 and Duel.IsExistingMatchingCard(c63175639.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
