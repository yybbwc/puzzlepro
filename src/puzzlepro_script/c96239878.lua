...
function c96239878.rmcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc:IsSummonPlayer(tp) and (tc:IsSummonType(SUMMON_TYPE_RITUAL) and tc:GetFlagEffect(96239878) ~= 0 or tc:IsSummonType(SUMMON_TYPE_FUSION) and tc:GetFlagEffect(96239879) ~= 0)
end
function c96239878.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return (chkc:IsOnField() or chkc:IsLocation(LOCATION_GRAVE)) and chkc:IsControler(1 - tp) and chkc:IsAbleToRemove()
  end
  if chk == 0 then
...
function c96239878.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c96239878.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c96239878.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
