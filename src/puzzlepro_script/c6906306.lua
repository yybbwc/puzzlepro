...
function s.atkcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bc and bc:IsFaceup() and bc:GetAttack() > c:GetAttack() and c:GetOverlayGroup():IsExists(s.ofilter, 1, nil, ATTRIBUTE_FIRE)
end
...
function s.negcon(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  return c:GetOverlayGroup():IsExists(s.ofilter, 1, nil, ATTRIBUTE_WATER) and Duel.GetTurnPlayer() == 1 - tp
end
...
function s.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ep == tp or not Duel.IsChainDisablable(ev) then
    return
  end
  ...
  if g and g:IsExists(s.disfilter, 1, nil, e, tp) then
...
function s.dreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return c:IsReason(REASON_EFFECT + REASON_BATTLE) and not c:IsReason(REASON_REPLACE) and c:CheckRemoveOverlayCard(tp, 1, REASON_EFFECT)
  end
