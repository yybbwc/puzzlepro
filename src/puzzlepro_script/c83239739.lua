...
function c83239739.condition(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and not (bit.band(r, REASON_BATTLE + REASON_DESTROY) == REASON_BATTLE + REASON_DESTROY)
end
...
function c83239739.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 83239740, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FISH, ATTRIBUTE_WATER) then
    return
  end
