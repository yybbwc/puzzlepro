...
function c15079028.thfilter(c)
  return c:IsRace(RACE_FAIRY) and c:IsAttribute(ATTRIBUTE_EARTH) and not c:IsCode(15079028) and c:IsAbleToHand()
end
function c15079028.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c15079028.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c15079028.cfilter(c, tp)
  return c:IsRace(RACE_FAIRY) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp)
end
