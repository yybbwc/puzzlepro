...
function c36795102.spfilter(c, e, tp)
  return c:IsLevelBelow(4) and c:IsSetCard(0x1034) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsExistingMatchingCard(c36795102.thfilter, tp, LOCATION_DECK, 0, 1, c)
end
...
function c36795102.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c36795102.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
