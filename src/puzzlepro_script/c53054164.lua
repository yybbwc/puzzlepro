...
function c53054164.datg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 53054164) == 0
  end
end
...
function c53054164.tetg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c53054164.tefilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c53054164.tefilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c53054164.teop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) then
    ...
    if Duel.SendtoDeck(tc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_EXTRA) and ct > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c53054164.spfilter), tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(53054164, 2)) then
      Duel.BreakEffect()
