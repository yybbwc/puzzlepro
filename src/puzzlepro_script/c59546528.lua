...
function c59546528.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c59546528.cfilter2, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c59546528.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(1 - tp) and c59546528.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c59546528.filter, tp, 0, LOCATION_SZONE, 1, nil)
  end
