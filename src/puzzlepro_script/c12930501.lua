...
function c12930501.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_GRAVE) and c12930501.filter(chkc, ft)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c12930501.filter, tp, 0, LOCATION_GRAVE, 1, nil, ft)
  end
...
function c12930501.spcfilter(c)
  return c:IsCode(83764718) and c:IsAbleToGraveAsCost() and (c:IsFacedown() or c:IsLocation(LOCATION_HAND))
end
function c12930501.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c12930501.spcfilter, tp, LOCATION_ONFIELD + LOCATION_HAND, 0, 1, nil)
  end
...
function c12930501.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c12930501.sptgfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp) and Duel.GetMZoneCount(tp) > 0
  end
...
function c12930501.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SpecialSummon(tc, 0, tp, tp, true, false, POS_FACEUP) > 0 then
    ...
    if Duel.IsExistingMatchingCard(c12930501.tgfilter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(12930501, 2)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
