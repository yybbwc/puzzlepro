...
function c34771947.tftg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c34771947.tffilter, tp, LOCATION_DECK + LOCATION_REMOVED + LOCATION_HAND, 0, 1, nil, tp)
  end
end
...
function c34771947.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c34771947.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c34771947.desfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
