...
function c33327029.regcon(e, tp, eg, ep, ev, re, r, rp)
  local v = 0
  if eg:IsExists(c33327029.cfilter, 1, nil, 0) then
    v = v + 1
  end
  if eg:IsExists(c33327029.cfilter, 1, nil, 1) then
    v = v + 2
  end
  if v == 0 then
    return false
...
function c33327029.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c33327029.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c33327029.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c33327029.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
