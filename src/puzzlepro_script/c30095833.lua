...
function c30095833.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsType(TYPE_XYZ) and Duel.GetMatchingGroupCount(aux.NecroValleyFilter(c30095833.mfilter), tp, LOCATION_GRAVE, 0, nil) > 0 and Duel.SelectYesNo(tp, aux.Stringid(30095833, 2)) then
...
function c30095833.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c30095833.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30095833.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
