...
function c14418464.filter(c)
  ...
  if seq > 4 or not c:IsSetCard(0x17d) or c:IsFacedown() then
    return false
  end
...
function c14418464.seqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c14418464.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c14418464.filter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c14418464.mvfilter(c)
  local seq = c:GetSequence()
  local tp = c:GetControler()
  return (seq == 0 and Duel.CheckLocation(tp, LOCATION_SZONE, 1)) or (seq == 4 and Duel.CheckLocation(tp, LOCATION_SZONE, 3))
end
function c14418464.mvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_PZONE) and c14418464.mvfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c14418464.mvfilter, tp, LOCATION_PZONE, LOCATION_PZONE, 1, nil)
  end
...
function c14418464.mvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local seq = tc:GetSequence()
  if not seq == 0 and not seq == 4 then
    return
  end
  local nseq = 0
  if seq == 0 then
