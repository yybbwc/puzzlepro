...
function c84125619.cfilter(c, ec, tp)
  if not c:IsRace(RACE_WARRIOR) then
    return false
  end
...
function c84125619.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    if e:GetLabel() == 1 then
      e:SetLabel(0)
      return Duel.CheckReleaseGroup(tp, c84125619.cfilter, 1, c, c, tp) and Duel.IsPlayerCanDraw(tp, 1)
    else
      return Duel.IsExistingTarget(aux.TRUE, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, c) and Duel.IsPlayerCanDraw(tp, 1)
    end
  end
  if e:GetLabel() == 1 then
    e:SetLabel(0)
