...
function c73898890.tdtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToDeck, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil) and Duel.IsExistingTarget(c73898890.tdfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c73898890.spfilter(c, e, tp)
  return c:IsType(TYPE_RITUAL) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
function c73898890.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c73898890.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c73898890.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
