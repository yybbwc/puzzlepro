...
function c57288708.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingMatchingCard(c57288708.spfilter, tp, LOCATION_DECK, 0, 2, nil, e, tp)
  end
...
function c57288708.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c57288708.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
