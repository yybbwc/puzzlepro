...
function c61840587.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c61840587.filter(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 1 and Duel.IsExistingTarget(c61840587.filter, tp, LOCATION_GRAVE, 0, 2, nil, e, tp)
  end
...
function c61840587.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sct == 0 or ft <= 0 or (sct > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) then
    return
  end
  if sct > ft then
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
...
function c61840587.descon2(e, tp, eg, ep, ev, re, r, rp)
  local c = e:GetHandler()
  local sg = e:GetLabelObject():GetLabelObject()
  if not sg or c:IsStatus(STATUS_DESTROY_CONFIRMED) then
    return false
  end
