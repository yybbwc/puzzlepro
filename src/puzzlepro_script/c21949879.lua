...
function c21949879.scon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return Duel.IsExistingMatchingCard(c21949879.cfilter, tp, LOCATION_PZONE, 0, 1, e:GetHandler()) and (g:GetCount() == 0 or g:FilterCount(c21949879.gfilter, nil) == g:GetCount())
end
...
function c21949879.stg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21949879.sfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c21949879.pentg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1)) and Duel.IsExistingMatchingCard(c21949879.penfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c21949879.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21949879.costfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
