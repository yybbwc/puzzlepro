...
function c95519486.descon(e, tp, eg, ep, ev, re, r, rp)
  return aux.dsercon(e, tp, eg, ep, ev, re, r, rp) and e:GetHandler() == Duel.GetAttacker() and Duel.GetAttackTarget() and Duel.IsExistingMatchingCard(c95519486.cfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
function c95519486.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetAttackTarget():IsRelateToBattle()
  end
...
function c95519486.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and c:IsOnField() and c:IsFaceup() and Duel.IsExistingMatchingCard(c95519486.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
