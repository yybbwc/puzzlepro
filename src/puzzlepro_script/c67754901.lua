...
function c67754901.indtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c67754901.indfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c67754901.indfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c67754901.cfilter(c, tp)
  return c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsSetCard(0x99) and c:IsType(TYPE_PENDULUM) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp)
end
...
function c67754901.pentg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, LOCATION_PZONE, 0) > 0 and Duel.IsExistingMatchingCard(c67754901.penfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
