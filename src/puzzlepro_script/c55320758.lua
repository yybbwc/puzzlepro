...
function s.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and (Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 or not Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_MZONE, 0, 1, nil))
end
...
function s.filter1(c, e, tp)
  return c:IsCanOverlay() and not (e and c:IsImmuneToEffect(e)) and Duel.IsExistingMatchingCard(s.filter2, tp, LOCATION_MZONE, 0, 1, c)
end
...
function s.mttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_ONFIELD) and s.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.filter1, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil, e, tp)
  end
