...
function c69522668.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c69522668.setfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
...
function c69522668.cfilter(c, tp)
  return c:IsSummonPlayer(1 - tp) and Duel.IsExistingMatchingCard(Card.IsAttribute, tp, LOCATION_GRAVE, 0, 1, nil, c:GetAttribute())
end
