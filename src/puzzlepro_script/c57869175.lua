...
function c57869175.lvop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsFacedown() or not c:IsRelateToEffect(e) or c:IsLevelAbove(12) then
    return
  end
  local e1 = Effect.CreateEffect(c)
  e1:SetType(EFFECT_TYPE_SINGLE)
...
function c57869175.tgfilter(c, tp)
  return c:IsSetCard(0x31) and c:IsLevelAbove(1) and c:IsFaceup() and Duel.IsExistingMatchingCard(c57869175.rmfilter, tp, LOCATION_MZONE + LOCATION_HAND + LOCATION_GRAVE, 0, 1, c)
end
...
function c57869175.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c57869175.tgfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57869175.tgfilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
  ...
  if rg:GetCount() > 0 and Duel.Remove(rg, POS_FACEUP, REASON_EFFECT) ~= 0 then
    ...
    if lv > 0 and tc:IsRelateToEffect(e) then
      ...
      if tc:IsLevelBelow(lv) then
        op = Duel.SelectOption(tp, aux.Stringid(57869175, 2))
      else
        op = Duel.SelectOption(tp, aux.Stringid(57869175, 2), aux.Stringid(57869175, 3))
      end
      local e1 = Effect.CreateEffect(c)
      e1:SetType(EFFECT_TYPE_SINGLE)
