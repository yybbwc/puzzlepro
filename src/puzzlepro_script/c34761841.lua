...
function c34761841.counterfilter(c)
  return not c:IsSummonLocation(LOCATION_EXTRA) or (c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsType(TYPE_SYNCHRO))
end
...
function c34761841.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(34761841, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c34761841.splimit(e, c)
  return c:IsLocation(LOCATION_EXTRA) and not (c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsType(TYPE_SYNCHRO))
end
...
function c34761841.spcon2(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c34761841.cfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) and Duel.GetMatchingGroupCount(aux.TRUE, tp, LOCATION_MZONE, 0, e:GetHandler()) == 1
end
...
function c34761841.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c34761841.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
