...
function c1965724.cfilter(c, tp)
  return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:GetPreviousRaceOnField() & RACE_FAIRY ~= 0
end
function c1965724.cfilter2(c)
  return c:IsFaceup() and c:IsCode(27288416)
end
function c1965724.regcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():GetFlagEffect(1965724) == 0 and eg:IsExists(c1965724.cfilter, 1, nil, tp) and Duel.IsExistingMatchingCard(c1965724.cfilter2, tp, LOCATION_MZONE, 0, 1, nil)
end
