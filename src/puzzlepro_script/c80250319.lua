...
function c80250319.sptg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c80250319.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c80250319.spfilter1, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c80250319.spop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    ...
    if g:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(80250319, 1)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
...
function c80250319.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c80250319.spfilter1, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
