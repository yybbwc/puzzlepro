...
function c32472237.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsDestructable() and cg:GetCount() > 0
  end
  cg:AddCard(c)
...
function c32472237.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c32472237.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
