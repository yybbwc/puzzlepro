...
function c98371278.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c98371278.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingTarget(c98371278.tgfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c98371278.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc1 and tc2 and tc1:IsRelateToEffect(e) and aux.NecroValleyFilter()(tc1) then
    if not Duel.Equip(tp, tc1, tc2) then
      return
    end
    local e1 = Effect.CreateEffect(c)
