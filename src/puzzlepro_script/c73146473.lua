...
function c73146473.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c73146473.cfilter(chkc, ft)
  end
  if chk == 0 then
    return ft > -1 and Duel.IsExistingTarget(c73146473.cfilter, tp, LOCATION_MZONE, 0, 1, nil, ft) and Duel.IsExistingMatchingCard(c73146473.filter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
...
function c73146473.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not tc:IsLocation(LOCATION_GRAVE) or Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 then
    return
  end
...
function c73146473.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c73146473.sfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
