...
function c10125011.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #cg > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(10125011, 0)) then
...
function c10125011.filter1(c, e, tp)
  return c:IsFaceup() and c10125011.setcardfilter(c) and c:IsType(TYPE_XYZ) and Duel.IsExistingMatchingCard(c10125011.filter2, tp, LOCATION_EXTRA, 0, 1, nil, e, tp, c) and aux.MustMaterialCheck(c, tp, EFFECT_MUST_BE_XMATERIAL)
end
...
function c10125011.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c10125011.filter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c10125011.filter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
