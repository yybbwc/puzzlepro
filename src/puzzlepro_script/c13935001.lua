...
function c13935001.tktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c13935001.atkfilter(chkc, e, tp)
  end
  if chk == 0 then
    return eg:IsExists(c13935001.atkfilter, 1, nil, e, tp) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 13935002, 0xdf, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_BEASTWARRIOR, ATTRIBUTE_DARK, POS_FACEUP, 1 - tp)
  end
...
function c13935001.tkop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 13935002, 0xdf, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_BEASTWARRIOR, ATTRIBUTE_DARK, POS_FACEUP, 1 - tp) then
...
function c13935001.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(Card.IsAbleToGrave, tp, LOCATION_HAND, 0, 1, nil) and Duel.IsExistingMatchingCard(c13935001.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
