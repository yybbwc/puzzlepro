...
function c36637374.selfilter(c, e, tp, check)
  return c:IsSetCard(0x164) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or check and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP_DEFENSE))
end
function c36637374.target(e, tp, eg, ep, ev, re, r, rp, chk)
  local check = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsDiscardable, tp, LOCATION_HAND, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c36637374.selfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, check)
  end
...
function c36637374.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.DiscardHand(tp, nil, 1, 1, REASON_EFFECT + REASON_DISCARD) ~= 0 then
    ...
    local tc = g:GetFirst()
    if check and tc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and (not tc:IsAbleToHand() or Duel.SelectOption(tp, 1190, 1152) == 1) then
...
function c36637374.repfilter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and c:IsReason(REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function c36637374.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAbleToRemove() and eg:IsExists(c36637374.repfilter, 1, nil, tp)
  end
