...
function c365213.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c365213.tdfilter, tp, LOCATION_DECK + LOCATION_HAND, 0, 1, nil)
  end
end
...
function c365213.optg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b1 = Duel.IsPlayerCanDraw(tp, 1) and Duel.GetFlagEffect(tp, 365213) == 0
  local b2 = Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c365213.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp) and Duel.GetFlagEffect(tp, 365214) == 0
  if chk == 0 then
    return b1 or b2
  end
end
function c365213.opop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local op = 0
  if b1 and b2 then
    op = Duel.SelectOption(tp, aux.Stringid(365213, 1), aux.Stringid(365213, 2))
  elseif b1 then
    op = Duel.SelectOption(tp, aux.Stringid(365213, 1))
  elseif b2 then
    op = Duel.SelectOption(tp, aux.Stringid(365213, 2)) + 1
  else
    return
  end
  if op == 0 then
