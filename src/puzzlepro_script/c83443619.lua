...
function c83443619.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and c83443619.desfilter(chkc)
  end
  if chk == 0 then
    ...
    e:SetLabel(loc)
    return Duel.IsExistingTarget(c83443619.desfilter, tp, loc, 0, 1, nil) and Duel.IsExistingMatchingCard(c83443619.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c83443619.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c83443619.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
