...
function s.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.IsPlayerCanDiscardDeck(tp, 1)
  end
end
function s.atkop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRace(RACE_DRAGON) and tc:IsAttribute(ATTRIBUTE_FIRE) then
    ...
    if Duel.SendtoGrave(tc, REASON_EFFECT + REASON_REVEAL) == 0 or not tc:IsLocation(LOCATION_GRAVE) then
      return
    end
