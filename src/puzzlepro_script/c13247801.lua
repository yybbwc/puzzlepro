...
function c13247801.sfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousRaceOnField(), RACE_MACHINE) ~= 0 and c:IsReason(REASON_BATTLE + REASON_EFFECT) and c:IsPreviousLocation(LOCATION_MZONE)
end
...
function c13247801.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c13247801.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c13247801.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
