...
function c27572350.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c27572350.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c27572350.spfilter, tp, LOCATION_REMOVED, LOCATION_REMOVED, 1, nil, e, tp)
  end
...
function c27572350.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToDeck, tp, 0, LOCATION_REMOVED, 1, nil)
  end
