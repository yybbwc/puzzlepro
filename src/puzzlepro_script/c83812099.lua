...
function c83812099.eqtg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c83812099.filter1, tp, LOCATION_SZONE, LOCATION_SZONE, 1, nil, e:GetHandler())
  end
...
function c83812099.eqtg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c83812099.filter2(chkc, eqc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c83812099.filter2, tp, LOCATION_MZONE, LOCATION_MZONE, 1, e:GetHandler(), eqc)
  end
