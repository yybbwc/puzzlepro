...
function c98204536.sumtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c98204536.sumfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c98204536.rcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return bit.band(r, REASON_COST) ~= 0 and re:IsActivated() and re:IsActiveType(TYPE_XYZ) and rc:GetOverlayCount() >= ev - 1 and (rc:IsSetCard(0x70) or rc:IsSetCard(0x48)) and e:GetHandler():IsAbleToGraveAsCost() and ep == e:GetOwnerPlayer()
end
