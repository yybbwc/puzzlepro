...
function c99054885.sscost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c99054885.ssfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c99054885.spfilter(c, e, tp)
  return c:IsCode(39552864) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsCanBeEffectTarget(e) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup())
end
function c99054885.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  local ft = Duel.GetLocationCount(tp, LOCATION_MZONE)
  if chk == 0 then
    return Duel.CheckLPCost(tp, 500, true) and ft > 0
  end
  ...
  if ct > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133) then
    ct = 1
  end
  local t = {}
  for i = 1, ct do
    if not Duel.CheckLPCost(tp, i * 500, true) then
      break
    end
    t[i] = i
  end
...
function c99054885.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE + LOCATION_REMOVED) and c99054885.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c99054885.spfilter, tp, LOCATION_GRAVE + LOCATION_REMOVED, 0, 1, nil, e, tp)
  end
  local ct = e:GetLabel()
