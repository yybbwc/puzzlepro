...
function c65025250.filter(c, tp)
  return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0xb3) and not c:IsReason(REASON_REPLACE)
end
function c65025250.reptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return eg:IsExists(c65025250.filter, 1, nil, tp) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
  end
