...
function c4333086.costfilter(c, ec, tp)
  if not c:IsRace(RACE_ZOMBIE) then
    return false
  end
...
function c4333086.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    if e:GetLabel() == 1 then
      e:SetLabel(0)
      return Duel.CheckReleaseGroup(tp, c4333086.costfilter, 1, c, c, tp) and Duel.IsExistingMatchingCard(c4333086.filter, tp, LOCATION_DECK, 0, 1, nil)
    else
      return Duel.IsExistingTarget(nil, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 2, c) and Duel.IsExistingMatchingCard(c4333086.filter, tp, LOCATION_DECK, 0, 1, nil)
    end
  end
  if e:GetLabel() == 1 then
    e:SetLabel(0)
