...
function c93503294.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c93503294.spcfilter, 1, c, tp, zone)
  end
...
function c93503294.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c93503294.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c93503294.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c93503294.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c93503294.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
