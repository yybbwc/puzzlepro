...
function c84899094.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return g and g:IsExists(c84899094.filter, 1, nil, tp) and Duel.IsChainNegatable(ev)
end
...
function c84899094.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c84899094.spfilter(chkc, e, tp, zone)
  end
  if chk == 0 then
    return zone ~= 0 and Duel.IsExistingTarget(c84899094.spfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler(), e, tp, zone)
  end
