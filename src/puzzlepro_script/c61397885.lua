...
function c61397885.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) > 0 and not Duel.IsExistingMatchingCard(c61397885.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and aux.dscon(e, tp, eg, ep, ev, re, r, rp)
end
...
function c61397885.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(1 - tp) and chkc:IsLocation(LOCATION_MZONE) and c61397885.mfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c61397885.mfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c61397885.efilter(c, tp)
  return c:IsFaceup() and c:IsType(TYPE_NORMAL) and Duel.IsExistingMatchingCard(c61397885.eqfilter, tp, LOCATION_GRAVE + LOCATION_HAND, 0, 1, nil, c)
end
...
function c61397885.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c61397885.efilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c61397885.efilter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_FACEUP)
