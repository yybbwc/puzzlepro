...
function c35487920.cost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c35487920.cfilter1, 1, nil, tp)
  end
...
function c35487920.tgfilter2(c, check)
  return c:IsSetCard(0x152, 0x153) and c:IsType(TYPE_MONSTER) and (c:IsAbleToDeck() or (check and c:IsAbleToHand()))
end
function c35487920.target2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local check = Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c35487920.tgfilter2(chkc, check)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c35487920.tgfilter2, tp, LOCATION_GRAVE, 0, 1, nil, check)
  end
...
function c35487920.operation2(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetMatchingGroupCount(nil, tp, LOCATION_MZONE, 0, nil) == 0 and tc:IsAbleToHand() and (not tc:IsAbleToDeck() or Duel.SelectOption(tp, 1190, aux.Stringid(35487920, 2)) == 0) then
