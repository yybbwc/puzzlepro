...
function c40460013.cfilter(c, e, tp, b1, b2)
  return c:IsRace(RACE_FIEND) and not c:IsPublic() and (b1 and c:IsDiscardable() or b2 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false))
end
...
function c40460013.thcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return rc:IsRelateToBattle() and rc:IsControler(tp) and rc:IsFaceup() and rc:IsRace(RACE_FIEND)
end
