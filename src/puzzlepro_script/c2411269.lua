...
function c2411269.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return #g > 0 and (g:FilterCount(Card.IsRace, nil, RACE_SEASERPENT) < #g or Duel.IsExistingMatchingCard(Card.IsFaceup, tp, 0, LOCATION_MZONE, 1, nil))
  end
...
function c2411269.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ct4 > 0 then
    if ct1 > 0 or ct2 > 0 or ct3 > 0 then
      Duel.BreakEffect()
    end
