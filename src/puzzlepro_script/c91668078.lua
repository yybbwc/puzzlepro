...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, LOCATION_GRAVE, 0, 1, nil, tp, POS_FACEDOWN)
  end
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if Duel.Remove(g, POS_FACEDOWN, REASON_EFFECT) ~= 0 and Duel.GetMatchingGroupCount(Card.IsFacedown, tp, LOCATION_REMOVED, 0, nil) >= 7 and Duel.IsExistingMatchingCard(Card.IsAbleToRemove, tp, 0, LOCATION_GRAVE, 1, nil, tp, POS_FACEDOWN) then
      Duel.BreakEffect()
