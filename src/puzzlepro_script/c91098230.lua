...
function c91098230.filter1(c, e, tp)
  return not c:IsCode(91098230) and c:IsSetCard(0x7) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e, 0, tp, true, false) and (c:IsLocation(LOCATION_HAND) or c:IsLocation(LOCATION_GRAVE) and Duel.IsExistingMatchingCard(nil, tp, 0, LOCATION_MZONE, 1, nil))
end
function c91098230.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c91098230.filter1, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
