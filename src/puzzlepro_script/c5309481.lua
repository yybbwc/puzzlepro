...
function c5309481.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  local tc = nil
  if a:GetControler() == p and a:IsRace(RACE_ZOMBIE) and d:IsStatus(STATUS_BATTLE_DESTROYED) then
    tc = d
  elseif d:GetControler() == p and d:IsRace(RACE_ZOMBIE) and a:IsStatus(STATUS_BATTLE_DESTROYED) then
    tc = a
  end
  if not tc then
    return
