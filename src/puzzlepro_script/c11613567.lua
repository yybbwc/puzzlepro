...
function c11613567.ntcon(e, c, minc)
  ...
  return minc == 0 and c:IsLevelAbove(5) and Duel.GetLocationCount(c:GetControler(), LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c11613567.ntfilter, c:GetControler(), LOCATION_MZONE, 0, 1, nil)
end
...
function c11613567.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c11613567.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c11613567.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
