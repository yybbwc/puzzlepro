...
function c1041278.rfilter2(c, tp)
  return c:IsType(TYPE_FUSION) and aux.IsMaterialListCode(c, 68468459) and (c:IsControler(tp) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c) > 1
end
function c1041278.spfilter0(c, e, tp)
  return not c:IsType(TYPE_FUSION) and c:IsFaceupEx() and c:IsCanBeEffectTarget(e) and (c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP) or c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp))
end
function c1041278.spfilter1(c, e, tp, g)
  return c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP) and g:IsExists(c1041278.spfilter2, 1, c, e, tp)
end
...
function c1041278.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  local b1 = Duel.CheckReleaseGroup(tp, c1041278.rfilter1, 1, nil, tp) and Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and g:IsExists(c1041278.spfilter1, 1, nil, e, tp, g)
  local b2 = Duel.CheckReleaseGroup(tp, c1041278.rfilter2, 1, nil, tp) and g:IsExists(c1041278.spfilter3, 2, nil, e, tp)
  if chk == 0 then
    return b1 or b2
  end
...
function c1041278.gcheck(g, e, tp, b1, b2)
  return b1 and g:IsExists(c1041278.spfilter1, 1, nil, e, tp, g) or b2 and g:IsExists(c1041278.spfilter3, 2, nil, e, tp)
end
function c1041278.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local g = Duel.GetMatchingGroup(c1041278.spfilter0, tp, LOCATION_GRAVE + LOCATION_REMOVED, LOCATION_GRAVE + LOCATION_REMOVED, nil, e, tp)
  local b1 = ft1 > 0 and ft2 > 0 and g:IsExists(c1041278.spfilter1, 1, nil, e, tp, g)
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and (b1 or e:IsCostChecked())
  end
  local check = e:IsCostChecked() and aux.IsMaterialListCode(e:GetLabelObject(), 68468459)
  e:SetLabel(check and 1 or 0)
  local b2 = check and ft1 > 1 and g:IsExists(c1041278.spfilter3, 2, nil, e, tp)
...
function c1041278.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() == 2 then
    local b1 = ft1 > 0 and ft2 > 0 and g:IsExists(c1041278.spfilter1, 1, nil, e, tp, g)
    local b2 = e:GetLabel() ~= 0 and ft1 > 1 and g:IsExists(c1041278.spfilter3, 2, nil, e, tp)
