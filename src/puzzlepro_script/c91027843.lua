...
function c91027843.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c91027843.eqfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c91027843.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsRelateToEffect(e) or not tc:IsRelateToEffect(e) or ft <= 0 or g:GetCount() <= 0 then
    return
  end
