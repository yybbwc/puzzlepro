...
function c76981308.cfilter(c, tp)
  return not c:IsPublic() and c:IsRace(RACE_BEAST) and c:IsAbleToDeck() and Duel.IsExistingMatchingCard(c76981308.cfilter2, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, c)
end
...
function c76981308.atktg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c76981308.atkfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
end
...
function c76981308.sum(c)
  if c:GetFlagEffect(76981308) == 0 then
    return 0
  end
