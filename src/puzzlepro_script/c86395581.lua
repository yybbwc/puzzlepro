...
function c86395581.srcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86395581.filter, tp, LOCATION_HAND, 0, 1, c) and c:IsDiscardable()
  end
...
function c86395581.srtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c86395581.srfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c86395581.cfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousAttributeOnField(), ATTRIBUTE_WIND) ~= 0
end
