...
function c39030163.mttg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return g:IsContains(chkc) and c39030163.mtfilter(chkc, e)
  end
  if chk == 0 then
    return e:GetHandler():IsType(TYPE_XYZ) and g:IsExists(c39030163.mtfilter, 1, nil, e)
  end
...
function c39030163.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c39030163.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c39030163.desfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
