...
function c70628672.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c70628672.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
end
...
function c70628672.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c70628672.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c70628672.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
