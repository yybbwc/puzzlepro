...
function c60530944.condition(e, tp, eg, ep, ev, re, r, rp)
  return Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) > 0 and not Duel.IsExistingMatchingCard(Card.IsFacedown, tp, LOCATION_MZONE, 0, 1, nil) and Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) > 0 and not Duel.IsExistingMatchingCard(Card.IsFacedown, tp, 0, LOCATION_MZONE, 1, nil)
end
...
function c60530944.operation(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFieldGroupCount(tp, LOCATION_MZONE, 0) == 0 or Duel.IsExistingMatchingCard(Card.IsFacedown, tp, LOCATION_MZONE, 0, 1, nil) or Duel.GetFieldGroupCount(tp, 0, LOCATION_MZONE) == 0 or Duel.IsExistingMatchingCard(Card.IsFacedown, tp, 0, LOCATION_MZONE, 1, nil) then
    return
  end
