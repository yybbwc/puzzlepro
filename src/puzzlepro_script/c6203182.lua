...
function c6203182.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1 - tp) and c6203182.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c6203182.spfilter, tp, 0, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function c6203182.operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and Duel.SpecialSummonStep(tc, 0, tp, 1 - tp, false, false, POS_FACEUP) then
    Duel.Equip(tp, c, tc)
...
function c6203182.ctcon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return ec and ec:GetReasonCard() and c:IsReason(REASON_LOST_TARGET) and ec:IsReason(REASON_MATERIAL) and (ec:IsReason(REASON_FUSION) or ec:IsReason(REASON_SYNCHRO) or ec:IsReason(REASON_XYZ) or ec:IsReason(REASON_LINK))
end
function c6203182.cttg(e, tp, eg, ep, ev, re, r, rp, chk)
  local ec = e:GetHandler():GetPreviousEquipTarget()
  local rc = ec:GetReasonCard()
  if chk == 0 then
    return rc:IsControlerCanBeChanged()
  end
