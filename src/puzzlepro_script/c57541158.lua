...
function c57541158.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c57541158.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c57541158.filter, tp, LOCATION_ONFIELD, 0, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c57541158.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c57541158.tgfilter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c57541158.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c57541158.eqfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
...
function c57541158.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 then
    ...
    local ec = g:GetFirst()
    if ec then
      if not Duel.Equip(tp, ec, tc) then
        return
      end
      --equip limit
