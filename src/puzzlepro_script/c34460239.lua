...
function c34460239.filter(c, tp)
  return c:IsFaceup() and Duel.IsExistingMatchingCard(c34460239.nfilter1, tp, LOCATION_DECK, 0, 1, nil, c)
end
...
function c34460239.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c34460239.filter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c34460239.filter, tp, LOCATION_MZONE, 0, 1, nil, tp)
  end
