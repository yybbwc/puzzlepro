...
function s.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return rg and rg:FilterCount(s.ovfilter, nil, tp) > 0
  end
  Duel.SetOperationInfo(0, CATEGORY_NEGATE, eg, 1, 0, 0)
end
function s.atkfilter(c, e)
  return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x160) and not c:IsImmuneToEffect(e)
end
  ...
  if rg and rg:FilterCount(s.ovfilter, nil, tp) > 0 then
    ...
    if Duel.SendtoHand(tc, nil, REASON_EFFECT) ~= 0 and tc:IsLocation(LOCATION_HAND) then
      ...
      if Duel.NegateActivation(ev) then
        ...
        if tc:IsType(TYPE_TRAP) and (tc:IsType(TYPE_FIELD) or Duel.GetLocationCount(tp, LOCATION_SZONE) > 0) and tc:IsSSetable(true) and Duel.SelectYesNo(tp, aux.Stringid(id, 2)) then
          Duel.SSet(tp, tc)
        end
      end
    end
