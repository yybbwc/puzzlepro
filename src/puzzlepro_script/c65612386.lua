...
function c65612386.cfilter(c, tp)
  return c:IsType(TYPE_SPELL) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c65612386.thfilter, tp, LOCATION_DECK, 0, 1, nil, c:GetCode())
end
...
function c65612386.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c65612386.cfilter, tp, LOCATION_GRAVE, 0, 1, nil, tp)
  end
