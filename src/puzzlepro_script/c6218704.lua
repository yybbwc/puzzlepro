...
function c6218704.hspfilter(c, tp, sc)
  return c:IsCode(13331639) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevel(12) and c:IsControler(tp) and Duel.GetLocationCountFromEx(tp, tp, c, sc) > 0 and c:IsCanBeFusionMaterial(sc, SUMMON_TYPE_SPECIAL)
end
...
function c6218704.pop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 then
    ...
    if g:GetCount() > 0 and Duel.SelectYesNo(tp, aux.Stringid(6218704, 1)) then
      ...
      if sc and Duel.SendtoDeck(sc, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and sc:IsLocation(LOCATION_EXTRA) and sc:IsControler(tp) and sc:IsCanBeSpecialSummoned(e, 0, tp, true, false) and Duel.GetLocationCountFromEx(tp, tp, nil, sc) > 0 and Duel.SelectYesNo(tp, aux.Stringid(6218704, 2)) then
        Duel.BreakEffect()
...
function c6218704.settg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return (Duel.CheckLocation(tp, LOCATION_PZONE, 0) or Duel.CheckLocation(tp, LOCATION_PZONE, 1)) and Duel.IsExistingMatchingCard(c6218704.setfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
end
