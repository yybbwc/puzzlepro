...
function c63972571.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:CheckSubGroup(c63972571.fselect, 1, g:GetCount(), tp)
  end
...
function c63972571.cfilter(c)
  return c:IsSetCard(0x13e) and c:IsType(TYPE_MONSTER) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsAbleToRemoveAsCost()
end
function c63972571.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c63972571.cfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c63972571.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.TURE, tp, 0, LOCATION_ONFIELD, 2, nil)
  end
