...
function s.spfilter1(c, e, tp)
  return c:IsFaceupEx() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsSetCard(0x19b) and Duel.GetMatchingGroupCount(Card.IsDiscardable, tp, LOCATION_HAND, 0, e:GetHandler(), REASON_EFFECT) > 0
end
function s.spfilter2(c, e, tp)
  return c:IsFaceupEx() and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsLocation(LOCATION_SZONE)
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local op = aux.SelectFromOptions(tp, { b1, aux.Stringid(id, 0), 1 }, { b2, aux.Stringid(id, 1), 2 })
  e:SetLabel(op)
  if op == 1 then
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if e:GetLabel() == 1 and Duel.DiscardHand(tp, nil, 1, 1, REASON_DISCARD + REASON_EFFECT, nil) == 0 then
    return
  end
