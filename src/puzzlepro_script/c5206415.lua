...
function c5206415.chainfilter(re, tp, cid)
  return not (re:GetHandler():IsRace(RACE_THUNDER) and re:IsActiveType(TYPE_MONSTER) and Duel.GetChainInfo(cid, CHAININFO_TRIGGERING_LOCATION) == LOCATION_HAND)
end
function c5206415.spfilter(c, tp)
  return (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsLevelBelow(8) and c:IsRace(RACE_THUNDER) and c:IsAbleToRemoveAsCost() and Duel.GetMZoneCount(tp, c) > 0
end
function c5206415.spcon(e, c)
  ...
  return (Duel.GetCustomActivityCount(5206415, tp, ACTIVITY_CHAIN) ~= 0 or Duel.GetCustomActivityCount(5206415, 1 - tp, ACTIVITY_CHAIN) ~= 0) and Duel.IsExistingMatchingCard(c5206415.spfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, c, tp)
end
...
function c5206415.etcost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:CheckSubGroup(c5206415.fselect, 2, 2)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
...
function c5206415.ettg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c5206415.etfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5206415.etfilter, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c5206415.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c5206415.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
