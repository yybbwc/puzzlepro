...
function s.spfilter(c, e, tp)
  return c:IsLevel(5) and (Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) or Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false, POS_FACEUP, 1 - tp))
end
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(s.spfilter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local toplayer = aux.SelectFromOptions(tp, { s1, aux.Stringid(id, 1), tp }, { s2, aux.Stringid(id, 2), 1 - tp })
  if toplayer ~= nil then
    Duel.SpecialSummon(tc, 0, tp, toplayer, false, false, POS_FACEUP)
  end
end
function s.cfilter(c, tp)
  return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousLevelOnField() == 5 and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer() == 1 - tp)
end
...
function s.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.IsExistingMatchingCard(s.dfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, ft)
  end
end
