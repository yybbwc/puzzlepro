...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE) > 0 and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if sc:IsRelateToEffect(e) and sc:IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
    Duel.BreakEffect()
...
function s.spfgfilter(c, tp, se)
  return c:IsFaceup() and c:IsSummonPlayer(tp) and c:IsCode(78371393) and c:IsType(TYPE_MONSTER) and (se == nil or c:GetReasonEffect() ~= se)
end
...
function s.spfgtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
