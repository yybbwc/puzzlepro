...
function c40916023.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c40916023.spfilter, tp, LOCATION_GRAVE, 0, 1, nil)
end
...
function c40916023.postg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c40916023.posfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c40916023.posfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
