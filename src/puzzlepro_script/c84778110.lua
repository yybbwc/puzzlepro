...
function c84778110.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0 and Duel.IsExistingMatchingCard(c84778110.tgfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c84778110.damop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.Damage(1 - tp, 500, REASON_EFFECT) ~= 0 and Duel.IsExistingMatchingCard(c84778110.cfilter, tp, LOCATION_GRAVE, 0, 1, nil) and Duel.IsExistingMatchingCard(c84778110.rfilter, tp, LOCATION_REMOVED, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(84778110, 2)) then
    Duel.BreakEffect()
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
