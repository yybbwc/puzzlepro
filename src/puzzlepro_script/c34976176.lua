...
function c34976176.discon2(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c34976176.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and aux.dscon(e, tp, eg, ep, ev, re, r, rp)
end
...
function c34976176.distg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c34976176.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c34976176.filter, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingMatchingCard(c34976176.tgfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c34976176.disop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoGrave(g, REASON_EFFECT) > 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) and tc:IsFaceup() and tc:IsRelateToEffect(e) then
