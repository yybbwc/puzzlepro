...
function c45584727.rmcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(45584727, tp, ACTIVITY_SPSUMMON) == 0
  end
...
function c45584727.rmcon(e, tp, eg, ep, ev, re, r, rp)
  return Duel.IsExistingMatchingCard(c45584727.cfilter, tp, LOCATION_MZONE, 0, 1, nil) and not Duel.IsExistingMatchingCard(c45584727.cfilter2, tp, LOCATION_MZONE, 0, 1, nil)
end
...
function c45584727.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c45584727.tgfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(c45584727.rmfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
