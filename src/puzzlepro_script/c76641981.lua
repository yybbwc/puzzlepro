...
function c76641981.filter1(c)
  ...
  return c:IsFaceup() and c:IsSetCard(0x27) and c:IsAbleToChangeControler() and Duel.GetMZoneCount(tp, c, tp, LOCATION_REASON_CONTROL) > 0
end
...
function c76641981.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c76641981.filter2, tp, 0, LOCATION_MZONE, 1, nil) and Duel.IsExistingTarget(c76641981.filter1, tp, LOCATION_MZONE, 0, 1, nil)
  end
