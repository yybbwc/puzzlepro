...
function c30284022.rmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(Card.IsAbleToRemove, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil) and Duel.IsExistingMatchingCard(c30284022.rmfilter, tp, LOCATION_HAND, 0, 1, nil)
  end
...
function c30284022.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #tg > 0 and Duel.Remove(tg, POS_FACEUP, REASON_EFFECT) > 0 and Duel.GetFieldGroupCount(tp, LOCATION_HAND, 0) > 0 then
    Duel.BreakEffect()
...
function c30284022.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c30284022.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c30284022.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
