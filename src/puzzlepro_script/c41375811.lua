...
function c41375811.xyzop(e, tp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 41375811) == 0
  end
...
function c41375811.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c41375811.spfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c41375811.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
