...
function c7617253.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c7617253.costfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c7617253.activate(e, tp, eg, ep, ev, re, r, rp)
  local tg = Duel.GetAttacker()
  if not tg:IsRelateToEffect(e) or tg:IsStatus(STATUS_ATTACK_CANCELED) or not Duel.NegateAttack() then
    return
  end
