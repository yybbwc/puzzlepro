...
function s.rdop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local op = aux.SelectFromOptions(1 - tp, { true, aux.Stringid(id, 2) }, { aux.nzatk(tc), aux.Stringid(id, 3) })
  if op == 1 then
...
function s.sfilter(c, e, tp, m, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsSetCard(0x21) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, nil, chkf)
end
...
function s.fstg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    local chkf = tp
    local mg1 = Duel.GetFusionMaterial(tp):Filter(s.mfilter, nil, e) + Duel.GetMatchingGroup(s.gfilter, tp, LOCATION_GRAVE, 0, nil, e)
