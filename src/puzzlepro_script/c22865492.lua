...
function c22865492.spcost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c22865492.cfilter1, 1, nil, tp)
  end
...
function c22865492.sptg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsPlayerCanSpecialSummonMonster(tp, 22865492, 0x5008, TYPE_MONSTER + TYPE_EFFECT, 900, 1100, 3, RACE_WARRIOR, ATTRIBUTE_DARK)
  end
...
function c22865492.sptg2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c22865492.spfilter2, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
