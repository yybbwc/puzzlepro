...
function c32549749.filter(c, tp)
  ...
  return c:IsSSetable(true) or Duel.IsExistingMatchingCard(c32549749.eqfilter, tp, LOCATION_MZONE, 0, 1, nil, c, tp)
end
...
function c32549749.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  local b1 = aux.dscon(e, tp, eg, ep, ev, re, r, rp) and Duel.IsExistingTarget(Card.IsFaceup, tp, LOCATION_MZONE, 0, 1, nil)
  local b2 = Duel.GetCurrentPhase() ~= PHASE_DAMAGE and ft > 0 and Duel.IsExistingTarget(c32549749.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, tp)
  if chk == 0 then
    return b1 or b2
  end
  local op = 0
