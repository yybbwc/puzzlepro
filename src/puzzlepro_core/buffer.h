...
template <typename T> inline T buffer_read(unsigned char *&p) {
  T ret{};
...
template <typename T> inline void buffer_write(unsigned char *&p, T value) {
...
template <typename T> inline void vector_write(std::vector<unsigned char> &buffer, T value) {
