...
function c76865611.check(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not c:IsDisabled() and c:IsDefensePos() then
    e:SetLabel(1)
  else
    e:SetLabel(0)
  end
...
function c76865611.cond(e, tp, eg, ep, ev, re, r, rp)
  return e:GetLabelObject():GetLabel() == 1 and e:GetHandler():IsLocation(LOCATION_GRAVE) and e:GetHandler():IsReason(REASON_BATTLE)
end
