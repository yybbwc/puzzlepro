...
function c21291696.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c21291696.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c21291696.thop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    Duel.ShuffleHand(tp)
    if Duel.IsExistingMatchingCard(c21291696.sumfilter, tp, LOCATION_HAND + LOCATION_MZONE, 0, 1, nil) and Duel.SelectYesNo(tp, aux.Stringid(21291696, 2)) then
      Duel.BreakEffect()
...
function c21291696.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c21291696.lvfilter(chkc, lv)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c21291696.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, lv)
  end
