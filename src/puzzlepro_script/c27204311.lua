...
function c27204311.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return g:GetCount() > 0 and Duel.GetMZoneCount(tp, g) > 0 and Duel.GetMZoneCount(1 - tp, g, tp) > 0 and Duel.IsPlayerCanRelease(tp) and Duel.IsPlayerCanSpecialSummonCount(tp, 2) and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and Duel.IsPlayerCanSpecialSummonMonster(tp, 27204312, 0, TYPES_TOKEN_MONSTER, -2, -2, 11, RACE_ROCK, ATTRIBUTE_LIGHT, POS_FACEUP, 1 - tp)
  end
...
function c27204311.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.Release(g, REASON_EFFECT) ~= 0 then
    ...
    if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) > 0 then
      ...
      if Duel.GetLocationCount(1 - tp, LOCATION_MZONE, tp) > 0 and Duel.IsPlayerCanSpecialSummonMonster(tp, 27204312, 0, TYPES_TOKEN_MONSTER, -2, -2, 11, RACE_ROCK, ATTRIBUTE_LIGHT, POS_FACEUP, 1 - tp) then
        Duel.BreakEffect()
