...
function c58468105.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  e:SetLabel(lv)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58468105.tgfilter, tp, LOCATION_DECK, lv, lv, nil)
  end
...
function c58468105.discost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c58468105.costfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c58468105.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if rp ~= 1 - tp or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) or c:IsStatus(STATUS_BATTLE_DESTROYED) then
    return false
  end
