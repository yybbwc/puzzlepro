...
function c59934749.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c59934749.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c59934749.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    ...
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and cg:GetCount() > 0 and Duel.IsExistingMatchingCard(c59934749.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, cg:GetClassCount(Card.GetCode))
  end
  local cg = Duel.GetMatchingGroup(c59934749.cfilter, tp, LOCATION_DECK, 0, nil)
