...
function c94585852.regop(e, tp, eg, ep, ev, re, r, rp)
  ...
  while tc do
    if tc:IsReason(REASON_DESTROY) and not tc:IsReason(REASON_BATTLE) and tc:GetPreviousTypeOnField() & TYPE_MONSTER ~= 0 and tc:IsSetCard(0x45) and tc:GetLevel() > 0 then
  ...
  if g1:GetCount() > 0 then
    Duel.RaiseEvent(g1, EVENT_CUSTOM + 94585852, re, r, rp, 0, lv1)
  end
  if g2:GetCount() > 0 then
    Duel.RaiseEvent(g2, EVENT_CUSTOM + 94585852, re, r, rp, 1, lv2)
  end
end
...
function c94585852.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c94585852.filter, tp, LOCATION_DECK, 0, 1, nil, ev)
  end
