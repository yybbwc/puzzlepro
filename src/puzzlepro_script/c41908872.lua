...
function c41908872.cfilter1(c, tp)
  return c:IsCode(13331639) and c:IsFaceup() and Duel.IsExistingMatchingCard(c41908872.cfilter2, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c41908872.filter(c, ec)
  return c:IsType(TYPE_PENDULUM) and c:IsFaceup() and c:IsLevelAbove(0) and (not c:IsAttribute(ec:GetAttribute()) or not c:IsLevel(ec:GetLevel()))
end
...
function c41908872.thtg1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c41908872.thfilter, tp, LOCATION_EXTRA, 0, 1, nil)
  end
...
function c41908872.thop1(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc and Duel.SendtoHand(tc, nil, REASON_EFFECT) and tc:IsLocation(LOCATION_HAND) then
    ...
    if (g1:GetCount() > 0 or g2:GetCount() > 0) and Duel.SelectYesNo(tp, aux.Stringid(41908872, 5)) then
      Duel.BreakEffect()
      local g = g1 + g2
