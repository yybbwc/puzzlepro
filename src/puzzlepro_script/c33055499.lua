...
function c33055499.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return e:IsCostChecked() and Duel.CheckLocation(tp, LOCATION_MZONE, seq) and Duel.IsPlayerCanSpecialSummonMonster(tp, 33055499, 0, TYPES_NORMAL_TRAP_MONSTER, 2100, 100, 5, RACE_INSECT, ATTRIBUTE_EARTH)
  end
...
function c33055499.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  local zone = 1 << seq
  if not Duel.IsPlayerCanSpecialSummonMonster(tp, 33055499, 0, TYPES_NORMAL_TRAP_MONSTER, 2100, 100, 5, RACE_INSECT, ATTRIBUTE_EARTH) then
    return
  end
...
function c33055499.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c33055499.thfilter, tp, LOCATION_DECK + LOCATION_REMOVED, 0, 1, nil)
  end
