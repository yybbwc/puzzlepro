...
function c5399521.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c5399521.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5399521.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
...
function c5399521.spcon(e, c)
  ...
  return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsCanRemoveCounter(tp, 1, 0, 0x35, 3, REASON_COST)
end
...
function c5399521.sumop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetFlagEffect(tp, 5399521) ~= 0 then
    return
  end
