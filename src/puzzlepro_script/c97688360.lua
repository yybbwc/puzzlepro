...
function c97688360.atkcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c97688360.atkfilter1, 1, nil, tp)
  end
...
function c97688360.atktg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c97688360.atkfilter2(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c97688360.atkfilter2, tp, LOCATION_MZONE, 0, 1, nil)
  end
...
function c97688360.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c97688360.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
