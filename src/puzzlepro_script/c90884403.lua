...
function c90884403.fselect(g, tp, sc)
  if not aux.gffcheck(g, Card.IsType, TYPE_TUNER, aux.NOT(Card.IsType), TYPE_TUNER) or Duel.GetLocationCountFromEx(tp, tp, g, sc) <= 0 then
    return false
  end
...
function c90884403.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ct > 0 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonMonster(tp, 90884404, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE) and Duel.IsPlayerCanSpecialSummonMonster(tp, 90884404, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE, 1 - tp)
  end
...
function c90884403.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  local ft2 = Duel.GetLocationCount(1 - tp, LOCATION_MZONE)
  local ct = math.min(ft1, ft2)
  if ct > 0 and not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.IsPlayerCanSpecialSummonMonster(tp, 90884404, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE) and Duel.IsPlayerCanSpecialSummonMonster(tp, 90884404, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_FIEND, ATTRIBUTE_DARK, POS_FACEUP_DEFENSE, 1 - tp) then
    for i = 1, ct do
