...
function c68250822.cfilter1(c, tp)
  return c:IsType(TYPE_MONSTER) and c:IsCanOverlay() and Duel.IsExistingMatchingCard(c68250822.filter1, tp, LOCATION_MZONE, 0, 1, c)
end
function c68250822.target1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c68250822.cfilter1, tp, LOCATION_MZONE + LOCATION_GRAVE, LOCATION_MZONE + LOCATION_GRAVE, 1, nil, tp)
  end
...
function c68250822.target2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c68250822.filter2, tp, 0, LOCATION_MZONE, 1, nil, zone)
  end
  Duel.Hint(HINT_OPSELECTED, tp, e:GetDescription())
...
function c68250822.target3(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c68250822.filter3, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil, e, tp, zone)
  end
  Duel.Hint(HINT_OPSELECTED, tp, e:GetDescription())
