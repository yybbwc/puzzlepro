...
function c25857977.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(tp) and c25857977.desfilter(chkc)
  end
  local g = Duel.GetMatchingGroup(c25857977.desfilter2, tp, LOCATION_ONFIELD, 0, nil, e)
  if chk == 0 then
    return e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false) and g:CheckSubGroup(aux.mzctcheck, 1, g:GetCount(), tp)
  end
...
function c25857977.effcon(e, tp, eg, ep, ev, re, r, rp)
  return bit.band(r, REASON_FUSION + REASON_SYNCHRO + REASON_XYZ + REASON_LINK) ~= 0 and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and e:GetHandler():GetReasonCard():IsSetCard(0x10af)
end
...
function c25857977.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c25857977.desfilter3(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c25857977.desfilter3, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil, tp)
  end
  Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_DESTROY)
...
function c25857977.desop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 and Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) ~= 0 and g:GetFirst():IsLocation(LOCATION_DECK + LOCATION_EXTRA) and tc:IsRelateToEffect(e) then
