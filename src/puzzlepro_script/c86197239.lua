...
function c86197239.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c86197239.filter(chkc, e, tp)
  end
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, 59822133) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c86197239.filter, tp, LOCATION_GRAVE, 0, 2, nil, e, tp)
  end
...
function c86197239.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft < sg:GetCount() or (sg:GetCount() > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133)) then
    return
  end
