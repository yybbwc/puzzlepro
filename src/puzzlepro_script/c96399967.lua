...
function c96399967.costfilter(c, e, tp)
  return aux.AtkEqualsDef(c) and c:IsRace(RACE_MACHINE) and c:GetLevel() > 1 and Duel.GetMZoneCount(tp, c) > 0 and (c:IsControler(tp) or c:IsFaceup()) and Duel.IsExistingMatchingCard(c96399967.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp, c:GetLevel())
end
function c96399967.spfilter(c, e, tp, lv)
  return aux.AtkEqualsDef(c) and c:IsRace(RACE_MACHINE) and c:GetLevel() < lv and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c96399967.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c96399967.costfilter, 1, nil, e, tp)
  end
...
function c96399967.thfilter(c)
  return c:IsFacedown() and aux.AtkEqualsDef(c) and c:IsRace(RACE_MACHINE) and c:IsAbleToHand()
end
function c96399967.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c96399967.thfilter, tp, LOCATION_REMOVED, 0, 1, nil)
  end
