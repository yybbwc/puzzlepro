...
function c23297235.filter(c, e, tp)
  return c:IsDefenseBelow(200) and c:IsAttribute(ATTRIBUTE_FIRE) and not c:IsCode(23297235) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
function c23297235.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c23297235.filter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
