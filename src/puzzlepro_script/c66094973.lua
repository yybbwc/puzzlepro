...
function c66094973.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c66094973.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.GetFieldGroup(tp, LOCATION_HAND, 0) ~= 0 and Duel.IsExistingTarget(c66094973.filter, tp, 0, LOCATION_MZONE, 1, nil)
  end
...
function c66094973.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.GetFieldGroup(tp, LOCATION_HAND, 0) == 0 then
    return
  end
