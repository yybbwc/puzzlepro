...
function c54578613.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c54578613.filter(chkc)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c54578613.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
...
function c54578613.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c54578613.costfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c54578613.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and c54578613.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c54578613.desfilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
