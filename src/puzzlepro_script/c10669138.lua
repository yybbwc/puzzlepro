...
function c10669138.descon(e, tp, eg, ep, ev, re, r, rp)
  ...
  return e:GetHandler():IsSummonType(SUMMON_TYPE_LINK) and att & ATTRIBUTE_DARK > 0 and att & ATTRIBUTE_EARTH > 0 and att & ATTRIBUTE_WATER > 0 and att & ATTRIBUTE_FIRE > 0 and att & ATTRIBUTE_WIND > 0
end
...
function c10669138.rmop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() >= 5 then
    ...
    if Duel.GetOperatedGroup():FilterCount(Card.IsLocation, nil, LOCATION_REMOVED) == 5 then
      res = true
    end
  end
