...
function c64230128.actlimcon(e)
  ...
  return ph >= PHASE_BATTLE_START and ph <= PHASE_BATTLE and Duel.IsExistingMatchingCard(c64230128.actlimfilter, tp, LOCATION_MZONE, 0, 1, nil)
end
function c64230128.damcon(e, tp, eg, ep, ev, re, r, rp)
  local rc = eg:GetFirst()
  return rc:IsRelateToBattle() and rc:IsStatus(STATUS_OPPO_BATTLE) and rc:IsFaceup() and rc:IsCode(3285552) and rc:IsControler(tp)
end
...
function c64230128.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c64230128.thfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
