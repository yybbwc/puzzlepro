...
function c27970830.cost1(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x3, 2, REASON_COST)
  end
...
function c27970830.tg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c27970830.filter1(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c27970830.filter1, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c27970830.cost2(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x3, 4, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c27970830.tg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c27970830.filter2, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil)
  end
...
function c27970830.cost3(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsCanRemoveCounter(tp, 1, 0, 0x3, 6, REASON_COST)
  end
  Duel.Hint(HINT_OPSELECTED, 1 - tp, e:GetDescription())
...
function c27970830.tg3(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c27970830.filter3(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingTarget(c27970830.filter3, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
