...
function c81119816.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c81119816.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c81119816.tgop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if #g > 0 and Duel.SendtoGrave(g, REASON_EFFECT) ~= 0 and g:GetFirst():IsLocation(LOCATION_GRAVE) and #g2 > 0 and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(81119816, 0)) then
    Duel.BreakEffect()
