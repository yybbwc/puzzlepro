...
function c36898537.immcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO) and bit.band(e:GetLabelObject():GetLabel(), TYPE_NORMAL) ~= 0
end
...
function c36898537.negcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO) and bit.band(e:GetLabelObject():GetLabel(), TYPE_EFFECT) ~= 0
end
...
function c36898537.negtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c36898537.negfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c36898537.negfilter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
...
function c36898537.ctcon(e, tp, eg, ep, ev, re, r, rp)
  return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO) and bit.band(e:GetLabelObject():GetLabel(), TYPE_PENDULUM) ~= 0
end
