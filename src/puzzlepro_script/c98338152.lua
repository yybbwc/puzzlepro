...
function c98338152.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(aux.NegateEffectMonsterFilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
...
function c98338152.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsCanBeDisabledByEffect(e) then
    ...
    if Duel.GetMatchingGroupCount(Card.IsType, tp, LOCATION_GRAVE, 0, nil, TYPE_SPELL) >= 3 and tc:IsDisabled() and tc:IsControler(1 - tp) and tc:IsControlerCanBeChanged() and Duel.SelectYesNo(tp, aux.Stringid(98338152, 0)) then
      Duel.BreakEffect()
