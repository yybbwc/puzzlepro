...
function c57624336.destg1(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and c57624336.filter1(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return e:GetHandler():IsDestructable() and Duel.IsExistingTarget(c57624336.filter1, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, e:GetHandler())
  end
