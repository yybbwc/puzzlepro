...
function c89399912.hspcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c89399912.rfilter, tp, LOCATION_HAND + LOCATION_GRAVE, 0, 2, e:GetHandler())
  end
...
function c89399912.shcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsDiscardable() and e:GetHandler():IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(c89399912.dfilter, tp, LOCATION_HAND, 0, 1, e:GetHandler())
  end
...
function c89399912.shtg(e, tp, eg, ep, ev, re, r, rp, chk, _, exc)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c89399912.shfilter, tp, LOCATION_DECK, 0, 1, exc)
  end
...
function c89399912.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c89399912.thfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
  Duel.SetOperationInfo(0, CATEGORY_TOHAND, nil, 1, tp, LOCATION_DECK)
end
