...
function c44227727.filter2(c, e, tp, m, f, chkf)
  return c:IsType(TYPE_FUSION) and c:IsRace(RACE_FIEND) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e, SUMMON_TYPE_FUSION, tp, false, false) and c:CheckFusionMaterial(m, nil, chkf)
end
...
function c44227727.eqfilter(c, ec, tp)
  return (c:IsType(TYPE_MONSTER) and c:IsSetCard(0x13f) or c:IsCode(80621422) and c:CheckEquipTarget(ec)) and not c:IsForbidden() and c:CheckUniqueOnField(tp, LOCATION_SZONE)
end
...
function c44227727.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c44227727.cfilter(chkc, tp)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44227727.cfilter, tp, LOCATION_MZONE, 0, 1, nil, tp) and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0
  end
...
function c44227727.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() then
    ...
    if not Duel.Equip(tp, sc, tc) then
      return
    end
    --equip limit
    local e1 = Effect.CreateEffect(c)
