...
function c63265554.eqtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingMatchingCard(c63265554.eqfilter, tp, LOCATION_DECK + LOCATION_GRAVE, 0, 1, nil, e:GetHandler(), tp)
  end
...
function c63265554.eqop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft <= 0 or c:IsFacedown() or not c:IsRelateToEffect(e) then
    return
  end
...
function c63265554.pncost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return cg:IsExists(c63265554.cfilter, 1, nil, tp)
  end
...
function c63265554.pntg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and c63265554.pnfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c63265554.pnfilter, tp, LOCATION_MZONE, LOCATION_MZONE, 1, nil)
  end
