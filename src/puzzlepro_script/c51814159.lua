...
function c51814159.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, Card.IsSetCard, 1, nil, 0xba)
  end
...
function c51814159.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c51814159.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c51814159.thfilter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
