...
function c57329501.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c57329501.spfilter(chkc, e, tp)
  end
  local b1 = Duel.IsExistingTarget(c57329501.spfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  local b2 = Duel.IsExistingMatchingCard(c57329501.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp) and Duel.IsEnvironment(91027843, tp, LOCATION_FZONE)
  if chk == 0 then
