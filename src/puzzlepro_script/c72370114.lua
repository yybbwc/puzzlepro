...
function c72370114.target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return c:IsCanTurnSet() and c:GetFlagEffect(72370114) == 0
  end
...
function c72370114.destg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72370114.cfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler()) and Duel.IsExistingMatchingCard(c72370114.filter, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, nil)
  end
