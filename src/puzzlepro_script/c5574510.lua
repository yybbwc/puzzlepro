...
function s.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1 - tp) and s.desilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.IsExistingTarget(s.desilter, tp, 0, LOCATION_MZONE, 1, nil)
  end
  ...
  if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsType(TYPE_MONSTER) then
    if dc > tc:GetLevel() then
      if Duel.Destroy(tc, REASON_EFFECT) ~= 0 then
        ...
        if lv > 0 and Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_DECK, 0, 1, nil, lv) and Duel.SelectYesNo(tp, aux.Stringid(id, 1)) then
