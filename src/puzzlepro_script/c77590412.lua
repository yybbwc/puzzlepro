...
function c77590412.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsAttackPos() and Duel.IsExistingMatchingCard(c77590412.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c77590412.cltg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local c = e:GetHandler()
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c77590412.clfilter(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp, c) > 0 and c:IsAbleToGrave() and Duel.IsExistingTarget(c77590412.clfilter, tp, LOCATION_GRAVE, 0, 1, nil, e, tp)
  end
...
function c77590412.clop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if c:IsRelateToEffect(e) and Duel.SendtoGrave(c, REASON_EFFECT) > 0 and c:IsLocation(LOCATION_GRAVE) and tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc, 0, tp, tp, false, false, POS_FACEUP) > 0 then
