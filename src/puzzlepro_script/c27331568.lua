...
function c27331568.cfilter(c, tp)
  return c:IsSetCard(0x2093) and c:IsType(TYPE_RITUAL) and c:IsLevelAbove(1) and Duel.IsExistingTarget(c27331568.filter, tp, LOCATION_MZONE, 0, 1, c)
end
...
function c27331568.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroupEx(tp, c27331568.cfilter, 1, REASON_COST, true, nil, tp)
  end
...
function c27331568.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c27331568.filter(chkc)
  end
  if chk == 0 then
    return true
  end
...
function c27331568.distg(e, c)
  if c:GetFlagEffect(27331568) > 0 then
    return true
  end
