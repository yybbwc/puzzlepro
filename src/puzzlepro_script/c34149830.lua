...
function c34149830.condition(e, tp, eg, ep, ev, re, r, rp)
  ...
  return tc and bc and not tc:IsHasEffect(EFFECT_INDESTRUCTABLE_BATTLE) and tc:IsPosition(POS_FACEUP_ATTACK) and tc:GetAttack() <= bc:GetAttack()
end
...
function c34149830.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return not e:GetHandler():IsStatus(STATUS_CHAINING) and Duel.IsExistingMatchingCard(c34149830.spfilter, tp, LOCATION_HAND, 0, 1, nil, e, tp)
  end
end
