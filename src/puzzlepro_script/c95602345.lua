...
function c95602345.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c95602345.cfilter, tp, LOCATION_ONFIELD, 0, 1, nil)
  end
...
function c95602345.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and Duel.IsExistingMatchingCard(c95602345.spfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c95602345.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if ft > 0 then
    ...
    if ft1 > 1 and Duel.IsPlayerAffectedByEffect(tp, 59822133) then
      ft = 1
    end
    if ft1 > 0 and Duel.IsExistingMatchingCard(nil, tp, 0, LOCATION_MZONE, 1, nil) and #g2 > 0 and Duel.SelectYesNo(tp, aux.Stringid(95602345, 0)) then
      Duel.BreakEffect()
      Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
