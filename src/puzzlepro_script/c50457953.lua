...
function c50457953.lvtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c50457953.lvfilter(chkc, e:GetHandler():GetLevel())
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c50457953.lvfilter, tp, LOCATION_MZONE, 0, 1, nil, e:GetHandler():GetLevel())
  end
