...
function c16832845.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c16832845.filter, tp, LOCATION_GRAVE, LOCATION_GRAVE, 1, nil)
  end
...
function c16832845.spfilter(c, e, tp)
  return c:IsRace(RACE_SPELLCASTER) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsLocation(LOCATION_REMOVED) and c:GetLevel() > 0
end
