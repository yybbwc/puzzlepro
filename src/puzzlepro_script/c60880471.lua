...
function c60880471.discon(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) or not re:IsActiveType(TYPE_MONSTER) then
    return false
  end
  local tg = Duel.GetChainInfo(ev, CHAININFO_TARGET_CARDS)
  if not Duel.IsExistingMatchingCard(c60880471.cfilter, tp, LOCATION_MZONE, 0, 1, nil) then
    return false
  end
...
function c60880471.spfilter(c, tp)
  return (c:IsControler(tp) or c:IsFaceup()) and Duel.GetMZoneCount(tp, c) > 0 and (c:IsRace(RACE_PLANT) or c:IsHasEffect(76869711, tp) and c:IsControler(1 - tp))
end
function c60880471.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, c60880471.spfilter, 1, nil, tp)
  end
