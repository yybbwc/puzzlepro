...
function c4064925.cost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetFlagEffect(tp, 4064925) == 0
  end
...
function c4064925.target(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c4064925.filter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c4064925.drcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.CheckReleaseGroup(tp, nil, 1, nil, tp)
  end
