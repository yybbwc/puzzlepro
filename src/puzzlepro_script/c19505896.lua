...
function c19505896.repfilter(c, e)
  return c:IsFaceup() and c:IsRace(RACE_PLANT) and c:IsDestructable(e) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
end
function c19505896.desreptg(e, tp, eg, ep, ev, re, r, rp, chk)
  local c = e:GetHandler()
  if chk == 0 then
    return not c:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(c19505896.repfilter, tp, LOCATION_MZONE, 0, 1, c, e)
  end
