...
function c5908650.thcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetCustomActivityCount(5908650, tp, ACTIVITY_CHAIN) == 0
  end
...
function c5908650.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c5908650.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c5908650.filter, tp, LOCATION_GRAVE, 0, 1, nil)
  end
