...
function c48445393.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c48445393.desfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c48445393.desfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingMatchingCard(c48445393.sfilter, tp, LOCATION_DECK, 0, 1, nil) and Duel.IsPlayerCanDraw(tp, 2)
  end
...
function c48445393.activate(e, tp, eg, ep, ev, re, r, rp)
  ...
  if g:GetCount() > 0 then
    ...
    if not g:GetFirst():IsLocation(LOCATION_GRAVE) then
      return
    end
    Duel.BreakEffect()
