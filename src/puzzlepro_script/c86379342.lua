...
function c86379342.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c86379342.spfilter1(chkc, e, tp)
  end
  if chk == 0 then
    return Duel.GetMZoneCount(tp) > 0 and Duel.IsExistingTarget(c86379342.spfilter1, tp, LOCATION_MZONE, 0, 1, nil, e, tp)
  end
...
function c86379342.cfilter(c, tp)
  return c:IsRace(RACE_BEAST) and c:IsFaceup() and Duel.IsExistingMatchingCard(c86379342.cfilter1, tp, LOCATION_MZONE, 0, 1, c, c, tp)
end
function c86379342.cfilter1(c, c1, tp)
  return c:IsRace(RACE_BEASTWARRIOR) and c:IsFaceup() and Duel.IsExistingMatchingCard(c86379342.cfilter2, tp, LOCATION_MZONE, 0, 1, Group.FromCards(c, c1))
end
...
function c86379342.negtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c86379342.negfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c86379342.negfilter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
