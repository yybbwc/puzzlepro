...
function c66399653.thfilter(c)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_UNION) and c:IsAbleToHand()
end
...
function c66399653.tgfilter(c, e, tp, chk)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_UNION) and c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsControler(tp) and c:IsCanBeEffectTarget(e) and (chk or Duel.IsExistingMatchingCard(c66399653.cfilter, tp, LOCATION_DECK, 0, 1, nil, c, tp))
end
function c66399653.cfilter(c, ec, tp)
  return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_UNION) and not c:IsCode(ec:GetCode()) and c:CheckUniqueOnField(tp) and not c:IsForbidden() and c:CheckUnionTarget(ec) and aux.CheckUnionEquip(c, ec)
end
function c66399653.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return eg:IsContains(chkc) and c66399653.tgfilter(chkc, e, tp, true)
  end
  local g = eg:Filter(c66399653.tgfilter, nil, e, tp, false)
  if chk == 0 then
    return g:GetCount() > 0 and Duel.GetLocationCount(tp, LOCATION_SZONE) > 0
  end
