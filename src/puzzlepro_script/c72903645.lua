...
function c72903645.descost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c72903645.cfilter, tp, LOCATION_SZONE, 0, 1, nil)
  end
...
function c72903645.destg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsOnField() and chkc:IsControler(1 - tp) and c72903645.filter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c72903645.filter, tp, 0, LOCATION_ONFIELD, 1, nil)
  end
