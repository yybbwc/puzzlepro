...
function c69436288.sptg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  ...
  if chk == 0 then
    return Duel.IsExistingTarget(c69436288.thfilter, tp, LOCATION_MZONE, 0, 1, nil) and Duel.IsExistingTarget(aux.TRUE, tp, 0, LOCATION_MZONE, 1, nil) and Duel.GetLocationCount(tp, LOCATION_MZONE) > 0 and e:GetHandler():IsCanBeSpecialSummoned(e, 0, tp, false, false)
  end
...
function c69436288.spop(e, tp, eg, ep, ev, re, r, rp)
  ...
  if Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP) ~= 0 then
    ...
    if hc:IsRelateToEffect(e) and hc:IsControler(tp) and Duel.SendtoHand(hc, nil, REASON_EFFECT) ~= 0 and hc:IsLocation(LOCATION_HAND) and tc:IsRelateToEffect(e) and tc:IsControler(1 - tp) then
