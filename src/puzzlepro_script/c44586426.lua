...
function c44586426.tgtg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.IsExistingMatchingCard(c44586426.tgfilter, tp, LOCATION_DECK, 0, 1, nil)
  end
...
function c44586426.tkop(e, tp, eg, ep, ev, re, r, rp)
  if Duel.GetLocationCount(tp, LOCATION_MZONE) <= 0 or not Duel.IsPlayerCanSpecialSummonMonster(tp, 44586427, 0, TYPES_TOKEN_MONSTER, 0, 0, 1, RACE_THUNDER, ATTRIBUTE_LIGHT) then
    return
  end
...
function c44586426.nmfilter(c, cd)
  return (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsType(TYPE_EFFECT) and c:IsSetCard(0x28) and not c:IsCode(cd)
end
function c44586426.nmtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  local cd = e:GetHandler():GetCode()
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE + LOCATION_GRAVE) and chkc:IsControler(tp) and c44586426.nmfilter(chkc, cd)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c44586426.nmfilter, tp, LOCATION_MZONE + LOCATION_GRAVE, 0, 1, nil, cd)
  end
