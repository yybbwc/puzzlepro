...
function c37433748.eqtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c37433748.eqfilter(chkc) and chkc ~= e:GetHandler()
  end
  if chk == 0 then
    return Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.IsExistingTarget(c37433748.eqfilter, tp, LOCATION_MZONE, 0, 1, e:GetHandler())
  end
...
function c37433748.dircost(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return ec and Duel.IsExistingMatchingCard(c37433748.cfilter, tp, LOCATION_ONFIELD, 0, 1, c, ec)
  end
