...
function c59851535.spcost(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return e:GetHandler():IsReleasable() and Duel.IsExistingMatchingCard(c59851535.costfilter, tp, LOCATION_HAND + LOCATION_SZONE, 0, 1, nil, tp)
  end
...
function c59851535.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
  if chk == 0 then
    return Duel.GetMZoneCount(tp, e:GetHandler()) > 0 and Duel.IsExistingMatchingCard(c59851535.spfilter, tp, LOCATION_DECK, 0, 1, nil, e, tp)
  end
...
function c59851535.thtg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
  if chkc then
    return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c59851535.thfilter(chkc)
  end
  if chk == 0 then
    return Duel.IsExistingTarget(c59851535.thfilter, tp, LOCATION_GRAVE, 0, 1, e:GetHandler())
  end
